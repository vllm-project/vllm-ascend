...
# Node 3 - Decode (kv_consumer) - Worker
# 16 NPU, DP size 8 (local 4, ranks 4-7), TP 4
# Requires: MASTER_IP environment variable
...
# Set MASTER_IP to node 2's IP address
# Auto-detect if not set (adjust the pattern if needed)
if [ -z "$MASTER_IP" ]; then
    echo "Warning: MASTER_IP not set. Please set it to node 2's IP address:"
    echo "  export MASTER_IP=\"<node2_ip>\""
    echo ""
    echo "Attempting to auto-detect from common patterns..."
    # You can customize this based on your environment
    # export MASTER_IP="<node2_ip>"
    exit 1
fi
