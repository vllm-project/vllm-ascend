...
using namespace DispatchGmmCombineImpl;
extern "C" __global__ __aicore__ void dispatch_gmm_combine(GM_ADDR x, GM_ADDR w1, GM_ADDR w2,  GM_ADDR expertId, GM_ADDR scale1, GM_ADDR scale2, GM_ADDR probs,
    GM_ADDR c, GM_ADDR workspaceGM,  GM_ADDR tilingGM)
{
    ...
    if (TILING_KEY_IS(1000000)) {
        ...
        GET_TILING_DATA_WITH_STRUCT(DispatchGmmCombineTilingData, tilingData, tilingGM);
        DispatchGmmCombine<int8_t, DTYPE_W1, DTYPE_OUT, false, true> op;
        ...
        GET_TILING_DATA_WITH_STRUCT(DispatchGmmCombineTilingData, tilingData, tilingGM);
        DispatchGmmCombine<int8_t, DTYPE_W1, DTYPE_OUT, true, false> op;
        op.Init(x, w1, w2, expertId, scale1, scale2, probs, c, workspaceGM, tilingGM);
        op.Process();
        ...
        GET_TILING_DATA_WITH_STRUCT(DispatchGmmCombineTilingData, tilingData, tilingGM);
        DispatchGmmCombine<int8_t, DTYPE_W1, DTYPE_OUT, true, true> op;
        op.Init(x, w1, w2, expertId, scale1, scale2, probs, c, workspaceGM, tilingGM);
        op.Process();
    }
}