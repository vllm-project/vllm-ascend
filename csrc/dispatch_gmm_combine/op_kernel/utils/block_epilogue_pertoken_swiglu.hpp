...
public:
    ...
    // Tile compute ops
    // using TileElemWiseMuls = TileElemWiseMuls_;
    ...
    )
    {
        ...
        for (uint32_t loopIdx = loopStartIdx; loopIdx < loopStartIdx + tasksForIdx; ++loopIdx) {
            ...
            AscendC::PipeBarrier<PIPE_V>();
            // AscendC::Max(ubMax, ubAbs, ubAbs[HalfChunkTileLen], HalfChunkTileLen);
            // AscendC::PipeBarrier<PIPE_V>();
            // AscendC::Muls(ubOutputTmp, ubCFp32ChunkN, 127.f, ChunkTileLen);
            // AscendC::PipeBarrier<PIPE_V>();
            // uint32_t reduceWidth = HalfChunkTileLen;
            // constexpr uint32_t colNumPerCompute = BYTE_PER_VECTOR_FRACTAL / sizeof(float);

            // while (reduceWidth > colNumPerCompute) {
            //     reduceWidth >>= 1;
            //     AscendC::Max(ubMax, ubMax, ubMax[reduceWidth], reduceWidth);
            //     AscendC::PipeBarrier<PIPE_V>();
            // }

            // AscendC::WholeReduceMax(ubReduceMax, ubMax, 64, 1, 0, 0, 0, AscendC::ReduceOrder::ORDER_ONLY_VALUE);
            ...
            AscendC::SetFlag<AscendC::HardEvent::S_V>(0);
            // ElementPerTokenScale GMubDequantScale = ubAbs.GetValue(24);
            //auto ubPerTokenScaleOutputOffset = alignedPerCoreData * subblockIdx + loopIdx - loopStartIdx;
        ...
        if(tasksForIdx > 0){
            ...
            // auto layoutGmPerTokenScale2 = params.layoutDequantScale.GetTileLayout(dequantScaleTileShape);
            // auto layoutUbPerTokenScale2 = LayoutPerTokenScale::template MakeLayoutInUb<ElementPerTokenScale>(MakeCoord(tasksForIdx));
...
private:
    ...
    // TileElemWiseMuls_ tileElemWiseMuls;
