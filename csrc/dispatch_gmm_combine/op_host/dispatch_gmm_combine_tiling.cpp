...
namespace {
    // 1. 常量定义
    const char *K_INNER_DEBUG = "DispatchGmmCombine Tiling Debug";
...
static ge::graphStatus DispatchGmmCombineCheckAttrAndSetTiling(gert::TilingContext *context, DispatchGmmCombineInfo& info)
{
...
static ge::graphStatus DispatchGmmCombineCheckShapeAndSetTiling(gert::TilingContext *context, DispatchGmmCombineInfo &info)
{
    const char *nodeName = context->GetNodeName();
    // OPS_LOG_I(nodeName, "DispatchGmmCombine DispatchGmmCombineCheckShapeAndSetTiling.");
...
static ge::graphStatus DispatchGmmCombineGetPlatformInfoAndSetTiling(gert::TilingContext *context, DispatchGmmCombineInfo& info)
{
...
void SetTilingData(CoCTiling &cocTilingData, DispatchGmmCombineInfo &info)
{
...
static ge::graphStatus DispatchGmmCombineTilingFuncImpl(gert::TilingContext *context)
{
    const char *nodeName = context->GetNodeName();
    OP_LOGI(nodeName, "Enter DispatchGmmCombine tiling func.");

    // 1. tilingData
    DispatchGmmCombineTilingData *tilingData = context->GetTilingData<DispatchGmmCombineTilingData>();
    OP_TILING_CHECK(tilingData == nullptr, OP_LOGE(nodeName, "tilingData is nullptr."),
        return ge::GRAPH_FAILED);
    OP_LOGI(nodeName, "DispatchGmmCombine get tilingData.");
    DispatchGmmCombineInfo& info = tilingData->dispatchGmmCombineInfo;
    OP_LOGI(nodeName, "DispatchGmmCombine get tilingData info.");

    OP_TILING_CHECK(DispatchGmmCombineCheckAttrAndSetTiling(context, info) != ge::GRAPH_SUCCESS,
        OP_LOGE(context->GetNodeName(), "DispatchGmmCombine CheckAttrAndSetTiling Failed"),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(DispatchGmmCombineCheckShapeAndSetTiling(context, info) != ge::GRAPH_SUCCESS,
        OP_LOGE(context->GetNodeName(), "DispatchGmmCombine CheckShapeAndSetTiling Failed"),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(DispatchGmmCombineGetPlatformInfoAndSetTiling(context, info) != ge::GRAPH_SUCCESS,
        OP_LOGE(context->GetNodeName(), "DispatchGmmCombine GetPlatformInfoAndSetTiling Failed"),
        return ge::GRAPH_FAILED);
    ...
    uint32_t opType = 8U; // batch write=18,
    ...
    OP_LOGI(nodeName, "Leave DispatchGmmCombine tiling func.");
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus DispatchGmmCombineTilingFunc(gert::TilingContext* context)
{
    return DispatchGmmCombineTilingFuncImpl(context);
}

struct DispatchGmmCombineCompileInfo {};
ge::graphStatus TilingParseForDispatchGmmCombine(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(DispatchGmmCombine)
    .Tiling(DispatchGmmCombineTilingFunc)
    .TilingParse<DispatchGmmCombineCompileInfo>(TilingParseForDispatchGmmCombine);
} // namespace optiling