...
if [[ "$ENABLE_TEST" == "TRUE" ]]; then
    ...
    if [[ "${ops_kernel_value}" == "ON" ]]; then
        # Kernel compile scripts are generated by a build target, while the
        # corresponding kernel targets are discovered at configure time.
        build generate_compile_cmd
        if [[ "$ENABLE_BUILD_PKG" == "TRUE" ]]; then
            cmake_config " -DENABLE_BUILD_PKG=ON"
        else
            cmake_config " -DENABLE_BUILD_PKG=OFF"
