...
if [[ "$SOC_VERSION" =~ ^ascend310 ]]; then
    ...
    CATLASS_BRANCH=$(git config -f "${ROOT_DIR}/.gitmodules" --get submodule.csrc/third_party/catlass.branch)
    if [[ ! -d "${CATLASS_PATH}" ]]; then
        ...
        git fetch origin
        git checkout "${CATLASS_BRANCH}" || exit 1
        git reset --hard "origin/${CATLASS_BRANCH}" || exit 1
        cd - || exit 1
    fi
