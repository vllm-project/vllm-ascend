...
template <typename T>
__aicore__ inline void MoeGatherDroppadQuant<T>::Init(
    GM_ADDR inputX, GM_ADDR scale, GM_ADDR offset, GM_ADDR expandedRowIdx, GM_ADDR expandedX, GM_ADDR workspace,
    const MoeInitRoutingCustomTilingData* tilingData, TPipe* tPipe)
{
    pipe_ = tPipe;
    ...
    pipe_->InitBuffer(inputXCopyOutQueue_, GATHER_OUT_DROPPAD_QUANT_BUFFER_NUM, AlignBytes(perLoopCols_, sizeof(int8_t)));
    pipe_->InitBuffer(expandRowIdxCopyInQueue_, GATHER_OUT_DROPPAD_QUANT_BUFFER_NUM, AlignBytes(perLoopRows_, sizeof(int32_t)));
