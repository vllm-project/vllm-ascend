...
private:
    ...
    const MoeCustomSrcToDstCapacityComputeTilingData* srcToDstTilingData; // 注意：适配
...
__aicore__ inline void MoeCustomSrcToDstWithCapacity<T, TilingData>::Init(
    ...
    GM_ADDR workspace, const TilingData* tilingData, TPipe* tPipe) // 增加scale
{
    ...
    expandDstToSrcRowGm.SetGlobalBuffer(
        ...
        Align(this->coreRows, sizeof(int32_t)));
    expertIdxValueGm.SetGlobalBuffer((__gm__ int32_t*)workspace + length * 2 + Align(this->expertNum, sizeof(int32_t)) * 2, this->coreNum * 2); // to do
