...
FORCE_INLINE void check_cpu_contiguous_tensor(const at::Tensor& tensor, const char* name) {
...
HOT_FUNCTION void sparse_kv_lru_resident_compact(
    ...
    int64_t miss_tokens_ptr, int64_t miss_slots_ptr, int64_t token_mark_workspace_ptr, int64_t token_pos_workspace_ptr,
    int64_t slot_workspace_ptr, int64_t miss_position_workspace_ptr, int64_t epochs_ptr, int64_t num_reqs, int64_t topk,
    int64_t capacity, int64_t max_token, int64_t workspace_threads, int64_t requested_threads) {
...
int64_t sparse_kv_compute_lru_resident_addrs(
    const at::Tensor& miss_count, const at::Tensor& miss_tokens, const at::Tensor& miss_slots,
    const at::Tensor& block_table, const int64_t block_size_arg, const int64_t token_size_bytes_k_arg,
    const int64_t token_size_bytes_v_arg, const int64_t gvas_k_base, const int64_t gvas_v_base,
    const int64_t addr_k_base, const int64_t addr_v_base, const int64_t resident_capacity_arg,
    const int64_t max_num_threads_arg, at::Tensor& gvas_buffer, at::Tensor& addr_buffer, at::Tensor& size_buffer,
  ...
  TORCH_CHECK(miss_count.dim() == 1, "miss_count must be 1D.");
  TORCH_CHECK(miss_tokens.dim() == 2, "miss_tokens must be 2D.");
  TORCH_CHECK(miss_slots.dim() == 2, "miss_slots must be 2D.");
  TORCH_CHECK(block_table.dim() == 2, "block_table must be 2D.");
  TORCH_CHECK(gvas_buffer.dim() == 1, "gvas_buffer must be 1D.");
  TORCH_CHECK(addr_buffer.dim() == 1, "addr_buffer must be 1D.");
  TORCH_CHECK(size_buffer.dim() == 1, "size_buffer must be 1D.");
  TORCH_CHECK(num_tokens_buffer.dim() == 1 && num_tokens_buffer.numel() >= 1,
              "num_tokens_buffer must be 1D with at least one element.");
  ...
  TORCH_CHECK(num_tokens_buffer.scalar_type() == at::kInt, "num_tokens_buffer wrong dtype, should be int32.");

  check_cpu_contiguous_tensor(miss_count, "miss_count");
  check_cpu_contiguous_tensor(miss_tokens, "miss_tokens");
  check_cpu_contiguous_tensor(miss_slots, "miss_slots");
  check_cpu_contiguous_tensor(block_table, "block_table");
  check_cpu_contiguous_tensor(gvas_buffer, "gvas_buffer");
  check_cpu_contiguous_tensor(addr_buffer, "addr_buffer");
  check_cpu_contiguous_tensor(size_buffer, "size_buffer");
  check_cpu_contiguous_tensor(num_tokens_buffer, "num_tokens_buffer");

  const int64_t num_reqs_arg = miss_tokens.size(0);
  const int64_t topk_arg = miss_tokens.size(1);
  const int64_t max_num_blocks_arg = block_table.size(1);
  TORCH_CHECK(num_reqs_arg <= std::numeric_limits<int32_t>::max(), "num_reqs is out of int32 range.");
  TORCH_CHECK(topk_arg <= std::numeric_limits<int32_t>::max(), "topk is out of int32 range.");
  TORCH_CHECK(max_num_blocks_arg <= std::numeric_limits<int32_t>::max(),
              "block_table width is out of int32 range.");
  TORCH_CHECK(num_reqs_arg == 0 || topk_arg <= std::numeric_limits<int32_t>::max() / 2 / num_reqs_arg,
              "The maximum number of tokens to load is out of int32 range.");
  ...
  const int32_t num_reqs = static_cast<int32_t>(num_reqs_arg);
  const int32_t topk = static_cast<int32_t>(topk_arg);
  const int32_t max_num_tokens_to_load = num_reqs * topk;
  const int32_t max_num_blocks = static_cast<int32_t>(max_num_blocks_arg);
  const int64_t block_size_bytes_k = block_size_arg * token_size_bytes_k_arg;
  const int64_t block_size_bytes_v = block_size_arg * token_size_bytes_v_arg;
  ...
  TORCH_CHECK(block_table.size(0) >= num_reqs, "block_table does not contain enough request rows.");
  const int64_t required_buffer_size = static_cast<int64_t>(max_num_tokens_to_load) * 2;
  TORCH_CHECK(gvas_buffer.size(0) >= required_buffer_size, "gvas_buffer size not enough for loading.");
  TORCH_CHECK(addr_buffer.size(0) >= required_buffer_size, "addr_buffer size not enough for loading.");
  TORCH_CHECK(size_buffer.size(0) >= required_buffer_size, "size_buffer size not enough for loading.");
...
#pragma omp parallel for num_threads(n_threads)
  for (size_t req_idx = 0; req_idx < num_reqs; ++req_idx) {
    ...
    for (int32_t idx = 0; idx < num_tokens_to_load_req[req_idx]; ++idx) {
      ...
      const int64_t resident_offset = static_cast<int64_t>(req_idx) * resident_capacity + slot;
      const int64_t addr_k = addr_k_base + resident_offset * token_size_bytes_k;
      const int64_t addr_v = addr_v_base + resident_offset * token_size_bytes_v;
...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.impl("sparse_kv_lru_resident_compact", c10::DispatchKey::CompositeExplicitAutograd,
             &vllm_ascend::sparse_kv_lru_resident_compact);
    ...
    ops.impl("sparse_kv_compute_lru_resident_addrs", torch::kCPU,
             &vllm_ascend::sparse_kv_compute_lru_resident_addrs);
#endif
