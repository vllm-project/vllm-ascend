...
class DataGenerator:
    def __init__(self, data_dir="data"):
        ...
        self.weight_data_nz = None
        self.weight_scale = None
        self.x_scale = None
        self.group_list_data = None
    ...
    def convert_nd_to_nz(self, x):
        ...
        x_shape = self.shape_nd_to_nz(x.shape)
    ...
    def generate_and_save_data(self):
        ...
        self.x_scale.cpu().numpy().tofile(os.path.join(self.data_dir, "xScaleHostData.bin"))
    ...
    def process_groups(self, x: torch.Tensor, weight: torch.Tensor, perChannelScale: torch.Tensor, perTokenScale: torch.Tensor, groupList: torch.Tensor):
        ...
        # 遍历 groupList,按组处理数据
        for i, v in tqdm(enumerate(groupList)):
            currV = v
...
if __name__ == "__main__":
    ...
    data_generator.process_swiglu_quant()
    print(f"所有二进制文件已生成并保存到 {data_generator.data_dir} 目录")
