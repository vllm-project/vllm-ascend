...
static uint64_t CalcMaxTmpSize(const uint32_t row, const uint64_t n) {
  ...
  uint32_t avarageTmp = (max + min) >> 1;
  GetAscendQuantMaxMinTmpSize(shape, 4, max, min);
  uint32_t avarage = (max + min) >> 1;
  avarage = avarage > avarageTmp ? avarage : avarageTmp;
  GetAscendDequantMaxMinTmpSize(shape, 4, max, min);
  avarageTmp = (max + min) >> 1;
  return avarage > avarageTmp ? avarage : avarageTmp;
}
