...
class VersionChecker:
    ...
    def main(cls) -> NoReturn:
        parser = argparse.ArgumentParser(
            description="Check Version Compatible", epilog="Best Regards!")
        ...
        # 子命令行(Check)
        p_chk = sub_parser.add_parser('check_code_compatible',
                                      help="Check Version Compatible.")
        p_chk.add_argument("--code_version_info_file",
                           required=True,
                           nargs=1,
                           type=str,
        ...
        # 子命令行(Get)
        p_get = sub_parser.add_parser('get_package_version',
                                      help="Get Package Version")
        p_get.set_defaults(func=VersionChecker._get_package_version)
        # 参数处理
        args = parser.parse_args()
        # 基本合法性检查, 版本号获取
        cann_version_info_file = Path(args.cann_path[0],
                                      args.cann_package_name[0],
                                      'version.info').absolute()
        if not cann_version_info_file.exists():
            raise ValueError(
                f"CANN version info file({cann_version_info_file}) not exist.")
        ret, cann_version = cls._get_version_str(file=cann_version_info_file)
        if not ret:
            raise ValueError(
                f"Can't get version from CANN version info file({cann_version_info_file})."
            )
    ...
    def _check_compatible(cls, cann_version: str, args) -> str:
        code_version_info_file = Path(
            args.code_version_info_file[0]).absolute()
        if not code_version_info_file.exists():
            raise ValueError(
                f"Code version info file({code_version_info_file}) not exist.")
        ret, code_version = cls._get_version_str(file=code_version_info_file)
        if not ret:
            raise ValueError(
                f"Can't get version from Code version info file({code_version_info_file})."
            )
        # 兼容性检查
