...
class GenCoverage:
    ...
    def main(cls):
        # 参数注册
        parser = argparse.ArgumentParser(description="Generate Coverage",
                                         epilog="Best Regards!")
        parser.add_argument(
            "-s",
            "--source_base_dir",
        ...
        parser.add_argument("-i",
                            ...
                            nargs=1,
                            type=Path,
                            help="Explicitly specify coverage info file path.")
        ...
        parser.add_argument(
            ...
            nargs=1,
            type=Path,
            help="Explicitly specify coverage html report dir.")
        # 参数解析, 默认值处理
        p = cls.Param()
        args = parser.parse_args()
        p.source_dir = Path(args.source_base_dir[0]).absolute(
        ) if args.source_base_dir else None
        p.data_dir = Path(args.coverage_data_dir[0]).absolute()
        p.info_file = args.info_file[0] if args.info_file else Path(
            p.data_dir, 'cov_result/coverage.info')
        p.info_file = Path(p.info_file).absolute()
        p.info_file_filtered = Path(
            p.info_file.parent,
            f"{p.info_file.stem}_filtered{p.info_file.suffix}")
        p.html_report_dir = args.html_report[0] if args.html_report else Path(
            p.info_file.parent, "html_report")
        ...
        if not p.data_dir.exists():
            logging.error(
                "The dir(%s) required to find the .da files not exist.",
                p.data_dir)
            return
    ...
    def _chk_env(cls):
        try:
            ret = subprocess.run('lcov --version'.split(),
                                 capture_output=True,
                                 check=True,
                                 encoding='utf-8')
            ...
            logging.error(
                "lcov is required to generate coverage data, please install.")
            return False
        try:
            ret = subprocess.run('genhtml --version'.split(),
                                 capture_output=True,
                                 check=True,
                                 encoding='utf-8')
            ret.check_returncode()
            ...
            logging.error(
                "genhtml is required to generate coverage html report, please install."
            )
            return False
        return True
    ...
    def _gen_cov(cls, param: Param):
        ...
        ret = subprocess.run(cmd.split(),
                             ...
                             encoding='utf-8')
        ret.check_returncode()
        logging.info("[END] generated origin coverage file %s",
                     param.info_file)
        # 滤掉某些文件/路径的覆盖率信息
        ...
        ret = subprocess.run(cmd.split(),
                             ...
                             encoding='utf-8')
        ret.check_returncode()
        logging.info("[END] generated filtered coverage file %s",
                     param.info_file_filtered)
        # 生成 html 报告
        ...
        logging.info("[BGN] generated filtered coverage html report, cmd=%s",
                     cmd)
        ret = subprocess.run(cmd.split(),
                             capture_output=False,
                             check=True,
                             encoding='utf-8')
        ret.check_returncode()
        logging.info("[END] generated filtered coverage html report. %s",
                     param.html_report_dir)
