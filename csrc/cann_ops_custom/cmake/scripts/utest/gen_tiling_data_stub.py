...
class Process:
    ...
    def _write_file(cls, file: Path, src: str):
        with os.fdopen(os.open(file, cls._WRITE_FLAGS, cls._WRITE_MODES),
                       'w') as fh:
            fh.write(src)
    ...
    def _gen_tiling_h(cls, ori_file: Path, gen_dir: Path):
        ...
        if not gen_file.exists():
            bgn_src = cls._get_begin_source(ori_file=ori_file,
                                            gen_file=gen_file)
    ...
    @classmethod
    def gen_tiling_data_h(cls, op: str, gen_files: List[Path],
                          data_file: Path):
        if not data_file.exists():
            bgn_src = cls._get_begin_source(ori_file=data_file,
                                            gen_file=data_file)
            def_src = ""
            for gen_f in gen_files:
                def_src += "#include \"tiling/{op}/{file_name}\"\n".format(
                    op=op, file_name=gen_f.name)
            source = bgn_src + def_src
    ...
    def gen_tiling_stub_h(cls, data_file: Path, stub_file: Path):
        if not stub_file.exists():
            bgn_src = cls._get_begin_source(ori_file=stub_file,
                                            gen_file=stub_file)
            def_src = ""
    ...
    def main(cls):
        # 参数注册
        parser = argparse.ArgumentParser(description="TilingData Generator",
                                         epilog="Best Regards!")
