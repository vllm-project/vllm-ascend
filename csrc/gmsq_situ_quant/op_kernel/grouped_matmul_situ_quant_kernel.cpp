...
constexpr int32_t BOUNDED_MAX_E = 128;
constexpr int32_t BOUNDED_HEADER = 16;
constexpr int32_t BOUNDED_MAP = BOUNDED_HEADER;
constexpr int32_t BOUNDED_MISS = BOUNDED_MAP + BOUNDED_MAX_E;
constexpr int32_t BOUNDED_NEXT_OWNER = BOUNDED_MISS + BOUNDED_MAX_E;
constexpr int32_t BOUNDED_OWNER_WORDS = BOUNDED_HEADER + BOUNDED_MAX_E;
constexpr int32_t BOUNDED_DECISION_WORDS = BOUNDED_NEXT_OWNER + BOUNDED_OWNER_WORDS;
...
public:
    ...
    __aicore__ inline void Init(const GlobalTensor<int64_t> &glGM, int32_t E, int32_t glType,
                                int32_t expertBegin = 0, int32_t expertEnd = BOUNDED_MAX_E)
    {
        ...
        for (int32_t e = 0; e < E_; e++) {
            ...
            start_[e] = sumAllM;
            sumAllM += cnt;
            cnt = (e >= expertBegin && e < expertEnd) ? cnt : 0;
            count_[e] = cnt;
            padOff_[e] = sumP;
            sumM += cnt;
            sumP += ((cnt + BM - 1) / BM) * BM;
...
public:
                                ...
                                int32_t expertBegin = 0, int32_t expertEnd = BOUNDED_MAX_E)
    {
        ...
        Parse(glType, expertBegin, expertEnd);
    }

    __aicore__ inline void Parse(int32_t glType, int32_t expertBegin, int32_t expertEnd)
    {
        ...
        for (int32_t e = 0; e < E_; e++) {
            ...
            int32_t cnt = (glType == 1) ? static_cast<int32_t>(gv)
                                        : static_cast<int32_t>(gv - prev);
            prev = gv;
            meta.SetValue(E_ + e, sumAllM);  // original, unfiltered x/y start
            sumAllM += cnt;
            cnt = (e >= expertBegin && e < expertEnd) ? cnt : 0;
            meta.SetValue(e, cnt);           // this batch's mE[e]
...
// Target decode geometry: at most twelve <=64-row experts fit the wider
// N schedule without the extra 1024-column wave paid by all fourteen experts.
// Both core types inspect current device routing; no host read or cached route.
template <typename Meta>
__aicore__ inline int32_t SelectProfileNBlock(Meta &meta, int32_t E, int32_t K,
                                             int32_t N, int32_t fallback)
{
    if (E != 14 || K != 3584 || N != 6144) {
        return fallback;
    }
    int32_t active = 0;
    for (int32_t e = 0; e < E; ++e) {
        const int32_t rows = meta.Count(e);
        if (rows > 64) {
            return fallback;
        }
        active += rows > 0 ? 1 : 0;
    }
    return active > 0 && active <= 12 ? 1024 : fallback;
}
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR w, GM_ADDR acc, GM_ADDR scPtrTbl,
                                ...
                                int32_t K, int32_t N, int32_t nBlock, TPipe *pipe, GM_ADDR decision = nullptr,
                                int32_t expertBegin = 0, int32_t expertEnd = BOUNDED_MAX_E)
    {
        K_ = K;
        N_ = N;
        ...
        meta_.Init(glGM, E, glType, expertBegin, expertEnd);
        nBlock_ = SelectProfileNBlock(meta_, E, K, N, nBlock);
        nNum_ = (N + nBlock_ - 1) / nBlock_;
        if (decision != nullptr) {
            GlobalTensor<int32_t> mapGM;
            mapGM.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(decision) + BOUNDED_MAP);
            int32_t lastCacheLine = -1;
            for (int32_t e = 0; e < E; ++e) {
                if (meta_.Count(e) == 0) {
                    continue;  // inactive slots are never consumed by GEMM
                }
                const int32_t cacheLine = e / 16;
                if (cacheLine != lastCacheLine) {
                    AscendC::DataCacheCleanAndInvalid<int32_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                                                    AscendC::DcciDst::CACHELINE_OUT>(mapGM[cacheLine * 16]);
                    lastCacheLine = cacheLine;
                }
                w8Slot_[e] = mapGM.GetValue(e);
            }
        } else {
            for (int32_t e = 0; e < E; ++e) {
                w8Slot_[e] = e;
        ...
        for (int32_t w = 0; w < totalWaves; w++) {
            ...
            if (t < totalTiles_) {
                ...
                if (curM > 0) {
                    ...
                    int64_t wBase = (int64_t)w8Slot_[e] * K_ * N_ + (int64_t)tailN * K_;
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR wPtrTbl, GM_ADDR scPtrTbl, GM_ADDR w8, GM_ADDR acc,
                                ...
                                int32_t nzInput, GM_ADDR decision = nullptr,
                                int32_t expertBegin = 0, int32_t expertEnd = BOUNDED_MAX_E, bool rawInt32 = false)
    {
        skipUnpack_ = skipUnpack;
        bounded_ = (decision != nullptr);
        // Act/quant never uses W8 mapping. A hit skips this extra DMA and
        // MTE2_S wait entirely; the map is only needed by the unpack producer.
        if (bounded_ && skipUnpack == 0) {
            pipe->InitBuffer(activeMapBuf_, 2 * BOUNDED_MAX_E * sizeof(int32_t));
            GlobalTensor<int32_t> mappingGM;
            mappingGM.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(decision) + BOUNDED_MAP);
            AscendC::DataCopy(activeMapBuf_.Get<int32_t>(), mappingGM, 2 * BOUNDED_MAX_E);
        ...
        meta_.Init(glGM, E, glType, pipe, expertBegin, expertEnd);
        realBase_ = expertBegin > 0 && expertBegin < E ? meta_.Start(expertBegin) : 0;
        ...
        pipe->InitBuffer(accGQueue_, 1, rawInt32 ? rowF32 : rowH16);
        // MSD reads up to 1024 uint64 scale carriers, even for short N rows.
        pipe->InitBuffer(accUQueue_, 1, rawInt32 ? (rowF32 < 8192 ? 8192 : rowF32) : rowH16);
        pipe->InitBuffer(xsQueue_, 1, rawInt32 ? 32 : XS_MAX_CHUNK * sizeof(float));
        ...
        const uint32_t reductionChunks = (N2 + ACT_RED_CHUNK - 1) / ACT_RED_CHUNK;
        pipe->InitBuffer(chunkMaxBuf_, (reductionChunks * sizeof(float) + 31) / 32 * 32);
    }
...
protected:
    ...
    __aicore__ inline void CacheFillUnpackAll(int32_t coreIdx, int32_t subIdx, int32_t cores)
    {
        ...
        for (int32_t idx = coreIdx; idx < totalStrips; idx += cores) {
            ...
            if (bounded_ && activeMapBuf_.Get<int32_t>().GetValue(BOUNDED_MAX_E + e) == 0) {
                continue;  // inactive or resident expert; no input read/output write
            }
    ...
    __aicore__ inline void UnpackBlockNz(int32_t e, int32_t row0, int32_t jb, int32_t nBlk)
    {
        ...
        int64_t eBase = (int64_t)W8Slot(e) * K_ * N_;
    ...
    // One ND unpack tile: 32 rows x 32 int32 carriers -> FP16 [32,256].
    // Eight strided Casts convert it directly to INT8 UB [8,32,32], where
    // the leading dimension selects the 32-column NZ block. The two 16-row
    // K-blocks are contiguous within each 1024-byte output block, so one
    // eight-block DMA preserves the GEMM's original NZ address convention:
    ...
    __aicore__ inline void UnpackTile(int32_t e, int32_t bn, int32_t bk, int32_t subIdx)
    {
        ...
        // mask=32 converts 32 half elements to one 32-byte INT8 row. Repeat
        // strides are 32-byte units: source +512B, destination +32B. Thus
        // out[nblk*1024 + row*32 + col] = int8(h16[row*256 + nblk*32 + col]).
        // The eight output regions are disjoint and fit the existing 8KiB
        // queue buffer; no Gather indices or extra synchronization IDs.
        for (int32_t nblk = 0; nblk < 8; ++nblk) {
            AscendC::Cast(out[nblk * 1024], h16[nblk * 32], AscendC::RoundMode::CAST_NONE,
                          static_cast<uint64_t>(32), static_cast<uint8_t>(32),
                          AscendC::UnaryRepeatParams{1, 1, 1, 16});
        }
        inQueue_.FreeTensor(pkl);
        outQueue_.EnQue(out);

        LocalTensor<int8_t> outl = outQueue_.DeQue<int8_t>();
        // EnQue/DeQue establishes V->MTE3 before consuming the NZ queue
        // buffer. UB blocks are contiguous (srcStride=0); GM block starts
        // are K*32 bytes apart. UB->GM blockLen and dstStride are bytes.
        // FreeTensor/AllocTensor protects this buffer until its DMA completes;
        // the FP16 temporary is never used as a DMA source.
        int64_t eBase = (int64_t)W8Slot(e) * K_ * N_;
        DataCopyExtParams nzcp{static_cast<uint16_t>(8), static_cast<uint32_t>(1024),
                               0, static_cast<uint32_t>(K_ * 32 - 1024), 0};
        int64_t dstOff = eBase + (int64_t)(bn * 8) * K_ * 32 + (int64_t)row0 * 32;
        AscendC::DataCopyPad(w8GM_[dstOff], outl, nzcp);
    ...
    __aicore__ inline void ActPhaseGlobal(int32_t slot, int32_t slots, int32_t mValid)
    {
        ...
        for (int32_t g0 = realBase_ + slot * chunk; g0 < realBase_ + mValid; g0 += slots * chunk) {
            int32_t len = realBase_ + mValid - g0 < chunk ? realBase_ + mValid - g0 : chunk;
    ...
    __aicore__ inline void ActRow(int32_t accRow, int32_t row, float xsi)
    {
        ...
        AscendC::PipeBarrier<PIPE_V>();
        FinishActRow(row, xsi);
    }

    __aicore__ inline void FinishActRow(int32_t row, float xsi)
    {
        ...
        AscendC::PipeBarrier<PIPE_V>();
        // a_ is dead until rowmax. Reuse its full row as explicit Sigmoid
        // temporary memory, avoiding an unaccounted automatic UB stack buffer.
        AscendC::Sigmoid(t_, t_, a_.ReinterpretCast<uint8_t>(), N2_);
        AscendC::PipeBarrier<PIPE_V>();
        AscendC::Sigmoid(g_, g_, a_.ReinterpretCast<uint8_t>(), N2_);
        AscendC::PipeBarrier<PIPE_V>();
        ...
        if (hasLinear_ != 0) {
            ...
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::Sigmoid(u_, u_, a_.ReinterpretCast<uint8_t>(), N2_);
            AscendC::PipeBarrier<PIPE_V>();
    ...
    __aicore__ inline int32_t W8Slot(int32_t e)
    {
        return bounded_ ? activeMapBuf_.Get<int32_t>().GetValue(e) : e;
    }

    int32_t realBase_ = 0;
    bool bounded_ = false;
    TBuf<AscendC::TPosition::VECCALC> activeMapBuf_;
