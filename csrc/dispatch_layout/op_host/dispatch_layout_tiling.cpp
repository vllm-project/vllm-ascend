                                               ...
                                               DispatchLayoutTilingData &tilingData)
{
    ...
    OPS_CHECK(localRankSizePtr == nullptr, OPS_LOG_E(nodeName, "localRankSizePtr is null."),
    ...
    OPS_CHECK((*numExpertsPtr <= 0) || (*numExpertsPtr > MAX_MOE_EXPERTS_NUM),
                    ...
                    return ge::GRAPH_FAILED);
    OPS_CHECK((*numExpertsPtr % *numRanksPtr) != 0,
                    OPS_LOG_E(nodeName, "numExperts must be divisible by numRanks, but numExperts=%ld and numRanks=%ld.",
...
static bool CheckTensorDataType(gert::TilingContext *context, const char *nodeName)
{
    ...
    OPS_CHECK(notifySendData == nullptr, OPS_LOG_E(nodeName, "notifySendData is null."), return false);
    ...
    OPS_CHECK((isTokenInRank->GetDataType() != ge::DT_INT32),
                    ...
                    return false);
    OPS_CHECK((notifySendData->GetDataType() != ge::DT_INT32),
...
{
    OPS_CHECK(!CheckTensorDataType(context, nodeName), OPS_LOG_E(nodeName, "params dataType is invalid."),
                    return ge::GRAPH_FAILED);

    OPS_CHECK(!CheckTensorShape(context, nodeName), OPS_LOG_E(nodeName, "params dataType is invalid."),
                    return ge::GRAPH_FAILED);
