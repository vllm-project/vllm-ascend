...
std::tuple<at::Tensor, at::Tensor, at::Tensor> npu_fused_infer_attention_score_meta(
    const at::Tensor &query, const at::Tensor &key, const at::Tensor &value,
    ...
    bool sparse_stats_flag, bool flash_decode)
{
    ...
    at::Tensor sparse_stats = at::empty_symint({16}, query.options().dtype(at::kInt));
    return {attention_out, softmax_lse, sparse_stats};
}

// Meta for npu_fused_infer_attention_score_out（图模式 out 变体）：写回传入 tensor，
// 无返回。签名与 torch_binding.cpp 的 def 一致。
void npu_fused_infer_attention_score_out_meta(
    const at::Tensor &query, const at::Tensor &key, const at::Tensor &value,
