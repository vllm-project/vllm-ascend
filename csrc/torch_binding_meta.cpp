...
std::tuple<at::Tensor, at::Tensor> attn_res_fwd_with_add_meta(
...
{
    return {at::empty_symint(prefix_sum.sym_sizes(), prefix_sum.options()),
            at::empty_symint(prefix_sum.sym_sizes(), prefix_sum.options())};
}
...
TORCH_LIBRARY_IMPL_EXPAND(CONCAT(_C, _ascend), Meta, ops) {
    ...
    ops.impl("attn_res_fwd_with_add", &vllm_ascend::meta::attn_res_fwd_with_add_meta);
