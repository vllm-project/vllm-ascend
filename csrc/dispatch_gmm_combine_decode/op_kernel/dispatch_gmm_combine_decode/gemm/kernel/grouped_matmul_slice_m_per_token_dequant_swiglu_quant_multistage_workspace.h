...
public:
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params const &params)
    {
        ...
        uint32_t coreNumPerGroup = recvCoreNum / localExpertNum;  // 与V接收分配逻辑保持一致
    ...
    void CalAndSendTokenCount()
    {
        ...
        AscendC::Duplicate(statusTensor_, (int32_t)0,
                           expertCntUp * INT32_COUNT_PER_BLOCK);  // 先清零再赋值
        if (state == 0) {
    ...
    void RecvCoreFunc(GM_ADDR gmX1, GM_ADDR gmX1Scale, GM_ADDR gmEpSendCount, GM_ADDR gmOutputRecvCount)
    {
        ...
        uint32_t recvCoreNumPerGroup = recvCoreNum / localExpertNum;  // 每个group由若干核处理，下取整，可能有空闲核
    ...
    void AivInitParams(Params const &params)
    {
        ...
        if (localExpertNum > 1) {
            ...
            isSendCore = ((aivIdx % ODD_EVEN_BASE) == 1);  // 奇数核发送
