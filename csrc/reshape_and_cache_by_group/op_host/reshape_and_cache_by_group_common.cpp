...
#include <cstdint>
namespace optiling {
#define PARAMS_2D(p, coreIdx, blockIdx)  (p[(coreIdx) * (MAX_BLOCK_NUM_PER_CORE) + (blockIdx)])
...
void ReshapeAndCacheByGroupCommonTiling::SetTiling()
{
...
ge::graphStatus ReshapeAndCacheByGroupCommonTiling::GetPlatformInfo()
{
...
ge::graphStatus ReshapeAndCacheByGroupCommonTiling::DoCommonTiling()
{
    auto kShape = context_->GetInputShape(DIM_0);
    auto kDimNum=kShape->GetStorageShape().GetDimNum();
    if (kDimNum<2||kDimNum>7){
        printf("[ERROR] ReshapeAndCacheByGroup Intput kDimNum dim < 2 || kDimNum>7");
...
ge::graphStatus ReshapeAndCacheByGroupCommonTiling::DoTiling()
{
