...
class Module:
    ...
    def _add_rel_path(self, src, dst: List[Path]):
        ...
        for p in src:
            p = Path(p)
            if p.is_absolute():
                logging.error("[%s]'s Path[%s] is absolute path.", self.name,
                              p)
                return False
            if p not in dst:
                dst.append(p)
    ...
    def _update_ut(self, desc: Dict[str, Any]) -> bool:
        for sub_name, sub_desc in desc.items():
            if sub_name == "ops_test":
                ...
                if not self._add_rel_path(
                        src=src_files, dst=self.tests_ut_ops_test_src_files):
                    return False
                options = sub_desc.get('options', [])
                if not self._add_str_cfg(src=options,
                                         dst=self.tests_ut_ops_test_options):
                    return False
        return True

    def _check(self) -> bool:
        if len(self.src_files) != 0 or len(
                self.tests_ut_ops_test_src_files) != 0:
            return True
