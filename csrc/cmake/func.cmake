...
function(add_bin_compile_target)
    ...
    foreach(_op_info ${BINARY_OP_INFO})
        ...
        # OpType-to-snake-case conversion may omit the underscore before a
        # numeric suffix (for example, "..._910c" becomes "...910c").
        string(REGEX REPLACE "_([0-9])" "\\1" _op_compact_name "${_op_name}")
        if (NOT "${_op_compact_name}" STREQUAL "${_op_name}")
            set(${_op_compact_name}_dir ${_op_info})
            set(${_op_compact_name}_apt_dir ${_op_info})
            set(${_op_compact_name}_source_name ${_op_name})
        endif()
    endforeach()
    ...
    foreach(bin_script ${compile_scripts})
        ...
        if (_compile_flag)
            ...
            list(APPEND _BUILD_COMMAND bash ${bin_script})
            if (DEFINED ${op_file}_source_name)
                list(APPEND _BUILD_COMMAND
                    --kernel-src=${${op_file}_dir}/op_kernel/${${op_file}_source_name}.cpp)
            endif()
            list(APPEND _BUILD_COMMAND ${OP_SRC_OUT_DIR}/${op_type}.py ${OP_BIN_OUT_DIR})
