 ...
 LocalTensor<inputT> kValueLocal = sortedValueLocal;
 LocalTensor<float> kValueLocalFp32 = sortedValueLocalFp32;
 DataCopyPad(
 kValueLocal, mGmSortedValue_[baseGmIdx_ + vocabSize_ - kValue], {1, static_cast<uint32_t>(sizeof(inputT)), 0, 0, 0},
 {false, 0, 0, 0});
 if constexpr (!IsSameType<inputT, float>::value) {
 MTE2ToVSync();
 Cast(kValueLocalFp32, kValueLocal, RoundMode::CAST_NONE, 1);
 VToSSync();
 } else {
 MTE2ToSSync();
 kValueLocalFp32 = kValueLocal;
 }
 kthValue = kValueLocalFp32.GetValue(0);
