#include "aclnn_zb_moe_distribute_dispatch_zero_buffer.h"
...
extern aclnnStatus aclnnInnerZbMoeDistributeDispatchZeroBufferGetWorkspaceSize(
...
extern aclnnStatus aclnnInnerZbMoeDistributeDispatchZeroBuffer(void *workspace, uint64_t workspaceSize,
                                                                   aclOpExecutor *executor, aclrtStream stream);

extern "C" void __attribute__((weak)) NnopbaseSetHcclServerType(void *executor, NnopbaseHcclServerType sType);

aclnnStatus aclnnZbMoeDistributeDispatchZeroBufferGetWorkspaceSize(
...
{
    return aclnnInnerZbMoeDistributeDispatchZeroBufferGetWorkspaceSize(
...
aclnnStatus aclnnZbMoeDistributeDispatchZeroBuffer(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
                                                      aclrtStream stream)
{
    if (NnopbaseSetHcclServerType) {
        NnopbaseSetHcclServerType(executor, NNOPBASE_HCCL_SERVER_TYPE_MTE);
    }
    return aclnnInnerZbMoeDistributeDispatchZeroBuffer(workspace, workspaceSize, executor, stream);
}
