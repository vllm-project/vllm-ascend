...
#include "../op_kernel/matmul_allreduce_add_rmsnorm_910c_tiling.h"
#include "matmul_allreduce_add_rmsnorm_910c_workspace.h"
...
constexpr int32_t SUPPORTED_RANK_SIZE = 2;
constexpr int32_t SUPPORTED_N = 5120;
constexpr int32_t SUPPORTED_DOWN_K = 12800;
constexpr int32_t SUPPORTED_O_PROJ_K = 4096;
constexpr int32_t MAX_SUPPORTED_M = 102;
...
static double GetMTETime(double mknGB, int32_t m0, int32_t n0, double aBindWidth = 3.0, double bBindWidth = 3.0)
{
    // 预估Matmul计算的MTE2搬运时间
    return DOUBLE * mknGB * (SECOND_TO_MS / ONE_K) * (1.0 / (n0 * aBindWidth) + 1.0 / (m0 * bBindWidth));
}

static int32_t AllReduceUbMoveNum(int m, int k, int n)
{
...
static int32_t AllReducePValue(int m, int k, int n)
{
    double commPredict = 1.0 * (m / ONE_K) * (n / ONE_K) * (SECOND_TO_MS / ONE_K) / 40;
    double cubePredict = DOUBLE * m * k / B1_FLOP_PER_MS * n;
    double mknGB = (m / ONE_K) * (k / ONE_K) * (n / ONE_K);
...
static int32_t GetValueFromMKNConditionMap(
...
static void AllReduceEightRankFP16GetDefaultTiling(
...
static ge::graphStatus GetAttrAndSetTilingData(
    gert::TilingContext *context, const char *nodeName, MatmulAllreduceAddRmsnorm910cTilingData &tilingData)

{
    ...
    OPS_ERR_IF(RankSizePtr == nullptr || RankIdPtr == nullptr,
        OPS_LOG_E(nodeName, "rank attributes are null."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(*RankSizePtr != SUPPORTED_RANK_SIZE,
        OPS_LOG_E(nodeName, "Only TP=2 is supported, got %ld.", *RankSizePtr), return ge::GRAPH_FAILED);
    OPS_ERR_IF(*RankIdPtr < 0 || *RankIdPtr >= *RankSizePtr,
        OPS_LOG_E(nodeName, "Invalid rank id %ld for rank size %ld.", *RankIdPtr, *RankSizePtr),
        return ge::GRAPH_FAILED);

    bool isTransB = *(attrs->GetAttrPointer<bool>(ATTR_IS_TRANS_B_INDEX));
    OPS_ERR_IF(!isTransB,
        OPS_LOG_E(nodeName, "Only is_trans_b=true is supported."), return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(opShape.m <= 0 || opShape.m > MAX_SUPPORTED_M,
        OPS_LOG_E(nodeName, "Only 1 <= M <= %d is supported, got %ld.", MAX_SUPPORTED_M, opShape.m),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(opShape.n != SUPPORTED_N,
        OPS_LOG_E(nodeName, "Only N=%d is supported, got %ld.", SUPPORTED_N, opShape.n),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(opShape.k != SUPPORTED_DOWN_K && opShape.k != SUPPORTED_O_PROJ_K,
        OPS_LOG_E(nodeName, "Only K=%d or K=%d is supported, got %ld.",
            SUPPORTED_DOWN_K, SUPPORTED_O_PROJ_K, opShape.k), return ge::GRAPH_FAILED);
    OPS_ERR_IF(context->GetInputTensor(0)->GetDataType() != ge::DT_BF16,
        OPS_LOG_E(nodeName, "Only BF16 is supported."), return ge::GRAPH_FAILED);
...
static int64_t GetAlignedMatrixSize(
...
static WorkspaceDetail GetWorkspaceDetail(CoCDataTypeDesc dataType, const MatMulInfo &mmInfo, const QuantInfo &quantInfo)
{
...
static void GetMmInfo(gert::TilingContext *context, MatmulAllreduceAddRmsnorm910cTilingData *tiling, MatMulInfo *mmInfo)
{
...
static size_t GetUserWorkspaceSize(gert::TilingContext *context, MatmulAllreduceAddRmsnorm910cTilingData *tiling)
{
    static constexpr size_t fineGrainReadyWorkspaceBytes = 28 * 64;
    MatMulInfo mmInfo;
    GetMmInfo(context, tiling, &mmInfo);
    QuantInfo quantInfo = tiling->matmulAllreduceAddRmsnormInfo.quantInfo;
    return fineGrainReadyWorkspaceBytes + GetWorkspaceDetail(FP16FP16_FP32_FP16, mmInfo, quantInfo).GetSize();
}
...
static ge::graphStatus SetWorkSpace(gert::TilingContext *context, const char *nodeName)
{
    ...
    MatmulAllreduceAddRmsnorm910cTilingData *tilingData = context->GetTilingData<MatmulAllreduceAddRmsnorm910cTilingData>();
...
static void SetHcommCfg(
    const gert::TilingContext *context, MatmulAllreduceAddRmsnorm910cTilingData *tiling, const std::string groupTp)
{
    const char *nodeName = context->GetNodeName();
...
static ge::graphStatus MatmulAllreduceAddRmsnorm910cTilingFuncImpl(gert::TilingContext *context)
{
    const char *nodeName = context->GetNodeName();
    MatmulAllreduceAddRmsnorm910cTilingData *tilingData = context->GetTilingData<MatmulAllreduceAddRmsnorm910cTilingData>();
    ...
    auto attrs = context->GetAttrs();
    OPS_ERR_IF(attrs == nullptr, OPS_LOG_E(nodeName, "attrs is nullptr."), return ge::GRAPH_FAILED);
    auto groupTpPtr = attrs->GetAttrPointer<char>(ATTR_TP_INDEX);
    OPS_ERR_IF(groupTpPtr == nullptr || groupTpPtr[0] == '\0',
        OPS_LOG_E(nodeName, "group_tp is invalid."), return ge::GRAPH_FAILED);
    SetHcommCfg(context, tilingData, std::string(groupTpPtr));
...
static ge::graphStatus MatmulAllreduceAddRmsnorm910cTilingFunc(gert::TilingContext *context)
{
    ge::graphStatus ret = MatmulAllreduceAddRmsnorm910cTilingFuncImpl(context);
    return ret;
}

struct MatmulAllreduceAddRmsnorm910cCompileInfo {};
ge::graphStatus TilingParseForMatmulAllreduceAddRmsnorm910c(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(MatmulAllreduceAddRmsnorm910c)
    .Tiling(MatmulAllreduceAddRmsnorm910cTilingFunc)
    .TilingParse<MatmulAllreduceAddRmsnorm910cCompileInfo>(TilingParseForMatmulAllreduceAddRmsnorm910c);
