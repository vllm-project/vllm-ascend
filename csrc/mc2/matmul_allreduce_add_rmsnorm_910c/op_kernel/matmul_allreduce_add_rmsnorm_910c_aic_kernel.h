...
#ifndef MATMUL_ALLREDUCE_ADD_RMSNORM_910C_AIC_KERNEL_H
#define MATMUL_ALLREDUCE_ADD_RMSNORM_910C_AIC_KERNEL_H
...
constexpr uint32_t MM_L1_TILE_SHAPE_M = 16;
constexpr uint32_t MM_L1_TILE_SHAPE_N = FINE_GRAIN_TILE_ELEMENTS;
constexpr uint32_t MM_L1_TILE_SHAPE_K = 512;
...
class MatmulAllreduceAddRmsnorm910cAicKernel {
    using T_ACCUM = typename GetAccumType<MmadDtype>::T;
public:
    int PIPE_DEPTH = 2;
    Arch::Resource<Arch::AtlasA2> resource;
    __aicore__ inline MatmulAllreduceAddRmsnorm910cAicKernel<MmadDtype, OutDtype>() { }
                                ...
                                const MatmulAllreduceAddRmsnorm910cTilingData* tilingData)
    {
        ...
        ready_flags_.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(workspace),
            FINE_GRAIN_READY_WORKSPACE_BYTES / sizeof(int32_t));
        auto matmulWorkspace = workspace + FINE_GRAIN_READY_WORKSPACE_BYTES;
        auto workspace_info = GetWorkspaceInfo(matmulWorkspace, this->batch_size, this->m, this->k, this->n,
        ...
        for (int splitIndex = 0; splitIndex < splitM; ++splitIndex) {
            ...
            for (uint32_t loopIdx = core_idx; loopIdx < coreLoops; loopIdx += core_num) {
                ...
                if constexpr (FINE_GRAIN_DEBUG_STAGE >= 0) {
                    int32_t readyEvent = GetTPipePtr()->FetchEventID(HardEvent::FIX_S);
                    SetFlag<HardEvent::FIX_S>(readyEvent);
                    WaitFlag<HardEvent::FIX_S>(readyEvent);
                    uint32_t readyOffset = core_idx *
                        (FINE_GRAIN_READY_FLAG_STRIDE_BYTES / sizeof(int32_t));
                    ready_flags_.SetValue(readyOffset, 1);
                    DataCacheCleanAndInvalid<int32_t, CacheLine::SINGLE_CACHE_LINE,
                        DcciDst::CACHELINE_OUT>(ready_flags_[readyOffset]);
...
private:
    ...
    AscendC::GlobalTensor<int32_t> ready_flags_;
...
#endif // MATMUL_ALLREDUCE_ADD_RMSNORM_910C_AIC_KERNEL_H
