...
struct alignas(8) AttnResFwdTilingData {
    ...
    // Fused inference fields are zero for the original ABI.
    uint64_t blockTokenStride;
    int32_t blockWriteIdx;
    uint32_t fusedChain;
    uint32_t fusedAdd;
    uint32_t mix;
    uint32_t saveMaterialized;
    float outputNormEps;
    uint32_t elemsPerToken;    // 2*blockCount；false 时可写 0
    uint32_t cacheOutputNorm;  // Fused inference: one BF16 weight row per core.
};
#pragma pack(pop)
} // namespace AttnResFwd
