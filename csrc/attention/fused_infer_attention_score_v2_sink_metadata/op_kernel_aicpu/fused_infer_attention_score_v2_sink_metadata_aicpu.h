...
#ifndef FUSED_INFER_ATTENTION_SCORE_V2_SINK_METADATA_AICPU_H
#define FUSED_INFER_ATTENTION_SCORE_V2_SINK_METADATA_AICPU_H
...
template <class T>
using BlockCost = std::array<std::array<T, static_cast<size_t>(BlockType::BLOCK_MAX_TYPE)>,
                             static_cast<size_t>(BlockType::BLOCK_MAX_TYPE)>;

template <typename T> static inline T Clip(T value, T minValue, T maxValue)
{
    if (value < minValue) {
        return minValue;
    }
...
template <typename T> static inline auto Align(T num, T rnd) -> T
{
    return (((rnd) == 0) ? 0 : (((num) + (rnd)-1) / (rnd) * (rnd)));
}

template <typename T> static inline bool IsWithinTolerance(T limit, T tolerance, T value)
{
...
struct S1GCache {
    ...
    int64_t firstTokenWithSinkNumber{0};
};
...
class FusedInferAttentionScoreV2SinkMetadataKernel : public CpuKernel {
  public:
    FusedInferAttentionScoreV2SinkMetadataKernel() = default;
    ~FusedInferAttentionScoreV2SinkMetadataKernel() noexcept = default;
  ...
  private:
    ...
    bool GetS2Size();
    bool GetBatchSize();
    bool GetActualSeqLenSize(uint32_t &size);
    bool GetActualSeqKVLenSize(uint32_t &size);
    bool CheckSingleParaInputLayout();
    bool CheckSingleParaNumHeads();
    bool GetPreNextToken();
    bool GetActualSeqInfo();
    bool ParamsCheck();
    void CheckIsMla();
    void CalcInnerSizeMla(uint32_t seqSize);
    void CalcMBaseSizeMla();
    void CreateSplitInputMla(BaseInfo &baseInfo, SplitParam &splitParam) const;
    bool DealSameSeqEachBatch();
    void CalcInnerSizeGqa(uint32_t seqSize);
    void CalcMBaseSizeGqa();
    void CreateSplitInputGqa(BaseInfo &baseInfo, SplitParam &splitParam) const;
    bool CheckFeature();
    bool CheckFeatureActualSeqLensQData();
    bool CheckFeatureActualSeqLensKvData();
    bool CheckFeatureAxisInfo() const;
    bool CheckMultiParaConsistency();
    bool CheckActualSeqLensQ() const;
    bool CheckActualSeqLensKv() const;
    bool CheckParamSinkNumber();
    bool CheckSinkNumber();
    bool CheckMlaSinkNumber();
    ...
    Range<uint32_t>
    CalcS2Range(uint32_t s1GIdx, const BaseInfo &baseInfo, const SplitParam &splitParam, const BatchCache &batchCache);
    int64_t CalcCost(uint32_t basicM, uint32_t basicS2);
    BlockCost<int64_t>
    CalcCostTable(uint32_t s1NormalSize, uint32_t s2NormalSize, uint32_t s1GTailSize, uint32_t s2TailSize);

    // cache calculation
    void CalcBatchCache(uint32_t bIdx, const SplitContext &splitContext, BatchCache &batchCache);
    void
    CalcS1GCache(uint32_t s1GIdx, const SplitContext &splitContext, const BatchCache &batchCache, S1GCache &s1GCache);
    void CopyTmpResult(SplitResult &tmpRes, SplitResult &splitRes);
    void ClearTmpResult(SplitResult &tmpResult);
    void CalcS1GCacheWithSinkNumber(uint32_t s1GIdx,
                                    const SplitContext &splitContext,
                                    const BatchCache &batchCache,
                                    S1GCache &s1GCache,
                                    uint32_t sinkNumber);
    void
    UpdateCurS2IdxWithSinkNumber(const SplitContext &splitContext, AssignContext &assignContext, uint32_t sinkNumber);
  ...
  private:
    enum class InputParamId : uint32_t {
        ACTUAL_SEQ_LENGTHS_OPTIONAL = 0,
        ACTUAL_SEQ_LENGTHS_KV_OPTIONAL = 1,
    };

    enum class OutputParamId : uint32_t {
        META_DATA = 0,
    };
  ...
  private:
    ...
    // input
    Tensor *actualSeqLengths_ = nullptr;
    Tensor *actualSeqLengthsKv_ = nullptr;
    ...
    uint32_t numHeadsKv_ = 1.0;
    uint32_t headDimQK_ = 0;
    uint32_t headDimV_ = 0;
    uint32_t ropeHeadDim_ = 0;
    uint32_t batchSize_ = 0;
    uint32_t sparseMode_ = 0;
    int64_t preTokens_ = 2147483647;
    int64_t nextTokens_ = 2147483647;
    int64_t blockSize_ = 0;
    std::string inputLayout_ = "TND";
    std::string inputLayoutKv_ = "TND";
    int64_t sinkNum_ = 0;
    int64_t kSinkNum_ = 0;
    bool batchInvariant_ = false;
    ...
    int64_t preToken = 0;
    int64_t nextToken = 0;
    bool attenMaskFlag = true;
    uint32_t actualSeqS1Size = 0;
    uint32_t actualSeqS2Size = 0;
    bool isMlaSink = false;
    ...
    bool batchContinuousFlag = false;
    ...
    uint32_t numOfFdHead = 0;
    uint32_t usedCoreNum = 0;
    uint32_t usedVecNumOfFd = 0;
...
#endif // FUSED_INFER_ATTENTION_SCORE_V2_SINK_METADATA_AICPU_H