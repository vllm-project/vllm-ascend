...
def run_tnd_case():
    block_counts = torch.tensor([[1, 2, 3, 4, 5, 6, 7, 8]], dtype=torch.int32, device="npu")
    ...
    for q_block, block_count in enumerate(block_counts[0].cpu().tolist()):
        sparse_block_idx[0, q_block, :block_count] = torch.arange(block_count, dtype=torch.int32, device="npu")
