...
namespace fused_rearrange_mix_qkv_gdn_gating_impl {
...
public:
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, const FusedRearrangeMixQkvGdnGatingTilingData& tiling, TPipe* pipe)
    {
 ...
 */
template <typename TALog, typename TDtBias>
...
public:
    __aicore__ inline void Init(GM_ADDR a, GM_ADDR b, GM_ADDR aLog, GM_ADDR dtBias, GM_ADDR g, GM_ADDR betaOut,
                                const FusedRearrangeMixQkvGdnGatingTilingData& tiling, TPipe* pipe)
    {
        tokens_ = tiling.tokens;
        ...
        aLog_.SetGlobalBuffer(reinterpret_cast<__gm__ TALog*>(aLog));
        dtBias_.SetGlobalBuffer(reinterpret_cast<__gm__ TDtBias*>(dtBias));
        ...
        pipe->InitBuffer(aLogRawBuf_, AlignUp(heads_ * static_cast<uint32_t>(sizeof(TALog)), ALIGN_BYTES));
        pipe->InitBuffer(dtBiasRawBuf_, AlignUp(heads_ * static_cast<uint32_t>(sizeof(TDtBias)), ALIGN_BYTES));
...
private:
    ...
    {
        LocalTensor<TALog> aLogRaw = aLogRawBuf_.Get<TALog>();
        LocalTensor<TDtBias> dtBiasRaw = dtBiasRawBuf_.Get<TDtBias>();
        ...
        LoadHead(dtBiasRaw, dtBias_);
        LoadHead(aLogRaw, aLog_);
        SetFlag<HardEvent::MTE2_V>(EVENT_HEAD_LOAD_TO_VEC);
        WaitFlag<HardEvent::MTE2_V>(EVENT_HEAD_LOAD_TO_VEC);
        if constexpr (IsSameType<TDtBias, float>::value) {
            Adds(biasHead, dtBiasRaw, 0.0f, heads_);
        } else {
            Cast(biasHead, dtBiasRaw, RoundMode::CAST_NONE, heads_);
        }
        if constexpr (IsSameType<TALog, float>::value) {
            Adds(scaleHead, aLogRaw, 0.0f, heads_);
        } else {
            Cast(scaleHead, aLogRaw, RoundMode::CAST_NONE, heads_);
        }
        PipeBarrier<PIPE_V>();
    ...
    template <typename T>
    __aicore__ inline void LoadHead(const LocalTensor<T>& dst, const GlobalTensor<T>& src)
    {
        if (aligned_) {
            ...
            params.blockCount = 1;
            params.blockLen = heads_ * sizeof(T);
            params.srcStride = 0;
            params.dstStride = 0;
    ...
    GlobalTensor<TALog> aLog_;
    GlobalTensor<TDtBias> dtBias_;
    ...
    TBuf<TPosition::VECCALC> aLogRawBuf_;
    TBuf<TPosition::VECCALC> dtBiasRawBuf_;
