...
/*!
 * \file COMPRESSOR_V2_tiling_datay.h
 * \brief
 */

#ifndef COMPRESSOR_V2_TILING_DATA_H
#define COMPRESSOR_V2_TILING_DATA_H
#include <cstdint>
...
namespace optiling {
struct CompressorV2SplitCoreParams {
    uint32_t mStart;
    uint32_t mEnd;
    uint32_t nStart;
    uint32_t nEnd;
...
struct CompressorV2BaseParams {
    ...
    uint32_t mLoopNum = 0;
    CompressorV2SplitCoreParams splitCoreParam[CMP_MAX_AIC_CORE_NUM];
};

struct CompressorV2PageAttentionParams {
    uint32_t blockNum = 0;
    uint32_t blockSize = 1;
    uint32_t maxBlockNumPerBatch = 1;
};

struct CompressorV2InnerSplitParams {
    uint32_t mBaseSize;
    uint32_t dBaseSize;
};

struct CompressorV2WorkspaceParams {
...
struct CompressorV2TilingData {
    CompressorV2BaseParams baseParams;
    CompressorV2PageAttentionParams pageAttentionParams;
    CompressorV2InnerSplitParams innerSplitParams;
    CompressorV2WorkspaceParams workspaceParams;
};
} // namespace optiling

#endif // COMPRESSOR_V2_TILING_DATA_H
