...
/*!
 * \file compressor_v2_kernel_full_load.h
 * \brief
 */

#ifndef COMPRESSOR_V2_KERNEL_FULL_LOAD_H
#define COMPRESSOR_V2_KERNEL_FULL_LOAD_H

#include "compressor_v2_comm_arch22.h"
#include "compressor_v2_template_tiling_key.h"
#include "compressor_v2_tiling_data.h"
#include "compressor_v2_tools.h"
#include "compressor_v2_block_cube_full_load.h"
#include "compressor_v2_block_vec_full_load.h"

using namespace AscendC;

namespace CompressorV2 {
template <typename COMP>
class CompressorV2KernelFullLoad {
public:
    __aicore__ inline CompressorV2KernelFullLoad(TPipe *pipe,
                                                 const optiling::CompressorV2TilingData *__restrict tilingData)
        : pipe_(pipe),
...
private:
    ...
    __aicore__ inline void ComputeVec();
    ...
    // ==============================Service Define==============================
    CompressorV2BlockCubeFullLoad<COMP> blockCube_;
    CompressorV2BlockVectorFullLoad<COMP> blockVec_;
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::Init(__gm__ uint8_t *x, __gm__ uint8_t *wKv,
                                                              ...
                                                              __gm__ uint8_t *workspace)
{
    ...
    if ASCEND_IS_AIC {
        ...
        blockCube_.Init(x, wKv, wGate, stateCache, stateBlockTable, cuSeqlens, seqUsed, startPos, cmpKvOut);
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::InitTilingData()
{
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::SkipInvalidBatch(BatchInfo &batchInfo)
{
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::CalcSplitCoreInfo()
{
    // D方向的基本块数量
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::InitWorkspace(__gm__ uint8_t *workspace)
{
    uint64_t offset = 0;
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::ComputeMm1(const RunInfo &info)
{
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::ComputeVec()
{
    blockVec_.PrepareForComputeVec();
    CrossCoreWaitFlag<SYNC_MODE2, PIPE_MTE2>(SYNC_C1_V1_FLAG);
    blockVec_.ComputeVec1();
}

template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::AllocEventID()
{
    if ASCEND_IS_AIC {
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::FreeEventID()
{
    if ASCEND_IS_AIC {
...
template <typename COMP>
__aicore__ inline bool CompressorV2KernelFullLoad<COMP>::IsNeedExcuteC1(RunInfo info)
{
    // B超出范围则cube不执行
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::CalcCubeParams(RunInfo &info)
{
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::CalcV1Params(Vec1RunInfo &vec1Info)
{
    vec1Info.bStart = 0;
    vec1Info.sStart = 0;
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelFullLoad<COMP>::Process()
{
    // 所有batch的有效序列都为0时, 直接退出
    if (constInfo.batchSize == 0) {
        return;
...
#endif // COMPRESSOR_V2_KERNEL_FULL_LOAD_H
