...
/*!
 * \file compressor_v2_block_vec_full_load.h
 * \brief
 */

#ifndef COMPRESSOR_V2_BLOCK_VEC_FULL_LOAD_H
#define COMPRESSOR_V2_BLOCK_VEC_FULL_LOAD_H
...
class CompressorV2BlockVectorFullLoad {
public:
...
template <typename COMP>
__aicore__ inline void CompressorV2BlockVectorFullLoad<COMP>::AllocEventID()
{}

template <typename COMP>
__aicore__ inline void CompressorV2BlockVectorFullLoad<COMP>::FreeEventID()
{}

template <typename COMP>
__aicore__ inline void CompressorV2BlockVectorFullLoad<COMP>::InitVec1GlobalTensor(GlobalTensor<T> kvMm1ResGm,
...
template <typename COMP>
__aicore__ inline uint32_t CompressorV2BlockVectorFullLoad<COMP>::GetSeqUsed(uint32_t bIdx)
{
    if (isExistSeqUsed_) {
...
template <typename COMP>
__aicore__ inline uint32_t CompressorV2BlockVectorFullLoad<COMP>::GetStartPos(uint32_t bIdx)
{
    if (isExistStartPos_) {
...
template <typename COMP>
__aicore__ inline uint32_t CompressorV2BlockVectorFullLoad<COMP>::GetSeqLength(uint32_t bIdx)
{
    ...
    const Vec1SliceInfo &sliceInfo, uint32_t dStartIdx, uint32_t dDealSize, uint32_t dBaseSize, uint32_t stateIdx)
{
...

    {
        ...
        srcBaseOffset += (validStartSeqIdx - startSeqIdx) * coff_ * dBaseSize;
...
template <typename COMP>
__aicore__ inline void CompressorV2BlockVectorFullLoad<COMP>::PrepareForComputeVec()
{
...
#endif // COMPRESSOR_V2_BLOCK_VEC_FULL_LOAD_H
