...
/*!
 * \file compressor_v2_kernel_perf.h
 * \brief
 */

#ifndef COMPRESSOR_V2_KERNEL_PERF_H
#define COMPRESSOR_V2_KERNEL_PERF_H
...
#include "compressor_v2_block_cube_perf.h"
#include "compressor_v2_block_vec_perf.h"
...
private:
    ...
    // ==============================Service Define==============================
    CompressorV2BlockCubePerf<COMP> blockCube_;
    CompressorV2BlockVectorPerf<COMP> blockVec_;
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelPerf<COMP>::InitTilingData()
{
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelPerf<COMP>::SetBaseSize()
{
    uint32_t mSize = 0;
...
template <typename COMP>
__aicore__ inline uint32_t CompressorV2KernelPerf<COMP>::GetLoopTimes()
{
    // 计算主循环次数
    uint32_t loopTimes = 0;
    BatchInfo batchInfo{};
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelPerf<COMP>::CalcSplitCoreInfo()
{
    // D方向的基本块数量
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelPerf<COMP>::AllocEventID()
{
    if ASCEND_IS_AIC {
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelPerf<COMP>::FreeEventID()
{
    if ASCEND_IS_AIC {
        for (int i = 0; i < constInfo.dbWorkspaceRatio; ++i) {
...
template <typename COMP>
__aicore__ inline bool CompressorV2KernelPerf<COMP>::IsNeedExcuteC1(RunInfo info)
{
    // B超出范围则cube不执行
...
template <typename COMP>
__aicore__ inline void CompressorV2KernelPerf<COMP>::Process()
{
    // 所有batch的有效序列都为0时, 直接退出
    if (constInfo.batchSize == 0) {
        return;
...
#endif // COMPRESSOR_V2_KERNEL_PERF_H
