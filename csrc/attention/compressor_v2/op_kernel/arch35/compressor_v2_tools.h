...
template <typename COMP>
__aicore__ inline uint32_t CompressorV2Tools<COMP>::GetStartPos(uint32_t bIdx)
{
    if (isExistStartPos_) {
...
template <typename COMP>
__aicore__ inline uint32_t CompressorV2Tools<COMP>::GetTIdxByBatch(uint32_t bIdx)
{
    if constexpr (COMP::xLayout == X_LAYOUT::TH) {
...
public:
    __aicore__ inline CompressorSliceIterator(CompressorV2Tools<COMP> &tools)
        : tools_(tools)
    {}
...
template <typename COMP>
__aicore__ inline void CompressorV2Vec1SliceIterator<COMP>::SetCompressoredScCnt(uint32_t compressoredScCnt)
{
