...
#include "compressor_v2_tiling_arch35.h"
...
ge::graphStatus CompressorV2Tiling::SetBaseInfo()
{
    ...
    OP_LOGI(context_->opName, "[TILING] bSize:%u  tSize:%u cmpRatio:%u", baseParams_->batchSize, baseParams_->tokenSize,
            baseParams_->cmpRatio, baseParams_->stateCacheStrideDim0);
...
ge::graphStatus CompressorV2Tiling::SetInnerSplitInfo()
{
    if (context_->templateId == TemplateId::FULL_LOAD) {
        ...
        innerSplitParams_->dBaseSize = 128;
...
{
    if (ge::GRAPH_SUCCESS != CheckSingleParaCmpRatio() || ge::GRAPH_SUCCESS != CheckSingleParaCacheMode() ||
...
ge::graphStatus CompressorV2Tiling::CheckSingleParaCacheMode() const
{
    return ge::GRAPH_SUCCESS;
}
