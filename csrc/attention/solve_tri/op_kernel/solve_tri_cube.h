...
template <int MATRIX_SIZE, typename T>
__aicore__ inline void SolveTriCube<MATRIX_SIZE, T>::LoadToL0A_Offset(
    int32_t slot, int32_t offset, bool packedDiagonal)
{
    ...
    int32_t srcStep = (slot == SLOT_INPUT && packedDiagonal)
                          ? (NUM_FRACS + 1) * FRAC_LEN
                          : FRAC_LEN;
    for (int32_t i = 0; i < NUM_FRACS; ++i) {
...
template <int MATRIX_SIZE, typename T>
__aicore__ inline void SolveTriCube<MATRIX_SIZE, T>::LoadToL0B_Offset(
    int32_t slot, int32_t offset, bool packedDiagonal)
{
    LoadData2DParams loadParams;
    loadParams.startIndex = 0;
...
__aicore__ inline void SolveTriCube<MATRIX_SIZE, T>::MCH_InitXY()
{
    ...
    LoadToL0A_Offset(SLOT_INPUT, Y_BUF, true);
    LoadToL0B_Offset(SLOT_INPUT, Y_BUF, true);
    ...
    LoadToL0B_Offset(SLOT_INPUT, X_BUF, true);
    SetFlag<HardEvent::MTE1_M>(EVT_X);
    // Y: 写回
...
__aicore__ inline void SolveTriCube<MATRIX_SIZE, T>::LoadInputTile(int64_t gmOffset, int64_t validSize)
{
    ...
    // 使用预计算的参数模板，只修改变化的字段
    if (validSize < MATRIX_SIZE) {
        DataCopyParams partialParams;
        partialParams.blockCount = static_cast<uint16_t>(numDiagFracs);
        partialParams.blockLen = 1;
        partialParams.srcStride = static_cast<uint16_t>(rowStride_ / FRAC - 1);
        partialParams.dstStride = 0;
        for (int32_t i = 0; i < numDiagFracs; ++i) {
            int32_t remaining = static_cast<int32_t>(validSize) - i * FRAC;
            partialParams.blockCount = static_cast<uint16_t>(remaining < FRAC ? remaining : FRAC);
            int64_t srcOffset = i * (FRAC * rowStride_ + FRAC);
            int32_t dstOffset = SLOT_INPUT * L1_SLOT_ELEMS
                                + i * (NUM_FRACS + 1) * FRAC_LEN;
            DataCopy(l1_[dstOffset], inputGM_[gmOffset + srcOffset], partialParams);
        }
    } else {
...
__aicore__ inline void SolveTriCube<MATRIX_SIZE, T>::StoreFinalResult(int64_t gmOffset, int64_t validSize)
{
    ...
    NsSolveTri::L0CToGM(
        ...
        static_cast<uint32_t>(MATRIX_SIZE), // nTileActual
    ...
    for (int32_t iter = 0; iter < NUM_ITERS; iter++) {
        ...
        if constexpr (MATRIX_SIZE > FRAC) {
            if (iter == NUM_ITERS - 1) {
                ...
                fullLoadParams_.dValue = static_cast<uint32_t>(MATRIX_SIZE);
        ...
        if constexpr (MATRIX_SIZE > FRAC) {
            if (iter == NUM_ITERS - 1) {
                ...
                LoadToL0B_Offset(SLOT_INPUT, Y_BUF);  // L0B[1] ← M (full input)
                SetFlag<HardEvent::MTE1_M>(EVT_Y);
            }
        }
