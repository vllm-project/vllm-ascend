...
/*!
 * \file turbo_quant_sparse_attn_sharedkv_tiling.h
 * \brief
 */
#ifndef TURBO_QUANT_SPARSE_ATTN_SHAREDKV_TILING_H
#define TURBO_QUANT_SPARSE_ATTN_SHAREDKV_TILING_H
...
constexpr uint32_t ATTR_KV_QUANT_MODE_INDEX = 11;
constexpr uint32_t TQ4_SLOT_SIZE = 258;
...
// -----------算子TilingData定义---------------
BEGIN_TILING_DATA_DEF(TurboQuantSparseAttnSharedkvSwaParams)
...
TILING_DATA_FIELD_DEF(uint32_t, kvQuantMode)
END_TILING_DATA_DEF
REGISTER_TILING_DATA_CLASS(TurboQuantSparseAttnSharedkvSwaParamsOp, TurboQuantSparseAttnSharedkvSwaParams)

BEGIN_TILING_DATA_DEF(TurboQuantSparseAttnSharedkvCmpParams)
...
END_TILING_DATA_DEF
REGISTER_TILING_DATA_CLASS(TurboQuantSparseAttnSharedkvCmpParamsOp, TurboQuantSparseAttnSharedkvCmpParams)

BEGIN_TILING_DATA_DEF(TurboQuantSparseAttnSharedkvTilingData)
TILING_DATA_FIELD_DEF_STRUCT(TurboQuantSparseAttnSharedkvSwaParams, baseParams);
TILING_DATA_FIELD_DEF_STRUCT(TurboQuantSparseAttnSharedkvCmpParams, cmpParams);
END_TILING_DATA_DEF

REGISTER_TILING_DATA_CLASS(TurboQuantSparseAttnSharedkv, TurboQuantSparseAttnSharedkvTilingData)
...
std::string TQSASLayoutToSerialString(SASLayout layout);
...
class TQSASTilingCheck {
public:
    explicit TQSASTilingCheck(const SASTilingInfo &sasInfo)
        : sasInfo_(sasInfo) {};
    ~TQSASTilingCheck() = default;
...
class TQSASInfoParser {
public:
    explicit TQSASInfoParser(gert::TilingContext *context)
        : context_(context)
    {}
    ~TQSASInfoParser() = default;
...
// ---------------算子Tiling类---------------
class TurboQuantSparseAttnSharedkvTiling {
public:
    explicit TurboQuantSparseAttnSharedkvTiling(gert::TilingContext *context)
        : context_(context) {};
...
private:
    ...
    gert::TilingContext *context_ = nullptr;
    SASTemplateMode perfMode_ = SASTemplateMode::SWA_TEMPLATE_MODE;
    TurboQuantSparseAttnSharedkvTilingData tilingData_;
