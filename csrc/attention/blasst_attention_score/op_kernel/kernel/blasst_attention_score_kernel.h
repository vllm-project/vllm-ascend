...
/*!
* \file blasst_attention_score_kernel.h
* \brief
*/
#ifndef BLASST_ATTENTION_SCORE_KERNEL_H
#define BLASST_ATTENTION_SCORE_KERNEL_H

#include "../blasst_attention_score_common.h"
...
namespace SplitFuse {
    template <
        ...
        FaiKernel::inputLayout INPUT_LAYOUT = FaiKernel::inputLayout::TND,
    ...
    public:
        ...
        struct GlobalTensorBundle {
            ...
            AscendC::GlobalTensor<ElementUpdate>& gOUpdate;
            AscendC::GlobalTensor<uint8_t>& gSp;
            AscendC::GlobalTensor<int32_t>& gSparseStats;
            AscendC::GlobalTensor<int32_t>& gSparseStatsOut;
        };
        ...
        void operator()(FAIKernelParams const &params)
        {
            ...
            sparseLamda = fATilingData->sparseLamda;
            sparseStatsEnabled = (fATilingData->sparseStatsFlag == 1U);
            updateSize = fATilingData->UpdateSize;
            uint64_t Lsesize = 0;
            uint64_t Losize = 0;
            if constexpr (IS_FD) {
            ...
            AscendC::GlobalTensor<ElementUpdate> gOUpdate;
            gOUpdate.SetGlobalBuffer((__gm__ ElementUpdate *)(params.workSpace + Lsesize + Losize +
                mm1OutSize + smOnlineOutSize + mm2OutSize));
            AscendC::GlobalTensor<uint8_t> gSp;
            gSp.SetGlobalBuffer((__gm__ uint8_t *)(params.workSpace + Lsesize + Losize +
                mm1OutSize + smOnlineOutSize + mm2OutSize + updateSize));
            uint64_t spFlagSize = static_cast<uint64_t>(AscendC::GetBlockNum()) * 2 * gGmSpLen * (PRE_LAUNCH + 1U);
            AscendC::GlobalTensor<int32_t> gSparseStats;
            gSparseStats.SetGlobalBuffer((__gm__ int32_t *)(params.workSpace + Lsesize + Losize +
                mm1OutSize + smOnlineOutSize + mm2OutSize + updateSize + spFlagSize));
            AscendC::GlobalTensor<int32_t> gSparseStatsOut;
            gSparseStatsOut.SetGlobalBuffer((__gm__ int32_t *)(params.sparseStats));

            GlobalTensorBundle globalTensors{
                gQ, gK, gV, gMask, gBlockTable,
                gActualQseqlen, gActualKvseqlen,
                gO, gLse, gLseFD, gOFD,
                gS, gP, gOTmp, gOUpdate, gSp,
                gSparseStats, gSparseStatsOut
            };
            ...
            blockMmadPV.SetSpEnabled(sparseLamda > -50.0f);
#endif
            ...
            if constexpr (IS_FD) {
                ...
                for (uint32_t BIdx = startBIdx; BIdx <= endBIdx; BIdx++) {
                    ...
                    uint32_t curQSBlockTileTmp = GetQSBlockTile(kvSeqlenCur, qSeqlenCur, groupSize);
                    ...
                    for (int32_t n1Idx = stN1IdxNow; n1Idx <= enN1IdxNow; n1Idx++) {
                        ...
                        for (int32_t s1Idx = stS1IdxNow; s1Idx <= enS1IdxNow; s1Idx++) {
                            // Causal work bound: prefill tasks never iterate
                            // KV tiles past their own diagonal, so span ends
                            // and split detection compare against the causal
                            // tile count (mirrors the noSkipKvS math in
                            // runMainLoop). Decode's single qS block makes the
                            // two counts equal.
                            int64_t diffSTmp = static_cast<int64_t>(kvSeqlenCur) - qSeqlenCur;
                            diffSTmp = (diffSTmp < 0) ? 0 : diffSTmp;
                            int64_t noSkipKvSTmp = static_cast<int64_t>(kvSeqlenCur);
                            if (maskType != 0U) {
                                noSkipKvSTmp = static_cast<int64_t>(s1Idx + 1) * curQSBlockTileTmp + diffSTmp;
                                noSkipKvSTmp = AscendC::Std::min(static_cast<int64_t>(kvSeqlenCur), noSkipKvSTmp);
                            }
                            int32_t causalKSBlockNumTmp = static_cast<int32_t>(
                                NpuArch::Detail::Alignment::CeilDiv(
                                    static_cast<uint32_t>(noSkipKvSTmp), MAX_KV_STACK_LEN));
                            ...
                            int32_t enS2IdxNow = (BIdx == endBIdx && n1Idx == enN1IdxNow && s1Idx == enS1IdxNow) ?
                                endS2Idx : causalKSBlockNumTmp;

                            bool isSplitKV = (enS2IdxNow - stS2IdxNow) > 0 &&
                                (enS2IdxNow - stS2IdxNow) < causalKSBlockNumTmp;
                ...
                // Task->core schedule: by default a round-robin stride, which
                // leaves cores at ceil/floor(totalTaskNum/coreNum) tasks -- a
                // +12% makespan spread when task costs are near-uniform (GQA8
                // prefill packing). When the host packed a cost-balanced LPT
                // assignment (taskAssignFlag), coreIdx instead owns the
                // taskPerm slice [coreTaskStart, coreTaskEnd). The
                // taskIdx->(batch, n1, s1) decode below is shared by both.
                const bool useTaskAssign = (fATilingData->taskAssignFlag == 1U);
                const uint32_t iterBegin = useTaskAssign ? fATilingData->coreTaskStart[coreIdx] : 0;
                const uint32_t iterEnd = useTaskAssign ? fATilingData->coreTaskEnd[coreIdx] :
                    ((totalTaskNum > coreIdx) ?
                        (totalTaskNum - coreIdx + uint32_t(coreNum) - 1) / uint32_t(coreNum) : 0);
                for (uint32_t iterIdx = iterBegin; iterIdx < iterEnd; ++iterIdx) {
                    uint32_t taskIdx = useTaskAssign ? fATilingData->taskPerm[iterIdx] :
                        (coreIdx + iterIdx * uint32_t(coreNum));
                    ...
                    while (taskIdx >= curTotalTaskNumTmp && curBatchTmp < batch - 1) {
                        ...
                        uint32_t qNBlockNumPerGroupTmp = NpuArch::Detail::Alignment::CeilDiv(groupSize, curQNBlockTileTmp);
                        uint32_t curQNBlockNumTmp = qNBlockNumPerGroupTmp * kvHeads;
                        uint32_t curQSBlockTileTmp = GetQSBlockTile(kvSeqlenTmp, qSeqlenTmp, groupSize);
            ...
            if (sparseStatsEnabled) {
                gSparseStats[coreIdx * 16].SetValue(0, static_cast<int32_t>(blockMmadPV.sparseNum));
                gSparseStats[coreIdx * 16 + 1].SetValue(0, static_cast<int32_t>(blockMmadPV.countNum));
                AscendC::DataCacheCleanAndInvalid<int32_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                    AscendC::DcciDst::CACHELINE_OUT>(gSparseStats[coreIdx * 16]);
            }
#ifdef BLASST_SPDBG  // SPDBG: print final sparse counters for core0
            if (coreIdx == 0) {
            ...
            if (sparseStatsEnabled) {
                ...
                if (coreIdx == 0) {
                    ...
                    for (uint32_t i = 0; i < coreNum; ++i) {
                        AscendC::DataCacheCleanAndInvalid<int32_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                            AscendC::DcciDst::CACHELINE_OUT>(gSparseStats[i * 16]);
                        sparseSum += gSparseStats[i * 16].GetValue(0);
                        countSum += gSparseStats[i * 16 + 1].GetValue(0);
                    }
                    gSparseStatsOut.SetValue(0, sparseSum);
                    gSparseStatsOut.SetValue(1, countSum);
                    AscendC::DataCacheCleanAndInvalid<int32_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                        AscendC::DcciDst::CACHELINE_OUT>(gSparseStatsOut);
                }
#endif
            }
        }
        ...
        __aicore__ inline void runMainLoop(
            ...
            uint32_t curQSBlockTile = GetQSBlockTile(kvSeqlen, qSeqlen, groupSize);
            ...
            uint32_t stackSeqTilePad = MAX_KV_STACK_LEN;
                ...
                for (uint32_t kvSIdx = kvStart; kvSIdx < kvEnd + preKVNum; kvSIdx++) {
                    if (kvSIdx < kvEnd) {
                        ...
                        uint64_t gmOffsetSp = static_cast<uint64_t>(
                            coreIdx * 2 * gGmSpLen * (PRE_LAUNCH + 1U) +
                            curStackTileMod * gGmSpLen * 2 +
                            AscendC::GetSubBlockIdx() * gGmSpLen);
                        ...
                        if constexpr (MASK_TYPE == FaiKernel::MaskType::MASK_CAUSAL) {
                            ...
                            if (doTriUMask) {
                                ...
                                int32_t lastNoMaskStackId = static_cast<int32_t>(noMaskStackSeqNum - 1);
                                if constexpr (IS_FD) {
                                    lastNoMaskStackId = static_cast<int32_t>(noMaskStackSeqNum - 1) - static_cast<int32_t>(kvStart);
                                }
                        ...
                        epilogueOnlineSoftmax.StoreSpFlagResIntoHmb(gSp, gmOffsetSp);
                    ...
                    if (kvSIdx >= kvStart + preKVNum) {
                        ...
                        uint64_t curGmOffsetSp = static_cast<uint64_t>(coreIdx * 2 * gGmSpLen * (PRE_LAUNCH + 1U) + curStackTileMod * gGmSpLen * 2);
                        ...
                        LayoutLse layoutgmLseFD(qSBlockSize, qNBlockSize);
                        LayoutOTmp layoutgmLoFD(qSBlockSize, embedV * qNBlockSize);
                        if constexpr (IS_FD) {
                            isLastStack = nowkvSIdx + 1 >= kvEnd;
                            splitParams.isSplitkv = isSplitKV;
                            splitParams.gCombineLse = gLseFD[gmOffsetLseFD];
                            splitParams.gCombineo = gOFD[gmOffsetOFD];
                            splitParams.layoutgmLse = &layoutgmLseFD;
                            splitParams.layoutgmLo = &layoutgmLoFD;
                        }

                        Arch::CrossCoreWaitFlag(pvReady);
                        // BLASST_ABLATE_OMIDDLE: perf-only ablation, skips the
                        // middle-tile O merge (OTmp load + upcast + rescale) to
                        // attribute the occ0 prefill gap. Output is WRONG.
#ifdef BLASST_ABLATE_OMIDDLE
                        if (isFirstStack || isLastStack) {
#else
                        if (!(sp_flag && !isFirstStack && !isLastStack)) {
#endif
                            epilogueRescaleO(
                                gO[gmOffsetO],
                                gOTmp[gmOffsetOTmp],
    ...
    private:
        ...
        bool gSpTempArr[PRE_LAUNCH + 1U] = {0};
        uint16_t gSpRowLoopArr[PRE_LAUNCH + 1U] = {0};
        uint32_t gGmSpLen = 32;
        bool sparseStatsEnabled = false;
    };
}
#endif
