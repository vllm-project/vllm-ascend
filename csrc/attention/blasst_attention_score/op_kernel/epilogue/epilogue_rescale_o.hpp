...
public:
    ...
    void init(Arch::Resource<ArchTag> &resource)
    {
        ...
        // fp16 OTmp staging aliased into the SECOND half of each 32KB fp32 band.
        // The GM OTmp round-trip is fp16 (ElementInput = half): MTE2 lands the
        // chunk in the half band and an exact upcast produces the fp32 view
        // before the rescale math. Indexing rule: fp16 element e sits at byte
        // 16384 + 2e of the 32KB region (plain e on the half view), while the
        // fp32 dst byte is 4e. The cast streams forward, so a dst write at byte
        // 4e can only touch src bytes 16384+2e' with e' < e + 128 -- already
        // consumed -- never unread data. (Do NOT scale the half index by 2:
        // the tensor is already half-typed, and 16384+4e overflows the 16KB
        // half band into the adjacent GO accumulator for off > 0.)
        lohUbTensor = resource.ubBuf.template GetBufferByByte<half>(
            LO_UB_TENSOR_OFFSET + MAX_UB_O_ELEM_NUM * sizeof(half));
        gohUbTensor = resource.ubBuf.template GetBufferByByte<half>(
            GO_UB_TENSOR_OFFSET + MAX_UB_O_ELEM_NUM * sizeof(half));
    ...
    void SubCoreCompute(
        ...
        uint32_t curQNBlockTile, uint16_t spFlagArr = 0,
        const SplitKVParams& splitParams = SplitKVParams())
    {
        ...
        if (!isFirstStackTile) {
            if (sp_flag == 1 || sp_flag_row_offset >= curRowNum * embedRoundV) {
                AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(EVENT_ID3);
                AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID0);
            } else {
                AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(EVENT_ID3);
                // OTmp chunk lands as fp16 in the half band; the fp32 view is
                // produced by the upcast below, behind this load's MTE2_V wait.
                AscendC::DataCopy(
                    lohUbTensor[sp_flag_row_offset], gInput,
                    AscendC::DataCopyParams(1, (curRowNum * embedRoundV - sp_flag_row_offset) / HALF_ELENUM_PER_BLK, 0, 0));
                AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID0);
                AscendC::Duplicate<float>(loUbTensor, 0, sp_flag_row_offset);
                AscendC::PipeBarrier<PIPE_V>();
            }
        }
        ...
        if (!isFirstStackTile) {
            if (sp_flag != 1) {
                ...
                // Exact fp16->fp32 upcast of the OTmp chunk into the LO fp32
                // view (margin proof in init()). The element span is always a
                // multiple of embedRoundV (=128): sp_flag_row_offset is a
                // multiple of 16*128 and the load length likewise. Guarded by
                // the same condition as the GM load above: when the whole
                // prefix is sparse (offset >= span) no fp16 data landed and
                // the count would underflow.
                if (sp_flag_row_offset < curRowNum * embedRoundV) {
                    AscendC::Cast<float, half, false>(
                        loUbTensor[sp_flag_row_offset],
                        lohUbTensor[sp_flag_row_offset],
                        AscendC::RoundMode::CAST_NONE,
                        (uint64_t)0,
                        NpuArch::Detail::Alignment::CeilDiv(
                            curRowNum * embedRoundV - sp_flag_row_offset, FLOAT_VECTOR_SIZE),
                        AscendC::UnaryRepeatParams(1, 1, 8, 4));
                    AscendC::PipeBarrier<PIPE_V>();
                }
                if (spFlagArr != 0U) {
                    uint32_t localStart = rowOffsetCurLoop - inRowOffsetThisSubBlock;
                    uint32_t gEndA = (localStart + curRowNum + 15U) / 16U;
                    for (uint32_t g = localStart / 16U; g < gEndA; ++g) {
                        if (g >= 16U || (spFlagArr & (1u << g)) == 0U) {
                            continue;
                        }
                        uint32_t rLo = (g * 16U > localStart) ? (g * 16U - localStart) : 0U;
                        uint32_t rHi = (g * 16U + 16U < localStart + curRowNum)
                            ? (g * 16U + 16U - localStart) : curRowNum;
                        if (rLo >= curRowNum) {
                            continue;
                        }
                        AscendC::Duplicate<float>(loUbTensor[rLo * embedRoundV], 0, (rHi - rLo) * embedRoundV);
                    }
                    AscendC::PipeBarrier<PIPE_V>();
                }
                // *** go = lo + go
            ...
            // *** go = lo
            // First stack tile: the fp16 OTmp chunk seeds the O accumulator.
            // Land it in the half band, then upcast into the fp32 GO view
            // (margin proof in init()) before the divide/copy-out consumers.
            AscendC::DataCopy(
                gohUbTensor, gInput,
                AscendC::DataCopyParams(1, curRowNum * embedRoundV / HALF_ELENUM_PER_BLK, 0, 0));
            AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID0);
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID0);
            AscendC::Cast<float, half, false>(
                goUbTensor32,
        ...
        if (isLastStackTile) {
            ...
            if (splitParams.isSplitkv) {
                CopyOToGmFp32(
                    splitParams.gCombineo, proTokenIdx, proTokenNum, epiTokenNum, integralHeadNum,
                    qSThisSubBlock, embedV, embedRoundV, oHiddenSize, oHiddenSize_gmlo);
            } else {
                CopyOToGm(
            ...
            if constexpr (LSE_MODE_ == LseMode::OUT_ONLY) {
                if (isLastRowLoop) {
                    ...
                    if (qNThisSubBlock == 0U) {
                        ...
                        // Per-head lse store: head qNIdx's tv block starts at
                        // qNIdx * qSThisSubBlock * 8 floats (Brcb broadcast
                        // layout, head-major rows). The local qSBlockSize is
                        // layoutOutput.shape(0) == qSeqlen, NOT the tile's q
                        // rows -- indexing tv with it over-reads the TV region.
                        for (uint32_t qNIdx = 0; qNIdx < qNThisSubBlock; qNIdx++) {
                            AscendC::DataCopyPad(
                                gLse[qNIdx],
                                tvUbTensor[qNIdx * qSThisSubBlock * FLOAT_BLOCK_SIZE],
                                AscendC::DataCopyExtParams(
                                    qSThisSubBlock, sizeof(float), 0, (qHeads - 1) * sizeof(float), 0));
                        }
                    }
                ...
                if (splitParams.isSplitkv) {
                    if (isLastRowLoop) {
                        ...
                        if (qNThisSubBlock == 0U) {
                            ...
                            // Same per-head tv indexing as the OUT_ONLY store
                            // above: qSThisSubBlock (tile q rows), never the
                            // local qSBlockSize (= qSeqlen from the GM layout).
                            for (uint32_t qNIdx = 0; qNIdx < qNThisSubBlock; qNIdx++) {
                                AscendC::DataCopyPad(
                                    splitParams.gCombineLse[qNIdx],
                                    tvUbTensor[qNIdx * qSThisSubBlock * FLOAT_BLOCK_SIZE],
                                    AscendC::DataCopyExtParams(
                                        qSThisSubBlock, sizeof(float), 0, (qHeads_gmlse - 1) * sizeof(float), 0));
                            }
                        }
                        AscendC::SetFlag<AscendC::HardEvent::MTE3_V>(EVENT_ID4);
                    }
        ...
        const SplitKVParams& splitParams = SplitKVParams())
    {
        ...
        if (inRowActualThisSubBlock > 0U) {
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                ...
                if (!isFirstStackTile && !isLastStackTile) {
                    uint32_t localStart = rowOffsetCurLoop - inRowOffsetThisSubBlock;
                    bool chunkAllSparse = true;
                    uint32_t gEnd = (localStart + rowActualCurLoop + 15U) / 16U;
                    for (uint32_t g = localStart / 16U; g < gEnd; ++g) {
                        if (g >= 16U || (sp_flag_arr & (1u << g)) == 0u) {
                            chunkAllSparse = false;
                            break;
                        }
                    }
                    if (chunkAllSparse) {
                ...
                if (splitParams.isSplitkv) {
                    int64_t gmloffset =
                        static_cast<int64_t>(rowLoopIdx * rowNumTile / qSThisSubBlock * embedV) +
                        gmlooutOffsetSubBlock;
                    blockParams.gCombineo = splitParams.gCombineo[gmloffset];
                }
                // compute sparse row offset from sp_flag_arr bitmap
                uint32_t sp_flag_row_offset = 0;
                uint16_t x = (~sp_flag_arr) & 0x0F;
                if (x != 0) {
                    int pos = 0;
                    while ((x & 1) == 0) {
                        x >>= 1;
...
private:
    ...
    AscendC::LocalTensor<half> lohUbTensor;
    AscendC::LocalTensor<half> gohUbTensor;
