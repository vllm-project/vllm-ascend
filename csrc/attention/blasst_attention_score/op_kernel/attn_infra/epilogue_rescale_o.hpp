        ...
        const SplitKVParams& splitParams = SplitKVParams())
    {
        ...
        if (!isFirstStackTile) {
            if (sp_flag == 1 || sp_flag_row_offset >= curRowNum * embedRoundV) {
                ...
                AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(EVENT_ID3);
                AscendC::DataCopy(
                    loUbTensor[sp_flag_row_offset], gInput,
                    AscendC::DataCopyParams(1, (curRowNum * embedRoundV - sp_flag_row_offset) / FLOAT_BLOCK_SIZE, 0, 0));
                AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(EVENT_ID0);
        ...
        if (inRowActualThisSubBlock > 0U) {
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                ...
                if (x != 0) {
                    ...
                    // NOTE(2026-09-14 verification): this 128 is NOT
                    // embedRoundV. It was once changed to pos*16*embedRoundV
                    // and the three-way golden comparison got worse
                    // (2.45e-2 -> 4.80e-2), so it was reverted. The exact
                    // semantics of this constant (suspected PV N-tile width)
                    // needs kernel-team review; do not change it casually.
