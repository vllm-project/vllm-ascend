...
public:
    ...
    // Dense mode (sparseLamda <= -50): skip never fires, so the cube side
    // skips the per-tile DCCI + scalar gSp reads entirely (the vector side
    // skips the matching flag store on the same condition).
    bool spEnabled = true;

    __aicore__ inline
    void SetSpEnabled(bool enabled)
    {
        spEnabled = enabled;
    }
        ...
        uint32_t blockStackNum, Arch::CrossCoreFlag softmaxFlag, Arch::CrossCoreFlag pvReady)
    {
        ...
        // Dense mode skips the DCCI + two scalar gSp GM reads on the cube
        // critical path (always-run PV); the vector side skips the matching
        // flag store on the same sparseLamda condition.
        bool sp_flag_res = false;
        if (spEnabled) {
            AscendC::DataCacheCleanAndInvalid<uint8_t, AscendC::CacheLine::SINGLE_CACHE_LINE, AscendC::DcciDst::CACHELINE_OUT>(gSp);
            uint8_t sp_flag1 = gSp.GetValue(0);
            uint8_t sp_flag2 = gSp.GetValue(32);
            sp_flag_res = sp_flag1 == 1 && sp_flag2 == 1;
        }
        countNum++;
        if (sp_flag_res) {
            sparseNum++;
