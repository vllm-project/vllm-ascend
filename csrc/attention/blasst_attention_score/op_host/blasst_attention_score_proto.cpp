...
/*!
 * \file blasst_attention_score_proto.cpp
 * \brief
 */
...
ge::graphStatus InferShapeVllmBlasstAttentionScore(gert::InferShapeContext *context)
{
    OPS_ERR_IF(context == nullptr, OPS_LOG_E("VllmBlasstAttentionScore", "InferShapeContext is nullptr"),
    ...
    *attentionOutShape = *queryShape;
    ...
    if (softmaxLseShape != nullptr) {
        OPS_ERR_IF(queryShape->GetDimNum() != 3,
                   OPS_LOG_E("VllmBlasstAttentionScore", "query must be 3D for TND layout"),
                   return ge::GRAPH_FAILED);
...
ge::graphStatus InferDataTypeVllmBlasstAttentionScore(gert::InferDataTypeContext *context)
{
    OPS_ERR_IF(context == nullptr, OPS_LOG_E("VllmBlasstAttentionScore", "InferDataTypeContext is nullptr"),
               return ge::GRAPH_FAILED);
...
IMPL_OP(VllmBlasstAttentionScore)
    .InferShape(InferShapeVllmBlasstAttentionScore)
    .InferDataType(InferDataTypeVllmBlasstAttentionScore);
} // namespace ops
