...
#ifndef FLASH_ATTN_C8_VEC128_PACKED_P_H
#define FLASH_ATTN_C8_VEC128_PACKED_P_H

#include "../../a5_mla_common/op_kernel/arch35/vf/vf_basic_block_utils.h"
...
// the live64 rows, instead of two per-row FP32 multiplies and divides.
__simd_vf__ inline void FlashAttnC8Update128PackedPVf(
    ...
    const float scale, const float kvScale)
{
    ...
    // pScale has128 FP32 slots but this AIV owns at most64 rows. Preserve
    // the first64 for PV, and use the otherwise unused tail for448/pScale.
    ...
    StoreAlign<float, Reg::StoreDist::DIST_NORM_B32>(pScale + 64, inversePScale, rows);
    LocalMemBar<MemType::VEC_STORE, MemType::VEC_LOAD>();
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        LoadAlign<float, Reg::LoadDist::DIST_BRC_B32>(inversePScale, pScale + 64 + i);
        ...
        StoreAlign<fp8_e4m3fn_t, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
            dstWrite, packed, 65, 1, bytes128);
    }
    StoreUnAlignPost<float, Reg::PostLiteral::POST_MODE_UPDATE>(sumWrite, sumStore, 0);
}

__aicore__ inline void FlashAttnC8Update128PackedP(
...
{
    // Update tiles only, full128 columns, M128 split over two AIVs (m<=64),
...
#endif // FLASH_ATTN_C8_VEC128_PACKED_P_H
