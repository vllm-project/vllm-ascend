...
public:
    ...
    __aicore__ inline void CalGCumExpFp32(GlobalTensor<float> gCum)
    {
        ...
        uint32_t alignedCount = Ceil(curChunkSize_, BLOCK_FLOAT_NUM) * BLOCK_FLOAT_NUM;
        auto gCumLocal = inQueue_.AllocTensor<float>();
        DataCopyParams copyParams{1, static_cast<uint16_t>(curChunkSize_ * sizeof(float)), 0, 0};
        DataCopyPadParams padParams{true, 0, static_cast<uint8_t>(alignedCount - curChunkSize_), 0};
        DataCopyPad(gCumLocal, gCum, copyParams, padParams);
        inQueue_.EnQue<float>(gCumLocal);
        gCumLocal = inQueue_.DeQue<float>();
        Exp<float, 0, true>(gCumLocal, gCumLocal, alignedCount);
        SetFlag<HardEvent::V_S>(V_S_EVENT);
        WaitFlag<HardEvent::V_S>(V_S_EVENT);
        float tmpFloat = gCumLocal.GetValue(curChunkSize_ - 1);
        inQueue_.FreeTensor(gCumLocal);
