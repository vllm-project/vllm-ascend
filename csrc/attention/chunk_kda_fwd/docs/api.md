...
当前 Kimi K3 接入的入参规格仅支持 `Kdim=Vdim=128`、`chunk_size=64`。
底层保留的其他泛化模板不属于当前 Kimi K3 接入规格和验证范围。
...
Kimi K3 直调样例仅覆盖 dense BNSD、`Kdim=Vdim=128`、`chunk_size=64`，并保留“调用方传入已累计 gk”的低层测试接口；
