...
#ifndef __fused_sigmoid_gating_delta_rule_update_KERNEL_H_
#define __fused_sigmoid_gating_delta_rule_update_KERNEL_H_
...
class RGDR {
public:
    __aicore__ inline RGDR(const FusedSigmoidGatingDeltaRuleUpdateTilingData *tilingData)
    {
        B_ = tilingData->b;
        T_ = tilingData->t;
        NK_ = tilingData->nk;
        ...
        softplusThreshold_ = tilingData->softplusThreshold;
    ...
    __aicore__ inline void InitLocalBuffers()
    {
        ...
        pipe_->InitBuffer(aInQueue_, BUFFER_NUM, MAX_MTP * NV_ * sizeof(inType));
        pipe_->InitBuffer(bInQueue_, BUFFER_NUM, MAX_MTP * NV_ * sizeof(inType));
        pipe_->InitBuffer(gamaInQueue_, BUFFER_NUM, MAX_MTP * NV_ * sizeof(float));
        pipe_->InitBuffer(aLogInQueue_, BUFFER_NUM, Ceil(NV_, FP32_NUM_PER_BLOCK) * FP32_NUM_PER_BLOCK * sizeof(float));
        pipe_->InitBuffer(dtBiasInQueue_, BUFFER_NUM, Ceil(NV_, FP32_NUM_PER_BLOCK) * FP32_NUM_PER_BLOCK * sizeof(float));
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint64_t batch_i = 0; batch_i < B_; batch_i++) {
            ...
            if (copyFlag != 0) {
                ...
                aLogInQueue_.FreeTensor(aLogInUb);
                dtBiasInQueue_.FreeTensor(dtBiasInUb);
            }
        }
    }
...
private:
    ...
    __aicore__ inline void CopyInGamaBeta(int32_t seq0, int32_t seq1)
    {
        ...
        DataCopyPad(aLogLocal, aLogGm_[0], headParamInParams, padParams);
        DataCopyPad(dtBiasLocal, dtBiasGm_[0], headParamInParams, padParams);
        aLogInQueue_.EnQue<float>(aLogLocal);
        dtBiasInQueue_.EnQue<float>(dtBiasLocal);
        ...
        DataCopyPad(aLocal, aGm_[seq0 * NV_], gateInParams, padParams);
        DataCopyPad(bLocal, bGm_[seq0 * NV_], gateInParams, padParams);
        aInQueue_.EnQue<inType>(aLocal);
        bInQueue_.EnQue<inType>(bLocal);
...
private:
    ...
    TQue<QuePosition::VECIN, 1> bInQueue_;
    TQue<QuePosition::VECIN, 1> gamaInQueue_;
    TQue<QuePosition::VECIN, 1> gamaKInQueue_;
    TQue<QuePosition::VECIN, 1> aLogInQueue_;
    TQue<QuePosition::VECIN, 1> dtBiasInQueue_;
    ...
    LocalTensor<float> aLogInUb;
    LocalTensor<float> dtBiasInUb;
