 ...
 * \brief Pruned tiling-key combinations.
 *   arch35: NO_QUANT + MXFP8(wq=3, cache=PA_BSND, kvq/qq in {0,1} matched)
 *   arch22: NO_QUANT + c8 INT8 (partial/full QUANT_MODE 1~4)
 */
...
ASCENDC_TPL_ARGS_DECL(
    ...
    // bit:6-11 QUANT_MODE（裁剪后保留）：
    // 0-非量化
    // 1-半量化(c8) 2-半量化+KV perchannel(c8)
    // 3-全量化INT8 4-全量化INT8+KV pertensor
    // 7-MXFP8+KV非量化 8-MXFP8+KV pertensor
    ASCENDC_TPL_UINT_DECL(QUANT_MODE, ASCENDC_TPL_4_BW, ASCENDC_TPL_UI_LIST, 0, 1, 2, 3, 4, 7, 8),
...
// __CCE_AICORE__==310 为 arch35；未定义时走本分支（host / arch22）
#if (!defined(__CCE_AICORE__) || (__CCE_AICORE__ != 310)) && \
    (MLA_PROLOG_VERSION == -1 || MLA_PROLOG_VERSION >= 1)
// 半量化 kv 非量化
...
#if (!defined(__CCE_AICORE__) || (__CCE_AICORE__ != 310)) && \
    (MLA_PROLOG_VERSION == -1 || MLA_PROLOG_VERSION >= 2)
// 全量化 kv 非量化
#if ORIG_DTYPE_TOKEN_X == -1 || ORIG_DTYPE_WEIGHT_UQ_QR == -1 || ORIG_DTYPE_KV_CACHE == -1 || \
...
// -------------------------- arch35：MXFP8（wq=3），仅 PA_BSND，QUANT_MODE=7/8 --------------------------
#if (defined(__CCE_AICORE__) && (__CCE_AICORE__ == 310)) && \
    (MLA_PROLOG_VERSION == -1 || MLA_PROLOG_VERSION == 3)
// MXFP8 全量化 + KV 非量化（query_quant_mode=0, kv_cache_quant_mode=0）
#if ORIG_DTYPE_TOKEN_X == -1 || ORIG_DTYPE_WEIGHT_UQ_QR == -1 || ORIG_DTYPE_KV_CACHE == -1 || \
...
// MXFP8 全量化 + KV pertensor（query_quant_mode=1, kv_cache_quant_mode=1）
#if ORIG_DTYPE_TOKEN_X == -1 || ORIG_DTYPE_WEIGHT_UQ_QR == -1 || ORIG_DTYPE_KV_CACHE == -1 || \
