...
public:
    ...
    __aicore__ inline void Init(GM_ADDR rowPtr, GM_ADDR scratch, const K2qCsrTilingData *tiling, TPipe *pipe)
    {
        ...
        rowsPer_ = rowsPer;
        pipe_->InitBuffer(rowPtrBuf_, K2qCsrCommon::Align32((bins_ > 0 ? bins_ : 1) * sizeof(int32_t)));
        pipe_->InitBuffer(tileSegBuf_, K2qCsrCommon::Align32(rowsPer * sizeof(int32_t)));
        pipe_->InitBuffer(absSegBuf_, K2qCsrCommon::Align32(rowsPer * sizeof(int32_t)));
        pipe_->InitBuffer(runBuf_, K2qCsrCommon::Align32(rowsPer * sizeof(int32_t)));
    }
    ...
    __aicore__ inline void Process()
    {
        ...
        LocalTensor<int32_t> rp = rowPtrBuf_.Get<int32_t>();
        ...
        for (int64_t hid = 0; hid < H_; ++hid) {
            K2qCsrCommon::CopyInInt32(rp, rowPtrGm_[hid * bins_], bins_);
            for (int64_t i = 0; i < nRow; ++i) {
                run.SetValue(i, rp.GetValue(r0 + i));
            }
            PipeBarrier<PIPE_ALL>();
            for (int64_t g = 0; g < G_; ++g) {
                int64_t off = (g * H_ + hid) * totalRows_;
                for (int64_t i = 0; i < nRow; ++i) {
                    absSeg.SetValue(i, run.GetValue(i));
                }
                PipeBarrier<PIPE_ALL>();
                ...
                if (g + 1 < G_) {
                    ...
                    for (int64_t i = 0; i < nRow; ++i) {
                        run.SetValue(i, run.GetValue(i) + tileSeg.GetValue(i));
                    }
                    PipeBarrier<PIPE_ALL>();
                }
            }
...
private:
    ...
    GlobalTensor<int32_t> rowPtrGm_;
    int64_t H_, totalRows_, bins_, G_, usedCores_, rowsPer_;
    TBuf<TPosition::VECCALC> rowPtrBuf_;
