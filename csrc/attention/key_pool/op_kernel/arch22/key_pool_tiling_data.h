...
/*!
 * \file KEY_POOL_tiling_datay.h
 * \brief
 */

#ifndef KEY_POOL_TILING_DATA_H
#define KEY_POOL_TILING_DATA_H
#include <cstdint>
...
struct KeyPoolBaseParams {
    ...
    uint32_t mLoopNum = 0;
    float normEps = 1e-6f;
    KeyPoolSplitCoreParams splitCoreParam[CMP_MAX_AIC_CORE_NUM];
};

struct KeyPoolPageAttentionParams {
    uint32_t blockNum = 0;
    uint32_t blockSize = 1;
    uint32_t maxBlockNumPerBatch = 1;
};

struct KeyPoolInnerSplitParams {
    uint32_t mBaseSize;
    uint32_t dBaseSize;
};

struct KeyPoolWorkspaceParams {
...
struct KeyPoolTilingData {
    KeyPoolBaseParams baseParams;
    KeyPoolPageAttentionParams pageAttentionParams;
    KeyPoolInnerSplitParams innerSplitParams;
    KeyPoolWorkspaceParams workspaceParams;
};
} // namespace optiling

#endif // KEY_POOL_TILING_DATA_H
