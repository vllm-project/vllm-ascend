...
#ifndef TILING_TEMPLATES_REGISTRY_H
#define TILING_TEMPLATES_REGISTRY_H
...
template <typename T>
inline std::unique_ptr<TilingBaseClass> TILING_CLASS(gert::TilingContext* context)
{
...
public:
    ...
    template <typename T>
    inline void AddTiling(int32_t priority)
    {
        OP_CHECK_IF(
...
public:
    ...
    static TilingRegistryNew& GetInstance()
    {
        static TilingRegistryNew registryImpl;
        return registryImpl;
    }
#endif

    std::shared_ptr<TilingCases> RegisterOp(const std::string& op_type, const int32_t soc_version)
    {
    ...
    ge::graphStatus DoTilingImpl(gert::TilingContext* context)
    {
        ...
        if (platformInfoPtr == nullptr) {
            ...
            OP_CHECK_IF(
                compileInfoPtr == nullptr, OP_LOGE(opType, "compileInfoPtr is null."), return ge::GRAPH_FAILED);
            ...
            if (soc_version == (int32_t)platform_ascendc::SocVersion::RESERVED_VERSION) {
                OP_LOGE(opType, "Do op tiling failed, cannot find soc version.");
                return ge::GRAPH_FAILED;
            }
        }
        auto tilingTemplateRegistryMap = GetTilingTemplates(opType, soc_version);
    ...
    const std::map<int32_t, TilingClassCase>& GetTilingTemplates(const std::string& op_type, const int32_t soc_version)
    {
        auto soc_iter = registry_map_.find(soc_version);
        OP_CHECK_IF(
...
public:
    ...
    template <typename T>
    inline RegisterNew& tiling(int32_t priority, int32_t soc_version)
    {
    ...
    template <typename T>
    inline RegisterNew& tiling(int32_t priority, const std::vector<int32_t>& soc_versions)
    {
...
public:
    ...
    static TilingRegistry& GetInstance()
    {
        static TilingRegistry registryImpl;
        return registryImpl;
    }
#endif
    ...
    {
        const char* opType = context->GetNodeType();
        auto tilingTemplateRegistryMap = GetTilingTemplates(opType);
        for (auto it = tilingTemplateRegistryMap.begin(); it != tilingTemplateRegistryMap.end(); ++it) {
            auto tilingTemplate = it->second(context);
            if (tilingTemplate != nullptr) {
                ge::graphStatus status = tilingTemplate->DoTiling();
...
public:
    ...
    template <typename T>
    inline Register& tiling(int32_t priority)
    {
...
#define REGISTER_TILING_TEMPLATE_WITH_SOCVERSION(op_type, class_name, soc_versions, priority)    \
    static AiInfraOps::Transformer::OpTiling::RegisterNew VAR_UNUSED##op_type##class_name##priority_register = \
        AiInfraOps::Transformer::OpTiling::RegisterNew(#op_type).tiling<class_name>(priority, soc_versions)
...
#define REGISTER_TILING_TEMPLATE(op_type, class_name, priority)                                \
    static AiInfraOps::Transformer::OpTiling::Register VAR_UNUSED##op_type_##class_name##priority_register = \
        AiInfraOps::Transformer::OpTiling::Register(op_type).tiling<class_name>(priority)
...
#define REGISTER_TILING_TEMPLATE_NEW(op_type, class_name, soc_version, priority)                 \
    static AiInfraOps::Transformer::OpTiling::RegisterNew VAR_UNUSED##op_type##class_name##priority_register = \
        AiInfraOps::Transformer::OpTiling::RegisterNew(#op_type).tiling<class_name>(priority, soc_version)

// op_type: 算子名称， class_name: 注册的 tiling 类,
// priority: tiling 类的优先级, 越小表示优先级越高, 即被选中的概率越大
// 取代 REGISTER_TILING_TEMPLATE , 传入的op_type如果是字符串常量，需要去掉引号
#define REGISTER_OPS_TILING_TEMPLATE(op_type, class_name, priority)                       \
    static AiInfraOps::Transformer::OpTiling::Register                                                  \
