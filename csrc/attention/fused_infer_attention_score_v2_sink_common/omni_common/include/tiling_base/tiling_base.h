...
protected:
    ...
    static uint32_t CalcTschBlockDim(const uint32_t sliceNum, const uint32_t aicCoreNum, const uint32_t aivCoreNum)
    {
        uint32_t ration;
    ...
    template <typename T>
    [[nodiscard]] inline std::string GetShapeDebugStr(const T& shape) const
    {
