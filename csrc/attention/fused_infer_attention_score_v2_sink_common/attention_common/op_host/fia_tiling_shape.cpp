...
static const std::map<FiaLayout, std::vector<FiaAxis>> FIA_LAYOUT_AXIS_MAP = {
    ...
    {FiaLayout::BnBsND, {FiaAxis::Bn, FiaAxis::Bs, FiaAxis::N, FiaAxis::D}},
...
static ge::graphStatus GetLayoutAxes(std::vector<FiaAxis> &layoutAxes,
...
ge::graphStatus FiaTilingShape::CheckHasAxis(const FiaAxis &axis, const std::string &funcName) const
{
    if (shape_.GetDimNum() == 0) {
        OPS_LOG_E(opName_, "[%s] the dim number of %s is 0.", funcName.c_str(), name_.c_str());
        return ge::GRAPH_FAILED;
    }
    ...
    if ((axis == FiaAxis::D)) {
        if (HasD()) {
            ...
            OPS_LOG_E(opName_,
                      "[%s] %s's layout is %s, do not have D and H.",
                      funcName.c_str(),
                      name_.c_str(),
                      LayoutToSerialString(layout_).c_str());
            return ge::GRAPH_FAILED;
            ...
            OPS_LOG_E(
                opName_, "[%s] %s's N is not specified, cannot calculate D by H.", funcName.c_str(), name_.c_str());
            return ge::GRAPH_FAILED;
        } else if (N_ == 0) {
            OPS_LOG_E(opName_, "[%s] %s's N is 0.", funcName.c_str(), name_.c_str());
            return ge::GRAPH_FAILED;
        } else if (GetH() % N_ != 0) {
            OPS_LOG_E(opName_,
                      "[%s] %s's H(%ld) should be an integer multiple of N(%ld).",
                      funcName.c_str(),
                      name_.c_str(),
                      GetH(),
                      N_);
    ...
    OPS_LOG_E(opName_,
              "[%s] %s's layout is %s, %s is not exists.",
              funcName.c_str(),
              name_.c_str(),
              LayoutToSerialString(layout_).c_str(),
                                                               ...
                                                               const std::string &funcName) const
{
    if (layout_ == FiaLayout::BNS1S2) {
        ...
        OPS_LOG_E(opName_, "[%s] layout %s is unsupported", funcName.c_str(), LayoutToSerialString(layout_).c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
                                                        ...
                                                        const std::string &funcName) const
{
    ...
    } else if (layout_ == FiaLayout::BnBsND) {
                                                      ...
                                                      const std::string &funcName) const
{
    ...
    if (it == compareFuncMap_.end()) {
        OPS_LOG_E(opName_,
                  "[%s] compare type %s is unsupported.",
                  funcName.c_str(),
    ...
    for (size_t i = 0; i < shape_.GetDimNum(); i++) {
        ...
        if (!compareFunc(shape_.GetDim(i), shapeExpected.GetDim(i))) {
            if (param.compareTypeMap.empty()) {
                OPS_LOG_E(opName_,
                          "[%s] %s layout is %s, shape %s should be equal to %s.",
                          funcName.c_str(),
                          name_.c_str(),
                          LayoutToSerialString(layout_).c_str(),
                          GetShapeStr(shape_).c_str(),
                ...
                OPS_LOG_E(
                    ...
                    AxisToSerialString(axis).c_str(),
                    shape_.GetDim(i),
                    CompareTypeToSerialSymbolString(compareType).c_str(),
                    shapeExpected.GetDim(i));
            }
            return ge::GRAPH_FAILED;
        }
    }
