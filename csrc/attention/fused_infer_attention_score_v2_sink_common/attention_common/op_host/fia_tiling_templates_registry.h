...
#ifndef FIA_TILING_TEMPLATES_REGISTRY_H
#define FIA_TILING_TEMPLATES_REGISTRY_H
...
public:
    ...
    {
        OPS_CHECK(cases_.find(priority) != cases_.end(),
...
public:
                                            ...
                                            int32_t soc_version)
    {
        ...
        OPS_CHECK((registry_map_[soc_version][op_type] == nullptr),
    ...
    ge::graphStatus DoTilingImpl(gert::TilingContext *context, TilingInfo *tilingInfo)
    {
        ...
        OPS_LOG_I(context, "soc version is %d", soc_version);
        ...
        OPS_LOG_I(context, "op_type: %s", op_type);
        for (auto it = tilingTemplateRegistryMap.begin(); it != tilingTemplateRegistryMap.end(); ++it) {
            ...
            if (tilingTemplate != nullptr) {
                ...
                if (status != ge::GRAPH_PARAM_INVALID) {
                    OPS_LOG_I(context, "Do general op tiling success priority=%d", it->first);
                    return status;
                }
                OPS_LOG_I(context, "Ignore general op tiling priority=%d", it->first);
            }
        }
                                                                    ...
                                                                    int32_t soc_version)
    {
        ...
        OPS_CHECK(op_iter == soc_iter->second.end(),
