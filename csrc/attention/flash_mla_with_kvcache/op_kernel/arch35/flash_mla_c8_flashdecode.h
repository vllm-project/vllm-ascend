...
/*!
 * \file flash_mla_c8_flashdecode.h
 * \brief
 */
...
namespace BaseApi {
template <typename INPUT_T, typename T, typename OUTPUT_T, FlashMlaC8Layout layout = FlashMlaC8Layout::None,
          FlashMlaC8Layout outLayout = FlashMlaC8Layout::None, S1TemplateType s1TemplateType = S1TemplateType::Aligned128,
...
public:
    ...
    __aicore__ inline void InitGlobalTensor(GlobalTensor<float> lseMaxFdGm, GlobalTensor<float> lseSumFdGm,
                                            ...
                                            GlobalTensor<float> attentionWireGm,
                                            GlobalTensor<uint32_t> actualSeqLengthsGmQ,
                                            GlobalTensor<uint32_t> actualSeqLengthsGm, __gm__ uint8_t *key)
    {
        this->lseMaxFdGm = lseMaxFdGm;
        this->lseSumFdGm = lseSumFdGm;
        this->accumOutGm = accumOutGm;
        this->attentionOutGm = attentionOutGm;
        this->attentionWireGm = attentionWireGm;
...
public:
    ...
    __aicore__ inline void CopySoftmaxLseOutput(uint32_t actualGSplitSize, uint32_t startRow)
    {
        if (constInfo.isSoftmaxLseEnable) {
            ...
            if constexpr (outLayout == FlashMlaC8Layout::LAYOUT_NTD_DCP) {
                // This is the final combined LSE, never a split workspace LSE.
                CopyDcpLse(attentionWireGm, maxLseUb, taskInfo.n2Idx, mOffset, actualGSplitSize,
                           qActSeqLensParser.GetTBase(taskInfo.bIdx), constInfo);
            }
            if constexpr (layout == FlashMlaC8Layout::LAYOUT_TND) {
                uint32_t prefixBS1 = qActSeqLensParser.GetTBase(taskInfo.bIdx);
                uint64_t bN2Offset =
                    taskInfo.n2Idx * constInfo.realGSize * constInfo.t1Size;
                DataCopySoftmaxLseTNDtoNTArch35<T, ConstInfoX>(softmaxLseGm, maxLseUb, bN2Offset, mOffset, actualGSplitSize,
                                                           prefixBS1, constInfo);
            ...
            if constexpr (outLayout == FlashMlaC8Layout::LAYOUT_NTD_DCP) {
                // Protect the shared LSE UB until both destinations have read it.
