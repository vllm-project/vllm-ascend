...
{
    // This imported route must receive real view descriptors. Fail closed on
    // older opbase paths: treating a padded view as dense corrupts live pages.
    OP_CHECK_IF(!context_->InputIsView(K_CACHE_INDEX),
                OP_LOGE(opName_, "A5 Flash MLA requires opbase cache view/stride metadata."),
                return ge::GRAPH_FAILED);
    ...
    if (layoutKV_ == FlashMlaWithKvcacheLayout::PA_BNBD) {
        ...
        OP_CHECK_IF(shape.GetDimNum() != 4 || keyStrides_->GetDimNum() != 4,
                    OP_LOGE(opName_, "Packed cache must have four dimensions."), return ge::GRAPH_FAILED);
        // A singleton head alias is normalized by the adapter, without copying.
        OP_CHECK_IF(shape.GetDim(1) != 1 || shape.GetDim(3) != 576 ||
                    keyStrides_->GetStride(3) != 1 || keyStrides_->GetStride(2) != 576 ||
                    keyStrides_->GetStride(1) != shape.GetDim(2) * int64_t{576} ||
                    keyStrides_->GetStride(0) < shape.GetDim(2) * int64_t{576},
                    OP_LOGE(opName_, "Packed cache requires strides (page, S*576, 576, 1)."),
                    return ge::GRAPH_FAILED);
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus FlashMlaWithKvcacheInfoParser::GetInAndOutLayout()
{
    ...
    const bool tndToNtd = (layoutQ_ == FlashMlaWithKvcacheLayout::TND && layoutOut_ == FlashMlaWithKvcacheLayout::NTD);
    if (layoutOut_ != layoutQ_ && !tndToNtd) {
        std::string reason =
            "The value of layout_out must equal layout_q, or be NTD when layout_q is TND (TND→NTD transpose)";
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "layout_out", opParamInfo_.layoutOut, reason.c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
