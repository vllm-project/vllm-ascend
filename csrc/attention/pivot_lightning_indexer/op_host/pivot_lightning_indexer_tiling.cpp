...
/*!
 * \file pivot_lightning_indexer_tiling.cpp
 * \brief
 */
...
using std::string;
namespace optiling {
// --------------------------PivotLIInfoParser类成员函数定义-------------------------------------
ge::graphStatus PivotLIInfoParser::CheckRequiredInOutExistence() const
{
...
ge::graphStatus PivotLIInfoParser::CheckRequiredAttrExistence() const
{
...
ge::graphStatus PivotLIInfoParser::CheckRequiredParaExistence() const
{
...
ge::graphStatus PivotLIInfoParser::GetOpName()
{
    if (context_->GetNodeName() == nullptr) {
        OP_LOGE("PivotLightningIndexer", "opName got from TilingContext is nullptr");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus PivotLIInfoParser::GetNpuInfo()
{
...
void PivotLIInfoParser::GetOptionalInputParaInfo()
{
...
void PivotLIInfoParser::GetInputParaInfo()
{
...
void PivotLIInfoParser::GetOutputParaInfo()
{
    opParamInfo_.attenOut.desc = context_->GetOutputDesc(PIVOT_LIGHTNING_INDEXER);
    opParamInfo_.attenOut.shape = context_->GetOutputShape(PIVOT_LIGHTNING_INDEXER);
...
ge::graphStatus PivotLIInfoParser::GetAndCheckAttrParaInfo()
{
...
ge::graphStatus PivotLIInfoParser::GetOpParaInfo()
{
    GetInputParaInfo();
    GetOutputParaInfo();
...
ge::graphStatus PivotLIInfoParser::GetAndCheckInOutDataType()
{
...
ge::graphStatus PivotLIInfoParser::GetQueryKeyAndOutLayout()
{
    // 获取query,key的Layout基准值
    const map<string, PivotDataLayout> layoutMap = {
        {"BSND", PivotDataLayout::BSND},
        {"TND", PivotDataLayout::TND},
        {"PA_BSND", PivotDataLayout::BnBsND}
    };
...
ge::graphStatus PivotLIInfoParser::GetAndCheckOptionalInput()
{
    if (kLayout_ == PivotDataLayout::BnBsND) {
    ...
    } else if (kLayout_ == PivotDataLayout::TND) {
    ...
    OP_CHECK_IF(opParamInfo_.actualSeqLengths.tensor != nullptr &&
               ...
               OP_LOGE(opName_, "input actual_seq_lengths_key data type only support int32"),
               return ge::GRAPH_FAILED);
    if (qLayout_ == PivotDataLayout::TND) {
    ...
    OP_CHECK_IF(opParamInfo_.actualSeqLengthsQ.tensor != nullptr &&
               ...
               return ge::GRAPH_FAILED);
    OP_CHECK_IF(kLayout_ != PivotDataLayout::BnBsND && opParamInfo_.blockTable.tensor != nullptr,
                   OP_LOGE(opName_, "when key layout is not PA_BSND, input block_table must be null"),
                   return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus PivotLIInfoParser::CheckShapeDim()
{
    OP_CHECK_IF((opParamInfo_.blockTable.tensor != nullptr) &&
    ...
    if (kLayout_ == PivotDataLayout::TND) {
...
ge::graphStatus PivotLIInfoParser::GetN1Size()
{
    if (qLayout_ == PivotDataLayout::BSND) {
...
ge::graphStatus PivotLIInfoParser::GetActualSeqLenSize(uint32_t &size, const gert::Tensor *tensor,
...
ge::graphStatus PivotLIInfoParser::GetAndCheckN2Size()
{
    uint32_t n2Index = (kLayout_ == PivotDataLayout::TND) ? DIM_IDX_ONE : DIM_IDX_TWO;
...
ge::graphStatus PivotLIInfoParser::GetGSize()
{
...
ge::graphStatus PivotLIInfoParser::GetBatchSize()
{
    // 获取B基准值
...
ge::graphStatus PivotLIInfoParser::GetHeadDim()
{
    // 以query的D维度为基准
    uint32_t dIndex = DIM_IDX_TWO;
    // 根据layout确定D维度在shape中的位置
    switch (qLayout_) {
        case PivotDataLayout::TND:
            // TND格式: [Total, N, D] -> D是第2维(索引2)
            dIndex = DIM_IDX_TWO;
            break;
        case PivotDataLayout::BSND:
...
ge::graphStatus PivotLIInfoParser::GetS1Size()
{
    if (qLayout_ == PivotDataLayout::BSND) {
...
ge::graphStatus PivotLIInfoParser::GetAndCheckBlockSize()
{
...
ge::graphStatus PivotLIInfoParser::CheckBlockCount()
{
...
ge::graphStatus PivotLIInfoParser::GetS2SizeForPageAttention()
{
...
ge::graphStatus PivotLIInfoParser::GetS2Size()
{
    // 获取S2基准值
    ...
    } else if (kLayout_ == PivotDataLayout::BSND) {
        s2Size_ = opParamInfo_.key.shape->GetStorageShape().GetDim(1);
    }
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus PivotLIInfoParser::ValidateInputShapesMatchQtnd()
{
    // -----------------------check BatchSize-------------------
    // bSize_ 来源于act_seq_q
    if (kLayout_ == PivotDataLayout::TND) {
...
ge::graphStatus PivotLIInfoParser::ValidateInputShapesMatchQbsnd()
{
    // -----------------------check BatchSize-------------------
    // bSize_ 来源于query
    if (kLayout_ == PivotDataLayout::BnBsND) {
...
ge::graphStatus PivotLIInfoParser::ValidateInputShapesMatch()
{
    /*
    TND:
    query [T,N1,D],
    ...
    */
    ...
    // -----------------------check D-------------------
    uint32_t keyDDim = kLayout_ == PivotDataLayout::TND ? DIM_IDX_TWO : DIM_IDX_THREE;
...
void PivotLIInfoParser::GenerateInfo(PivotLITilingInfo &liInfo)
{
...
ge::graphStatus PivotLIInfoParser::ParseAndCheck(PivotLITilingInfo &liInfo)
{
...
// --------------------------TilingPrepare函数定义-------------------------------------
static ge::graphStatus TilingPrepareForPivotLightningIndexer(gert::TilingParseContext * /* context */)
{
    return ge::GRAPH_SUCCESS;
}

// --------------------------PivotLightningIndexerTiling类成员函数定义-----------------------
ge::graphStatus PivotLightningIndexerTiling::DoTiling(PivotLITilingInfo *tilingInfo)
{
    // -------------set blockdim-----------------
...
// --------------------------Tiling函数定义---------------------------
ge::graphStatus TilingForPivotLightningIndexer(gert::TilingContext *context)
{
    OP_CHECK_IF(context == nullptr, OPS_REPORT_VECTOR_INNER_ERR("PivotLightningIndexer", "Tiling context is null."),
               return ge::GRAPH_FAILED);
    PivotLITilingInfo liInfo;
    PivotLIInfoParser infoParser(context);
    if (infoParser.ParseAndCheck(liInfo) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    PivotLightningIndexerTiling liTiling(context);
    return liTiling.DoTiling(&liInfo);
}

// --------------------------Tiling函数及TilingPrepare函数注册--------
IMPL_OP_OPTILING(PivotLightningIndexer)
    .Tiling(TilingForPivotLightningIndexer)
    .TilingParse<PivotLICompileInfo>(TilingPrepareForPivotLightningIndexer);
