...
/*!
 * \file pivot_lightning_indexer_tiling.h
 * \brief
 */

#ifndef PIVOT_LIGHTNING_INDEXER_TILING_H_
#define PIVOT_LIGHTNING_INDEXER_TILING_H_
...
namespace optiling {
// ------------------公共定义--------------------------
struct PivotTilingRequiredParaInfo {
    const gert::CompileTimeTensorDesc *desc;
    const gert::StorageShape *shape;
};

struct PivotTilingOptionalParaInfo {
    const gert::CompileTimeTensorDesc *desc;
    const gert::Tensor *tensor;
};

enum class PivotDataLayout : uint32_t {
    BSND = 0,
    TND = 1,
    BnBsND = 2
};
...
//Outputs Index
constexpr uint32_t PIVOT_LIGHTNING_INDEXER = 0;
...
// -----------算子TilingData定义---------------
BEGIN_TILING_DATA_DEF(PivotLITilingData)
...
END_TILING_DATA_DEF
REGISTER_TILING_DATA_CLASS(PivotLightningIndexer, PivotLITilingData)

// -----------算子CompileInfo定义-------------------
struct PivotLICompileInfo {};

// -----------算子Tiling入参结构体定义---------------
struct PivotLIParaInfo {
    PivotTilingRequiredParaInfo query = {nullptr, nullptr};
    PivotTilingRequiredParaInfo key = {nullptr, nullptr};
    PivotTilingRequiredParaInfo weights = {nullptr, nullptr};
    PivotTilingOptionalParaInfo actualSeqLengthsQ = {nullptr, nullptr};
    PivotTilingOptionalParaInfo actualSeqLengths = {nullptr, nullptr};
    PivotTilingOptionalParaInfo blockTable = {nullptr, nullptr};
    PivotTilingRequiredParaInfo attenOut = {nullptr, nullptr};
    PivotTilingRequiredParaInfo valuesOut = {nullptr, nullptr};
...
// -----------算子Tiling入参信息类---------------
class PivotLITilingInfo {
public:
    const char *opName = nullptr;
    fe::PlatFormInfos *platformInfo = nullptr;
    PivotLIParaInfo opParamInfo;
    // Base Param
    ...
    // Layout
    PivotDataLayout inputQLayout = PivotDataLayout::BSND;
    PivotDataLayout inputKLayout = PivotDataLayout::BnBsND;
};

// -----------算子Tiling入参信息解析及Check类---------------
class PivotLIInfoParser {
public:
    explicit PivotLIInfoParser(gert::TilingContext *context) : context_(context)
    {
    }
    ~PivotLIInfoParser() = default;
    ...
    void GenerateInfo(PivotLITilingInfo &liInfo);
    ge::graphStatus ParseAndCheck(PivotLITilingInfo &liInfo);

public:
    gert::TilingContext *context_ = nullptr;
    const char *opName_;
    fe::PlatFormInfos *platformInfo_;
    PivotLIParaInfo opParamInfo_;
    ...
    uint32_t headDim_ = 0;
    // Layout
    PivotDataLayout qLayout_ = PivotDataLayout::BSND;
    PivotDataLayout kLayout_ = PivotDataLayout::BnBsND;
    // PageAttention
...
// ---------------算子Tiling类---------------
class PivotLightningIndexerTiling {
public:
    explicit PivotLightningIndexerTiling(gert::TilingContext *context) : context_(context){};
    ge::graphStatus DoTiling(PivotLITilingInfo *tilingInfo);

private:
    gert::TilingContext *context_ = nullptr;
    PivotLITilingData tilingData_;
};

} // namespace optiling
#endif // PIVOT_LIGHTNING_INDEXER_TILING_H_
