...
/*!
 * \file pivot_lightning_indexer_kernel.h
 * \brief
 */

#ifndef PIVOT_LIGHTNING_INDEXER_KERNEL_H
#define PIVOT_LIGHTNING_INDEXER_KERNEL_H
...
#include "pivot_lightning_indexer_service_vector.h"
#include "pivot_lightning_indexer_service_cube.h"
...
class PivotLightningIndexerKernel {
public:
    __aicore__ inline PivotLightningIndexerKernel(){};
    ...
    using W_T = typename PivotLightningIndexerTypeTraits<Q_T,
                                                typename std::conditional<DT_W_FLAG, float, void>::type>::weightsType;

    using MM1_OUT_T = float;

    PivotLightningIndexerServiceCube<LIT> matmulService;
    PivotLightningIndexerServiceVector<LIT> vectorService;
...
protected:
    ...
    // ================================Init functions==================================
    __aicore__ inline void InitTilingData(const PivotLITilingData *__restrict tilingData);
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::InitTilingData(const PivotLITilingData *__restrict tilingData)
{
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::InitBuffers()
{
    if ASCEND_IS_AIV {
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ,
...
template <typename LIT>
__aicore__ inline uint32_t PivotLightningIndexerKernel<LIT>::GetActualSeqLen(uint32_t bIdx,
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::GetS1S2ActualSeqLen(uint32_t bIdx,
                                                         uint32_t &actS1Size, uint32_t &actS2Size)
{
...
template <typename LIT>
__aicore__ inline uint32_t PivotLightningIndexerKernel<LIT>::GetTotalBaseBlockNum()
{
...
// 多核版本，双闭区间
template <typename LIT>
__aicore__ void inline PivotLightningIndexerKernel<LIT>::SplitCore(uint32_t curCoreIdx,
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::DealActSeqLenIsZero(uint32_t bIdx, uint32_t n2Idx, uint32_t s1Start)
{
    if ASCEND_IS_AIV {
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::Init(__gm__ uint8_t *query,
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::GetBN2Idx(uint32_t bN2Idx)
{
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::CalcS2LoopParams(uint32_t bN2LoopIdx, uint32_t gS1LoopIdx)
{
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::CalcGS1LoopParams(uint32_t bN2LoopIdx)
{
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::CalcRunInfo(uint32_t loop,
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::Process()
{
    if (usedCoreNum == 0) {
        // 没有计算任务，直接清理输出
        ProcessInvalid();
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::ProcessInvalid()
{
    if ASCEND_IS_AIV {
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::ProcessMain()
{
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::ProcessBaseBlock(uint32_t loop,
...
template <typename LIT>
__aicore__ inline void PivotLightningIndexerKernel<LIT>::ProcessDecode()
{
    if ASCEND_IS_AIV {
...
#endif // PIVOT_LIGHTNING_INDEXER_KERNEL_H
