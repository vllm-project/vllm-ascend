...
/*!
 * \file fused_infer_attention_score_v2_sink_tiling_check_single_para.cpp
 * \brief
 */
...
const std::map<std::string, std::vector<FiaLayout>> LAYOUT_SUPPORT_MAP = {
    ...
    {VALUE_NAME,
     {FiaLayout::BSH,
      ...
      FiaLayout::BnNBsD}},
    {ATTEN_OUT_NAME,
     {FiaLayout::BSH, FiaLayout::BSND, FiaLayout::BNSD, FiaLayout::TND, FiaLayout::NTD, FiaLayout::NBSD}},
};

const std::set<ge::Format> FORMAT_SUPPORT_SET = {
    ge::FORMAT_ND, ge::FORMAT_NCHW, ge::FORMAT_NHWC, ge::FORMAT_NCDHW, ge::FORMAT_NCL};

void FiaTilingCheck::LogErrorDtypeSupport(const std::vector<ge::DataType> &expectDtypeList,
                                          const ge::DataType &actualDtype,
                                          const std::string &name) const
{
    ...
    OPS_LOG_E(opName_,
              "Tensor %s only supports dtype %s, but got %s",
              name.c_str(),
              oss.str().c_str(),
              FusedDataTypeToSerialString(actualDtype).c_str());
}
                                                   ...
                                                   const std::string &name) const
{
    if (desc != nullptr) {
        ...
        OPS_CHECK((FORMAT_SUPPORT_SET.find(format) == FORMAT_SUPPORT_SET.end()),
                  OPS_LOG_E(opName_,
                            "%s format only supports ND/NCHW/NHWC/NCDHW/NCL, but got %d",
                            name.c_str(),
                                                   ...
                                                   const std::string &name) const
{
    ...
    if (std::find(expectDimNumList.begin(), expectDimNumList.end(), tensor->GetStorageShape().GetDimNum()) ==
        expectDimNumList.end()) {
...
ge::graphStatus FiaTilingCheck::CheckShapeSupport(const gert::Tensor *tensor,
                                                  const std::vector<int64_t> &expectShapeList,
                                                  const std::string &name) const
{
    if (tensor == nullptr) {
        return ge::GRAPH_SUCCESS;
                                           ...
                                           const std::string &name) const
{
    ...
    OPS_LOG_E(opName_,
              "%s only supports layout %s, but got %s",
              name.c_str(),
              oss.str().c_str(),
              LayoutToSerialString(actualLayout).c_str());
}
...
ge::graphStatus FiaTilingCheck::CheckLayoutSupport(const FiaLayout &actualLayout, const std::string &name) const
{
    ...
    OPS_CHECK(std::find(expectLayoutList.begin(), expectLayoutList.end(), actualLayout) == expectLayoutList.end(),
...
ge::graphStatus FiaTilingCheck::CheckSingleParaQuery() const
{
    ...
    OPS_CHECK(opParamInfo_.query.shape->GetStorageShape().GetShapeSize() == 0,
              OPS_LOG_E(opName_, "%s shapesize is 0.", QUERY_NAME.c_str()),
              return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus FiaTilingCheck::CheckSingleParaQueryPaddingSize() const
{
    ...
    if (ge::GRAPH_SUCCESS != CheckShapeSupport(opParamInfo_.queryPaddingSize.tensor,
                                               querypaddingsizeShapeNumList,
                                               QUERY_PADDING_SIZE_NAME)) {
        return ge::GRAPH_FAILED;
    }

    const std::vector<size_t> querypaddingsizeDimNumList = {DIM_NUM_ONE};
    if (ge::GRAPH_SUCCESS !=
        CheckDimNumSupport(opParamInfo_.queryPaddingSize.tensor, querypaddingsizeDimNumList, QUERY_PADDING_SIZE_NAME)) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus FiaTilingCheck::CheckSingleParaKvPaddingSize() const
{
    ...
    if (ge::GRAPH_SUCCESS !=
        CheckDimNumSupport(opParamInfo_.kvPaddingSize.tensor, kvpaddingsizeDimNumList, KV_PADDING_SIZE_NAME)) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus FiaTilingCheck::CheckSingleParaMetadata() const
{
    ...
    if (opParamInfo_.metadata.tensor->GetShapeSize() != METADATA_LIMIT) {
        OPS_LOG_E(opName_, "input metadata total size must be %u.", METADATA_LIMIT);
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus FiaTilingCheck::CheckSingleParaLayout() const
{
    ...
    if (std::find(inputLayoutList.begin(), inputLayoutList.end(), inputLayout) == inputLayoutList.end()) {
        OPS_LOG_E(opName_,
                  "input layout only supports BSH, BSND, BNSD, TND, NTD, BSH_NBSD, BSND_NBSD, BNSD_NBSD, TND_NTD, "
                  "NTD_TND, BSH_BNSD, BSND_BNSD, BNSD_BSND, but got %s",
                  inputLayout.c_str());
        return ge::GRAPH_FAILED;
    }
...
{
    const std::vector<int32_t> innerPreciseList = {INNER_PRECISE_HIGH_PRECISION,
...
{
    const std::vector<int64_t> antiquantModeList = {ANTIQUANT_PER_CHANNEL_MODE, ANTIQUANT_PER_TOKEN_MODE};
    if (ge::GRAPH_SUCCESS !=
        CheckAttrValueSupport(opParamInfo_.antiquantMode, antiquantModeList, ANTIQUANT_MODE_NAME)) {
        return ge::GRAPH_FAILED;
    }
...
{
    const std::vector<int64_t> keyAntiquantModeList = {ANTIQUANT_PER_CHANNEL_MODE,
    ...
    if (ge::GRAPH_SUCCESS !=
        CheckAttrValueSupport(opParamInfo_.keyAntiquantMode, keyAntiquantModeList, KEY_ANTIQUANT_MODE_NAME)) {
        return ge::GRAPH_FAILED;
    }
...
{
    const std::vector<int64_t> valueAntiquantModeList = {ANTIQUANT_PER_CHANNEL_MODE,
                                                         ANTIQUANT_PER_TOKEN_MODE,
                                                         ANTIQUANT_PER_TENSOR_HEAD_MODE,
                                                         ANTIQUANT_PER_TOKEN_HEAD_MODE,
                                                         ANTIQUANT_PER_TOKEN_PA_MODE,
                                                         ANTIQUANT_PER_TOKEN_HEAD_PA_MODE};
    if (ge::GRAPH_SUCCESS !=
        CheckAttrValueSupport(opParamInfo_.valueAntiquantMode, valueAntiquantModeList, VALUE_ANTIQUANT_MODE_NAME)) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus FiaTilingCheck::CheckSingleParaSparseMode() const
{
    ...
    if (ge::GRAPH_SUCCESS != CheckAttrValueSupport(opParamInfo_.sparseMode, sparseModeList, SPARSE_MODE_NAME)) {
        OPS_LOG_E(opName_, "sparseMode only supports 0/1/2/3/4, but got %u", *opParamInfo_.sparseMode);
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus FiaTilingCheck::CheckSinglePara() const
{
    if (ge::GRAPH_SUCCESS != CheckSingleParaQuery() || ge::GRAPH_SUCCESS != CheckSingleParaKey() ||
        ge::GRAPH_SUCCESS != CheckSingleParaValue() || ge::GRAPH_SUCCESS != CheckSingleParaPseShift() ||
        ge::GRAPH_SUCCESS != CheckSingleParaAttenMask() || ge::GRAPH_SUCCESS != CheckSingleParaDeqScale1() ||
        ge::GRAPH_SUCCESS != CheckSingleParaQuantScale1() || ge::GRAPH_SUCCESS != CheckSingleParaDeqScale2() ||
        ge::GRAPH_SUCCESS != CheckSingleParaQuantScale2() || ge::GRAPH_SUCCESS != CheckSingleParaQuantOffset2() ||
        ge::GRAPH_SUCCESS != CheckSingleParaAntiquantScale() || ge::GRAPH_SUCCESS != CheckSingleParaAntiquantOffset() ||
        ge::GRAPH_SUCCESS != CheckSingleParaQueryPaddingSize() || ge::GRAPH_SUCCESS != CheckSingleParaKvPaddingSize() ||
        ...
        ge::GRAPH_SUCCESS != CheckSingleParaValueAntiquantOffset() || ge::GRAPH_SUCCESS != CheckSingleParaQueryRope() ||
        ge::GRAPH_SUCCESS != CheckSingleParaKeyRope() || ge::GRAPH_SUCCESS != CheckSingleParaDequantScaleQuery() ||
        ge::GRAPH_SUCCESS != CheckSingleParaMetadata() || ge::GRAPH_SUCCESS != CheckSingleParaAttenOut() ||
        ge::GRAPH_SUCCESS != CheckSingleParaLseOut() || ge::GRAPH_SUCCESS != CheckSingleParaLayout() ||
        ge::GRAPH_SUCCESS != CheckSingleParaInnerPrecise() || ge::GRAPH_SUCCESS != CheckSingleParaAntiquantMode() ||
        ge::GRAPH_SUCCESS != CheckSingleParaKeyAntiquantMode() ||
        ge::GRAPH_SUCCESS != CheckSingleParaValueAntiquantMode() || ge::GRAPH_SUCCESS != CheckSingleParaSparseMode()) {
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
