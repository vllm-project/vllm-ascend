...
/*!
 * \file fused_infer_attention_score_v2_sink_tiling_check.h
 * \brief
 */
#ifndef FUSED_INFER_ATTENTION_SCORE_V2_SINK_TILING_CHECK_H
#define FUSED_INFER_ATTENTION_SCORE_V2_SINK_TILING_CHECK_H

#include "fia_tiling_info.h"
#include "fia_tiling_shape.h"

namespace optiling {
static constexpr int32_t INNER_PRECISE_HIGH_PRECISION = 0;
static constexpr int32_t INNER_PRECISE_HIGH_PERFORMANCE = 1;
static constexpr int32_t INNER_PRECISE_HIGH_PRECISION_ROW_INVALID = 2;
static constexpr int32_t INNER_PRECISE_HIGH_PERFORMANCE_ROW_INVALID = 3;

static constexpr int64_t ANTIQUANT_PER_CHANNEL_MODE = 0;
static constexpr int64_t ANTIQUANT_PER_TOKEN_MODE = 1;
static constexpr int64_t ANTIQUANT_PER_TENSOR_HEAD_MODE = 2;
static constexpr int64_t ANTIQUANT_PER_TOKEN_HEAD_MODE = 3;
static constexpr int64_t ANTIQUANT_PER_TOKEN_PA_MODE = 4;
static constexpr int64_t ANTIQUANT_PER_TOKEN_HEAD_PA_MODE = 5;
static constexpr int64_t ANTIQUANT_PER_CHANNEL_TOKEN_MODE = 2;

static constexpr uint32_t PRETOKEN_LIMIT_2K = 2048;
static constexpr uint32_t KVS_LIMIT = 131088;

static constexpr size_t DIM_NUM_ONE = 1;
static constexpr size_t DIM_NUM_TWO = 2;
static constexpr size_t DIM_NUM_THREE = 3;
static constexpr size_t DIM_NUM_FOUR = 4;

static constexpr size_t HEAD_DIM_512 = 512;
static constexpr size_t SHAPE_NUM_ONE = 1;
static constexpr uint32_t METADATA_LIMIT = 1024;
std::string FusedDataTypeToSerialString(ge::DataType type);
std::string RopeModeToSerialString(const RopeMode &ropeMode);


template <typename vecT, typename T>
inline bool VecContains(const vecT& vec, const T& value)
{
...
protected:
    ...
    virtual ~TilingCheck() noexcept = default;
...
public:
    ...
    virtual ~FiaTilingCheck() noexcept override = default;
...
private:
    ...
    ge::graphStatus CheckSoftmaxLseTypeDTypeAndShape(const FIARequiredParaInfo &outInfo, const std::string &name);
    ge::graphStatus CheckSoftmaxMaxSum();
    ge::graphStatus CheckMultiParaConsistency();
    ge::graphStatus CheckParamSinkShape();
    ge::graphStatus CheckParamSinkNumber();
    ge::graphStatus CheckSinkNumber();
    ge::graphStatus CheckMlaSinkNumber();
...
#endif // FUSED_INFER_ATTENTION_SCORE_V2_SINK_TILING_CHECK_H
