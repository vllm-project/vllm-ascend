...
/*!
 * \file fia_tiling_nonquant_mla_sink.cpp
 * \brief
 */
...
void FiaTilingNonQuantMla::CalcInnerSize(uint32_t seqSize)
{
    ...
    */
    if (fiaInfo_->batchInvariant) {
         ...
         * s1G >= 128为实测最优值
        */
        if (fiaInfo_->sinkNumber > 0 && fiaInfo_->sinkNumber <= 256 && fiaInfo_->sparseMode == SPARSE_MODE_BAND &&
            s1G >= 128) {
            sInnerSize_ = 256U;
        }
        ...
        */
        if ((fiaInfo_->keySinkNumber > 0 && fiaInfo_->sparseMode == SPARSE_MODE_BAND && fiaInfo_->bSize <= 6 &&
            (fiaInfo_->preToken + fiaInfo_ ->nextToken < 512))) {
            sInnerSize_ = 256U;
        }
...
void FiaTilingNonQuantMla::CalcMBaseSize()
{
    ...
    mBaseSize_ = 256; // 真实mbaseSize由fused_infer_attention_score_v2_sink_metadata计算，此次仅保证计算最大workspace
}
...
void FiaTilingNonQuantMla::FillTilingBaseParams()
{
    ...
    tilingData_.baseParams.set_softmaxMaxSumFlag(fiaInfo_->softmaxMaxSumFlag ? 1 : 0);
}
...
uint64_t FiaTilingNonQuantMla::CalcNormalWorkspaceSize(uint32_t coreNum,
                                                       int64_t mm1ResSize,
                                                       int64_t mm2ResSize,
                                                       uint32_t mBaseSize) const
{
    ...
    workspaceSize += PRE_LOAD_NUM_MLA * coreNum * mm2ResSize * V2_RES_ELEM_SIZE; // vec2ResGm
    if (!fiaInfo_->batchInvariant) {
        // aMla nUpdate, mBaseSize=128
        workspaceSize += PRE_LOAD_NUM_MLA * coreNum * mBaseSize * N_UPDATE_ELEM_SIZE;
        // aMla softmaxSum, mBaseSize=128
        workspaceSize += PRE_LOAD_NUM_MLA * coreNum * mBaseSize * SOFTMAX_SUM_ELEM_SIZE;
    }
...
ge::graphStatus FiaTilingNonQuantMla::DoOpTiling()
{
    ...
    if (fiaInfo_->isMaxWorkspace) {
        ...
        CalcScheduleMode();
        CalcWorkspaceSize(); // 按照全核申请workspace
        GenTilingKey();
    }
...
REGISTER_TILING_TEMPLATE_FIA(FusedInferAttentionScoreV2Sink,
                             ...
                             std::vector<int32_t>({static_cast<int32_t>(platform_ascendc::SocVersion::ASCEND910B), static_cast<int32_t>(platform_ascendc::SocVersion::ASCEND910_93)}),
                             9);
} // namespace optiling
