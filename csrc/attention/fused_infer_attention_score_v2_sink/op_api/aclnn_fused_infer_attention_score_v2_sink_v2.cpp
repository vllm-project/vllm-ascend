...
#include "aclnn_fused_infer_attention_score_v2_sink_v2.h"
...
static void TensorPreProcessV2(const aclTensorList *&tensorListKey, const aclTensorList *&tensorListValue)
{
    if (tensorListKey == nullptr) {
        OP_LOGD("TensorListKey is nullptr,TensorPreProcessV2 exit.");
        return;
    }
    if (tensorListValue == nullptr) {
        OP_LOGD("tensorListValue is nullptr,TensorPreProcessV2 exit.");
        return;
    }
...
static void PrefixTensorPreProcessV2(const aclTensor *&tensorKey, const aclTensor *&tensorValue)
{
...
static aclnnStatus FakeArrayV2(const aclIntArray *inArray, aclTensor *&outTensor)
{
...
static void FusedInferAttentionScoreV2SinkProcessSoftmaxLseV2(bool softmaxLseFlag,
...
// aclnnV2 永不返回真实的 softmaxMax/softmaxSum（v1 PTA 接口为 2 元组），故对二者
// 始终创建 shape=[0]、假地址的 placeholder 占位 tensor，仅用于满足 L0 的 4 出参签名。
// 调用方负责在末尾 aclDestroyTensor 释放。
aclTensor *FusedInferAttentionScoreV2SinkCreateMaxSumPlaceholderV2()
{
    std::vector<int64_t> shape = {0};
    int64_t addr = 0xff;
 ...
 */
__attribute__((visibility("default"))) aclnnStatus aclnnFusedInferAttentionScoreV2SinkV2GetMaxWorkspaceSize(
...
aclnnStatus aclnnFusedInferAttentionScoreV2SinkV2GetMaxWorkspaceSize(const aclTensor *query,
                                                                     const aclTensorList *tensorListKey,
                                                                     const aclTensorList *tensorListValue,
                                                                     const aclTensor *pseShiftOptional,
                                                                     const aclTensor *attenMaskOptional,
                                                                     ...
                                                                     aclOpExecutor **executor)
{
    ...
    TensorPreProcessV2(tensorListKey, tensorListValue);

    const aclTensor *placeHolder = nullptr;
    const aclTensor *tempTensor = nullptr;
    FusedInferAttentionScoreV2SinkProcessSoftmaxLseV2(softmaxLseFlag, softmaxLse, tempTensor, placeHolder);

    auto ret = aclnnFusedInferAttentionScoreV2SinkV2GetWorkspaceSize(query,
                                                                     tensorListKey,
                                                                     tensorListValue,
                                                                     pseShiftOptional,
                                                                     attenMaskOptional,
...
aclnnStatus aclnnFusedInferAttentionScoreV2SinkV2GetWorkspaceSize(const aclTensor *query,
                                                                  ...
                                                                  aclOpExecutor **executor)
{
    L2_DFX_PHASE_1(aclnnFusedInferAttentionScoreV2SinkV2,
            DFX_IN(query,
                   key,
                   value,
                   pseShiftOptional,
    ...
    // aclnnV2 不返回 softmaxMax/softmaxSum，始终创建 shape=[0] placeholder 占位，仅满足 L0 4 出参签名
    auto *softmaxMaxPlaceholder = FusedInferAttentionScoreV2SinkCreateMaxSumPlaceholderV2();
    auto *softmaxSumPlaceholder = FusedInferAttentionScoreV2SinkCreateMaxSumPlaceholderV2();
    ...
    // max/sum 占位输出丢弃（v1/v2 不返回），仅 ViewCopy 到 placeholder 保证 launch 图完整
...
aclnnStatus aclnnFusedInferAttentionScoreV2SinkV2(void *workspace,
                                                  uint64_t workspaceSize,
                                                  aclOpExecutor *executor,
                                                  const aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnFusedInferAttentionScoreV2SinkV2);
