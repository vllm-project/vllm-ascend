  ...
  private:
    ...
    ProcessAntiqPA(FaUbTensor<T> &dstTensor, FaGmTensor<T, GM_FORMAT> &srcTensor, AntiqGmCoord &antiqGmCoord)
    {
        ...
        while (copyFinishElmeCnt < antiqGmCoord.s2DealSize) {
          ...
          CopySingleMatrixNDToND<T>(
              dstTensor.tensor[dstOffset], srcTensor.gmTensor[srcOffset], 1, copyElemCnt, copyElemCnt, copyElemCnt);
    ...
    __aicore__ inline void ProcessGS1(FaUbTensor<T> &dstTensor, FaGmTensor<T, GM_FORMAT> &srcTensor, GmCoord &gmCoord)
    {
        ...
        if (gIdxEnd - gIdxStart >= 1) {
            ...
            for (uint32_t i = gIdxStart + 1; i < gIdxEnd; i++) {
                CopySingleMatrixNDToND<T>(dstTensor.tensor[ubOffset],
...
template <typename KV_T, GmFormat GM_FORMAT> class CopyKvGmToUb {
  ...
  private:
                                                         ...
                                                         GmKvCoord &gmCoord)
    {
        ...
        CopySingleMatrixNDToND<KV_T>(dstTensor.tensor,
                                     srcTensor.gmTensor[offset],
                                     gmCoord.s2DealSize,
    ...
    ProcessPageAttention(FaUbTensor<KV_T> &dstTensor, FaGmTensor<KV_T, GM_FORMAT> &srcTensor, GmKvCoord &gmCoord)
    {
        ...
        uint32_t blockElementCnt = AiInfraInferenceAttentionCommon::BYTE_BLOCK / sizeof(KV_T);
        ...
        if constexpr (GM_FORMAT == GmFormat::PA_NZ) {
            ...
            while (copyFinishRowCnt < gmCoord.s2DealSize) {
                ...
                // DataCopyPad
                CopySingleMatrixNDToND<KV_T>(dstTensor.tensor[l1Offset],
                                             srcTensor.gmTensor[gmOffset],
                                             copyRowCnt,
                                             gmCoord.dDealSize,
...
template <typename PSE_T, GmFormat GM_FORMAT, UbFormat UB_FORMAT> class CopyPSEGmToUb {
  public:
    __aicore__ inline void operator()(FaUbTensor<PSE_T> &dstTensor,
                                      FaGmTensor<PSE_T, GM_FORMAT> &srcTensor,
                                      ...
                                      bool qsEqualOne = false) // qsEqualOne用于适配qs = 1时，pseshifts1 > qs的场景
    {
        if constexpr (UB_FORMAT == UbFormat::GS1) {
            ...
            uint64_t pseOffset = offsetCalculator.GetOffset(gmPseCoord.bIdx,
                                                            gmPseCoord.n2Idx,
                                                            gIdxStart,
            ...
            // 统一的接口
            if (qsEqualOne) {
                CopySingleMatrixNDToND<PSE_T>(dstTensor.tensor,
                                              srcTensor.gmTensor[pseOffset],
                                              gmPseCoord.gS1DealSize,
                                              gmPseCoord.s2DealSize,
                                              offsetCalculator.GetStrideG(),
                                              dstTensor.colCount);
            } else {
                CopySingleMatrixNDToND<PSE_T>(dstTensor.tensor,
            ...
            if (s1IdxEnd - s1IdxStart >= 1) {
                ...
                // 处理中间块
                for (uint32_t i = s1IdxStart + 1; i < s1IdxEnd; i++) {
                    CopySingleMatrixNDToND<PSE_T>(dstTensor.tensor[ubOffset],
                                                  srcTensor.gmTensor[gmOffset],
                                                  offsetCalculator.GetDimG(),
                                                  gmPseCoord.s2DealSize,
                                                  offsetCalculator.GetStrideG(),
