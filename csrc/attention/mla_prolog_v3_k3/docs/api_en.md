# MlaPrologV3K3 API and Usage Examples
...
| --- | --- | --- |
| vllm-ascend custom op | `torch.ops._C_ascend.npu_mla_prolog_v3_k3` | Supported |
| aclnn | `aclnnMlaPrologV3K3WeightNzGetWorkspaceSize` / `aclnnMlaPrologV3K3WeightNz` | Supported |
| Ascend C `<<<>>>` | `mla_prolog_v3_k3<<<blockDim, nullptr, stream>>>` | Supported (diagnostics / direct invoke; caller must provide tiling) |
...
The underlying operator name is **MlaPrologV3K3**. Weights `weight_dq` / `weight_uq_qr` / `weight_dkv_kr` must be passed in **FRACTAL_NZ** format.
