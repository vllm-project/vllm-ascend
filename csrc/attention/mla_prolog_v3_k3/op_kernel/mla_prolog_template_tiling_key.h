...
ASCENDC_TPL_ARGS_DECL(
    ...
    // Ascend950 supports non-quantized and MXFP8 weights with matched query/KV quantization.
...
ASCENDC_TPL_SEL(
    ...
    // CV1:1 is supported only by non-quantized PA_BSND and PA_NZ.
    ...
    // BSND/TND use CV1:2 and cannot have an empty cache with nonempty query.
    ...
    // Both actual-sequence modes remain supported for PA_BLK layouts with CV1:2.
...
// -------------------------- 全量化kv pertensor量化 --------------------------
    ...
    // MXFP8 requires dequantization; preserve empty-cache, SplitM and RoPE choices.
#if ORIG_DTYPE_TOKEN_X == -1 || ORIG_DTYPE_WEIGHT_UQ_QR == -1 || ORIG_DTYPE_KV_CACHE == -1 || \
    ...
    // GenTilingKey canonicalizes empty query to this key with ENABLE_ROPE=true.
    ASCENDC_TPL_ARGS_SEL(ASCENDC_TPL_UINT_SEL(CACHE_MODE, ASCENDC_TPL_UI_LIST, 0),
