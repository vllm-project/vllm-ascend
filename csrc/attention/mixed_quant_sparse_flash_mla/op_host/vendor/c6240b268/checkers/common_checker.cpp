...
ge::graphStatus CommonChecker::CheckQueryAxes(const CheckContext &context) const
{
    ...
    OP_CHECK_IF(qHeads <= 0 || qHeads > 128, // 128：最大支持的注意力头数
                ...
                return ge::GRAPH_FAILED);
    const int64_t expectedQDim =
        context.variant == OperatorVariant::MIXED_QUANT && context.ropeHeadDim == 0 ? 448 : 512;
    OP_CHECK_IF(qDim != expectedQDim,
                OP_LOGE_FOR_INVALID_VALUE(Op(context), "q head dimension", std::to_string(qDim).c_str(),
                                          std::to_string(expectedQDim).c_str()),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus CommonChecker::CheckKvAxes(const CheckContext &context, const TensorParam &kv, const char *name) const
{
    ...
    if (context.variant == OperatorVariant::MIXED_QUANT) {
        // RoPE0 removes 128 input bytes; NOPE448 and seven tile scales remain.
        expectedDim = context.ropeHeadDim == 0 ? (context.quantMode == 1 ? 480 : 456) :
                                               (context.quantMode == 1 ? 608 : 584);
    }
