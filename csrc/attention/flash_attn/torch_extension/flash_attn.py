...
def _calculate_batch_size(batch_size, cu_seqlens_q, seqused_q):
    ...
    if batch_size is None:
        raise ValueError("When seqused_q and cu_seqlens_q are not provided, batch_size must be provided")
    elif batch_size < 0:
        raise ValueError(
            f"When seqused_q and cu_seqlens_q are not provided, batch_size must be greater than 0, but got {batch_size}"
        )
    return batch_size
...
class FlashAttenOpBuilder(OpBuilder):
    ...
    def register_meta(self):
        ...
        def flash_attn_metadata_meta(
            ...
            win_right: int | None = None,
            layout_q: str | None = None,
            layout_kv: str | None = None,
            layout_out: str | None = None,
            is_grad_enabled: bool | None = False,
        ):
        ...
        def flash_attn_meta(
            ...
            mask_mode: int | None = 0,
            win_left: int | None = -1,
            win_right: int | None = -1,
            max_seqlen_q: int | None = -1,
            max_seqlen_kv: int | None = -1,
            layout_q: str | None = "BSND",
            layout_kv: str | None = "BSND",
            layout_out: str | None = "BSND",
