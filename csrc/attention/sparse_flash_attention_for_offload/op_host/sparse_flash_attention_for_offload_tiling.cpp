...
/*!
 * \file sparse_flash_attention_for_offload_tiling.cpp
 * \brief
 */
...
#include "../op_kernel/sparse_flash_attention_for_offload_template_tiling_key.h"
#include "sparse_flash_attention_for_offload_tiling.h"
...
struct SparseFlashAttentionForOffloadCompileInfo {
    int64_t core_num;
};
...
static std::string SFADataTypeToSerialString(ge::DataType type)
{
    ...
    if (it != DATATYPE_TO_STRING_MAP.end()) {
        ...
        OPS_LOG_E("SparseFlashAttentionForOffload", "datatype %d not support", type);
        return "UNDEFINED";
    }
}
...
ge::graphStatus TilingSparseFlashAttentionForOffload(gert::TilingContext *context)
{
...
ge::graphStatus TilingPrepareForSparseFlashAttentionForOffload(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SFATilingCheck::CheckDecodeOnly() const
{
    ...
    OPS_ERR_IF(sparseBlockSize_ != 1 || *opParamInfo_.sparseMode != 3,
        ...
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(n2Size_ != 1 || gSize_ != n1Size_,
        OPS_LOG_E(opName_,
            "decode requires N2=1 and group size equal to N1, but got N1=%u, N2=%u, group size=%u",
            n1Size_, n2Size_, gSize_),
        return ge::GRAPH_FAILED);
    ...
    const gert::Shape &tailShape = opParamInfo_.tailInfo.shape->GetStorageShape();
    OPS_ERR_IF(tailShape.GetDimNum() != DIM_NUM_TWO || tailShape.GetDim(0) != bSize_ || tailShape.GetDim(1) != 2,
        OPS_LOG_E(opName_, "tail_info must have shape [B, 2]"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(opParamInfo_.tailInfo.desc->GetDataType() != ge::DT_INT32,
        OPS_LOG_E(opName_, "tail_info must be int32"), return ge::GRAPH_FAILED);
...
ge::graphStatus SFAInfoParser::CheckRequiredInOutExistence() const
{
    ...
    OPS_ERR_IF(opParamInfo_.sparseIndices.desc == nullptr, OPS_LOG_E(opName_, "Desc of tensor sparseIndices is nullptr"),
               return ge::GRAPH_FAILED);
    OPS_ERR_IF(opParamInfo_.tailInfo.shape == nullptr, OPS_LOG_E(opName_, "Shape of tensor tailInfo is nullptr"),
               return ge::GRAPH_FAILED);
    OPS_ERR_IF(opParamInfo_.tailInfo.desc == nullptr, OPS_LOG_E(opName_, "Desc of tensor tailInfo is nullptr"),
               return ge::GRAPH_FAILED);
...
ge::graphStatus SFAInfoParser::GetOpName()
{
    if (context_->GetNodeName() == nullptr) {
        OPS_LOG_E("SparseFlashAttentionForOffload", "opName got from TilingContext is nullptr");
        return ge::GRAPH_FAILED;
    }
...
void SFAInfoParser::GetInputParaInfo()
{
    ...
    opParamInfo_.tailInfo.desc = context_->GetInputDesc(TAIL_INFO_INPUT_INDEX);
    opParamInfo_.tailInfo.shape = context_->GetInputShape(TAIL_INFO_INPUT_INDEX);
...
{
    if (context_ == nullptr) {
        OPS_LOG_E("SparseFlashAttentionForOffload", "tiling context is nullptr!");
        return ge::GRAPH_FAILED;
    }
    OPS_LOG_FULL(DLOG_INFO, "SparseFlashAttentionForOffload", "TilingContext: %s", SFADebugTilingContext(context_).c_str());
...
IMPL_OP_OPTILING(SparseFlashAttentionForOffload)
    .Tiling(sfa_offload::TilingSparseFlashAttentionForOffload)
    .TilingParse<sfa_offload::SparseFlashAttentionForOffloadCompileInfo>(
        sfa_offload::TilingPrepareForSparseFlashAttentionForOffload);
} // namespace optiling
