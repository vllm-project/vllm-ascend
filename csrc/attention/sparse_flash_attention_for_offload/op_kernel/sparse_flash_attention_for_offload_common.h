...
/*!
 * \file sparse_flash_attention_for_offload_common.h
 * \brief
 */

#ifndef SPARSE_FLASH_ATTENTION_FOR_OFFLOAD_COMMON_H
#define SPARSE_FLASH_ATTENTION_FOR_OFFLOAD_COMMON_H
...
constexpr uint32_t SFA_OFFLOAD_SPARSE_INDICES_CAPACITY = 16384;
constexpr uint32_t SFA_OFFLOAD_SPARSE_COMPUTE_COUNT = 2048;
constexpr uint32_t SFA_OFFLOAD_TAIL_CAPACITY = 128;
...
template <typename Q_T, typename KV_T, typename OUT_T, const bool FLASH_DECODE = false,
	  SFA_LAYOUT LAYOUT_T = SFA_LAYOUT::BSND, SFA_LAYOUT KV_LAYOUT_T = SFA_LAYOUT::BSND,
          const int TEMPLATE_MODE = C_TEMPLATE, const int STAGE_MODE = SFA_STAGE_NORMAL, typename... Args>
struct SFAType {
    using queryType = Q_T;
    using kvType = KV_T;
...
#endif // SPARSE_FLASH_ATTENTION_FOR_OFFLOAD_COMMON_H
