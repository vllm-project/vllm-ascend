...
/*!
 * \file sparse_flash_attention_for_offload_kernel_mla.h
 * \brief
 */

#ifndef SPARSE_FLASH_ATTENTION_FOR_OFFLOAD_KERNEL_MLA_H
#define SPARSE_FLASH_ATTENTION_FOR_OFFLOAD_KERNEL_MLA_H
...
#include "sparse_flash_attention_for_offload_common.h"
#include "sparse_flash_attention_for_offload_service_cube_mla.h"
#include "sparse_flash_attention_for_offload_service_vector_mla.h"
...
template <typename SFAT> class SparseFlashAttentionForOffloadMla {
public:
    using T = float;
    ...
    __aicore__ inline SparseFlashAttentionForOffloadMla(){};
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                __gm__ uint8_t *sparseIndices, __gm__ uint8_t *tailInfo,
                                ...
                                const SparseFlashAttentionForOffloadTilingDataMla *__restrict tiling,
...
private:
    ...
    const SparseFlashAttentionForOffloadTilingDataMla *__restrict tilingData = nullptr;
    ...
    GlobalTensor<int32_t> tailInfoGm;
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::InitTilingData()
{
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::InitBuffers()
{
    if ASCEND_IS_AIV {
...
SparseFlashAttentionForOffloadMla<SFAT>::InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ,
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::InitAllZeroOutput(uint32_t bIdx, uint32_t s1Idx, uint32_t n2Idx)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::InitOutputSingleCore()
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::GetActualSeqLen(uint32_t bIdx, uint32_t s1Idx)
{
    (void)s1Idx;
    tempLoopInfo.curActualSeqLenOri = GetActualSeqLenKV(bIdx);
    tempLoopInfo.actS1Size = 1;

    int32_t tailSlotStart = tailInfoGm.GetValue(static_cast<int64_t>(bIdx) * 2);
    int32_t tailTokenCount = tailInfoGm.GetValue(static_cast<int64_t>(bIdx) * 2 + 1);
    if (tailSlotStart >= 0 && tailTokenCount > 0 &&
        tailTokenCount <= static_cast<int32_t>(SFA_OFFLOAD_TAIL_CAPACITY)) {
        tempLoopInfo.tailSlotStart = tailSlotStart;
        tempLoopInfo.tailTokenCount = static_cast<uint32_t>(tailTokenCount);
        tempLoopInfo.sparseTokenCount = SFA_OFFLOAD_SPARSE_COMPUTE_COUNT;
    } else {
        tempLoopInfo.tailSlotStart = -1;
        tempLoopInfo.tailTokenCount = 0;
        tempLoopInfo.sparseTokenCount = Min(tempLoopInfo.curActualSeqLenOri,
                                             static_cast<uint64_t>(SFA_OFFLOAD_SPARSE_COMPUTE_COUNT));
    }
}

template <typename SFAT>
__aicore__ inline uint64_t SparseFlashAttentionForOffloadMla<SFAT>::GetTopKRowOffset(uint32_t bIdx, uint32_t s1Idx,
                                                                           uint32_t n2Idx)
{
    (void)s1Idx;
    (void)n2Idx;
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::GetSparseActualSeqLen(uint32_t bIdx, uint32_t s1Idx,
                                                                            uint32_t n2Idx)
{
    (void)bIdx;
    (void)s1Idx;
...
template <typename SFAT>
__aicore__ inline uint32_t SparseFlashAttentionForOffloadMla<SFAT>::GetActualSeqLenKV(uint32_t bIdx)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::DealActSeqLenIsZero(uint32_t bIdx, uint32_t s1Idx, uint32_t n2Idx)
{
    if ASCEND_IS_AIV {
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::GetPreNextTokensLeftUp()
{
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::UpdateInnerLoopCond()
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::UpdateInner(uint32_t &s2End, uint32_t &curS2End,
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::Init(__gm__ uint8_t *query,
                       ...
                       __gm__ uint8_t *gmTiling, TPipe *tPipe)
{
    ...
    tailInfoGm.SetGlobalBuffer((__gm__ int32_t *)tailInfo, constInfo.batchSize * 2);
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::InitCalcParamsEach()
{
...
template <typename SFAT>
__aicore__ inline void
SparseFlashAttentionForOffloadMla<SFAT>::Bmm2DataCopyOut(uint64_t attenOutOffset, LocalTensor<OUT_T> &attenOutUb,
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::CalcParams(uint32_t loop, uint64_t s2Start,
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::ComputeMm1(const RunInfo &info)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::ComputeMm2(const RunInfo &info)
{
    uint32_t nBufferLoopTimes = (info.actMBaseSize + constInfo.nBufferMBaseSize - 1) / constInfo.nBufferMBaseSize;
    uint32_t nBufferTail = info.actMBaseSize - (nBufferLoopTimes - 1) * constInfo.nBufferMBaseSize;
    for (uint32_t i = 0; i < nBufferLoopTimes; i++) {
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::Process()
{
    if (aiCoreIdx < usedCoreNum) {
        if ASCEND_IS_AIV {
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::GetBN2Idx(uint32_t bN2Idx, uint32_t &bIdx,
                                                                                uint32_t &n2Idx)
{
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::ProcessBalance()
{
...
template <typename SFAT>
__aicore__ inline void
SparseFlashAttentionForOffloadMla<SFAT>::PreloadPipeline(uint32_t loop, uint64_t s2Start, uint64_t s2LoopIdx,
                                                               RunInfo extraInfo[SFA_PRELOAD_TASK_CACHE_SIZE], uint32_t &curTopKIdx, uint64_t &curOffsetInSparseBlock)
{
...
SparseFlashAttentionForOffloadMla<SFAT>::GetBalanceActualSeqLengths(GlobalTensor<int32_t> &actualSeqLengths,
                                                                          uint32_t bIdx)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::GetAxisStartIdx(uint32_t bN2EndPrev,
                                                                                      uint32_t s1GEndPrev,
                                                                                      uint32_t s2EndPrev)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionForOffloadMla<SFAT>::CalcSinnerTopKBegin(RunInfo &info, uint32_t &curTopKIdx, uint64_t &curOffsetInSparseBlock)
...
#endif // SPARSE_FLASH_ATTENTION_FOR_OFFLOAD_KERNEL_MLA_H
