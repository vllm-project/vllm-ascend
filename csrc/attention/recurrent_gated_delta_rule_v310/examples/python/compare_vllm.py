...
def call_ops_transformer(
    query, key, value, beta, state, actual_seq_lengths, ssm_state_indices, g, num_accepted_tokens, scale
):
...
def call_vllm(query, key, value, beta, state, actual_seq_lengths, ssm_state_indices, g, num_accepted_tokens, scale):
...
def compare(label, dtype, rtol, b, mtp, nk, nv, dk, dv, num_cache_slots):
    ...
    print(f"\n=== {label} ({dtype}) ===")
    print(
        f"  q={list(query.shape)} k={list(key.shape)} v={list(value.shape)} state={list(state.shape)} b={b} mtp={mtp}"
    )
    ...
    out_ot = call_ops_transformer(q_npu, k_npu, v_npu, b_npu, state_ot, sl_npu, ss_npu, g_npu, nat_npu, scale)
    out_ot = out_ot.float().cpu()
    state_ot = state_ot.float().cpu()

    # vllm
    state_vllm = state.clone().npu()
    out_vllm = call_vllm(q_npu, k_npu, v_npu, b_npu, state_vllm, sl_npu, ss_npu, g_npu, nat_npu, scale)
    ...
    for _ in range(warmup):
        call_ops_transformer(q_npu, k_npu, v_npu, b_npu, state_bench, sl_npu, ss_npu, g_npu, nat_npu, scale)
    torch.npu.synchronize()
    ...
    state_bench = state.clone().npu()
    for _ in range(warmup):
        call_vllm(q_npu, k_npu, v_npu, b_npu, state_bench, sl_npu, ss_npu, g_npu, nat_npu, scale)
    torch.npu.synchronize()
    t0 = torch.npu.Event(enable_timing=True)
    t1 = torch.npu.Event(enable_timing=True)
    t0.record()
...
def profile_ours(dtype, b, mtp, nk, nv, dk, dv, num_cache_slots):
    ...
    call_ops_transformer(
        query, key, value, beta, state, actual_seq_lengths.npu(), ssm_state_indices, g, num_accepted_tokens, scale
    )
    torch.npu.synchronize()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--profile", action="store_true", help="single call to our op (for msprof)")
    ...
    if args.profile:
        if args.qwen:
            profile_ours(dtype_map[args.dtype], b=1, mtp=1, nk=8, nv=16, dk=128, dv=128, num_cache_slots=444)
        else:
            profile_ours(dtype_map[args.dtype], b=4, mtp=2, nk=4, nv=4, dk=64, dv=64, num_cache_slots=32)
        sys.exit(0)
