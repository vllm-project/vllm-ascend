...
#ifndef GENERIC_BLOCK_SPARSE_ATTENTION_METADATA_KERNEL_H
#define GENERIC_BLOCK_SPARSE_ATTENTION_METADATA_KERNEL_H

#include <cstdint>

// Kernel-side mirror of attention/sparse_attention_score_metadata/sparse_attention_score_metadata.h.
// Keep layout / indices / used-size in sync with AICPU EncodeMetadata.
namespace GsaMetadata {
...
constexpr uint32_t COMBINE_SCHEDULE_OFFSET =
    DECODE_SCHEDULE_OFFSET + MAX_DECODE_CORE_NUM * DECODE_SCHEDULE_FIELD_NUM;
constexpr uint32_t METADATA_USED_SIZE =
    COMBINE_SCHEDULE_OFFSET + MAX_COMBINE_TASK_NUM * COMBINE_SCHEDULE_FIELD_NUM;
...
// Packed overlay of the AICPU INT32[1024] protocol buffer (used region first).
struct DecodeSchedule {
    MetadataType baseTaskStart;
    MetadataType baseTaskEnd;
    MetadataType firstBlockStart;
    MetadataType lastBlockEnd;
};

struct CombineSchedule {
    MetadataType baseTask;
    MetadataType firstCore;
    MetadataType partialStart;
    MetadataType partialCount;
};
...
static_assert(sizeof(Metadata) == METADATA_TOTAL_SIZE * sizeof(MetadataType),
              "GsaMetadata::Metadata must overlay INT32[1024]");
static_assert(sizeof(DecodeSchedule) == DECODE_SCHEDULE_FIELD_NUM * sizeof(MetadataType),
              "DecodeSchedule layout mismatch");
static_assert(sizeof(CombineSchedule) == COMBINE_SCHEDULE_FIELD_NUM * sizeof(MetadataType),
              "CombineSchedule layout mismatch");

}  // namespace GsaMetadata

#endif  // GENERIC_BLOCK_SPARSE_ATTENTION_METADATA_KERNEL_H
