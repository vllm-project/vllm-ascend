...
public:
    ...
    __aicore__ inline
    GenericBlockSparseAttentionFdCombineArch22(Resource &resource)
    {
    ...
    __aicore__ inline
    void operator()(__gm__ const GsaMetadata::Metadata *meta,
                    ...
                    bool hasSeqUsedQ)
    {
        ...
        for (uint32_t work = aivIdx; work < workCount; work += aivNum) {
            ...
            uint32_t kvHead = 0U;
            if (!GsaFd::DecodeTaskStorage(baseTask, tilingData->kvHeads, tilingData->batch,
                gCuSeqLengths, gSeqUsedQ, hasSeqUsedQ, qStorageToken, qTokenInBatch, batchIdx, kvHead)) {
                continue;
            }
