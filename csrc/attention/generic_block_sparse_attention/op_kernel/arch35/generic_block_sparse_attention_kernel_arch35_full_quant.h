...
template <
    ...
    Format qFormat,
    Format kvFormat>
class GsaFullQuantKernelArch35 {
public:
    ...
    __aicore__ inline
    GsaFullQuantKernelArch35() {}

    __aicore__ inline
    void operator()(GsaFullQuantKernelParamsArch35 const &params)
    {
        __gm__ GenericBlockSparseAttn::GenericBlockSparseAttentionTilingData *gsaTilingData =
            reinterpret_cast<__gm__ GenericBlockSparseAttn::GenericBlockSparseAttentionTilingData *>(params.tiling);
        FetchBaseShapeInfo(gsaTilingData, params.metaData);
        CalcOnChipBufTileInfo(gsaTilingData);
        ...
        if (!GsaFd::ValidateMetadata(meta, gsaTilingData, coreNum)) {
            return;
        }
        const bool fdEnabled = gsaTilingData->fdStaticEnabled != 0U &&
        ...
        gPartialLse.SetGlobalBuffer((__gm__ float *)(params.workSpace + gsaTilingData->fdPartialLseOffset));
        AscendC::GlobalTensor<float> gPartialO;
        gPartialO.SetGlobalBuffer((__gm__ float *)(params.workSpace + gsaTilingData->fdPartialOOffset));
...
#ifdef __DAV_VEC__
        // Initialize storage padding which is intentionally absent from the
        // packed actual-token metadata schedule.
        if (hasSequsedQ) {
            ...
            if (kvSLoopNum == 0U) {
                ...
                if (isFdPartial) {
                    epilogueRescaleO.WriteNeutralPartial(gPartialO, gPartialLse, fdPartialTaskId,
                        groupSize, embed_, gsaTilingData->fdLseSubStride);
                } else {
                    epilogueRescaleO.WriteEmptyOutput(
                        gmOTensorTla, gmLseTensorTla, GemmCoord{groupSize, embed_, 0});
                }
                ...
                if (kvBlockIdx >= PRE_LAUNCH) {
                    ...
                    if (isFdPartial) {
                        epilogueRescaleO.ProcessPartial(
                            ...
                            gPartialO, gPartialLse, fdPartialTaskId, gsaTilingData->fdLseSubStride);
                    } else {
                        epilogueRescaleO(
                            gmOTensorTla, gmLseTensorTla, actualBlockShapePV,
                            curTileMod, kvBlockIdxDe, (kvBlockIdxDe == 0),
        ...
        if (fdEnabled) {
            ...
            combine(meta, gsaTilingData, gPartialLse, gPartialO, gO,
