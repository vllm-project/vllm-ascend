...
#ifndef GENERIC_BLOCK_SPARSE_ATTENTION_FD_COMBINE_ARCH35_H
#define GENERIC_BLOCK_SPARSE_ATTENTION_FD_COMBINE_ARCH35_H
...
class GenericBlockSparseAttentionFdCombineArch35 {
public:
    static constexpr uint32_t MAX_SPLIT_NUM = GsaMetadata::MAX_DECODE_CORE_NUM;
    static constexpr uint32_t MAX_ROW_TILE = 8U;
    static constexpr uint32_t FLOATS_PER_BLOCK = 8U;
    static constexpr uint32_t HEAD_DIM = 128U;
    ...
    __aicore__ inline explicit GenericBlockSparseAttentionFdCombineArch35(Resource &resource)
    {
    ...
    __aicore__ inline void operator()(
        ...
        CuSeqTensor &gCuSeqLengths,
        SeqUsedTensor &gSeqUsedQ,
        bool hasSeqUsedQ)
    {
        ...
        if (aivNum == 0U || combineTaskNum == 0U) {
            return;
        }
        ...
        const uint32_t lsePartialStride = 2U * lseSubStride;
        ...
        for (uint32_t work = aivIdx; work < workCount; work += aivNum) {
            ...
            const uint32_t tileInSubBlock = sourceSubBlock == 0U ? tileInTask : tileInTask - sub0Tiles;
            const uint32_t sourceRows = sourceSubBlock == 0U ? sub0Rows : sub1Rows;
            const uint32_t sourceLocalRow = tileInSubBlock * rowTile;
            const uint32_t rowCount = Min(rowTile, sourceRows - sourceLocalRow);
            const uint32_t groupRowStart = (sourceSubBlock == 0U ? 0U : rowSplit) + sourceLocalRow;
            ...
            PrepareWeights(gPartialLse, firstPartialTask, splitCount, splitCountAlign, sourceSubBlock,
                sourceLocalRow, rowCount, rowCountAlign, lseSubStride, lsePartialStride);
            ...
            if (!GsaFd::DecodeTaskStorage(baseTask, tilingData->kvHeads, tilingData->batch, gCuSeqLengths,
                gSeqUsedQ, hasSeqUsedQ, qStorageToken, qTokenInBatch, batchIdx, kvHeadIdx)) {
                continue;
            }
            const uint64_t outputOffset =
                (static_cast<uint64_t>(qStorageToken) * tilingData->numHeads + kvHeadIdx * groupSize +
                 groupRowStart) * HEAD_DIM;
            ...
            for (uint32_t split = 0U; split < splitCount; ++split) {
                const uint64_t partialOOffset =
                    (static_cast<uint64_t>(firstPartialTask + split) * groupSize + groupRowStart) * HEAD_DIM;
                ...
                for (uint32_t row = 0U; row < rowCount; ++row) {
                    AscendC::Muls(oTmpUb_[row * HEAD_DIM], oTmpUb_[row * HEAD_DIM],
                                  weightUb_.GetValue(split * rowCountAlign + row), HEAD_DIM);
                }
                AscendC::PipeBarrier<PIPE_V>();
            ...
            AscendC::DataCopyPad(gO[outputOffset], oOutUb_,
...
private:
    __aicore__ inline void PrepareWeights(
        AscendC::GlobalTensor<float> &gPartialLse, uint32_t firstPartialTask, uint32_t splitCount,
        uint32_t splitCountAlign, uint32_t sourceSubBlock, uint32_t sourceLocalRow, uint32_t rowCount,
        uint32_t rowCountAlign, uint32_t lseSubStride, uint32_t lsePartialStride)
    {
        ...
        AscendC::DataCopyPad(lseUb_, gPartialLse[lseOffset],
            ...
            AscendC::DataCopyPadExtParams<float>(true, 0, rowCountAlign - rowCount, -3.402823466e+38F));
    ...
    __aicore__ inline void BroadcastRows(const AscendC::LocalTensor<float> &src,
                                         uint32_t rowCountAlign, uint32_t splitCountAlign)
    {
        for (uint32_t split = 0U; split < splitCountAlign; ++split) {
    ...
    __aicore__ inline static uint32_t Min(uint32_t lhs, uint32_t rhs) { return lhs < rhs ? lhs : rhs; }
    __aicore__ inline static uint32_t Max(uint32_t lhs, uint32_t rhs) { return lhs > rhs ? lhs : rhs; }
...
#endif // GENERIC_BLOCK_SPARSE_ATTENTION_FD_COMBINE_ARCH35_H
