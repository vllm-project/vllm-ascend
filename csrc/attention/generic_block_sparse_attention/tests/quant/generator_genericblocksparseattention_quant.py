...
@GENERATOR_REGISTRY.register("generate_generic_block_sparse_attention_quant")
class GenericBlockSparseAttentionQuantGenerator(CaseGenerator):
    """arch35 (A5/950) full-quant GBSA cases — quantType=5, FP8 Q/K/V.

    Fixed (tiling/kernel):
      layoutQ=TND, layoutKv=PAGED_BBND, maskType=1, blockShape=[1,128], D=128
      quantType=5, isPackedGQA=1, returnSoftmaxlse=0, softmaxPrecision=1
      no dequant scales (all optional scale tensors nulled at runtime)

    Randomized:
      B, q_seqlen, kv_seqlen, Nkv, groupSize, topK, physical block headroom
      attentionOut dtype: fp16 or bf16
    """

    def __init__(self, config):
        super().__init__(config)
        self.qkv_dtype = "fp8e4m3"
    ...
    def after_input_config(
        ...
        if index in (1, 2) and input_case.name in ("key", "value"):
            input_case.dtype = self.qkv_dtype
    ...
    def after_case_config(self, case_config: CaseConfig) -> CaseConfig:
        ...
        case_config.inputs[1].dtype = self.qkv_dtype
        case_config.inputs[2].shape = [num_physical, PAGED_BLOCK_SIZE, kv_heads, HEAD_DIM]
        case_config.inputs[2].dtype = self.qkv_dtype
        ...
        case_config.inputs[22].range_values = 5  # quantType full FP8
        case_config.inputs[23].range_values = 0.0
        case_config.inputs[24].range_values = 1  # softmaxPrecision=1 on 950
        case_config.inputs[25].range_values = -1
        case_config.inputs[26].range_values = -1
