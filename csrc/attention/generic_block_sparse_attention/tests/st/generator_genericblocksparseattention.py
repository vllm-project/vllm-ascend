...
class GenericBlockSparseAttentionGenerator(CaseGenerator):
    """arch22 (ascend910b) non-quant GBSA cases.

    Fixed (tiling/kernel):
      layoutQ=TND, layoutKv=PAGED_BBND, maskType=1, blockShape=[1,128], D=128
      quantType=0, isPackedGQA=1, returnSoftmaxlse=0|1
      softmaxPrecision:
        fp16 -> 0|1 (float SM / half SM), align BSA ST innerPrecise
        bf16 -> 0 only (bf16+1 rejected by tiling, align BSA)
    ...
    def _softmax_precision(self) -> int:
        # arch22: bf16 only float SM; fp16 may exercise half SM (align BSA ST).
        if self.dtype == "bf16":
            return 0
    ...
    def after_case_config(self, case_config: CaseConfig) -> CaseConfig:
        ...
        case_config.inputs[24].range_values = self._softmax_precision()
