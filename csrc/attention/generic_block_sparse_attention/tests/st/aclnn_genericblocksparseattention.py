"""ST ATK golden: numpy ref_flash/ref_attention (950-style) + metadata sim (arch22).

Default plugin for:
  atk task ... -p aclnn_genericblocksparseattention.py
...
def decode_cu_and_seqused(cu_q, cu_kv, seqused_q=None, seqused_kv=None, total_q=None):
    ...
    Query tensor T must match sum(storage_q). Actual may be shorter (pad at segment end).
    Returns (actual_q, actual_kv, storage_q, storage_kv) or None.
    """
...
def apply_init_tensors(input_data, device="cpu"):
    """Rewrite cuSeq / sparse / blockTable / metadata; keep QKV as generated."""
...
class GenericBlockSparseAttentionInputProcess(AclnnBaseApi):
    ...
    def init_by_input_data(self, input_data):
        ...
        # Most cases leave seqused null; only sparse apply_init fills them.
...
class TestGenericBlockSparseAttentionNumpy:
    """950-style numpy golden; ST arch22 prec=0|1 via use_low_sm in ref_flash."""
    ...
    def ref_flash(cls, query, key, value, scale, np_dtype, use_low_sm):
        """Tiled flash bm. prec=1 half-SM; prec=0 fp32 SM + DownCastP before PV."""
...
class GenericBlockSparseAttentionApi(BaseApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        test_obj = TestGenericBlockSparseAttentionNumpy()
