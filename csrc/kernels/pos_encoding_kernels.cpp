...
template <typename scalar_t, bool isNeox> class RotaryEmbedding {
    static int constexpr strideSize = 32 * 8;
    // NOTE(ganyi): 910B have 192KB on chip memory, we use 32K as load stride for pipe, need to find another way to
...
public:
    ...
    __aicore__ inline void compute()
    {
        ...
        uint8_t loopCntMask = (embedDim_ + strideSize - 1) / strideSize;
        int32_t startVal = 0;
