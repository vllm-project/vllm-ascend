...
ge::graphStatus CausalConv1dUpdate::GetOpParam() {
    ...
    xShape_ = EnsureNotScalar_UPDATE_UPDATE(xShapePtr->GetStorageShape());
    OP_CHECK_IF(
    ...
    if (locShapePtr != nullptr && locShapePtr->GetStorageShape().GetDimNum() != 0) {
        locShape_ = EnsureNotScalar_UPDATE_UPDATE(locShapePtr->GetStorageShape());
    ...
    wShape_ = EnsureNotScalar_UPDATE_UPDATE(wShapePtr->GetStorageShape());
    ...
    convStateShape_ = EnsureNotScalar_UPDATE_UPDATE(convStateShapePtr->GetStorageShape());
    ...
    if (indicesShapePtr != nullptr && indicesShapePtr->GetStorageShape().GetDimNum() != 0) {
        stateIndicesShape_ = EnsureNotScalar_UPDATE_UPDATE(indicesShapePtr->GetStorageShape());
    ...
    if (biasShapePtr != nullptr && biasShapePtr->GetStorageShape().GetDimNum() != 0) {
        biasShape_ = EnsureNotScalar_UPDATE_UPDATE(biasShapePtr->GetStorageShape());
    ...
    if (numShapePtr != nullptr && numShapePtr->GetStorageShape().GetDimNum() != 0) {
        numShape_ = EnsureNotScalar_UPDATE_UPDATE(numShapePtr->GetStorageShape());
