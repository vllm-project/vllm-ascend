...
ge::graphStatus TilingAddRmsNormBiasRegbase(gert::TilingContext* context)
{
    ...
    // Gamma and optional beta each occupy a single vector; retain double buffering for all row queues.
    const uint64_t parameterBytes = numColAlign * dtypeBytes * (hasBeta ? 2 : 1);
