...
public:
                                   ...
                                   const DataCopyPadExtParams<T>& pad, const DataCopyExtParams& pout)
    {
        ...
        for (uint64_t k = 0; k < batchRows; ++k) {
            int64_t linearIndex = static_cast<int64_t>(indicesLocal_.GetValue(idx));
            // Compressor metadata pads unused rows with a negative linear
            // index. Match the no-sort path and leave the cache untouched.
            if (linearIndex >= 0) {
