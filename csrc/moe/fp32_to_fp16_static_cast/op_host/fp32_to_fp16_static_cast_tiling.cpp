...
#include "fp32_to_fp16_static_cast_tiling.h"
...
static ge::graphStatus TilingForFp32ToFp16StaticCast(gert::TilingContext* context)
{
    OPS_ERR_IF(context == nullptr,
        OPS_REPORT_VECTOR_INNER_ERR("Fp32ToFp16StaticCast", "Tiling context is null"),
    ...
    OPS_ERR_IF(xTensor->GetDataType() != ge::DT_FLOAT,
        OPS_LOG_E(context, "Fp32ToFp16StaticCast requires FP32 input."),
        return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(tileElements == 0 || tileElements > static_cast<uint64_t>(std::numeric_limits<uint32_t>::max()),
        OPS_LOG_E(context, "UB is insufficient for a valid FP32-to-FP16 double-buffer tile."),
        return ge::GRAPH_FAILED);

    Fp32ToFp16StaticCastTilingData tiling;
...
static ge::graphStatus TilingPrepareForFp32ToFp16StaticCast(gert::TilingParseContext* context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(Fp32ToFp16StaticCast)
    .Tiling(TilingForFp32ToFp16StaticCast)
    .TilingParse<Fp32ToFp16StaticCastCompileInfo>(TilingPrepareForFp32ToFp16StaticCast);
} // namespace optiling
