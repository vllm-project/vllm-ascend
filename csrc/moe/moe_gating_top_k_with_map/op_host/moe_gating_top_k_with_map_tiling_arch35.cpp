...
/* !
 * \file moe_gating_top_k_with_map_tiling_arch35.cpp
 * \brief
 */

#include "tiling_base/error_log.h"
#include "moe_gating_top_k_with_map_tiling.h"
...
#endif
namespace optiling {
const static uint64_t MOE_GATING_TOP_K_WITH_MAP_ASCEND_950_TILING_KEY = 10000;
...
class MoeGatingTopKWithMapTilingRegbase : public Ops::Transformer::OpTiling::TilingBaseClass {
public:
    explicit MoeGatingTopKWithMapTilingRegbase(gert::TilingContext *context) : Ops::Transformer::OpTiling::TilingBaseClass(context)
    {
        Reset();
    }
    ~MoeGatingTopKWithMapTilingRegbase() override = default;
...
private:
    ...
    const char *opName_ = "";
    MoeGatingTopKWithMapRegbaseTilingData moeGatingTopKTilingData_;
    platform_ascendc::SocVersion socVersion;
};

ge::graphStatus MoeGatingTopKWithMapTilingRegbase::CheckInputShape()
{
    ...
    const auto *mapShape = context_->GetInputShape(2);
    const auto *mapDesc = context_->GetInputDesc(2);
    OP_CHECK_IF(mapShape == nullptr || mapDesc == nullptr,
                OP_LOGE(context_, "log2phy is required"), return ge::GRAPH_FAILED);
    const auto &mapStorageShape = mapShape->GetStorageShape();
    OP_CHECK_IF(mapStorageShape.GetDimNum() != 1 || mapStorageShape.GetDim(0) != expertCount_ ||
                    mapDesc->GetDataType() != ge::DT_INT32,
                OP_LOGE(context_, "log2phy must be int32 [expert_count]"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(rows_ <= 0, OP_LOGE(context_, "rows must be positive"), return ge::GRAPH_FAILED);
...
ge::graphStatus MoeGatingTopKWithMapTilingRegbase::CheckAttr()
{
...
ge::graphStatus MoeGatingTopKWithMapTilingRegbase::GetShapeAttrsInfo()
{
...
ge::graphStatus MoeGatingTopKWithMapTilingRegbase::GetPlatformInfo()
{
...
ge::graphStatus MoeGatingTopKWithMapTilingRegbase::CheckOutShape()
{
...
void MoeGatingTopKWithMapTilingRegbase::CalTmpBufUbSize() {
...
void MoeGatingTopKWithMapTilingRegbase::SplitRows()
{
...
ge::graphStatus MoeGatingTopKWithMapTilingRegbase::DoOpTiling()
{
...
ge::graphStatus MoeGatingTopKWithMapTilingRegbase::DoLibApiTiling()
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeGatingTopKWithMapTilingRegbase::GetWorkspaceSize()
{
...
ge::graphStatus MoeGatingTopKWithMapTilingRegbase::PostTiling()
{
...
uint64_t MoeGatingTopKWithMapTilingRegbase::GetTilingKey() const
{
    return MOE_GATING_TOP_K_WITH_MAP_ASCEND_950_TILING_KEY;
}

void MoeGatingTopKWithMapTilingRegbase::Reset()
{
    opName_ = nullptr;
    return;
}

REGISTER_OPS_TILING_TEMPLATE(MoeGatingTopKWithMap, MoeGatingTopKWithMapTilingRegbase, 1000);
} // namespace optiling
