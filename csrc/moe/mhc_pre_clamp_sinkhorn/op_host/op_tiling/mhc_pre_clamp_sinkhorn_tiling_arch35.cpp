...
/*!
 * \file mhc_pre_clamp_sinkhorn_tiling_arch35.cpp
 * \brief
 */
...
MhcPreClampSinkhornTilingRegbase::MhcPreClampSinkhornTilingRegbase(gert::TilingContext *tilingContext) : context_(tilingContext)
{
}

ge::graphStatus MhcPreClampSinkhornTilingRegbase::GetPlatformInfo()
{
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::GetAttr()
{
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::GetShapeAttrsInfoInner()
{
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::CalcRegbaseOpTiling()
{
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::CalcRegbaseOpGradoutTiling()
{
    rowOfFormerBlock_ = CeilDiv(bs_, static_cast<int64_t>(aivCoreNum_));
    usedCoreNums_ = std::min(CeilDiv(bs_, rowOfFormerBlock_), static_cast<int64_t>(aivCoreNum_));
    rowOfTailBlock_ = bs_ - (usedCoreNums_ - 1) * rowOfFormerBlock_;
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::CalcMKSplitCorePart2Tiling()
{
    uint64_t kUbSize = tilingData_.get_kL1Size() / 2; // 先按2倍系数计算，m最大256，需保证kub小于256
    uint64_t mUbSize = CeilDiv(tilingData_.get_mL1Size(), 2);
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::CalcMKSplitCorePart2GradoutTiling()
{
    uint64_t kUbSize = tilingData_.get_kL1Size() / 2; // 先按2倍系数计算，m最大256，需保证kub小于256
    uint64_t mUbSize = CeilDiv(tilingData_.get_mL1Size(), 2);
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::CalcOpTiling()
{
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::DoOpTiling()
{
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::GetWorkspaceSize()
{
...
ge::graphStatus MhcPreClampSinkhornTilingRegbase::PostTiling()
{
