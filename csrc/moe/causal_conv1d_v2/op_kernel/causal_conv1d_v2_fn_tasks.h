...
/*!
 * \file causal_conv1d_v2_fn_tasks.h
 * \brief CausalConv1dV2 FN per-core task resolution and chunk processing.
 */

#ifndef CAUSAL_CONV1D_V2_FN_TASKS_H
#define CAUSAL_CONV1D_V2_FN_TASKS_H
...
__aicore__ inline int32_t CAUSAL_CONV1D_V2_CLASS::FindVarlenSeqByToken(int32_t tokenIdx) const
{
    int32_t left = 0;
    int32_t right = static_cast<int32_t>(tilingData_->batch);
    while (left < right) {
        const int32_t mid = left + ((right - left) >> 1);
        const int64_t endVal = ReadQueryStartLoc(mid + 1);
        if (tokenIdx < endVal) {
            right = mid;
        } else {
            left = mid + 1;
...
template <CAUSAL_CONV1D_V2_TEMPLATE_ARGS>
__aicore__ inline bool CAUSAL_CONV1D_V2_CLASS::ResolveExplicitTokenTileSeqRange(int32_t tokenTileId, int32_t &startSeq,
...
template <CAUSAL_CONV1D_V2_TEMPLATE_ARGS>
__aicore__ inline void CAUSAL_CONV1D_V2_CLASS::InitRingSeqSplit(int32_t seq, int32_t cacheIdx, bool hasInit,
...
template <CAUSAL_CONV1D_V2_TEMPLATE_ARGS>
__aicore__ inline void CAUSAL_CONV1D_V2_CLASS::ProcessFnChunk(int32_t seq, int32_t cacheIdx, bool hasInit,
...
CAUSAL_CONV1D_V2_CLASS::MaybeWriteBackSeqSplitTailChunk(int32_t chunkStart, int32_t chunkLen, int32_t seqStart,
...
template <CAUSAL_CONV1D_V2_TEMPLATE_ARGS>
__aicore__ inline void CAUSAL_CONV1D_V2_CLASS::PrefetchInitStatesToWorkspace(
    int32_t channelStart, int32_t baseDimSize, int32_t ownerIndex, int32_t ownerCount)
{
    ...
    for (int32_t seq = ownerIndex; seq < batch; seq += ownerCount) {
        int32_t seqStart = 0;
        int32_t seqLen = 0;
        if (!ResolveSeqTaskWindow(seq, tilingData_->inputMode, tilingData_->seqLen, seqStart, seqLen) ||
            !ResolveSeqHasInit(seq, hasInitialState)) {
            continue;
        }
...
template <CAUSAL_CONV1D_V2_TEMPLATE_ARGS>
__aicore__ inline void CAUSAL_CONV1D_V2_CLASS::ProcessVarlenTokenTiled()
{
    const int32_t dim = tilingData_->dim;
    const int32_t batch = tilingData_->batch;
