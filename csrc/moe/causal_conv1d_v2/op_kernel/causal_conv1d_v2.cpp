...
/*!
 * \file causal_conv1d_v2.cpp
 * \brief CausalConv1dV2 kernel entry functions.
 */

#include "causal_conv1d_v2_fn.h"
#include "causal_conv1d_v2_update.h"

namespace {

template <typename T, uint32_t runModeKey, uint32_t widthKey, uint32_t fnPlanKey>
__aicore__ inline void RunCausalConv1dV2(GM_ADDR x, GM_ADDR weight, GM_ADDR bias, GM_ADDR convStates,
...
{
    if constexpr (runModeKey == CAUSAL_CONV1D_V2_TPL_RUN_MODE_FN) {
        NsCausalConv1dV2::RunCausalConv1dV2Fn<T, widthKey, fnPlanKey>(
            x, weight, bias, convStates, queryStartLoc, cacheIndices, hasInitialState, numAcceptedTokens, y, workspace,
            tilingData);
    } else {
        NsCausalConv1dV2::RunCausalConv1dV2Update<T>(
            x, weight, bias, convStates, queryStartLoc, cacheIndices, hasInitialState, numAcceptedTokens, y, workspace,
            tilingData);
    }
}
...
__global__ __aicore__ void causal_conv1d_v2(GM_ADDR x, GM_ADDR weight, GM_ADDR bias, GM_ADDR convStates,
                                         GM_ADDR queryStartLoc, GM_ADDR cacheIndices, GM_ADDR hasInitialState,
...
{
    REGISTER_TILING_DEFAULT(CausalConv1dV2TilingData);
    ...
    RunCausalConv1dV2<DTYPE_X, runModeKey, widthKey, fnPlanKey>(
