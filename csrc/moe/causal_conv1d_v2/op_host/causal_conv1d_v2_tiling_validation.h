...
/*!
 * \file causal_conv1d_v2_tiling_validation.h
 * \brief CausalConv1dV2 tiling input validation and shape/dtype parsing.
 */

#ifndef CAUSAL_CONV1D_V2_TILING_VALIDATION_H
#define CAUSAL_CONV1D_V2_TILING_VALIDATION_H
...
inline ge::graphStatus ResolveMetadataInputs(gert::TilingContext *context, ResolvedMetadataInputs &inputs)
{
    ...
    if (inputs.queryStartLocUseCpu) {
        OP_LOGW(context,
                "CausalConv1dV2 host-array input queryStartLocCpu is deprecated and will be removed after "
                "December 2026; use the device Tensor input queryStartLoc instead.");
    }
    if (inputs.cacheIndicesUseCpu) {
        OP_LOGW(context,
                "CausalConv1dV2 host-array input cacheIndicesCpu is deprecated and will be removed after "
                "December 2026; use the device Tensor input cacheIndices instead.");
    }
    if (inputs.hasInitialStateUseCpu) {
        OP_LOGW(context,
                "CausalConv1dV2 host-array input hasInitialStateCpu is deprecated and will be removed after "
                "December 2026; use the device Tensor input hasInitialState instead.");
    }
    if (inputs.numAcceptedTokensUseCpu) {
        OP_LOGW(context,
                "CausalConv1dV2 host-array input numAcceptedTokensCpu is deprecated and will be removed after "
...
inline ge::graphStatus GetAttrsInfo(gert::TilingContext *context, CausalConv1dV2AttrInfo &attrInfo)
{
...
inline ge::graphStatus ValidateAlignedDim(gert::TilingContext *context, int64_t dim)
{
    OP_CHECK_IF(dim % DIM_ALIGN_ELEMS != 0,
                OP_LOGE(context,
                        "dim must satisfy dim %% %ld == 0 for causal_conv1d_v2; "
...
inline ge::graphStatus GetShapeDtypeInfo(gert::TilingContext *context, const CausalConv1dV2AttrInfo &attrInfo,
                                         CausalConv1dV2TilingData &tiling, bool &hasBias)
{
    ...
    if (hasConvStates) {
        ...
        if (inputStride != nullptr && inputStride->GetDimNum() == 3) {
            ...
            OP_CHECK_IF(stride0 <= 0 || stride1 <= 0 || stride2 != 1,
                        ...
                        return ge::GRAPH_FAILED);
            const int64_t maxOffset = std::numeric_limits<int64_t>::max();
            OP_CHECK_IF(stride1 < sDim ||
                            (stateLen - 1) > (maxOffset - sDim) / stride1 ||
                            stride0 < (stateLen - 1) * stride1 + sDim,
                        OP_LOGE(context, "convStates history rows or cache slots overlap, or stride extent overflows"),
                        return ge::GRAPH_FAILED);
            const int64_t slotExtent = (stateLen - 1) * stride1 + sDim;
            OP_CHECK_IF((numCacheLines - 1) > (maxOffset - slotExtent) / stride0,
                        OP_LOGE(context, "convStates pool extent overflows int64"), return ge::GRAPH_FAILED);
            tiling.convStateStride0 = stride0;
            tiling.convStateStride1 = stride1;
        } else {
            OP_CHECK_IF(stateLen > std::numeric_limits<int64_t>::max() / sDim ||
                            numCacheLines > std::numeric_limits<int64_t>::max() / (stateLen * sDim),
                        OP_LOGE(context, "dense convStates extent overflows int64"), return ge::GRAPH_FAILED);
    ...
    if (!qslAbsent && isDecodeMode && inputMode == 2) {
        ...
        // Supplied queryStartLoc remains authoritative during graph replay.
        {
            inputMode = 0;
            cuSeqlen = xShape.GetDim(0);
            batch = batchFromQsl;
    ...
    if (ciShapePtr != nullptr) {
        ...
        if (!ciAbsent) {
            ...
            // Match the original conv: speculative indices are [batch, columns],
            // and each sequence reads its first column inside the kernel.
            ...
            if (HasVisibleMetadataData(ciTensor, tiling.cacheIndicesDtype)) {
                for (int64_t i = 0; i < batch; ++i) {
                    const int64_t v =
                        ReadVisibleMetadataValue(ciTensor, tiling.cacheIndicesDtype, i * tiling.cacheIndicesStride);
