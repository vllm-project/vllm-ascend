...
/*!
 * \file causal_conv1d_v2_tiling_planner.h
 * \brief CausalConv1dV2 tiling planning utilities.
 */

#ifndef CAUSAL_CONV1D_V2_TILING_PLANNER_H
#define CAUSAL_CONV1D_V2_TILING_PLANNER_H

#include "causal_conv1d_v2_tiling_utils.h"
#include "../op_kernel/causal_conv1d_v2_tiling_data.h"
...
inline DimTileChoice ChooseCanonicalUpdateBaseDimChoice(gert::TilingContext *context, int64_t batch, int64_t dim,
                                                        uint32_t coreNum)
{
    const int64_t candidates[] = {4096, 2048, 1152, 1024, 576, 512, 384, 192};
...
inline VarlenTokenTileChoice ChooseUnifiedFnTokenBlockPlan(gert::TilingContext *context,
                                                           const CausalConv1dV2TilingData &tiling,
...
inline FnHostPlan ChooseFnHostPlan(gert::TilingContext *context, const CausalConv1dV2TilingData &tiling, uint64_t ubSize,
                                   uint32_t coreNum)
{
    FnHostPlan plan;
    if ((tiling.inputMode != 0 && tiling.inputMode != 1) || tiling.batch <= 0 || tiling.cuSeqlen <= 0 ||
