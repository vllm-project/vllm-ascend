 ...
 class KernelComputeWy {
  public:
   __aicore__ inline void Init(GM_ADDR q, GM_ADDR k, GM_ADDR v, GM_ADDR g, GM_ADDR beta, GM_ADDR qKernel,
                               GM_ADDR kKernel, GM_ADDR wKernel, GM_ADDR uKernel, GM_ADDR gKernel,
   ...
   __aicore__ inline void Process() {
     ...
     const uint32_t blockNum = GetBlockNum() == 0 ? 1 : GetBlockNum();
  ...
  private:
   ...
   // Load strided [B,T,H] scalars for one head over a chunk.
   // Prefer one contiguous MTE2 DataCopy of (chunk * numHeads), then gather from UB.
   // Do NOT use DataCopy with blockLen=1 into a 64-elem dst: each block is 32B and
   // would write 8 floats per row (overflow). Fallback to GM GetValue if scratch is short.
   ...
   __aicore__ inline void BuildAttnStrictLower(LocalTensor<float> attnLocal, const LocalTensor<float> kBetaLocal,
                                               const LocalTensor<float> kLocal, const LocalTensor<float> expGLocal,
                                               const LocalTensor<float> mulTmp, uint32_t headDim, uint32_t lda) {
     // k_beta @ k^T via vector Mul; decay = exp(g_cum[i]-g_cum[j]) = expG[i]/expG[j].
     vectorGemm_.GemmATransB(attnLocal, kBetaLocal, kLocal, mulTmp, FIXED_CHUNK_SIZE, headDim, lda, lda);
     for (uint32_t i = 0; i < FIXED_CHUNK_SIZE; ++i) {
   ...
   __aicore__ inline void ProcessOneTask(uint32_t b, uint32_t kHeadIdx, uint32_t vHeadIdx, uint32_t chunkIdx) {
     ...
     // Load k for local WY math. q/k kernel outputs are written after compute
     // to avoid reusing the same UB buffer while MTE3 is still reading it.
     ...
     SyncEvent<HardEvent::MTE3_MTE2>(HardEvent::MTE3_MTE2);
     // kHalf is free after Cast; fence V before MTE2 reuses it for beta staging.
     ...
     BuildAttnStrictLower(attnLocal, kBeta, kFloat, expGLocal, scratch, kHeadDim_, alignK_);
     ...
     vectorGemm_.GemmAttnB(uAcc, attnLocal, vFloat, scratch, FIXED_CHUNK_SIZE, vHeadDim_, alignV_, alignV_);
     vectorGemm_.GemmAttnB(kFloat, attnLocal, kBeta, scratch, FIXED_CHUNK_SIZE, kHeadDim_, alignK_, alignK_);
     ...
     // Final fence before leaving the task to avoid pending GM writes.
   ...
   WyVectorGemm vectorGemm_;
   TBuf<TPosition::VECCALC> kHalfBuf_, qHalfBuf_, vHalfBuf_, kFloatBuf_, vFloatBuf_, kBetaBuf_, uAccBuf_, attnBuf_, gBuf_, expGBuf_, tmpBuf_, rowBuf_;
 };
