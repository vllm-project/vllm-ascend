...
#include "apply_top_k_top_p_custom_opt.h"
using namespace AscendC;
using namespace ApplyTopKTopPCustomOp;
using namespace ApplyTopPCustomOp;
using namespace ApplyTopKTopPCustomOptOp;
                                                                    ...
                                                                    GM_ADDR workSpace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(0)) {
        ...
        ApplyTopKTopPCustomOptOp::ApplyTopKTopPCustomOpt<DTYPE_OUT, float, DTYPE_OUT> op;
        op.InitTilingData(tilingData, sorted_value, sorted_indices, p, k, logits, out, workSpace);
        op.InitBuffer(&pipe);
