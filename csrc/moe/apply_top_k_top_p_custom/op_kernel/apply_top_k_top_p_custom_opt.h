...
#ifndef APPLY_TOP_K_TOP_P_CUSTOM_OPT_H_KERNEL
#define APPLY_TOP_K_TOP_P_CUSTOM_OPT_H_KERNEL

#include "kernel_operator.h"

using namespace AscendC;
namespace ApplyTopKTopPCustomOptOp {
...
class ApplyTopKTopPCustomOpt {
public:
    __aicore__ inline ApplyTopKTopPCustomOpt(){};
...
// --- InitTilingData ---
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::InitTilingData(
...
// --- InitBuffer ---
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::InitBuffer(TPipe* inputPipe)
{
    pipe_ = inputPipe;
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::GetMaxValue(int64_t baseGmIdx)
{
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::ComputeSoftmaxSum(int64_t baseGmIdx, float kthVal,
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::WriteSoftmaxToGm(int64_t baseGmIdx,
                                                                                 uint32_t kthStartIdx)
{
    uint32_t loopDataNum = softmaxLength;
    for (uint32_t loopInner = 0; loopInner < lineSfLoopTimes; loopInner++) {
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::CumsumKoggleStone(int64_t baseGmIdx)
{
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::FindFirstIndex(int64_t baseGmIdx, uint32_t& firstIdx)
{
    firstIdx = vocabSize_;
    bool found = false;
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::ScatterBoundary(int64_t batchGmBase, int64_t sortedBase,
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::CopyOutLast(int64_t batchGmBase, int64_t sortedBase)
{
    constexpr uint32_t DATA_PER_BLOCK = BLOCK_BYTES / sizeof(inputT);
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline uint32_t ApplyTopKTopPCustomOpt<inputT, calT, outputT>::FindKthStartIdx(int64_t sortedBase, int32_t kValue)
{
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::FillNegInfAndScatterTopK(int64_t batchGmBase,
                                                                                         int64_t sortedBase,
                                                                                         uint32_t kthStartIdx)
{
    constexpr uint32_t DATA_PER_BLOCK = BLOCK_BYTES / sizeof(inputT);
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::ProcessTopKTopPOpt()
{
    for (uint32_t loopBatch = 0; loopBatch < loopBatch_; loopBatch++) {
        ...
        if (*reinterpret_cast<int32_t*>(&maxValue) == FLOAT32_NEG_INF ||
            *reinterpret_cast<int32_t*>(&maxValue) == FLOAT32_INF) {
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::ProcessTopKOpt()
{
    for (uint32_t loopBatch = 0; loopBatch < loopBatch_; loopBatch++) {
        int64_t sortedBase = (batchOffset_ + loopBatch) * vocabSize_;
        int64_t batchGmBase = sortedBase;
...
template <typename inputT, typename calT, typename outputT>
__aicore__ inline void ApplyTopKTopPCustomOpt<inputT, calT, outputT>::ProcessTopPOpt()
{
    for (uint32_t loopBatch = 0; loopBatch < loopBatch_; loopBatch++) {
        int64_t sortedBase = (batchOffset_ + loopBatch) * vocabSize_;
        int64_t batchGmBase = sortedBase;
