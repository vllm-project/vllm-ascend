...
// List all dtypes supported by the API definition.
...
{
    // Check whether input dtypes are supported.
                                               ...
                                               aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    if (logits->IsEmpty() || pIsEmpty || kIsEmpty) {
        // Empty tensors need no workspace.
        *workspaceSize = 0;
    ...
    if (isLastDimSizeOne) {
        ...
        // Copy the result into out, which may be non-contiguous.
        viewCopyResult = l0op::ViewCopy(res, out, uniqueExecutor.get());
    }
    CHECK_RET(viewCopyResult != nullptr, ACLNN_ERR_INNER_NULLPTR);
    // Get the workspace size required by the executor.
