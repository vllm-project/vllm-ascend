...
#include "bf16_to_fp16_static_cast_tiling.h"
...
static ge::graphStatus TilingForBf16ToFp16StaticCast(gert::TilingContext* context)
{
    OPS_ERR_IF(context == nullptr,
        OPS_REPORT_VECTOR_INNER_ERR("Bf16ToFp16StaticCast", "Tiling context is null"),
    ...
    OPS_ERR_IF(xTensor->GetDataType() != ge::DT_BF16,
        OPS_LOG_E(context, "Bf16ToFp16StaticCast requires BF16 input."),
        return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(tileElements == 0 || tileElements > static_cast<uint64_t>(std::numeric_limits<uint32_t>::max()),
        OPS_LOG_E(context, "UB is insufficient for a valid BF16-to-FP16 double-buffer tile."),
        return ge::GRAPH_FAILED);

    Bf16ToFp16StaticCastTilingData tiling;
...
static ge::graphStatus TilingPrepareForBf16ToFp16StaticCast(gert::TilingParseContext* context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(Bf16ToFp16StaticCast)
    .Tiling(TilingForBf16ToFp16StaticCast)
    .TilingParse<Bf16ToFp16StaticCastCompileInfo>(
        TilingPrepareForBf16ToFp16StaticCast);
} // namespace optiling
