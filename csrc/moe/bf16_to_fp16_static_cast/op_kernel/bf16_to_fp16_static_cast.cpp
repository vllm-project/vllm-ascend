...
class Bf16ToFp16StaticCastKernel {
public:
    __aicore__ inline void Init(
        GM_ADDR x, GM_ADDR y, const Bf16ToFp16StaticCastTilingData* tilingData)
    {
        ...
        yGm_.SetGlobalBuffer(reinterpret_cast<__gm__ half*>(y) + start, blockElementNum_);
        pipe_.InitBuffer(xQueue_, 2, static_cast<uint32_t>(tileElementNum_ * sizeof(bfloat16_t)));
        pipe_.InitBuffer(yQueue_, 2, static_cast<uint32_t>(tileElementNum_ * sizeof(half)));
        pipe_.InitBuffer(fp32Buffer_, static_cast<uint32_t>(tileElementNum_ * sizeof(float)));
    }
...
private:
    __aicore__ inline void CopyIn(int64_t offset, uint32_t count)
    {
        ...
        DataCopyPadExtParams<bfloat16_t> padParams{false, 0, 0, static_cast<bfloat16_t>(0)};
    ...
    __aicore__ inline void Compute(uint32_t count)
    {
        ...
        LocalTensor<float> fp32Local = fp32Buffer_.Get<float>();
        Cast(fp32Local, xLocal, RoundMode::CAST_NONE, count);
        PipeBarrier<PIPE_V>();
        Cast(yLocal, fp32Local, RoundMode::CAST_RINT, count);
    ...
    __aicore__ inline void CopyOut(int64_t offset, uint32_t count)
    {
        ...
        DataCopyExtParams copyParams{
            1, count * static_cast<uint32_t>(sizeof(half)), 0, 0, 0};
    ...
    TBuf<TPosition::VECCALC> fp32Buffer_;
    int64_t blockElementNum_ = 0;
    int64_t tileElementNum_ = 0;
};

extern "C" __global__ __aicore__ void bf16_to_fp16_static_cast(
    GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling)
{
    (void)workspace;
    GET_TILING_DATA(tilingData, tiling);
    Bf16ToFp16StaticCastKernel op;
