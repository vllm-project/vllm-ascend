...
ge::graphStatus Tiling4ChunkFwdH(gert::TilingContext *context)
{
    ...
    bool useScalarGate = gTensor != nullptr;
    bool useGk = gkTensor != nullptr;
    OP_CHECK_IF(useScalarGate == useGk,
    ...
    if ((useScalarGate && logicalVHeads % logicalKHeads != 0) || (useGk && logicalKHeads != logicalVHeads)) {
        OP_LOGE(context->GetNodeName(),
                "g-only requires HV %% HK == 0; gk-only requires k/ kg head count equal HV. "
                "Current mode useScalarGate=%d, useGk=%d, HK=%ld, HV=%ld.",
                useScalarGate, useGk, logicalKHeads, logicalVHeads);
        return ge::GRAPH_FAILED;
    }
