...
#include "bf16_to_fp32_static_cast_tiling.h"
...
constexpr int64_t kInputDmaAlignElements = 16;
constexpr uint64_t kInputBytes = 2;
constexpr uint64_t kOutputBytes = 4;
constexpr uint64_t kDoubleBufferBytesPerElement = 2 * (kInputBytes + kOutputBytes);
...
static ge::graphStatus TilingForBf16ToFp32StaticCast(gert::TilingContext* context)
{
    OPS_ERR_IF(context == nullptr,
        OPS_REPORT_VECTOR_INNER_ERR("Bf16ToFp32StaticCast", "Tiling context is null"),
        return ge::GRAPH_FAILED);

    const gert::Tensor* xTensor = context->GetInputTensor(kInputX);
    OPS_LOG_E_IF_NULL(context, xTensor, return ge::GRAPH_FAILED);
    OPS_ERR_IF(xTensor->GetDataType() != ge::DT_BF16,
        OPS_LOG_E(context, "Bf16ToFp32StaticCast requires BF16 input."),
        return ge::GRAPH_FAILED);
    ...
    const uint64_t rawTileElements = ubSize / kDoubleBufferBytesPerElement;
    ...
    Bf16ToFp32StaticCastTilingData tiling;
    tiling.set_elementNum(elementNum);
    tiling.set_blockFormer(blockFormer);
    tiling.set_ubFormer(static_cast<int64_t>(tileElements));
    // blockFormer and ubFormer are aligned; only the final tile needs checking.
    tiling.set_useAlignedCopy(elementNum % kInputDmaAlignElements == 0);
...
static ge::graphStatus TilingPrepareForBf16ToFp32StaticCast(gert::TilingParseContext* context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(Bf16ToFp32StaticCast)
    .Tiling(TilingForBf16ToFp32StaticCast)
    .TilingParse<Bf16ToFp32StaticCastCompileInfo>(TilingPrepareForBf16ToFp32StaticCast);
} // namespace optiling
