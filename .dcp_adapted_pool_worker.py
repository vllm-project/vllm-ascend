...
class KVPoolWorker:
    ...
    def _init_parallelism_info(self, model_config, parallel_config) -> None:
        ...
        try:
            ...
            logger.warning(
                "Failed to get PP layer indices (pp_size=%d), falling back to "
                "local layer addressing: %s",
                self.pp_size,
                e,
            )
            self.pp_layer_offset = 0
    ...
    def _build_group_layer_builders(self) -> list[LayerBatchBuilder]:
        ...
        for group_id in range(self.num_kv_cache_groups):
            ...
            # Global byte offset for the shared GVA region:
            #   PP: stage offset = pp_layer_offset * per_layer_bytes.
            #   DCP (shard-major): shard d occupies
            #       [d * per_layer * n_global, (d+1) * per_layer * n_global),
            #   so the shard base is added on top of the PP layer offset.
            layer_byte_offset = 0
            ...
            if gbl and group_num_layers > 0:
                ...
                if cp_scale > 1:
                    ...
                    if total_layers and isinstance(total_layers, int):
                        n_global = max(total_layers, int(self.num_layers), group_num_layers)
                        layer_byte_offset += getattr(self, "dcp_rank", 0) * per_layer * n_global
            builders.append(
                LayerBatchBuilder(
                    self.token_database,
                    group_page_size,
    ...
    def _global_group_alloc_size(self, group_id: int) -> int:
        ...
        # Each (pcp, dcp) shard holds a slice of every logical block; the
        # shared region must cover the union of ALL shards (shard-major
        # layout: shard d occupies [d * per_layer * n_global, ...)).
        cp_scale = getattr(self, "pcp_size", 1) * getattr(self, "dcp_size", 1)
        return per_layer * n_global * cp_scale
    ...
    def _is_layerwise_save_leader(self) -> bool:
        ...
        shard, and the put_step groups span DIFFERENT shards -- the plain
