...
| 规范编号 | 规范名称 | 编译选项 | 严重级别 |
|---------|---------|---------|---------|
| 1 | 打开地址随机化选项 | `-fPIC` `-fPIE -pie` | 要求 |
| 2 | 打开栈保护选项 | `-fstack-protector-all` | 要求 |
| 3 | 打开 GOT 表重定位只读选项 | `-Wl,-z,relro` | 要求 |
| 4 | 打开立即绑定选项 | `-Wl,-z,now` | 要求 |
| 5 | 打开堆栈不可执行选项 | `-Wl,-z,noexecstack` | 要求 |
| 6 | 打开 -s 选项或使用 strip 工具 | `-s` 或 `strip` | 要求 |
| 7 | 禁止使用 rpath 选项 | 禁止 `-Wl,--rpath` | 要求 |

> **说明**：以上 7 条规范全部仅适用于 Tiling 侧，Kernel 侧不适用。

---

### 打开地址随机化选项 `[适用: Tiling]`

> **Kernel 侧不适用**：NPU 执行模型不同，不适用 ASLR。
...
### 打开GOT表重定位只读选项 `[适用: Tiling]`

> **Kernel 侧不适用**：Kernel 不是 ELF 格式。
...
### 打开堆栈不可执行选项 `[适用: Tiling]`

> **Kernel 侧不适用**：Kernel 执行模型不同。
...
### 打开-s选项或者使用strip工具 `[适用: Tiling]`

> **Kernel 侧不适用**：Kernel 二进制格式不同。
...
### 禁止使用rpath选项 `[适用: Tiling]`

> **Kernel 侧不适用**：Kernel 不涉及动态库路径搜索。
