...
> 性能收益折线图与最终记分板。原始实验记录（含全部 profile 数据与调试过程）见
> `RMS_NORM_CAST_OPT_NOTES.md`。
...
| CANN | 9.1.0（`/usr/local/Ascend/cann-9.1.0`），驱动 npu-smi 25.5.1 |
...
| 候选 | 结论 |
|---|---|
| 双行批处理（省 widen+平方两遍 pass，≈-14% vec） | 需 30B/col = 215KB **> 191KB UB 上限，被硬件阻断** |
| Axpy 合并标量链 | `dst=a·src+dst` 累加语义破坏加数，op 数不省，**死路** |
| 预取深度 2 | MTE2（0.19µs/行）从来不是约束，**无关键路径收益** |
| 小 shape 专用精简路径 | 发射瓶颈封顶 ~0.2µs，ROI 不足，**留档不做** |
| DataCopyPad→对齐 DataCopy | MTE 已全部移出关键路径，**无收益** |

**重启触发条件**（已留档）：CANN 提供 bf16 元素级算子/点积指令、调用方 hidden
变化、更大 UB 的硬件。
...
> 噪声 ±3%，孤立口径 kernel 线性扩展）不列入折线，完整数据见 §9 记分板。
...
### 8.4 关键 shape 绝对耗时演进（2048 tokens）
...
## 9. 最终记分板（NPUGraph，µs/次，hidden=7168）
