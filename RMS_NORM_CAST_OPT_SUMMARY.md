...
**遗留**：vec 占比 61%，但绝对空转扩大到约 31µs——行间串行成为下一轮目标。
...
提升 24–29%；**vec 占比 61%→91%**，MTE2/MTE3 时间不变但全部移出关键路径；
...
- x 行 0 的载入先上 MTE2（首位），gamma 次之；gamma 的等待（及 bf16 的宽化
  ...
  每核省约 0.15µs 启动串行（少行数/核的 shape 占比更高）；
- 行不变常量 1.0f 外提到每核一次（原每行 `Duplicate`），每行省 1 op + 1 barrier。

**结果**：64–1024 tokens 稳定提升 **4–8%**（如 bf16 128: 8.83→8.20µs）；
...
（提速为正 = 变快。1–4 tokens 变慢 0.1–0.3µs，为发射瓶颈导致的结构权衡，见 §6。）
