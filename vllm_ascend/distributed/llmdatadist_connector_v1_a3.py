    ...
    def __init__(self, vllm_config: VllmConfig, role: KVConnectorRole):
        ...
        if role == KVConnectorRole.SCHEDULER:
            self.connector_scheduler: Optional[
                LLMDataDistConnectorScheduler] = LLMDataDistConnectorScheduler(
...
class LLMDataDistConnectorWorker():
  ...
  """
    ...
    def listen_for_agent_metadata_req(self, event: threading.Event):
        ...
        logger.info(
            f"LLMDataDistConnectorWorker: Cluster {self.local_agent_metadata.cluster_id} start to listen request from peers"
        )
        with zmq_ctx(zmq.ROUTER, url) as sock:
            ...
            while True:
                ...
                if "cluster_id" in decode_msg:
                    ...
                    logger.info(
                        f"LLMDataDistConnectorWorker: Receive message from cluster {decode_msg.cluster_id}"
                    )
                    sock.send_multipart((identity, b"", msg_to_send))
                    self.add_remote_agent(decode_msg)
                else:
                    logger.warning(
                        f"LLMDataDistConnectorWorker: receiving unrecognized data {decode_msg}"
                    )
    ...
    def register_kv_caches(self, kv_caches: dict[str, Tuple[torch.Tensor]]):
        ...
        if self.use_mla:
            ...
            try:
                ...
                raise RuntimeError(
                    f"LLMDataDistConnectorWorker: Passing unexpected parameter to register_block_cache, receiving [cache_desc: {self.cache_desc}, cache_addr: {self.cache_addr}, cache_key: {self.cache_key}]"
                )
        else:
            for cache_or_caches in kv_caches.values():
    ...
    def add_remote_agent(self, metadata: LLMDataDistAgentMetadata) -> int:
        ...
        if remote_cluster_id in self.linked_cluster:
            logger.debug(
                f"LLMDataDistConnectorWorker: remote cluster_id: {metadata.cluster_id} already linked with this server, skip the connection"
            )
        ...
        logger.info(
            f"LLMDataDistConnectorWorker: try link with remote, comm id: {comm_name}"
        )
        ...
        while True:
            ...
            if ret == llm_datadist.RegisterMemStatus.OK:
                logger.info(
                    f"LLMDataDistConnectorWorker: Linking success, comm id: {comm_id}"
                )
                break
            elif ret == llm_datadist.RegisterMemStatus.FAILED:
                raise RuntimeError(
                    f"LLMDataDistConnectorWorker: Linking failed, comm id: {comm_id}"
                )
            time.sleep(1)
    ...
    def remove_remote_agent(self, cluster_id: int):
        if cluster_id not in self.linked_cluster:
            logger.warning(
                f"LLMDataDistConnectorWorker: Warning! Can't remove remote client with cluster id {cluster_id} for its not exist in linked_cluster list"
            )
    ...
    def _read_blocks(
        ...
        if self.use_mla:
            ...
            try:
                ...
                raise RuntimeError(
                    f"LLMDataDistConnectorWorker: Passing unexpected parameter to pull_blocks remote_cache_key: {remote_cache_key_k_normed} {remote_cache_key_k_pe}, cache: {self.cache}, local_block_ids: {local_block_ids}, remote_block_ids: {remote_block_ids}"
                )
            except LLMException:
                raise RuntimeError(
                    "LLMDataDistConnectorWorker: Timeout during pull_blocks, you can try to increase the sync_kv_timeout config or checking your connect status"
                )
        else:
            ...
            try:
                ...
                raise RuntimeError(
                    f"LLMDataDistConnectorWorker: Passing unexpected parameter to pull_blocks remote_cache_key: {remote_cache_key}, cache: {self.cache}, local_block_ids: {local_block_ids}, remote_block_ids: {remote_block_ids}"
                )
            except LLMException:
                raise RuntimeError(
                    "LLMDataDistConnectorWorker: Timeout during pull_blocks, you can try to increase the sync_kv_timeout config or checking your connect status"
