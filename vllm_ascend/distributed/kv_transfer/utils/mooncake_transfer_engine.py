...
def _verify_heterogeneous_mooncake() -> None:
    ...
    if not envs.VLLM_ASCEND_MOONCAKE_HETERO_VERIFY:
        return
    try:
        import mooncake.engine as me  # type: ignore[import-not-found]
    except ImportError:
    ...
    try:
        ...
        # Avoid shell interpolation: count symbols in Python instead of piping
        # through `grep`; so_path comes from mooncake.engine.__file__ but a
        # quote in a custom install path could otherwise inject commands.
        symbol_count = result.stdout.count("HeterogeneousRdma")
        ...
        if self.transfer_engine is None:
            with self.transfer_engine_lock:
                ...
                if self.transfer_engine is None:
                    ...
                    # Same-vendor NPU connectors (Layerwise/Hybrid/MooncakeConnectorV1/Store) use
                    # the old ascend transport from a USE_ASCEND build, which lacks the
                    # HeterogeneousRdmaTransport symbol; a false check would fail their startup.
                    if heterogeneous:
