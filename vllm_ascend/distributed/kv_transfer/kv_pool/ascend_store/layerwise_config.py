...
def get_layerwise_num_shared_buffers(
    ...
    value = extra_config.get(_NUM_SHARED_BUFFERS) if extra_config else None
    if value is None:
        if num_layers < 1:
            raise ValueError("num_layers must be at least 1")
        return num_layers
    num_shared_buffers = _parse_int_config(value, _NUM_SHARED_BUFFERS)
...
def get_layerwise_num_prefetch_layers(
    ...
    value = extra_config.get(_PREFETCH_LAYERS) if extra_config else None
    if value is None:
        return min(num_shared_buffers, _DEFAULT_MAX_PREFETCH_LAYERS)
    num_prefetch_layers = _parse_int_config(value, _PREFETCH_LAYERS)
...
def get_layerwise_independent_layers(
    ...
    value = extra_config.get(_INDEPENDENT_LAYERS) if extra_config else None
    if value is None:
...
def get_layerwise_storage_indices(
    ...
    """Group independent layers and round-robin shared-buffer layers."""
    config = get_layerwise_config(num_layers, extra_config)
