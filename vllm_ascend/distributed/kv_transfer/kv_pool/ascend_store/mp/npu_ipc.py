"""NPU IPC mappings and ordering events for the transfer subprocess.
...
the transfer subprocess can recover the correct local device even when logical indices
...
class ExportedKVCache:
    ...
    def describe_range(self, address: int, size: int) -> tuple[int, int, int]:
        ...
        for index, storage in enumerate(self._storages):
            ...
            allocation_size = self.spec.storages[index].size_bytes
...
class TorchNPUIPCAdapter:
    ...
    def import_storage(self, spec: KVCacheStorageSpec) -> tuple[torch.Tensor, int]:
        ...
        # Logical device indices may differ between the Worker and transfer subprocess.
...
def import_worker_kv_caches(spec: WorkerKVCacheSpec, adapter: KVCacheStorageAdapter | None = None) -> ImportedKVCache:
    ...
    try:
        ...
        return ImportedKVCache(
            ...
            tuple(storage.size_bytes for storage in spec.storages),
        )
    except Exception:
...
# event inside the transfer subprocess.
