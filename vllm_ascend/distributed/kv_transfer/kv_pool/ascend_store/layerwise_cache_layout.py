...
_LAYERWISE_REUSE_CONNECTORS = ("AscendStoreConnector", "AscendStoreMPConnector", "MooncakeConnectorStoreV1")
...
def get_layerwise_reuse_config(kv_transfer_config: Any) -> dict[str, Any] | None:
    ...
    if connector_name in _LAYERWISE_REUSE_CONNECTORS:
        connector_configs = [
            {
    ...
    for connector_config in connector_configs:
        if not isinstance(connector_config, dict):
            continue
        if connector_config.get("kv_connector") not in _LAYERWISE_REUSE_CONNECTORS:
            continue
