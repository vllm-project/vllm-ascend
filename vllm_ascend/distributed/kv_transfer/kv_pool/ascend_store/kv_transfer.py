...
class KVCacheStoreLayerRecvingThread(KVTransferThread):
    ...
    def _handle_request(  # type: ignore[override]
        ...
        finished_req_ids: set[str] = set()
        transferred_req_ids: set[str] = set()
        all_gvas = []
        all_addrs = []
        all_sizes = []
        for task, arrays in task_arrays:
            assert task.completion is not None
            transferred_req_ids.update(task.completion.req_ids)
            finished_req_ids.update(task.finished_req_ids)
            all_gvas.append(arrays.gvas_array)
            all_addrs.append(arrays.addr_array)
            all_sizes.append(arrays.size_array)
        ...
        if res != 0:
            try:
                if transferred_req_ids and self.load_lease_releaser is not None:
                    self.load_lease_releaser(transferred_req_ids)
            finally:
                self.request_queue.task_done()
