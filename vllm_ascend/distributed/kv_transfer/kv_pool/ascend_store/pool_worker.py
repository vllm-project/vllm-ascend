        ...
        for request in requests:
            ...
            if self.use_gva_layerwise and save_start_block < save_end_block:
                ...
                while save_start_block < save_end_block and save_start_block < len(group_block_hashes):
                    key = self._make_layerwise_gva_key(group_id, group_block_hashes[save_start_block].hex())  # type: ignore[union-attr]
        ...
        for request in requests:
            ...
            for group_id in range(self.num_kv_cache_groups):
                ...
                for blk_idx in range(save_start_block, min(save_end_block, len(group_block_hashes))):
                    key = self._make_layerwise_gva_key(group_id, group_block_hashes[blk_idx].hex())  # type: ignore[union-attr]
                ...
                full_gvas = [0] * (len(block_ids_by_group) if block_ids_by_group is not None else 0)
    ...
    def _prepare_load_gvas(self, requests: list[ReqMeta]) -> None:
        ...
        for request in requests:
            ...
            for group_id in range(self.num_kv_cache_groups):
                ...
                keys = [
                    self._make_layerwise_gva_key(group_id, group_block_hashes[i].hex())  # type: ignore[union-attr]
