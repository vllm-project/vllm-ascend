...
class KVPoolWorker:
    ...
    def __init__(
        ...
        if self.tp_mismatch:
            ...
            self.local_heads_per_rank = self.num_kv_head // self.tp_size if self.tp_size <= self.num_kv_head else 1
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self.tp_mismatch:
            ...
            # flatten the per-group address bookkeeping for the strided I/O path.
            self.kv_caches_base_addr = self.group_kv_caches_base_addr[0]
            self.block_len = self.group_block_len[0]
            self.block_stride = self.group_block_stride[0]
    ...
    def _make_sub_key_str(self, base_key, effective_rank: int) -> str:
        ...
        return key_str.replace(
            f"@head_or_tp_rank:{self.metadata[0].head_or_tp_rank}",
    ...
    def _build_strided_addrs(self, block_id: int, token_count: int, sub_idx: int) -> tuple[list[int], list[int]]:
        ...
        ``block_stride`` because the kernel may pad between blocks (e.g. when the
        external block size is a multiple of the kernel block size).
        """
        ...
        for i, base_addr in enumerate(self.kv_caches_base_addr):
            ...
            entry_block_stride = self.block_stride[i % num_entry_types]
    ...
    def _build_tp_mismatch_keys_and_addrs(
        ...
        """Walk chunks x sub-keys; emit (keys, addrs, sizes) for backend put/get.
        ...
        for start, end, base_key in self.token_database.process_tokens(token_len, block_hashes, mask_num):
    ...
    def _load_kv_tp_mismatch(
        ...
        keys, addrs, sizes = self._build_tp_mismatch_keys_and_addrs(block_hashes, block_ids, token_len, mask_num)
        if not keys:
            return
    ...
    def lookup_scheduler(
        ...
        try:
            ...
            for group_id in kv_cache_group_ids:
                ...
                # (= max(local_tp, peer_tp)); otherwise fall back to local tp_size.
                if self.tp_mismatch:
                    check_tp_size = min(self.effective_tp_size, self._get_group_num_kv_heads(group_id))
                else:
                    check_tp_size = self.get_group_tp_size(group_id)
                base_rank = self.metadata[group_id].head_or_tp_rank
                multi_tp_keys = keys[:]
                for i in range(check_tp_size):
                    if i == base_rank:
                        continue
                    for item in keys:
                ...
                base_pp = self.metadata[group_id].pp_rank
                ...
                multi_tp_values = [
                    ...
                    for i in range(check_tp_size * self.pp_size)
                ]
                logger.debug(
