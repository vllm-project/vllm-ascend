...
class MooncakeBackend(Backend):
    def __init__(self, parallel_config: ParallelConfig, lazy_init: bool = False, **kwargs: Any):
        """Mooncake KV-store backend with optional disk-offload support.

        ``lazy_init`` (forwarded by ``KVPoolWorker`` for the DSV4/compressed-
        model path, see vllm-ascend PR #9771) defers
        ``MooncakeDistributedStore.setup`` until the first ``put()`` so
        DSV4 + fabric-memory ranks don't eagerly fail.

        ``kwargs`` may include ``kv_connector_extra_config`` (the dict from
        the engine's ``kv_transfer_config``); when supplied, it's consulted
        for the ``preferred_segment`` override that pins this rank's PUTs
        to a specific owner segment (required for SSD-tier replication).
        """
        # Validate mooncake is importable upfront so we fail fast with a
        # helpful message rather than later in a worker thread on first put.
        try:
            from mooncake.store import MooncakeDistributedStore  # type: ignore # noqa: F401
        ...
        # Store handle + per-rank state. The store itself is set up either
        # eagerly below or lazily on the first ``put()`` via
        # ``_ensure_initialized`` / ``_setup_store``.
        ...
        # --- disk-offload wiring (config-only, no I/O) ---------------------
        ...
        logger.info(
            ...
            "preferred_segment=%s, enable_offload=%s, lazy_init=%s)",
            self.config.mode,
        ...
        # Eager init unless DSV4 + fabric-mem lazy mode is requested.
    ...
    def _setup_store(self) -> Any:
        ...
        # ASCEND_ENABLE_USE_FABRIC_MEM: Enable unified memory address direct
        # transmission scheme; only valid on 800 I/T A3 series.
    ...
    def get(self, keys: list[str], addrs: list[list[int]], sizes: list[list[int]]):
        ...
        In lazy-init mode (DSV4 + fabric-mem), the store is only created
        after the first ``put()``; ``get()`` short-circuits if the store
        isn't ready yet.
        """
        if not keys:
            return
        if self._lazy_init and not self._store_initialized:
            logger.error("MooncakeBackend.get called before store initialization, keys=%s", keys[:3])
            return
        assert self.store is not None
