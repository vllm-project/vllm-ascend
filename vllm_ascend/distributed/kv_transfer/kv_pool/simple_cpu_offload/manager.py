...
class PreemptedRequestState:
    ...
    store_transfer_meta: TransferMeta | None = None
...
class SimpleCPUOffloadScheduler:
    ...
    def __init__(
        ...
        enable_prefix_offload: bool = True,
        enable_preempt_offload: bool = False,
    ):
        ...
        self._prefix_offload_enabled = enable_prefix_offload
        self._preempt_offload_enabled = enable_preempt_offload

        logger.info(
            "SimpleCPUOffloadScheduler: Allocating %d CPU blocks "
            "(%.2f GB, prefix=%s, preempt=%s, mode=%s)",
            self.num_cpu_blocks,
            cpu_capacity_bytes / (1024**3),
            self._prefix_offload_enabled,
            self._preempt_offload_enabled,
        ...
        # Preemption offload metadata. This request-level cache does not rely
        # on block hashes, so it can preserve the final partial block.
        ...
        self._preempt_store_event_to_hashes: dict[
            int, set[BlockHashWithGroupId]
        ] = {}
        self._preempt_load_event_to_reqs: dict[int, list[str]] = {}
        # Hash blocks allocated by preempt offload but not submitted to the
        # worker yet. Requests preempted in the same scheduling step can share
        # these blocks instead of scheduling duplicate D2H copies.
        self._pending_preempt_hash_blocks: dict[
            BlockHashWithGroupId, KVCacheBlock
        ] = {}
        self._inflight_preempt_hash_counts: dict[BlockHashWithGroupId, int] = {}
    ...
    def get_num_new_matched_tokens(
        ...
        preempt_state = self._preempted_req_states.get(request.request_id)
        if preempt_state is not None:
            if not preempt_state.ready:
                return None, False
            restorable_tokens = min(preempt_state.num_computed_tokens, request.num_tokens)
    ...
    def update_state_after_alloc(
        ...
        if self._prepare_preempt_load_after_alloc(
            request,
            block_ids_by_group,
            num_external_tokens,
        ):
    ...
    def build_connector_meta(
        ...
        preempt_store_gpu, preempt_store_cpu, preempt_store_req_ids = (
            self.prepare_preempt_store_specs()
        )
        if preempt_store_gpu:
            preempt_store_event = self._store_event_counter
            self._store_event_counter += 1
            self._preempt_store_event_to_blocks[preempt_store_event] = TransferMeta(
                preempt_store_gpu,
                preempt_store_cpu,
            )
            self._preempt_store_event_to_reqs[preempt_store_event] = (
                preempt_store_req_ids
            )
            event_hashes = {
                block_hash
                for cpu_block_id in preempt_store_cpu
                if (
                    block_hash := self.cpu_block_pool.blocks[
                        cpu_block_id
                    ].block_hash
                )
                is not None
            }
            self._preempt_store_event_to_hashes[preempt_store_event] = event_hashes
            for block_hash in event_hashes:
                self._inflight_preempt_hash_counts[block_hash] = (
                    self._inflight_preempt_hash_counts.get(block_hash, 0) + 1
                )
            for req_id in preempt_store_req_ids:
                self._preempted_req_states[req_id].store_event = preempt_store_event
            self._pending_preempt_hash_blocks.clear()

        # --- Preemption loads ---
        preempt_load_event = -1
        preempt_load_gpu: list[int] = []
        preempt_load_cpu: list[int] = []
        preempt_load_req_ids: list[str] = []
        for req_id, preempt_state in self._preempted_req_states.items():
            if (
                preempt_state.load_transfer_meta is None
                or preempt_state.load_event is not None
            ):
                continue
            preempt_load_gpu.extend(preempt_state.load_transfer_meta.gpu_block_ids)
            preempt_load_cpu.extend(preempt_state.load_transfer_meta.cpu_block_ids)
            preempt_load_req_ids.append(req_id)
        if preempt_load_req_ids:
            preempt_load_event = self._load_event_counter
            self._load_event_counter += 1
            for req_id in preempt_load_req_ids:
                self._preempted_req_states[req_id].load_event = preempt_load_event
            self._preempt_load_event_to_reqs[preempt_load_event] = (
                preempt_load_req_ids
            )

        # --- Stores ---
        store_event = -1
        if self._prefix_offload_enabled:
            store_gpu, store_cpu, store_req_ids = self.prepare_store_specs(
                scheduler_output
            )
        else:
        ...
        result = SimpleCPUOffloadMetadata(
            ...
            preempt_store_event=preempt_store_event,
            preempt_store_gpu_blocks=preempt_store_gpu,
            preempt_store_cpu_blocks=preempt_store_cpu,
            preempt_load_event=preempt_load_event,
            preempt_load_gpu_blocks=preempt_load_gpu,
            preempt_load_cpu_blocks=preempt_load_cpu,
    ...
    def update_state_before_preempt(
        ...
        if not self._preempt_offload_enabled:
            return
    ...
    def _create_preempt_state(
        ...
        if not self._preempt_offload_enabled or num_computed_tokens <= 0:
            return False
        ...
        for g, group_gpu_ids in enumerate(block_ids_by_group):
            ...
            for gpu_block in gpu_blocks:
                ...
                if not self._prefix_offload_enabled or block_hash is None:
                    num_unhashed += 1
                elif (
                    ...
                    is None
                    and block_hash not in self._pending_preempt_hash_blocks
                ):
                    missing_hashes.add(block_hash)

        num_needed = num_unhashed + len(missing_hashes)
        if num_needed == 0:
            has_blocks = any(group_gpu_blocks)
            if not has_blocks:
                return False
        if num_needed > cpu_block_pool.get_num_free_blocks():
            logger.warning(
                "Skip preemption offload for request %s because CPU "
                "offload cache has only %d free blocks, need %d.",
                req_id,
                cpu_block_pool.get_num_free_blocks(),
                num_needed,
            )
            return False

        cpu_block_iter = iter(cpu_block_pool.get_new_blocks(num_needed))
        ...
        for gpu_blocks in group_gpu_blocks:
            ...
            for gpu_block in gpu_blocks:
                ...
                if self._prefix_offload_enabled and block_hash is not None:
                    cpu_block = (
                        cpu_block_pool.cached_block_hash_to_block.get_one_block(
                            block_hash
                        )
                    ...
                    if cpu_block is not None:
                        cpu_block_pool.touch([cpu_block])
                    else:
                        cpu_block = self._pending_preempt_hash_blocks.get(block_hash)
                        if cpu_block is not None:
                            cpu_block_pool.touch([cpu_block])
                            ...
                            self._pending_preempt_hash_blocks[block_hash] = cpu_block
        ...
        if store_gpu_block_ids:
            self._gpu_block_pool.touch(
                [
                    self._gpu_block_pool.blocks[block_id]
                    for block_id in store_gpu_block_ids
                ]
            )
        return True

    def _is_prefix_store_scheduled(
        self, block_hash: BlockHashWithGroupId
    ) -> bool:
        return (
            block_hash in self._pending_preempt_hash_blocks
            or self._inflight_preempt_hash_counts.get(block_hash, 0) > 0
        )

    def prepare_preempt_store_specs(self) -> tuple[list[int], list[int], list[str]]:
        if not self._preempt_offload_enabled:
            return [], [], []
        ...
        for req_id, state in self._preempted_req_states.items():
            if state.store_event is not None or state.ready:
                continue
            if state.store_transfer_meta is not None:
                merged_gpu_block_ids.extend(
                    state.store_transfer_meta.gpu_block_ids
                )
                merged_cpu_block_ids.extend(
                    state.store_transfer_meta.cpu_block_ids
                )
    ...
    def _prepare_preempt_load_after_alloc(
        ...
        preempt_state = self._preempted_req_states.get(request.request_id)
        if preempt_state is None or not preempt_state.ready:
            return False

        num_tokens_to_load = min(num_external_tokens, preempt_state.num_computed_tokens)
        ...
        for g, group_cpu_ids in enumerate(preempt_state.cpu_block_ids):
            group_block_size = kv_cache_groups[g].kv_cache_spec.block_size
            num_blocks = min(
                len(group_cpu_ids),
                len(block_ids_by_group[g]),
                cdiv(num_tokens_to_load, group_block_size),
            )
            cpu_block_ids.extend(group_cpu_ids[:num_blocks])
            gpu_block_ids.extend(block_ids_by_group[g][:num_blocks])
        ...
        preempt_state.load_transfer_meta = TransferMeta(gpu_block_ids, cpu_block_ids)
        return True
    ...
    def _prepare_lazy_store_specs(
        ...
        while (
            ...
            if (
                ...
                and not self._is_prefix_store_scheduled(bhash)
            ):
            ...
            for g in range(num_groups):
                ...
                for gpu_block_id in scannable:
                    ...
                    if (
                        ...
                        is not None
                        or self._is_prefix_store_scheduled(bhash_with_group)
                    ):
                        advanced_per_group[g] += 1
                        continue
    ...
    def _process_store_event(self, event_idx: int) -> None:
        ...
        if event_idx in self._preempt_store_event_to_blocks:
    ...
    def _process_preempt_store_event(self, event_idx: int) -> None:
        ...
        event_hashes = self._preempt_store_event_to_hashes.pop(event_idx, set())
        for block_hash in event_hashes:
            count = self._inflight_preempt_hash_counts[block_hash] - 1
            if count == 0:
                self._inflight_preempt_hash_counts.pop(block_hash)
            else:
                self._inflight_preempt_hash_counts[block_hash] = count
        if self._prefix_offload_enabled:
        ...
        self._gpu_block_pool.free_blocks(
            self._gpu_block_pool.blocks[block_id]
            for block_id in transfer.gpu_block_ids
        )
        logger.debug(
            "Preemption store event %d completed: cached %d blocks for %d requests",
            event_idx,
            len(transfer.cpu_block_ids),
            len(req_ids),
        )
    ...
    def has_pending_transfers(self) -> bool:
        """Return True if there are in-flight load or store transfers."""
        return bool(
            self._reqs_to_load
            or self._store_event_to_blocks
            or self._store_event_pending_counts
    ...
    def reset_cache(self) -> bool:
        """Reset CPU prefix cache when no transfer is in flight."""
        if self.has_pending_transfers():
            logger.warning(
                "Failed to reset SimpleCPUOffload cache because transfers "
        ...
        self._preempt_store_event_to_hashes.clear()
        self._preempt_load_event_to_reqs.clear()
        self._pending_preempt_hash_blocks.clear()
        self._inflight_preempt_hash_counts.clear()
        self._cursor = None
    ...
    def request_finished(
        ...
        preempt_state = self._preempted_req_states.get(req_id)
        if preempt_state is not None and preempt_state.load_event is None:
            if preempt_state.ready:
                self._cleanup_preempt_cache_request(req_id)
            else:
                preempt_state.finished = True
    ...
    def _cleanup_preempt_load_request(self, req_id: str) -> None:
        ...
        if state.load_transfer_meta is not None:
            ...
            self._gpu_block_pool.free_blocks(
                self._gpu_block_pool.blocks[bid]
                for bid in state.load_transfer_meta.gpu_block_ids
            )
