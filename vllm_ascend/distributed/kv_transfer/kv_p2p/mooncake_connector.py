...
class MooncakeConnectorWorker:
    ...
    def _get_kv_split_metadata(
        ...
        def get_local_remote_block_port_mappings():
            ...
            p_node_cp_group_meta = get_cp_group_meta(prefill_tp_size,
    ...
    def start_load_kv(self, metadata: MooncakeConnectorMetadata):
        ...
        for req_id, meta in metadata.requests.items():
            ...
            prefill_tp_size = meta.remote_ptp_size if getattr(
                meta, "remote_ptp_size", None) else self._prefill_tp_size
            tp_num_need_pulls = self._get_tp_num_need_pulls(prefill_tp_size)
            ...
            if meta.remote_pcp_size * meta.remote_dcp_size > 1:
                ...
                for i in range(tp_num_need_pulls * self._prefill_pp_size):
                    ...
                    self.kv_recv_thread.add_request(
                        ...
                        offset=i,
                        tp_num_need_pulls=tp_num_need_pulls,
                        all_task_done=(i == tp_num_need_pulls *
    ...
    def _get_tp_num_need_pulls(self, prefill_tp_size: int) -> int:
        ...
        if self.vllm_config.model_config.is_deepseek_mla:
            ...
            num_d_block_heads = max(1,
                                    self.num_key_value_heads // self.tp_size)
            num_p_block_heads = max(
                1, self.num_key_value_heads // prefill_tp_size)
    ...
    def _get_remote_rank(self,
                         req_id: str,
                         prefill_tp_size: Optional[int] = None) -> List[int]:
        return self._get_remote_ranks_for_req(
            req_id, prefill_tp_size)[self.tp_rank]

    def _get_remote_tp_ranks(self, tp_ori_data: np.ndarray,
                             rand_group_index: list[int],
                             num_groups: int,
                             prefill_tp_size: int) -> List[List[int]]:
        tp_num_need_pulls = self._get_tp_num_need_pulls(prefill_tp_size)
        # random split prefill tp list
        tp_sampled_nums = []
        if prefill_tp_size > self.num_key_value_heads or self.vllm_config.model_config.is_deepseek_mla or self.use_sparse:
    ...
    def _get_remote_ranks_for_req(self,
                                  req_id: str,
                                  prefill_tp_size: Optional[int] = None) -> List[List[int]]:
        if prefill_tp_size is None:
            prefill_tp_size = self._prefill_tp_size
