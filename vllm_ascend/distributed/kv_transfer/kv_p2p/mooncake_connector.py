...
class MooncakeConnectorWorker:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str, kv_cache_config: KVCacheConfig):
        ...
        if ascend_envs.VLLM_ASCEND_ENABLE_FLASH_MLA and self._prefill_tp_size != self._decode_tp_size:
            raise ValueError("Flash MLA PD currently requires equal prefill/decode TP sizes")
    ...
    def _group_skip_kv_reformat(group_spec: dict[str, Any]) -> bool:
        ...
        return (
            group_spec.get("kv_cache_spec_type") == "AscendSFAIndexerCacheSpec"
            or group_spec.get("kv_cache_layout") == "HND"
        )
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if ascend_envs.VLLM_ASCEND_ENABLE_FLASH_MLA:
            # Keep canonical model caches unchanged. The transfer engine needs
            # separate K/V addresses; each alias retains the shared page stride.
            kv_caches = dict(kv_caches)
            for group_spec, _ in self.kv_group2layeridx.values():
                for layer_name in group_spec["layer_names"]:
                    spec = self._get_layer_spec(layer_name)
                    if isinstance(spec, FullAttentionSpec) and not isinstance(spec, MLAAttentionSpec):
                        cache = kv_caches[layer_name]
                        if isinstance(cache, torch.Tensor):
                            kv_caches[layer_name] = split_kv_cache_head_slots(cache, spec.num_kv_heads)
                        group_spec["kv_cache_layout"] = "HND"
            self.kv_caches = kv_caches
        ...
        if has_mamba_group:
            ...
            # Standardized descriptors can all name the same backing buffer.
            # Register that allocation once while retaining per-layer metadata.
            regions = list(dict.fromkeys(zip(ptrs, lengths)))
            ptrs = [ptr for ptr, _ in regions]
            lengths = [length for _, length in regions]
    ...
    def _get_decode_only_dcp_metadata(
        ...
        if not any(meta.local_block_ids) and meta.num_external_tokens == 0:
            # Cancelled requests and full prefix hits still notify the selected
            # P workers to release their KV. They have no D pages to map.
        ...
        for group_idx, (group_spec, layer_indices) in self.kv_group2layeridx.items():
            ...
            if spec_type == "AscendSFAIndexerCacheSpec":
                ...
                continue
            if spec_type == "AscendDCPReplicatedDraftAttentionSpec":
                # The GQA draft keeps the complete sequence on each D rank.
                # P has ordinary pages; D stores DCP lanes inside each local
                # manager block, followed by the manager/kernel subdivision.
                local_blocks = (meta.local_full_block_ids or meta.local_block_ids)[group_id]
                remote_blocks = meta.remote_block_ids[group_id]
                global_blocks = range(
                    meta.num_computed_tokens // self.block_size,
                    min(meta.num_prompt_blocks, len(remote_blocks)),
                )
                scale = self._get_kernel_block_scale(layer_indices) // self.dcp_size
                local_block_ids[block_id_idx] = self._expand_block_ids(
                    [
                        local_blocks[block // self.dcp_size] * self.dcp_size + block % self.dcp_size
    ...
    def start_load_kv(self, metadata: MooncakeConnectorMetadata):
        ...
        for req_id, meta in metadata.requests.items():
            if meta.do_virtual:
                ...
                continue
            if ascend_envs.VLLM_ASCEND_ENABLE_FLASH_MLA:
                remote_tp = meta.remote_ptp_size if meta.remote_ptp_size is not None else self._prefill_tp_size
                if (
                    remote_tp != self.tp_size
                    or meta.remote_dcp_size not in (1, self.dcp_size)
                    or meta.remote_pcp_size != 1
                    or meta.remote_block_size != self.block_size
                ):
                    raise ValueError(
                        "Flash MLA PD requires matching P/D TP and block size, PCP=1, "
                        "and either matching DCP or prefill DCP=1"
                    )
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
