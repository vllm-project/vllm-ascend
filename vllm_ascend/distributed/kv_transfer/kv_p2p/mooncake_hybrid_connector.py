                ...
                if len(req_meta["local_block_ids"]) > 0:
                    if not transfer_ok:
                        logger.error(
                            ...
                            "decoding may read uninitialized, stale, or misranged data. "
    ...
    def _get_entry_windows(self) -> dict[str, int]:
        ...
        if self._entry_windows is None:
            ...
            for group in self.kv_cache_config.kv_cache_groups:
                ...
                items = per_layer.items() if per_layer else [(entry_name, spec) for entry_name in group.layer_names]
                for entry_name, layer_spec in items:
                    if isinstance(layer_spec, SlidingWindowSpec):
                        ew[entry_name] = layer_spec.sliding_window
    ...
    def _append_entry_routing_segments(
        ...
        for i in range(self.hma_group_size):
            ...
            if n != len(cur_local_block_ids) or n != len(cur_remote_block_ids):
                ...
                vals = [entry_windows.get(entry_name, 0) for entry_name in entry_names]
