...
"""Lightweight NumPy planner and subprocess entry point for STAIR."""
...
_PLANNER_CONFIG_FIELDS = (
    "load_window_bins",
    "load_risk_quantile",
    "relative_balance_threshold",
    "absolute_balance_threshold",
    "rank_transfer_limit",
    "cross_node_transfer_limit",
    "replica_search_num_stages",
    "replica_search_radius",
    "replica_search_beam_size",
    "placement_search_backtrack_limit",
)


@dataclass(frozen=True)
class StairPlannerConfig:
    """Dependency-free view of the settings used by the CPU planner."""

    load_window_bins: int
    load_risk_quantile: float
    relative_balance_threshold: float
    absolute_balance_threshold: float
    rank_transfer_limit: int
    cross_node_transfer_limit: int
    replica_search_num_stages: int
    replica_search_radius: int
    replica_search_beam_size: int
    placement_search_backtrack_limit: int
...
def planner_config_values(config: object) -> tuple[object, ...]:
    """Serialize planner settings without coupling the child to Ascend config."""

    return tuple(getattr(config, field) for field in _PLANNER_CONFIG_FIELDS)
...
def _read_exact(stream: BinaryIO, size: int) -> bytearray:
    ...
    offset = 0
    while offset < size:
        bytes_read = stream.readinto(view[offset:])
        if not bytes_read:
            raise EOFError("STAIR planner socket closed mid-message")
        offset += bytes_read
    return data


def _write_message(stream: BinaryIO, metadata: dict[str, object], values: Sequence[np.ndarray]) -> None:
    arrays = [np.ascontiguousarray(value) for value in values]
    if any(array.dtype.hasobject for array in arrays):
        raise TypeError("STAIR planner messages do not support object arrays")
    message = dict(metadata)
    message["arrays"] = [{"dtype": array.dtype.str, "shape": array.shape, "size": array.nbytes} for array in arrays]
    encoded = json.dumps(message, separators=(",", ":")).encode()
    stream.write(_MESSAGE_HEADER.pack(len(encoded)))
...
def _read_message(stream: BinaryIO) -> tuple[dict[str, object], list[np.ndarray]]:
    (header_size,) = _MESSAGE_HEADER.unpack(_read_exact(stream, _MESSAGE_HEADER.size))
    metadata = json.loads(_read_exact(stream, header_size))
    ...
    for descriptor in descriptors:
        ...
        if dtype.hasobject or size != dtype.itemsize * int(np.prod(shape, dtype=np.int64)):
            raise ValueError("STAIR planner received an invalid array descriptor")
        arrays.append(np.frombuffer(_read_exact(stream, size), dtype=dtype).reshape(shape).copy())
...
def send_planner_request(
    ...
    """Send one dependency-free planning request to the child process."""

    arrays = [logical_load_values, current_rank_expert_ids, last_committed_mean_ratios, rank_node_ids]
    if sample_counts is not None:
        arrays.append(sample_counts)
    _write_message(
        stream,
        {
            "config_values": config_values,
            "layer_ids": None if layer_ids is None else tuple(layer_ids),
            "has_sample_counts": sample_counts is not None,
        },
        arrays,
    )


def receive_planner_response(stream: BinaryIO) -> tuple[str | None, str | None, tuple[np.ndarray, ...] | None]:
    """Receive either a four-array plan or the child's traceback."""

    metadata, arrays = _read_message(stream)
    error = metadata["error"]
    if error is not None:
        return str(metadata["error_type"]), str(error), None
    if len(arrays) != 4:
        raise ValueError("STAIR planner response must contain four arrays")
    return None, None, tuple(arrays)


def _receive_planner_request(stream: BinaryIO) -> tuple[object, ...]:
    metadata, arrays = _read_message(stream)
    has_sample_counts = metadata["has_sample_counts"]
    if len(arrays) != 4 + int(has_sample_counts):
        raise ValueError("STAIR planner request has an invalid array count")
    return (
        *arrays[:4],
        tuple(metadata["config_values"]),
        metadata["layer_ids"],
        arrays[4] if has_sample_counts else None,
    )


def _send_planner_response(stream: BinaryIO, error_type: str | None, error: str | None, plan: StairPlan | None) -> None:
    fields = (
        ()
        if plan is None
        else (plan.rank_expert_ids, plan.source_rank_ids, plan.source_slot_ids, plan.predicted_mean_ratios)
    )
    _write_message(stream, {"error_type": error_type, "error": error}, fields)


class StairPlanner:
    """Pure NumPy load modeling and placement planning."""
    ...
    def compress_load_window(load_samples: np.ndarray, max_bins: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        boundaries, sample_counts = StairPlanner._load_bin_boundaries(values.shape[0], max_bins)
        compressed = np.stack(
    ...
    def _migration_sources(
        ...
        # candidates then stabilize the selected source ranks.
...
def _serve(socket_fd: int) -> None:
    """Serve planning requests until the owning worker closes the socket."""
    ...
    with socket.socket(fileno=socket_fd) as planner_socket, planner_socket.makefile("rwb") as stream:
        while True:
            ...
            try:
                ...
                plan = StairPlanner.plan_rebalance(
                    logical_load_values,
                    current_rank_expert_ids,
                    last_committed_mean_ratios,
                    rank_node_ids,
                    StairPlannerConfig(*config_values),
                    layer_ids=layer_ids,
                ...
                plan = None
            _send_planner_response(stream, error_type, error, plan)


if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SystemExit("usage: _stair_planner.py SOCKET_FD")
