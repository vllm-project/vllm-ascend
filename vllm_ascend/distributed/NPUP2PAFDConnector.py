...
class NPUP2PAFDConnector(AFDConnectorBase):
    ...
    def send_ffn_output(
        ...
        * To send the intermediate tensors generated by FFN instances back to
        ...
        """
        # 诊断: FFN 侧 P2P 回传前 (hc_post 输出的 3D 张量)
        try:
            from vllm.logger import logger as _logger
            _logger.info(
                "[FFN_SEND_PRE] hs dim=%d shape=%s dtype=%s mean=%.6f std=%.6f "
    ...
    def recv_ffn_output(self,
        ...
        * And (Maybe) dispatch them from the receiver to other GPUs.
        ...
        # 诊断: Attention 侧 P2P 接收后 (从 FFN 侧回传的 3D 张量)
        try:
            from vllm.logger import logger as _logger
            _logger.info(
                "[ATTN_RECV_POST] hs dim=%d shape=%s dtype=%s mean=%.6f std=%.6f "
                "min=%.6f max=%.6f has_nan=%s",
                recv_hs.dim(), tuple(recv_hs.shape),
                recv_hs.dtype,
                recv_hs.float().mean().item(),
                recv_hs.float().std().item(),
                recv_hs.float().min().item(),
                recv_hs.float().max().item(),
                torch.isnan(recv_hs).any().item(),
            )
        except Exception:
            pass
        return recv_hs
