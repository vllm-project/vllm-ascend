...
import cannbotdsl  # type: ignore[import-not-found]
import torch
from cannbotdsl import dtypes  # type: ignore[import-not-found]
from cannbotdsl.arch import get_block_idx, get_block_num  # type: ignore[import-not-found]
from cannbotdsl.buffer import Buffer  # type: ignore[import-not-found]
from cannbotdsl.channel import Channel  # type: ignore[import-not-found]
from cannbotdsl.control_flow import range as dsl_range  # type: ignore[import-not-found]
from cannbotdsl.core.frontend.compiler import compile_function  # type: ignore[import-not-found]
from cannbotdsl.dtypes import (  # type: ignore[import-not-found]
...
from cannbotdsl.integer import Int64  # type: ignore[import-not-found]
from cannbotdsl.jit_runner import jit  # type: ignore[import-not-found]
from cannbotdsl.kernel_launcher import kernel  # type: ignore[import-not-found]
from cannbotdsl.raw_reg import (  # type: ignore[import-not-found]
    UnpackMode,
    full_mask,
    update_mask,
    vadd,
...
from cannbotdsl.tensor import local_slice, mem_copy, tile_view  # type: ignore[import-not-found]
from cannbotdsl.typing.types import MemLoc, Tensor  # type: ignore[import-not-found]
from cannbotdsl.vf import vf  # type: ignore[import-not-found]
...
def _use_decode_slot_parallel(
    ...
    """
    return 0 < int(tokens) <= DECODE_SLOT_PARALLEL_MAX_TOKENS and int(slots) > 1 and int(required_ub) <= UB_BYTES
...
def _decode_online_query_group(tokens: int, slots: int, d: int) -> int:
    ...
    for group in range(int(wanted), 0, -1):
        required = (2 * group + 1) * w * 4 + (2 * group + 3) * VL * 4
...
def _hold_inv_rms_body_fp32(v_ub, inv_rms_ub, nplus1, num_seg, w, avg, eps, num_col):
    ...
    with vf(mode="raw"):
        ...
        for n in range(Int64(0), Int64(nplus1)):
            ...
            for seg in range(Int64(4 * ng), Int64(num_seg)):
                mask, _ = update_mask(
                    Int64(num_col) - seg * VL,
                    elem_bits=32,
                )
                value = vload(v_ub, base_n + seg * VL)
                ssq = vadd(
                    ssq,
                    vmul(value, value, mask=mask),
                    mask=full,
                )
            denom = vsqrt(
                vadds(
            ...
            vstore_first(
                inv_rms_ub,
                n,
                vdiv(one, denom, mask=full),
            )


def _score_hold_dot_body_fp32(v_ub, qe_ub, logits_ub, inv_rms_ub, nplus1, num_seg, w, num_col):
...
def _online_init_body_fp32(v_ub, numerator_ub, max_state_ub, sum_state_ub, logits_ub, query_lane, num_seg, w, num_col):
...
def _online_merge_body_fp32(v_ub, numerator_ub, max_state_ub, sum_state_ub, logits_ub, query_lane, num_seg, w, num_col):
...
def _online_zero_body_fp32(numerator_ub, max_state_ub, sum_state_ub, query_lane, num_seg, w, num_col):
...
def _agg_body_16bit(v_ub, logits_ub, h_ub, nplus1, num_seg, w, logits_off):
    ng = nplus1 // 4
    with vf(mode="raw"):
        full = full_mask(32)
        for seg in range(Int64(0), Int64(num_seg)):
...
def _agg_body_fp32(v_ub, logits_ub, h_ub, nplus1, num_seg, w, logits_off):
    ng = nplus1 // 4
    with vf(mode="raw"):
        full = full_mask(32)
        for seg in range(Int64(0), Int64(num_seg)):
...
def _score_stream_dot_body_fp32(v_dt, qe_ub, pdot, nplus1, num_seg_dt, w_dt):
...
def _score_stream_dot2_body_fp32(v_dt, qe_pair_ub, pdot, nplus1, num_seg_dt, w_dt):
    ...
    with vf(mode="raw"):
        ...
        for n in range(Int64(0), Int64(nplus1)):
            ...
            for seg in range(Int64(2 * ng), Int64(num_seg_dt)):
                ...
                dot0 = vadd(
                    dot0,
                    vmul(value, query0, mask=full),
                    mask=full,
                )
                dot1 = vadd(
                    dot1,
                    vmul(value, query1, mask=full),
                    mask=full,
                )
...
class BlockAttnResPrepareVector:
    ...
    def load_effective_query_dt(self, gm_effective_queries, slot_idx, db_idx):
    ...
    def load_effective_query_pair_dt(self, gm_effective_queries, slot_idx, db_idx):
    ...
    def store_stats(self, gm_max, gm_sum, row):
        ...
        mem_copy(tile_view(gm_max, (1, 1), (row, Int64(0))), local_slice(stats_cur, (1, 1), offset=0))
        mem_copy(tile_view(gm_sum, (1, 1), (row, Int64(0))), local_slice(stats_cur, (1, 1), offset=32))
    ...
    def score_stream_block(self, v_dt, effective_q_ub, valid_blocks, query_lane):
        if self.is16:
            _score_stream_body_16bit(
                v_dt,
                effective_q_ub,
    ...
    def score_stream_dot_block(self, v_dt, effective_q_ub, valid_blocks, query_lane):
        if self.is16:
    ...
    def score_stream_dot2_block(self, v_dt, query_pair_ub, valid_blocks, query_lane):
    ...
    def agg_stream_block(self, v_dt, gm_h, row, db_idx, valid_blocks, query_lane):
        h_slot = self.h_dt_ub.acquire()
        if self.is16:
            _agg_body_16bit(
                v_dt,
        ...
        h_cur = self.h_dt_ub.wait()
        mem_copy(tile_view(gm_h, (1, self.dt), (row, Int64(db_idx))), local_slice(h_cur, (1, self.dt), offset=0))
        self.h_dt_ub.release(h_cur)
...
class BlockAttnResPrepareMultiQueryKernel:
    ...
    def __init__(self, max_blocks: int, num_row: int, num_query: int, d: int, eps: float, dtype=BFloat16):
...
class BlockAttnResPrepareDecodeOnlineVector:
    ...
    def store_stats(self, gm_max, gm_sum, out_row, query_lane):
        ...
        with vf(mode="raw"):
            vstore_first(
                stats_slot,
                0,
                vload(self.max_state_ub, state_off),
            )
            vstore_first(
                stats_slot,
                8,
                vload(self.sum_state_ub, state_off),
            )
            vmem_bar(mode="vst_vld")
        self.stats_ub.commit(stats_slot)
        stats_cur = self.stats_ub.wait()
    ...
    @jit
    def store_group(self, gm_h, gm_max, gm_sum, group_start, active_queries, row, num_row):
...
class BlockAttnResPrepareMultiQueryKernelDecodeOnline:
    ...
    def __init__(self, max_blocks: int, num_row: int, num_query: int, d: int, query_group: int, eps: float):
        self.max_blocks = int(max_blocks)
        self.num_row = int(num_row)
        self.num_query = int(num_query)
        self.d = int(d)
...
class BlockAttnResPrepareMultiQueryKernelStream:
    ...
    def __init__(self, max_blocks: int, num_row: int, num_query: int, d: int, dt: int, eps: float, dtype=BFloat16):
        self.max_blocks = int(max_blocks)
        self.num_row = int(num_row)
        self.num_query = int(num_query)
        self.d = int(d)
            ...
            for row in range(row_start, row_end):
                if valid_blocks > Int64(0):
                    ...
                    for db in range(Int64(0), Int64(num_dt)):
                        ...
                        vector.effective_q_ub.release(q_cur)
                        pair_end = self.num_query - (1 if self.num_query % 2 == 0 else 0)
...
class BlockAttnResPrepare:
    ...
    def __init__(self, plan: PrepareLaunchPlan):
        ...
        if plan.mode == "decode_online":
            ...
            self._kernel_cls = BlockAttnResPrepareMultiQueryKernelDecodeOnline
            self._kernel_args = (
                self.max_blocks,
                self.num_row,
                self.num_query,
            ...
            self._kernel_cls = BlockAttnResPrepareMultiQueryKernelStream  # type: ignore[assignment]
            self._kernel_args = (  # type: ignore[assignment]
                self.max_blocks,
                self.num_row,
                self.num_query,
                self.d,
            ...
            self._kernel_cls = BlockAttnResPrepareMultiQueryKernelStreamDecode  # type: ignore[assignment]
            self._kernel_args = (  # type: ignore[assignment]
                self.max_blocks,
                self.num_row,
                self.num_query,
            ...
            self._kernel_cls = BlockAttnResPrepareMultiQueryKernelDecode  # type: ignore[assignment]
            self._kernel_args = (  # type: ignore[assignment]
                self.max_blocks,
                self.num_row,
                self.num_query,
            ...
            self._kernel_cls = BlockAttnResPrepareMultiQueryKernel  # type: ignore[assignment]
            self._kernel_args = (  # type: ignore[assignment]
                self.max_blocks,
                self.num_row,
                self.num_query,
    ...
    # pylint: disable=too-many-arguments,too-many-positional-arguments
    @jit
    def run(self, gm_v2d, gm_effective_queries, gm_valid_blocks, gm_h, gm_max, gm_sum):
        op = self._kernel_cls(*self._kernel_args)
        op[self.block_num](  # type: ignore[index]
            gm_v2d,
            gm_effective_queries,
            gm_valid_blocks,
            gm_h,
...
def _prepare_launch_plan(
    ...
    required_ub = max_blocks * w_pad * ebytes + w_pad * 4 + w_pad * 4 + 3 * VL * 4
    mode = "hold"
    dt = 0
    if required_ub > UB_BYTES:
        ...
        use_stream_prefill_group = DEFAULT_BLOCK_NUM // 2 <= tokens < DEFAULT_BLOCK_NUM and prefill_query_group >= 2
        online_group = _decode_online_query_group(tokens, slots, d) if use_stream_decode else 0
        if online_group > 0:
            mode = "decode_online"
        else:
            _require(
            ...
            dt_max = ((UB_BYTES - fixed_stream_bytes) // stream_bytes_per_d // VL) * VL
            ...
            while candidate >= VL:
                ...
                candidate -= VL
            if dt < VL:
                raise RuntimeError("整网多 query D 分块仍超 UB: max_blocks={max_blocks}, D={d}")
            if use_stream_decode:
                mode = "stream_decode"
            elif use_stream_prefill_group:
...
def _block_attn_res_prepare_eager(
    ...
    *,
    ...
    _require(
        v.is_contiguous() and effective_queries.is_contiguous() and valid_blocks.is_contiguous(),
