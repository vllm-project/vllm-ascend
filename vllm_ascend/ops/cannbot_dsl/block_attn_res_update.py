...
def _block_attn_res_update_body(
    partial_ub, delta_ub, inter_ub, query_ub, stats_ub, partial_out_ub, h_out_ub, num_seg, d, avg, epsilon, h_dtype
):
...
class BlockAttnResUpdateVector:
    ...
    def __init__(self, d: int, delta_dtype=Float16, pipeline_rows: bool = False):
        self.d = int(d)
    ...
    def load_row(self, gm_partial, gm_delta, gm_inter_numerator, gm_inter_max, gm_inter_sum, row):
...
class BlockAttnResUpdateKernel:
    ...
    def __init__(self, num_tokens: int, d: int, epsilon: float, delta_dtype=Float16, pipeline_rows: bool = False):
...
def _block_attn_res_update_eager(
    ...
    _require(
        effective_query.dtype == torch.float32 and effective_query.dim() == 1 and int(effective_query.shape[0]) == d,
        f"effective_query 必须为 [{d}] fp32",
    )
    _require(
        effective_query.is_contiguous() and effective_query.device == partial_block.device,
    ...
    for name, value in (
        ...
        _require(
            value.dtype == torch.float32 and tuple(value.shape) == stats_shape,
    ...
    _require(
        inter_numerator.is_contiguous() and inter_numerator.device == partial_block.device,
