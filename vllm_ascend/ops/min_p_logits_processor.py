...
from vllm.v1.sample.logits_processor impot MinPLogitsProcessor
...
def min_p_logits_processor_init_func(self, *args, **kwargs):
    ...
    vllml_config = get_current_vllm_config()
    decode_max_num_seqs = getattr(vllml_config.scheduler_config,
                                  'decode_max_num_seqs', 0)
    # reinit MinPLogitsProcessor if decode_max_num_seqs configured
    if decode_max_num_seqs != 0:
        device = args[1]
        is_pin_memory = args[2]
        max_num_reqs = decode_max_num_seqs
