...
class KimiOProjMMReduceScatterOp(CustomRowParallelOp):
    ...
    def apply_into(self, input_: torch.Tensor, output: torch.Tensor) -> torch.Tensor:
        """Write the ordinary decode projection directly into its TP output shard.

        The caller supplies TP-aligned, live-masked input, so no padding or
        post-collective mask/copy is needed. Keep the prefill MC2 path separate.
        """
        input_parallel = self.get_input_parallel(input_)
    ...
    def _can_fuse(self, input_parallel: torch.Tensor, weight: torch.Tensor) -> bool:
        ...
        # MRv2 stores this flag in additional_kwargs, not on the base context.
        if is_forward_context_available() and not _EXTRA_CTX.is_pure_prefill:
            return False
