...
def uses_kimi_k3_global_inputs_embeds(vllm_config: VllmConfig) -> bool:
    ...
    return bool(
        multimodal_config.enable_mm_embeds or multimodal_config.get_limit_per_prompt("vision_chunk") > 0
    )
...
class AscendKimiGatedDeltaNetAttention(KimiGatedDeltaNetAttention):
    ...
    def __init__(self, config, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        self.is_vl_first_layer = bool(
            uses_kimi_k3_global_inputs_embeds(vllm_config) and parse_layer_idx(prefix) == 0
        )
