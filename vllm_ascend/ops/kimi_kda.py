...
class AscendKimiK3DeltaAttention(KimiK3DeltaAttention):
    ...
    def _conv_weights_t(self) -> torch.Tensor:
        return self.get_parameter(_PACKED_CONV_WEIGHT_NAME)
    ...
    def _apply_output_norm_gate(
        ...
        # on Ascend. Calling the module's CUDA/Triton helper directly bypasses
        # platform dispatch and reintroduces the release branch's Triton-only
        # optimization into the AscendC model path.
    ...
    def _run_prefill(
        ...
        if get_pcp_group().world_size > 1:
            raise NotImplementedError("Kimi K3 KDA prefill does not yet support PCP.")
        ...
        if keep is not None:
            if keep.numel() != state_indices.shape[0] or keep.numel() != has_initial_state.numel():
                raise ValueError("Kimi K3 KDA prefill metadata must contain one keep_meta entry per sequence.")
        ...
        if state_indices.shape[0] != num_sequences or has_initial_state.numel() != num_sequences:
            raise ValueError(
                "Kimi K3 KDA compact cu_seqlens, state indices, and "
                "initial-state flags describe different sequence counts."
            )
        ...
        if self.gate_lower_bound is None:
            raise ValueError("The AscendC Kimi K3 prefill path requires a bounded gate.")
    ...
    def _forward(
        ...
        normalized = self._apply_output_norm_gate(
