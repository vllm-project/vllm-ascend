...
def has_inplace_partial_rotary_mul_dsa_by_cache_kernel() -> bool:
    return _c_aclnn_api_available("aclnnInplacePartialRotaryMulDsaByCache")
...
def has_compressor_dsa_by_cache_kernel() -> bool:
    return _c_aclnn_api_available("aclnnCompressorDsaByCache")
...
def _raise_missing_true_by_cache_backend(op_name: str) -> None:
    raise RuntimeError(
...
def _try_triton_interleave_rope_by_cache(
    ...
    qk = x.reshape(x.shape[0], x.shape[1], x.shape[3])
    qk_work = qk.contiguous().clone()
...
def _supports_native_inplace_partial_rotary_mul_dsa_by_cache(
    x: torch.Tensor,
    positions: torch.Tensor,
    ...
    *,
    ...
    if rotary_mode != "interleave" or len(partial_slice) != 2:
        return False
    start, end = partial_slice
    head_dim = end - start
    if start < 0 or head_dim <= 0:
        return False
    if x.dim() != 4 or positions.dim() != 1 or cos_cache.dim() < 2:
        return False
    if positions.dtype != torch.long or positions.shape[0] != x.shape[0]:
        return False
    if cos_cache.shape != sin_cache.shape or cos_cache.dtype != sin_cache.dtype:
        return False
    if end > x.shape[-1] or cos_cache.shape[-1] != head_dim or rope_dim != head_dim:
        return False
    if cos_cache.dtype != x.dtype and cos_cache.dtype != torch.float32:
        return False
    if head_dim > 64:
        return False

    element_size = x.element_size()
    if element_size <= 0 or 32 % element_size != 0:
        return False
    return head_dim % (32 // element_size) == 0


def _iter_dsa_by_cache_chunks(
    x: torch.Tensor,
    start: int,
    end: int,
    *,
    max_chunk_dim: int = 64,
):
    if start < 0 or end <= start:
        _raise_missing_true_by_cache_backend("inplace_partial_rotary_mul_dsa_by_cache")
    element_size = x.element_size()
    if element_size <= 0 or 32 % element_size != 0:
        _raise_missing_true_by_cache_backend("inplace_partial_rotary_mul_dsa_by_cache")
    align = 32 // element_size
    chunk_start = start
    while chunk_start < end:
        remaining = end - chunk_start
        chunk_dim = min(max_chunk_dim, remaining)
        if remaining > max_chunk_dim:
            chunk_dim = max_chunk_dim
        if chunk_dim % align != 0:
            _raise_missing_true_by_cache_backend("inplace_partial_rotary_mul_dsa_by_cache")
        chunk_end = chunk_start + chunk_dim
...
def _get_dsa_cos_sin_cache(dsa_rope_cache, ref_tensor: torch.Tensor | None = None) -> torch.Tensor:
    cos_sin_cache = dsa_rope_cache.cos_sin_cache
    if isinstance(cos_sin_cache, tuple):
        _raise_missing_true_by_cache_backend("DSA cos_sin_cache")
    return _match_cache_to_ref_tensor(cos_sin_cache, ref_tensor)


def _split_dsa_cos_sin_cache(cos_sin_cache: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
def _slice_dsa_cos_sin_cache_for_chunk(
    ...
    *,
    ...
    if cache_start < 0 or cache_end <= cache_start or cache_end > cos_cache.shape[-1]:
        _raise_missing_true_by_cache_backend("inplace_partial_rotary_mul_dsa_by_cache")
    if cache_start == 0 and cache_end == cos_cache.shape[-1]:
        return cos_sin_cache, cos_cache, sin_cache

    chunk_dim = cache_end - cache_start
    chunk_cos_cache = cos_cache.narrow(-1, cache_start, chunk_dim).contiguous()
    chunk_sin_cache = sin_cache.narrow(-1, cache_start, chunk_dim).contiguous()
    return torch.cat((chunk_cos_cache, chunk_sin_cache), dim=-1), chunk_cos_cache, chunk_sin_cache
...
def interleave_rope_by_cache(
    ...
    native_op = _get_torch_npu_op("npu_interleave_rope_by_cache")
...
def rotary_mul_materialized(
    ...
    *,
    ...
    """Legacy helper for APIs that still receive already materialized cos/sin.

    New NPU hot paths must use the by-cache adapters above so the backend reads
    positions and cos_sin_cache directly.
    """
...
def kv_rmsnorm_rope_cache_by_cache(
    ...
    *,
    ...
    # write" sentinel. The Triton backend masks slot >= 0; keep native off
    # this path until the same contract is proven there.
    native_op = (
        None
        if allow_negative_slots
        else _get_torch_npu_op("npu_kv_rmsnorm_rope_cache_by_cache")
    )
    if native_op is not None:
...
def mla_prolog_decode_by_cache(*args, version: int = 2, **kwargs):
    ...
    if version == 3:
        return mla_prolog_v3_by_cache(**kwargs)
...
def inplace_partial_rotary_mul_dsa_by_cache(
    x: torch.Tensor,
    dsa_rope_cache,
    *,
    rotary_mode: str,
    ...
    native_op = (
        _get_c_ascend_op("inplace_partial_rotary_mul_dsa_by_cache")
        if has_inplace_partial_rotary_mul_dsa_by_cache_kernel()
        else None
    )
    if native_op is not None:
        cos_sin_cache = _get_dsa_cos_sin_cache(dsa_rope_cache, x)
        cos_cache, sin_cache = _split_dsa_cos_sin_cache(cos_sin_cache)
        start, end = partial_slice
        try:
            for chunk_start, chunk_end in _iter_dsa_by_cache_chunks(x, start, end):
                cache_start = chunk_start - start
                cache_end = chunk_end - start
                chunk_cos_sin_cache, chunk_cos_cache, chunk_sin_cache = _slice_dsa_cos_sin_cache_for_chunk(
                    cos_sin_cache,
                    cos_cache,
                    sin_cache,
                ...
                chunk_slice = [chunk_start, chunk_end]
                if not _supports_native_inplace_partial_rotary_mul_dsa_by_cache(
                    x,
                    dsa_rope_cache.positions,
                    chunk_cos_cache,
                    chunk_sin_cache,
                    rotary_mode=rotary_mode,
                    partial_slice=chunk_slice,
                    rope_dim=chunk_end - chunk_start,
                ):
                    _raise_missing_true_by_cache_backend("inplace_partial_rotary_mul_dsa_by_cache")
                native_op(
                    x,
            ...
            if not _is_inplace_partial_rotary_mul_unsupported(exc):
                raise
            _mark_c_ascend_op_unsupported("inplace_partial_rotary_mul_dsa_by_cache")
...
def compressor_dsa_by_cache(
    ...
    **kwargs,
    ...
    native_op = _get_c_ascend_op("compressor_dsa_by_cache") if has_compressor_dsa_by_cache_kernel() else None
    if native_op is not None:
        cos_sin_cache = _get_dsa_cos_sin_cache(dsa_rope_cache, x)
        return native_op(
            x,
    ...
    _raise_missing_true_by_cache_backend("compressor_dsa_by_cache")
...
def split_qkv_rmsnorm_mrope_by_cache(
    ...
    *,
    ...
    if fused_op is None:
        raise RuntimeError("triton_split_qkv_rmsnorm_mrope_by_cache is not registered for the NPU backend.")
