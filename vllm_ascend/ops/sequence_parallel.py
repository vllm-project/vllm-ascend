...
class MetadataForPadding:
    def __init__(self, padding_flag=False, lengths_sum_padding=0, lengths_sum_unpadding=0, pad_size=0, not_dummy_and_is_prefill=False):
    ...
    def padding_aligned_reduce_scatter(self, data: torch.Tensor) -> torch.Tensor:
        if self.padding_flag:
            pad_size = self.pad_size    
            padded_data = F.pad(data, (0, 0, 0, pad_size))            
        else:
            padded_data = data
        padded_data_reduce_scatter = tensor_model_parallel_reduce_scatter(padded_data, 0)

        return padded_data_reduce_scatter

    def allgather_unpadding_aligned(self, padded_data: torch.Tensor) -> torch.Tensor:
        padded_data_allgather = tensor_model_parallel_all_gather(padded_data, 0)
        if self.padding_flag:
    ...
    def padding_aligned_scatter(self, data: torch.Tensor) -> torch.Tensor:
        if self.padding_flag:
            ...
            padded_data = data
        # padded_data = data
        padded_data = torch.tensor_split(padded_data,
                                         self.tp_size,
                                         dim=0)
...
def init_metadata_for_sp(input_ids):
    ...
    if envs.VLLM_ASCEND_ENABLE_FLASHCOMM != 1:
        lengths_sum_unpadding = input_ids.shape[0]
        lengths_sum_padding = ((lengths_sum_unpadding + tp_size - 1) // tp_size) * tp_size
        return MetadataForPadding(lengths_sum_unpadding=lengths_sum_unpadding, lengths_sum_padding=lengths_sum_padding, padding_flag=False, not_dummy_and_is_prefill=False)
    is_perifll = 0 
