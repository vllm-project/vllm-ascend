...
class WeightPrefetchMethod:
    ...
    def maybe_prefetch_attn_weight_preprocess(
        ...
        weight_size = (
            weight.data.element_size()
            * weight.data.numel()
            * self.attn.prefetch_ratio.get(prefix, 0)
        )

        torch.ops.vllm.prefetch_preprocess(
            weight=weight, start_flag=start_flag, max_weight_size=int(weight_size)
        )
    ...
    def maybe_prefetch_moe_weight_preprocess(self, hidden_states, prefix):
        self.moe.is_active_this_forward = (
            hidden_states.shape[0] >= MOE_PREFETCH_TOKEN_THRESHOLD
            if self.moe.enable
            else False
        )
        ...
        torch.ops.vllm.prefetch_preprocess(
            weight=weight, start_flag=None, max_weight_size=int(weight_size)
        )
