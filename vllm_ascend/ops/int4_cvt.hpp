...
PTO_INTERNAL void GenCastCallFp16ToInt4(__ubuf__ void* dst, __ubuf__ typename TileDataS::DType* src, uint8_t repeatNum,
...
PTO_INST void TCvtHeadFp16ToInt4Packed(__ubuf__ typename TileDataD::DType* dstPtr,
                                       ...
                                       unsigned dstBytesPerRepeat, unsigned dstRepeatStride, unsigned srcRepeatStride) {
    ...
    if (numLoop > 0) {
      for (uint32_t j = 0; j < numLoop; j++) {
        GenCastCallFp16ToInt4<TileDataD, TileDataS>(
            (__ubuf__ void*)(dstPtr + i * DS + j * dstBytesPerRepeat * REPEAT_MAX),
            srcPtr + i * SS + j * srcElementsPerRepeat * REPEAT_MAX, (uint8_t)REPEAT_MAX, mode, 1, 1,
            (uint16_t)dstRepeatStride, (uint16_t)srcRepeatStride);
      }
    }
    if (remainAfterLoop > 0) {
      GenCastCallFp16ToInt4<TileDataD, TileDataS>(
          (__ubuf__ void*)(dstPtr + i * DS + numLoop * dstBytesPerRepeat * REPEAT_MAX),
          srcPtr + i * SS + numLoop * srcElementsPerRepeat * REPEAT_MAX, (uint8_t)remainAfterLoop, mode, 1, 1,
          (uint16_t)dstRepeatStride, (uint16_t)srcRepeatStride);
    }
  }
}
...
template <typename TileDataD, typename TileDataS, unsigned SS, unsigned DS>
__tf__ AICORE void TCvtFp16ToInt4Packed(typename TileDataD::TileDType __out__ dst,
  ...
  if (numRepeatPerLine > 0) {
    TCvtHeadFp16ToInt4Packed<TileDataD, TileDataS, SS, DS>(dstPtr, srcPtr, mode, numRepeatPerLine, validRow,
  ...
  if (numRemainPerLine > 0) {
    ...
    if (numLoop > 0) {
      for (uint32_t j = 0; j < numLoop; j++) {
        GenCastCallFp16ToInt4<TileDataD, TileDataS>((__ubuf__ void*)(dstPtr + j * DS * REPEAT_MAX),
                                                    srcPtr + j * SS * REPEAT_MAX, (uint8_t)REPEAT_MAX, mode, 1, 1,
                                                    (uint16_t)DS / dstNElemPerBlock, (uint16_t)SS / srcNElemPerBlock);
      }
    }
    if (remainAfterLoop > 0) {
      GenCastCallFp16ToInt4<TileDataD, TileDataS>((__ubuf__ void*)(dstPtr + numLoop * DS * REPEAT_MAX),
...
template <typename TileDataD, typename TileDataS>
PTO_INTERNAL void TCVT_FP16_TO_INT4_PACKED_IMPL(TileDataD& dst, TileDataS& src, RoundMode mode) {
  static_assert(std::is_same<typename TileDataD::DType, int8_t>::value, "Packed int4 destination must use int8_t.");
  static_assert(std::is_same<typename TileDataS::DType, half>::value, "Packed int4 conversion expects fp16 source.");
  ...
  unsigned srcElementsPerRepeat = REPEAT_BYTE / sizeof(typename TileDataS::DType);
  unsigned dstBytesPerRepeat = srcElementsPerRepeat / 2;
  unsigned dstRepeatStride = dstBytesPerRepeat / (BLOCK_BYTE_SIZE / sizeof(typename TileDataD::DType));
  unsigned srcRepeatStride = srcElementsPerRepeat / (BLOCK_BYTE_SIZE / sizeof(typename TileDataS::DType));
  ...
  TCvtFp16ToInt4Packed<TileDataD, TileDataS, SS, DS>(dst.data(), src.data(), mode, numRepeatPerLine, numRemainPerLine,
                                                     validRow, srcElementsPerRepeat, dstBytesPerRepeat, dstRepeatStride,
                                                     srcRepeatStride);
}

template <typename DstTile, typename SrcTile>
AICORE void TCVT_FP16_TO_INT4_PACKED(DstTile& dst, SrcTile& src, RoundMode mode) {
