...
class LMHeadTensorParallel(torch.nn.Module):
    ...
    def __init__(self,
        ...
        tp_rank = get_lmheadtp_group().rank
        ...
        is_embedding_layer = type(self) is LMHeadTensorParallel
    ...
    def _get_indices(self, vocab_size_padded: int, org_vocab_size_padded: int,
...
class CustomLogitsProcessor(LogitsProcessor):
    ...
    def _get_logits(
        ...
        Args:
            hidden_states: Current hidden states from the model
            lm_head: Parallel embedding layer for vocabulary
            cu_tokens_across_dp_cpu: Cumulative token counts across data parallel groups
            embedding_bias: Optional bias to add to logits
            
        Returns:
            Logits tensor for next token prediction or None
        """
        # Get parallel size and global batch sizes
        ...
        lmhead_group_batch_size = [global_dp_batch_size[x] for x in get_lmheadtp_group().ranks]
        ...
        gathered_input = [torch.empty(batch_size, hidden_states.size(
            1), dtype=hidden_states.dtype, device='npu') for batch_size in lmhead_group_batch_size]
        torch.distributed.all_gather(
            gathered_input, hidden_states, group=get_lmheadtp_group().device_group)
        ...
        output_splits = [local_batch_size *
                         local_output_dim for _ in lmhead_group_batch_size]
        all_to_all_result = torch.empty(
            local_batch_size * (local_output_dim * lmhead_tp_size),
        ...
        reshaped = all_to_all_result.view(lmhead_tp_size, local_batch_size, local_output_dim)
        output_logits = reshaped.permute(1, 0, 2).reshape(local_batch_size, -1)

        # Remove paddings in vocab (if any).
        if output_logits is not None:
            output_logits = output_logits[..., :self.org_vocab_size]
