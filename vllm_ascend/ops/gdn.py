...
class AscendGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    # Whether ``in_proj_qkvz`` weight can be directly sliced instead of going
    # through the linear module's forward. Precomputed at load time by
    # ``precompute_gdn_weight_split`` so it stays a plain Python bool that
    # torch.compile can specialize. ``None`` before the weight is loaded.
    _use_weight_split: bool | None = None
    ...
    def forward(
        ...
        if hasattr(self, "in_proj_qkv"):
            ...
            if not self.gqa_interleaved_layout:
                ...
                if self.enable_lora:
                    use_weight_split: bool | None = False
                else:
