...
class TokenDispatcherWithMC2(MoETokenDispatcher):
    ...
    def token_dispatch(
        ...
        output = torch_npu.npu_moe_distribute_dispatch_v2(**kwargs_mc2) if self.enable_dispatch_v2 \
    ...
    def get_combine_mc_kwargs(self, hidden_states: torch.Tensor,
                              dispatch_metadata: dict):
        expert_map = dispatch_metadata["expert_map"]
        topk_ids = dispatch_metadata["topk_ids"]
        topk_weights = dispatch_metadata["topk_weights"]
        output = dispatch_metadata["output"]
        ep_recv_counts = dispatch_metadata["ep_recv_counts"]
        assist_info_for_combine = dispatch_metadata["assist_info_for_combine"]
        mc2_mask = dispatch_metadata["mc2_mask"]
    ...
    def token_combine(
        ...
        shared_experts = dispatch_metadata["shared_experts"]
        if shared_experts is None:
            return combined_output

        shared_act = dispatch_metadata["shared_act"]
        if self.with_quant:
            swiglu_out_scale = dispatch_metadata["swiglu_out_scale"]
...
class TokenDispatcherWithAllGather(MoETokenDispatcher):
    ...
    def token_combine(self,
        ...
        final_hidden_states = torch_npu.npu_moe_token_unpermute(
            ...
            sorted_indices=torch.abs(dispatch_metadata["expanded_row_idx"]),
            probs=dispatch_metadata["topk_weights"])
...
class TokenDispatcherWithAll2AllV(MoETokenDispatcher):
    ...
    def token_dispatch(
        ...
        print("using all2all dispatcher")
        self.with_quant = with_quant
    ...
    def token_combine(
        ...
        _, permutated_local_input_tokens, handle = async_all_to_all(
            hidden_states,
            dispatch_metadata["input_splits"],
            dispatch_metadata["output_splits"],
            self.ep_group,
        )
        handle.wait()
    ...
    def _combine_postprocess(self, permutated_local_input_tokens: torch.Tensor,
        ...
        output = torch_npu.npu_moe_token_unpermute(
            ...
            sorted_indices=dispatch_metadata[
