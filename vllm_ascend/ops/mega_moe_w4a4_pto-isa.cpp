...
// NPU 4 only.
  ...
  // AscendC headers MUST precede <pto/pto-inst.hpp>: pto-isa (CANN 9.0.0 line)
  // pulls in `using namespace std` via pto/common/fifo.hpp, which makes AscendC's
  // unqualified `conditional` in common_types.h ambiguous against std::conditional.
  // Parsing AscendC first keeps `conditional` unambiguous.
...
AICORE void stage1_int4_routed(__gm__ half* x_gm, __gm__ int32_t* eri_gm, __gm__ int8_t* q_gm, __gm__ float* s_gm,
  ...
  using GmI8P_2D =
      GlobalTensor<int8_t, TileShape2D<int8_t, DYNAMIC, DYNAMIC, Layout::ND>, Stride<1, 1, 1, DYNAMIC, 1>, Layout::ND>;
  ...
  for (uint32_t base = my_lo; base < my_hi; base += S1_R) {
    ...
    TROWEXPANDDIV(xT, xT, rmaxCm);
    pipe_barrier(PIPE_V);
    TMULS(xT, xT, (half)kScaleDivisor);
    pipe_barrier(PIPE_V);
    #endif
...
AICORE void stage1_int4_routed(__gm__ half* x_gm, __gm__ int32_t* eri_gm, __gm__ int8_t* q_gm, __gm__ float* s_gm,
  ...
  const uint32_t m_end = (t_hi == 0) ? M_total : t_hi;
    ...
    #define S1_COMPUTE(B, NEEDFREE)                                                      \
      do {                                                                               \
        ...
        TROWMAX(rmax##B, x##B, redS);                                                    \
        pipe_barrier(PIPE_V);                                                            \
        TROWMIN(rmin##B, x##B, redS);                                                    \
        pipe_barrier(PIPE_V);                                                            \
        TMULS(rmin##B, rmin##B, (half) - 1.0f);                                          \
        pipe_barrier(PIPE_V);                                                            \
        TMAX(rmax##B, rmax##B, rmin##B);                                                 \
        pipe_barrier(PIPE_V);                                                            \
  ...
  for (uint32_t M = m_first; M < m_end; M += num_cores, ++j) {
    ...
    if (cb0) {
      S1_COMPUTE(0, (set_qsS[0] != 0));
      set_qsS[0] = 1;
      set_difS[0] = 1;
    } else {
      S1_COMPUTE(1, (set_qsS[1] != 0));
      set_qsS[1] = 1;
      set_difS[1] = 1;
    }
  }
...
AICORE void stage1_int4_routed(__gm__ half* x_gm, __gm__ int32_t* eri_gm, __gm__ int8_t* q_gm, __gm__ float* s_gm,
  ...
  TASSIGN(rMax, RMAX);
  TASSIGN(rMin, RMIN);
  TASSIGN(qTile, Q_BASE);
  TASSIGN(sTile, S_BASE);
  ...
  for (uint32_t M = t_lo + vid; M < m_end; M += num_cores) {
    ...
    TROWMAX(rMax, xTile, redS);
    pipe_barrier(PIPE_V);
    TROWMIN(rMin, xTile, redS);
    pipe_barrier(PIPE_V);
    TMULS(rMin, rMin, (half)-1.0f);
    pipe_barrier(PIPE_V);
    TMAX(rMax, rMax, rMin);
    pipe_barrier(PIPE_V);
    set_flag(PIPE_V, PIPE_S, EVENT_ID0);
...
AICORE void stage3_int4_swiglu_quant_grouped_v2(__gm__ half* gu_gm, __gm__ float* xs_gm, __gm__ float* ws_gm,
  ...
  #else
AICORE void stage3_int4_swiglu_quant_grouped_v2(__gm__ int32_t* gu_gm, __gm__ float* xs_gm, __gm__ float* ws_gm,
                                                __gm__ int64_t* gl_gm, __gm__ int8_t* iq_gm, __gm__ float* is_gm,
                                                uint32_t T, uint32_t E, uint32_t e_lo = 0, uint32_t e_hi = 0) {
  set_mask_norm();
  set_vector_mask(-1, -1);
  ...
  TI32 gI32;
  TASSIGN(gI32, GATE_I32);
  TI32 uI32;
  TASSIGN(uI32, UP_I32);
  TF32 gF32;
  TASSIGN(gF32, GATE_F32);
  TF32 uF32;
  TASSIGN(uF32, UP_F32);
  TF32 tmp;
  TASSIGN(tmp, TMP_F32);
  ...
  for (uint32_t e = e_lo + vid; e < e_end3; e += num_cores) {
    ...
    GmF32_1D wsgGm(ws_gm + (int64_t)e * 2 * I_DIM, TileShape2D<float, 1, DYNAMIC, Layout::ND>(1, I_DIM));
    GmF32_1D wsuGm(ws_gm + (int64_t)e * 2 * I_DIM + I_DIM, TileShape2D<float, 1, DYNAMIC, Layout::ND>(1, I_DIM));
    TLOAD(wsg, wsgGm);
    TLOAD(wsu, wsuGm);
    set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
    wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
    ...
    for (int32_t mt = 0; mt < M_e; mt += S3_STEP) {
      ...
      TI32 gI32_dyn(rows);
      TASSIGN(gI32_dyn, GATE_I32);
      TI32 uI32_dyn(rows);
      TASSIGN(uI32_dyn, UP_I32);
      TLOAD(gI32_dyn, gGm);
      TLOAD(uI32_dyn, uGm);
      ...
      TCOLEXPANDMUL(gF32_dyn, gF32_dyn, wsg);
      pipe_barrier(PIPE_V);
      TCOLEXPANDMUL(uF32_dyn, uF32_dyn, wsu);
      pipe_barrier(PIPE_V);
      ...
      TRESHAPE(rmaxCm_dyn, rmaxRm_dyn);
      pipe_barrier(PIPE_V);
      TROWEXPANDDIV(swF16_dyn, swF16_dyn, rmaxCm_dyn);
      pipe_barrier(PIPE_V);
      TMULS(swF16_dyn, swF16_dyn, (half)kScaleDivisor);
      pipe_barrier(PIPE_V);

      TI4P iq_dyn(rows);
      TASSIGN(iq_dyn, IQ_BASE);
      fast_hadamard_int4::TCVT_FP16_TO_INT4_PACKED(iq_dyn, swF16_dyn, RoundMode::CAST_RINT);
      pipe_barrier(PIPE_V);
...
AICORE void stage5_scatter_combine_int4(__gm__ int32_t* d_gm_i32, __gm__ float* is_gm, __gm__ float* /*w2s_gm unused*/,
                                        __gm__ int32_t* sort_idx_gm, __gm__ half* topk_w_gm, __gm__ int64_t* gl_gm,
                                        __gm__ half* y_gm, uint32_t M_total, uint32_t E, uint32_t top_k,
                                        uint32_t T_orig, uint32_t e_lo = 0, uint32_t e_hi = 0) {
  set_mask_norm();
  set_vector_mask(-1, -1);
...
AICORE void stage5_scatter_combine_int4(__gm__ int32_t* d_gm, __gm__ float* is_gm, __gm__ float* w2s_gm,
                                        __gm__ int32_t* sort_idx_gm, __gm__ half* topk_w_gm, __gm__ int64_t* gl_gm,
                                        __gm__ half* y_gm, uint32_t M_total, uint32_t E, uint32_t top_k,
                                        uint32_t T_orig, uint32_t e_lo = 0, uint32_t e_hi = 0) {
  set_mask_norm();
  ...
  const uint32_t e_end = (e_hi == 0) ? E : e_hi;
    ...
    #define S5_COMPUTE(B, NEEDFREE)                                             \
      do {                                                                      \
        wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID##B);                              \
        if (NEEDFREE) wait_flag(PIPE_MTE3, PIPE_V, (event_t)(EVENT_ID4 + (B))); \
        TCVT(dF##B, dI##B, RoundMode::CAST_NONE);                               \
        pipe_barrier(PIPE_V);                                                   \
        set_flag(PIPE_V, PIPE_MTE2, (event_t)(EVENT_ID2 + (B)));                \
        TMULS(dF##B, dF##B, ld_sc##B);                                          \
        pipe_barrier(PIPE_V);                                                   \
        TMUL(dF##B, dF##B, ws);                                                 \
        pipe_barrier(PIPE_V);                                                   \
        TCVT(y##B, dF##B, RoundMode::CAST_NONE);                                \
        pipe_barrier(PIPE_V);                                                   \
        set_flag(PIPE_V, PIPE_MTE3, EVENT_ID##B);                               \
        wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID##B);                              \
        GmF16 _yg(y_gm + (int64_t)ld_orig##B * H_DIM, f16sh);                   \
  ...
  for (uint32_t e = e_lo + vid; e < e_end; e += num_cores) {
    ...
    if (n_rows < 4) {
      for (int64_t r = 0; r < n_rows; ++r) {
        ...
        set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
        wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
        TCVT(dF0, dI0, RoundMode::CAST_NONE);
        pipe_barrier(PIPE_V);
        TMULS(dF0, dF0, ld_sc0);
        pipe_barrier(PIPE_V);
        TMUL(dF0, dF0, ws);
        pipe_barrier(PIPE_V);
        TCVT(y0, dF0, RoundMode::CAST_NONE);
        pipe_barrier(PIPE_V);
        set_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
        wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
        GmF16 _yg(y_gm + (int64_t)ld_orig0 * H_DIM, f16sh);
    ...
    for (int64_t r = 0; r < n_rows; ++r) {
      ...
      if (cb0) {
        S5_COMPUTE(0, (set_buf[0] != 0));
        set_buf[0] = 1;
        set_dif[0] = 1;
      } else {
        S5_COMPUTE(1, (set_buf[1] != 0));
        set_buf[1] = 1;
        set_dif[1] = 1;
      }
    }
...
AICORE void stage5_scatter_combine_int4(__gm__ int32_t* d_gm, __gm__ float* is_gm, __gm__ float* w2s_gm,
  ...
  for (uint32_t e = e_lo + vid; e < e_end; e += num_cores) {
    ...
    for (int64_t m = t_start; m < t_end; ++m) {
      ...
      set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
      wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
      TCVT(dF, dI, RoundMode::CAST_NONE);
      pipe_barrier(PIPE_V);
      TMULS(dF, dF, is * tw);
      pipe_barrier(PIPE_V);
      TMUL(dF, dF, ws);
      pipe_barrier(PIPE_V);
      TCVT(y, dF, RoundMode::CAST_NONE);
      pipe_barrier(PIPE_V);
      set_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
      wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
  ...
  for (uint32_t g = e_lo; g < e_end; ++g) {
    ...
    for (uint32_t mIdx = 0; mIdx < blockDimM; ++mIdx) {
      for (uint32_t nIdx = 0; nIdx < blockDimN; ++nIdx, ++globalBlk) {
        ...
        uint32_t curSingleM = (mIdx == blockDimM - 1) ? (static_cast<uint32_t>(m) - mIdx * tilingPtr->singleCoreM)
...
AICORE void stage0_hadamard(__gm__ half* x_gm, __gm__ half* b1_gm, __gm__ half* xrot_gm, uint32_t T_orig) {
  ...
  for (uint32_t pair = cid; pair < total; pair += num_cores) {
    ...
    pto::GlobalTensor<half, pto::TileShape2D<half, HN0, HN0, pto::Layout::DN>, pto::Stride<1, 1, 1, 1, pto::DYNAMIC>,
                      pto::Layout::DN>
    ...
    pto::GlobalTensor<half, decltype(cShape), pto::Stride<1, 1, 1, H_DIM, 1>> cGm(
        xrot_gm + (int64_t)m_base * H_DIM + k_base, cShape, pto::Stride<1, 1, 1, H_DIM, 1>(1, 1, 1, (int32_t)H_DIM, 1));
    TSTORE(cGm, c_view);
    set_flag(PIPE_FIX, PIPE_M, we);
  }
  wait_flag(PIPE_MTE1, PIPE_MTE2, we);
...
__global__ AICORE void mega_kernel_hybrid(
    __gm__ void* x_gm, __gm__ void* w13_gm, __gm__ void* w13_scale_gm, __gm__ void* w2_gm, __gm__ void* w2_scale_gm,
    __gm__ void* group_list_gm, __gm__ void* eri_gm, __gm__ void* sort_idx_gm, __gm__ void* topk_w_gm,
    __gm__ void* xq_ws, __gm__ void* xs_ws, __gm__ void* gu_ws, __gm__ void* iq_ws, __gm__ void* is_ws,
    __gm__ void* d_ws, __gm__ void* y_gm,
  ...
  HxSyncAllImpl<false>();  // B0
  {
    ...
    pipe_barrier(PIPE_ALL);
    stage1_int4_routed(s1x, (__gm__ int32_t*)sort_idx_gm, (__gm__ int8_t*)xq_ws, (__gm__ float*)xs_ws, M_total, top_k,
                       0u, (uint32_t)th);
  }
  ...
  stage3_int4_swiglu_quant_grouped_v2((__gm__ half*)gu_ws, (__gm__ float*)xs_ws, (__gm__ float*)w13_scale_gm, gl_ov,
                                      (__gm__ int8_t*)iq_ws, (__gm__ float*)is_ws, M_total, E, 0u, eA);
  #endif
  HxSyncAllImpl<false>();  // B3
  #if MEGA_STOP_AFTER_N >= 3
  stage3_int4_swiglu_quant_grouped_v2((__gm__ half*)gu_ws, (__gm__ float*)xs_ws, (__gm__ float*)w13_scale_gm, gl_ov,
                                      (__gm__ int8_t*)iq_ws, (__gm__ float*)is_ws, M_total, E, eA, E);
  #endif
  ...
  stage5_scatter_combine_int4((__gm__ int32_t*)d_ws, (__gm__ float*)is_ws, (__gm__ float*)w2_scale_gm,
                              (__gm__ int32_t*)sort_idx_gm, (__gm__ half*)topk_w_gm, gl_ov, (__gm__ half*)y_gm, M_total,
                              E, top_k, T_orig, 0u, eA);
  #endif
  ...
  grouped_matmul_int4_dequant_impl(mm_dq, &tiling_dn, (__gm__ int8_t*)iq_ws, (__gm__ int8_t*)w2_gm, (__gm__ half*)d_ws,
                                   (__gm__ uint64_t*)w2_scale_gm, gl_ov, E, 0u, eA);
    #else
  mm.Init(&tiling_dn, &pipe);
  grouped_matmul_int4_impl(mm, &tiling_dn, (__gm__ int8_t*)iq_ws, (__gm__ int8_t*)w2_gm, (__gm__ int32_t*)d_ws, gl_ov,
                           E, 0u, eA);
    #endif
  #endif
  HxSyncAllImpl<false>();  // B4
    ...
    #ifdef MEGA_S5_FP16
  grouped_matmul_int4_dequant_impl(mm_dq, &tiling_dn, (__gm__ int8_t*)iq_ws, (__gm__ int8_t*)w2_gm, (__gm__ half*)d_ws,
                                   (__gm__ uint64_t*)w2_scale_gm, gl_ov, E, eA, E);
  mm_dq.End();
    #else
  grouped_matmul_int4_impl(mm, &tiling_dn, (__gm__ int8_t*)iq_ws, (__gm__ int8_t*)w2_gm, (__gm__ int32_t*)d_ws, gl_ov,
                           E, eA, E);
...
// ---------------- host launcher ----------------
extern "C" void call_mega_kernel_hybrid(uint32_t block_dim, void* stream, uint8_t* x, uint8_t* w13, uint8_t* w13s,
                                        uint8_t* w2, uint8_t* w2s, uint8_t* group_list, uint8_t* eri, uint8_t* sort_idx,
                                        uint8_t* topk_w, uint8_t* xq_ws, uint8_t* xs_ws, uint8_t* gu_ws, uint8_t* iq_ws,
                                        uint8_t* is_ws, uint8_t* d_ws, uint8_t* y, uint8_t* tiling_gu,
                                        uint8_t* tiling_dn, uint8_t* b1, uint8_t* xrot_ws, uint32_t M_total, uint32_t E,
                                        uint32_t top_k, uint32_t T_orig) {
  uint32_t fftsLen{0};
  uint64_t fftsAddr{0};
  rtGetC2cCtrlAddr(&fftsAddr, &fftsLen);
  mega_kernel_hybrid<<<block_dim, nullptr, stream>>>(x, w13, w13s, w2, w2s, group_list, eri, sort_idx, topk_w, xq_ws,
                                                     xs_ws, gu_ws, iq_ws, is_ws, d_ws, y, tiling_gu, tiling_dn, b1,
                                                     xrot_ws, M_total, E, top_k, T_orig, fftsAddr);
}
