...
WEIGHT_FORMAT_ND = "nd"  # codespell:ignore nd
WEIGHT_FORMAT_NZ = "nz"
...
WeightType: TypeAlias = torch.Tensor | list[torch.Tensor] | tuple[torch.Tensor, ...]
...
def grouped_matmul_situ_quant(
    ...
    *,
    ...
    weight_format: str = WEIGHT_FORMAT_NZ,
):
    ...
    Args:
        ...
        weight_format: "nz" (recommended; weights cast once via to_weight_nz)
            or the lowercase ND form (entry-layer cast, per-call cost).
    ...
    if weight_format not in (WEIGHT_FORMAT_NZ, WEIGHT_FORMAT_ND):
        raise ValueError(f"weight_format must be {WEIGHT_FORMAT_NZ!r} or {WEIGHT_FORMAT_ND!r}, got {weight_format!r}")

    is_list = isinstance(weight, (list, tuple))
    op = (
        torch.ops._C_ascend.grouped_matmul_situ_quant
        if weight_format == WEIGHT_FORMAT_ND
