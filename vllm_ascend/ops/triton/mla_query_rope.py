...
@triton.jit
def _quantize_mla_kv_and_local_query_kernel(
    latent,
    kv_rope,
    latent_cache,
    rope_cache,
    ...
    rc = tl.arange(0, 64)
    for h in tl.static_range(12):
        q = tl.load(query + token * QUERY_T + (LOCAL_HEAD_START + h) * QUERY_H + cols)
        r = tl.load(rope + token * ROPE_T + (LOCAL_HEAD_START + h) * ROPE_H + rc)
        offset = (token * 12 + h) * 576
        tl.store(local_query + offset + cols, q)
        tl.store(local_query + offset + 512 + rc, r)


def quantize_mla_kv_and_local_query(
    latent,
    kv_rope,
    latent_cache,
    rope_cache,
    ...
    *,
    ...
    """Finish DCP8 MLAPO: update owned C8 slots and preserve the local BF16 Q."""
    assert query.shape == (64, 96, 512) and rope.shape == (64, 96, 64)
    assert query.dtype == rope.dtype == local_query.dtype == torch.bfloat16
    assert query.stride(-1) == rope.stride(-1) == 1
    assert local_query.shape == (64, 12, 576) and local_query.is_contiguous()
    assert slots.numel() == source_slots.numel() == 64
    assert 0 <= local_head_start <= 84
    _quantize_mla_kv_and_local_query_kernel[(64,)](
        latent,
        kv_rope,
        latent_cache,
        rope_cache,
        ...
        INPUT_LATENT_STRIDE=latent.stride(0),
        INPUT_ROPE_STRIDE=kv_rope.stride(0),
        INPUT_LATENT_ROW_STRIDE=latent.stride(1),
        INPUT_ROPE_ROW_STRIDE=kv_rope.stride(1),
        SOURCE_PAGE_SIZE=latent.shape[1],
        LATENT_PAGE_STRIDE=latent_cache.stride(0),
        LATENT_ROW_STRIDE=latent_cache.stride(1),
        ROPE_PAGE_STRIDE=rope_cache.stride(0),
        ROPE_ROW_STRIDE=rope_cache.stride(1),
