...
def _qsa_mqa_paged_kernel(
    ...
    if PAGE_PROGRAM:
        ...
        # base address across the three contiguous 64-row tiles of a 192-row
        ...
        physical_page = tl.load(
            ...
            mask=(request >= 0) & (request < num_requests) & (logical_page < PAGE_TABLE_WIDTH),
            other=-1,
        )
        ...
        for tile_in_page in tl.static_range(0, TILES_PER_PAGE):
            page_offset = tile_in_page * BLOCK_N + column_offsets
            columns = logical_page * PAGE_SIZE + page_offset
            live = columns < visible
            page_valid = live & physical_page_valid
            keys = tl.load(
                page_base + page_offset[:, None] * stride_cache_token + dims[None, :] * stride_cache_dim,
        ...
        for tile in tl.range(tile_start, tile_end, num_stages=STAGES):
            logical_page = tile // TILES_PER_PAGE
            page_offset = (tile % TILES_PER_PAGE) * BLOCK_N + column_offsets
            columns = logical_page * PAGE_SIZE + page_offset
            live = columns < visible
            physical_page = tl.load(
                page_table_ptr + safe_request * stride_table_req + logical_page * stride_table_page,
...
def _gemma_rmsnorm_neox_rope(
    ...
    valid_t = row_valid[:, None] & (pos_t[:, None] >= 0) & (pos_t[:, None] < num_rope_positions)

    if IS_MROPE:
        valid_h = row_valid[:, None] & (pos_h[:, None] >= 0) & (pos_h[:, None] < num_rope_positions)

        valid_w = row_valid[:, None] & (pos_w[:, None] >= 0) & (pos_w[:, None] < num_rope_positions)
        ...
        cos = tl.load(base + pos_t[:, None] * cos_sin_stride, mask=valid_t & t_mask[None, :], other=0.0)

        cos += tl.load(base + pos_h[:, None] * cos_sin_stride, mask=valid_h & h_mask[None, :], other=0.0)

        cos += tl.load(base + pos_w[:, None] * cos_sin_stride, mask=valid_w & w_mask[None, :], other=0.0)

        sin = tl.load(base + pos_t[:, None] * cos_sin_stride + rotary_half, mask=valid_t & t_mask[None, :], other=0.0)

        sin += tl.load(base + pos_h[:, None] * cos_sin_stride + rotary_half, mask=valid_h & h_mask[None, :], other=0.0)

        sin += tl.load(base + pos_w[:, None] * cos_sin_stride + rotary_half, mask=valid_w & w_mask[None, :], other=0.0)
    else:
        cos = tl.load(cos_sin_ptr + pos_t[:, None] * cos_sin_stride + pairs[None, :], mask=valid_t, other=0.0)

        sin = tl.load(
            cos_sin_ptr + pos_t[:, None] * cos_sin_stride + rotary_half + pairs[None, :], mask=valid_t, other=0.0
        )
    ...
    weight = tl.load(norm_weight_ptr + tl.arange(0, head_dim)).to(tl.float32)
    weight += 1.0
...
def _qsa_update_compressed_cache_kernel(
    ...
    request = tl.load(k_work_metadata_ptr + work_id * k_work_metadata_stride_work)
    work_in_request = tl.load(
        k_work_metadata_ptr + work_id * k_work_metadata_stride_work + k_work_metadata_stride_field
    )
    valid_request = (request >= 0) & (request < num_requests) & (work_in_request >= 0)
    ...
    valid_query = (query_start >= 0) & (query_end > query_start) & (query_end <= num_tokens)
    ...
    valid_boundary = (boundary_token >= query_start) & (boundary_token < query_end) & (boundary_token < num_tokens)
    if not valid_boundary:
        return

    compressed_slot = tl.load(compressed_slots_ptr + boundary_token)
    valid_compressed_slot = (compressed_slot >= 0) & (compressed_slot < num_compressed_blocks * COMPRESSED_PAGE_SIZE)
    if not valid_compressed_slot:
        return

    state_block = tl.load(state_block_table_ptr + request * state_block_table_stride_request)
    valid_state_block = (state_block >= 0) & (state_block < num_state_blocks)

    dims = tl.arange(0, HEAD_DIM)
    compressed_block = (compressed_slot // COMPRESSED_PAGE_SIZE).to(tl.int64)
    compressed_offset = compressed_slot % COMPRESSED_PAGE_SIZE
    compressed_row_ptr = (
        compressed_cache_ptr
        + compressed_block * compressed_cache_stride_block
        + compressed_offset * compressed_cache_stride_token
        + dims * compressed_cache_stride_dim
    )
    ...
    valid_source_tokens = (source_tokens >= query_start) & (source_tokens < query_end) & (source_tokens < num_tokens)
    ...
    cached_values = tl.load(
        ...
        + state_offsets[:, None] * state_cache_stride_token
        + dims[None, :] * state_cache_stride_dim,
        mask=((~source_in_current_chunk)[:, None] & valid_state_block),
        other=0.0,
    ).to(tl.float32)

    members = tl.where(source_in_current_chunk[:, None], current_values, cached_values)
    pooled = (tl.sum(members, axis=0) / COMPRESS_RATIO).to(tl.bfloat16)
    ...
    if CACHE_HAS_ROPE_POSITIONS:
        ...
        valid_first_token = (first_token >= query_start) & (first_token < query_end) & (first_token < num_tokens)
        ...
        cached_position_address = (
            state_positions_ptr
            + safe_state_block * state_positions_stride_block
            + (first_position % STATE_SIZE) * state_positions_stride_token
        )

        load_cached_position = (~first_in_current_chunk) & valid_state_block
        ...
        cached_pos_h = tl.load(
            cached_position_address + state_positions_stride_axis, mask=load_cached_position, other=0
        )
        cached_pos_w = tl.load(
            cached_position_address + 2 * state_positions_stride_axis, mask=load_cached_position, other=0
        )
    ...
    valid_position = (pos_t_scalar >= 0) & (pos_t_scalar < num_rope_positions)
    if IS_MROPE:
        valid_position &= (
            (pos_h_scalar >= 0)
    ...
    normalized_rotated = tl.reshape(normalized_rotated, (HEAD_DIM,))
    tl.store(compressed_row_ptr, normalized_rotated)
...
def qsa_mqa_paged(
    ...
    _qsa_mqa_paged_kernel[(q.shape[0], triton.cdiv(columns, BLOCK_N * tiles_per_program))](
        ...
        TILES_PER_PAGE=k_cache.shape[1] // BLOCK_N,
