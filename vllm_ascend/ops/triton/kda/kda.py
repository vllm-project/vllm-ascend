...
def kda_gate_fwd_kernel(
    ...
    if USE_LOWER_BOUND:
        # safe gate: lower_bound * sigmoid(exp(A_log) * (x + bias))
        b_y = LOWER_BOUND * tl.sigmoid(tl.exp(b_a) * b_g)
    else:
        # standard: -exp(A_log) * softplus(x + bias)
        b_a = -tl.exp(b_a)
        g_scaled = b_g * beta
...
def fused_kda_gate(
    ...
    Forward pass for KDA gate:
      ...
      safe_gate: if true, use the lower-bound sigmoid gate
      lower_bound: lower-bound value for safe gate; also enables safe gate
          when safe_gate is None
      return  : [..., H, D]
    """
    ...
    if safe_gate is not None:
        use_lower_bound = bool(safe_gate) and use_lower_bound
