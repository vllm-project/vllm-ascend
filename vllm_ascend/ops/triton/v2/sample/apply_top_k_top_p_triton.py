    ...
    for row_id in tl.range(pid, BATCH_SIZE, num_programs):
        ...
        if TOPK_ENABLED:
            ...
            if k < VOCAB_SIZE:
                ...
                sigma = tl.sum(tl.where(table_offs == percentile, tl.load(PERCENTILE_TO_STD_TABLE + table_offs, mask=table_offs < 200, other=0.0), 0.0))
        ...
        if TOPP_ENABLED and final_pivot == -float("inf"):
            ...
            if p < 1.0:
                ...
                sigma = tl.sum(tl.where((table_offs < 200) & (scaled_p >= table_offs_f32) & (scaled_p < table_offs_f32 + 1.0), tl.load(NORMAL_CDF_TO_SIGMA_TABLE + table_offs, mask=table_offs < 200, other=0.0), 0.0))
...
def apply_top_k_top_p_triton(
    ...
    elif logits.device.type in ("npu", "privateuseone"):
        block_size = min(1024, max(128, next_power_of_2(vocab_size)))
        block_size_trunc = max(64, block_size // 2)
    else:
