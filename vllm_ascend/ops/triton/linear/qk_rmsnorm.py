...
def qk_rmsnorm_triton_kernel(
    ...
    for _ in tl.range(row_pid, batch_size, row_step):
        ...
        tl.store(q_ptr + output_offset + col_indices,
                 output_values.to(tl.bfloat16).reshape(Q_BLOCK_SIZE),
                 mask=valid_mask)
    ...
    for _ in tl.range(row_pid, batch_size, row_step):
        ...
        normalized_values = input_values * reciprocal_std
        output_values = normalized_values * weight_values
        tl.store(k_ptr + output_offset + col_indices,
                 output_values.to(tl.bfloat16).reshape(KV_BLOCK_SIZE),
                 mask=valid_mask)
        input_offset += input_offset_step
        output_offset += output_offset_step
...
direct_register_custom_op(op_name="qk_rmsnorm",
                          op_func=qk_rmsnorm_impl,
                          fake_impl=qk_rmsnorm_impl_fake,
                          mutates_args=[],
