...
def _norm_and_rope(
    ...
    for row_idx in tl.range(pid, n_tokens, row_block_size):
        ...
        n_heads = BLOCK_SIZE // head_dim
        q = tl.load(q_start_ptr + col_offsets, mask=col_mask, other=0).to(tl.float32).reshape(n_heads, head_dim)
        # Normilzation
        q_square = q * q
        square_sum = tl.sum(q_square, axis=1)
        q_rsqrt = (1 / tl.sqrt(square_sum + eps)).reshape(n_heads, 1)
        q_norm = q * q_rsqrt
        ...
        cos_offsets = tl.arange(0, pad_rope_dim)
        cos_mask = cos_offsets < rope_dim
        ...
        tl.store(q_start_ptr + col_offsets, q_norm, mask=col_mask)
