...
def _triton_interleave_rope_by_cache(
    ...
    for row_idx in tl.range(pid, num_tokens, row_block_size):
        ...
        even_offsets = heads[:, None] * hd + (2 * dims[None, :])
        odd_offsets = even_offsets + 1
        out_first_offsets = heads[:, None] * hd + dims[None, :]
        out_second_offsets = out_first_offsets + half_dim

        if IS_NEOX_STYLE:
            first_cache_offsets = dims
            second_cache_offsets = dims
        else:
            first_cache_offsets = dims // 2
            second_cache_offsets = (dims + half_dim) // 2
        cache_mask = dims < half_dim

        cos_first = tl.load(cos_sin_start_ptr + first_cache_offsets, mask=cache_mask, other=0).to(tl.float32)
        sin_first = tl.load(cos_sin_start_ptr + half_dim + first_cache_offsets, mask=cache_mask, other=0).to(tl.float32)
        cos_second = tl.load(cos_sin_start_ptr + second_cache_offsets, mask=cache_mask, other=0).to(tl.float32)
        sin_second = tl.load(cos_sin_start_ptr + half_dim + second_cache_offsets, mask=cache_mask, other=0).to(
            tl.float32
        )
        ...
        out_first = qk_even * cos_first[None, :] - qk_odd * sin_first[None, :]
        out_second = qk_odd * cos_second[None, :] + qk_even * sin_second[None, :]
        tl.store(qk_start_ptr + out_first_offsets, out_first, mask=mask)
        tl.store(qk_start_ptr + out_second_offsets, out_second, mask=mask)
...
def _triton_mrope_by_cache_forward(
    ...
    first_q_mask = (tl.arange(0, pad_n_qh)[:, None] < n_qh) & (
        tl.arange(0, pad_hd // 2)[None, :] < half_rd
    )
