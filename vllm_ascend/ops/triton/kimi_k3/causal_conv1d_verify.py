"""Kimi K3 fixed-width causal-convolution target verification.

This preserves the supplied kernel and its snapshot-buffer contract. The
ordinary K3 path does not own that buffer, so callers must opt in explicitly.
"""
...
def _causal_conv1d_linear_verify_kernel(
    ...
    state_base = state_ptr + state_idx * stride_state_slot + channel * stride_state_channel
    ...
    if KERNEL_WIDTH >= 3:
        history1 = tl.load(state_base + stride_state_window, mask=mask, other=0.0).to(tl.float32)
    if KERNEL_WIDTH >= 4:
        history2 = tl.load(state_base + 2 * stride_state_window, mask=mask, other=0.0).to(tl.float32)
    if KERNEL_WIDTH >= 5:
        history3 = tl.load(state_base + 3 * stride_state_window, mask=mask, other=0.0).to(tl.float32)
    if KERNEL_WIDTH >= 6:
        history4 = tl.load(state_base + 4 * stride_state_window, mask=mask, other=0.0).to(tl.float32)
    ...
    if KERNEL_WIDTH >= 2:
        weight1 = tl.load(weight_base + stride_weight_window, mask=channel_mask, other=0.0).to(tl.float32)
    if KERNEL_WIDTH >= 3:
        weight2 = tl.load(weight_base + 2 * stride_weight_window, mask=channel_mask, other=0.0).to(tl.float32)
    if KERNEL_WIDTH >= 4:
        weight3 = tl.load(weight_base + 3 * stride_weight_window, mask=channel_mask, other=0.0).to(tl.float32)
    if KERNEL_WIDTH >= 5:
        weight4 = tl.load(weight_base + 4 * stride_weight_window, mask=channel_mask, other=0.0).to(tl.float32)
    if KERNEL_WIDTH >= 6:
        weight5 = tl.load(weight_base + 5 * stride_weight_window, mask=channel_mask, other=0.0).to(tl.float32)
    ...
    for step in tl.static_range(0, STEPS):
        current = tl.load(
            x_ptr + pid_batch * stride_x_batch + channel * stride_x_channel + step * stride_x_step,
            mask=mask,
            other=0.0,
        ).to(tl.float32)
        acc = bias
        if KERNEL_WIDTH == 1:
            ...
            acc += history0 * weight0 + history1 * weight1 + history2 * weight2 + current * weight3
            history0 = history1
            history1 = history2
            history2 = current
        elif KERNEL_WIDTH == 5:
            acc += history0 * weight0 + history1 * weight1 + history2 * weight2 + history3 * weight3 + current * weight4
            history0 = history1
            history1 = history2
            history2 = history3
            history3 = current
        ...
        tl.store(
            out_ptr + pid_batch * stride_out_batch + channel * stride_out_channel + step * stride_out_step,
            acc,
            mask=mask,
        )
        ...
        if KERNEL_WIDTH >= 4:
            tl.store(snapshot_base + 2 * stride_snapshot_window, history2, mask=mask)
        if KERNEL_WIDTH >= 5:
            tl.store(snapshot_base + 3 * stride_snapshot_window, history3, mask=mask)
        if KERNEL_WIDTH >= 6:
            tl.store(snapshot_base + 4 * stride_snapshot_window, history4, mask=mask)
...
def causal_conv1d_linear_verify_npu(
    ...
    """Run supplied fixed-width verification and save every conv state."""
    ...
    if intermediate_conv_window.ndim != 4 or tuple(intermediate_conv_window.shape[1:]) != (
        steps,
        channels,
        state_window,
    ):
    ...
    if intermediate_conv_window.dtype != conv_state.dtype:
        raise ValueError("intermediate_conv_window and conv_state must have the same dtype")
    if any(not t.is_contiguous() for t in tensors):
        raise ValueError("all data tensors must be contiguous")
    if conv_state_indices.shape != (batch,) or intermediate_state_indices.shape != (batch,):
        raise ValueError("state index tensors must have shape [B]")
    if conv_state_indices.device != x.device or intermediate_state_indices.device != x.device:
