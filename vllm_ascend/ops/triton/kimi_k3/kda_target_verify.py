"""Kimi K3 fixed-width KDA target verification with state snapshots.

The kernel body and validation contract are the supplied implementation. It is
kept separate from the ordinary K3 recurrent path because that path has no
...
def kda_target_verify_npu(
    *,
    ...
    """Run supplied KDA target verification with per-step state snapshots."""
    ...
    if not gates_are_preactivated and (A_log.numel() != h_k or tuple(dt_bias.shape) != (h_k, key_dim)):
        raise ValueError("A_log and dt_bias shapes do not match KDA heads")
    if initial_state_source.ndim != 4 or tuple(initial_state_source.shape[1:]) != (h_v, value_dim, key_dim):
        raise ValueError("initial state must have shape [pool, H_v, V, K]")
    if intermediate_states_buffer.ndim != 5 or tuple(intermediate_states_buffer.shape[1:]) != (
        cache_steps,
        h_v,
        value_dim,
        key_dim,
    ...
    if intermediate_state_indices.ndim != 1 or intermediate_state_indices.numel() < batch:
