...
"""Triton-Ascend Mamba postprocess precision-fix kernel."""
...
def postprocess_mamba_fused_kernel(
    ...
    if CONV_STATE_DIM_FIRST and is_conv_state:
        ...
        for dim_idx in range(0, dim_rows):
            ...
            # Triton-Ascend requires the pointer cast outside the copy loop.
    ...
    if is_conv_state:
        ...
        actual_src_block_id = tl.load(block_table_base + src_block_idx + token_bias).to(tl.int64)
        src_addr = state_base_addr + actual_src_block_id * state_block_stride
        copy_size = state_inner_size * state_elem_size

    # Keep the existing Ascend precision fix, but do not introduce another
    # Triton helper/operator: this remains inside the one postprocess kernel.
