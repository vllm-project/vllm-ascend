...
def postprocess_mamba_fused_kernel(
    ...
    # TEMPORAL_TILES: the upstream caller launches a 3D grid
    # (num_reqs, num_states, TEMPORAL_TILES) and passes this constexpr; the
    # Ascend kernel does not tile the copy, so it is accepted and ignored.
    ...
    # The upstream caller launches a 3D grid with one program per temporal
    # tile; the Ascend kernel performs the whole copy in a single program, so
    # only tile 0 does the work.
    if tile_idx > 0:
        return
