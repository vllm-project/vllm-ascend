...
def postprocess_mamba_fused_kernel(
    ...
    # V2 indexes the block table by batch row; V1 by req_idx (== batch_idx).
    ...
    if CONV_STATE_DIM_FIRST and is_conv_state:
        ...
        for d in range(0, dim_rows):
            ...
            # assertion `idx < size()`); casting once here and doing plain
            # pointer arithmetic inside the loop mirrors the fix vllm-ascend
            # applies to batch_memcpy_kernel.
            src_ptr = row_src.to(tl.pointer_type(tl.uint8))
            dst_ptr = row_dst.to(tl.pointer_type(tl.uint8))
    ...
    if is_conv_state:
        # SD conv: copy
        #   state[bt[src_block_idx], accept_token_bias:]
        # to
        #   state[bt[dest_block_idx], :conv_width - accept_token_bias]
        src_block_id = tl.load(block_table_base + src_block_idx).to(tl.int64)
