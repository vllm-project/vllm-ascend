...
def postprocess_mamba_fused_kernel(
    ...
    # Optional: batch_idx -> req_idx mapping (V2 model runner / PP).
    idx_mapping_ptr,
    ...
    # Update accepted-token count before early exits. V2 updates in place;
    # V1 writes the dedicated output buffer.
    ...
    if CONV_STATE_DIM_FIRST and is_conv_state:
        ...
        src_ptr = src_block_addr.to(tl.pointer_type(tl.uint8))
        dst_ptr = dst_addr.to(tl.pointer_type(tl.uint8))
        offsets = tl.arange(0, COPY_BLOCK_SIZE)
        for d in range(0, dim_rows):
            row_src = src_ptr + d * row_stride + bias_bytes
            row_dst = dst_ptr + d * row_stride
