...
def postprocess_mamba_fused_kernel(
    ...
    # TEMPORAL_TILES: upstream launches a 3D grid (num_reqs, total_states,
    # TEMPORAL_TILES) to split the temporal copy across CTAs. Ascend keeps the
    # single-CTA copy, so only tile 0 performs it and tiles > 0 return early.
    ...
    # Upstream partitions the temporal copy across TEMPORAL_TILES CTAs (3D
    # grid). Ascend's copy is single-CTA, so only tile 0 performs it and the
    # remaining tiles return immediately, preserving the pre-tiling behavior.
