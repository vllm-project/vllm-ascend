...
def fused_sfa_dcp_lse_combine(
    ...
    """Merge rank outputs; optionally append FP32 LSE to the last dimension."""
    # MLA requests FP32 [output, LSE] for a subsequent local current-KV merge.
...
def sfa_dcp_a2a_fused_combine(
    ...
    """Pack, exchange DCP outputs, and merge with optional FP32 LSE."""
    send = pack_sfa_dcp_output_lse(sfa_output, softmax_lse, dcp_size, scatter_dim)
    if dcp_size > 1:
        if group is None:
...
def sfa_dcp_a2a_fused(
    ...
    """Merge DCP output, optionally retaining LSE for MLA current-KV merge."""
    ...
    return sfa_dcp_a2a_fused_combine(
        sfa_output, softmax_lse, dcp_size, scatter_dim, device_group, return_lse=return_lse
    )
...
def sfa_dcp_a2a_fused_fake(
    ...
    """Describe the local output shape without executing communication."""
