...
def _kv_rmsnorm_rope_cache_by_cache_kernel(
    ...
    for token_idx in tl.range(pid, num_tokens, row_block_size):
        ...
        if is_neox_style:
            first_cache_offsets = half_offsets
            second_cache_offsets = half_offsets
        else:
            first_cache_offsets = half_offsets // 2
            second_cache_offsets = (half_offsets + half_rope_dim) // 2

        cos_first = tl.load(cos_sin_row + first_cache_offsets, mask=half_mask, other=0.0).to(tl.float32)
        sin_first = tl.load(cos_sin_row + half_rope_dim + first_cache_offsets, mask=half_mask, other=0.0).to(
            tl.float32
        )
        cos_second = tl.load(cos_sin_row + second_cache_offsets, mask=half_mask, other=0.0).to(tl.float32)
        sin_second = tl.load(cos_sin_row + half_rope_dim + second_cache_offsets, mask=half_mask, other=0.0).to(
            tl.float32
        )
        ...
        rope_first = rope_even * cos_first - rope_odd * sin_first
        rope_second = rope_odd * cos_second + rope_even * sin_second

        if cache_mode_is_nz:
            rope_first_dims = half_offsets
            rope_second_dims = half_rope_dim + half_offsets
            ...
            cache_rope_second_offsets = cache_rope_row + half_rope_dim + half_offsets
        ...
        if is_output_kv:
            ...
            tl.store(out_rope_ptr + out_rope_row + half_rope_dim + half_offsets, rope_second, mask=half_mask)
