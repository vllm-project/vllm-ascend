...
- `q` and `k` must be 3-D `[num_tokens, num_heads, head_dim]` and contiguous; the wrapper calls `.contiguous()` when needed, and both tensors are modified in place and also returned.
...
- With `cos`/`sin`, each row holds `rope_dim // 2` coefficients (they must not be duplicated to `rope_dim`); passing `rope_dim = -1` makes the wrapper infer `rope_dim = cos.shape[-1] * 2`. With `cos_sin_cache`, each row holds `rope_dim` values laid out as `[cos | sin]`.
