...
- `qk` must be 3-D `[num_tokens, n_head, head_dim]` and contiguous; the wrapper calls `.contiguous()` when needed, and the tensor is modified in place and also returned.
...
- With `cos`/`sin`, each row holds `rope_dim // 2` coefficients (they must not be duplicated to `rope_dim`). Unlike `rope_forward_triton`, `rope_forward_triton_siso` must be called with an explicit `rope_dim`: it computes `pad_rope_dim` before the `rope_dim == -1` inference, so the `-1` default would propagate an invalid padded dimension to the kernel.
