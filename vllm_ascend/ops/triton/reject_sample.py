...
def rejection_greedy_sample_triton(
    ...
    start_idx = tl.where(
        offset == 0, 0, tl.load(cu_num_draft_tokens_ptr + offset - 1, is_greedy_mask)
    )
...
def rejection_random_sample_kernel(
    ...
    start_idxs = tl.where(
        offsets == 0, 0, tl.load(cu_num_draft_tokens_ptr + offsets - 1, not_greedy_mask)
    )
...
def expand_kernel(
    ...
    start_idx = tl.where(
        offset == 0, 0, tl.load(cu_num_tokens_ptr + offset - 1, len_mask)
    )
    ...
    for i in tl.range(0, BLOCK_SIZE):
        ...
        tl.store(
            output_ptr + start_idx1 + offset1, src_val1, mask=offset1 < num_tokens1
        )
...
def expand_triton(
    batch_size, expanded_x, x, cu_num_tokens, replace_from, replace_to, max_num_tokens
):
    vec_len = batch_size
