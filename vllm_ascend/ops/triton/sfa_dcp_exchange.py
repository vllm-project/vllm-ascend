...
def pack_raw_dcp_output_lse(
    output: torch.Tensor, lse: torch.Tensor, raw_row_words: int = 257
) -> torch.Tensor:
    ...
    _pack[(programs,)](
        words, lse, send, *words.stride()[:2], *lse.stride()[:2],
        tokens, block_rows, raw_row_words, multibuffer=False,
    )
    return send
