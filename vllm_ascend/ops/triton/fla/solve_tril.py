...
def solve_tril_16x16_kernel(
    ...
    for task_id in range(pid, total_tasks, core_num):
        ...
        for taskid in range(0, NTASKS):
            ...
            p_Ai = tl.make_block_ptr(Ad_ptr, (T, 16), (H * 16, 1), (base_t, 0), (N_BLOCKS * 16, 16), (1, 0))

            offs_rows_to_store = tl.arange(0, N_BLOCKS * 16)
            offs_cols_to_store = tl.arange(0, 16)

            p_Ai = Ad_ptr + base_t * H * 16 + 0 + offs_rows_to_store[:, None] * H * 16 + offs_cols_to_store[None, :]
...
def merge_16x16_to_32x32_inverse_kernel(
    ...
    for task_id in range(pid, total_tasks, core_num):
        ...
        A_ptr = A + (bos * H + i_h) * 32
        Ad_ptr = Ad + (bos * H + i_h) * 16
        Ai_ptr = Ai + (bos * H + i_h) * 32

        p_A_21 = tl.make_block_ptr(A_ptr, (T, 32), (H * 32, 1), (i_t * 32 + 16, 0), (16, 16), (1, 0))
        p_Ad_11 = tl.make_block_ptr(Ad_ptr, (T, 16), (H * 16, 1), (i_t * 32, 0), (16, 16), (1, 0))
        p_Ad_22 = tl.make_block_ptr(Ad_ptr, (T, 16), (H * 16, 1), (i_t * 32 + 16, 0), (16, 16), (1, 0))
        p_Ai_11 = tl.make_block_ptr(Ai_ptr, (T, 32), (H * 32, 1), (i_t * 32, 0), (16, 16), (1, 0))
        p_Ai_22 = tl.make_block_ptr(Ai_ptr, (T, 32), (H * 32, 1), (i_t * 32 + 16, 16), (16, 16), (1, 0))
        p_Ai_21 = tl.make_block_ptr(Ai_ptr, (T, 32), (H * 32, 1), (i_t * 32 + 16, 0), (16, 16), (1, 0))
...
def merge_16x16_to_64x64_inverse_kernel(
    ...
    for task_id in range(pid, total_tasks, core_num):
        ...
        ptr_Ad = Ad_ptr + offs_m[:, None] * (H * 16) + offs_n[None, :]
        ...
        ptr_A = A_ptr + offs_m[:, None] * (H * 64) + offs_n[None, :]
        ...
        ptr_Ai = Ai_ptr + offs_m[:, None] * (H * 64) + offs_n[None, :]
        ...
        ptr_Ai = Ai_ptr + offs_m[:, None] * (H * BT) + offs_n[None, :]
...
def solve_tril(
    ...
    core_num_vec = AIVECCORE_NUM
    solve_tril_16x16_kernel[(core_num_vec,)](
        A=A,
        Ad=Ad,
    ...
    core_num_cube = AICORE_NUM
    merge_fn[(core_num_cube,)](
        A=A,
        Ad=Ad,
        Ai=Ai,
        cu_seqlens=cu_seqlens,
