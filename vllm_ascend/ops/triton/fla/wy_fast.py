...
def _get_aicore_num():
    global _AICORE_NUM
    if _AICORE_NUM is None:
        device = torch.npu.current_device()
        properties = driver.active.utils.get_device_properties(device)
        _AICORE_NUM = properties["num_aicore"]
    return _AICORE_NUM


@triton.heuristics({"IS_VARLEN": lambda args: args["cu_seqlens"] is not None})
@triton.jit(do_not_specialize=["T", "H", "Hg", "K", "V", "grid0", "grid1"])
...
def recompute_w_u_fwd(
    ...
    AICORE_NUM = _get_aicore_num()
    recompute_w_u_fwd_kernel[(AICORE_NUM,)](
        k=k,
        v=v,
        beta=beta,
        w=w,
