...
def probe_cann_mega_moe_api() -> CannMegaMoeApiCapability:
    ...
    return CannMegaMoeApiCapability(
        True,
        supports_situ={"activation", "activation_alpha", "activation_beta"}.issubset(parameters),
...
def evaluate_cann_mega_moe_layer(
    ...
    *,
    ...
    if resolved_activation.name == "situ" and not api_capability.supports_situ:
        return _unsupported("installed MegaMoe API does not expose SiTU parameters", quant_type)
    if resolved_activation.name == "situ" and device_type != AscendDeviceType.A5:
