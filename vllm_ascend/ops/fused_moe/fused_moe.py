...
class AscendUnquantizedFusedMoEMethod(UnquantizedFusedMoEMethod):
    ...
    def process_weights_after_loading(self, layer):
        ...
        if enable_fused_mc2:
            ...
            if enable_fused_mc2 == 1 and getattr(self, "dynamic_eplb", False):
    ...
    def apply(
        ...
        if _EXTRA_CTX.moe_comm_type == MoECommType.FUSED_MC2:
            ...
            num_weight_tensors = len(w1)
            w1_scale = [torch.tensor([], dtype=torch.int64) for _ in range(num_weight_tensors)]
            w2_scale = [torch.tensor([], dtype=torch.int64) for _ in range(num_weight_tensors)]
            w1_scale_bias = [torch.tensor([], dtype=torch.float32) for _ in range(num_weight_tensors)]
            w2_scale_bias = [torch.tensor([], dtype=torch.float32) for _ in range(num_weight_tensors)]
        else:
            w1 = w13_weight_list if isinstance(w13_weight_list, list) else layer.w13_weight
            w1_scale = None
            w2 = w2_weight_list if isinstance(w2_weight_list, list) else layer.w2_weight
