...
class MoEMLPFamily(Enum):
    UNQUANTIZED = "unquantized"
    STANDARD_QUANT = "standard_quant"
...
class MoEMLPPlanKey:
    ...
    Communication type is intentionally absent. AllGather, AllToAll, and MC2
    share the same MLP kernels after dispatch; FusedMC2 bypasses this executor.
    """
...
def build_mlp_plan(key: MoEMLPPlanKey) -> MoEMLPPlan:
    if key.quant_type == QuantType.NONE:
        return MoEMLPPlan(key, MoEMLPFamily.UNQUANTIZED, GateUpKernel.UNQUANTIZED)
    if key.quant_type == QuantType.W4A16:
        return MoEMLPPlan(key, MoEMLPFamily.WEIGHT_ONLY, GateUpKernel.ANTIQUANT)
    if key.quant_type == QuantType.W4A8:
        ...
        return MoEMLPPlan(key, MoEMLPFamily.W4A8_PER_CHANNEL, kernel)
    ...
    return MoEMLPPlan(key, MoEMLPFamily.STANDARD_QUANT, kernel)
