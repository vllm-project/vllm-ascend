...
class AscendUnquantizedFusedMoEMethod(UnquantizedFusedMoEMethod):
    ...
    def __init__(self, moe: FusedMoEConfig = None, tid2eid=None):
        ...
        # Per-forward LoRA routing state shared between apply_gmm1 and
        # apply_gmm2 (the hook interface has no channel to thread it through).
    ...
    def get_moe_weights(self, layer):
        ...
        if self.dynamic_eplb and not (isinstance(w13_weight_list, list) and isinstance(w2_weight_list, list)):
            logger.warning_once(
    ...
    def apply_gmm1(self, mlp_compute_input: MoEMlpComputeInput):
        ...
        w1, _ = self._get_mlp_weights(layer)
    ...
    def apply_gmm2(self, mlp_compute_input: MoEMlpComputeInput, hidden_states, act_out_scale):
        layer = mlp_compute_input.layer
        _, w2 = self._get_mlp_weights(layer)
