...
from vllm_ascend.ops.fused_moe.moe_mlp_plan import GateUpKernel, MoEMLPFamily, MoEMLPPlan
...
def quant_apply_mlp(
    ...
    if kernel_plan.family == MoEMLPFamily.W4A8_PER_CHANNEL and (
        w1_scale_bias is None or w2_scale_bias is None
    ):
    ...
    if kernel_plan.family == MoEMLPFamily.STANDARD_QUANT:
        if kernel_plan.gate_up_kernel == GateUpKernel.CUSTOM_NZ:
        ...
        elif kernel_plan.gate_up_kernel == GateUpKernel.FUSED:
            # gmm1: gate_up_proj & act_fn: swiglu
        ...
        elif kernel_plan.gate_up_kernel == GateUpKernel.DECOMPOSED:
            # Step3.5/3.7 needs to clamp in swiglu: out = silu(gate).clamp(max=limit) * up.clamp(-limit, limit)
            gmm1_output_dtype = (
                torch.bfloat16
                if use_mxfp_quant or kernel_plan.key.activation == MoEMLPActivationKind.SWIGLUSTEP
                else _output_dtype
            )
            gmm1_kwargs = {
                "x": [hidden_states],
        ...
        elif kernel_plan.gate_up_kernel == GateUpKernel.DEQUANT_SWIGLU:
            # gmm1: gate_up_proj
            ...
            raise ValueError(
                f"Unsupported gate/up kernel {kernel_plan.gate_up_kernel} "
                f"for MLP family {kernel_plan.family}."
            )
    ...
    elif kernel_plan.family == MoEMLPFamily.WEIGHT_ONLY:
        ...
        dispose_tensor(unquantized_hidden_states)
        # act_fn: swiglu
        hidden_states = apply_unquantized_activation(
            hidden_states,
            kernel_plan.key.activation,
    ...
    elif kernel_plan.family == MoEMLPFamily.W4A8_PER_CHANNEL:
        if group_list_type == 0:
            group_list = convert_group_list(group_list, group_list_type, 1)
            group_list_type = 1
        bias1 = w1_scale_bias
        bias2 = w2_scale_bias
        # W4A8 per-channel kernels produce BF16 between the two GMMs.
        _output_dtype = torch.bfloat16

        if kernel_plan.gate_up_kernel == GateUpKernel.W4A8_PER_CHANNEL:
        ...
        raise ValueError(f"Unsupported quantized MoE MLP family: {kernel_plan.family}")
...
def execute_mlp_plan(
    ...
    if kernel_plan.family == MoEMLPFamily.UNQUANTIZED:
