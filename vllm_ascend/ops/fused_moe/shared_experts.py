...
class AscendSharedExperts:
    ...
    def _run_a8_int_mlp(
        ...
        if self.situ_activation is not None:
            quantized_x, swiglu_out_scale = torch.ops._C_ascend.dequant_situ_quant(
                ...
                # projection output to BF16. Other quantized shared experts
                # still enter with INT32 accumulator output.
                weight_scale=None if uses_kimi_k3_w4a8 else self.layer.gate_up_proj.weight_scale_fp32,
                activation_scale=None if uses_kimi_k3_w4a8 else pertoken_scale,
                bias=None,
                quant_scale=None,
                quant_offset=None,
                group_index=None,
    ...
    def _select_mlp_path(self) -> SharedExpertMLPPath:
        ...
        if (
            ...
            and (not uses_kimi_k3_w4a8 or self.situ_activation is not None)
        ):
