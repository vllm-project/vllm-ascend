...
class AscendUnquantizedFusedMoEMethod(UnquantizedFusedMoEMethod):
    ...
    def fused_experts_moge(
        ...
        if is_310p():
            gate_up_out = torch_npu.npu_swiglu(gate_up_out.to(
                torch.float32)).to(torch.float16)
        else:
        ...
        unsorted_topk_ids = torch.argsort(sorted_topk_ids.float()).to(
            torch.int32)
        unsorted_hidden_states = down_out_list.index_select(
            0, unsorted_topk_ids)
    ...
    def forward_oot_v01011(
        ...
        if topk_ids.shape[1] < top_k or is_310p():
            ...
            return self.fused_experts_moge(
                hidden_states=x,
                w1=layer.w13_weight,
                w2=layer.w2_weight,
    ...
    def forward_oot(
        ...
        if vllm_version_is(">=", "0.10.11"):
            ...
            return self.forward_oot_normal(
                layer=layer,
                x=x,
                use_grouped_topk=use_grouped_topk,
                top_k=top_k,
