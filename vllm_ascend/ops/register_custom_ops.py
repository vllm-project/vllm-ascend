    ...
    if forward_context.prefetch_mlp_gate_up_proj:
        ...
        with torch.npu.stream(weight_prefetch_stream):
            ...
            torch_npu.npu_prefetch(
                model_instance.model.layers[layer_idx].mlp.gate_up_proj.weight, x_dependency, mlp_gate_up_prefetch_size
            )
    return
...
def _maybe_prefetch_mlp_down_proj_impl(x_dependency: torch.Tensor) -> None:
    ...
    with torch.npu.stream(weight_prefetch_stream):
        ...
        torch_npu.npu_prefetch(
            model_instance.model.layers[layer_idx].mlp.down_proj.weight, x_dependency, mlp_down_prefetch_size
        )
