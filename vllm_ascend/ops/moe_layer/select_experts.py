...
class ExpertsSelector:
    ...
    def select_experts(self,
        ...
        Args:
            ...
            x: Hidden states of shape (num_tokens, hidden_size).
            top_k: number of top k experts.
            is_w8a8: whether to use global_num_experts.
        ...
        topk_weights, topk_ids = self.select_experts_with_fusion_ops(
        ...
        if topk_weights is None:
            topk_weights, topk_ids = self.native_select_experts(
                hidden_states=x,
                router_logits=router_logits,
                top_k=top_k,
                use_grouped_topk=use_grouped_topk,
                ...
                scoring_func=scoring_func,
                e_score_correction_bias=e_score_correction_bias,
                global_num_experts=global_num_experts if is_w8a8 else None,
            )
    ...
    def select_expert_use_group_topk(
            self, topk_weights: torch.Tensor, topk_group: Optional[int],
        ...
        topk_weights = self.native_grouped_topk(topk_weights, num_expert_group,
                                                topk_group)
    ...
    def select_experts_with_fusion_ops(
            self,
            router_logits: torch.Tensor,
            top_k: int,
            use_grouped_topk: bool,
        ...
        if not use_grouped_topk and custom_routing_function is None and scoring_func == "softmax" and is_unquantized:
            ...
            topk_ids = topk_ids.to(torch.int32)
            topk_weights = self._renormalize_topk_weights(
                topk_weights, renormalize)

        return topk_weights, topk_ids

    def native_select_experts(
        self,
        ...
        if use_grouped_topk:
            return self.select_expert_use_group_topk(
