...
class AscendVocabParallelEmbedding(VocabParallelEmbedding):
    ...
    def _forward_origin(self, input_):
        ...
        # A replicated layer (e.g. the DSpark Markov head) has tp_size==1 and
        # already holds the full output locally, so the reduce must be skipped
        # even when the global TP group is larger than 1.
        if self.tp_size == 1:
