...
class RejectionSamplerConfig:
    ...
    def _validate(self):
        ...
        if self.fly_entropy_threshold < 0:
            raise ValueError(
                f"rejection_sampler_config.fly_entropy_threshold must be >= 0, got {self.fly_entropy_threshold}"
            )
    ...
    def derive_and_validate(self, vllm_config: VllmConfig) -> AscendConfig:
        ...
        if fly_config.enable_fly_verify:
            ...
            if spec_config is None:
                raise ValueError("enable_fly_verify requires speculative decoding to be enabled.")
            num_speculative_tokens = spec_config.num_speculative_tokens
            if num_speculative_tokens < 2:
                raise ValueError(
                    f"enable_fly_verify requires num_speculative_tokens >= 2, got {num_speculative_tokens}."
                )
            ...
            if spec_config.rejection_sample_method != "standard":
                raise ValueError("enable_fly_verify currently requires rejection_sample_method='standard'.")
            if vc.use_v2_model_runner:
                raise ValueError("enable_fly_verify currently supports ModelRunner V1 only.")
            logger.warning_once(
