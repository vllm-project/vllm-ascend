...
# """Inference-only OpenPangu/DeepseekV3 model."""
...
from vllm.model_executor.models.deepseek_v2 import \
    yarn_get_mscale, get_spec_layer_idx_from_weight_name 
from vllm.model_executor.models.openpangu import (
    OpenPanguDecoderLayer, OpenPanguMLAAttention)
...
class TorchairOpenPanguSiluAndMul(SiluAndMul):
...
class TorchairOpenPanguMergedReplicatedLinear(ReplicatedLinear):
...
class TorchairOpenPanguRowParallelLinearReplaceAllreduce(RowParallelLinear):
...
class TorchairOpenPanguRowParallelLinear(RowParallelLinear):
...
class TorchairOpenPanguMLP(nn.Module):
    ...
    def __init__(
        ...
        if not force_replicate:
            ...
            self.gate_up_proj = TorchairOpenPanguMergedReplicatedLinear(
                hidden_size, [intermediate_size] * 2,
                bias=False,
                quant_config=quant_config,
                prefix=f"{prefix}.gate_up_proj")
        ...
        if isinstance(quant_method, UnquantizedLinearMethod):
            self.act_fn = TorchairOpenPanguSiluAndMul()
...
class TorchairOpenPanguMoE(nn.Module):
    ...
    def __init__(
        ...
        if config.n_shared_experts is not None:
            ...
            self.shared_experts = TorchairOpenPanguMLP(
                hidden_size=config.hidden_size,
        ...
        TorchairOpenPanguMoE.top_k = config.num_experts_per_tok
    ...
    def forward(self,
        ...
        experts_hidden_states = self.experts(
            ...
            top_k=TorchairOpenPanguMoE.top_k,
...
class TorchairOpenPanguMLAAttention(OpenPanguMLAAttention):
    ...
    def __init__(
        ...
        if oproj_tp_enable():
            ...
            self.o_proj = TorchairOpenPanguRowParallelLinearReplaceAllreduce(
            ...
            self.o_proj = TorchairOpenPanguRowParallelLinear(
                self.num_heads * self.v_head_dim,
                self.hidden_size,
                bias=False,
                quant_config=quant_config,
...
class TorchairOpenPanguDecoderLayer(OpenPanguDecoderLayer):
    ...
    def __init__(
        ...
        if model_config.use_mla:
            attn_cls = TorchairOpenPanguMLAAttention  # type: ignore[assignment]
            self.use_mla = True
    ...
    def forward(
        ...
        if isinstance(self.mlp, TorchairOpenPanguMoE):
        ...
        if isinstance(self.mlp, TorchairOpenPanguMLP
                      ) and hidden_states.dtype == torch.float16:
            # Fix FP16 overflow
            # Scaling the OpenPanguMLP output, it is the input of
            # input_layernorm of next decoder layer.
            # The scaling of OpenPanguMOE output would be done in the forward
            # of OpenPanguMOE
            hidden_states *= 1. / self.routed_scaling_factor
        if self.sandwich_norm:
...
class TorchairOpenPanguModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: TorchairOpenPanguDecoderLayer(
                config,
                prefix,
...
class TorchairPanguUltraMoEForCausalLM(PanguUltraMoEForCausalLM):
    # add `packed_modules_mapping` in `PanguUltraMoEForCausalLM` to support weight merging
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.model = TorchairOpenPanguModel(vllm_config=vllm_config,
                                             prefix=maybe_prefix(
                                                 prefix, "model"))
        if get_pp_group().is_last_rank:
