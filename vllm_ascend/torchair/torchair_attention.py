...
@dataclass
class AscendTorchairMetadataBuildInfo(AscendAttentionMetadataBuildInfo):
    num_reqs: int = 0
...
class AscendAttentionTorchairMetadataBuilder(AscendAttentionMetadataBuilder):
    ...
    def _assemble_build_info(
        ...
        *args,
        **kwargs,
    ) -> "AscendTorchairMetadataBuildInfo":
        ...
        build_info = AscendTorchairMetadataBuildInfo(
            num_reqs=num_reqs,
    ...
    def _assemble_attn_metadata(
        ...
        graph_pad_size = build_info.graph_pad_size
        input_positions = build_info.input_positions

        decode_metadata = None
        use_torchair_graph = graph_pad_size > -1

        # Assemble `AscendDecodeMetadata`
        if attn_state in [AscendAttentionState.DecodeOnly]:
            max_seq_lens = seq_lens.max().item()
            num_seqs = len(seq_lens)
            if use_torchair_graph and attn_state in [
