...
def _ascend_get_kv_cache_config_from_groups(
    ...
    Attach ``kv_transfer_config`` onto every built config: vLLM 0.28.0's
    ``KVCacheConfig`` has no native field, and the PD prefill-producer role
    read back by the kv-cache coordinator (see
    ``patch_kv_cache_coordinator.py``) must ride on the config object - the
    coordinator factory receives ``KVCacheConfig`` but not ``VllmConfig``.
    The attribute survives the scheduler-side ``copy.deepcopy`` in
    ``generate_scheduler_kv_cache_config`` and is dropped by worker pickle
    IPC, which never reads it.
    """
    if _get_glm5_next_cache_layout(kv_cache_groups) is not None:
        kv_cache_config = get_glm5_next_kv_cache_config(vllm_config, kv_cache_groups, available_memory)
    elif vllm_version_is("0.28.0") or not _is_deepseek_v4_groups(kv_cache_groups):
