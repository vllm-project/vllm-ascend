...
def _replicated_draft_pool_bytes_per_block(groups: list[KVCacheGroupSpec]) -> int | None:
...
def _replicated_draft_max_memory_usage_bytes(config: VllmConfig, groups: list[KVCacheGroupSpec]) -> int | None:
    """Reserve enough shared pool slots for all groups of one max-length request."""
    bytes_per_block = _replicated_draft_pool_bytes_per_block(groups)
...
def _ascend_pool_bytes_per_block(kv_cache_groups: list[KVCacheGroupSpec]) -> int:
    """Match Ascend's replicated-draft and DSV4 pool layouts.
    ...
    """
    replicated_bytes = _replicated_draft_pool_bytes_per_block(kv_cache_groups)
...
def _ascend_max_memory_usage_bytes_from_groups(
    ...
    """Use layout-specific admission estimates for replicated drafts and DSV4."""
    replicated_bytes = _replicated_draft_max_memory_usage_bytes(vllm_config, kv_cache_groups)
    if replicated_bytes is not None:
        return replicated_bytes
    if not _is_deepseek_v4_groups(kv_cache_groups):
