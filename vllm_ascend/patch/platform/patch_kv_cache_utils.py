...
from vllm_ascend.core.deepseek_v41_kv_cache import (
    ...
    uses_deepseek_v41_cache_layout,
)
from vllm_ascend.core.deepseek_v41_kv_cache import (
...
def _ascend_max_concurrency(vllm_config, kv_cache_config):
    ...
    if uses_deepseek_v41_cache_layout(groups):
...
def _ascend_resolve_kv_cache_block_sizes(
    ...
    if filtered_private_groups and not uses_deepseek_v41_cache_layout(groups):
...
def _get_kv_cache_groups_uniform_groups(
    ...
    if any(is_deepseek_v41_cache_spec(s) for g in grouped_specs for s in g.kv_cache_specs.values()):
...
def _get_kv_cache_config_deepseek_v4(
    ...
    if uses_deepseek_v41_cache_layout(kv_cache_groups):
