...
def verify_and_update_config(cls, vllm_config) -> None:
    ...
    if model_config.use_mla:
        ...
        if _uses_mla_fa_quant_cache(vllm_config):
            # MLA C8 quantizes only latent K; the positional cache stays in
            # model dtype. Use the actual K bytes so the logical block grows
            # from 384 to 768 tokens and remains aligned with the Mamba state.
            ...
            logger.info(
                "Using hybrid MLA 8-bit latent-cache layout: K=%d B/token, V=%d B/token.",
