...
def verify_and_update_config(cls, vllm_config) -> None:
    ...
    is_sparse_glm5 = _is_sparse_glm5_next(model_config)
    if is_sparse_glm5:
        ...
        if cache_config.block_size != attn_block_size:
            ...
            logger.info(
                "Setting GLM-Next attention block size to %d tokens to align "
                "MLA, KDA, and compressed indexer cache pages.",
                attn_block_size,
            )
    else:
    ...
    # GLM-Next packs the complete recurrent state into the same large-page
