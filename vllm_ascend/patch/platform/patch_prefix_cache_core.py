...
"""Compatibility patch for DSv4 partial compressed prefix-cache hits.

This carries the small-core hooks needed by the DeepSeek V4 prefix-cache
coordinator patch: forwarding the scheduler cache-hit granularity on vLLM
versions that do not thread it natively, cleaning up partial-prefix cache
entries on block eviction, and forwarding the per-request KV copy block ids to
the scheduler output. Remove it when the supported vLLM version provides these
APIs and behaviors natively.
"""
...
def _patch_kv_cache_manager_scheduler_block_size() -> None:
    ...
    kv_cache_manager_cls.__init__ = __init__  # type: ignore[method-assign]
...
def _patch_scheduler_scheduler_block_size() -> None:
    ...
    __init__._vllm_ascend_scheduler_block_size_patch = True  # type: ignore[attr-defined]
    scheduler_cls.__init__ = __init__  # type: ignore[method-assign]
...
def _patch_partial_prefix_cache_cleanup() -> None:
    ...
    _maybe_evict_cached_block._vllm_ascend_partial_prefix_cache_cleanup_patch = True  # type: ignore[attr-defined]
...
def _patch_scheduler_copy_blocks() -> None:
    ...
    for scheduler_cls in scheduler_classes:
        ...
        schedule._vllm_ascend_copy_blocks_patch = True  # type: ignore[attr-defined]
        scheduler_cls.schedule = schedule  # type: ignore[method-assign]
