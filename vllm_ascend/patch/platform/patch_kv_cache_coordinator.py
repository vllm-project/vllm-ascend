...
class AscendHybridKVCacheCoordinator(HybridKVCacheCoordinator):
    ...
    def __init__(  # type: ignore[misc]
        ...
        # in ``patch_kv_cache_utils`` (vLLM 0.28.0's ``KVCacheConfig`` has no
        # native field; the attach survives the scheduler-side deepcopy and
        # is dropped by worker pickle IPC, which never reads it). Configs
        # built without it (e.g. unit tests) read as standalone
