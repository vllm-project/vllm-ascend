...
class AscendHybridKVCacheCoordinator(HybridKVCacheCoordinator):
    ...
    def __init__(  # type: ignore[misc]
        ...
        # Mirror upstream KVCacheCoordinator.__init__: per-group resolved block
        # sizes, consumed by the simple CPU offload scheduler. Fall back to the
        # hash block size for manager doubles that do not expose ``block_size``.
        self.group_block_sizes = tuple(
            getattr(manager, "block_size", hash_block_size) for manager in self.single_type_managers
        )
