...
def _mamba_block_aligned_split(
    ...
    if num_computed_tokens < max(request.num_prompt_tokens, request.num_tokens - 1):
        ...
        # Upstream vLLM PR #37898 added Marconi-style prefix-cache admission for
        # hybrid Mamba models: the scheduler may pass the uncached common-prefix
        # length as an extra argument, and the split should stop on that aligned
        # boundary so the Mamba state is cached consistently with the attention
        # prefix cache.
