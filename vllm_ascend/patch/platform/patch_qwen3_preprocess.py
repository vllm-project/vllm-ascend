...
"""Qwen3-family image preprocessing offload."""
...
def _apply_hf_processor_main_with_npu_preprocess(
    ...
    # the upstream contract until video preprocessing offload is implemented.
    enabled = (
...
def _preprocess_with_npu_preprocess(
    ...
    **kwargs,
    ...
    for shape, stacked_images in grouped_images.items():
        if do_resize and not _is_resize_offload_enabled():
    ...
    processed_grids = {}
    processed_image_hw = {} if _is_resize_offload_enabled() else None
    for shape, stacked_images in grouped_images.items():
        batch_size, _, input_height, input_width = stacked_images.shape
        if _is_resize_offload_enabled() and do_resize:
            resized_height, resized_width = smart_resize(
                input_height,
                input_width,
                factor=patch_size * merge_size,
            ...
            resized_height, resized_width = input_height, input_width

        if _is_rescale_normalize_offload_enabled():
            processed_images = stacked_images
            if processed_images.dtype != torch.uint8:
                raise ValueError(
                    f"Qwen3 image preprocessing stage 2 or 3 expects UINT8 images, got {processed_images.dtype}"
                )
        else:
        ...
        # below produces the flat CHW transport contract consumed by the Worker.
        processed_images_grouped[shape] = processed_images.flatten(1)
        processed_grids[shape] = [[1, grid_h, grid_w]] * batch_size
        if processed_image_hw is not None:
            processed_image_hw[shape] = [[input_height, input_width]] * batch_size
    ...
    if processed_image_hw is not None:
        image_hw_ordered = reorder_images(processed_image_hw, grouped_images_index)
        data["image_hw"] = torch.tensor(image_hw_ordered, dtype=torch.long)
...
def _get_mm_fields_config_with_npu_preprocess(
    ...
    if _is_resize_offload_enabled():
        ...
        size_per_image = image_hw.prod(-1) * channels
        configs["image_hw"] = MultiModalFieldConfig.batched("image", keep_on_cpu=True)
    else:
