...
# - MiMo-V2 fp8 checkpoint on NPU: disable fp8 quantization (load bf16
...
_DISABLE_FP8_LOG = (
    "Detected fp8 MiMo-V2 checkpoint on NPU. "
...
def _patched_verify_cuda_graph(self: ModelConfig) -> None:
    ...
    if (
        ...
        if expansion_mode is None:
            ...
            logger.info("Set HCCL_OP_EXPANSION_MODE=AIV for MiMo-V2 ACL graph capture on NPU.")
        elif expansion_mode != "AIV":
            logger.warning(
                "HCCL_OP_EXPANSION_MODE=%s may reduce ACL graph shape "
                "coverage for MiMo-V2 on NPU. Recommended value: AIV.",
                expansion_mode,
            )
