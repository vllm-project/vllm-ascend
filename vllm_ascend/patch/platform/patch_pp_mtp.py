...
def _patch_scheduler_dynamic_gate_compat() -> None:
            ...
            if not _supports_adaptive_physical_k(vllm_config):
                ...
                try:
                    ...
                    self._ascend_physical_k_controller = AdaptiveDraftKController(
                        ...
                        hybrid_min_batch_size=int(
                            method_params.get("hybrid_min_batch_size", 8)
                        ),
                        hybrid_acceptance_threshold=float(
                            method_params.get(
                                "hybrid_acceptance_threshold", 0.6
                            )
                        ),
                        hybrid_low_steps=int(
                            method_params.get("hybrid_low_steps", 4)
                        ),
                        hybrid_high_steps=int(
                            method_params.get("hybrid_high_steps", 2)
                        ),
                        hybrid_probe_interval=int(
                            method_params.get("hybrid_probe_interval", 32)
                        ),
                    )
    ...
    if not getattr(original_update, "_vllm_ascend_physical_k_patched", False):
        ...
        def _patched_update_after_schedule(self, scheduler_output):
            ...
            configured_k = int(
                getattr(scheduler_output, "num_spec_tokens_to_schedule", 0)
            )
...
def _patch_scheduler_update_from_output() -> None:
    ...
    def _observe_physical_k(
        ...
        if controller.observation_count and log_count < 16:
            logger.warning(
                "V2 physical-K feedback #%d: scheduled=%s accepted=%s "
                "previous_k=%s next_k=%s",
                log_count + 1,
    ...
    def _patched_update_from_output(self, scheduler_output, model_runner_output):
        ...
        _update_proposal_lengths(self, scheduler_output, model_runner_output)
