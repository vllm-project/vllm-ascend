...
Parallelism (VPP) scheme2 on top of vLLM v0.22.1.

vLLM v0.22.1 ships the lower-level VPP plumbing
(``VppContinuationOutput`` in ``vllm.v1.outputs``, the corresponding
scheduler hooks in ``vllm.v1.core.sched.scheduler``), but does NOT wire
it into the engine core. This patch closes that gap by:
  ...
  * ``EngineCore.step_with_batch_queue`` — respect
    ...
    replay ``_deferred_scheduler_output`` on the next visit.
...
patched separately via ``patch_vpp`` / ``patch_vpp_multiproc_executor``.
"""
...
def _ascend_vpp_post_init(self: EngineCore) -> None:
    ...
    if self.vpp_enabled and self.batch_queue is not None:
        ...
        # schedule can over-subscribe; +10 headroom mirrors the upstream
        # VPP-engine invariant.
        self.batch_queue_size += 1
        self.batch_queue = deque(maxlen=self.batch_queue_size + 10)
...
def _ascend_vpp_step_with_batch_queue(
    ...
    Mirrors the VPP-enabled implementation: respects
    ``allow_over_batch_queue``, recognizes ``VppContinuationOutput``,
    and replays ``_deferred_scheduler_output`` across calls.
    """
