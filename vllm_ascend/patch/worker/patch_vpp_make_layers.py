...
``make_layers`` -> ``setup_vpp_layers`` (see ``patch_vpp_qwen3`` and the
now-removed ``patch_vpp_deepseek``).  ``make_layers`` is the single entry
point *every* model uses to build its decoder stack under pipeline
parallelism, so patching it here makes V-shaped fold-back layer assignment
available to all of them automatically.
...
handled centrally, so most models need no per-model shim:
...
* Forward-pass range selection -- under ``ENABLE_VPP`` the model runner
  ...
  change.  ``patch_vpp_qwen3`` is retained only as a fallback forward
  shim for a model whose upstream path has not yet been verified
  end-to-end.
"""
