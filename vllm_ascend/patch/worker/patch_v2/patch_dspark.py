...
def _load_dspark_model_with_target_quant(target_model, vllm_config):
    ...
    try:
        # Upstream load_dspark_model now supports pipeline parallelism (the
        # draft loads with a single-PP draft parallel config and
        # `maybe_share_target_embed` handles the PPMissingLayer embed), so the
        # PP-group / embed-sharing shims this wrapper used to install are no
        # longer needed. Only the config-level PP bypass remains so the draft
        # model builds unsharded.
