...
_EMPTY_PATCHED = "_vllm_ascend_eagle3_pp_empty_patched"
...
def install_qwen3_eagle3_pp_aux(model: nn.Module) -> nn.Module:
    from vllm.model_executor.models.qwen3 import Qwen3Model

    inner_model = getattr(model, "model", None)
    if not isinstance(inner_model, Qwen3Model):
        raise TypeError(
            f"Ascend MRV2 Eagle3 PP currently supports only Qwen3Model targets, got {type(inner_model).__name__}."
        )
...
def _patch_make_empty_intermediate_tensors(inner_model: nn.Module) -> None:
    if getattr(inner_model, _EMPTY_PATCHED, False):
        return
    ...
    setattr(inner_model, _EMPTY_PATCHED, True)
