...
if Step3p5Attention is not None:
    ...
    def _patched_forward(
        ...
        if self.use_rope:
            ...
            if not hasattr(self, "_q_w_cached"):
                self._q_w_cached = 1.0 + self.q_norm.weight.float()
                self._k_w_cached = 1.0 + self.k_norm.weight.float()
            q_w = self._q_w_cached
            k_w = self._k_w_cached
