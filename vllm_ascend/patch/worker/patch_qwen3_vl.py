...
from .patch_qwen2_5_vl import (Qwen2_5_VisionAttention, all_gather_2d,
                               get_rank_world)
...
class AscendQwen3_VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        attn_backend_override = (multimodal_config.mm_encoder_attn_backend
                                 if multimodal_config else None)
    ...
    def forward(
        ...
        if not self.use_data_parallel:
            ...
            padding_size = math.ceil(
                math.ceil(seq_len / tp_world_size) /
                merge_size) * merge_size * tp_world_size - seq_len
            if padding_size > 0:
            ...
            hidden_states = hidden_states.chunk(tp_world_size,
                                                dim=0)[tp_rank % tp_world_size]
        ...
        if not self.use_data_parallel:
            hidden_states = all_gather_2d(hidden_states,
