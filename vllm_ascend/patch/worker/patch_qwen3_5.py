...
def _all_gather_hidden_and_residual(
    ...
    # Local copy of the Qwen3-Next helper removed from upstream when the model
    # moved to ``sequence_parallel_chunk``; the Ascend MTP forward still needs it.
    # FlashComm maintains its own sequence-parallel communication, so let the
    # patched linear layers gather the sharded input instead of gathering it
    # once here and again in the column-parallel projection.
