...
def _build_non_spec_decode_causal_conv1d_host_meta(
    ...
    if attn_metadata.non_spec_state_indices_tensor is None:
        raise RuntimeError(
            "Expected attn_metadata.non_spec_state_indices_tensor for patched GDN non-spec decode path."
        )
...
def _build_spec_causal_conv1d_host_meta(
    ...
    if attn_metadata.spec_state_indices_tensor is None:
        raise RuntimeError(
            "Expected attn_metadata.spec_state_indices_tensor for patched GDN speculative path."
        )
