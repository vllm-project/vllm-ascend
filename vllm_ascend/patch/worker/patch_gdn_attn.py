...
def _get_gdn_num_heads(builder) -> int:
    ...
    if hf_text_config is not None and hasattr(hf_text_config, "linear_num_value_heads"):
        return (
            ...
            // builder.vllm_config.parallel_config.tensor_parallel_size
        )
...
def _allocate_chunked_prefill_slot(builder, device: torch.device):
    ...
    return _GDNChunkedPrefillBufferSlot(
        ...
        chunk_indices_64_device=torch.empty(
            (max_num_batched_tokens, 2), **device_kwargs
        ),
        ...
        chunk_indices_large_block_cpu=torch.empty(
            (max_num_batched_tokens, 2), **cpu_kwargs
        ),
...
def _build_non_spec_query_start_loc_cpu(
    ...
    if (
        ...
        or num_decode_draft_tokens_cpu[num_decode_draft_tokens_cpu >= 0].sum().item()
        == 0
    ):
...
def _build_non_spec_chunked_prefill_meta_cpu(
    builder, cu_seqlens_cpu: torch.Tensor
) -> GDNChunkedPrefillMetadata:
    chunk_counts_64 = _prepare_chunk_counts_cpu(
        cu_seqlens_cpu, builder._ascend_gdn_chunk_size
    )
    chunk_counts_large = _prepare_chunk_counts_cpu(
        cu_seqlens_cpu, builder._ascend_gdn_large_block_size
    )
    chunk_counts_cumsum = _prepare_chunk_counts_cpu(
        cu_seqlens_cpu, builder._ascend_gdn_cumsum_block_size
    )
    ...
    return GDNChunkedPrefillMetadata(
        ...
        update_chunk_offsets_64=update_chunk_offsets_64.to(
            builder._ascend_gdn_chunk_meta_device
        ),
        final_chunk_indices_64=final_chunk_indices_64.to(
            builder._ascend_gdn_chunk_meta_device
        ),
        chunk_indices_large_block=chunk_indices_large_block.to(
            builder._ascend_gdn_chunk_meta_device
        ),
        block_indices_cumsum=block_indices_cumsum.to(
            builder._ascend_gdn_chunk_meta_device
        ),
    )
...
def _build_non_spec_chunked_prefill_meta(
    ...
    builder._ascend_gdn_chunked_prefill_pool_idx = (
        builder._ascend_gdn_chunked_prefill_pool_idx + 1
    ) % len(builder._ascend_gdn_chunked_prefill_pool)
...
def _patched_build(
    ...
    setattr(attn_metadata, "non_spec_chunked_prefill_meta", None)
    if attn_metadata.num_prefills <= 0:
    ...
    attn_metadata.non_spec_chunked_prefill_meta = (
        _build_non_spec_chunked_prefill_meta(self, non_spec_query_start_loc_cpu)
    )
    return attn_metadata
