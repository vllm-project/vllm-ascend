    ...
    *,
    ...
    if rope_type == "rope_2d":
        embed_dim_reduction_factor = vision_model.merge_kernel_size[
            0] * vision_model.merge_kernel_size[1]
    else:
...
class AscendQwen3VLMoeForConditionalGeneration(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if not multimodal_config.get_limit_per_prompt(
                "image") and not multimodal_config.get_limit_per_prompt(
                    "video"):
            self.visual = None
        else:
        ...
        self.image_post_process_config(config.vision_config,
    ...
    def image_post_process_config(self, vision_config, model_config):
        ...
        self.rescale_factor = processor.info.get_hf_processor(
    ...
    def _fuse_mean_std_and_rescale_factor(self, do_normalize, image_mean,
                                          image_std, do_rescale,
                                          rescale_factor, device):
        if do_rescale and do_normalize:
            image_mean = torch.tensor(image_mean,
                                      device=device) * (1.0 / rescale_factor)
    ...
    def rescale_and_normalize(self, images, do_rescale, rescale_factor,
        ...
        if do_normalize:
            ...
            images = functional.normalize(images.to(torch.float32), image_mean,
                                          image_std).to(origin_dtype)
        elif do_rescale:
    ...
    def _process_image_input(
        ...
        if image_input["type"] == "image_embeds":
            ...
            pixel_values = pixel_values.reshape(-1, self.channel,
                                                self.patch_size,
                                                self.patch_size)
            ...
            pixel_values = pixel_values.reshape(
                -1, self.channel * self.temporal_patch_size * self.patch_size *
                self.patch_size)
