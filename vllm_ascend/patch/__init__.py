...
#
# ** 6. File: platform/patch_hunyuan_vl_processor.py**
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   1. `vllm.model_executor.models.hunyuan_vision.HunYuanVLProcessingInfo.get_hf_processor`
#      `vllm.model_executor.models.hunyuan_vision.HunYuanVLMultiModalProcessor._call_hf_processor`
#    Why:
#       Older HunyuanOCR tokenizer snapshots do not expose the named image-token
#       attributes required by the native Transformers processor. In addition,
#       vLLM v0.25.1 uses the legacy `use_fast` loader and accepts bare image
#       tokens, while the native processor requires the PIL backend and
#       `<no_100><no_102><no_101>` framing.
#    How:
#       On all supported vLLM versions, use a thin native processor subclass to
#       restore and validate the actual tokenizer's named image-token schema
#       before native initialization. On v0.25.1 only, skip the bundled image
#       processor's obsolete registration while importing the model and wrap
#       bare image tokens before processing.
#    Related PR:
#       1. https://github.com/vllm-project/vllm/pull/47872
#       2. https://github.com/vllm-project/vllm/pull/47867
#    Future Plan:
#       Remove the v0.25.1 branches when that lane is retired, and remove the
#       tokenizer compatibility after cached model snapshots carry the schema.
#
