...
This module is only imported when the 310P Model Runner V2 is created. Model
Runner V1 never imports it, so V1 stays free of dispatcher side effects.

The dispatcher is the vendored shim in ``vllm_ascend.worker.v2.triton_dispatcher``
(a copy of upstream vLLM PR #43048). When upstream merges that PR, switch the
import below to ``vllm.model_executor.triton_dispatcher``.
"""
