...
class CompressAttentionManager(FullAttentionManager):
    ...
    # NOTE: Two-phase KV allocation (vLLM PR #44409). The coordinator now calls
    # ``add_local_computed_blocks`` (touch every group's local cache-hit blocks)
    # for all groups first, then ``allocate_external_computed_blocks`` for all
    # groups, so an earlier group's external ``get_new_blocks`` can no longer
    # evict a later group's not-yet-touched cache-hit blocks.
    #
    # The local phase is inherited unchanged from the base
    # ``SingleTypeKVCacheManager.add_local_computed_blocks``: compressed
    # attention never skips blocks (``get_num_skipped_tokens`` == 0) and the hit
    # blocks handed in by ``find_longest_cache_hit`` are already compressed
    # logical blocks, so the base touch/extend/num_cached_block logic is correct
    # as-is. Only the external phase below must account for ``compress_ratio``.
    def allocate_external_computed_blocks(
        ...
        ``add_local_computed_blocks`` (issue #33775). Block counts are derived
        from the *compressed* token length (``// compress_ratio``), matching the
        physical blocks a compressed-attention group actually consumes.
        ...
        # Compressed attention stores one physical block per ``compress_ratio``
        # logical tokens.
        num_total_computed_tokens //= self.compress_ratio
        # Compressed (full-attention-derived) groups never skip blocks, but keep
        # the guard to mirror the base contract.
