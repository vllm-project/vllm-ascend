...
class ProfilingChunkScheduler(Scheduler):
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:  # noqa: C901
        ...
        # Mamba boundary offers. Block tables resolve lazily through the
        # manager so the connector only copies tables it reads. (vLLM main only)
        ...
        if KVConnectorBlockState is not None:
            ...
            if self.connector is not None:
                # Any request scheduled this step can become a connector job,
                # not only the ones that allocated blocks.
