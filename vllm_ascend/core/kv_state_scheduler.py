...
class KVStateSchedulerConfig(SchedulerConfig):
    scheduler_cls: str | type[object] = "vllm_ascend.core.kv_state_scheduler.KVStateScheduler"
    ...
    def initialize_from_config(cls, vllm_config: VllmConfig):
        ...
        if vllm_scheduler_config.async_scheduling:
            scheduler_config["scheduler_cls"] = "vllm_ascend.core.kv_state_scheduler.AsyncKVStateScheduler"
        else:
            scheduler_config["scheduler_cls"] = "vllm_ascend.core.kv_state_scheduler.KVStateScheduler"
...
class KVStateScheduler(Scheduler):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if not preempted_reqs:
            ...
            while self.waiting and token_budget > 0 and waiting_token_budget > 0:
                ...
                if (
                    self.kv_state_manager is not None
                    and request.request_id not in self.kv_state_manager.req_to_state_id
                ):
                ...
                # NOTE(zxr): when pd disaggregation, new_state can be None, use kv_state_manager.req_to_state_id to
                # replace
    ...
    def _update_waiting_for_remote_kv(self, request: Request) -> bool:
        ...
        if request.request_id in self.failed_recving_kv_req_ids:
            ...
            # For deepseek-v4, we assume that we can get all tokens from P-node because we don't use prefix cache and
            # chunked prefill
...
class AsyncKVStateScheduler(AsyncScheduler, KVStateScheduler):
    ...
    def _update_after_schedule(
        ...
        for req_id in scheduler_output.num_scheduled_tokens:
            ...
            if (
                request.num_computed_tokens
                == request.num_tokens + request.num_output_placeholders + cur_num_spec_tokens
            ):
