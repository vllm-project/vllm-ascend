...
class AscendSFAMetadata:
    ...
    li_reorder_indices: torch.Tensor | None = None
...
class AscendSFAMetadataBuilder(MLACommonMetadataBuilder[AscendSFAMetadata]):
    ...
    def build(
        ...
        # ====================================================================
        # Step 1: lightning_indexer_skip
        # Reorder the batch: non-skip tokens first, skip tokens appended as
        ...
        if self.enable_lightning_indexer_skip:
            (li_reorder_indices, li_cum_query_lens, li_seq_lens, li_skipped_query_mask, num_of_non_skip_tokens) = (
                get_li_skip_indices(seq_lens - tokens, tokens)
            )
            ...
            if li_reorder_indices is not None:
                ...
                li_seq_lens = (
                    torch.from_numpy(li_seq_lens)
                    .pin_memory()
                    .to(dtype=torch.int32, device=self.device, non_blocking=True)
                )
                li_skipped_query_mask = (
                    torch.from_numpy(li_skipped_query_mask)
                    .pin_memory()
                ...
                block_table = torch.cat([block_table, block_table[li_skipped_query_mask]], dim=0)
        ...
        # ====================================================================
        # Step 2: dsa_cp
        # Split tokens evenly across TP devices, recompute per-device
        # actual_seq_lengths_query and actual_seq_lengths_key.
        # When combined with lightning_indexer_skip, operates on the
        # already-reordered cum_query_lens/seq_lens and uses skip-aware
        ...
        if self.enable_lightning_indexer_skip and self.enable_dsa_cp:
            top_k_indices_of_skipped_queries_numpy = get_index_of_skipped_queries_numpy(
                actual_seq_lengths_query, actual_seq_lengths_key, num_reqs, SPARSE_COUNT
            )
        if self.enable_lightning_indexer_skip:
            if self.enable_dsa_cp:
                top_k_indices_of_skipped_queries_numpy = get_index_of_skipped_queries_numpy(
                ...
                top_k_indices_of_skipped_queries_numpy = get_index_of_skipped_queries_numpy(
                    cum_query_lens, seq_lens, num_reqs, SPARSE_COUNT
                )
        top_k_indices_of_skipped_queries_numpy = (
            torch.from_numpy(top_k_indices_of_skipped_queries_numpy)
            .pin_memory()
            .to(dtype=torch.int32, device=self.device, non_blocking=True)
            if top_k_indices_of_skipped_queries_numpy is not None
            else None
        )
        ...
        return self.metadata_cls(  # type: ignore
            ...
            li_reorder_indices=li_reorder_indices,
...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def indexer_select_post_process(
        ...
        if attn_metadata.skip:
            num_of_non_skip_tokens = attn_metadata.num_of_non_skip_tokens
            if num_of_non_skip_tokens > 0:
                x = x[:num_of_non_skip_tokens]
                q_c = q_c[:num_of_non_skip_tokens]
                cos = cos[:num_of_non_skip_tokens]
                sin = sin[:num_of_non_skip_tokens]
        else:
            num_of_non_skip_tokens = x.shape[0]
        ...
        if attn_metadata.skip and num_of_non_skip_tokens == 0:
            ...
            top_k_indices_no_skip_li_query = torch.ops._C_ascend.npu_lightning_indexer_quant(
    ...
    def forward(
        ...
        # dsa_cp mode: ql_nope/q_pe have num_tokens_per_device positions (including
        # pad), but the skip path in indexer_select_post_process may produce fewer
        # rows (only covering real tokens). Pad topk_indices with -1 so that its
        # first dimension matches the query tensors — the pad positions are masked
        # out by actual_seq_lengths_query anyway.
        if topk_indices.shape[0] < ql_nope.shape[0]:
            pad_count = ql_nope.shape[0] - topk_indices.shape[0]
            topk_indices = torch.cat(
                [
                    topk_indices,
                    torch.full(
                        (pad_count, topk_indices.shape[1], topk_indices.shape[2]),
                        -1,
