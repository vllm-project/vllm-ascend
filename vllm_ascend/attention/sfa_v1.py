...
class AscendSFAMetadataBuilder(MLACommonMetadataBuilder[AscendSFAMetadata]):
    ...
    def __init__(
        ...
        self.enable_li_skip = enable_li_skip()
    ...
    def build(
        ...
        if self.enable_li_skip:
            ...
            input_positions_pad[:num_actual_tokens] = input_positions[
                li_reorder_indices]
            slot_mapping_pad = torch.torch.zeros_like(slot_mapping)
            slot_mapping_pad[:num_actual_tokens] = slot_mapping[
                li_reorder_indices]
            ...
            block_table = torch.cat(
                [block_table, block_table[li_skip_request_mask]], dim=0)
...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def get_index_of_skipped_queries(self,
                                     actual_seq_lengths_query: torch.Tensor,
                                     actual_seq_lengths_key: torch.Tensor,
                                     num_actual_seqs: int, sparse_count: int):
        device = actual_seq_lengths_query.device
        # get the number of tokens moved to the end
        num_tokens = actual_seq_lengths_query[-1] - actual_seq_lengths_query[
            num_actual_seqs - 1]

        # get the seq length of each request
        actual_query_lens = torch.diff(actual_seq_lengths_query,
        ...
        top_k_indices_of_skipped_queries = -1 * torch.ones(
            (num_tokens, 1, sparse_count), device=device, dtype=torch.int32)
        ...
        for seq_id in range(skip_li_seq_num):
            for token_id in range(actual_query_lens[num_actual_seqs + seq_id]):
                ...
                top_k_indices_of_skipped_queries[token_count][
                    0][:seq_length_key] = torch.arange(seq_length_key)
                token_count += 1
    ...
    def indexer_select_post_process(
        ...
        if num_tokens > 0:
            ...
            top_k_indices_no_skip_li_query = torch.empty((0, 1, 2048),
                                                         device=q.device,
                                                         dtype=torch.int32)
        ...
        if attn_metadata.num_actual_seqs != actual_seq_lengths_key.shape[0]:
            ...
            top_k_indices = torch.cat(
                [top_k_indices_no_skip_li_query, top_k_indices_skip_li_query],
                dim=0)
        else:
