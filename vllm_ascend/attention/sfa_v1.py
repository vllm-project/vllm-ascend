...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        if self.use_sparse_c8_indexer and AscendSFAImpl.qk_hadamard is None:
            AscendSFAImpl.qk_hadamard = torch.tensor(scipy.linalg.hadamard(128),dtype=torch.bfloat16, device='npu') / (128 ** 0.5)
    ...
    def forward(
        ...
        if kv_cache is not None:
            ...
            if self.use_sparse_c8_indexer:
                torch_npu.npu_scatter_nd_update_(
                    kv_cache[3].view(-1, k_li_scale.shape[-1]), slot_mapping.view(-1, 1), k_li_scale.view(-1, k_li_scale.shape[-1])
                )
            if self.is_kv_producer:
