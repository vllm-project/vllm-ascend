...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def _fused_preprocess_type(self) -> PreprocessType | None:
        ...
        # PROLOG_V3 takes precedence over MLAPO. It serves PD decode
        # (KV-consumer) workers by default; plain non-PD serving may opt in
        # via the enable_sfa_prolog_v3 config: decode steps then take the
        # fused single-op path while prefill keeps the NATIVE path. The
        # opt-in requires a quantized fused_qkv_a_proj because the
        # unquantized weight preparation transposes fused_qkv_a_proj.weight
        # in place, which the NATIVE prefill fallback still consumes.
