...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def forward(
        ...
        if self.enable_mlapo and not has_prefill:
            ...
            # split indexer_select into `necessary_process` and `optional_process`,
            # such that the optional process can be skipped in short sequence case,
            # and `necessary_process` includes the storage of index-k cache.
            q, k = self.indexer_select_necessary_process(
                x=hidden_states,
                qr=q_c,
                cos=cos,
