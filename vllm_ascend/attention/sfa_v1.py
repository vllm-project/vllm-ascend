...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def forward(
        ...
        if kv_cache is not None:
            ...
            if self.is_kv_producer and envs.VLLM_ASCEND_ENABLE_RESHAPE_OPTIM:
                torch.ops._C_ascend.reshape_and_cache_by_group( k_li, kv_cache[2], attn_metadata.group_len, attn_metadata.group_key_idx, attn_metadata.group_key_cache_idx, attn_metadata.block_size)
            else:
            ...
            if self.use_sparse_c8_indexer:
                ...
                assert k_li_scale is not None
                if self.is_kv_producer and envs.VLLM_ASCEND_ENABLE_RESHAPE_OPTIM:
                    torch.ops._C_ascend.reshape_and_cache_by_group(k_li_scale, kv_cache[3], attn_metadata.group_len, attn_metadata.group_key_idx, attn_metadata.group_key_cache_idx, attn_metadata.block_size)
                else:
                    torch_npu.npu_scatter_nd_update_(
