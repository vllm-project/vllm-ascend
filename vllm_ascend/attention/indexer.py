...
class AscendSFAIndexerMetadataBuilder(AttentionMetadataBuilder[AscendSFAIndexerMetadata]):
    ...
    def _build(
        ...
        **kwargs,
        ...
        num_decode_tokens = 0
        if self._pcp_active and self._dcp_active:
            # Match the SFA metadata used before the split: only the DCP
            # builder computed this boundary for PCP's cache-write gather.
            # Dummy decode metadata does not always carry is_prefilling, so
            # treat its short queries as decode instead of requiring it.
            _, _, num_decode_tokens, _ = split_decodes_and_prefills(
                common_attn_metadata,
                decode_threshold=self.decode_threshold,
                treat_short_extends_as_decodes=(getattr(common_attn_metadata, "is_prefilling", None) is None),
            )
