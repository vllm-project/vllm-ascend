...
def _resize_req_axis_tensor(
    ...
    if tensor.shape[0] == target_num_reqs:
        return tensor

    if tensor.shape[0] > target_num_reqs:
        return tensor[:target_num_reqs]

    pad_shape = (target_num_reqs - tensor.shape[0],) + tuple(tensor.shape[1:])
    pad_kwargs: dict[str, Any] = {"dtype": tensor.dtype, "device": tensor.device}
    if tensor.device.type == "cpu":
        pad_kwargs["pin_memory"] = tensor.is_pinned()
    pad_tensor = torch.full(pad_shape, pad_value, **pad_kwargs)
    return torch.cat([tensor, pad_tensor], dim=0)


def normalize_req_axis_by_query_start_loc(
    common_attn_metadata: AscendCommonAttentionMetadata,
) -> bool:
    """
    Keep req-shaped metadata aligned with query_start_loc.

    In speculative decoding with graph padding, query_start_loc may keep one or
    more dummy requests while seq_lens / block_table are still sized to the
    surviving requests. FIA expects both qlen and kvlen arrays to have the same
    request dimension, so we normalize the req axis here and pad missing KV-side
    rows with dummy values.
    """
    query_start_loc_cpu = common_attn_metadata.query_start_loc_cpu
    if query_start_loc_cpu is None:
        return False

    target_num_reqs = max(query_start_loc_cpu.shape[0] - 1, 0)
    updated = common_attn_metadata.num_reqs != target_num_reqs
    common_attn_metadata.num_reqs = target_num_reqs

    resized_seq_lens_cpu = _resize_req_axis_tensor(
        common_attn_metadata.seq_lens_cpu,
        target_num_reqs,
        pad_value=0,
    )
    updated = updated or resized_seq_lens_cpu is not common_attn_metadata.seq_lens_cpu
    common_attn_metadata.seq_lens_cpu = resized_seq_lens_cpu

    resized_seq_lens = _resize_req_axis_tensor(
        common_attn_metadata.seq_lens,
        target_num_reqs,
        pad_value=0,
    )
    updated = updated or resized_seq_lens is not common_attn_metadata.seq_lens
    common_attn_metadata.seq_lens = resized_seq_lens

    resized_num_computed = _resize_req_axis_tensor(
        common_attn_metadata.num_computed_tokens_cpu,
        target_num_reqs,
        pad_value=0,
    )
    updated = updated or resized_num_computed is not common_attn_metadata.num_computed_tokens_cpu
    common_attn_metadata.num_computed_tokens_cpu = resized_num_computed

    resized_block_table = _resize_req_axis_tensor(
        common_attn_metadata.block_table_tensor,
        target_num_reqs,
        pad_value=0,
    )
    updated = updated or resized_block_table is not common_attn_metadata.block_table_tensor
    common_attn_metadata.block_table_tensor = resized_block_table
