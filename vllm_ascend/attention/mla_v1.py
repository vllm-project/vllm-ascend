...
class AscendMLAMetadataBuilder(MLACommonMetadataBuilder[AscendMLAMetadata]):
    ...
    def build_chunked_metadata(
        ...
        return ChunkedContextMetadata(
            ...
            # FIX (EZ9999 MTE invalid GM address): chunk_starts is a plain CPU
            # tensor, so `chunk_starts.pin_memory().to(device, non_blocking=True)`
            # materializes a *temporary* pinned staging buffer that is freed at
            # the end of the statement; the pinned allocator can then hand the
            # same block to the next .pin_memory() while this DMA is still
            # queued, and the device receives corrupt chunk starts. This runs
            # once per prefill step (returns early for pure decode), so a
            # blocking copy costs nothing on the decode path.
            starts=chunk_starts.pin_memory().to(self.device, non_blocking=False),
...
class AscendMLAImpl(MLAAttentionImpl):
    ...
    def _forward_decode(
        ...
        if self.qk_rope_head_dim == 0:
            rope_dim = 64
            q_pe = _mla_nope_zero_rope(q_nope, rope_dim)
            k_pe = _mla_nope_zero_rope(k_nope, rope_dim)
...
def _mla_nope_zero_rope(ref: torch.Tensor, rope_dim: int) -> torch.Tensor:
    ...
    buf = _MLA_NOPE_ZERO_ROPE.get(key)
    if buf is None:
        buf = torch.zeros(shape, dtype=ref.dtype, device=ref.device)
        _MLA_NOPE_ZERO_ROPE[key] = buf
    return buf
