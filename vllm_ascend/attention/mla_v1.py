...
class AscendMLADecodeMetadata:
    ...
    # Device-side lengths share the model runner's persistent graph input
    # buffer. They are used to mask empty FIA splits inside the captured graph.
...
class AscendMLAMetadataBuilder(MLACommonMetadataBuilder[AscendMLAMetadata]):
    ...
    def build_decode_metadata(
        ...
        decode_metadata = AscendMLADecodeMetadata(
            ...
            # Keep the whole graph bucket, including zero-length padding rows.
            # The persistent device buffer is refreshed before every replay.
            seq_lens_device=(
                common_attn_metadata.seq_lens[: self.block_table.shape[0]]
...
class AscendMLAImpl(MLAAttentionImpl):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.use_mla_rope and (self.fa_quant_layer or self.enable_mlapo):
            ...
            # q/kv prolog. Kimi K3's positional slice must remain in checkpoint
    ...
    def exec_kv_prefill(
        ...
        if not self.use_mla_rope:
            # Prefill attention also needs the just-written current-token KV.
    ...
    def _forward_decode(
        ...
        if fia_num_splits > 1:
            ...
            if decode_meta.seq_lens_device is None:
                raise ValueError("MLA FIA split requires device-side decode sequence lengths")
