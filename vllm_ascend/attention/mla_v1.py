...
def _finalize_pending_parallel_draft_rejects(
    ...
    event = cast(Any, attn_metadata.pending_reject_event)
    counts_cpu = cast(torch.Tensor, attn_metadata.pending_reject_cpu)
    decode_meta = cast(AscendMLADecodeMetadata, attn_metadata.decode)
...
class AscendMLAMetadataBuilder(MLACommonMetadataBuilder[AscendMLAMetadata]):
    ...
    def __init__(
        ...
        self.use_mla_rope = any(layer_rope_modes) if layer_rope_modes else True
        self.cos_cache = None
        self.sin_cache = None
