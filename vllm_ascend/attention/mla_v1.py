    ...
    def _compute_prefill_context(
        ...
        for i in range(iters):
            ...
            if is_a3_c8_cache:
                ...
                # mla_nope_chunked_load: with qk_rope_head_dim == 0 the rope
                # cache and output buffer are zero-width, so use the latent
                # cache as the second gather input and discard that duplicate.
                if rope_dim == 0:
                    pe_cache = cache_kv_c_for_load
                    k_pe = torch.empty_like(kv_c_normed)
                else:
                    pe_cache = cache_k_pe_for_load
                DeviceOperator.kv_cache_load(
                    cache_kv_c_for_load,
                    pe_cache,
                    gather_block_table,
