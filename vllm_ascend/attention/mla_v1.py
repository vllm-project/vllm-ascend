...
class AscendMLAMetadataBuilder(MLACommonMetadataBuilder[AscendMLAMetadata]):
    ...
    def __init__(
        ...
        has_sliding_window = isinstance(kv_cache_spec, SlidingWindowMLASpec)
        attention_layer = (
            vllm_config.compilation_config.static_forward_context.get(layer_names[0]) if layer_names else None
        )
        self.needs_prefix_context = has_sliding_window or (
            getattr(getattr(attention_layer, "impl", None), "has_mla_extras", False) is True
        )
    ...
    def build_chunked_metadata(
        ...
        if not self.chunked_prefill_enabled and not self.needs_prefix_context:
            return None
    ...
    def build_prefill_metadata(
        ...
        return AscendMLAPrefillMetadata(
            ...
            seq_lens=self.seq_lens,
            context_lens=(self.context_lens_cpu if self.needs_prefix_context else self.seq_lens[reqs_start:]),
...
class AscendMLAImpl(MLAAttentionImpl):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.use_output_gate = self.g_proj is not None and kwargs.get("sdpa_gate_type") is None
        ...
        self.has_mla_extras = (
            ...
            or self.sdpa_gate_type is not None
    ...
    def _needs_sliding_window_decode(self) -> bool:
        return self.sliding_window is not None and self.kv_lora_rank not in (128, 512)

    def _needs_unfused_prefill(self) -> bool:
        return self.has_mla_extras and self.v_head_dim != self.qk_nope_head_dim
    ...
    def _forward_prefill_unfused(
        ...
        assert self.num_kv_heads == 1, "MLA prefill fallback expects MQA latent KV cache."
        if _EXTRA_CTX.capturing:
            raise RuntimeError(
                "MLA prefill fallback requires --enforce-eager because "
    ...
    def _gather_decode_latent_cache(
        ...
        assert self.num_kv_heads == 1, "MLA decode fallback expects MQA latent KV cache."
        if seq_len == 0:
    ...
    def _forward_decode_sliding_window(
        ...
        from vllm_ascend.models.dots3_note.sliding_window_mla import paged_sliding_window_gather

        decode_meta = attn_metadata.decode
        assert decode_meta is not None
        assert self.num_kv_heads == 1, "MLA decode fallback expects MQA latent KV cache."
        assert self.sliding_window is not None, "MLA decode fallback requires sliding-window attention."
        if self.enable_kv_nz:
            raise RuntimeError("Sliding-window MLA fallback does not support enable_kv_nz.")
    ...
    def _forward_prefill(
        ...
        if self._needs_unfused_prefill() or self.use_layer_rotary_emb:
            context_lens = prefill_meta.context_lens
            if isinstance(context_lens, torch.Tensor):
                has_prefix_context = bool((context_lens > 0).any())
            else:
                has_prefix_context = context_lens is not None and any(context_lens)
            if self._needs_unfused_prefill() or has_prefix_context:
    ...
    def mla_preprocess_prefill(self, q_c, kv_no_split, kv_cache, attn_metadata):
        ...
        return PrefillMLAPreprocessResult(
            ...
            prefill_k_pe,
            prefill_value,
            prefill_k_c_normed if self.has_mla_extras else None,
            prefill_k_pe_cache.squeeze(1) if self.has_mla_extras else None,
        )
    ...
    def forward(
        ...
        elif self.sdpa_gate_type is not None:
            assert self.g_proj is not None
            gate_hidden_states = torch.ops.vllm.maybe_all_gather_and_maybe_unpad(hidden_states.contiguous())
