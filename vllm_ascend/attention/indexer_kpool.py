...
class AscendIndexerKPoolTailMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(
        ...
        # KeyPool uses absolute logical pages. Every page of this request
        # aliases its single private tail ring. Materialize a contiguous table
        # once per step in persistent storage, also used by ACLGraph replay.
        self._block_table_buffer = torch.empty(
    ...
    def build(
        ...
        **kwargs,
        ...
        block_table.copy_(common_attn_metadata.block_table_tensor[:num_reqs, :1].expand_as(block_table))
