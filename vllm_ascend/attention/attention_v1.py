...
class BlasstParamProvider:
    """ParamProvider for BlasST full-graph tasks. Mirrors FIAParamProvider
    but the BlasST op spells the block-table kwarg ``blocktable`` (single
    word) rather than ``block_table``."""
...
class AscendAttentionBackendImpl(AttentionImpl):
    ...
    def _can_use_blasst(
        ...
        if _EXTRA_CTX.capturing:
            ...
            if not (get_ascend_config().blasst_config.full_graph
    ...
    def forward_blasst_attention(
        ...
        """
        if _EXTRA_CTX.capturing:
            attn_output, num_tokens = self.full_graph_blasst(
        ...
        attn_output, _, _ = torch.ops._C_ascend.npu_blasst_attention_score(
            ...
            block_size=block_size,
            **_blasst_op_kwargs(get_ascend_config().blasst_config,
    ...
    def full_graph_blasst(
        ...
        """BlasST 的 full-graph capture 路径（UpdatableGraph 风格，镜像
        full_graph_fia）。
        ...
        def _make_workspace():
            ...
            return torch.empty(max(ws_size, ws_size_fd),
                               dtype=torch.uint8,
                               device=query.device)

        workspace = get_capture_resource(
            _BLASST_WORKSPACE_KEY,
            _make_workspace,
        ...
        register_task(
            ...
            BlasstParamProvider(self._layer_name),
        )
    ...
    def forward_impl(
        ...
        if (
            ...
            output = self.forward_blasst_attention(
                query, key, value, attn_metadata, output)
        else:
