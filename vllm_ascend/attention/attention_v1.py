...
class AscendC8AttentionBackendImpl(AscendAttentionBackendImpl):
    ...
    def forward(
        ...
        assert output is not None, "Output tensor must be provided."
        # Last-resort runtime guard: this C8 subclass overrides forward() without
        # the hamming hooks the base class installs, so the two features cannot
        # coexist. AscendConfig / is_enable_hamming_sparse should have caught it
        # earlier; fail loudly here if a C8+hamming layer still reaches forward.
        assert not self.enable_hamming_sparse, "Hamming sparse attention does not support C8 KV cache quantization."
        if self._use_layer_aware_fia_graph_replay:
            self._layer_name = layer.layer_name
