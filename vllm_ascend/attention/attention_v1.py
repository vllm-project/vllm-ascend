...
from vllm_ascend.worker.kvcomp_utils import KVCompMetaData, recover_request_lengths
...
class AscendMetadata:
    ...
    max_seq_len_for_hamming: int | None = None
...
class AscendAttentionMetadataBuilder(AttentionMetadataBuilder[AscendMetadata]):
    ...
    def _build_kvcomp_metadata(
        ...
        """Initialize KVComp related metadata, extracted from the build function."""
        kvcomp_metadata = attn_metadata.kvcomp_metadata
        assert kvcomp_metadata is not None, "KVComp metadata is not initialized properly"
        ...
        real_batch_size = attn_metadata.seq_lens_gpu.shape[0]
        runtime_seq_lens_list = attn_metadata.seq_lens_list[:real_batch_size]

        if num_actual_tokens < real_batch_size:
            runtime_seq_lens_list = runtime_seq_lens_list[:num_actual_tokens] + [0] * (
                real_batch_size - num_actual_tokens
            )
            attn_metadata.slot_mapping[num_actual_tokens:] = -1
        ...
        attn_metadata.max_seq_len_for_hamming = (
            attn_metadata.max_seq_len if attn_metadata.max_seq_len is not None else runtime_max_seq_len_for_hamming
        )
        attn_metadata.block_tables_for_hamming = attn_metadata.block_tables[:real_batch_size]

        # process top_k_for_hamming
        ...
        kvcomp_metadata.seq_lens_from_hamming = new_seq_lens.tolist()

        # check and calculate mask
        ...
        torch.where(
            ...
            out=kvcomp_metadata.valid_query_mask[:real_batch_size],
        )
        ...
        torch.where(
            ...
            actual_query_lens,
            torch.zeros_like(actual_query_lens),
            out=kvcomp_metadata.seq_lens_for_reshape[:real_batch_size],
        )
    ...
    def build(
        ...
        if common_attn_metadata._seq_lens_cpu is not None:
            ...
            seq_lens_gpu = common_attn_metadata._seq_lens_cpu[:num_reqs].to("npu")
        elif common_attn_metadata.seq_lens_cpu is not None:
            seq_lens = common_attn_metadata.seq_lens_cpu[:num_reqs]
            seq_lens_gpu = common_attn_metadata.seq_lens_cpu[:num_reqs].to("npu")
        else:
...
class AscendAttentionBackendImpl(AttentionImpl):
    ...
    def full_graph_fia(
        ...
        if self.enable_kvcomp:
            block_table, actual_seq_lengths_kv = self._get_kvcomp_params(
                query, passed_key, attn_metadata, block_table, actual_seq_lengths_kv
            )
    ...
    def _get_kvcomp_params(
        ...
        kvcomp_metadata = attn_metadata.kvcomp_metadata
        assert self.layerIndex is not None, "layerIndex must be set"
        assert kvcomp_metadata is not None, "KVComp metadata is not initialized properly"
        assert kvcomp_metadata.hashk_caches is not None, "hashk_caches is not initialized"

        if kvcomp_metadata.hashk_caches[self.layerIndex] is None:
            return block_table, actual_seq_lengths_kv
        kvcomp_config = kvcomp_metadata.kvcomp_config

        if (
            kvcomp_config.vllm_hash_attention_skip_layers[self.layerIndex]
            ...
            torch.ops._C_ascend.npu_reshape_and_cache_bnsd(
                hashk_op, hashk_cache_op, slot_mapping_op, seq_lens_for_reshape, hashk_cache_op
            )
            ...
            # run decode here
            seq_lens_for_hamming = attn_metadata.seq_lens_gpu[:real_batch_size]
        ...
        return new_block_table, kvcomp_metadata.seq_lens_from_hamming
