...
def full_prefill(
    ...
    *,
    ...
    `cache` is the existing first-axis-strided [pages,128,576] view. Callers
    pass DeviceOperator.kv_cache_load and torch_npu.npu_attention_update.
    """
    tokens, heads, nope_dim = q_nope.shape
    if tokens != metadata.plan.num_tokens or cache.shape[1:] != (128, 576):
    ...
    def expand_attention(latent, position, packed_query, call):
        ...
        key = torch.cat((key_nope, position.reshape(-1, 1, 64).expand(-1, heads, -1)), dim=-1)
        out, lse = attention(packed_query, key, value.contiguous(), call)
    ...
    for chunk in metadata.history:
        ...
        latent = torch.empty((gather_tokens, 1, 512), dtype=cache.dtype, device=cache.device)
        position = torch.empty((gather_tokens, 1, 64), dtype=cache.dtype, device=cache.device)
        table = block_table if chunk.request_indices is None else block_table.index_select(0, chunk.request_indices)
        kv_cache_load(
            cache[..., :512].unsqueeze(2),
            cache[..., 512:].unsqueeze(2),
            table,
            chunk.call.kv_used,
            chunk.starts,
...
def native_flash_adapters(num_heads: int, scale: float, attn_mask: torch.Tensor):
    ...
    def attention(query, key, value, call):
        return flash_attn(
            query,
            key,
            value,
