...
def full_prefill(
    ...
    *,
    ...
    history_load: Callable | None = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compute only prefill rows; online merging bounds history output memory.

    `cache` is the first-axis-strided BF16 [pages,128,576] view, or the
    persistent C8 pair (FP8 latent [pages,128,1,512], BF16 positional keys
    [pages,128,1,64]). Only C8 history latent is dequantized by `cache_scale`;
    current rows and the positional component retain their original precision.
    """
    tokens, heads, nope_dim = q_nope.shape
    if isinstance(cache, tuple):
        if len(cache) != 2 or cache_scale is None:
            raise ValueError("C8 prefill requires two caches and the calibrated latent scale")
        latent_cache, position_cache = cache
        if latent_cache.dtype != torch.float8_e4m3fn or position_cache.dtype != q_nope.dtype:
            raise ValueError("C8 prefill requires FP8 latent and model-dtype positional keys")
    else:
        if cache_scale is not None or cache.shape[1:] != (128, 576):
            raise ValueError("BF16 prefill cache layout or scale mismatch")
    ...
    def expand_attention(latent, position, packed_query, call):
        ...
        # Keep the projection's split views: the C8 adapter can read their
        # strides and fuse quantization without materializing K192 or V128.
    ...
    for chunk in metadata.history:
        ...
        if cache_scale is not None and history_load is not None:
            latent, position = history_load(
                latent_cache,
                position_cache,
                table,
                chunk.call.kv_cu,
                ...
                chunk.starts,
                cache_scale,
                num_tokens=gather_tokens,
                max_seq_len=max(chunk.plan.lengths),
            )
        else:
            ...
            if cache_scale is not None:
                latent = (latent.float() * cache_scale).to(current_latent.dtype)
