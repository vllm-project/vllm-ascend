...
def _build_c4_boundary_replay(
    *,
    ...
    for req_idx, query_len in enumerate(query_lens):
        ...
        replay_start_pos.append(
            start_positions[req_idx] + replay_start - req_start
        )
    ...
    return {
        ...
        "boundary_replay_compressed_row_indices": tuple(
            compressed_row_indices
        ),
        "boundary_replay_compressed_row_slice": _contiguous_slice(
            compressed_row_indices
        ),
    }
...
def sync_boundary_state_blocks(
    *,
    ...
    if not (
        boundary_req_indices.shape
        == boundary_positions.shape
        == boundary_owner_mask.shape
    ):
        raise RuntimeError(
            "Chunked Compressor SP boundary-state metadata shape mismatch"
        )
    ...
    boundary_state_rows = (
        request_block_tables.gather(1, boundary_block_indices.unsqueeze(1))
        .squeeze(1)
        .long()
    )
    ...
    owner_state = boundary_state * boundary_owner_mask.view(owner_shape).to(
        boundary_state.dtype
    )
...
def build_compressor_sp_plan(
    *,
    ...
    boundary_req_indices = [
        req_idx for req_idx, query_len in enumerate(query_lens) if query_len > 0
    ]
    boundary_positions = [seq_lens[req_idx] - 1 for req_idx in boundary_req_indices]
    boundary_owner_mask = [
        owned_start <= query_start_loc[req_idx + 1] - 1 < owned_end
        for req_idx in boundary_req_indices
    ]
    global_compressed_row_count = sum(
        (position + 1) % compress_ratio == 0 for position in input_positions
    )
    seq_len_aligned = all(
        seq_len % compress_ratio == 0 for seq_len in seq_lens[:num_reqs]
    )
    ...
    base_plan_fields = dict(
        ...
        requires_boundary_state_sync=(
            is_chunked_prefill
            and bool(boundary_req_indices)
            and global_compressed_row_count > 0
        ),
    ...
    history_pad = (
        compress_ratio
        if compress_ratio == 4 and any(pos > 0 for pos in start_positions)
        else 0
    )
    ...
    for req_idx, query_len in enumerate(query_lens):
        ...
        if seq_lens[req_idx] % compress_ratio != 0:
            reason = (
                "seq_len_not_aligned_c4"
                if compress_ratio == 4
                else "seq_len_not_aligned_c128"
            )
            allow_non_aligned = (
                allow_c4_non_aligned if compress_ratio == 4 else allow_c128_non_aligned
            )
            if not allow_non_aligned:
                return disabled(reason, **base_plan_fields)
        if (
            compress_ratio != 4
            and start_positions[req_idx] % compress_ratio != 0
            and not allow_c128_non_aligned
        ):
    ...
    for req_idx in range(num_reqs):
        ...
        dependency_tokens = (
            2 * compress_ratio if compress_ratio == 4 else compress_ratio
        )
        expanded_start = max(
            req_start, first_output_idx - dependency_tokens + 1 - history_pad
        )
        expected_position = max(
            0, output_position - dependency_tokens + 1 - history_pad
        )
        while (
            expanded_start < first_output_idx
            and input_positions[expanded_start] < expected_position
        ):
            expanded_start += 1
        if compress_ratio == 4:
            while (
                expanded_start > req_start and input_positions[expanded_start] % 8 != 0
            ):
                expanded_start -= 1
        ...
        if expanded_end < req_end:
            ...
            while (
                expanded_end < req_end
                and (input_positions[expanded_end - 1] + 1) % right_alignment != 0
            ):
                expanded_end += 1
        ...
        if seq_lens[req_idx] % compress_ratio != 0:
            ...
            if compress_ratio == 4:
                ...
                while (
                    state_tail_start > req_start
                    and input_positions[state_tail_start] % 8 != 0
                ):
                    state_tail_start -= 1
            state_tail_range = (state_tail_start, req_end)

            tail_is_covered = any(
                range_start <= state_tail_start and req_end <= range_end
                for range_start, range_end in expanded_ranges
            )
            if not tail_is_covered:
                ...
                start_pos.append(
                    start_positions[req_idx] + state_tail_start - req_start
                )
    ...
    if not output_keep_indices and not can_replay_boundary_without_local_rows:
        reason = (
            ...
            if tail_token_ranges
            else "no_local_compressed_rows"
        )
        return disabled(
            reason, **base_plan_fields, tail_token_ranges=tuple(tail_token_ranges)
        )
    ...
    local_output_rows = min(
        len(token_indices), len(token_indices) // compress_ratio + len(req_indices)
    )
    ...
    return CompressorSPPlan(
        ...
        supports_boundary_state_replay=base_plan_fields.get(
            "supports_boundary_state_replay", False
        ),
        boundary_replay_token_ranges=base_plan_fields.get(
            "boundary_replay_token_ranges", ()
        ),
        boundary_replay_token_indices=base_plan_fields.get(
            "boundary_replay_token_indices", ()
        ),
        boundary_replay_token_slice=base_plan_fields.get(
            "boundary_replay_token_slice"
        ),
        boundary_replay_req_indices=base_plan_fields.get(
            "boundary_replay_req_indices", ()
        ),
        boundary_replay_req_slice=base_plan_fields.get(
            "boundary_replay_req_slice"
        ),
        boundary_replay_cu_seqlens=base_plan_fields.get(
            "boundary_replay_cu_seqlens", ()
        ),
        boundary_replay_start_pos=base_plan_fields.get(
            "boundary_replay_start_pos", ()
        ),
        boundary_replay_compressed_row_indices=base_plan_fields.get(
            "boundary_replay_compressed_row_indices", ()
        ),
        boundary_replay_compressed_row_slice=base_plan_fields.get(
