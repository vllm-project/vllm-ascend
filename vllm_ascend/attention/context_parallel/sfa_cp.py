...
from vllm_ascend.attention.sfa_v1 import AscendSFAMetadataBuilder, AscendSFAMetadata, AscendSFAImpl

from vllm_ascend.attention.utils import (AscendCommonAttentionMetadata, split_decodes_and_prefills, enabling_mlapo)
...
class AscendSFACPMetadataBuilder(AscendSFAMetadataBuilder):
    ...
    def build(
        ...
        sfa_cp_metadata = self.build_cp_metadata(block_table_cp, valid_block_ids, metadata_cls.seq_lens, common_attn_metadata)
        ...
        if self.pcp_size > 1:
            ...
            if self.enable_mlapo:
                slot_mapping[: num_decode_tokens] = slot_mapping[
            ...
            if num_prefills > 0 and num_decode_tokens > 0:
                prefill_q_cum_seqlens = actual_seq_lengths_query[num_decode_tokens:] - actual_seq_lengths_query[num_decode_tokens - 1]
            else:
...
class AscendSFACPImpl(AscendSFAImpl):
    ...
    def _execute_sparse_flash_attention_process(self, ql_nope, q_pe, kv_cache, topk_indices, 
        ...
        if self.pcp_size == 1:
            return self._execute_sparse_flash_attention(ql_nope, q_pe, kv, key_rope, block_table,
                                                        topk_indices, actual_seq_lengths_query, actual_seq_lengths_key)
        num_decode_tokens = attn_metadata.num_decode_tokens
        num_prefills = attn_metadata.num_prefills
        decode_attn_out = None
        if num_decode_tokens > 0:
            decode_attn_out = self._execute_sparse_flash_attention(
                ql_nope[: num_decode_tokens], q_pe[: num_decode_tokens], kv, key_rope, block_table[: num_decode_tokens],
                topk_indices[: num_decode_tokens], actual_seq_lengths_query[: num_decode_tokens], actual_seq_lengths_key[: num_decode_tokens])
        
        if num_prefills < 1:
            return decode_attn_out
        
        # q splite for head and tail
        ...
        q_head_output = self._execute_sparse_flash_attention(
            torch.index_select(ql_nope, 0, q_head_idx), torch.index_select(q_pe, 0, q_head_idx), kv, key_rope, block_table, 
            torch.index_select(topk_indices, 0, q_head_idx), attn_metadata.sfa_cp_metadata.prefill_q_cum_seqlens // 2, q_head_actual_seq_lengths_key)
        ...
        q_tail_output = self._execute_sparse_flash_attention(
            torch.index_select(ql_nope, 0, q_tail_idx), torch.index_select(q_pe, 0, q_tail_idx), kv, key_rope, block_table,
            torch.index_select(topk_indices, 0, q_tail_idx), attn_metadata.sfa_cp_metadata.prefill_q_cum_seqlens // 2, q_tail_actual_seq_lengths_key)
        
        q_full_idx = attn_metadata.sfa_cp_metadata.q_full_idx
        attn_output = torch.index_select(
            torch.cat([q_head_output, q_tail_output], dim=0), 0, q_full_idx)

        if decode_attn_out is not None:
            attn_output = torch.cat([decode_attn_out, attn_output], dim=0)
        return attn_output
    
    def _execute_sparse_flash_attention(self, ql_nope, q_pe, kv, key_rope, block_table,
                                        topk_indices, actual_seq_lengths_query, actual_seq_lengths_key):
    ...
    def indexer_select_post_process(
        ...
        if self.pcp_size == 1:
            return self._execute_indexer_select(q, key, weights, actual_seq_lengths_query, actual_seq_lengths_key, block_table)
        ...
        if num_decode_tokens > 0:
            decode_topk_indices = self._execute_indexer_select(
                q[:num_decode_tokens], key, weights[:num_decode_tokens], actual_seq_lengths_query[:num_decode_tokens],
                actual_seq_lengths_key[:num_decode_tokens], block_table[:num_decode_tokens])
        ...
        q_full_idx = attn_metadata.sfa_cp_metadata.q_full_idx
        topk_indices = torch.index_select(
            torch.cat([q_head_topk_indices, q_tail_topk_indices], dim=0), 0, q_full_idx)
        if decode_topk_indices is not None:
            topk_indices = torch.cat([decode_topk_indices, topk_indices], dim=0)
        return topk_indices
    
    def _execute_indexer_select(self, q, key, weights, actual_seq_lengths_query,
    ...
    def exec_kv(
        ...
        kv_c_k_pe = torch.index_select(
            kv_c_k_pe, 0, attn_metadata.sfa_cp_metadata.pcp_allgather_restore_idx
        )
