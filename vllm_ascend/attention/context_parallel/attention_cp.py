...
class AscendAttentionCPMetadataBuilder(AscendAttentionMetadataBuilder):
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            if self.chunked_prefill_enabled and max_context_len_cpu > 0:
                ...
                actual_seq_lengths_kv = torch.cumsum(
                    local_chunked_kv_lens_rank, dim=0
                ).tolist()
                ...
                cp_kv_recover_idx_for_chunk = (
                    common_long_seq_metadata.cp_kv_recover_idx_for_chunk
                )
                ...
                batch_chunk_seq_mask = (
                    local_context_lens_allranks[:, self.pcp_rank, self.dcp_rank] == 0
                )
                ...
                chunk_seq_mask_filtered_indices = filter_chunked_req_indices(
                    query_lens, chunked_req_mask
                ).to(self.device)
                chunked_context_metadata = (
                    AscendMetadataForPrefill.ChunkedContextMetadata(
                        actual_chunk_seq_lengths=torch.cumsum(
                            query_lens * pcp_size, dim=0
                        ),
            ...
            if pcp_size > 1:
                ...
                head_attn_nomask_seqlens = torch.cumsum(
                    head_attn_nomask_seqlens[1], dim=0
                ).tolist()
                tail_attn_nomask_seqlens = torch.cumsum(
                    tail_attn_nomask_seqlens[1], dim=0
                ).tolist()
        ...
        if num_decodes > 0:
            ...
            batch_seq_mask = (
                num_computed_tokens_array[:, self.pcp_rank, self.dcp_rank] == 0
            )
...
class AscendAttentionCPImpl(AscendAttentionBackendImpl):
    ...
    def _attention_with_nomask_and_mask(
        ...
        if k_nomask is not None:
            if (
                ...
                output = self._npu_attn_out_lse_update(
    ...
    def _npu_attn_out_lse_update(
        self, attn_lse_mask, attn_lse_nomask, attn_out_mask, attn_out_nomask
    ):
    ...
    def _forward_prefill_cp_pre(
        ...
        k_head_nomask = (
            torch.index_select(key, 0, kv_with_q_head_nomask_idx)
            if self.pcp_rank > 0
            else None
        )
        v_head_nomask = (
            torch.index_select(value, 0, kv_with_q_head_nomask_idx)
            if self.pcp_rank > 0
            else None
        )
    ...
    def _out_lse_reshape(
        self, attn_out: torch.Tensor, attn_lse: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _forward_decode_pcp_dcp(
        self, query: torch.Tensor, attn_metadata: AscendMetadata
    ) -> torch.Tensor:
        ...
        k_nope = self.key_cache.view(
            self.key_cache.shape[0], self.key_cache.shape[1], -1
        )
        ...
        if forward_context.capturing:
            ...
            attn_lse = torch.empty(
                (num_tokens, num_heads, 1), dtype=torch.float, device=query.device
            )
    ...
    def _update_out_and_lse(
        self, out_list: torch.Tensor, lse_list: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _update_chunk_attn_out_lse_with_current_attn_out_lse(
        ...
        if self.pcp_size > 1:
            ...
            attn_lse_full_chunk = torch.index_select(
                attn_lse_full_chunk, 0, inverse_idx
            )
        ...
        attn_output_filtered = self._npu_attn_out_lse_update(
            attn_lse_prefill_filtered,
            attn_lse_full_chunk,
            attn_output_prefill_filtered,
            attn_output_full_chunk,
        )

        current_attn_output_prefill[filtered_indices, :, :] = attn_output_filtered.to(
    ...
    def _compute_prefill_context(
        ...
        local_chunked_kv_lens = (
            prefill_metadata.chunked_context.local_context_lens_allranks
        )
        ...
        key, value = self._load_kv_for_chunk(
        ...
        if total_toks > 0:
            ...
            batch_chunk_seq_mask = (
                attn_metadata.prefill.chunked_context.batch_chunk_seq_mask
            )
    ...
    def _load_kv_for_chunk(
        self, attn_metadata, kv_cache, local_chunked_kv_lens_rank, query, total_toks
    ):
        ...
        key = torch.empty(
            total_toks, num_heads, head_size, dtype=query.dtype, device=query.device
        )
        value = torch.empty(
            total_toks, num_heads, head_size, dtype=query.dtype, device=query.device
        )
    ...
    def reshape_and_cache(
        ...
        if len(kv_cache) > 1:
            ...
            if has_prefill:
                ...
                prefill_key = key[
                    self.pcp_size
                    * num_decode_tokens : attn_metadata.num_actual_tokens_pcp_padded
                ]
                prefill_value = value[
                    self.pcp_size
                    * num_decode_tokens : attn_metadata.num_actual_tokens_pcp_padded
    ...
    def _gather_global_context_output(self, local_context_attn_output):
        if self.dcp_size > 1:
            ...
            dist.all_to_all_single(
                dcp_context_attn_output, local_context_attn_output, group=self.dcp_group
            )
        else:
    ...
    def forward_impl(
        ...
        if has_prefill:
            ...
            output[
                num_decode_tokens : attn_output_prefill.shape[0] + num_decode_tokens
            ] = attn_output_prefill
        return output
