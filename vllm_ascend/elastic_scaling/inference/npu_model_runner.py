    ...
    with DeviceMemoryProfiler() as m:  # noqa: SIM117
        if int(os.getenv("ELASTIC", "0")) > 0:
            if int(os.getenv("INFER_STATUS", "0")) > 0:
                ...
                with set_default_torch_dtype(self.model_config.dtype), target_device:
                    self.cpu_model = initialize_model(vllm_config=self.vllm_config, model_config=self.model_config)
                ...
                self.model = self.loader.zero_copy_model(
                    self.vllm_config, self.ipc_engine, self.cpu_model, self.model_config
                )
            else:
