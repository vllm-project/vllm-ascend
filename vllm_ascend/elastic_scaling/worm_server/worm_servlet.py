...
class WORMServlet:
    def __init__(self, rank: int, global_world_size: int, tensor_parallel_size: int, node_id: str, phy_npu_id:int):
        ...
        assert(self.tensor_parallel_size%self.global_world_size==0, f'TP SIZE {self.tensor_parallel_size} should be divisble by WORLD SIZE {self.global_world_size}') # Scaling can only happen by TP
        ...
        self._pg_cache = {}
        self._npu_ipc_init()
        self.IPCSafeAllocator = IPCSafeAllocator(dtype_str=str(self.vllm_config.model_config.dtype).replace("torch.", ""), device_id=self.device_id)

        print(f'WORMServlet Initialized -> Rank:{self.rank}, Node -> {node_id}, Physical NPU #{os.environ["ASCEND_RT_VISIBLE_DEVICES"]}, Total visible devices:{torch.npu.device_count()}')
    ...
    def _init_global_world(self):
        ...
        init_distributed_environment(world_size=self.global_world_size,
                                     rank=self.rank,
                                     distributed_init_method=f"tcp://{self.master_addr}:{self.master_port}",
                                     local_rank=self.local_rank,
                                     backend="hccl",
        )
        initialize_fake_model_parallel(rank=self.rank,
    ...
    def _init_kv_cache(self, num_gpu_blocks=None, kv_cache_configs=[None]):
        ...
        from worm_ipc.utils import initialize_kv_cache
        import torch
    ...
    def broadcast_metadata_dict(self, peers=None): #peers: list[ray.actor.ActorHandle]
        ...
        futures = [peer.set_attribute.remote(name='metadata_dict', value_or_oid=oid) for peer in peers if peer is not self]
        return futures
    ...
    def ring_probe(self):
        ...
        if self.rank % 2 == 0:
            ...
            w = dist.isend(buf, dst=dst); w.wait()
    ...
    def scaleup(self, num_npus=4):
        ...
        destination_rank_map = {
            tp_rank: list(range(self.model_world_size + tp_rank,
    ...
    def scaledown(self, num_npus=2):
        ...
        if scaled_down_model_size < self.tensor_parallel_size:
            self.print(f"Can't scale down below the TP Size limit. Target size:{scaled_down_model_size} < TP Size:{self.tensor_parallel_size}")
            return "cant_scale"
        else:
