...
def initialize_fake_model_parallel(
    ...
    all_ranks = torch.arange(model_world_size).reshape(-1, data_parallel_size, pipeline_model_parallel_size, tensor_model_parallel_size)

    group_ranks = all_ranks.view(-1, tensor_model_parallel_size).unbind(0)
    group_ranks = [x.tolist() for x in group_ranks]
    ps._TP = FakeGroupCoordinator(rank=rank, size=tensor_model_parallel_size, group_ranks=group_ranks, backend=backend, group_name="tp", fake_group=True)

    group_ranks = all_ranks.transpose(2, 3).reshape(-1, pipeline_model_parallel_size).unbind(0)
    group_ranks = [x.tolist() for x in group_ranks]
    ps._PP = FakeGroupCoordinator(rank=rank, size=pipeline_model_parallel_size, group_ranks=group_ranks, backend=backend, group_name="pp", fake_group=True)

    group_ranks = all_ranks.transpose(1, 3).reshape(-1, data_parallel_size).unbind(0)
    group_ranks = [x.tolist() for x in group_ranks]
    ps._DP = FakeGroupCoordinator(rank=rank, size=data_parallel_size, group_ranks=group_ranks, backend=backend, group_name="dp", fake_group=True)

    group_ranks = all_ranks.view(-1, expert_parallel_size).unbind(0)
    group_ranks = [x.tolist() for x in group_ranks]
    group_ranks = [group_ranks[0]]
    ps._EP = FakeGroupCoordinator(rank=rank, size=expert_parallel_size, group_ranks=group_ranks, backend=backend, group_name="ep", fake_group=True)
    
    # PCP needs all ranks to have a group. Hence forcing it on all ranks.
    pcp_all_ranks = torch.arange(world_size).reshape(-1, data_parallel_size, pipeline_model_parallel_size, tensor_model_parallel_size)
    group_ranks = pcp_all_ranks.transpose(2, 3).reshape(-1, pipeline_model_parallel_size).unbind(0)
    group_ranks = [x.tolist() for x in group_ranks]
    ps._PCP = FakeGroupCoordinator(rank=rank, size=prefill_context_parallel_size, group_ranks=group_ranks, backend=backend, group_name="pcp", fake_group=True, force_world_size=1)

    group_ranks = all_ranks.transpose(2, 3).reshape(-1, expert_tensor_parallel_size).unbind(0)
    group_ranks = [x.tolist() for x in group_ranks]
    ps._ETP = FakeGroupCoordinator(rank=rank, size=expert_tensor_parallel_size, group_ranks=group_ranks, backend=backend, group_name="etp", fake_group=True)

    all_ranks = torch.arange(model_world_size).reshape(-1, data_parallel_size * tensor_model_parallel_size)
    group_ranks = all_ranks.unbind(0)
    group_ranks = [x.tolist() for x in group_ranks]
    ascend_ps._MC2 = FakeGroupCoordinator(rank=rank, size=len(group_ranks), group_ranks=group_ranks, backend=backend, group_name="mc2", fake_group=True)
