...
class WormNPUClient(WORMBase):
    ...
    def handshake(self):
        ...
        response = self.send_request("handshake", {"client_tgid": my_tgid,
                                                   'client_device': self.ipc_core.device_id})
    ...
    def zero_copy(self, name):
        response = self.send_request(end_point_name='zero_copy',
                                     end_point_args={"param_name": name})
    ...
    def zero_copy_model(self, model, is_clone=False):
        ...
        print(f"[WormNPUClient] | DP {self.ipc_core.dp_rank} TP {self.ipc_core.tp_rank} | Done loading model. NCCL fallback: {len(zero_copy_failed)}/{num_tensors} ({100*len(zero_copy_failed)/num_tensors:.2f}%)")
