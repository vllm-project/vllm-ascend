...
class WormServer:
    def __init__(self, world_size: int, model_size: int, tensor_parallel_size: int, api_port: int = 8000, actor_name_fmt: str = "actor-{}"):
        ...
        print(f'_init_kv_cache() {ray.get([a._init_kv_cache.remote(num_gpu_blocks=(int(os.getenv("KV_GPU_BLOCKS")))) for a in self.actors[:self.tensor_parallel_size]])}')
        print(f'set_attribute(peers) -> {ray.get([a.set_attribute.remote(name="peers", value_or_oid=self.actors) for a in self.actors])}')
        ...
        print(f'scaleup() -> {ray.get([a.scaleup.remote(num_npus=self.model_size - self.tensor_parallel_size) for a in self.actors])}')
    ...
    def _create_placement_plan(self):
        ...
        if not nic_for_node: raise RuntimeError("No alive Ray nodes found")
        head_node_id = next((k for k, v in nic_for_node.items() if v["ip"] == self.HEAD_IP), None)
        if head_node_id is None: raise RuntimeError(f"HEAD_IP={self.HEAD_IP} not found among Ray nodes: {nic_for_node}")
        nodes_in_order = [(head_node_id, nic_for_node[head_node_id]["capacity"])]
        for nid in sorted([k for k in nic_for_node.keys() if k != head_node_id], key=lambda x: nic_for_node[x]["ip"]):
        ...
        for nid, cap in nodes_in_order:
            ...
            if r >= self.world_size:
                break
        if len(plan) < self.world_size: raise RuntimeError(f"Insufficient NPUs for WORLD_SIZE={self.world_size}. Available {len(plan)} across nodes: {nic_for_node}")
        assert nic_for_node[head_node_id]["ip"] == self.MASTER_ADDR, f"Placement bug: rank-0 isn't on MASTER_ADDR={self.MASTER_ADDR}"
    ...
    def _create_placement_plan_for(self, target_world_size: int):
        ...
        nodes_in_order = [(head_node_id, nic_for_node[head_node_id]["capacity"])]
        for nid in sorted([k for k in nic_for_node.keys() if k != head_node_id],
                          key=lambda x: nic_for_node[x]["ip"]):
            nodes_in_order.append((nid, nic_for_node[nid]["capacity"]))
        ...
        if len(plan) < target_world_size:
            raise RuntimeError(
                f"Insufficient NPUs: need {target_world_size}, planned {len(plan)}"
            )
        return nic_for_node, plan
