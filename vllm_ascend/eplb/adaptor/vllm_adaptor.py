...
class VllmEplbAdaptor(EplbAdaptor):
    ...
    def do_update_log2phy_map(self, layer_id, updated_log2phy_map):
        ...
        self.pinned_log2phy_map = updated_log2phy_map.pin_memory()
        self.async_copy_stream.wait_stream(cur_stream)
        with torch.npu.stream(self.async_copy_stream):
            if self.log2phy_map_per_layer[layer_id] is not None:
                self.log2phy_map_per_layer[layer_id].copy_(self.pinned_log2phy_map.to(self.log2phy_map_per_layer[layer_id].device, non_blocking=True))
