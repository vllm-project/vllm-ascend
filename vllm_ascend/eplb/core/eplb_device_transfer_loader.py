...
class D2DExpertWeightLoader:
    ...
    def generate_expert_d2d_transfer_task(self, expert_send_info, expert_recv_info, updated_expert_map, layer_id):
        ...
        logger.debug(
            "[eplb/d2d_loader] D2D transfer task generated for layer %s, send_ops=%s recv_ops=%s",
    ...
    def asyn_expert_weight_transfer(self, reqs):
        ...
        if self.comm_op_list:
            ...
            logger.debug(
                "[eplb/d2d_loader] Launched async D2D transfer for layer %s, comm_ops=%s",
                self.layer_id, len(self.comm_op_list),
            )
    ...
    def update_expert_map_and_weight(self, reqs):
        ...
        logger.debug(
            ...
            self.layer_id, len(self.recv_expert_list),
        )
