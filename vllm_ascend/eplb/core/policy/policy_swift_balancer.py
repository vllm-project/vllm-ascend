...
class SwiftBalanceEplb(EplbPolicy):
        ...
        for layer_idx in range(layer_num):
            ...
            for npu_idx in range(npu_num):
                for expert_idx in range(experts_per_npu):
                    workload_dict[int(placement_layer[npu_idx][expert_idx])] += (  # type: ignore[assignment]
    ...
    def statistics_expert_distribution(
        ...
        expert_from_rank = np.zeros(self.num_original_experts, dtype=np.int64)  # type: ignore[var-annotated]
    ...
    def compute_redundant_assignments(
        ...
        redundant_assignments = np.zeros(self.num_original_experts, dtype=np.int64)  # type: ignore[var-annotated]
        ...
        update_weight = np.zeros(self.num_original_experts, dtype=np.float32)  # type: ignore[var-annotated]
    ...
    def fill_in_undeployed_ranks(
        ...
        rank_loads = np.zeros(len(rank_assignments), dtype=np.float32)  # type: ignore[var-annotated]
    ...
    def recomputing_initial_weight(
        ...
        num_per_existing_expert = np.zeros(self.num_original_experts, dtype=np.int64)  # type: ignore[var-annotated]
        ...
        update_workload = np.full(  # type: ignore[var-annotated]
            self.num_original_experts, fill_value=-1, dtype=np.float32
        )
    ...
    def distribute_redundant_experts(
        ...
        num_com_between_rank = np.zeros((num_ranks, num_ranks), dtype=np.int64)  # type: ignore[var-annotated]
