...
class EplbUpdator:
    ...
    def init_eplb(self, expert_map_path, process):
        ...
        try:
            ...
            logger.debug(
                "[eplb/updator] VLLM_ALLOW_EXPERT_LOAD_COLLECTING unavailable in current vllm version."
            )
            self.num_expert_load_gather = self.expert_heat_collection_interval
            self.periodic_load_gather = False
    ...
    def update_iteration(self):
        ...
        if self.cur_iterations == (
            ...
            logger.debug(
                "[eplb/updator] Full EPLB cycle completed, clearing moe loads and resetting iteration counter"
            )
