...
def compute_audio_token_length(num_samples: int, config) -> int:
...
class Dots3NoteAudioEncoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn_layer_norm = RMSNorm(hidden_size, eps=1e-6)
        self.fc1 = nn.Linear(hidden_size, ffn_size * 2)
        self.fc2 = nn.Linear(ffn_size, hidden_size)
        self.final_layer_norm = RMSNorm(hidden_size, eps=1e-6)
...
class Dots3NoteAudioSpeechEncoder(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.layer_norm = RMSNorm(hidden_size, eps=1e-6)
...
class Dots3NoteDotsAudioEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.speech_encoder = Dots3NoteAudioSpeechEncoder(config)
...
class Dots3NoteAudioTower(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.config = config
        self.dots_encoder = Dots3NoteDotsAudioEncoder(config)
    ...
    def device(self) -> torch.device:
        return self.dots_encoder.speech_encoder.conv2d1.weight.device
