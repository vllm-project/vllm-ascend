...
class Dots3NoteMLABackend(AscendMLABackend):
    @classmethod
    def supported_kv_cache_layouts(cls) -> tuple[KVCacheLayout, ...]:
        # Full MLA, sliding MLA and indexer pages have different sizes.
        return (KVCacheLayout.BLNHC,)
...
class Dots3NoteAttention(DeepseekV2MLAAttention):
    def __init__(
        ...
        if sliding_window is not None:
            self.mla_attn.mla_attn.attn_backend = Dots3NoteMLABackend
...
class Dots3NoteDecoderLayer(DeepseekV2DecoderLayer):
    def __init__(
        ...
        is_moe_layer = (
            ...
            and (layer_idx < config.num_hidden_layers or getattr(config, "mtp_use_moe", False))
        )
