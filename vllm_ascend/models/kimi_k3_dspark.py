...
Execution shape (v1 ``AscendDSparkProposer``):
  ...
  * The (per request) 7-token draft block then runs one bidirectional forward:
    ``_EXTRA_CTX.is_draft_model`` drives the ``sparse_mode=0`` (non-causal)
    gate in ``mla_v1.py``, and the draft attention-group metadata builders are
...
class K3DSparkMLAAttention(nn.Module):
    ...
    def __init__(
        ...
        if rope_parameters:
            ...
            if rope_parameters.get("rope_type", "default") == "yarn":
            ...
            if rope_parameters.get("rope_type") == "deepseek_yarn":
                ...
                if scaling_factor > 1 and mscale_all_dim:
                    mscale = 0.1 * float(mscale_all_dim) * math.log(scaling_factor) + 1.0
