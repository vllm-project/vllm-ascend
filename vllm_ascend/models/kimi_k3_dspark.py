...
class AscendK3DSparkDecoderLayer(nn.Module):
    def __init__(
        self,
        *,
...
class AscendK3DSparkModel(nn.Module):
    def __init__(
        self,
        *,
        vllm_config: VllmConfig,
    ...
    def precompute_and_store_context_kv(
        ...
        if per_layer_slot_mapping and len(context_slot_mapping) != len(self.layers):
            raise ValueError(
                "context_slot_mapping must contain one entry per draft layer: "
                f"got {len(context_slot_mapping)} entries for "
    ...
    def _build_fused_context_kv_buffers(self) -> None:
        # The Ascend path keeps the quantization-aware per-layer projections.
        self._context_kv_fusion_available = False
