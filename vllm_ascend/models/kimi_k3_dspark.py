...
This is the Ascend port of upstream vllm's
``vllm/models/kimi_k3/nvidia/dspark_mla.py`` (reference only, not runnable on
Ascend). The draft mirrors Kimi-K3's own MLA attention (RoPE on, no output
gate) and shares the target's 576-element latent KV layout.

Ascend-specific deltas vs the NVIDIA reference:
  * The MLA layer is built via ``MultiHeadLatentAttentionWrapper`` (which
    vllm-ascend OOT-overrides with ``AscendMultiHeadLatentAttention``), so the
    projection / RoPE / cache / attention path runs inside ``AscendMLAImpl``.
    ``use_mla_rope=True`` drives the RoPE cos/sin via
    ``get_cos_and_sin_mla`` (the yarn ``RotaryEmbedding`` records its cache at
    construction); the bidirectional (non-causal) draft attention is enabled by
    the ``_EXTRA_CTX.is_draft_model`` gate added to ``mla_v1.py``.
  * ``precompute_and_store_context_kv`` writes the context KV through the
    Ascend impl's ``exec_kv_prefill`` (fused rmsnorm+RoPE+cache-insert) instead
    of the NVIDIA ``do_kv_cache_update`` / grouped-insert CUDA ops.
"""
...
class K3DSparkMLAAttention(nn.Module):
    ...
    Mirrors ``KimiK3MLAAttention`` (vllm-ascend) but flips the two K3-target
    constraints: the draft is trained with YaRN RoPE on ``k_pe``
    (``use_mla_rope=True``) and without an output gate
    (``use_output_gate=False``, no ``g_proj``). All projection / RoPE / cache /
    attention work is delegated to ``AscendMLAImpl`` through the
    ``MultiHeadLatentAttentionWrapper`` (OOT-overridden on Ascend).
    """
    ...
    def __init__(
        ...
        # Fused q-down + kv-down projection, replicated (disable_tp): the
        # low-rank latents are shared across TP ranks; TP splitting happens at
        # q_b_proj / kv_b_proj. Checkpoint keys ``q_a_proj`` and
        ...
        self.q_b_proj = MergedColumnParallelLinear(
            q_lora_rank,
        ...
        self.kv_b_proj = MergedColumnParallelLinear(
            kv_lora_rank,
        ...
        # YaRN RoPE on k_pe (draft is trained with RoPE, unlike the K3 target
        # which is NoPE-only). Building the rotary_emb records its cos/sin cache
        # globally (AscendRotaryEmbedding.__init__), which get_cos_and_sin_mla
        # reads during the draft forward / context-KV precompute.
        ...
        if rope_parameters is not None:
            ...
            if rope_parameters.get("rope_type") == "deepseek_yarn":
                ...
                mscale = (
                    1.0
                    if scaling_factor <= 1
                    else 0.1 * float(mscale_all_dim) * math.log(scaling_factor)
                    + 1.0
                )
        ...
        # Convenience aliases used by the speculator / precompute hooks.
...
class K3DSparkMLP(nn.Module):
    ...
    def __init__(
        ...
        if hidden_act == "silu":
            ...
            raise ValueError(
                f"Unsupported K3 dspark activation: {hidden_act}. "
                "Only 'silu' is supported (draft config has no hidden_act)."
            )
...
class K3DSparkDecoderLayer(nn.Module):
    def __init__(
        ...
        *,
        ...
        # load_dspark_model builds the draft VllmConfig via replace() and does
        # NOT override quant_config, so vllm_config/quant_config here would
        # inherit the target's w4a8/w8a8 quant -- which then fails the draft's
        # process_weights_after_loading (maybe_trans_nz) on bf16 weights. Force
        # None: the draft is always bf16.
        quant_config = None
...
class K3DSparkModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        # The frozen target embedding is aliased by the speculator's loader after
        # the draft checkpoint loads (has_own_embed_tokens=False).
    ...
    def embed_input_ids(
        ...
        multimodal_embeddings: torch.Tensor | list | None = None,
        *,
        is_multimodal: torch.Tensor | None = None,
    ) -> torch.Tensor:
        # K3 dspark draft is text-only (mirrors vllm dspark_mla.py): vision
        # content reaches the draft through the target's aux hidden states
        # (combine_hidden_states), not through these input embeddings. The v1
        # proposer passes multimodal_embeddings because the K3 *target* is
        # multimodal (supports_mm_inputs=True); accept and ignore them here.
    ...
    def precompute_and_store_context_kv(
        ...
        Mirrors the NVIDIA reference's per-layer fallback, but writes through
        the Ascend impl's ``exec_kv_prefill`` (fused kv_a_layernorm + YaRN RoPE
        + paged-cache insert). ``context_slot_mapping`` is a per-layer list (the
        hybrid manager may place draft layers in different cache groups); a
        ``None`` entry (or ``None`` overall) runs the projection for workspace
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            kv_no_split = qkv_lora[..., attn.q_lora_rank :]
            slot_mapping = (
    ...
    def get_draft_kv_cache_layer_names(self) -> list[str]:
        ...
        # -- the key used by the speculator for all_attn_layers / kv-cache groups
    ...
    def get_draft_attn_causal(self) -> list[bool]:
        # Bidirectional (non-causal) draft attention -- drives the
        # is_draft_model -> sparse_mode=0 gate in mla_v1.py.
...
class K3DSparkForCausalLM(nn.Module):
    # Draft shares the target's embedding / lm_head (aliased by load_dspark_model).
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # Aliased from the target by load_dspark_model; keep no placeholder to
        # avoid a transient full-vocabulary allocation for this 163k-vocab model.
