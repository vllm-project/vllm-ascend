...
from vllm_ascend.ops.fused_moe import AscendSparseMoeBlock, apply_mlp, select_experts
...
class Qwen3MoeDecoderLayerDBO(Qwen3MoeDecoderLayer):
    ...
    def __init__(
        ...
        super(Qwen3MoeDecoderLayerDBO, self).__init__(
    ...
    def _forward_op_gating(
        ...
        if self.tp_size > 1:
            ...
            chunk_hidden_states = torch.tensor_split(hidden_states, self.tp_size, dim=0)
    ...
    def _forward_combine_comm(
        ...
        if self.tp_size > 1:
            final_hidden_states = gather_from_sequence_parallel_region(
                final_hidden_states, self.tp_group, chunked_hidden_states_sizes
            )
            if num_tokens < self.tp_size:
    ...
    def _forward_ms_layer_alltoallv_finegrained(
        ...
        for i in range(num_micro_batchs):
            ...
            # input layernorm
            hidden_states[i], residual[i] = self._forward_ms_op_input_layernorm(
                hidden_states[i], residual[i]
            )
            # attention and tp allreduce
            hidden_states[i], residual[i] = self._forward_ms_op_attn(
                positions[i], hidden_states[i], residual[i], kv_cache, attn_metadata[i]
            )
        ...
        for i in range(num_micro_batchs):
            ...
            with torch.npu.stream(combine_context.comm_stream):
                ...
                ms_metadata.ms_events[layer_index][i][
                    MSEventKey.FFN_AR_FINISH
                ] = combine_context.comm_stream.record_event()
...
class CustomQwen3DBOMoEModel(Qwen3MoeModel):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if VLLM_ASCEND_ENABLE_DBO:
            ...
            self.ms_pre_layer = MultiStreamPreTransformerLayer(multistream_metadata)
            self.ms_post_layer = MultiStreamPostTransformerLayer(multistream_metadata)
...
class CustomQwen3MoeForCausalLMDBO(Qwen3MoeForCausalLM):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.model = CustomQwen3DBOMoEModel(
