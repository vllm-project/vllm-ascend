...
class PanguProMoEMLP(nn.Module):
    ...
    def forward(self, x, attn_metadata: Optional[AttentionMetadata] = None):
...
class PanguProMoEDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if (layer_idx not in mlp_only_layers) and hasattr(config, "num_experts") and (config.num_experts > 0):
...
class SophonForCausalLM(PanguProMoEForCausalLM):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
