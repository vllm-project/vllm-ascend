...
from vllm_ascend.models.common.deepseek_vl import DeepseekMultiModalMixin
...
class AscendDeepseekV41ForCausalLM(
    ...
    def __init__(self, *, vllm_config, prefix: str = "") -> None:
        ...
        with self._mark_tower_model(vllm_config, {"image"}):
            self.vision: DeepseekViT | None = None
            self.aligner: DeepseekAligner | None = None
            self.image_start: nn.Parameter | None = None
            self.image_end: nn.Parameter | None = None
            self.image_newline: nn.Parameter | None = None
            if image_enabled:
                self.vision = DeepseekViT(config)
                self.aligner = DeepseekAligner(config)
