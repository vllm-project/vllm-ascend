...
class DeepseekPatchEmbed(nn.Module):
...
class DeepseekVisionAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class DeepseekVisionMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class DeepseekViT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.patch_embed = DeepseekPatchEmbed(config)
        self.blocks = nn.ModuleList([DeepseekVisionBlock(config) for _ in range(config.vision_n_layers)])
...
class DeepseekAligner(nn.Module):
