...
def _is_glm5_main_spec(spec: KVCacheSpec) -> bool:
    return isinstance(spec, MLAAttentionSpec) and _is_glm5_spec(spec) and get_kv_cache_compression_ratio(spec) == 1
...
def _align_glm5_cache_specs(kv_cache_spec: dict[str, KVCacheSpec]) -> None:
    ...
    indexer_specs = [spec for spec in kv_cache_spec.values() if _is_glm5_indexer_spec(spec)]
...
def _create_glm5_attention_groups(
    ...
    main_names = _sorted_layer_names([name for name, spec in kv_cache_spec.items() if _is_glm5_main_spec(spec)])
    indexer_names = _sorted_layer_names([name for name, spec in kv_cache_spec.items() if _is_glm5_indexer_spec(spec)])
    state_names = _sorted_layer_names([name for name, spec in kv_cache_spec.items() if _is_glm5_state_spec(spec)])
...
def _get_glm5_cache_layout(
    ...
    for group in kv_cache_groups:
        ...
        if (
            values
            and all(isinstance(spec, MLAAttentionSpec) and _is_glm5_spec(spec) for spec in values)
    ...
    mla_names = _sorted_layer_names([name for name in full_group.layer_names if _is_glm5_main_spec(full_specs[name])])
    indexer_names = _sorted_layer_names(
        [name for name in full_group.layer_names if _is_glm5_indexer_spec(full_specs[name])]
    )
    ...
    main_page_sizes = {spec.page_size_bytes for spec in full_specs.values() if _is_glm5_main_spec(spec)} | {
        group.kv_cache_spec.page_size_bytes for group in mamba_groups
    }
    small_page_sizes = {spec.page_size_bytes for spec in full_specs.values() if _is_glm5_indexer_spec(spec)} | {
