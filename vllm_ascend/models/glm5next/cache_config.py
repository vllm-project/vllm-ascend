...
def _align_glm5_cache_specs(kv_cache_spec: dict[str, KVCacheSpec]) -> None:
    ...
    if not main_specs or not indexer_specs or not state_specs:
        raise ValueError(
            "GLM-Next cache layout requires main MLA, compressed indexer, "
            "and compressor-state specs."
        )
...
def _create_glm5_attention_groups(
    ...
    if not (len(main_names) == len(indexer_names) == len(state_names) > 0):
        raise ValueError(
            "Every GLM-Next MLA layer requires one main MLA, compressed "
            "indexer, and compressor-state cache."
        )
    ...
    if main_indices is not None and (
        ...
        raise ValueError(
            "GLM-Next MLA, indexer, and state cache layer indices do not match."
        )
    ...
    if len(full_block_sizes) != 1:
        raise ValueError(
            "GLM-Next main MLA and compressed indexer caches must use one "
            "logical block size."
        )
    ...
    for main_name, indexer_name, state_name in zip(
        ...
        if (
            ...
            or state_spec.sliding_window != compress_ratio
        ):
            raise ValueError(
                "GLM-Next indexer state block/window size must equal the paired "
                f"compression ratio {compress_ratio}."
            )
    ...
    if full_uniform_spec is None:
        raise ValueError(
            "GLM-Next main MLA and compressed indexer caches must have uniform "
    ...
    if state_uniform_spec is None:
        raise ValueError(
            "GLM-Next compressor-state caches must have uniform sliding-window "
...
def _get_glm5_cache_layout(
    ...
    for group in kv_cache_groups:
        ...
        elif values and all(_is_glm5_state_spec(spec) for spec in values):
    ...
    if len(full_groups) != 1 or len(state_groups) != 1:
        raise ValueError(
            "GLM-Next requires exactly one combined main/indexer group and "
            "one compressor-state KV cache group."
        )
    if len(full_groups) + len(state_groups) + len(mamba_groups) != len(
        kv_cache_groups
    ):
    ...
    mla_names = _sorted_layer_names(
        [
            ...
            if _is_glm5_main_spec(full_specs[name])
        ]
    )
    indexer_names = _sorted_layer_names(
        [
    ...
    if not (len(mla_names) == len(indexer_names) == len(state_names)):
        raise ValueError(
            "Every GLM-Next MLA layer must own one compressed indexer and one "
            "compressor-state cache."
        )
    mla_indices = _layer_indices(mla_names)
    if mla_indices is not None and (
        mla_indices != _layer_indices(indexer_names)
        or mla_indices != _layer_indices(state_names)
    ):
        raise ValueError(
            "GLM-Next MLA, indexer, and state cache layer indices do not match."
        )
    ...
    if len(main_page_sizes) != 1 or len(small_page_sizes) != 1:
        raise ValueError(
            "GLM-Next cache specs were not aligned to two physical page sizes."
        )
...
def _group_glm5_mamba_layer_names(
    ...
    for name in kv_cache_spec:
        try:
            ...
            raise ValueError(
                "GLM-Next Mamba grouping requires layer names with numeric "
        ...
        if previous_kind != is_mamba:
            raise ValueError(
                "A GLM-Next model layer cannot contain both Mamba and MLA "
                f"cache specs: layer index {layer_idx}."
            )
        if is_mamba:
            if layer_idx in mamba_name_by_index:
                raise ValueError(
                    "A GLM-Next model layer must own exactly one Mamba cache "
                    f"spec: layer index {layer_idx}."
                )
    ...
    for layer_idx in sorted(layer_is_mamba):
        is_consecutive = (
            previous_layer_idx is not None
            and layer_idx == previous_layer_idx + 1
        )
    ...
    if max_run_length == 0:
        raise ValueError(
            "GLM-Next Mamba specs were provided but no Mamba layers were found."
        )

    sorted_mamba_names = [
        mamba_name_by_index[index] for index in sorted(mamba_name_by_index)
    ]
    return [
        sorted_mamba_names[offset::max_run_length]
        for offset in range(max_run_length)
    ]
...
def _create_mamba_groups(
    ...
    sorted_groups = [
        list(_sorted_layer_names(layer_names)) for layer_names in grouped_layer_names
    ]
    return create_kv_cache_group_specs(mamba_specs, sorted_groups)


def get_glm5_kv_cache_groups(
    vllm_config: VllmConfig,
    kv_cache_spec: dict[str, KVCacheSpec],
) -> list[KVCacheGroupSpec]:
    """Build GLM-Next scheduler groups and align their physical page sizes."""

    if not any(_is_glm5_spec(spec) for spec in kv_cache_spec.values()):
    ...
    if getattr(scheduler_config, "disable_hybrid_kv_cache_manager", False):
        raise ValueError(
            "GLM-Next's paired MLA/indexer and sliding state layout requires "
    ...
    grouped_names = _group_glm5_mamba_layer_names(kv_cache_spec, mamba_specs)
    groups.extend(_create_mamba_groups(mamba_specs, grouped_names))
    return groups


def get_glm5_pool_bytes_per_block(groups: list[KVCacheGroupSpec]) -> int:
...
def get_glm5_kv_cache_config(
    ...
    for indexer_name, state_name in zip(
