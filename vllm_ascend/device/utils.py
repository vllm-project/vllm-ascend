...
FIA_TND_PAGED_PREFILL_FALLBACK_HEAD_SIZE = 256
...
def _gather_paged_kv_to_dense(
    ...
    block_size = key_cache.shape[2] if use_bnsd_kv_cache else key_cache.shape[1]
    ...
    gathered_key = key_cache.index_select(0, flat_block_ids)
    gathered_value = value_cache.index_select(0, flat_block_ids)
    if use_bnsd_kv_cache:
        gathered_key = gathered_key.transpose(1, 2)
        gathered_value = gathered_value.transpose(1, 2)
    gathered_key = gathered_key.reshape(dense_shape)
    gathered_value = gathered_value.reshape(dense_shape)
