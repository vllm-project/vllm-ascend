...
class BaseDeviceAdaptor:
    ...
    def mla_preprocess_only_decode(atten_obj, hidden_states, kv_cache, attn_metadata):
        ...
        decode_q_nope, decode_q_pe, decode_k_nope, decode_k_pe, dequant_scale_q_nope = torch_npu.npu_mla_prolog_v2(
            ...
            quant_scale_ckv=atten_obj.quant_kscale if atten_obj.fa_quant_layer else None,
            cache_mode="PA_NZ" if atten_obj.enable_kv_nz else "PA_BSND",
        )
        # prolog_v2 returns a real scale only when the kv cache is quantized (fa_quant);
        # downstream selects the int8 attention path when it is not None.
        if not atten_obj.fa_quant_layer:
