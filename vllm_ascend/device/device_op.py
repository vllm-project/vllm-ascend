...
class BaseDeviceAdaptor:
    ...
    def dsa_kv_compress_scatter(cache, x, slot_mapping):
        ...
        torch.ops._C_ascend.npu_scatter_nd_update_asc(cache, slot_mapping, x)
    ...
    def indexer_quant_scatter(q, kv, indexer_k_cache, indexer_scale_cache, indexer_full_cache, slot_mapping):
        ...
        if kv is not None:
            ...
            torch.ops._C_ascend.npu_scatter_nd_update_asc(indexer_k_cache, slot_mapping, kv_out)
            torch.ops._C_ascend.npu_scatter_nd_update_asc(indexer_scale_cache, slot_mapping, kv_scale_out)
    ...
    def dsa_indexer_scatter_scale_part3(kv_scale, indexer_scale_cache, slot_mapping):
        ...
        if kv_scale.ndim < 4:
            kv_scale = kv_scale.unsqueeze(-1)
        torch.ops._C_ascend.npu_scatter_nd_update_asc(indexer_scale_cache, slot_mapping, kv_scale)
    ...
    def warmup_indexer_quant_scatter(hidden_states, slot_mapping):
        ...
        torch.ops._C_ascend.npu_scatter_nd_update_asc(indexer_k_cache, slot_mapping, kv_dummy)
        torch.ops._C_ascend.npu_scatter_nd_update_asc(indexer_scale_cache, slot_mapping, kv_scale_dummy)
