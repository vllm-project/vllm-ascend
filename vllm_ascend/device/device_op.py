...
class BaseDeviceAdaptor:
    ...
    def mla_preprocess_only_decode(
        ...
        # Keep the shared adaptor signature. A2 only reaches this prolog when
        # RoPE is enabled; no-RoPE MLA uses the generic preprocessing path.
...
class A5DeviceAdaptor(BaseDeviceAdaptor):
    ...
    def mla_preprocess_only_decode(
        ...
        bsz = attn_metadata.num_decode_tokens
        hidden_states = hidden_states[:bsz]
        weight_quant_mode = atten_obj.mlapo_weight_quant_mode
        if weight_quant_mode == 0:
            token_x = hidden_states
            dynamic_scale = None
        else:
            token_x = hidden_states.unsqueeze(1)
            token_x, dynamic_scale = torch_npu.npu_dynamic_mx_quant(
                token_x,
                dst_type=torch.float8_e4m3fn,
            )
            dynamic_scale = dynamic_scale.reshape(
                token_x.shape[0] * token_x.shape[1],
                -1,
            )
        ...
        if use_mla_rope:
            ...
            cos = token_x.new_empty((0,), dtype=torch.bfloat16)
            sin = token_x.new_empty((0,), dtype=torch.bfloat16)
        ...
        decode_q_nope, decode_q_pe, dequant_scale_q_nope, _, _ = torch_npu.npu_mla_prolog_v3(
            ...
            weight_uk=atten_obj.mlapo_W_UK_T,
        ...
        prolog_num_heads = atten_obj.mlapo_num_heads
        decode_q_nope = decode_q_nope.view(
            bsz,
            prolog_num_heads,
            atten_obj.kv_lora_rank,
        )
