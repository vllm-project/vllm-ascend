    ...
    def _restore_mxfp_semantic_dtype(
        ...
        def restore_dtype(tensor: torch.Tensor) -> torch.Tensor:
            if tensor.dtype != torch.uint8:
                return tensor
            # Plain four-dimensional scale storage is tagged NCHW by
            # torch_npu. Its bytes need the same E8M0 view as ND scales.
            if isinstance(tensor, torch.Tensor) and torch_npu.get_npu_format(tensor) not in (
                torch_npu.Format.ND,
                torch_npu.Format.NCHW,
            ):
                return tensor
