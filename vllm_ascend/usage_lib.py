...
class UsageMessage:
    ...
    def _report_usage_once(self, model_architecture: str,
        ...
        if current_platform.is_npu():
            try:
                import torch_npu
                self.gpu_count = torch_npu.npu.device_count()
                self.gpu_type = torch_npu.npu.get_device_name()
                self.gpu_memory_per_device = torch_npu.npu.get_device_properties().total_memory
            except Exception:
                pass
