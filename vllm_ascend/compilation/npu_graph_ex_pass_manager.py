...
class NpuGraphEXPassManager:
    ...
    def configure(self, config: VllmConfig):
        ...
        if self.ascend_compilation_config.get("graphex_norm_quant", True):
            from .npugraph_ex_passes.add_rms_norm_quant import \
                GraphEXAddRMSNormFusionPass
            self.passes.append(GraphEXAddRMSNormFusionPass(config))

        if self.ascend_compilation_config.get("graphex_qknorm_rope", True):
            from .npugraph_ex_passes.add_qvnorm_rope_fusion import \
