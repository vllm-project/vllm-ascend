...
class QKNormRopeFusionPattern:
    ...
    def register(self, pm_pass: PatternMatcherPass):
        ...
        def pattern(qkv: torch.Tensor, q_weight: torch.Tensor,
                    k_weight: torch.Tensor, cos_sin_cache: torch.Tensor,
            ...
            q_rope, k_rope = torch.ops.vllm.rope_cos_sin(
                q_flat, k_flat, cos_sin_cache, positions, self.head_dim, self.head_dim, True)
...
class QKNormRopeFusionPatternWithBias:
    ...
    def register(self, pm_pass: PatternMatcherPass):
        ...
        def pattern(qkv: torch.Tensor, q_weight: torch.Tensor,
                    ...
                    k_bias: torch.Tensor, cos_sin_cache: torch.Tensor,
                    positions: torch.Tensor):
            q, k, v = qkv.split([self.q_size, self.kv_size, self.kv_size],
                                dim=-1)
