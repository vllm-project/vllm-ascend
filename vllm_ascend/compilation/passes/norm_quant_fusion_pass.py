...
class AddRMSNormDynamicQuantPattern:
    ...
    def register(self, pm_pass: PatternMatcherPass):
        ...
        def replacement(rms_norm_input: torch.Tensor, residual: torch.Tensor,
            ...
            Replacement for the AddRMSNormQuant fusion.
            """
            output = torch.ops.npu.npu_add_rms_norm_dynamic_quant(rms_norm_input,
                                                          residual,
                                                          rms_norm_weight,
                                                          epsilon=self.eps,output_mask=[True, True])
...
class AddRMSNormDynamicQuantSPPattern:
    ...
    def register(self, pm_pass: PatternMatcherPass):
        ...
        def replacement(rms_norm_input: torch.Tensor, residual: torch.Tensor,
            ...
            quantized_output = torch.ops.vllm.maybe_all_gather_and_maybe_unpad(
                output[0], True)
            output[3] = torch.ops.vllm.maybe_all_gather_and_maybe_unpad(
                output[3], True)
            return quantized_output, output[3], output[2]
