...
def _compute_decode_cudagraph_batch_sizes(vllm_config: VllmConfig) -> list[int]:
    ...
    if (
        ...
        decode_query_lens = {entry[2] + 1 for entry in schedule}
        max_capture_size = vllm_config.compilation_config.max_cudagraph_capture_size
        assert max_capture_size is not None

        # Keep static-kernel shapes aligned with the FULL graph descriptors
        # generated by vLLM's CudaGraphManager._init_candidates(). Dynamic SD
        # rounds each raw capture size using the query length selected for the
        # current batch instead of the configured maximum query length.
        return sorted(
            {
                rounded_size
                for query_len in decode_query_lens
                for size in vllm_config.compilation_config.cudagraph_capture_sizes
                if (rounded_size := round_up(size, query_len)) <= max_num_tokens
                and rounded_size <= max_capture_size
                and rounded_size // query_len <= vllm_config.scheduler_config.max_num_seqs
            }
        )
