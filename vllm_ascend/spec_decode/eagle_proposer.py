...
class SpecDecodeBaseProposer(Proposer):
    ...
    def dummy_run(
        ...
        with set_ascend_forward_context(
            None, self.vllm_config, moe_comm_type=moe_comm_type, num_tokens=num_tokens
        ):
            model_kwargs = dict(
    ...
    def generate_token_ids(
        ...
        if spec_decode_metadata is None:
            ...
            cu_num_tokens, token_indices = self._prepare_inputs(
                eagle_attn_metadata, num_rejected_tokens
            )
    ...
    def _get_atten_dict(
        ...
        req_indices = np.repeat(self.runner.arange_np[:num_reqs], num_scheduled_tokens)
        ...
        np.add(
            self.runner.input_batch.num_computed_tokens_cpu[req_indices],
            arange,
            out=positions_np,
        )
        ...
        token_indices = (
            positions_np + req_indices * self.runner.input_batch.token_ids_cpu.shape[1]
        )
        ...
        for kv_cache_group_id, kv_cache_group_spec in enumerate(
            ...
            block_table = self.runner.input_batch.block_table[kv_cache_group_id]
            # E.g., [0, 1, 0, 1, 2, 3, 4, 0, 1, 2]
        ...
        self.runner.input_ids[:total_num_scheduled_tokens].copy_(
            self.runner.input_ids_cpu[:total_num_scheduled_tokens], non_blocking=True
        )
        if self.runner.uses_mrope:
            # Only relevant for models using M-RoPE (e.g, Qwen2-VL)
            self.runner.mrope_positions[:, :total_num_scheduled_tokens].copy_(
                self.runner.mrope_positions_cpu[:, :total_num_scheduled_tokens],
                non_blocking=True,
            )
        else:
    ...
    def _propose(
        ...
        attn_mask = self.attn_mask_builder.get_splitfuse_attn_mask(
            seq_lens, target_positions, self.vllm_config.model_config.dtype, self.device
        )
        ...
        moe_comm_type = self.runner._select_moe_comm_method(input_batch_size, False)
        ...
        for now_speculative in range(
            ...
            exceeds_max_model_len = (
                positions_cpu >= self.vllm_config.model_config.max_model_len
            )
