...
class SpecDecodeBaseProposer(EagleProposer):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device, pass_hidden_states_to_model: bool, runner=None):
        ...
        logger.info(
            "MTP graph config: use_cuda_graph=%s, use_async_scheduling=%s, "
            "disable_padded=%s, use_compress=%s, "
            "has_full_cudagraphs=%s",
            self.use_cuda_graph,
            self.use_async_scheduling,
            self.speculative_config.disable_padded_drafter_batch if self.method == "mtp" else "N/A",
            self.use_compress,
            self.vllm_config.compilation_config.cudagraph_mode.has_full_cudagraphs() if self.method == "mtp" else "N/A",
        )
    ...
    def _maybe_share_lm_head(self, model: nn.Module) -> None:
        ...
        if self.vllm_config.compilation_config.cudagraph_mode.has_full_cudagraphs() and self.use_cuda_graph:
            ...
            if self.method == "mtp":
                logger.info(
                    "MTP graph setup: fused_with_main_graph=%s",
                    getattr(self, "fused_with_main_graph", False),
                )
                if not getattr(self, "fused_with_main_graph", False):
                    ...
                    logger.info("MTP: fused_with_main_graph=True, skipping independent ACLGraphWrapper")
            else:
    ...
    def _propose(
        ...
        num_input_tokens = self._get_mtp_eager_padded_num_input_tokens(
            num_input_tokens,
            aclgraph_runtime_mode,
        )
        if num_tokens_across_dp is not None and num_input_tokens > int(num_tokens_across_dp.max().item()):
            num_tokens_across_dp[:] = num_input_tokens
        uses_mtp_eager_input_padding = self._uses_mtp_eager_input_padding(
            num_tokens,
            num_input_tokens,
            aclgraph_runtime_mode,
        )
        ...
        if aclgraph_runtime_mode == CUDAGraphMode.FULL:
            ...
            num_reqs_padded = self.runner._pad_mtp_query_start_loc_for_graph(
                num_input_tokens, common_attn_metadata.num_reqs
            )
        ...
        self._pad_mtp_full_graph_inputs(
            num_tokens,
            num_input_tokens,
            aclgraph_runtime_mode,
            force=uses_mtp_eager_input_padding,
        )
        ...
        self.slot_mapping_group[0][slot_mapping_lens:].fill_(self._graph_padding_slot_id(aclgraph_runtime_mode))
        if self._uses_mtp_full_graph_padding(aclgraph_runtime_mode) or uses_mtp_eager_input_padding:
            pad_start = min(num_tokens, self.slot_mapping_group[0].shape[0])
            pad_end = min(num_input_tokens, self.slot_mapping_group[0].shape[0])
            if pad_end > pad_start:
                self.slot_mapping_group[0][pad_start:pad_end].fill_(0)
    ...
    def attn_update_stack_num_spec_norm(
        ...
        if self.pcp_size * self.dcp_size > 1:
            ...
            self.slot_mapping_group[draft_step][slot_mapping.shape[0] :].fill_(
                self._graph_padding_slot_id(aclgraph_runtime_mode)
            )
