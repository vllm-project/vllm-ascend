...
class AdaptiveDraftKController:
    ...
    def __post_init__(self) -> None:
        ...
        self.hybrid_acceptance_threshold = min(
            max(float(self.hybrid_acceptance_threshold), 0.0), 1.0
        )
    ...
    def update(self, lengths: Iterable[int]) -> None:
        ...
        if not observed_lengths:
            return
        percentile_idx = math.ceil(
            self.percentile * (len(observed_lengths) - 1)
        )
    ...
    def observe(
        ...
        pairs = [
            ...
            for width, tokens in zip(
                scheduled_widths, sampled_token_ids, strict=True
            )
            if int(width) > 0
        ]
        if not pairs:
        ...
        accepted = [
            min(width, max(len(tokens) - 1, 0)) for width, tokens in pairs
        ]
    ...
    def _update_hybrid(
        self, scheduled_widths: Sequence[int], accepted_lengths: Sequence[int]
    ) -> None:
