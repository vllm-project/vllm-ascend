...
class _FusedModelWithMTP:
    """Wrap main-model forward and all MTP draft steps in one ACL graph."""
    ...
    def _prepare_next_token_ids_from_sampled(self) -> torch.Tensor:
        ...
        self.valid_sampled_tokens_count_buf.copy_(valid_count)
        last_valid_indices = torch.clamp(valid_count.to(torch.long) - 1, min=0)
    ...
    def _run_rejection_verifier(
        ...
        if max_spec_len == 1:
            ...
            first_token_ids = torch.where(has_draft_mask, target_token_ids, bonus_token_ids)
            ...
            target_matrix = target_token_ids[flat_indices]
            draft_matrix = draft_token_ids[flat_indices]
            ...
            for draft_idx in range(max_spec_len):
                ...
                sampled_columns.append(
                    torch.where(
                        emit_target,
                        target_matrix[:, draft_idx],
                        placeholder_req,
                    )
                )
                prefix_accepted = prefix_accepted & (
                    ~has_draft | (draft_matrix[:, draft_idx] == target_matrix[:, draft_idx])
                )
            ...
            self.sampled_token_ids_buf.copy_(
                torch.where(
                    bonus_mask,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if is_capturing and self.capture_mtp_enabled and not isinstance(hidden_states, IntermediateTensors):
            ...
            draft_hidden = getattr(self.raw_model, "get_mtp_target_hidden_states", lambda: None)()
            if draft_hidden is None:
                draft_hidden = raw_hidden
...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    def _maybe_resize_mtp_hidden_states_buffer(self) -> None:
        if self.method != "mtp":
            return

        target_hf_config = self.vllm_config.model_config.hf_config
        draft_hf_config = self.draft_model_config.hf_config
        if not hasattr(target_hf_config, "compress_ratios"):
            return

        hc_mult = getattr(draft_hf_config, "hc_mult", getattr(target_hf_config, "hc_mult", None))
        if hc_mult is None:
            return
    ...
    def build_graph_capture_attn_metadata(
        ...
        if self.pcp_size * self.dcp_size > 1 and self.block_table_tensor_clone is None:
        ...
        common_attn_metadata = AscendCommonAttentionMetadata(
            ...
            positions_cpu=self.runner._dsa_positions_cpu_buf,
        ...
        for draft_step in range(self.num_speculative_tokens):
            ...
            if self.pcp_size * self.dcp_size > 1 and draft_step > 0:
                assert self.block_table_tensor_clone is not None
    ...
    def propose_all_in_graph(
        ...
        """Run all MTP draft steps inside the main model ACL graph."""
        ...
        current_input_ids.copy_(
            torch.where(
                bonus_update_mask,
                bonus_values.to(current_input_ids.dtype),
                current_input_ids,
            )
        )
        ...
        self._set_positions(
            ...
            torch.where(
                position_mask,
                incoming_positions,
                torch.zeros_like(current_positions),
            ),
        )
    ...
    def dummy_run(
        ...
        if (
            ...
            and self._draft_graph_capture_enabled()
            and not is_profile
