...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device, pass_hidden_states_to_model: bool, runner=None):
        ...
        graph_supported = self._glm_draft_graph_supported or _supports_spec_decode_graph(
            self.vllm_config.model_config, self.method
        )
        if self.use_cuda_graph and not graph_supported:
            logger.warning(
                "GLM graph-mode speculative decoding is not supported for the "
                "%s draft model. It has been "
                "automatically switched to eager mode.",
                self.method,
            )
            self.use_cuda_graph = False
        elif self.use_cuda_graph and is_glm_model:
            logger.info("Enabling ACL graph mode for the GLM-MoE-DSA %s draft model.", self.method)
    ...
    def _propose(
        ...
        if self.method == "dspark":
            ...
            assert draft_token_ids.shape[0] >= batch_size, (
                f"DSpark graph returned {draft_token_ids.shape[0]} request rows for a real batch of {batch_size}."
            )
    ...
    def _update_full_graph_params(self, forward_context, num_tokens, draft_attn_metadatas=None):
        ...
        if self.method == "dspark" and draft_attn_metadatas:
            ...
            assert graph_key == num_tokens, (
                f"DSpark graph update key {graph_key} does not match captured query size {num_tokens}."
            )
