...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    def load_model(self, model: nn.Module) -> None:
        ...
        logger.info(
            "[spec_decode/base] Draft model loaded successfully: method=%s,"
            " num_draft_attn_layers=%d, block_size=%d",
            self.method,
    ...
    def _maybe_share_lm_head(self, model: nn.Module) -> None:
        ...
        if self.method in ("eagle", "dflash"):
            ...
            if draft_has_own_lm_head:
                ...
                logger.info(
                    "[spec_decode/base] Loading EAGLE/DFLASH LM head weights from"
                    " the target model."
                )
