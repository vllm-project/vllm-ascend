...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    def _propose(
        ...
        if should_update_next_steps:
            ...
            for draft_index in range(1, self.num_speculative_tokens):
                per_layer_attn_metadata = dict()
                if cache_only_groups:
                    # Attention and its caches have distinct metadata groups,
    ...
    def build_draft_attn_metadata(
        ...
        # Copy the old attn_metadata and update
        primary_group = self._get_primary_draft_attn_group() if self.method == "mtp" else self.draft_attn_groups[0]
