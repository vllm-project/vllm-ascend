...
def prepare_eagle_input_sequential(
    out_tensor: torch.Tensor,        # 对应 out_ptr，一个 PyTorch Tensor 作为输出
    cu_query_lens: torch.Tensor,     # 对应 cu_query_lens_ptr，累积查询长度 Tensor
    cu_num_tokens: torch.Tensor,     # 对应 cu_num_tokens_ptr，累积 token 数量 Tensor
    block_size: int                  # 对应 BLOCK_SIZE，概念上的块大小
):
    """
    使用 PyTorch Tensor 模拟 Triton prepare_input_kernel 的串行版本。

    Args:
        out_tensor (torch.Tensor): 用于存储结果的输出 PyTorch Tensor。
                                   应已在 CPU 或 GPU 上分配好内存。
        cu_query_lens (torch.Tensor): 累积查询长度 Tensor，表示每个查询的起始索引。
        cu_num_tokens (torch.Tensor): 累积 token 数量 Tensor，表示每个序列的起始位置。
        block_size (int): 概念上的块大小，用于计算内部偏移。
                          在串行实现中没有并行含义，但保持与Triton接口一致。
    """
    # 确保所有输入都在同一个设备上，并且数据类型正确
    # （可选，但推荐在实际使用中检查）
    # device = out_tensor.device
    # cu_query_lens = cu_query_lens.to(device)
    # cu_num_tokens = cu_num_tokens.to(device)

    # 遍历每个“程序”或“序列”
    # 在 GPU 上，pid 是并行执行的，这里我们用一个循环来模拟
    num_programs = len(cu_num_tokens) - 1 # 通常 cu_num_tokens 会比程序数多一个，包含总长度

    for pid in range(num_programs):
        # [start_pos, end_pos)
        # 从累积 token 数量 Tensor 中加载当前序列的起始和结束位置
        # 注意：使用 .item() 将单个 Tensor 元素转换为 Python int
        start_pos = cu_num_tokens[pid].item()
        end_pos = cu_num_tokens[pid + 1].item()
        num_tokens = end_pos - start_pos # 当前序列的 token 数量

        # 从累积查询长度 Tensor 中加载当前查询的起始索引
        index_start = cu_query_lens[pid].item()

        # 计算需要多少“块”来覆盖当前序列的 token
        # torch.ceil(torch.tensor(num_tokens / block_size)).int().item() 相当于 Triton 的 tl.cdiv
        num_blocks = int(torch.ceil(torch.tensor(num_tokens / block_size)).item())

        # 遍历每个“块”
        for i in range(num_blocks):
            # 计算当前块内的偏移量
            # 这里的 offset_tensor 是一个 PyTorch Tensor，代表当前块内所有元素的相对位置
            # 注意：torch.arange 默认生成 float 类型，这里需要指定 dtype=torch.int32
            offset_tensor = torch.arange(0, block_size, dtype=torch.int32, device=out_tensor.device)

            # 计算当前块在整个扁平化输出 Tensor 中的起始位置
            global_start_offset = i * block_size

            # 计算当前块内所有元素的全局绝对索引
            # start_pos 是当前序列在整个 out_tensor 中的起始位置
            # global_start_offset 是当前块相对于 start_pos 的偏移
            # offset_tensor 是当前块内元素的相对偏移
            target_indices = torch.tensor(start_pos + global_start_offset, dtype=torch.int32, device=out_tensor.device) + offset_tensor

            # 计算要存储的值
            # index_start 是当前查询的起始索引，offset_tensor 是相对偏移
            values_to_store = torch.tensor(index_start, dtype=torch.int32, device=out_tensor.device) + offset_tensor

            # 应用掩码：只处理在当前序列范围内的有效 token
            # 这行代码在 PyTorch 中实现了 Triton 的 mask=offset < num_tokens
            mask = (target_indices >= start_pos) & \
                   (target_indices < end_pos) & \
                   (offset_tensor < num_tokens)

            # 将计算出的值存储到输出 Tensor 中
            # 使用布尔索引来根据掩码进行有条件的赋值
            # 确保索引和值的设备一致
