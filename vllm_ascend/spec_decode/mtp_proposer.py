...
class MtpProposer(EagleProposer):
    ...
    def _propose(
        ...
        if self.runner.use_aclgraph and num_scheduled_tokens <= self.runner.cudagraph_batch_sizes[-1]:
        ...
        for step in range(self.num_speculative_tokens):
            with (
                ...
                ProfileExecuteDuration().capture_async("mtp_forward"),
            ):
                model_kwargs = {}
                ...
                for layer_name in self.attn_layer_name:
                    decode_metadata = getattr(attn_metadata[layer_name], "decode", None)
                    if self.use_async_scheduling and decode_metadata is not None:
                        actual_size = len(decode_metadata.actual_seq_lengths_q)

                        decode_metadata.seq_lens_list = decode_metadata.seq_lens_list[:actual_size]
                        decode_metadata.block_table = decode_metadata.block_table[:actual_size]
            ...
            if self.pcp_size * self.dcp_size > 1:
                ...
                builder.batch_seq_mask_buf[: batch_seq_mask.shape[0]].copy_(batch_seq_mask, non_blocking=True)
