...
class AscendRejectionSampler(RejectionSampler):
    ...
    def forward(
        ...
        # FLy requires at least fly_window_size subsequent draft tokens.
        # If the longest request does not have enough lookahead tokens,
        # no FLy rescue can occur, so skip all FLy statistics.
        fly_stats_needed = (
            self._fly_verify and not sampling_metadata.all_random and metadata.max_spec_len > self._fly_window_size
        )
...
def _compute_fly_greedy_stats(
    ...
    if not get_ascend_config().enable_reduce_sample:
        ...
        valid_draft = (draft_token_ids_long >= 0) & (draft_token_ids_long < local_vocab_size)
    ...
    draft_is_local = (draft_token_ids_long >= local_vocab_start) & (draft_token_ids_long < local_vocab_end)
    safe_local_draft_ids = (draft_token_ids_long - local_vocab_start).clamp(min=0, max=local_vocab_size - 1)
    ...
    max_global_token_id = tp_size * local_vocab_size - 1
    can_pack_argmax_id = logits.dtype == torch.float32 and max_global_token_id <= (1 << 24)

    if can_pack_argmax_id:
        local_stats = torch.cat(
            (
                local_top_logits,
                local_log_normalizer.unsqueeze(-1),
                local_draft_logits.unsqueeze(-1),
                local_argmax_ids.to(torch.float32).unsqueeze(-1),
            ),
            dim=-1,
        )
        stats_per_rank = local_top_k + 3
    else:
        local_stats = torch.cat(
            (
                local_top_logits,
    ...
    gathered_stats = tp_group.all_gather(
        local_stats,
        dim=-1,
    )
    ...
    if can_pack_argmax_id:
        gathered_argmax_ids = gathered_stats[:, :, local_top_k + 2]
        target_argmax = (
            gathered_argmax_ids.gather(
                dim=-1,
    ...
    entropy_candidates = gathered_stats[:, :, :local_top_k].reshape(num_tokens, -1)
    ...
    global_draft_logits = gathered_stats[:, :, local_top_k + 1].amax(dim=-1)
    ...
    if synthetic_mode:
        ...
        if fly_verify:
            ...
            can_defer_matrix[token_req_ids, token_positions] = (
                (~native_accept_per_token) & fly_draft_allowed & (fly_entropy >= fly_entropy_threshold)
            )
            ...
            for offset in range(1, fly_window_size + 1):
                future_native_accept = torch.zeros_like(native_accept_matrix)
                future_native_accept[:, :-offset] = native_accept_matrix[:, offset:]
