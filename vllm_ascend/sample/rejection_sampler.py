...
class AscendRejectionSampler(RejectionSampler):
    ...
    def forward(
        ...
        if (
            not self.is_processed_logprobs_mode and sampling_metadata.max_num_logprobs is not None
            ...
            # the original raw logits for logprobs computation or entropy
            # verification, since apply_logits_processors modifies the tensor
            # in-place. When neither is needed, preserving the raw logits is
...
def apply_sampling_constraints(
    ...
    then top-k and top-p. For greedy decoding, it returns
    the original logits.
    ...
    Returns:
        torch.Tensor:
            - Processed logits of shape [num_tokens, vocab_size//tp_size].
    """
...
def rejection_sample(
    ...
    Args:
        ...
        target_logits: Target logits [num_tokens, vocab_size//tp_size]
    ...
    if using_block_verify or using_entropy_verify:
        logger.info_once(
            ...
            "posterior_threshold=%s, posterior_alpha=%s, "
