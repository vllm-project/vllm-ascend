...
class AscendRejectionSampler(RejectionSampler):
    ...
    def apply_logits_processors(
        ...
        """Override base class to use pin_memory before non_blocking transfer.

        On Ascend NPU, non-blocking CPU-to-NPU transfers require pinned
        (page-locked) memory. The base class creates repeat_indices_cpu
        without pin_memory, which causes aclrtMemcpy failures.
        """
        ...
        if sampling_metadata.allowed_token_ids_mask is not None or has_penalties:
            ...
            repeat_indices = repeat_indices_cpu.pin_memory().to(
