...
_METHODS_TO_PATCH = {
    "debug_once": _VllmAscendLogger.debug_once,
    "info_once": _VllmAscendLogger.info_once,
    "warning_once": _VllmAscendLogger.warning_once,
}
...
def init_logger(name: str, module: str | None = None) -> _VllmAscendLogger:
    ...
    logger.vllm_ascend_module = module  # type: ignore[attr-defined]
...
def current_formatter_type(logger: Logger) -> Literal["color", "newline", None]:
    ...
    while lgr is not None:
        if lgr.handlers and len(lgr.handlers) == 1 and lgr.handlers[0].name == "vllm":
            ...
            if isinstance(formatter, (ColoredFormatter, AscendColoredFormatter)):
                return "color"
            if isinstance(formatter, (NewLineFormatter, AscendFormatter)):
                return "newline"
        lgr = lgr.parent
    return None
