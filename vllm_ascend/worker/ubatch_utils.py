...
def _make_ascend_common_metadata_with_slice(
    ...
    query_start_loc = slice_query_start_locs(common_attn_metadata.query_start_loc, request_slice)
    ...
    max_query_len = int((query_start_loc_cpu[1:] - query_start_loc_cpu[:-1]).max().item())
...
def split_ascend_common_metadata(
    ...
    for i, ubatch_slice in enumerate(ubatch_slices):
        is_first_chunk = i == 0
        results.append(_make_ascend_common_metadata_with_slice(ubatch_slice, common_attn_metadata, is_first_chunk))
    return results
...
def slice_model_inputs_for_ubatch(
    ...
    sliced_inputs_embeds = inputs_embeds[token_slice] if inputs_embeds is not None else None
...
class UBatchRuntimeManager:
        ...
        def run(self) -> None:
            while True:
                ...
                try:
                    ...
                    logger.exception("ubatch worker thread %s failed", self.batch_idx)
    ...
    def thread_pool(self) -> list["UBatchRuntimeManager._PersistentThread"]:
        if self._thread_pool is None:
            self._thread_pool = [
                UBatchRuntimeManager._PersistentThread(self, ubid) for ubid in range(self.num_ubatches)
            ]
    ...
    def comm_section(self):
        ...
        peer ubatch worker (so the peer resumes its MoE expert FFN — the grouped
        matmul sandwiched between two comm blocks — on its own NPU stream while
        our comm runs on ours) on enter, and switches back to compute on exit.
        When ubatch is disabled it is a no-op, so callers can wrap any MoE comm
        call unconditionally instead of repeating the ``if rt.is_ubatch_running``
        guard on both sides.
        """
    ...
    def log(self):
        ...
        logger.info(
            f"running ubatch, ubatch_rate: {ubatch_rate * 100}%, tokens_ubatch_rate: {tokens_ubatch_rate * 100}%"
        )
...
def resolve_num_tokens_across_dp(
    ...
    ``None`` because ``_sync_metadata_across_dp`` short-circuits. ubatch overlap is
