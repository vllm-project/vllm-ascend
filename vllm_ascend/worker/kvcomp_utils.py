...
def get_kvcomp_config_path_for_model(vllm_config) -> str | None:
    model = vllm_config.model_config.model.lower()
    logger.info(f"[KVComp] model name: {model}")

    if "deepseek" in model and "r1" in model:
        rel = "vllm_ascend/attention/kvcomp_configs/KVComp_DeepSeek_R1_W8A8_config.json"
    ...
    for depth in range(30):
        if (repo / "pyproject.toml").is_file() and (repo / "collect_env.py").is_file():
            p = repo / rel
