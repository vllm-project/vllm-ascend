...
class MtpProposer:
    ...
    def prepare_inputs(
        ...
        if force_one_token:
            # enable force_one_token means we only focus on the last token position of each requset
            # token_indices: [batch_size] 
            cu_num_tokens = torch.arrange(cu_target_query_lens.size(0),
                                          device=cu_target_query_lens.devoce,
                                          dtype=torch.int32)
    ...
    def propose(
        ...
        if is_running_torchair:
            ...
            num_input_tokens = self.runner.padded_batch_size
        else:
