...
class OptimizedBlockTable(BlockTable):
    """Block table that commits only the ranges changed since the last step."""
...
class MultiGroupBlockTable:
    ...
    def __init__(
        ...
        disable_optimization = os.getenv("VLLM_ASCEND_BLOCK_TABLE_NO_COMMIT_OPTIMIZE", "0")
        if disable_optimization not in ("0", "1"):
            raise ValueError("VLLM_ASCEND_BLOCK_TABLE_NO_COMMIT_OPTIMIZE must be 0 or 1")
        block_table_cls = BlockTable if disable_optimization == "1" else OptimizedBlockTable
