...
class NPUModelRunner(GPUModelRunner):
    ...
    def _postprocess_after_forward(
        ...
        if not self.broadcast_pp_output:
            if isinstance(hidden_states, (IntermediateTensors, VppContinuationOutput)):
                hidden_states.kv_connector_output = kv_connector_output
                self.kv_connector_output = kv_connector_output
                if self.debugger is not None:
                    self.debugger.stop()
    ...
    def _model_forward(
        ...
        **model_kwargs: dict[str, Any],
        ...
        # Virtual Pipeline Parallism: fix the start_layer and end_layer for inner_model
        if os.getenv("ENABLE_VPP", "false").lower() in ("true", "1"):
