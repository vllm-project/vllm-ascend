...
def init_model_parallel_group(pp_group: GroupCoordinator, usage: str) -> GroupCoordinator:
    ...
    # Eagerly initialize the device-side (HCCL) and cpu-side (gloo)
    # communicators.  The first real use of this second channel is the VPP
    # fold-point isend, whose matching recv is only posted in a LATER RPC.
    # HCCL comm init is a TCPStore rendezvous requiring BOTH ranks; left
    # lazy, the fold-point isend blocks in store.get waiting for the peer,
    # which deadlocks whenever the executor must collect replies from ALL
    # workers (e.g. kv_output_aggregator with a KV connector) instead of
    # just the output rank.  All workers reach this during model-runner
    # init, so the rendezvous always completes here.
...
@dataclass
class VppAsyncSampleState:
    """Async sample result waiting for the next VPP batch to consume it."""

    sampled_token_ids_cpu: torch.Tensor | None
    async_copy_ready_event: torch.Event | None
    producer_batch_id: int
    needs_output_token_ids: bool
...
class NPUModelRunner(GPUModelRunner):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self._vpp_async_sample_state: VppAsyncSampleState | None = None
        if self._get_vpp_size() > 1:
            ...
            self._warmup_vpp_fold_channel()

    def _warmup_vpp_fold_channel(self) -> None:
        """Force-establish the HCCL p2p connection pairs on the fold-back
        channel at startup.

        HCCL p2p does a TCPStore rendezvous with the PEER at the first
        send/recv on a (group, peer) pair.  On this channel the first real
        use is the fold-point isend whose matching recv is posted only in a
        LATER RPC, so a lazy handshake there blocks the sender in
        ``store.get`` until the peer's next RPC arrives.  That is survivable
        only when the executor waits for the output rank alone; with a KV
        connector the executor aggregates replies from ALL workers and the
        wait becomes a deadlock.  Both PP ranks run this during model-runner
        init, so the rendezvous always completes here.

        The fold-back direction for vp_size=2 is rank i -> rank i-1, and the
        matching recv direction is rank i <- rank i+1; warm exactly those
        pairs.  Ordering (send lower, then recv higher) forms an acyclic
        chain, so blocking send/recv cannot deadlock here.
        """
        group = self._vpp_pp_group_next
        if group is None or group.world_size <= 1:
            return
        probe_size = torch.Size([8])
        probe_dtype = torch.float32
        rank = group.rank_in_group
        if rank > 0:
            group.send(
                torch.zeros(probe_size, dtype=probe_dtype, device=self.device),
                dst=rank - 1,
            )
        if rank < group.world_size - 1:
            group.recv(probe_size, probe_dtype, src=rank + 1)
    ...
    def _run_vpp_stage(
        ...
        while ctx.next_vp_stage < ctx.vp_size:
            ...
            if comm.need_recv:
                ...
                recv_group = (
                    self._vpp_pp_group_next if fold_recv
                    else self._vpp_pp_group_prev
                )
                intermediate_tensors = IntermediateTensors(
                    recv_group.recv_tensor_dict(
                        src=comm.recv_src,
    ...
    def _bind_vpp_async_sample_state(
        self,
        pending_state: VppPendingSampleState,
    ) -> VppAsyncSampleState | None:
        """Bind the latest completed sample to the next VPP snapshot.

        VPP snapshots are created before older in-flight batches finish
        sampling. Consequently, cloning the async sample tensor while creating
        the snapshot captures a stale tensor. Sampling is FIFO, so bind the
        latest completed result only when the pending snapshot is consumed.
        """
        async_state = self._vpp_async_sample_state
        self._vpp_async_sample_state = None

        input_batch = pending_state.input_batch
        if async_state is not None and async_state.needs_output_token_ids:
            input_batch.sampled_token_ids_cpu = (
                async_state.sampled_token_ids_cpu
            )
            input_batch.async_copy_ready_event = (
                async_state.async_copy_ready_event
            )
        else:
    ...
    def sample_tokens(
        ...
        if execute_model_state is None:
            ...
            if not kv_connector_output:
                if self._get_vpp_size() > 1:
                    # Under VPP a sample_tokens RPC reaches every worker, but
                    # non-final-fold ranks legitimately have nothing pending.
                    # With a KV connector the executor aggregates replies
                    # from ALL workers and KVOutputAggregator asserts on None,
                    # so return an explicit empty output instead.
                    return EMPTY_MODEL_RUNNER_OUTPUT
                return None  # noqa
        ...
        if pending_vpp_state is not None and self.use_async_scheduling:
            self._bind_vpp_async_sample_state(pending_vpp_state)

        producer_needs_output_token_ids = False
        sample_context = (
        ...
        with sample_context:
            ...
            producer_needs_output_token_ids = bool(
        ...
        if pending_vpp_state is not None:
            assert self._vpp_async_sample_state is None
            self._vpp_async_sample_state = VppAsyncSampleState(
                sampled_token_ids_cpu=(
                    async_output.sampled_token_ids_cpu
                    if producer_needs_output_token_ids
                    else None
                ),
                ...
                producer_batch_id=scheduler_output.batch_id,
                needs_output_token_ids=producer_needs_output_token_ids,
            )
        else:
    ...
    def _bookkeeping_sync(
        ...
        for req_idx in range(num_sampled_tokens):
            ...
            # VPP×PD producer: the scheduler finishes a request as soon as
            # its KV send completes (_update_from_kv_xfer_finished), which
            # can land BEFORE this batch's sample_tokens RPC is consumed.
            # The follow-up execute_model then removes the request from
            # self.requests.  Its sampled tokens are dead (the scheduler
            # already finished it), so skipping the bookkeeping is safe and
            # matches the defensive .get() style used elsewhere in this file.
            req_state = self.requests.get(req_id)
            if req_state is None:
                logger.debug(
                    "sample_tokens: req %s already removed (finished via KV "
                    "xfer before sample), skipping bookkeeping", req_id)
                continue
    ...
    def _model_forward(
        ...
        **model_kwargs: dict[str, Any],
        ...
        # the decoder stack owned by the current model to the active virtual stage.
        if self._get_vpp_size() > 1:
            raw_model = self.get_model()
            if hasattr(raw_model, "language_model") and hasattr(raw_model.language_model, "model"):
                inner_model = raw_model.language_model.model
            elif hasattr(raw_model, "model"):
                inner_model = raw_model.model
            else:
                inner_model = raw_model
            vpp_ranges = getattr(getattr(inner_model, "layers", None), "vpp_layer_ranges", None)
