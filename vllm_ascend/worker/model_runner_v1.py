...
class NPUModelRunner(LoRAModelRunnerMixin, ECConnectorModelRunnerMixin):
    ...
    def _prepare_inputs(
        ...
        # TODO move this to _generate_pcp_mtp_input
        if self.pcp_size * self.dcp_size > 1 and self.decode_threshold > 2 and not with_prefill:
            num_tokens_ori = num_scheduled_tokens.sum()
            ...
            for req_id in range(num_reqs):
                ori_req_indice = req_indices_split[req_id]
                ori_position = positions_split[req_id]
                req_indices_split[req_id] = np.append(ori_req_indice, np.repeat(ori_req_indice[-1], self.decode_threshold - 2))
                positions_split[req_id] = np.append(ori_position, np.arange(ori_position[-1] + 1, ori_position[-1] + self.decode_threshold - 1))
            ...
            mtp_slot_ori = self.input_batch.block_table.block_tables[0].slot_mapping_cpu[:num_tokens_mtp]
            ...
            self.mtp_slot_pad = mtp_slot_pad
        ...
        for kv_cache_group_id, kv_cache_group_spec in enumerate(
            ...
            if self.speculative_config and self.pcp_size * self.dcp_size > 1:
                ...
                ori_query_lens_cpu = self.query_start_loc_pcp_full_cpu[1:num_reqs+1] - \
