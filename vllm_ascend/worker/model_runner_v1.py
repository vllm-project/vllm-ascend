...
class NPUModelRunner(GPUModelRunner):
    ...
    def _build_attention_metadata(
        ...
        # Compressor-SP ownership bookkeeping. This does several CPU
        # `.tolist()` transfers and per-request Python work every step, so it is
        # skipped entirely unless the opt-in feature is active. Disabled runs
        # fall back to the metadata defaults (no SP planning downstream).
        tp_size = self.parallel_config.tensor_parallel_size
        compressor_sp_active = bool(
            self.use_compress
