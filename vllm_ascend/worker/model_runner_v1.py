...
class NPUModelRunner(GPUModelRunner):
    ...
    def initialize_kv_cache(
        ...
        # A single KV cache group can still need virtual blocks: Kimi K3's
        # 768-token manager page is consumed by MLA as six 128-token blocks.
        self.use_hybrid_blocks |= any(
            kernel_size > 0 and kernel_size != group.kv_cache_spec.block_size
            for group, kernel_sizes in zip(
                (
                    group
    ...
    @staticmethod
    def _uses_mla_mixed_dtype_fa_quant_cache(attn_layer: MLAAttention) -> bool:
        checker = getattr(attn_layer.impl, "_uses_a3_mla_fa_quant_cache", None)
        return bool(checker()) if callable(checker) else False

    def _get_mla_mixed_dtype_fa_quant_cache_dtypes(
        self,
        layer_name: str,
        kv_cache_spec: AttentionSpec,
    ) -> tuple[torch.dtype, torch.dtype] | None:
        """Return physical dtypes for Kimi-K3's A3 mixed C8/BF16 cache."""
        ...
        attn_layer = attn_layers[layer_name]
        if isinstance(attn_layer, MLAAttention) and self._uses_mla_mixed_dtype_fa_quant_cache(attn_layer):
            ...
            for idx in range(len(shared_layers)):
                ...
                if (
                    ...
                    if current_sparse_sfa_c8:
                        ...
                        mla_fa_quant_dtypes = self._get_mla_mixed_dtype_fa_quant_cache_dtypes(
                            layer_name,
                            current_kv_cache_spec,
                        )
                    ...
                    if should_trim_page_padding:
                        ...
                        if not isinstance(current_kv_cache_spec, AscendMLAAttentionSpec):
                            ...
                            if mla_fa_quant_dtypes is None:
                                ...
                                k_cache_dtype, v_cache_dtype = mla_fa_quant_dtypes
                            nope_size = int(np.prod(kv_cache_shape[:-1])) * k_dim * get_dtype_size(k_cache_dtype)
                            rope_size = int(np.prod(kv_cache_shape[:-1])) * v_dim * get_dtype_size(v_cache_dtype)
                            if raw_kv_is_combined:
    ...
    def may_reinitialize_input_batch(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        for kv_cache_group_id, kv_cache_group in enumerate(kv_cache_config.kv_cache_groups):
            ...
            if isinstance(kv_cache_spec, UniformTypeKVCacheSpecs):
                # Packed hybrid groups can include Mamba specs alongside
                # attention specs. Select attention when present so its kernel
                # block size is used for slot mapping and virtual splitting.
                specs = kv_cache_spec.kv_cache_specs.values()
                kv_cache_spec = next(
                    (spec for spec in specs if isinstance(spec, AttentionSpec)),
                    next(iter(kv_cache_spec.kv_cache_specs.values())),
                )
            if isinstance(kv_cache_spec, EncoderOnlyAttentionSpec):
                continue
        ...
        for layer_name, attn_module in attn_layers.items():
            if (isinstance(attn_module, Attention)
                ...
                if self.use_sparse:
                    ...
                    if self._uses_mla_mixed_dtype_fa_quant_cache(attn_module):
                        dtype, cache_dtype_str = attn_module.impl.dtype, None
                        dtype_size = get_dtype_size(dtype)
                        # Latent KV is C8, while the positional cache remains
                        # in model dtype. Encode the combined byte size in the
                        # spec's quantized element dtype.
                        head_size_bytes = (
