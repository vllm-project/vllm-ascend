...
def normalize_mamba_kv_cache_config(kv_cache_config: KVCacheConfig) -> KVCacheConfig:
    ...
    Ascend groups same-type per-layer specs in ``UniformTypeKVCacheSpecs``,
    while upstream ``MambaHybridModelState`` expects a bare ``MambaSpec``.
    Only identical Mamba wrappers are unwrapped; attention cache specs,
    tensor ownership, offsets, sizes and block strides remain unchanged.
    """
    groups = []
    for group in kv_cache_config.kv_cache_groups:
        ...
        if isinstance(spec, UniformTypeKVCacheSpecs):
            ...
            if inner_specs and isinstance(inner_specs[0], MambaSpec) and all(inner == inner_specs[0] for inner in inner_specs):
