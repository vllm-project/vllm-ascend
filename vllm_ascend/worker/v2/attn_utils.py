...
def validate_kv_cache_tensor_layouts(kv_cache_config: KVCacheConfig) -> None:
    ...
    # vLLM v0.28.0 describes a legacy shared allocation with ``shared_by``
    # and does not expose the per-layer stride added on main by #51718.
    # Validate the common descriptor contract here, but leave region/overlap
    # checks to the main layout where every byte range is representable.
    has_per_layer_layout = all(hasattr(tensor, "layer_stride") for tensor in kv_cache_config.kv_cache_tensors)
    ...
    for tensor_idx, tensor in enumerate(kv_cache_config.kv_cache_tensors):
        ...
        if tensor.offset < 0 or tensor.block_stride < 0:
            raise ValueError(f"{label} offset and block stride must be nonnegative")
...
def _allocate_kv_cache(
    ...
    if is_dsv4_model and not (vllm_version_is("0.27.1") or vllm_version_is("0.28.0")):
    ...
    if use_hybrid_layout and not is_dsv4_model and not (vllm_version_is("0.27.1") or vllm_version_is("0.28.0")):
    ...
    for kv_cache_tensor in kv_cache_config.kv_cache_tensors:
        ...
        if isinstance(example_spec, AscendSFAIndexerCacheSpec):
            ...
            if not (vllm_version_is("0.27.1") or vllm_version_is("0.28.0")):
    ...
    for group in attn_groups:
        ...
        for layer_name in group.layer_names:
            ...
            if is_hidden_state_cache_spec(kv_cache_spec):
                ...
                # v0.28 uses [B, N, H, C]. Upstream #51718 changed cache-only
                # writes to kv_cache[block, :, offset], i.e. [B, H, N, C].
                if vllm_version_is("0.27.1") or vllm_version_is("0.28.0"):
                    kv_cache_shape = (
                        num_blocks,
