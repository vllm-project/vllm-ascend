...
#
# AscendAutoRegressiveSpeculator: shared Ascend mixin for Eagle and MTP. All
# common overrides live here; MTP only adds MLA `.decode` hooks.
import logging
...
class AscendAutoRegressiveSpeculator(AutoRegressiveSpeculator):
    """Shared Ascend mixin for autoregressive speculators (eagle / mtp).

    Subclasses (AscendEagleSpeculator, AscendMTPSpeculator) override
    ``init_cudagraph_manager`` themselves. MTP additionally overrides
    ``build_draft_attn_metadatas``/``_init_decode_draft_attn_metadatas``/
    ``_update_decode_attn_metadata``/``_build_draft_attn_metadata``/
    ``get_draft_decode_num_reqs_padded`` for MLA ``.decode`` handling.
    """

    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        """Override AutoRegressiveSpeculator.__init__ for Ascend NPUs.
    ...
    def init_cudagraph_manager(self, cudagraph_mode: CUDAGraphMode) -> None:
        ...
        # Back-fill speculator: super() creates the patched Ascend managers
        # without it (they need it for run_fullgraph).
    ...
    def _get_decode_base_metadata(self, attn_metadata, num_reqs_padded):
        # Hook: base metadata for _init. Flat reuses live; MLA rebuilds if
        # stale (.decode=None prefill residual).
        return attn_metadata
    ...
    def _prepare_step_decode_fields(self, metadata, num_reqs_padded: int) -> None:
        # Hook: per-step backend prep after attn_state/seq_lens_cpu. MLA
        # overrides for .decode copy + block_table snapshot.
        pass
    ...
    def _write_decode_step_fields(self, metadata, query_lens_list, seq_lens_list) -> None:
        # Hook: write per-step fields to nested sub-object after top-level.
        # MLA writes .decode; flat is no-op.
        pass
