...
class AscendAutoRegressiveSpeculator(AutoRegressiveSpeculator):
    ...
    if vllm_version_is("0.28.0"):
        ...
        # and routes `_build_uniform_attn_metadata` through it, so the Ascend
        # wrapper (factory context + forced DecodeOnly state) must live on the
        # new method or those callers capture a non-TND draft layout.
