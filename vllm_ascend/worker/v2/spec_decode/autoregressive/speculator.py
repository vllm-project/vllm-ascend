...
class AscendAutoRegressiveSpeculator(AutoRegressiveSpeculator):
    ...
    def _build_draft_attn_metadata(
        ...
        """Ascend helper: build draft metadata for an explicit padded layout."""
        if vllm_version_is("0.28.0"):
            # The release speculator still owns the padded-layout builder and
            # reads ``self.arange``; wrap it with the Ascend metadata factory
            # and force DecodeOnly state for the draft model.
            assert self.input_batch is not None
            with build_draft_attn_metadata_factory(
                self.input_buffers.positions,
                num_tokens_padded,
        ...
        # Upstream #56181 replaced ``DraftModelSpeculator._build_draft_attn_metadata``
        # in favour of ``_build_attn_metadata``/``_build_uniform_attn_metadata``;
        # Ascend callers that own their padded layout keep this thin adapter.
