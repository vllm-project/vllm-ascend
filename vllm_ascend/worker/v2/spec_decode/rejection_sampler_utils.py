...
from vllm.v1.worker.gpu.spec_decode import rejection_sampler_utils as _vllm_rejection_sampler_utils

_compute_block_stats_kernel = getattr(
    _vllm_rejection_sampler_utils,
    "_compute_block_stats_kernel",
    getattr(_vllm_rejection_sampler_utils, "_compute_local_logits_stats_kernel", None),
)
_compute_global_lse = getattr(
    _vllm_rejection_sampler_utils,
    "_compute_global_lse",
    getattr(_vllm_rejection_sampler_utils, "_compute_global_logsumexp", None),
)
_insert_resampled_kernel = _vllm_rejection_sampler_utils._insert_resampled_kernel

assert _compute_block_stats_kernel is not None
assert _compute_global_lse is not None
...
def _npu_uniform(seed, pos):
    # NPU Triton does not support the upstream fp64 RNG. Use the same fp32
    # tl.rand route as draft Gumbel sampling, clamped away from zero for log(u).
