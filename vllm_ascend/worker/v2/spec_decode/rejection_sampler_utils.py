...
def _npu_gumbel_block_argmax(
    ...
    #   1. pos is cast to int32: triton-ascend's philox (umulhi) only
    #      supports int32/uint32 operands.
    #   2. Noise uses the same fp32 eps-clamped tl.rand formula as
    #      gumbel.py::_gumbel_sample_kernel (tldevice.log1p and fp64
    #      tl_rand64 are unavailable on triton-ascend).
    #   3. Plain scalar loads instead of upstream's mask=is_valid_req
