...
if vllm_version_is("0.25.0"):
    ...
    # The unified class does not exist on v0.25.0. This alias is only the base
    # for the unused main-lane class definition below.
    SpeculatorCudaGraphManager = DecodeSpeculatorCudaGraphManager
else:
    from vllm.v1.worker.gpu.spec_decode.autoregressive.cudagraph_utils import (
        SpeculatorCudaGraphManager,
    )

    # Keep the release-lane class definitions importable on main. Registration
    # selects only EagleAclGraphManager on this lane.
    AttentionStatePair = Any
    DecodeSpeculatorCudaGraphManager = SpeculatorCudaGraphManager
    PrefillSpeculatorCudaGraphManager = SpeculatorCudaGraphManager


class PrefillEagleAclGraphManager(PrefillSpeculatorCudaGraphManager):
    """v0.25.0 prefill graph manager."""
