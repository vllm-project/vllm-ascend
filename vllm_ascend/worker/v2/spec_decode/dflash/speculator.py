...
if vllm_version_is("0.27.1"):
    ...
    cp_local_slot = _cp_local_slot_release
else:
    from vllm.v1.worker.gpu.cp_utils import cp_local_slot as _cp_local_slot_main  # type: ignore[import-not-found]

    cp_local_slot = _cp_local_slot_main
...
# Main-branch variant: the upstream kernel launch added `cp_rank` and the
# `CP_SIZE`/`CP_INTERLEAVE` constexprs (vllm-project/vllm#52573). Ascend does
# not use context parallelism (cp_size=1), where `cp_local_slot` reduces to the
# plain block-relative slot formula, so behavior is unchanged.
@triton.jit
def _prepare_dflash_inputs_kernel_ascend_v2(
    # Outputs
    out_input_ids_ptr,
    out_query_positions_ptr,
    out_query_start_loc_ptr,
