...
class AscendDSparkSpeculator(DSparkSpeculator):
    ...
    def _build_draft_attn_metadata(self, *, num_reqs_padded, **kwargs):  # type: ignore[misc]
        if self.attn_architecture not in ("GQA", "MLA"):
    ...
    if not vllm_version_is("0.29.0"):
        ...
        def _build_attn_metadata(
            ...
            """vLLM main draft-metadata entry point for DSpark on Ascend.

            Upstream renamed ``_build_draft_attn_metadata``, so restore the
            Ascend factory + padded query-length fixup on the new entry point.
            """
            if self.attn_architecture not in ("GQA", "MLA"):
                return super()._build_attn_metadata(  # type: ignore[attr-defined]
                    num_reqs=num_reqs,
            ...
            assert num_tokens % self.num_query_per_req == 0, "Draft tokens must contain whole query groups"
            num_reqs_padded = num_tokens // self.num_query_per_req
            with (
                build_attn_metadata_wrapper(),
                build_draft_attn_metadata_factory(
                    self.input_buffers.positions,
