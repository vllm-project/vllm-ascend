...
def _is_qwen3_eagle3_pp(vllm_config: VllmConfig) -> bool:
...
def _is_spec_decode_pp(vllm_config: VllmConfig) -> bool:
    return vllm_config.speculative_config is not None and vllm_config.parallel_config.pipeline_parallel_size > 1
...
class NPUModelRunner(GPUModelRunner):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        is_eagle3_pp = _is_qwen3_eagle3_pp(vllm_config)
        speculative_config = vllm_config.speculative_config
        if is_eagle3_pp:
        ...
        if self.speculative_config is not None and (not is_spec_decode_pp or self.is_last_pp_rank):
    ...
    def load_model(self, load_dummy_weights: bool = False, *args, **kwargs) -> None:
        is_eagle3_pp = _is_qwen3_eagle3_pp(self.vllm_config)
        if is_eagle3_pp:
            from vllm_ascend.patch.worker.patch_v2.patch_spec_decode_pp import (
    ...
    def _update_seq_lens_cpu(
        ...
        is_prefilling = [
            self.req_states.num_computed_tokens_np[req_index] < self.req_states.prefill_len.np[req_index]
            for req_index in cached_req_indices
        ]
        if not all(is_prefilling):
            # Decode counts include speculative rejection adjustments made on
            # device, so wait for the corresponding D2H copy.
            self.num_computed_tokens_event.synchronize()

        for req_id in scheduler_output.scheduled_cached_reqs.req_ids:
            req_index = self.req_states.req_id_to_index[req_id]
            if self.req_states.num_computed_tokens_np[req_index] < self.req_states.prefill_len.np[req_index]:
                # Non-final PP chunks have no sampled-token postprocess/D2H
                # copy. The scheduler count is authoritative during prefill.
