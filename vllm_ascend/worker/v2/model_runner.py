...
class NPUModelRunner(GPUModelRunner):
    ...
    def execute_model(
        ...
        if (
            ...
            # Without PCP, discard SP padding after the TP all-gather. With
            # PCP, retain the padded per-rank stride until the PCP all-gather
            # restores the global token order and removes all padding.
            num_tokens = (
                state.input_batch.num_tokens if self.pcp_manager is None else state.input_batch.num_tokens_after_padding
            )
