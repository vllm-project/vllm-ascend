...
class NPUModelRunner(GPUModelRunner):
    def __init__(
        ...
        self.output_copy_stream = torch.npu.Stream(self.device)
        self.output_copy_event = torch.npu.Event()
        if self.use_async_scheduling:
            self.input_prep_event = torch.npu.Event()
            self.structured_outputs_event = torch.npu.Event()
            self.spec_decode_event = torch.npu.Event()
        else:
        ...
        # CUDA graphs.
        self.cudagraph_manager = AclGraphManager(
