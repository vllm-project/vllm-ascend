...
class PCPManager:
    ...
    def __init__(
        ...
        self.full_indices = list(
            range(
                self.max_num_tokens * self.pcp_size * self.dcp_size + self.pcp_size * self.dcp_size * self.max_num_reqs
            )
        )
        ...
        self.pcp_enter_fa_restore_idx = torch.zeros(
            self.max_num_tokens + 2 * self.pcp_size * self.max_num_reqs, dtype=torch.int32, device=self.device
        )
        self.pcp_use_hybrid_attn = self.vllm_config.model_config.hf_config.model_type == "qwen3_next"

        self.pcp_pads_logits_hybrid_attn = torch.ones(self.max_num_reqs, dtype=torch.int32) * (self.pcp_size - 1)
    ...
    def update_tokens_for_pcp(
        ...
        num_padded_scheduled_tokens = np.ceil(num_scheduled_tokens / (2 * self.pcp_size)).astype(np.int32) * (
            2 * self.pcp_size
        )
        ...
        num_padded_scheduled_tokens[: self.num_decode_reqs] = (
            num_scheduled_tokens[: self.num_decode_reqs] * self.pcp_size
        )
        ...
        unpad_mask_decode = self.pcp_unpad_mask_cpu[: self.num_decode_tokens * self.pcp_size]
        ...
        def get_current_rank_positions(positions_start_loc: int | np.ndarray, rank: int):
            ...
            tail_start_loc = positions_start_loc + (2 * self.pcp_size - rank - 1) * pcp_chunk_sizes
        ...
        if self.pcp_use_hybrid_attn:
            ...
            if self.num_decode_reqs > 0:
                num_padded_scheduled_tokens[: self.num_decode_reqs] = (
                    num_padded_scheduled_tokens[: self.num_decode_reqs] // self.pcp_size
                )
            ...
            if self.num_reqs - self.num_decode_reqs > 0:
                ...
                num_prefill_tokens_allranks = (
                    self._get_cp_local_seq_lens(prefill_tokens_tensor, self.pcp_size, 1, 1).long().numpy()
                )
                # [3] [2]  |  [2,2] [2,1] [2,1] [1,1]
                ...
                num_prefill_tokens_start_loc = np.zeros(
                    (self.num_reqs - self.num_decode_reqs, self.pcp_size + 1), dtype=np.int64
                )
            ...
            if self.num_reqs > self.num_decode_reqs:
                all_positions_prefill = [
                    ...
                    - self.num_decode_tokens * self.pcp_size
                ...
                unpad_mask_prefill = self.pcp_unpad_mask_cpu[: self.pcp_padded_tokens_length][
                    self.num_decode_reqs * self.pcp_size :
                ]
                # [0] | [0,7]
            ...
            all_positions_lst = [
                get_current_rank_positions(padded_pos_start_loc, rank_i) for rank_i in range(self.pcp_size)
            ]
    ...
    def get_padded_slot_mapping(self, num_tokens: int, num_tokens_padded: int, slot_mapping: torch.Tensor):
        ...
        pcp_padded_slot_mapping = (
            self.pcp_padded_slot_mapping[: num_tokens_padded * self.pcp_size]
            if not self.pcp_use_hybrid_attn
            else self.pcp_padded_slot_mapping[: num_tokens * self.pcp_size]
        )
        cp_unpad_mask = self.pcp_unpad_mask_cpu_tensor[: num_tokens * self.pcp_size]
        pcp_padded_slot_mapping.fill_(-1)
        pcp_padded_slot_mapping[: num_tokens * self.pcp_size][cp_unpad_mask] = slot_mapping
        if self.pcp_use_hybrid_attn:
    ...
    def get_restore_hidden_states(
        ...
        if not self.pcp_use_hybrid_attn:
            hidden_states = get_pcp_group().all_gather(
                hidden_states[: self.num_actual_tokens_pcp_padded // self.pcp_size],
                0,
            )
    ...
    def _get_cp_local_seq_lens(
        ...
        total_size = pcp_size * dcp_size
        seq_lens_tiled = seq_lens.unsqueeze(-1).repeat(1, total_size)
        rank_offsets = torch.arange(total_size, dtype=torch.int32).unsqueeze(0).repeat(num_requests, 1)
        base = seq_lens_tiled // cp_kv_cache_interleave_size // total_size * cp_kv_cache_interleave_size
        remainder = seq_lens_tiled - base * total_size
        remainder = torch.clip(
            remainder - rank_offsets * cp_kv_cache_interleave_size,
            0,
            cp_kv_cache_interleave_size,
        )
        dcp_local_seq_lens = (base + remainder).reshape([-1, pcp_size, dcp_size])
    ...
    def generate_pcp_metadata(
        ...
        if self.pcp_size * self.dcp_size > 1:
            ...
            num_computed_tokens_of_pcp_dcp = torch.zeros(
                [self.num_reqs * self.decode_threshold, self.pcp_size, self.dcp_size],
                dtype=torch.int32,
            )
        ...
        if self.pcp_size > 1 and self.num_prefill_reqs:
            prefill_query_lens = query_lens[self.num_decode_reqs : self.num_reqs].to(self.device)
            q_head_idx, q_tail_idx = get_pcp_query_indices(prefill_query_lens)
            (kv_with_q_head_nomask_idx, kv_with_q_head_mask_idx, kv_with_q_tail_nomask_idx, kv_with_q_tail_mask_idx) = (
                get_pcp_kv_indices(prefill_query_lens * self.pcp_size, self.pcp_rank, self.pcp_size)
            )
            chunk_seqlens = query_lens[self.num_decode_reqs : self.num_reqs] // 2
            ...
            if self.vllm_config.model_config.use_mla:
                long_seq_metadata.kv_with_q_head_nomask_idx_lst, kv_with_q_head_nomask_seqlens_lst = split_indices(
                    kv_with_q_head_nomask_idx, kv_with_q_head_nomask_seqlens
                )
                long_seq_metadata.kv_with_q_tail_nomask_idx_lst, kv_with_q_tail_nomask_seqlens_lst = split_indices(
                    kv_with_q_tail_nomask_idx, kv_with_q_tail_nomask_seqlens
                )
                long_seq_metadata.head_attn_nomask_seqlens_lst = [
                    torch.stack([chunk_seqlens, lens]) for lens in kv_with_q_head_nomask_seqlens_lst
                ]
                long_seq_metadata.tail_attn_nomask_seqlens_lst = [
                    torch.stack([chunk_seqlens, lens]) for lens in kv_with_q_tail_nomask_seqlens_lst
                ]
...
def get_pcp_part_indices(
    ...
    select_num_tokens = ends[-1] * (M - 1) // N  # 8
    ...
    # mask part
    # we need update select_len and starts at the same time
    select_len, starts = (ends - starts) // N, starts + select_len  # [1, 1, 2], [2,6,12] M=3, N=4
    select_num_tokens = ends[-1] // N  # 4
    ...
    local_offsets = torch.arange(select_num_tokens, device=device) - start_loc[seq_ids]  # [1,0,0,1]
...
def get_pcp_kv_indices(
    ...
    return (kv_head_nomask_indices, kv_head_mask_indices, kv_tail_nomask_indices, kv_tail_mask_indices)
...
def split_indices(
    ...
    seq_starts = torch.cat([torch.tensor([0]), seq_ends[:-1]])
    chunk_starts = torch.min(
        seq_starts.unsqueeze(0) + torch.arange(num_chunks, dtype=torch.int32).unsqueeze(1) * chunk_size, seq_ends
    )
    chunk_ends = torch.cat([chunk_starts[1:], seq_ends.unsqueeze(0)])
    chunk_lens = chunk_ends - chunk_starts
    # TODO(qcs): compare the performance between this impl and index_select
    chunked_indices = [
        torch.cat([indices[chunk_starts[cidx, ridx] : chunk_ends[cidx, ridx]] for ridx in range(chunk_starts.shape[1])])
