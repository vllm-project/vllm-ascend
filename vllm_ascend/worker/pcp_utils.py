...
class PCPManager:
    ...
    def get_logits_indices(self, cu_num_tokens: np.ndarray):
        return torch.from_numpy(cu_num_tokens) * self.pcp_size - self.num_pcp_pads_cpu_tensor[: self.num_reqs] - 1
    ...
    def get_padded_slot_mapping(self, num_tokens: int, num_tokens_padded: int, slot_mapping: torch.Tensor):
        ...
        pcp_padded_slot_mapping = self.pcp_padded_slot_mapping[: num_tokens_padded * self.pcp_size]
    ...
    def generate_pcp_metadata(self, total_num_scheduled_tokens, query_lens, input_batch, num_scheduled_tokens):
        ...
        decode_context_lens = (
            input_batch.num_computed_tokens_cpu[: self.num_decode_reqs] + num_scheduled_tokens[: self.num_decode_reqs]
        )
