...
from .quant_utils import quant_per_tensor, SRC_DTYPE_TO_ACL_DTYPE, TYPE_QUANT_QKV_ONLINE
        ...
        **optional_args
        ...
        if is_prefill: 
            if key_cache is not None:
                ...
                torch_npu._npu_reshape_and_cache(key_int8, value_int8, key_cache, value_cache, slots)
            if mask is None:
                raise ValueError("attn_metadata.attn_mask is Null. Please check.")
            if output is not None:
                output = output.view(query.shape)
                torch_npu._npu_flash_attention(query, key, value, mask, 
                                               torch.tensor(seq_lens_tensor_cpu, dtype=torch.int32),
                                               scale, layer.num_heads, layer.num_kv_heads, out=output)
            else:
                output = torch.empty_like(query, dtype=query.dtype).to(query.device)
                torch_npu._npu_flash_attention(query, key, value, mask, 
                                               torch.tensor(seq_lens_tensor_cpu, dtype=torch.int32),
                                               scale, layer.num_heads, layer.num_kv_heads, out=output)
            ...
            if output is not None:
                output = output.view(query.shape)
                torch_npu._npu_paged_attention_quant(query_int8, key_cache, value_cache, layer.num_kv_heads,
                        layer.num_heads, scale, block_tables, torch.tensor(seq_lens_tensor_cpu, dtype=torch.int32), 
