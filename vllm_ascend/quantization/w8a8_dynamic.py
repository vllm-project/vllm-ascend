...
def process_topk_ids(topk_ids: torch.Tensor, expert_num: int, ep_size: int,
    ...
    start_offset_for_each_token = torch.cummax(temp_start_markers.float(),
                                               dim=0)[0].to(
                                                   temp_start_markers.dtype)
...
def fused_experts_with_all2all(
    ...
    if expert_map is not None:
        ...
        local_buffer_rows = (num_tokens // ep_group.world_size +
                             1) * ep_group.world_size * top_k * 2
