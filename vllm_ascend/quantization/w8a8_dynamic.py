...
def fused_experts_with_allgather(hidden_states: torch.Tensor,
    ...
    sorted_topk_weight = torch.empty_like(expanded_x_idx,
                                          dtype=topk_weights.dtype,
                                          device=topk_weights.device)
    sorted_topk_weight[:valid_token_num] = torch.index_select(
        topk_weights.view(-1), 0, expanded_x_idx[:valid_token_num])
