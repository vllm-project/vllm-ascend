...
class AscendW4A8MXFPDynamicLinearMethod(AscendLinearScheme):
    ...
    def apply(
        ...
        pertoken_scale = dynamic_scale.reshape(dynamic_scale.shape[0], -1) # W4A8MXFP scale是两维
        ...
        # output = patch_for_pseudo_quantization_precision(output) # 修复w4a8mxfp patch
...
class AscendW4A8MXFPDynamicFusedMoEMethod:
    ...
    def process_weights_after_loading(self, layer):
        layer.w13_weight.data = torch_npu.npu_format_cast(layer.w13_weight.data.to(torch.float32), 29, torch.float8_e4m3fn) # w4a8mxfp只支持nz格式
        layer.w13_weight.data = torch_npu.npu_convert_weight_to_int4pack(layer.w13_weight.data)
        layer.w2_weight.data = torch_npu.npu_format_cast(layer.w2_weight.data.to(torch.float32), 29, torch.float8_e4m3fn)
