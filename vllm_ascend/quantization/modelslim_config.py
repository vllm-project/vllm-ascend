...
packed_modules_model_mapping: dict[str, dict[str, list[str]]] = {
    ...
    # text_config.model_type for Gemma4ForConditionalGeneration language stack
    "gemma4_text": {
        "qkv_proj": [
            "q_proj",
            "k_proj",
...
_FUSED_MOE_EXPERTS_SHARDS = ["experts.0.gate_proj", "experts.0.up_proj", "experts.0.down_proj"]


def _is_gemma4_model_type(model_type: str | None) -> bool:
    return model_type in _GEMMA4_MODEL_TYPES


def _normalize_moe_experts_prefix(prefix: str) -> str:
    """Map vLLM FusedMoE path (*.moe.experts) to ModelSlim quant keys (*.experts)."""
    if ".moe.experts" in prefix:
        return prefix.replace(".moe.experts", ".experts")
    return prefix


def _resolve_modelslim_model_type(model_type: str | None) -> str | None:
...
def _lookup_shard_quant_value(
    quant_description: dict[str, Any],
    shard_prefix: str,
    shard_proj_name: str,
) -> str:
    """Look up quant type for a fused-module shard.

    Gemma4 ``attention_k_eq_v`` full-attention layers only quantize ``k_proj``;
    vLLM still exposes a ``v_proj`` QKV slot that reuses ``k_proj`` weights.
    """
    weight_key = f"{shard_prefix}.weight"
    if weight_key in quant_description:
        return quant_description[weight_key]
    if shard_proj_name == "v_proj":
        k_weight_key = weight_key.replace(".v_proj.", ".k_proj.").replace(
            ".v_proj.weight", ".k_proj.weight"
        )
        if k_weight_key in quant_description:
            return quant_description[k_weight_key]
    raise KeyError(
        f"Missing quant key '{weight_key}' for shard '{shard_prefix}'. "
        f"shard_proj_name={shard_proj_name}, "
        f"v_proj_fallback_key={weight_key.replace('.v_proj.', '.k_proj.')}, "
        f"sample_matching_keys={[k for k in quant_description if shard_prefix.rsplit('.', 1)[0] in k][:5]}"
    )
...
def get_linear_quant_type(
    ...
    """
    prefix = _normalize_moe_experts_prefix(prefix)
    proj_name = prefix.split(".")[-1]
    expert_shards = packed_modules_mapping.get(proj_name)
    if expert_shards is None and proj_name == "experts":
        expert_shards = _FUSED_MOE_EXPERTS_SHARDS
    if expert_shards is not None:
        quant_type = None
        shard_prefixes = [
            prefix.replace(proj_name, shard_proj_name) for shard_proj_name in expert_shards
        ]
        for shard_prefix, shard_proj_name in zip(
            shard_prefixes, expert_shards, strict=False
        ):
            if proj_name == "experts":
                weight_key = shard_prefix + ".weight"
                if weight_key not in quant_description:
                    sample_keys = [
                        k for k in quant_description if "experts" in k and k.endswith(".weight")
                    ][:5]
                    raise KeyError(
                        f"Missing quant key '{weight_key}' for MoE experts layer '{prefix}'. "
                        f"proj_name={proj_name}, expert_shards={expert_shards}, "
                        f"sample_expert_keys={sample_keys}"
                    )
                shard_quant_type = quant_description[weight_key]
            else:
                shard_quant_type = _lookup_shard_quant_value(
                    quant_description, shard_prefix, shard_proj_name
                )
        ...
        if weight_key not in quant_description:
            sample_keys = [k for k in quant_description if prefix in k][:5]
            raise KeyError(
                f"Missing quant key '{weight_key}' for layer '{prefix}'. "
                f"proj_name={proj_name}, packed_modules_keys={list(packed_modules_mapping.keys())}, "
                f"sample_matching_keys={sample_keys}"
            )
...
class AscendModelSlimConfig(QuantizationConfig):
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str, tid2eid=None) -> Optional["QuantizeMethodBase"]:
        ...
        # Multimodal Gemma4 uses text_config.model_type == "gemma4_text"
        text_config = getattr(hf_config, "text_config", None)
        text_model_type = getattr(text_config, "model_type", None) if text_config is not None else None
        ...
        modelslim_model_type = _resolve_modelslim_model_type(model_type)
        if modelslim_model_type is None and _is_gemma4_model_type(text_model_type):
            modelslim_model_type = "gemma4"
        if modelslim_model_type in packed_modules_model_mapping:
            self.packed_modules_mapping = dict(packed_modules_model_mapping[modelslim_model_type])
        elif isinstance(layer, FusedMoE) and (
            _is_gemma4_model_type(model_type) or _is_gemma4_model_type(text_model_type)
        ):
            self.packed_modules_mapping = dict(packed_modules_model_mapping["gemma4"])
        prefix = self.quant_prefix_mapper(model_type, prefix)
        if _is_gemma4_model_type(model_type) or _is_gemma4_model_type(text_model_type):
            prefix = _normalize_moe_experts_prefix(prefix)
        elif isinstance(layer, FusedMoE):
            prefix = _normalize_moe_experts_prefix(prefix)

        if isinstance(layer, FusedMoE) and ".moe.experts" in prefix:
            logger.warning(
                "ModelSlim MoE prefix still contains '.moe.experts' after normalization: "
                "model_type=%s, text_model_type=%s, prefix=%s, packed_modules_keys=%s",
                model_type,
                text_model_type,
                prefix,
                list((self.packed_modules_mapping or {}).keys()),
            )
    ...
    def is_layer_skipped_ascend(self, prefix: str, fused_mapping: Mapping[str, list[str]] = MappingProxyType({})):
        ...
        if proj_name in fused_mapping:
            ...
            is_skipped = None
            for shard_prefix, shard_proj_name in zip(
                shard_prefixes, fused_mapping[proj_name], strict=False
            ):
                is_shard_skipped = _lookup_shard_quant_value(
                    self.quant_description, shard_prefix, shard_proj_name
                ) == "FLOAT"
