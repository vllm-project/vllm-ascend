...
# Devices that do not reserve IRQ-dedicated CPUs (e.g. Ascend 950 / A5). For
# them the IRQ pool is folded into the worker/main pool and the minimum CPU
# count per NPU is lower.
NO_IRQ_BINDING_DEVICE_TYPES = {AscendDeviceType.A5}
MIN_CPUS_PER_NPU_WITHOUT_IRQ = MIN_CPUS_PER_NPU - IRQ_CPUS_PER_NPU  # 1(main) + 1(acl) + 1(release)
...
class CpuAlloc:
    def __init__(self, rank_id: int, reserve_memcache: bool = False):
        self.rank_id = rank_id
        # When False (default), the per-NPU memcache CPU pool is not carved out and
        # those CPUs flow to the worker/main pool. The memcache backend opts in via
        # get_memcache_client_cpus() so its client threads get a dedicated CPU set.
        self.reserve_memcache: bool = reserve_memcache
    ...
    def _reserve_irq_cpus() -> bool:
        # Ascend 950 (A5) does not require IRQ-dedicated CPUs.
        return get_ascend_device_type() not in NO_IRQ_BINDING_DEVICE_TYPES

    @staticmethod
    def _min_cpus_per_npu() -> int:
        return MIN_CPUS_PER_NPU if CpuAlloc._reserve_irq_cpus() else MIN_CPUS_PER_NPU_WITHOUT_IRQ

    def allocate(self) -> None:
        # The memcache pool is only carved out for the memcache backend path; on
        # other deployments those CPUs flow to the worker/main pool instead.
        memcache_cpu_count = self.memcache_cpu_count()
        if not self.reserve_memcache:
            freed_memcache_cpus = memcache_cpu_count
            memcache_cpu_count = 0
        else:
            freed_memcache_cpus = 0
        worker_cpu_count = self.worker_cpu_count()
        reserve_irq_cpus = CpuAlloc._reserve_irq_cpus()
        min_cpus_per_npu = CpuAlloc._min_cpus_per_npu()
        for npu, pool in self.npu_cpu_pool.items():
            ...
            irq_end = min(IRQ_CPUS_PER_NPU, len(pool)) if reserve_irq_cpus else 0
            ...
            # Absorb the CPUs that would have been memcache into the worker pool.
            effective_worker_cpu_count = worker_cpu_count + freed_memcache_cpus
            main_count = min(effective_worker_cpu_count, max_main_count)
            ...
            if len(main) < effective_worker_cpu_count:
                logger.warning(
                    "NPU%d worker CPU count is %d, less than requested %d.",
                    npu,
                    len(main),
                    effective_worker_cpu_count,
                )
...
def get_memcache_client_cpus(rank_id: int) -> list[int]:
    if not is_arm_cpu():
        return []
    binder = CpuAlloc(rank_id, reserve_memcache=True)
