...
class CpuAlloc:
    ...
    def build_cpu_pools(self) -> None:
        ...
        for npu in self.device_info.running_npu_list:
            base_cpu_list = [cpu for cpu in self.device_info.npu_affinity.get(npu, []) if cpu in self.device_info.allowed_cpus]
    ...
    def allocate(self) -> None:
        for npu, pool in self.npu_cpu_pool.items():
            if len(pool) >= 3:
                ...
                raise RuntimeError("The number of CPUs is insufficient to bind to the NPUs. Each NPU requires at least 3 CPUs.")
