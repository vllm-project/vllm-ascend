...
class RequestDfxStore:
    ...
    def get_prompt_token_ids(self, req_id: str) -> list[int] | None:
        state = self.get_state(req_id)
        return list(state.prompt_token_ids) if state is not None and state.prompt_token_ids is not None else None
    ...
    def set_filter_allowed(self, req_id: str, allowed: bool) -> None:
        ...
        with self._lock:
            ...
            if state is None:
                state = RequestDfxState(req_id=str(req_id))
                self._by_req[str(req_id)] = state
            state.filter_allowed = bool(allowed)

    def clear_filter_allowed(self, req_id: str) -> None:
        # S15 fix: lock-protected mutation (was via get_state which drops lock).
        if not req_id:
            return
        with self._lock:
            state = self._by_req.get(str(req_id))
    ...
    def clear_sample_waves(self, req_id: str) -> None:
        # S15 fix: lock-protected — was via get_state (drops lock before .clear()),
        # racing take_sample_wave's popleft.
        if not req_id:
            return
        with self._lock:
            state = self._by_req.get(str(req_id))
