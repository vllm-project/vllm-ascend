...
_DEFAULTS: dict[str, Any] = {
    ...
    "dump": {
        ...
        # Needs dfx_config_reload_interval>0. Skips auto quota/cooldown/filters.
        "manual_dump": False,
...
class DfxRuntimeConfig:
    ...
    def _read_msprobe_dump_enable(path: str | None) -> tuple[bool, bool]:
        ...
        if "dump_enable" not in data:
            return (True, False)
        return (DfxRuntimeConfig._parse_msprobe_dump_enable_raw(data.get("dump_enable")), True)
    ...
    def _validate_dump_mutual_exclusive(dump: dict[str, Any]) -> None:
        auto_on = DfxRuntimeConfig._auto_on_from_dump(dump)
        manual_on = DfxRuntimeConfig._manual_dump_active(dump.get("manual_dump", False))
    ...
    def consume_manual_trigger(self) -> bool:
        ...
        def _derive(data: dict[str, Any]) -> dict[str, Any]:
            ...
            cur_raw = dump.get("manual_dump", remaining)
    ...
    def _apply_loaded(
        ...
        *,
        ...
        size: int | None = None,  # backward-compat unused; digest is authoritative
        announce: bool = True,
    ) -> bool:
        ...
        if content_digest is not None:
            ...
            # Legacy callers passed size only; refresh digest from disk if possible.
    ...
    def _validate(data: dict[str, Any]) -> None:
        ...
        if isinstance(auto_max_times, bool) or not isinstance(auto_max_times, (int, float)):
            raise ValueError("dump.auto_max_times must be a non-negative int")
        if int(auto_max_times) < 0:
            raise ValueError("dump.auto_max_times must be >= 0")
        data["dump"]["auto_max_times"] = int(auto_max_times)
        auto_cd = data["dump"].get("auto_cooldown_seconds", 300)
        if isinstance(auto_cd, bool) or not isinstance(auto_cd, (int, float)):
            raise ValueError("dump.auto_cooldown_seconds must be a non-negative int")
        if int(auto_cd) < 0:
            raise ValueError("dump.auto_cooldown_seconds must be >= 0")
        data["dump"]["auto_cooldown_seconds"] = int(auto_cd)
        ...
        DfxRuntimeConfig._validate_dump_mutual_exclusive(data["dump"])
