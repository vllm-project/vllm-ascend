...
The FLA tests compare NPU outputs and mutated state against independent PyTorch
golden implementations. End-to-end model tests are still required for each
graph, connector, and distributed configuration claimed as supported.

Representative NPU commands are:

```bash
pytest -q tests/fla_ops/test_causal_conv1d.py
pytest -q tests/fla_ops/test_recurrent_gated_delta_rule.py
pytest -q tests/ut/ops/test_mamba_state_index.py
