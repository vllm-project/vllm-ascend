...
This tutorial uses the vLLM-Ascend `v0.11.0rc2` version for demonstration, showcasing the `Qwen3-VL-235B-A22B` model as an example for multi-NPU deployment.
...
- `Qwen3-VL-235B-A22B`(BF16 version): require 1 Atlas 800 A3 (64G × 16) node，2 Atlas 800 A2（64G * 8）nodes. [Download model weight](https://modelscope.cn/models/Qwen/Qwen3-VL-235B-A22B-Instruct/)
...
vllm serve vllm-ascend/Qwen3-VL-235B-A22B \
--host 0.0.0.0 \
--port 8000 \
...
2. After execution, you can get the result, here is the result of `Qwen3-VL-235B-A22B` in `vllm-ascend:0.11.0rc2` for reference only.
...
Run performance evaluation of `Qwen3-VL-235B-A22B` as an example.
...
```shell
export VLLM_USE_MODELSCOPE=true
vllm bench serve --model vllm-ascend/Qwen3-VL-235B-A22B  --dataset-name random --random-input 200 --num-prompt 200 --request-rate 1 --save-result --result-dir ./
```
