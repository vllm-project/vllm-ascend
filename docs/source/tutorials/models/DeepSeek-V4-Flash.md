...
For ultra-long sequence scenarios, support can be achieved by adjusting the PD (Prefill/Decode) ratio and the model parallelism strategy. For example, in a 1M sequence scenario, a 4\*1P-1\*4D ratio can be used, Prefill node，with the model parallelism set to DP2TP4 mode，and Decode node,with the model parallelism set to DP32TP1 mode.
