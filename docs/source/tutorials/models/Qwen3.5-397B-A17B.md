...
- Issue2: There are still some accuracy problem with PD disaggregation feature. One recommend scenario is turning off MC2 fused op for both P and D node and turning on the 'recompute schedule' for noly D node, but this configuration will not achieve optimal performance.We are continuously working hard to resolve the accuracy issues related to PD separation.Relevant issue tracking link:[Issue8421](https://github.com/vllm-project/vllm-ascend/issues/8421)
