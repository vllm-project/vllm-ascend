...
Single-node deployment completes both Prefill and Decode within the same node. Both the bfloat and quantized model can be deployed on 1 Atlas 800 A3 (64GB × 16). W8A8 quantized model can be deployed on 1 Atlas 800 A2 (64GB × 8). MXFP8 quantized model can be deployed on 1 Ascend 950DT (96GB × 8).
...
- `--max-num-batched-tokens 1024` (950DT Prefill) vs `32768` (A3 Prefill): Keep the verified Prefill batching; raise it only after re-checking HBM.
