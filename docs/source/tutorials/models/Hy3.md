...
Hy3 is a Mixture-of-Experts model, with 295B total parameters, 21B active parameters and 3.8B MTP layer parameters, developed by the Tencent Hy Team.. Following the release of Hy3 Preview in late April, we gathered feedback from over 50 products and conducted post-training extensions with higher-quality data. Today, we are officially launching Hy3, which outperforms models of comparable scale and matches the performance of flagship open-source models with 2–5× the parameter count. At the same time, the model demonstrates significant practical improvements across a wide range of products and productivity tasks.

This guide records the verified vLLM Ascend serving path for Hy3 on one Atlas A3 16-NPU node. The verified default path is TP8 + EP + MTP + ACLGraph.
...
- ModelScope: [Tencent-Hunyuan/Hy3](https://www.modelscope.cn/models/Tencent-Hunyuan/Hy3)
- GitCode: [tencent_hunyuan/Hy3](https://ai.gitcode.com/tencent_hunyuan/Hy3)

Download or mount the checkpoint to a path shared by the runtime container, for example `/models/Hy3`.
...
You can use our official docker image to run Hy3 directly. For Atlas A3 machines, select the image variant with the `-a3` suffix. The official image already includes the vLLM and vLLM Ascend runtime needed for the verified serving path.
...
**Deploying the Hy3 model with vllm-ascend requires these three PRs.**  
https://github.com/vllm-project/vllm/pull/47192  
https://github.com/vllm-project/vllm/pull/47813  
https://github.com/vllm-project/vllm-ascend/pull/11527  
