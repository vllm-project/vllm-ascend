...
This document describes the main validation steps for Gemma4 on Ascend 950 Products, including supported features, prerequisites, installation, single-node online deployment, functional verification, offline inference, accuracy and performance evaluation, performance tuning, and FAQs.

This document is written based on the latest vLLM Ascend main branch. Gemma4 is supported on Ascend 950 Products in this version.
...
Gemma4 supports both eager execution and ACLGraph execution on Ascend 950 Products. For graph execution, `FULL_DECODE_ONLY` can reduce decode-phase dispatch overhead, while `PIECEWISE` is also supported.
...
| ---------- | ----------- | -------------------- |
| Gemma4 dense model | Dense Gemma4 weight. | A single Ascend 950 node. Adjust the number of visible NPUs according to model size. |
| Gemma4 MoE model | Mixture-of-Experts Gemma4 weight. | A single Ascend 950 node. Use tensor parallelism or expert parallelism according to the model size and deployment plan. |

The examples below assume a single Ascend 950 node with 8 visible NPUs. Adjust `--tensor-parallel-size` according to the model size and available devices.
...
The public vLLM Ascend image for Ascend 950 Products has not been released yet. If you need a prebuilt image, contact Huawei engineers to obtain the appropriate image and software stack.
...
Expected result: `npu-smi info` lists the expected Ascend 950 devices.

### 4.2 Source Code Installation

Prepare the source build dependencies according to [set up using Python](../../installation.md#set-up-using-python), including the CANN environment, build tools, and Python packages required by vLLM Ascend. Then install vLLM Ascend from the main branch:
...
Single-node deployment runs both Prefill and Decode on the same node, and is suitable for functional verification and single-node serving. The following examples show eager and ACLGraph startup commands.
...
This document focuses on single-node online deployment for Gemma4 on Ascend 950 Products. Multi-node Prefill-Decode separation deployment is not covered in this tutorial.
...
Using the GPQA-Diamond dataset as an example test dataset, run the accuracy evaluation for the selected Gemma4 dense or MoE model in online mode.
...
Run performance evaluation for the selected Gemma4 dense or MoE model as an example. Refer to [vLLM benchmark](https://docs.vllm.ai/en/latest/benchmarking/) for more details.
...
| -------- | --------------- | ---------- | ---------- | ------------------ |
| Functional verification | Single-node online serving | 8 | Dense or MoE | Use eager mode first to verify model loading and basic generation. |
| Generation serving | Single-node online serving | 8 | Dense or MoE | Use ACLGraph with `FULL_DECODE_ONLY` to reduce decode dispatch overhead. |
| Long context | Single-node online serving | 8 | Dense or MoE | Increase `--max-model-len` only when enough KV cache is available; lower concurrency if OOM occurs. |
| MoE serving | Single-node online serving | 8 | MoE | Use tensor parallelism or expert parallelism according to model size and deployment plan. |

| Scenario | NPUs | TP | Max Num Seqs | Max Model Len | Max Num Batched Tokens | ACLGraph Mode | Notes |
| -------- | ---- | -- | ------------ | ------------- | ---------------------- | ------------- | ----- |
| Functional verification | 8 | 8 | Tune by memory | 32768 | Tune by workload | Eager or disabled | Start from eager mode to establish a baseline. |
| Generation serving | 8 | 8 | Tune by concurrency | 32768 | Tune by workload | `FULL_DECODE_ONLY` | Recommended graph mode when decode tokens are generated. |
| Long context | 8 | 8 | Lower than short-context serving | Adjust by model context | Tune by memory | `FULL_DECODE_ONLY` or `PIECEWISE` | Long context increases KV cache usage. |
| MoE serving | 8 | Tune by model size | Tune by concurrency | Adjust by model context | Tune by memory | `FULL_DECODE_ONLY` | Enable EP only when the deployment plan uses expert parallelism. |
