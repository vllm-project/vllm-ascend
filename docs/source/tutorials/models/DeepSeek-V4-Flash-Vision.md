...
deployment on one Atlas 800 A3 server. Currently, only colocated deployment is
supported: Prefill and Decode run in the same service. Do not use
...
```text
quay.io/ascend/vllm-ascend:deepseekv4-flash-vision-exp-a3
```
...
`.github/vllm-main-verified.commit`. Use `Dockerfile.a3` for the A3 build.
