...
```{note}
The following dependency adjustments are **manual steps** required only in environments where the official vllm-ascend Docker image has not yet pinned these versions. They are not executed automatically by CI.
```
