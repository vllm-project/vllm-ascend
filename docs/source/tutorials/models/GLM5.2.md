...
2. **Expert Parallel communication configuration**
    ...
    - If the EP communication domain spans physical nodes, Fused MC2 is not supported. Optimized MC2 communication is required (hierarchy for dispatch and fullmesh_v2 for combine) and must be enabled by adding `"enable_mc2_optimized_comm": true` to `--additional-config`.
...
```shell
--additional-config '{..., "enable_fused_mc2": 0, "enable_mc2_optimized_comm": true}'
```

Merge `"enable_mc2_optimized_comm": true` into the existing `--additional-config` JSON object. The following pattern uses `...` to represent the existing fields:

```text
--additional-config '{..., "enable_mc2_optimized_comm": true}'
```
