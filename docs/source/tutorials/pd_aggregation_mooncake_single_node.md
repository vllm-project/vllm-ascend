# pd_aggregation_mooncake_single_node.md（Qwen2.5-72B）



##  **Getting Started**

vLLM-Ascend now supports PD aggregation deployment with Mooncake features. This guide provides step-by-step instructions to verify these features with constrained resources.

Using the Qwen2.5-72B-Instruct model as an example, this guide demonstrates how to use vllm-ascend v0.11.0rc2 (with vLLM v0.11.0) on one Atlas 800T A2 server to deploy two vllm service instances in one docker container. Each service instance occupies 4 NPU cards, and both use a PD aggregation deployment architecture.
...
vllm serve <path_to_your_model>/Qwen2.5-72B-Instruct/  \	#Please replace with your own model weight path.
        --served-model-name qwen \
        --dtype bfloat16 \
        --max-model-len 25600 \
        --tensor-parallel-size 4 \
        --host <your_server_ip> \	#Please replace with your own server_ip.
        --port 8002 \
        --enforce-eager \
...
We recommend using the aisbench tool to assess performance. The test dataset A is as follows: input/output 1024/10, a total of 100 data entries, 25 concurrency. The dataset contains completely random characters. The test steps are as follows:
...
Test step 3: Send dataset A to the service of instance 2, and the TTFT is 867ms (cross-instance hit kvcache);
