...
- `--additional-config '{"enable_flashcomm1": true}'` indicates that Flashcomm1 optimization is enabled. Currently, this optimization is only supported for MoE in scenarios where tp_size > 1.
