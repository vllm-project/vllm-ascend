...
### 27. How do I update the Kimi K3 causal convolution registration?

Kimi K3 uses `cann_ops_transformer.causal_conv1d_fn` for prefill and
`cann_ops_transformer.causal_conv1d_update` for decode. The separate custom
implementation used by GDN is registered as `VllmCausalConv1d`, with the ACLNN
entry point `aclnnVllmCausalConv1d`. Its Python name remains
`torch.ops._C_ascend.npu_causal_conv1d_custom`.

Older builds registered the custom implementation as `CausalConv1d`, the same
name used by CANN's implementation, despite having a different operator
definition. A stale custom bundle can shadow the official registration and
cause an OpDef/bin mismatch such as `Cannot find any bin for output 1` during
prefill. The error text alone does not identify which vendor was loaded.

After updating the source, stop the service and its Ray workers on every node,
then rebuild in the same Python and CANN environment used by the service:

```bash
COMPILE_CUSTOM_KERNELS=1 python3 -m pip install -e . --no-build-isolation --no-deps
```

Keep the node's actual `SOC_VERSION` if it is configured; otherwise the build
detects the chip with `npu-smi`. This command updates the current vLLM-Ascend
installation and rebuilds both its custom OPP package and C++ extension.
`--no-deps` prevents dependency upgrades. A source-only update or a build with
`COMPILE_CUSTOM_KERNELS=0` cannot apply the registration change.

The build regenerates its custom-op artifacts and replaces the repo-local
`vllm_ascend/_cann_ops_custom` installation. Startup checks that this bundle no
longer registers the old `CausalConv1d` name and reports a rebuild instruction
if stale metadata remains. Restart all service and Ray worker processes after
installation, since they cache operator libraries and function pointers.

Before loading the model, run the small NPU coexistence test in that environment:
...
It checks the custom kernel and official prefill/decode kernels in fresh
processes, including output values and cache updates for FP16 and BF16.
If the official call still fails after a clean rebuild, inspect other vendor
directories in `ASCEND_CUSTOM_OPP_PATH` and `ASCEND_OPP_PATH/vendors/config.ini`
for old copied registrations, and verify the official CANN operator package.
The repo-local rebuild does not update those external installations.
