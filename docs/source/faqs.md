...
Enable FlashComm_v1 (`--additional-config '{"enable_flashcomm1":true}'`) when using Tensor Parallelism (TP ≥ 2) with high concurrency. It is threshold-protected and will not activate in low-concurrency scenarios where it could degrade performance.
