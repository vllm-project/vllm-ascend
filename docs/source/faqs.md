...
It's usually because the model is too large to fit in a single NPU, and you can refer to [vLLM's documentation](https://docs.vllm.ai/en/latest/getting_started/troubleshooting.html#out-of-memory).

And when your NPU's HBM memory size is small, it can also lead to out of memory due to memory fragments generated by dynamic apply and release memory during the reasoning process. We can handle this situation through the following means.

- **set `--gpu-memory-utilization`**: We can set this param to reserve more memory to cope with memory fragments. Here is the [param description](https://docs.vllm.ai/en/latest/serving/engine_args.html#vllm.engine.arg_utils-_engine_args_parser-cacheconfig).

- **set environment variable `PYTORCH_NPU_ALLOC_CONF`**: this env can control the behavior of cache allocator, such as let the memory allocator actively recycle unused NPU memory blocks. This will reduce the memory size occupied by memory fragments. Find more details [here](https://www.hiascend.com/document/detail/zh/Pytorch/700/comref/Envvariables/Envir_012.html) with this env.
