...
- [Experimental] Full decode only graph mode is supported now. Although it is not enabled by default, we suggest enabling it by `--compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}'` in most cases. Let us know if you hit any error. We'll keep improving it and enable it by default in the next few releases.
- Many Triton kernels have been added. The performance of vLLM Ascend, especially for Qwen3-Next and DeepSeek 3.2, has been improved. Please note that Triton is not installed and enabled by default, but we suggest enabling it in most cases. You can download and install it manually from the [package url](https://vllm-ascend.obs.cn-north-4.myhuaweicloud.com/vllm-ascend/triton_ascend-3.2.0.dev2025110717-cp311-cp311-manylinux_2_27_aarch64.whl). If you're running vLLM Ascend on x86, you need to build Triton Ascend from [source](https://gitcode.com/Ascend/triton-ascend).
...
- speculative decode method `MTP` is more stable now. It can be enabled in most cases and decode token number can be 1,2,or 3.
...
- We notice that bf16/fp16 models don't perform well. This is mainly because `VLLM_ASCEND_ENABLE_NZ` is enabled by default. Please set `VLLM_ASCEND_ENABLE_NZ=0` to disable it. We'll add the auto detection mechanism in the next release.
...
- For long sequence input cases, there is no response sometimes and the kv cache usage becomes higher. This is a scheduler bug. We are working on it.
