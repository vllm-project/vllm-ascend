...
For sliding-window DSpark drafts, the worker applies upstream's full-cache allocation conversion before returning cache specifications. KVPP budgeting and upstream cache grouping use those same specifications. This retains sliding-window attention computation, but allocates draft cache blocks for the full sequence rather than evicting blocks outside the window.
