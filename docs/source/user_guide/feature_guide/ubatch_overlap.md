...
NPU streams. The lock-step handoff is inserted exactly around each MoE layer's
collective communication (`prepare()`/`finalize()` or
`token_dispatch()`/`token_combine()`), so while one ubatch's stream runs MoE
collective communication (all-gather / reduce-scatter / all-to-all / MC2), the
peer ubatch's stream runs the MoE expert FFN (grouped matmul) that is sandwiched
between the two comm blocks. This hides the MoE communication latency behind
expert compute for large prefill batches, which is the dominant cost for
sparse-MoE models such as MiniMax-M2 on Ascend NPUs.

Per-MoE-layer timeline under overlap (the handoff lives inside the two
`comm_section` blocks — everything else in each ubatch runs sequentially):

```
           ┌── comm_section ──┐                    ┌── comm_section ──┐
ubatch A:  │ prepare/dispatch │ ── expert FFN ───► │ finalize/combine │
           │ (allgather/a2a)  │   (grouped matmul) │ (rs/a2a-combine) │
           └──────────────────┘                    └──────────────────┘
                                 ▲ overlapped ▲
                                 │            │
ubatch B:  ──── expert FFN ────► │ prepare/.. │ ──── expert FFN ────► ...
                                 (grouped matmul runs on AI cores while
                                  A's comm runs on the HCCS engines)
```

What gets hidden is the MoE collective-communication time; what fills it is the
peer ubatch's MoE expert FFN (grouped matmul). Layers outside MoE (attention,
dense projections) are not part of the overlap.
...
batch runs on one stream, the MoE expert FFN (grouped matmul) and the MoE
collective communication are serialized — the AI cores sit idle during
communication while the communication engines (HCCS) sit idle during expert
compute.

UBatch overlap exploits the fact that **one ubatch's MoE collective
communication and the peer ubatch's MoE expert FFN (grouped matmul) touch
independent buffers and independent hardware engines**, so they can be overlapped
on different streams. The overlap is concentrated at MoE collective boundaries
(the two `comm_section` blocks per MoE layer); everything outside MoE still runs
sequentially within each ubatch. The net effect is a shorter critical path at the
cost of slightly higher scheduling overhead, which is only worth paying when the
...
4. In data-parallel mode, **all** DP ranks agree the step should be ubatched.
   ...
   `NPUModelRunner._sync_ubatch_across_dp` (the single 4-row collective that
   also carries the synced token count and cudagraph mode; the legacy
   `_sync_metadata_across_dp` delegates to it with `should_ubatch=False`).
...
│     └── _ubatch_blocked_reason() gate (DeepStack)
├── if should_ubatch:
...
- **Works for all MoE models, including DSA.** The compute/comm handoff is
  inlined into `AscendFusedMoE.forward_impl`, the shared MoE base layer, so any
  model that routes experts through `AscendFusedMoE` / `AscendSharedFusedMoE`
  gets ubatch overlap automatically. No per-model patch is needed.
  **DSA (Distributed Sparse Attention) models** (e.g. GLM-5, DeepSeek-V3.2)
  are supported via two ubatch-specific fixes rather than blocked: each ubid
  gets a private DSA metadata-cache dict set (so per-step result caches do not
  cross-contaminate across ubatches), and the DSA decode cos/sin are cloned
  per ubatch (so the global rotary buffer is not overwritten by a peer worker
  thread). The only model-level gate that remains is the **DeepStack
  cross-layer buffer**: `_ubatch_blocked_reason` disables ubatch for a model
  that uses DeepStack *and* whose class lacks the `_ubatch_deepstack_patched`
  marker set by `patch_qwen3vl` (logged via `logger.warning_once`).
  > **Note:** The v0.23.0 backport additionally blocked **PCP** (prefill
  > context parallelism) because the per-ubatch attention metadata splitter
  > did not carry PCP slot-mapping state. This `main-ubatch` port omits that
  > gate entirely because main-ubatch has no PCP support.
...
| qwen3-vl / qwen3-vl-moe | Yes | Yes | Yes (via `patch_qwen3vl`) |
| qwen3-omni-moe-thinker | Yes | Yes | Yes (via `patch_qwen3vl`) |
...
2. **DeepStack models** are handled by `patch_qwen3vl` (auto-registered via
   `patch/worker/__init__.py`), which combines TP chunking (flash_comm_v1) with
   ubatch-overlap thread safety for the cross-layer buffer accessors
   `_get/_clear_deepstack_input_embeds`. Each accessor reads/clears the
   per-ubatch `[start:stop]` slice bound by the thread-local `token_slice`
   (same mechanism as rotary embedding), and the upstream resize guard is
   preserved. A defensive gate (`_ubatch_blocked_reason` on `NPUModelRunner`)
   checks the `_ubatch_deepstack_patched` marker and disables ubatch with
   `logger.warning_once` if the patch is somehow absent.
