...
4. After enabling EAGLE, the main model needs to verify `(1 + K)` tokens generated by the main model and the draft model in one decoding process.
   ...
   so the the number of capture size needs to be set as `[N * (K + 1)]`. where `N` is the batch size.
   For instance, when `batch_size = 4` and `num_speculative_tokens = 4`, the cudagraph_capture_sizes should be set to `[5, 10, 15, 20]`.
