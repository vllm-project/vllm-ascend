...
- 区分 `moe_load1`（新策略）和 `moe_load`（旧策略）
- 支持 `pd_dynamic_delay` 参数控制动态延迟
- 公式：`all_moe_loads = all_moe_loads * pd_delay + all_moe_loads_local * (1 - pd_delay)`

## 新增配置参数

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `max_batch_token` | int | 128 | 最大批次 token 数 |
| `pd_dynamic_delay` | int | 0 | PD 动态延迟参数 |
