...
Requires vLLM main branch and vLLM-Ascend main branch, mooncake >= 0.3.11.post1, CANN >= 9.1.0, supported since the aforementioned versions.
...
| Memcache | Based on MemFabric, supports A3 HCCS high-speed interconnect, separated deployment, SSD Cache; supports `use_layerwise` per-layer KV access (only supported on the Prefill node). |
...
| :--- | :--- | :--- | :--- |
| Mooncake backend | Multi-tenant quota management needed; supports PD disaggregation, PD-Mixed deployment, and SSD Offload | A2/A3/Ascend 950 Products, ROCE/HCCS interconnect | All nodes must sync `PYTHONHASHSEED`; Store/PD traffic separation requires CANN >= 9.1.0; SSD Offload must align to 1 GB, `ssd_offload_path` must be an absolute path, and `MOONCAKE_OFFLOAD_TOTAL_SIZE_LIMIT_BYTES` must be set explicitly |
| Memcache backend | A3 HCCS high-speed interconnect or layerwise access needed; supports PD disaggregation, PD-Mixed deployment, and SSD Cache | A2/A3/Ascend 950 Products; `use_layerwise` only supported on Prefill node; SSD Cache requires `memcache_hybrid >= 1.2.0` | Separated deployment mode currently only supports A3 HCCS |
...
| :--- | :--- |
| Hardware | A2 (800I/800T A2): HDK >= 25.5 recommended, CANN >= 9.1.0, ROCE direct transfer needs `HCCL_INTRA_ROCE_ENABLE=1`. A3 (800I/800T A3): HDK >= 26.0 (or HDK >= 25.5 with mooncake >= v0.3.11), CANN >= 9.1.0, LingQu Computing Network >= 1.5. Ascend 950 Products (950PR/950DT): HDK >= 25.6 with mooncake >= v0.3.11, CANN >= 9.1.0, requires extra mounts `/dev/ummu`, `/dev/uburma`, `/usr/bin/urma_admin`, `/lib/route.conf`, `/etc/hccl_rootinfo.json`. Mooncake wheel requires glibc >= 2.35. |
...
| Feature Mutex | `use_layerwise` supports both Mooncake and Memcache backends, and is only supported on the Prefill node. Yuanrong Worker cannot configure both Coordinator and etcd discovery backends simultaneously. Under `P2P_TRANSFER` or FabricMem mode, client-side device memory pre-registration is always skipped regardless of `enable_dev_mem_pregister` value. |
| Software Dependencies | CANN >= 9.1.0; mooncake >= 0.3.11.post1 (non-default tenant requires >= 0.3.12); Memcache requires `memfabric-hybrid` and `memcache-hybrid` (SSD Cache needs `memcache_hybrid >= 1.2.0`); Yuanrong requires `openyuanrong-datasystem`. |
...
> Note: Memcache-specific prerequisite checks (memory scan, Ascend 950 Products signature verification disabling and container mounts, SSD disk status checks) are part of the scenario steps — see "Step 1: Prerequisite Checks" of [Scenario 2: Memcache Backend](#scenario-2-memcache-backend).
...
> Before proceeding, review the following Mooncake guides:
>
...
For A3 and Ascend 950 Products Store/PD traffic separation, set `ASCEND_GLOBAL_RESOURCE_CONFIG` on both the prefill and decode nodes and use **CANN >= 9.1.0**. The top-level resource configuration controls `MooncakeConnectorV1` PD traffic, and the `store` section controls `AscendStoreConnector` Mooncake Store traffic, so the two traffic classes do not compete on the same physical link.
...
Set `DATASYSTEM_CLIENT_LOG_DIR` before starting vLLM because the Yuanrong client reads it during logging initialization. Client SDK logs, whose base name is normally `ds_client`, are written to this directory. Use a directory separate from the worker logs.
...
| prefill_pp_size | int | 1 | Required when Prefill enables PP | Positive integer | Prefill PP size. |
...
| ssd_offload_path | str | No default | Required when enable_ssd_offload=true | Absolute path | Absolute path for SSD offload data storage (for example, /nvme/mooncake_offload). The directory must exist and be writable by the vLLM process; create it before startup (`mkdir -p <path>`). Relative paths, symlinks, and paths containing `..` are rejected. |
| tenant_id | str | default | No | String | Mooncake tenant namespace. Missing, `null`, empty, or whitespace-only values use `default`; surrounding whitespace is removed. All Prefill, Decode, scheduler, and replica instances that share KV entries must use the same tenant ID. Non-default tenants require Mooncake >= 0.3.12. |
...
| enable_offload | bool | false | Required when SSD offload is enabled | true / false | Set to true to enable SSD offload in master. |
...
| tenant_quota_connector_type | str | No default | Required when multi-tenant is enabled | file / etcd | Tenant quota connector type. |
| tenant_quota_connector_uri | str | No default | Required when multi-tenant is enabled | Quota file path or etcd endpoints | Tenant quota file path or etcd endpoints. |
...
| ock.mmc.local_service.protocol | str | host_rdma | Yes | host_rdma / host_urma / host_tcp / host_shm / device_sdma / device_rdma / device_urma / device_uboe | Communication protocol. A2 recommended: `device_rdma` (RoCE); A3 HCCS recommended: `device_sdma`; A5 UB: `device_urma`; A5 UBOE: `device_uboe`. `host_shm` requires DRAM > 0 and HBM = 0. |
...
| Variable | Type | Default | Required | Value Range | Description |
| :--- | :--- | :--- | :--- | :--- | :--- |
| ASCEND_GLOBAL_RESOURCE_CONFIG | str (JSON string) | No default | Required for Ascend 950 Products UBOE | JSON string | UBOE protocol descriptor, e.g., `{"comm_resource_config.protocol_desc":["uboe:device"]}`. |
| ASCEND_LOCAL_COMM_RES | str (JSON string) | No default | Required for Ascend 950 Products UB | JSON string | e.g., `{"version":"1.3"}`. |
| ASCEND_ENABLE_USE_FABRIC_MEM | int | No default | Recommended for A3 HCCS | 1 / 0 | Enables unified memory address direct transfer. Recommended for A3 series; with SSD offload, memory sizes must be aligned to 1 GB. |
| HCCL_INTRA_ROCE_ENABLE | int | No default | Required for A2 ROCE | 1 / 0 | Required for A2 series ROCE direct transfer. |
...
| 800 I/T A3 | >= 26.0 or >= 25.5 (with mooncake >= v0.3.11) | >= 9.1.0 | LingQu Computing Network >= 1.5; recommended `ASCEND_ENABLE_USE_FABRIC_MEM=1` |
...
Pre-operation checks (memory inspection, A3 available memory scanning, Ascend 950 Products signature verification disabling and container mounts, SSD disk status checks) are described in "Step 1: Prerequisite Checks" of [Scenario 2: Memcache Backend](#scenario-2-memcache-backend). For Memcache troubleshooting, refer to the [Memcache FAQ](https://gitcode.com/Ascend/memcache/wiki/FAQ.md).
