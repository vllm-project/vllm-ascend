...
| `layerwise_independent_layers` | `[0]` | Layers that keep dedicated buffers. Accepts an integer, an integer list, a comma-separated string, or `"all"`. Negative indices are resolved against all physical layers, including MTP layers. |
