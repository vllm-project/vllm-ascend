...
- the service uses the synchronous FCFS scheduler

Keep it disabled if the workload is mostly uniform, or if FCFS ordering is more important than short-request latency.

## Configuration

Add `short_request_first_config` to `scheduler_config`. It is supported by the synchronous scheduler in ordinary deployments, on PD-disaggregated prefill (P) nodes, and in PD-mixed deployments. It does not require `recompute_scheduler_enable`:
...
ShortRequestFirst changes only the waiting-queue policy. It requires the synchronous FCFS scheduler and is not supported with asynchronous scheduling, batch-job-aware scheduling, profiling-chunk scheduling, or on PD-disaggregated D nodes (`kv_role='kv_consumer'`).
