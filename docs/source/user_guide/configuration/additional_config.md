...
| `enable_sfa_prolog_v3`              | bool | `False` | Whether to enable the SFA PROLOG_V3 fused decode preprocessing (a single `npu_mla_prolog_v3` op replacing the per-layer NATIVE K chain) outside PD-disaggregated KV-consumer workers, i.e. in plain serving. Decode steps take the fused path while prefill keeps the NATIVE path. Requires a quantized `qkv_a_proj` (W8A8Dynamic together with `enable_sparse_sfa_c8`, or W8A8MXFP8). The original qkv_a/q_b weights are retained for the prefill fallback, so this trades extra NPU weight memory for decode kernel savings. |
