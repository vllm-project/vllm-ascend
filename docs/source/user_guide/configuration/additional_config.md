...
`rl_config` is a one-click RL mode switch. When `enabled` is `True`, it applies RL best-practice defaults automatically and each explicitly set sub-field overrides the corresponding top-level `additional_config` key or environment variable.

Priority: `rl_config` sub-fields > `additional_config` top-level keys > environment variables > code defaults.
...
| `enable_training_consistency` | bool | `false` | Both modes. Aligns the inference operators with the training operators to improve training-inference consistency. This is independent of batch consistency. The operator-alignment mechanism is provided by a follow-up. |
| `enable_batch_invariant` | bool | `false` | Both modes. Enables batch-invariant deterministic computation: sets `VLLM_BATCH_INVARIANT=1`, `HCCL_DETERMINISTIC=strict`, `LCCL_DETERMINISTIC=1`, and, in the worker, `torch.use_deterministic_algorithms(True)` plus batch-invariant Triton/AscendC operators. Requires building vllm-ascend from source with `COMPILE_CUSTOM_KERNELS=1`. Prevails over any previously set environment variables. |
| `enable_dev_endpoints` | bool | `true` | Online mode. Sets `VLLM_SERVER_DEV_MODE=1` to expose the `/sleep`, `/wake_up`, `/pause`, and `/resume` HTTP endpoints. No effect in offline `LLM()` mode. Set to `false` to opt out. |
...
| `export VLLM_BATCH_INVARIANT=1` + `export HCCL_DETERMINISTIC=strict` + `export LCCL_DETERMINISTIC=1` | `"rl_config": {"enable_batch_invariant": true}` |
| top-level `"weight_nz_mode": 0` | `"rl_config": {"weight_nz_mode": 0}` |
| top-level `"enable_sleep_mode_extra_cleanup": true` | `"rl_config": {"sleep_mode_extra_cleanup": true}` |
