...
| `ascend_gdn_decode_backend`        | str  | `None` | Selects the external Ascend GDN decode implementation. The only supported value is `"fla_npu"`. Replaces origin ops `torch.ops._C_ascend.npu_recurrent_gated_delta_rule`. |
