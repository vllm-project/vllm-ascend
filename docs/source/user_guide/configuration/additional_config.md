...
| `mega_moe_max_tokens`               | int  | `65536` | Global A5 MegaMoE input-token limit. Its per-rank symmetric-buffer capacity is `min(mega_moe_max_tokens // ep_size, execution_tokens_per_rank)`, where prefill producers use `max_num_batched_tokens` and decode or mixed roles use the graph/MC2 capacity. On A2/A3 this remains the reference per-rank receive capacity after dispatch for fused MC2/MegaMoE. |
