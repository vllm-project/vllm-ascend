...
| `mega_moe_max_tokens`               | int  | `65536` | Per-rank token capacity after dispatch for the `dispatch_ffn_combine` fused operator. It is passed as the operator's `max_output_size`. CANN MegaMoe uses its automatic receive-capacity calculation instead. |
