...
| `enable_dspark_draft_kv_optimistic_bound` | bool | `False` | Parallel drafting (DFlash / DSpark) only: let a draft build take its KV lengths from the host-side optimistic bound instead of reading them back from the device, and mask the rolled-back tail on the device. Removes a blocking device-to-host copy from every step. The bound overstates by at most `num_speculative_tokens`, and the mask restores the exact result, so outputs are unchanged (issue [#16271](https://github.com/vllm-project/vllm-ascend/issues/16271)). |
