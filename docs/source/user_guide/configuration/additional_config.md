...
| `enable_moe_lora_dual_stream`       | bool | `False` | Whether to overlap quantized MoE LoRA with base expert GMM on an auxiliary NPU stream. Currently limited to eager TP with the AllGather backend, without EP or fully sharded LoRA. The torch.compile/ACLGraph path falls back to the graph-safe single-stream implementation. |
