...
| `enable_fused_mc2`                  | int  | `0`     | Fused MC2 configuration. `0` disables the fused path and `1` enables it when the model and parallel configuration support it. On A5 standalone and prefill nodes, eligible W4A8 MXFP routed experts, including Kimi K3 SiTU MoE, use CANN MegaMoE; decode nodes retain the existing path. The legacy `VLLM_ASCEND_ENABLE_FUSED_MC2` environment variable is no longer supported. |
