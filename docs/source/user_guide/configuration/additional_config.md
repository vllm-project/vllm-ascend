...
| `enable_pivot_lightning_indexer`    | bool | `False` | Whether to use the experimental PIVOT LightningIndexer on Ascend 910B/910_93 with a BF16/FP16 indexer cache. The operator shares one proxy query per causal group of up to four rows after the 4096-token boundary and falls back to native LightningIndexer semantics for unsupported shapes. It cannot be combined with sparse LightningIndexer C8. |
| `pivot_lightning_indexer_backend`   | str | `"v1"` | Backend selected when `enable_pivot_lightning_indexer` is enabled. `"v1"` uses the standalone PivotLightningIndexer operator; `"v2"` calls LightningIndexerV2 and requires a CANN package that implements PIVOT reuse for that operator. |
