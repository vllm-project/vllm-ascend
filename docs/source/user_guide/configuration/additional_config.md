...
| `enable_fused_mc2`                  | int  | `0`     | Fused MC2 selection. `0` disables fused MC2; `1` allows the legacy `dispatch_ffn_combine` path when its capability and `mega_moe_max_tokens` capacity checks pass; `2` allows the `dispatch_gmm_combine_decode` path when its speculative/MTP, W8A8 dynamic quantization, and operator capacity checks pass. Can also be configured via the `VLLM_ASCEND_ENABLE_FUSED_MC2` environment variable during the migration period. See [MoE Communication Method Selection](../../developer_guide/Design_Documents/moe_comm_method.md). |
