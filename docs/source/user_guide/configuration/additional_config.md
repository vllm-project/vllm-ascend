...
| `mega_moe_max_tokens`               | int  | `65536` | Per-rank token capacity for the `dispatch_ffn_combine` operator. On A5, MegaMoE instead treats it as a global token ceiling and allocates one shared symmetric buffer for `min(mega_moe_max_tokens // expert_parallel_size, mc2_tokens_capacity)` tokens per rank. On a D node, `mc2_tokens_capacity` is the largest ACLGraph execution bucket; on a P node with `enable_prefill_mc2`, it is the prefill MC2 execution capacity. The operator uses its dynamic token mode for each batch; workspace memory scales linearly with the allocated capacity. |
