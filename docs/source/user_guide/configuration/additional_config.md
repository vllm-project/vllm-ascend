...
| `sfa_dcp_replicated_indexer`        | bool | `False` | Whether to enable the SFA replicated-indexer layout for decode context parallelism. When enabled, LightningIndexer cache is replicated on each DCP rank for global sparse index selection, while SFA KV cache remains DCP-sharded. This option only takes effect for sparse SFA with `decode_context_parallel_size > 1`. |
