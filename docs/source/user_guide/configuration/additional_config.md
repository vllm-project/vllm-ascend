...
| `enable_fused_mc2`                  | int  | `0`     | Selects the Fused MC2 operator: `0` disables the fused path; `1` uses the legacy `dispatch_ffn_combine` operator; `2` opts into CANN `mega_moe` for testing. Existing model, hardware, and parallel-configuration constraints still apply. The legacy `VLLM_ASCEND_ENABLE_FUSED_MC2` environment variable is no longer supported. |
