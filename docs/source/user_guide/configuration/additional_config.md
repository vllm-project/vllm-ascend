...
| `short_request_first_config` | dict | `{}` | Configuration options for ShortRequestFirst prefill scheduling on synchronous, PD-prefill (P), or PD-mixed nodes. |
...
ShortRequestFirst is a waiting-queue policy for the synchronous scheduler on prefill and PD-mixed paths. It requires FCFS scheduling and does not support asynchronous, batch-job-aware, profiling-chunk, or PD-disaggregated D-node scheduling. See [ShortRequestFirst Prefill Scheduling](../feature_guide/short_request_first.md) for usage, behavior, and tuning guidance.
