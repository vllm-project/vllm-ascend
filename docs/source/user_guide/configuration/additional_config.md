...
| `enbale_prefill_mc2`                | bool | `False` | Whether to reserve MC2 token capacity for prefill batches. When enabled, `max_num_batched_tokens` is used to calculate the MC2 token capacity instead of the decode-only capacity. In this scenario, the recommended maximum value of `max_num_batched_tokens` is `tp_size * 512`. This is a temporary switch; once MC2 operators are complete for all scenarios, this switch will be removed and MC2 will be enabled by default. |
