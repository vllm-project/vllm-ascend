...
| `moe_comm_method_for_a5_prefill`  | str  | `None`  | Overrides the A5 prefill MoE communication method after the decode MC2 path. Valid values: `ALLGATHER` or `ALLTOALL` (case-insensitive). `None` keeps the default selection logic. This option only affects A5 MoE models. |
