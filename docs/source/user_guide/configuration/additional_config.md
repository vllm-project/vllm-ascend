...
| `enable_moe_lora_dual_stream`       | bool | `False` | Whether to overlap quantized MoE LoRA with base expert GMM on an auxiliary NPU stream. Supports eager and ACLGraph execution for TP with the AllGather backend, without EP or fully sharded LoRA. The auxiliary stream and event dependencies are captured and replayed as part of ACLGraph. |
