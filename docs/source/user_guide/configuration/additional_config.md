...
| `enable_sfa_split_kv`               | bool | `False` | Whether to split full SparseFlashAttention V1 KV selections into parallel partial calls on A5 and merge them with Triton. This experimental path supports eager, unquantized, single-request decode with RoPE and falls back to the original operator for graph execution, unsupported shapes, early context, and DCP. |
