...
本 PR 是 [历史交接 Draft #2](https://github.com/Henry-Avery/vllm-ascend/pull/2)
的后续，依赖尚未合并的 [非连续缓存 #16456](https://github.com/vllm-project/vllm-ascend/pull/16456)。
由于依赖分支在另一个 fork，当前上游 PR 以 `main` 为目标，暂时包含缓存依赖改动。
等待 #16456 合入后，再整理为算子增量并重新验证最终提交。
不要将历史交接分支整包合入，也不要将本 Draft 作为已验收版本合并。

| 角色 | 固定 SHA |
| --- | --- |
| Ascend 集成时的 main | `fde460648b159410eac68f4d32fec97587cc8f3d` |
| #16456 | `4839002d74a5cac8965a5dfee11c627442bdaa79` |
| 已合入的 MRV2 DSpark #16347 | `d4511510e15eb52744089e40d8cd7c9fc23ac0f4` |
| main + #16456 集成基线 | `c8ca7b135f4cd989dd4367ac81919c7759be5514` |
| 历史交接 Draft | `16ceabb4068919592e34768c6f9cfe26683e9ac9` |
| 配套 vLLM | `84030bbe3d74d99bad477a3d2e37a973ccd8865c` |

缓存 PR #16456 和固定 main 均已包含 #16347，不重复合入 DSpark。
发布准备时上游 main 已前进到 `79eeed81c739974444a407c66acff1e113bd59aa`，
新增的一次 CI/bisect 回滚未涉及此处运行时文件；本候选仍保留上表的集成基线。
...
本地 macOS arm64、Python 3.12、CPU torch 2.8.0：
...
共 40 项 CPU/mock 检查：attention 17、DSpark 15、integration 8。
...
另有 #16456 的 4 项生命周期测试通过隔离 CPU harness 运行。
只修正了原测试中将字节数当 BF16 元素数、以及子页复制预期的问题，生产 COW/清零代码未改。
