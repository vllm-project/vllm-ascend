...
输入过滤（**detect 阶段**，`InputFilterManager`）：`input_filter.filters`。
空链=不过滤；否则全部 include 命中且无一 exclude 命中才跑检测。
`dump_once` **不**走过滤。见 `vllm_ascend/dfx/input_filters.py`。

## 2. 代码路径

- 核心实现：`vllm_ascend/dfx/dumper/`（`core` / `msprobe` / `pending`）
- 输入过滤：`vllm_ascend/dfx/input_filters.py`
...
2. `execute_model()`：`dfx.sync_for_step(allow_arm=not dummy_run)` → `dfx.start_dump_data()` → `super().execute_model` → `finally: dfx.finalize_dump_data(dump=not dummy_run)`
3. `postprocess_sampled()`：`dfx.check_after_spec`
4. `sample_tokens()`：sync 当场 `dfx.check_after_sample`；async 包装为 `AscendAsyncOutput`，在 `get_output()` 后检测
...
4. async / sync+TP>1 下 last PP 每步 CPU all_reduce（全员参与）；不能「仅 pending 的 rank 进 collective」。
