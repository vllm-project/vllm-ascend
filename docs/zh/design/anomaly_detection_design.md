...
- **DetectorManager 门面**：具体 detector 私有；runner / `DfxProcessor` 只调阶段钩子。
...
1. `dump.max_times=0`（或配额用尽）时不跑 auto 检测 / 不强开 top-k；`dump_once` 仍可手动 arm。
...
- **连续子序列匹配**：对**累计输出 token id** 做连续子序列匹配（不做逐 token decode），命中即告警。
- **每请求一次**：同一 `req_id` 最多告警一次，直到 `clear_finished`。
...
```json
{
  ...
  "detector": {
    "output_substring": { "enabled": true, "patterns": ["[CENSORED]", [100, 200, 300]], "add_special_tokens": false }
  }
}
```
...
- **Tokenzier**：编译 pattern 需要模型 tokenizer（懒加载；`tokenizer_provider` 或共享 `load_model_tokenizer`）。尚未就绪时 pattern 暂缓编译，就绪后自动重试；encode/decode 失败的单项 pattern 跳过并打 warning。
