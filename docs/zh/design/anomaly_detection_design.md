...
- **InputFilterManager 门控**：每个 detector 在 `check_all` 内 `_passes_input_filter`（`manual_dump` / `ManualTriggerManager` 除外）。
...
- **与 dump 正交**：detect 不依赖 dump 激活 / `auto_max_times`；dump arm 失败仍写 report。
...
```json
{
  "dump": { "auto_max_times": 3, "auto_cooldown_seconds": 300, "manual_dump": false },
  "detector": {
...
1. **detect ⊥ dump**：`auto_max_times=0`（或配额用尽）只阻止 **auto-arm dump**；auto 检测与 `ensure_logprobs_for_detection`（top-k）仍按 `detector.token_logprob.enabled` 运行；`manual_dump` 仍可手动 arm。
...
```json
{
  "dump": { "auto_max_times": 3, "manual_dump": false },
  "detector": {
    "output_substring": {
      "enabled": true,
...
- **msprobe**：纯 DFX 原生检测；命中后与其它 detector 相同：`AnomalyAlert` → `Dumper.handle_anomaly_alert`（dump 激活时在后续 forward 采 msprobe 张量）。不调用 ILLDetector。
...
- **msprobe**：不依赖 msprobe 包；若 dump 激活，alert 后下一波 forward 由 msprobe `PrecisionDebugger` / `AclGraphDumper` 落盘（与 `manual_dump` / auto dump 相同通路）。
### msprobe 适配总表（全部 detector）

| detector | 依赖 msprobe | 命中后 dump |
|----------|-------------|-------------|
| `token_logprob` | **是**（ILLDetector） | dump 激活 + `auto_max_times` / cooldown |
...
共性：`detect-only`（dump 未激活）仅写 report；`detect+auto_dump` 需 `auto_max_times>0`（或 `manual_dump`）。msprobe JSON `dump_enable` 由 DFX dumper 写/读；bootstrap 兼容见 [dfx_ops.md](./dfx_ops.md) §1.1。
