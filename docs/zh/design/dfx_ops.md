...
| 项 | 说明 |
|----|------|
| `dfx_config_reload_interval > 0` | **必须**，否则改 JSON / `dump_once` 不生效 |
| `dump_config_path` | msprobe 配置；无则无法落 dump |
| 每 EngineCore 可读 JSON | 多 DP 不互相广播 config；共享盘一份或每节点一份 |
...
- `dump.enabled=true` 且无 detector：合法；auto 不会触发；可用 `dump_once`（日志 warn）。
- `max_times: 0`：不 auto-arm dump；detect / `dump_once` 仍可。
- token 检测开启后 worker 会强制 top-k logprobs，请求侧可不设 `logprobs`。
- **输出关键词**（`detector.output_substring.enabled`）：`patterns` 可混用字符串与 token id 列表，例如 `["ERR", [1,2,3]]`。热更后日志打印每条 pattern 的 text↔token_ids；命中后该 req 不再检；report 含 `matched_text` / `matched_token_ids`。

### 2.2 手动 `dump_once`

1. 确认热更已开、`dump.enabled=true`、`dump_config_path` 有效（**不必**开 detector）。  
2. 将 JSON 中 `"dump_once": true`（仅本 EngineCore 的 writer 路径）。  
3. 等待**有真实请求**的下一拍 `execute_model`（空闲 / `execute_dummy_batch` **不会**消费）。  
4. 若 `dump.enabled=false`：**不消费** flag（保持 true）并打日志，修好后再等下一拍。  
5. 成功后写回 `false`；日志可搜 `manual_dump` / `[DFX manual_dump]`。  
6. **Report**：arm 成功后写一份 `manual_dump_once` 报告；`detail.requests` 含当前 batch **全部**请求的 prompt/output（`save_sensitive_info` 控制是否带 token ids）。
...
配置在 `input_filter.filters`（**detect 阶段**；不挡 `dump_once`）：
...
语义：全部 `include` 命中 **且** 无一 `exclude` 命中 → 才跑 Spec / TokenLogprob。  
...
| `dump_once` 一直 true / 无 dump | `dump.enabled=false`；服务空闲只走 dummy；或热更关 | 开 `dump.enabled`；打真实请求；确认 interval |
...
| 完全无检测日志 | 未开 `detector.enable_*`；或 rank / filter | 打开至少一个 detector；查 `[DFX filter]` |
...
| `[DFX manual_dump]` / `manual_dump` | `dump_once` |
| `[Anomaly spec short]` / `[Anomaly token_logprob` | 检测 short |
