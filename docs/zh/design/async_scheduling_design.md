...
> 时序与 pending-OR 语义仍以本文为准。完整 I/O 走 DFX report（`save_sensitive_info`），日志只打长度。

---

## 目录

- [异步调度下的 Dump / 检测时序设计](#异步调度下的-dump--检测时序设计)
  - [目录](#目录)
  - [1. 核心问题概览](#1-核心问题概览)
  - [2. 背景：同步 vs 异步调度模型](#2-背景同步-vs-异步调度模型)
    - [2.1 同步调度](#21-同步调度)
    - [2.2 异步调度](#22-异步调度)
  - [3. 问题一：sampled\_token\_ids=-1 占位符](#3-问题一sampled_token_ids-1-占位符)
    - [3.1 根因](#31-根因)
    - [3.2 真值回填链路](#32-真值回填链路)
    - [3.3 对 Dumper 的影响](#33-对-dumper-的影响)
    - [3.4 时序图](#34-时序图)
  - [4. 问题三：Dumper start/finalize 的"下步生效"机制](#4-问题三dumper-startfinalize-的下步生效机制)
    - [5.1 核心概念](#51-核心概念)
    - [5.2 为什么"下步生效"](#52-为什么下步生效)
    - [5.3 关键守卫逻辑](#53-关键守卫逻辑)
    - [5.4 时序图](#54-时序图)
  - [5. 问题四：异步模式下 TP 跨 Rank Dump 对齐](#5-问题四异步模式下-tp-跨-rank-dump-对齐)
    - [6.1 问题](#61-问题)
    - [6.2 "Pending + OR Sync" 机制](#62-pending--or-sync-机制)
    - [6.3 时序图](#63-时序图)
  - [6. 解决方案总结](#6-解决方案总结)
    - [6.1 已落地修复](#61-已落地修复)
    - [6.2 仍存在的局限性](#62-仍存在的局限性)
    - [6.3 未来改进方向](#63-未来改进方向)
...
## 4. 问题三：Dumper start/finalize 的"下步生效"机制
...
| 编号 | 修复 | 位置 |
|------|------|------|
| 1 | "Pending + OR Sync" 机制：异步下 TP0 arm pending，下步 `sync_dump_pending_or` 广播 | `dumper.py:sync_dump_pending_or` |
| 2 | `disable_msprobe_dump_if_needed` 的 `_dump_needs_forward` 守卫，防止没 forward 就关 | `dumper.py:353` |
| 3 | `finalize_dump_data(dump=False)` 在 dummy/capture 时不消费 pending | `dumper.py:197` |
