...
```json
{
  ...
  "report": { "save_sensitive_info": false, "print_sampling_meta": false },
  "detector": {
...
| `report` | `save_sensitive_info`：默认 `false` 只存 `*_token_count`（不写 token ids）；`true` 时落盘 id（受 `max_*` 截断）并可 decode。`max_prompt_token_ids` / `max_output_token_ids` 默认 1000，`0`=不截断。`print_sampling_meta`：默认 `false` |
...
| `prompt_contains_token_ids`（别名 `contains`） | `token_ids` + `match`: `any` \| `subsequence` |
...
Detector 通过基类 `_passes_input_filter` → `InputFilterManager.get().allow(...)`。运维见 [dfx_ops.md](./dfx_ops.md) §2.3。

## 3. Detector

| 类 | 文件 | `anomaly_type` |
|----|------|----------------|
| `DetectorManager` | `detector/registry.py` | 阶段 hook 门面（隐藏具体 detector） |
...
- **OutputSubstring**：`detector.output_substring.patterns` 支持 `str`（text）或 `list[int]`（token ids）；config 刷新时 encode/decode 并打日志；对累计 output **token id 连续子序列**匹配；每 `req_id` 告警一次；report `detail` 含 `matched_text` / `matched_token_ids`
...
3. `sync_dump_pending_or()`（仅 last-PP TP；**不含** config / report）
...
arm 成功后写 **一份** report：`detail.requests[]` 快照当前 batch 全部请求的 I/O（与单请求 anomaly report 不同）。  
