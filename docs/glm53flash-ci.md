...
边界包括3/4/5、127/128/129、511/512/513、2047/2048/2049、2051/2052/2053、2177，再加入实际block size前后边界。采集器和analyzer共用`glm53flash_protocol.py`，按运行时block size验证完整、有序且无重复的场景列表，不再硬编码最少19项。block size为128时去重后16项，为1152时19项；两者均可作为对应配置的完整采集。缺失、替换、重复或多余场景均拒绝；`--smoke`只有128长度，不能用作完整基线。
