...
# This layer only matters for INTERNAL CI builds. Our CI passes --build-arg with
# cluster-internal mirrors (PIP_INDEX_URL, APTMIRROR, GIT_PROXY, CRATES_IO_INDEX)
# to accelerate package downloads; those mirrors are unreachable from the published
# image, so we restore the public defaults here.
# If you build with the default build-args (all public / empty), every branch below
# is skipped and no extra operations run - you can ignore this layer entirely:
...
RUN if [ "$PIP_INDEX_URL" != "$PUBLIC_PIP_INDEX_URL" ]; then \
