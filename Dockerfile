...
RUN bash -c 'f=/vllm-workspace/vllm-ascend/CANN/vendors/hwcomputing/bin/set_env.bash; \
    if [ -f "$f" ]; then \
        echo "Sourcing $f"; \
        source "$f"; \
    else \
...
RUN bash -c 'f=/__w/vllm-ascend/CANN/vendors/hwcomputing/bin/set_env.bash; \
    if [ -f "$f" ]; then \
        echo "Sourcing $f"; \
        source "$f"; \
    else \
