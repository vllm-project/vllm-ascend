...
COPY ./tools/mooncake_installer.sh ./tools/sccache_installer.sh /vllm-workspace/
...
RUN --mount=type=secret,id=ACTIONS_RESULTS_URL,required=false \
    ...
    if [ "${SCCACHE_GHA_ENABLED}" = "true" ]; then \
        bash /vllm-workspace/sccache_installer.sh; \
        rm -f /vllm-workspace/sccache_installer.sh; \
    fi
...
RUN --mount=type=secret,id=ACTIONS_RESULTS_URL,required=false \
    ...
    if [ "${SCCACHE_GHA_ENABLED}" = "true" ]; then \
        ...
        SCCACHE_CMAKE_FLAG="-DENABLE_SCCACHE=ON"; \
    else \
        SCCACHE_CMAKE_FLAG=""; \
    fi && \
