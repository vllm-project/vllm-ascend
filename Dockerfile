...
RUN apt-get update -y && \
    ...
    python3 -m pip uninstall -y mooncake-transfer-engine mooncake-transfer-engine-ascend mooncake-transfer-engine-npu && \
    find /usr/local -type d -path '*/site-packages/mooncake' -prune -exec rm -rf {} + && \
    python3 -m pip install --no-cache-dir mooncake-transfer-engine-npu==${MOONCAKE_TAG} --index-url https://mirrors.aliyun.com/pypi/web/simple && \
    python3 -c "from pathlib import Path; import mooncake.engine; module = Path(mooncake.engine.__file__); assert module.name == 'engine.so', f'Unexpected Mooncake engine module: {module}'" && \
    rm -rf /var/cache/apt/* && \
