...
# Install pypto-isa dependencies
ARG PYPTO_VERSION=8.5.0
RUN ARCH=$(case "${TARGETPLATFORM}" in \
        "linux/amd64") echo "x86_64" ;; \
        "linux/arm64") echo "aarch64" ;; \
        *) echo "Unsupported TARGETPLATFORM: ${TARGETPLATFORM}" && exit 1 ;; \
    esac) && \
    ARCH_SHORT=$(case "${TARGETPLATFORM}" in \
...
RUN apt-get update -y && \
    apt-get install -y git vim wget net-tools gcc g++ cmake make && \
    git clone --depth 1 -b ${PYPTO_BRANCH} https://gitcode.com/cann/pypto.git /vllm-workspace/pypto && \
    cd /vllm-workspace/pypto && \
