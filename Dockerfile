...
RUN if [ -n "$APTMIRROR" ]; then \
    ...
    apt-get update -y && \
    apt-get install -y git vim wget curl protobuf-compiler net-tools gcc g++ cmake numactl libnuma-dev libibverbs-dev libjemalloc2 libhiredis-dev clang-15 ca-certificates && \
