...
class Repro(torch.nn.Module):
    ...
    def forward(self, arg0_1):
        ...
        full_default = torch.ops.aten.full.default([2, 1, 2], 0, dtype = torch.float32, layout = torch.strided, device = device(type='npu', index=0), pin_memory = False)
...
def load_args(reader):
    buf0 = reader.storage(None, 16, device=device(type='npu', index=0))
