...
Please report security issues privately using [the vulnerability submission form](https://github.com/vllm-project/vllm-ascend/security/advisories/new).

## Issue triage

Reports will then be triaged by the vLLM Ascend maintainers in coordination with the [vLLM vulnerability management team](https://docs.vllm.ai/en/latest/contributing/vulnerability_management.html).
...
For certain security issues of CRITICAL, HIGH, or MODERATE severity level, we may prenotify certain organizations or vendors that ship vLLM Ascend. The purpose of this prenotification is to allow for a coordinated release of fixes for severe issues.
...
* If you wish to be added to the prenotification group, please send an email to the vLLM Ascend maintainers copying the members of the [vLLM vulnerability management team](https://docs.vllm.ai/en/latest/contributing/vulnerability_management.html). Each vendor contact will be analyzed on a case-by-case basis.

* Organizations and vendors who either ship or use vLLM Ascend, are eligible to join the prenotification group if they meet at least one of the following qualifications
    * Substantial internal deployment leveraging the upstream vLLM Ascend project.
    * Established internal security teams and comprehensive compliance measures.
    * Active and consistent contributions to the upstream vLLM Ascend project.
