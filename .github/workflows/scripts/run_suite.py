...
def run_tests(
    ...
    NOTE:
        The emitted START / PASSED / FAILED log lines are used as stable
        per-test invocation boundaries in suite-level CI logs. Keep this
        output format stable when changing the runner.
    ...
    for i, test in enumerate(files):
        ...
        try:
            print(f"\n{'.' * 60}", flush=True)
            # Keep this START line format stable for suite-level log boundaries.
            print(
            ...
            # Keep this PASSED / FAILED line format stable for suite end detection.
            print(
