...
| `test_issues_csv.py` | Batch LLM evaluation — calls the LLM with production prompts on CSV datasets |
...
python test_issues_csv.py --mode issue --input issues.csv --start 10 --limit 20

# Re-process all rows (skip existing results off)
python test_issues_csv.py --mode issue --input issues.csv --skip-existing

# Use a different output file
python test_issues_csv.py --mode issue --input issues.csv --output results.csv
```
