...
jobs:
  lint-and-select-tests:
    if: ${{ github.event.action != 'labeled' || github.event.label.name == 'ready' || github.event.label.name == 'ready-a5' }}
    ...
    outputs:
      ...
      has_tests_a5: ${{ steps.scope_a5.outputs.has_tests || steps.noop_a5.outputs.has_tests }}
      test_groups_a5: ${{ steps.scope_a5.outputs.test_groups || steps.noop_a5.outputs.test_groups }}
  ...
  run-selected-tests-a5:
    needs: lint-and-select-tests
    if: ${{ needs.lint-and-select-tests.outputs.has_tests_a5 == 'true' && contains(github.event.pull_request.labels.*.name, 'ready-a5') }}
    strategy:
      matrix:
        vllm_version:
          - ${{ needs.lint-and-select-tests.outputs.release_tag }}
    ...
    with:
      ...
      ref: ${{ github.event.pull_request.head.sha }}
      base_ref: ${{ github.base_ref }}
      test_groups: ${{ needs.lint-and-select-tests.outputs.test_groups_a5 }}
