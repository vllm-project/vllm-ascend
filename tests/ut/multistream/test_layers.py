...
def test_forward_no_multistream_metadata(mock_set_ctx, mock_get_ctx, input_tensors):
...
@patch("vllm_ascend.multistream.layers.get_forward_context")
@patch("vllm_ascend.multistream.layers.set_multistream_layer_context")
def test_forward_no_attn_metadata(mock_set_ctx, mock_get_ctx, input_tensors):
    mock_get_ctx.return_value = DummyContext(attn_metadata=None)
    dummy_metadata = MagicMock(spec=MultiStreamMetadata)
    layer = MultiStreamPreTransformerLayer(multistream_metadata=dummy_metadata)
...
@patch("vllm_ascend.multistream.layers.get_forward_context")
@patch("vllm_ascend.multistream.layers.set_multistream_layer_context")
def test_forward_no_split(mock_set_ctx, mock_get_ctx, input_tensors):
    dummy_attn = "original_attn"
    mock_get_ctx.return_value = DummyContext(attn_metadata=dummy_attn)

    dummy_metadata = MagicMock(spec=MultiStreamMetadata)
    dummy_metadata.split_micro_batch.return_value = (
        False, "same_attn", input_tensors, None
    )
...
@patch("vllm_ascend.multistream.layers.get_forward_context")
@patch("vllm_ascend.multistream.layers.set_multistream_layer_context")
def test_forward_split(mock_set_ctx, mock_get_ctx, input_tensors):
    dummy_attn = "original_attn"
    mock_get_ctx.return_value = DummyContext(attn_metadata=dummy_attn)
    ...
    dummy_metadata.split_micro_batch.return_value = (
        True, ["attn1", "attn2"], split_inputs, None
    )
    ...
    mock_set_ctx.assert_called_once_with(2, dummy_metadata, ["attn1", "attn2"])
    
    
def test_post_forward_metadata_none(input_tensors):
    ...
    layer = MultiStreamPostTransformerLayer(multistream_metadata=dummy_metadata)
    output = layer.forward(input_tensors)
    assert output == input_tensors
...
def test_post_forward_normal_flow(mock_reset_ctx, mock_get_ctx, input_tensors):
...
@patch("vllm_ascend.multistream.layers.get_multistream_layer_context")
@patch("vllm_ascend.multistream.layers.reset_multistream_layer_context")
def test_post_forward_with_custom_wait_layer(mock_reset_ctx, mock_get_ctx, input_tensors):
    A_instance_of_MultiStreamMetadata = MultiStreamMetadata(
        calculate_stream=MagicMock(),
        communicate_stream=MagicMock(),
        start_layer=0,
