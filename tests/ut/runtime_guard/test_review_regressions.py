...
def test_v15b_report_writer_wave_backoff(tmp_path: Path):
    ...
    assert w.write(**kw, dump_arm_wave=100 + SAME_PAIR_BACKOFF_BASE_WAVES - 1) is None
    assert w.write(**kw, dump_arm_wave=100 + SAME_PAIR_BACKOFF_BASE_WAVES) is not None
    # Next gap doubles: 128 waves from the 2nd write's wave.
    second_wave = 100 + SAME_PAIR_BACKOFF_BASE_WAVES
    assert w.write(**kw, dump_arm_wave=second_wave + 2 * SAME_PAIR_BACKOFF_BASE_WAVES - 1) is None
    assert w.write(**kw, dump_arm_wave=second_wave + 2 * SAME_PAIR_BACKOFF_BASE_WAVES) is not None
    assert store.get_state("r1").detection_stopped is True
