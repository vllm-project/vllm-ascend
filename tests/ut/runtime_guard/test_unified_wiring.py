...
def _make_runner(logits, dfx):
    model = _FakeModel(logits)
    return SimpleNamespace(model=model, runtime_guard=dfx), model


def test_wrap_fires_once_for_two_compute_logits_calls():
    dfx = _FakeDfx()
    runner, model = _make_runner("logits-tensor", dfx)
    ...
    assert "scheduler_output" not in dfx.calls[0]
...
def test_from_batch_falls_back_to_runner_logits_indices():
    ...
    check_before_sample_from_batch(dfx, "L", SimpleNamespace())
    call = dfx.calls[0]
    assert call["logits_indices"] == [1, 2]
    assert "positions" not in call
    assert "scheduler_output" not in call
...
def test_from_batch_prefers_input_batch_logits_indices():
    ...
    check_before_sample_from_batch(dfx, "L", batch)
    call = dfx.calls[0]
