...
def test_flash_builder_refinement_preserves_scheduler_groups_and_query_heads():
    ...
    layers = {
        "mla.rope.0": SimpleNamespace(impl=SimpleNamespace(use_mla_rope=True, scale=0.4)),
        "mla.plain": SimpleNamespace(impl=SimpleNamespace(use_mla_rope=False, scale=0.2)),
        "mla.rope.1": SimpleNamespace(impl=SimpleNamespace(use_mla_rope=True, scale=0.4)),
...
def test_flash_draft_graph_replay_does_not_rebuild_metadata_or_update_fia():
    ...
    assert namespace[cls.name]().run_fullgraph("bucket") == ("replayed", "bucket")
