...
def test_mla_backend_accepts_upstream_cache_dtype_keyword():
    if hasattr(AscendMLABackend, "get_kv_cache_block_dim"):
...
def test_compressed_cache_uses_physical_block_size():
    # Match the installed upstream cache-spec API without discarding main's
    # tokens_per_state coverage on newer vLLM versions.
    ratio_key = (
        "compress_ratio" if "compress_ratio" in AscendMLAAttentionSpec.__dataclass_fields__ else "tokens_per_state"
    )
    spec = AscendMLAAttentionSpec(block_size=128, num_kv_heads=1, head_size=8, dtype=torch.bfloat16, **{ratio_key: 4})
