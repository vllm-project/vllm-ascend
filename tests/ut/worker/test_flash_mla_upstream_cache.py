...
class _ReplicatedDraftSpec(SimpleNamespace):
    pass
...
def _runner_methods():
    ...
    interface = source.parents[1] / "core/kv_cache_interface.py"
    helper = next(
        node
        for node in ast.parse(interface.read_text(encoding="utf-8")).body
        if getattr(node, "name", None) == "requires_padded_page_layout"
    )
    scope = {
        "torch": torch,
        "math": math,
        ...
        "AscendDCPReplicatedDraftAttentionSpec": _ReplicatedDraftSpec,
        "MLAAttention": _MLAAttention,
        "UniformTypeKVCacheSpecs": type("UniformTypeKVCacheSpecs", (), {}),
        "AttentionLayerBase": type("AttentionLayerBase", (), {}),
    }
    module = ast.Module(
        body=[ast.ImportFrom(module="__future__", names=[ast.alias(name="annotations")], level=0), helper, *methods],
        type_ignores=[],
    )
...
def test_replicated_draft_pages_preserve_target_strides_and_separate_storage(replication_size):
    runner = _runner_methods()
    blocks, block_size, heads = 3, 16, 4
    target_payload = block_size * 576 * 2
    target_pitch = 2 * target_payload + 256
    draft_payload = heads * block_size * 64 * 2
    draft_offset = blocks * target_pitch
    backing_size = draft_offset + blocks * replication_size * draft_payload
    target_spec = SimpleNamespace(
        block_size=block_size,
        num_heads=1,
        num_states=block_size,
        state_content_size_bytes=576 * 2,
        dtype=torch.bfloat16,
        get_num_kernel_states=lambda kernel_size: kernel_size,
    )
    draft_spec = _ReplicatedDraftSpec(
        block_size=block_size,
        num_heads=heads,
        num_states=block_size,
        state_content_size_bytes=64 * 2,
        dtype=torch.bfloat16,
        get_num_kernel_states=lambda kernel_size: kernel_size,
        dcp_replication_size=replication_size,
        lane_page_size_bytes=draft_payload,
        page_size_bytes=draft_payload * replication_size,
    )
    groups = [
        SimpleNamespace(layer_names=[name], kv_cache_spec=spec, backend=None, kv_cache_group_id=idx)
        for idx, (name, spec) in enumerate((("target", target_spec), ("draft", draft_spec)))
    ]
    config = SimpleNamespace(
        num_blocks=blocks,
        kv_cache_groups=groups,
        kv_cache_tensors=[
            SimpleNamespace(size=backing_size, layers=["target"], offset=0, layer_stride=0, block_stride=target_pitch),
            SimpleNamespace(
                size=backing_size,
                layers=["draft"],
                offset=draft_offset,
                layer_stride=0,
                block_stride=draft_payload * replication_size,
            ),
        ],
    )
    runner.ascend_config = SimpleNamespace(kvpp_config=SimpleNamespace(size=1))
    ...
    runner.device = torch.device("cpu")
    runner.compilation_config = SimpleNamespace(static_forward_context={"target": _MLAAttention(), "draft": object()})
    runner.runner_only_attn_layers = set()
    runner.kernel_block_sizes = [[block_size], [block_size]]
    runner._kv_cache_spec_attn_group_iterator = lambda: iter(groups)
    raw_caches = runner._allocate_kv_cache_tensors(config)
    caches = runner._reshape_kv_cache_tensors(config, raw_caches)
    target, draft = caches["target"], caches["draft"]
    assert target.stride(0) == target_pitch // 2
    assert draft.shape == (blocks * replication_size, heads, block_size, 64)
    assert draft.stride() == (draft_payload // 2, block_size * 64, 64, 1)
    assert draft.data_ptr() - target.data_ptr() == draft_offset
    assert draft.untyped_storage().data_ptr() == target.untyped_storage().data_ptr()
    target.fill_(17)
    for logical_block in range(blocks):
        for lane in range(replication_size):
            physical_block = logical_block * replication_size + lane
            draft[physical_block].fill_(physical_block + 1)
    assert torch.all(target == 17)
    for physical_block in range(blocks * replication_size):
        assert torch.all(draft[physical_block] == physical_block + 1)
