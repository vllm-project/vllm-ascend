...
def test_partition_batch_refreshes_local_ascend_input_batch_metadata():
    ...
    # vLLM main orders PCP chunks by ascending global slice start (#56157) —
    # rank 0 owns the head chunk then the tail chunk. The release pin keeps the
    # old "pure prefills last" ordering (tail chunk then head chunk).
    ...
    if vllm_version_is("0.28.0"):
        expected_num_scheduled_tokens = np.array([3, 5], dtype=np.int32)
        ...
        expected_num_scheduled_tokens = np.array([5, 3], dtype=np.int32)
    ...
    np.testing.assert_array_equal(result.num_scheduled_tokens, expected_num_scheduled_tokens)
