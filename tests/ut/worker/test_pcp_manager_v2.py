...
def test_partition_batch_refreshes_local_ascend_input_batch_metadata():
    ...
    with (
        ...
        # Upstream removed this helper from PCPManager (main); create=True keeps
        # the release-lane patch working without requiring the attribute.
        patch(
    ...
    # PCP=2 rank 0 owns two chunks of the split prefill. vLLM main orders the
    # local segments canonically (head chunk before tail); the release pin
    # keeps the old tail-then-head order.
    if vllm_version_is("0.29.0"):
        expected_scheduled = np.array([3, 5], dtype=np.int32)
        expected_query_start_loc = np.array([0, 3, 8], dtype=np.int32)
        expected_local_tokens = torch.tensor([15, 16, 17, 0, 1, 2, 3, 4], dtype=torch.int32)
        expected_seq_lens = np.array([18, 5], dtype=np.int32)
    else:
        expected_scheduled = np.array([5, 3], dtype=np.int32)
        expected_query_start_loc = np.array([0, 5, 8], dtype=np.int32)
        expected_local_tokens = torch.tensor([0, 1, 2, 3, 4, 15, 16, 17], dtype=torch.int32)
    ...
    np.testing.assert_array_equal(result.num_scheduled_tokens, expected_scheduled)
    ...
    assert torch.equal(result.input_ids[:8], expected_local_tokens)
