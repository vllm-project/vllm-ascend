...
class TestNPUModelRunnerKVCache(unittest.TestCase):
    ...
    def test_a3_fa_quant_mla_cache_accounts_for_mixed_dtype_bytes(
        ...
        attn_module.impl = SimpleNamespace(
            fa_quant_layer=True,
            dtype=torch.int8,
            _uses_a3_mla_fa_quant_cache=lambda: True,
        )
        ...
        self.assertEqual(layer_spec.dtype, torch.int8)
        self.assertEqual(layer_spec.head_size, expected_bytes_per_token)
        self.assertEqual(
            layer_spec.page_size_bytes,
            runner.block_size * expected_bytes_per_token,
        )
    ...
    def test_hybrid_a3_c8_mla_cache_keeps_mamba_page_alignment(self):
        runner = self._build_runner()
        runner.hybrid_with_attn_and_mamba = True
        runner.model_config.dtype = torch.bfloat16

        # Stop after runner initialization has selected the kernel geometry.
        # A single attention group must still activate virtual block splitting.
        runner.attn_groups = []
        ...
        runner.initialize_attn_backend = MagicMock(
            side_effect=lambda _config: runner.attn_groups.append([SimpleNamespace(backend=AscendMLABackend)])
        )
        runner.may_reinitialize_input_batch = MagicMock(
            side_effect=lambda _config: setattr(
                runner,
                "kernel_block_sizes",
                [[select_common_block_size(768, [AscendMLABackend])]],
            )
        )
        runner.initialize_kv_cache_tensors = MagicMock(side_effect=RuntimeError("cache geometry initialized"))
        manager_config = SimpleNamespace(
            has_mamba_layers=True,
            kv_cache_groups=[SimpleNamespace(kv_cache_spec=SimpleNamespace(block_size=768))],
        )
        with (
            patch("vllm_ascend.worker.model_runner_v1.apply_layerwise_kv_cache_plan"),
            self.assertRaisesRegex(RuntimeError, "cache geometry initialized"),
        ):
            runner.initialize_kv_cache(manager_config)
        self.assertTrue(runner.use_hybrid_blocks)
        ...
        runner._get_mla_mixed_dtype_fa_quant_cache_dtypes = lambda _name, _spec: (
            torch.int8,
            torch.bfloat16,
        )
