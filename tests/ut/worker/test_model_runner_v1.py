...
def test_generate_pcp_metadata_basic(
    ...
    mock_runner.input_batch.num_computed_tokens_cpu = torch.tensor(num_computed_tokens)
    ...
    mock_runner._get_cp_local_seq_lens = NPUModelRunner._get_cp_local_seq_lens.__get__(mock_runner, NPUModelRunner)
    ...
    method = NPUModelRunner._generate_pcp_metadata.__get__(mock_runner, NPUModelRunner)
    ...
    if not expect_not_none:
        ...
        if pcp_size > 1:
            ...
            if has_prefill_requests:
                ...
                if hasattr(result, 'pcp_prefill_mask') and result.pcp_prefill_mask is not None:
                    if use_mla:
...
def test_generate_pcp_metadata_edge_cases():
    ...
    assert (mock_runner.input_batch.num_reqs or mock_runner.query_lens.size(0)) == 3
...
def test_pcp_allgather_restore_idx_slicing():
    ...
    expected_slice = mock_runner.pcp_allgather_restore_idx[:num_actual_tokens_pcp_padded]
...
def test_update_tokens_for_pcp_basic(
    tokens, num_reqs, num_computed_tokens, 
    num_prompt_tokens, pcp_size, pcp_rank, expected_pcp_tokens
):
    mock_runner = MagicMock(spec=NPUModelRunner)
    mock_runner.pcp_size = pcp_size
    ...
    mock_runner.input_batch = MagicMock()
    mock_runner.input_batch.num_reqs = num_reqs
    mock_runner.input_batch.num_computed_tokens_cpu = np.array(num_computed_tokens, dtype=np.int32)
    mock_runner.input_batch.num_prompt_tokens = np.array(num_prompt_tokens, dtype=np.int32)
    ...
    mock_runner._update_tokens_for_pcp = NPUModelRunner._update_tokens_for_pcp.__get__(mock_runner, NPUModelRunner)
    mock_runner._get_cumsum_and_arange = NPUModelRunner._get_cumsum_and_arange.__get__(mock_runner, NPUModelRunner)
    
    pcp_tokens_result, positions_result, unpad_mask_result = mock_runner._update_tokens_for_pcp(tokens)
    ...
    padded_tokens = [(t + 2 * pcp_size - 1) // (2 * pcp_size) * (2 * pcp_size) if num_computed_tokens[i] == 0 
                     else t * pcp_size for i, t in enumerate(tokens)]
...
def test_update_tokens_for_pcp_with_padding():
    ...
    mock_runner.input_batch.num_computed_tokens_cpu = np.array([0, 0, 0], dtype=np.int32)
    mock_runner.input_batch.num_prompt_tokens = np.array([5, 9, 13], dtype=np.int32)
    ...
    pcp_tokens, positions, unpad_mask = mock_runner._update_tokens_for_pcp(tokens)
...
def test_update_tokens_for_pcp_unpad_mask():
    ...
    mock_runner.input_batch.num_computed_tokens_cpu = np.array([0, 0], dtype=np.int32)
    mock_runner.input_batch.num_prompt_tokens = np.array([5, 7], dtype=np.int32)
