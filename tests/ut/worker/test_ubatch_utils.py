...
def _build_common_attn_metadata(
    ...
    return AscendCommonAttentionMetadata(
        ...
        max_query_len=max(query_start_loc_cpu[i + 1] - query_start_loc_cpu[i] for i in range(n_reqs)),
        ...
        actual_seq_lengths_q=[query_start_loc_cpu[i + 1] - query_start_loc_cpu[i] for i in range(n_reqs)],
...
class TestUBatchRuntimeConcurrency(unittest.TestCase):
    ...
    def test_yield_thread_timeout_on_divergence(self):
        ...
        with patch.object(uu, "UBATCH_HANDOFF_TIMEOUT", 0.5):
            ...
            t.start()
            t.join(timeout=5)
            self.assertTrue(error_raised.get("timeout", False), "Expected TimeoutError from yield_thread")
...
class TestSliceModelInputsForUbatch(unittest.TestCase):
    ...
    def test_1d_inputs_sliced_on_dim0(self):
        ...
        s_ids, s_pos, s_emb = slice_model_inputs_for_ubatch(slice(4, 8), ids, pos, emb)
    ...
    def test_mrope_2d_positions_sliced_on_dim1(self):
        ...
        ids = torch.arange(12)
        s_ids, s_pos, _ = slice_model_inputs_for_ubatch(slice(4, 8), ids, pos, None)
        self.assertEqual(s_ids.tolist(), [4, 5, 6, 7])
    ...
    def test_none_inputs_pass_through(self):
        s_ids, s_pos, s_emb = slice_model_inputs_for_ubatch(slice(0, 4), None, None, None)
    ...
    def test_partial_none_inputs(self):
        ids = torch.arange(8)
        s_ids, s_pos, s_emb = slice_model_inputs_for_ubatch(slice(2, 5), ids, None, None)
...
class TestCommSectionSymmetryGuard(unittest.TestCase):
    ...
    def test_symmetric_counts_do_not_raise(self):
        ...
        with (
            patch.object(rt, "yield_and_switch_from_compute_to_comm"),
    ...
    def test_divergent_counts_raise_assertion(self):
        ...
        with (
            ...
            with self.assertRaises(AssertionError) as ctx, rt.comm_section():
                pass
