...
class TestNPUModelRunnerGraphProfiling(TestBase):
    def test_profile_cudagraph_memory_cleans_ascend_state_on_error(self):
        ...
        impl = SimpleNamespace(key_cache=object(), value_cache=object())
        runner.compilation_config = SimpleNamespace(static_forward_context={"layer": SimpleNamespace(impl=impl)})

        reset_graph_params()
        self.addCleanup(reset_graph_params)
        set_graph_params([1])
        ...
        with (
            ...
            patch.object(
                model_runner_v1,
                "_get_gpu_model_runner_module_name",
            ...
            patch.object(model_runner_v1.gc, "collect"),
            patch.object(torch.accelerator, "empty_cache"),
            self.assertRaisesRegex(RuntimeError, "profile failed"),
        ):
            runner.profile_cudagraph_memory()

        self.assertIsNone(get_graph_params())
        self.assertIsNone(impl.key_cache)
        self.assertIsNone(impl.value_cache)
