...
class TestNPUWorker(TestBase):
    ...
    @patch("vllm_ascend.worker.worker_v1.get_pp_group")
    @patch("vllm_ascend.worker.worker_v1.get_tp_group")
    @patch("vllm_ascend.worker.worker_v1.copy")
    @patch("vllm_ascend.worker.worker_v1.EMPTY_MODEL_RUNNER_OUTPUT")
    def test_execute_model_kv_connector_finished_sending(
            self, mock_empty_output, mock_copy, mock_get_tp_group,
            mock_get_pp_group):
        """Test execute_model method - kv_connector_output finished sending but not recving"""
        from vllm.sequence import IntermediateTensors
        ...
        with patch.object(NPUWorker, "__init__", lambda x, **kwargs: None):
            ...
            # Create mock kv_connector_output - finished_sending is True, finished_recving is False
            mock_kv_connector_output = MagicMock()
            ...
            # Setup mock for copy operation
            mock_empty_output_copy = MagicMock()
            mock_copy.copy.return_value = mock_empty_output_copy
            ...
            # Verify copy operation - should copy EMPTY_MODEL_RUNNER_OUTPUT when one is finished but not both
            mock_copy.copy.assert_called_once_with(mock_empty_output)

            # Verify kv_connector_output is assigned to the copied output
            self.assertEqual(mock_empty_output_copy.kv_connector_output,
                             mock_kv_connector_output)

            # Should return the copied empty output with kv_connector_output
            self.assertEqual(result, mock_empty_output_copy)

    @patch("vllm_ascend.worker.worker_v1.get_pp_group")
    @patch("vllm_ascend.worker.worker_v1.get_tp_group")
    @patch("vllm_ascend.worker.worker_v1.copy")
    @patch("vllm_ascend.worker.worker_v1.EMPTY_MODEL_RUNNER_OUTPUT")
    def test_execute_model_kv_connector_finished_recving(
            self, mock_empty_output, mock_copy, mock_get_tp_group,
            mock_get_pp_group):
        """Test execute_model method - kv_connector_output finished recving but not sending"""
        from vllm.sequence import IntermediateTensors
        ...
        with patch.object(NPUWorker, "__init__", lambda x, **kwargs: None):
            ...
            # Create mock kv_connector_output - finished_sending is False, finished_recving is True
            mock_kv_connector_output = MagicMock()
            mock_kv_connector_output.finished_sending = False
    ...
    @patch("vllm_ascend.worker.worker_v1.copy")
    @patch("vllm_ascend.worker.worker_v1.EMPTY_MODEL_RUNNER_OUTPUT")
    def test_execute_model_kv_connector_both_finished(self, mock_empty_output,
                                                      mock_copy,
                                                      mock_get_tp_group,
                                                      mock_get_pp_group):
        """Test execute_model method - kv_connector_output both finished sending and recving"""
        from vllm.sequence import IntermediateTensors
        ...
        with patch.object(NPUWorker, "__init__", lambda x, **kwargs: None):
            ...
            # Verify copy operation - should copy EMPTY_MODEL_RUNNER_OUTPUT when both are finished
            mock_copy.copy.assert_called_once_with(mock_empty_output)
