...
class TestNPUWorker(TestBase):
    ...
    def test_synchronize_device_is_defined_on_the_npu_worker(self):
        ...
        vLLM #52914 added ``WorkerBase.synchronize_device`` on main, but the
        verified release tag (v0.28.0) predates it, while ``wake_up`` reaches the
        barrier unconditionally -- so the pause/wake contract cannot rely on
        inheriting it.
        """
        from vllm_ascend.worker.worker import NPUWorker

        assert "synchronize_device" in NPUWorker.__dict__
