...
class TestNPUModelRunnerKVCache(unittest.TestCase):
    def _build_runner(self):
        ...
        runner.is_dots3_note_model = False
    ...
    def test_dots3_note_keeps_full_sliding_and_indexer_specs_separate(
        self,
        mock_get_layers,
        ...
        runner.is_dots3_note_model = True
        ...
        full_spec = Dots3NoteMLAAttentionSpec(
            block_size=16,
            num_kv_heads=1,
            head_size=576,
        ...
        sliding_spec = Dots3NoteSlidingWindowMLASpec(
            block_size=16,
            num_kv_heads=1,
            head_size=1088,
            dtype=torch.bfloat16,
        ...
        def make_mla_attention(spec, *, use_sparse, kv_lora_rank, qk_rope_head_dim):
            ...
            attn._vllm_ascend_dots3_note = True
        ...
        self.assertIsInstance(specs[full_layer_name], Dots3NoteMLAAttentionSpec)
        ...
        self.assertTrue(runner._is_sparse_mla_cache_spec(specs[full_layer_name]))
        self.assertFalse(runner._is_sparse_mla_cache_spec(specs[sliding_layer_name]))
        self.assertFalse(runner._is_sparse_mla_cache_spec(specs[indexer_layer_name]))

    @patch("vllm_ascend.worker.model_runner_v1.get_layers_from_vllm_config")
    def test_dots3_note_sliding_cache_skips_sparse_offload(self, mock_get_layers):
        runner = self._build_runner()
        runner.use_sparse = True
        runner.is_dots3_note_model = True
        runner.sparse_kv_offload_enabled = True
        layer_name = "model.layers.2.self_attn.attn"
        spec = Dots3NoteSlidingWindowMLASpec(
            block_size=16,
            num_kv_heads=1,
            head_size=1088,
            dtype=torch.bfloat16,
