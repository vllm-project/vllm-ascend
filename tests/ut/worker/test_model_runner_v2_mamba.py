...
def _bind_mamba_cache(
    spec: MambaSpec,
    page_cache: torch.Tensor,
) -> tuple[torch.Tensor, ...]:
    layer = MagicMock()
    layer.get_state_shape.return_value = spec.shapes
    layer.get_state_dtype.return_value = spec.dtypes
    MambaBase.bind_kv_cache(layer, page_cache)
...
def test_prepare_inputs_keeps_full_graph_request_metadata_padded():
    ...
    assert ("self.input_buffers.query_start_loc[\n            : num_reqs_padded + 1\n        ]") in prepare_inputs
...
def test_mamba_cache_allocate_and_reshape(_mock_config):
    ...
    page_cache = caches["linear_attn"]
    assert isinstance(page_cache, torch.Tensor)
    assert page_cache.shape == (3, 1, 1, spec.page_size_bytes)
    assert page_cache.dtype == torch.int8
    assert page_cache.data_ptr() == raw_cache.data_ptr()
    assert page_cache.stride(0) == spec.page_size_bytes
    assert not hasattr(page_cache, "_vllm_ascend_state_major")

    conv_state, ssm_state = _bind_mamba_cache(spec, page_cache)
    ...
    assert ssm_state.data_ptr() - raw_cache.data_ptr() == 2 * 3 * 2
...
def test_packed_mamba_cache_uses_layer_offset_and_block_stride(_mock_config):
    ...
    assert isinstance(page_cache, torch.Tensor)
    assert page_cache.shape == (3, 1, 1, spec.page_size_bytes)
    assert page_cache.data_ptr() - raw_cache.data_ptr() == 8
    assert page_cache.stride(0) == 64
    assert not getattr(page_cache, "_vllm_ascend_state_major", False)
...
def test_hybrid_attention_and_mamba_share_aligned_cache(_mock_config):
    ...
    page_cache = caches["linear_attn"]
    assert isinstance(page_cache, torch.Tensor)
    assert page_cache.shape == (2, 1, 1, mamba_spec.page_size_bytes)
    assert not hasattr(page_cache, "_vllm_ascend_state_major")
    conv_state, ssm_state = _bind_mamba_cache(mamba_spec, page_cache)
    assert conv_state.data_ptr() == raw_cache.data_ptr()
    assert ssm_state.data_ptr() - raw_cache.data_ptr() == 2 * 2
...
def test_hybrid_attention_expands_virtual_kernel_blocks(
    _mock_config,
    _mock_cache_dims,
):
