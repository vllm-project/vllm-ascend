...
class TestKVPoolWorkerRegisterAndTransfer(unittest.TestCase):
    ...
    def test_transfer_threads_use_grouped_block_sizes(self, mock_send_thread, mock_recv_thread, mock_event):
        ...
        self.assertIsNone(mock_send_thread.call_args.kwargs["worker"])
        self.assertIsNone(mock_recv_thread.call_args.kwargs["worker"])
...
class TestKVPoolWorkerProcessLayerData(unittest.TestCase):
    ...
    def test_mid_segment_cached_gva_is_revalidated_before_save(self):
        ...
        worker = self._make_gva_worker()
        cached_key = worker._make_layerwise_gva_key(0, "h1")
        ...
        expected_keys = [worker._make_layerwise_gva_key(0, block_hash) for block_hash in ("h0", "h1", "h2")]
...
class TestKVPoolWorkerTpMismatch(unittest.TestCase):
    ...
    def test_start_load_kv_sync_dispatches_tp_mismatch_reader(self):
        worker = self._make_worker(extra_config={"backend": "mooncake", "prefill_tp_size": 4})
