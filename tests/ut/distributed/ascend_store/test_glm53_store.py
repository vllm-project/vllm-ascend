...
class TestGLM53Store(unittest.TestCase):
    ...
    def test_nonlayerwise_round_trip_preserves_private_tail_and_group_ids(self):
        plan = make_glm53_plan()
        hashes = [bytes([i]) * 32 for i in (1, 2)]
        for load_async in (False, True):
            with self.subTest(load_async=load_async):
                worker = make_worker(
                    self, kv_cache_config=plan, use_mla=True, num_layers=4, extra_config={"load_async": load_async}
                )
                ...
                for group_id, group in enumerate(plan.kv_cache_groups):
                    for name in group.layer_names:
                        for cache in caches[name]:
                            cache[source[group_id]] = 10 + group_id
                                ...
                                for block_idx, allowed in enumerate(masks[group_id]):
                                    if allowed:
                                        torch.testing.assert_close(
                                            cache[target[group_id][block_idx]], cache[source[group_id][block_idx]]
                                        )
                missing = next(key for key in stored if "@group:4@" in key and key.endswith(hashes[-1].hex()))
                del stored[missing]
