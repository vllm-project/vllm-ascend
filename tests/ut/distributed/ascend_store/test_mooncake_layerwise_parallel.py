...
class TestMooncakeLayerwiseParallel(unittest.TestCase):
    ...
    def test_pp_dcp_round_trip_all_shards_and_stage_local_layers(self):
        for tp, pp, dcp, heads in [(2, 2, 1, 1), (2, 1, 2, 1), (4, 2, 2, 1), (8, 2, 2, 2)]:
            with self.subTest(tp=tp, pp=pp, dcp=dcp, heads=heads):
                ...
                self.assertEqual(scheduler._lookup_block_key_contiguous(query, 32 * dcp, 0), 32 * dcp)
                ...
                for missing in {worker._make_mooncake_layerwise_key(b"h1".hex()) for worker, _, _ in workers}:
                    ...
                    self.assertEqual(scheduler._lookup_block_key_contiguous(query, 32 * dcp, 0), 16 * dcp)
                    store.complete.add(missing)

    def test_session_keys_are_isolated_by_stage_and_dcp_shard(self):
        """The same block tail must map to a distinct object per stage and shard.

        Hybrid keys carry the PP and DCP coordinates, so two stages holding
        different layer stacks (or two DCP shards holding different token
        shards) never share an object.
        """
        keys = set()
    ...
    def test_namespace_separates_partitions_and_interleave(self):
        ...
        self.assertNotEqual(second, mooncake_layerwise.layerwise_topology_namespace(model, parallel))
