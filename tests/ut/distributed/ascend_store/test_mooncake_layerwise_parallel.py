...
"""CPU range-session integration tests for pipeline parallelism, without an NPU runtime."""
...
class MemoryRangeStore:
    ...
    def batch_is_readable(self, keys):
        # Mirrors MooncakeBackend: readability is the committed/exists state,
        # reported as real bools (the scheduler validates the type).
...
class TestMooncakeLayerwiseParallel(unittest.TestCase):
    def make_scheduler(self, tp, pp, heads):
    ...
    def test_pp_round_trip_stage_local_layers(self):
        for tp, pp, heads in [(2, 2, 1), (4, 2, 1), (8, 2, 2)]:
            with self.subTest(tp=tp, pp=pp, heads=heads):
                store = MemoryRangeStore()
                scheduler = self.make_scheduler(tp, pp, heads)
                ...
                for pp_rank in range(pp):
                    for tp_rank in range(tp):
                        ...
                        # Every PP stage carries a different layer stack; ranks
                        # inside one replicated KV-head group share bytes.
                        value = 1 + pp_rank * 32 + worker.head_or_tp_rank * 8
                        ...
                        # The object size is the group's summed per-layer bytes;
                        # group 0 holds both of this stage's layers.
                        self.assertEqual(sum(worker.group_block_len[0]), 32)
                ...
                self.assertEqual(len(store.complete), pp * heads * 2)
                self.assertEqual(scheduler._lookup_block_key_contiguous(query, 32, 0), 32)
                ...
                # Missing just one stage's keys stops the contiguous hit prefix.
                for missing in {worker._make_mooncake_layerwise_key(b"h1".hex()) for worker, _, _ in workers}:
                    store.complete.remove(missing)
                    self.assertEqual(scheduler._lookup_block_key_contiguous(query, 32, 0), 16)
                    store.complete.add(missing)

    def test_session_keys_are_isolated_by_stage(self):
        """The same block tail must map to a distinct object per stage.

        The key carries the PP coordinate, so two stages holding different layer
        stacks never share an object.
        """
        keys = set()
        for pp_rank in range(2):
            ...
            key = worker._make_mooncake_layerwise_key("shared-request-id_lastblock")
            self.assertNotIn(key, keys)
            keys.add(key)
        self.assertEqual(len(keys), 2)

    def test_namespace_separates_partitions(self):
        ...
        first = mooncake_layerwise.layerwise_topology_namespace(model, parallel)
        ...
        second = mooncake_layerwise.layerwise_topology_namespace(model, parallel)
        self.assertNotEqual(first, second)
        self.assertEqual(mooncake_layerwise.make_block_key("model", "hash", 0), "model@hash@0")
        with self.assertRaisesRegex(ValueError, "topology namespace"):
            mooncake_layerwise.make_block_key("model", "hash", 0, pp_rank=1)
        ...
        with self.assertRaisesRegex(ValueError, "at least one model layer"):
            mooncake_layerwise.layerwise_topology_namespace(model, parallel)

    def test_pcp_remains_rejected(self):
        parallel = SimpleNamespace(
            prefill_context_parallel_size=2, pipeline_parallel_size=2
        )
        with self.assertRaisesRegex(ValueError, "PCP"):
            validate_layerwise_topology(mooncake_layerwise, parallel, True)
        validate_layerwise_topology(mooncake_layerwise, parallel, False)
