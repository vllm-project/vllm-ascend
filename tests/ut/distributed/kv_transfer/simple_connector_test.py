...
class TestSimpleConnector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch('vllm_ascend.distributed.kv_transfer.simple_buffer.SimpleBuffer')
    ...
    @patch('vllm_ascend.distributed.kv_transfer.simple_connector.SimpleBuffer')
    @patch('llm_datadist.LLMDataDist') 
    def test_send_kv_caches_and_hidden_states(self,mock_pipe,mock_buffer,MockLLMDataDist,mock_insert,mock_rank):
        ...
        mock_model_input.attn_metadata.slot_mapping = torch.randint(0, 100, (10,))
    ...
    @patch('torch_npu._npu_reshape_and_cache_siso')
    @patch.object(SimpleConnector, 'select')
    @patch('torch.distributed.get_rank', return_value=0)
    @patch('vllm_ascend.distributed.kv_transfer.simple_connector.SimplePipe')
    @patch('vllm_ascend.distributed.kv_transfer.simple_connector.SimpleBuffer')
    @patch('llm_datadist.LLMDataDist')
    def test_recv_kv_caches_and_hidden_states(
        self, MockLLMDataDist, mock_buffer, mock_pipe, mock_rank, mock_select, mock_reshape_and_cache
    ):
        """Test receiving KV caches and hidden states"""
        connector = SimpleConnector(
            rank=0,
            local_rank=0,
            config=self._create_mock_config("kv_consumer")
        ...
        mock_model_input.attn_metadata.slot_mapping = torch.randint(0, 100, (5,))
        ...
        hidden_states, bypass_flag, updated_input = result
        assert isinstance(hidden_states, torch.Tensor)
        assert bypass_flag is True
        assert isinstance(updated_input, ModelInputForGPUWithSamplingMetadata)
        assert mock_reshape_and_cache.call_count == 3 
