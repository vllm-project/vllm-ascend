...
class TestSimpleConnector(unittest.TestCase):
    ...
    @patch('vllm_ascend.distributed.kv_transfer.simple_connector.SimpleBuffer')
    @patch('llm_datadist.LLMDataDist')
    def test_recv_kv_caches_and_hidden_states(self, MockLLMDataDist,
                                              mock_buffer, mock_pipe,
                                              mock_rank, mock_select,
        ...
        result = connector.recv_kv_caches_and_hidden_states(
            mock_model_executable, mock_model_input, kv_caches)
