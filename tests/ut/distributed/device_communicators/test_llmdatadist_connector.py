...
def test_get_device_ips(mocker: MockerFixture):
    ...
    mock_subprocess_run = mocker.patch(
        "subprocess.run", side_effect=subprocess_run
    )
...
class TestKVTransferEngine():
    ...
    def mock_and_generate_kv_transfer_engine(
        world_size, n_layer, role, local_rank,
        ...
        mock_llm_datadist = mocker.patch(
            "llm_datadist.LLMDataDist"
        )
        
        os.environ["PROMPT_DEVICE_ID"] = "0, 1, 2, 3"
        os.environ["DECODE_DEVICE_ID"] = "4, 5, 6, 7"

        kv_transfer_engine = KVTransferEngine(
            world_size, n_layer, role, local_rank
        )
...
class TestLLMDataDistConnector():
    ...
    def mock_and_generate_LLMDataDistConnector(
        kv_role, tensor_parallel_size, world_size, num_layers,
        rank, local_rank,
        mocker: MockerFixture,
    ):
    ...
    def test_send_kv_caches_and_hidden_states(self, mocker: MockerFixture):
        ...
        def mock_make_cache_desc(
                num_layer, data_shape, data_type, seq_len_dim_index
            ):
            cache_desc_return = mocker.MagicMock()
            cache_desc_return.shape = data_shape
            return cache_desc_return

        mock_cache_desc = mocker.patch(
            "llm_datadist.CacheDesc",
            side_effect=mock_make_cache_desc
        )
        mock_cache_key = mocker.patch(
            "llm_datadist.CacheKey",
            return_value=None
        )
        ...
        kv_caches = [
            torch.arange(
                2 * tokens_num * kv_cache_num_head * head_size
            ).view(2, -1, kv_cache_num_head, head_size).to(dtype)
        ] * num_layers

        hidden_or_intermediate_states = (
            torch.arange(
                tokens_num * hidden_size
            ).view([tokens_num, hidden_size]).to(dtype)
        )
        ...
        scatter_update_call_times = len(seq_lens) * (2 * (end_layer - start_layer) + 2)
    ...
    def test_recv_kv_caches_and_hidden_states(self, mocker: MockerFixture):
        ...
        mock_reshape_and_cache = mocker.patch("torch_npu._npu_reshape_and_cache")
        ...
        reshape_and_cache_call_times = len(seq_lens) * (end_layer - start_layer)
