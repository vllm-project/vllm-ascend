...
from vllm_ascend.distributed.cpu_offload_connector import (ReqMeta,CPUOffloadingConnectorMetadata,
    CPUOffloadingConnectorWorker,CPUOffloadingConnectorScheduler,CPUOffloadingConnector,get_kv_cache_spec)
from vllm.v1.core.sched.output import SchedulerOutput
from unittest.mock import patch, MagicMock, call, mock_open
from vllm_ascend.worker.metadata import MetadataServer
...
class TestCPUOffloadingConnector(unittest.TestCase):
    def setUp(self):
        ...
        self.patches = [
            ...
            patch('vllm_ascend.worker.metadata.MetadataServer.ZMQRPCClient',MagicMock()),
    ...
    def test_initialization(self):
        ...
        self.connector = CPUOffloadingConnector(self.mock_vllm_config,KVConnectorRole.SCHEDULER)
        ...
        self.connector = CPUOffloadingConnector(self.mock_vllm_config,KVConnectorRole.SCHEDULER)
        self.assertIsInstance(self.connector.connector_scheduler, CPUOffloadingConnectorScheduler)
        self.assertIsNone(self.connector.connector_worker)

        #case3
        self.mock_vllm_config.cache_config.enable_prefix_caching = True
        self.connector = CPUOffloadingConnector(self.mock_vllm_config,KVConnectorRole.WORKER)
        self.assertIsNone(self.connector.connector_scheduler)
        self.assertIsInstance(self.connector.connector_worker, CPUOffloadingConnectorWorker)
    ...
    def test_bind_connector_metadata(self):
        ...
        self.connector.connector_worker.bind_connector_metadata.assert_called_once_with(mock_metadata)
    ...
    def test_register_kv_caches(self):
        ...
        self.connector.connector_worker.register_kv_caches.assert_called_once_with(self.test_kv_caches)
    ...
    def test_get_finished(self):
        ...
        self.connector.connector_worker.get_finished = MagicMock(return_value=mock_worker_result)
        result, none_result = self.connector.get_finished(self.test_finished_ids)
    ...
    def test_scheduler_get_num_new_matched_tokens(self):
        ...
        self.connector.connector_scheduler.get_num_new_matched_tokens = MagicMock(return_value=mock_scheduler_result)
    ...
    def test_scheduler_build_connector_meta(self):
        ...
        self.connector.connector_scheduler.build_connector_meta = MagicMock(return_value=mock_meta)
        
        result = self.connector.build_connector_meta(self.test_scheduler_output)
...
class TestCPUOffloadingConnectorScheduler(unittest.TestCase):
    ...
    def test_build_connector_meta(self):
        ...
        mock_new_req = MagicMock(req_id="req1", num_computed_tokens=10, block_ids=[[5,6]])
    ...
    def test_request_finished(self):
        ...
        self.scheduler.zmq_rpc_client.call.assert_has_calls(calls, any_order=False)
...
class TestCPUOffloadingConnectorWorker(unittest.TestCase):
    ...
    @patch('vllm_ascend.worker.metadata.MetadataServerProc.run_metadata_server')
    ...
    def test_register_kv_caches_without_mla(self):
        ...
        kv_caches = {
            "layer1": [torch.randn(1, 10, 512), torch.randn(1, 10, 512)],
            "layer2": [torch.randn(1, 10, 512), torch.randn(1, 10, 512)]
        }
        
        mock_rpc_result = {"cache1": MagicMock(), "cache2": MagicMock()}
        self.worker.zmq_rpc_client.call.return_value = mock_rpc_result
        
        with patch('vllm_ascend.distributed.cpu_offload_connector.get_kv_cache_spec') as mock_get_spec:
            ...
            self.assertEqual(self.worker.cpu_kv_caches, list(mock_rpc_result.values()))
    ...
    def test_load_kv_layer(self,mock_npu_stream):
        ...
        self.worker.gpu_kv_caches_load_iter = iter([
            self.mock_gpu_kv_cache_1,
            self.mock_gpu_kv_cache_2
        ])
    ...
    def test_get_finished_tp1(self):
        ...
        self.worker.save_output_queue.get_nowait.side_effect = ["req_1", "req_2", queue.Empty]
        ...
        self.assertEqual(self.worker.save_output_queue.get_nowait.call_count, 3)
    ...
    def test_get_finished_tp3(self):
        ...
        with patch('threading.Thread') as mock_thread_class:
            ...
            mock_thread_class.assert_called_once_with(
                target=self.worker._sending_finished,
                args=(set(),)  
            )
    ...
    def test_save_listener_normal_operation(self):
        ...
        self.worker.save_input_queue.get.side_effect = [("req_1", req), KeyboardInterrupt]        
        ...
        with patch('torch.npu.stream') as mock_npu_stream:
            ...
            for cpu_kv_caches, gpu_kv_caches in zip(
                self.worker.cpu_kv_caches, self.worker.gpu_kv_caches.values()
            ):
