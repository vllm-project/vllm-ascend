...
from vllm_ascend.distributed.cpu_offload_manager.cpu_kv_cache_manager import (
    CPUCacheStats, CPUKVCacheManager)
...
class TestCPUCacheStats(unittest.TestCase):
    ...
    def test_initialization_with_log_stats_true(self, mock_time):
        ...
        self.assertIsInstance(stats.cpu_prefix_cache_metrics,
                              PrefixCachingMetrics)
    ...
    def test_make_prefix_cache_stats(self):
        self.stats_with_log = CPUCacheStats(enable_prefix_caching=True,
                                            log_stats=True)
        self.stats_without_log = CPUCacheStats(enable_prefix_caching=True,
                                               log_stats=False)
        ...
        self.assertIsNot(self.stats_with_log.prefix_cache_stats,
                         self.original_stats)
        self.assertIsInstance(self.stats_with_log.prefix_cache_stats,
                              PrefixCacheStats)
    ...
    def test_update(self):
        self.enabled_both = CPUCacheStats(enable_prefix_caching=True,
                                          log_stats=True)
        self.disabled_log = CPUCacheStats(enable_prefix_caching=True,
                                          log_stats=False)
        self.disabled_caching = CPUCacheStats(enable_prefix_caching=False,
                                              log_stats=True)
        ...
        self.assertEqual(self.enabled_both.prefix_cache_stats.requests,
                         self.initial_requests + 1)
        self.assertEqual(self.enabled_both.prefix_cache_stats.queries,
                         self.initial_queries + num_tokens)
        self.assertEqual(self.enabled_both.prefix_cache_stats.hits,
...
class TestCPUKVCacheManager(unittest.TestCase):
    ...
    def setUp(self):
        self.kv_cache_spec = FullAttentionSpec(block_size=64,
                                               num_kv_heads=8,
                                               head_size=64,
                                               dtype=torch.float32,
                                               use_mla=False)
        ...
        self.manager_patcher = patch(
            'vllm.v1.core.single_type_kv_cache_manager.get_manager_for_kv_cache_spec',
            return_value=self.mock_manager)
        self.mock_get_manager = self.manager_patcher.start()

        self.stats_patcher = patch(
            'vllm_ascend.distributed.cpu_offload_manager.cpu_kv_cache_manager.CPUCacheStats',
            return_value=self.mock_cpu_cache_stats)
        self.mock_stats_class = self.stats_patcher.start()

        self.block_pool_patcher = patch('vllm.v1.core.block_pool.BlockPool',
                                        return_value=self.mock_block_pool)
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.cache_manager.single_type_manager,
                         self.mock_manager)

        self.assertIsInstance(self.cache_manager.req_to_block_hashes,
                              defaultdict)
        self.assertIsInstance(self.cache_manager.req_to_computed_blocks,
                              defaultdict)
        self.assertIsInstance(self.cache_manager.req_failed_to_allocate,
                              defaultdict)
        self.assertIsInstance(self.cache_manager.req_to_num_tokens,
                              defaultdict)
        self.assertIsInstance(self.cache_manager.req_to_free, defaultdict)
        self.assertEqual(self.cache_manager.cpu_cache_stats,
                         self.mock_cpu_cache_stats)
    ...
    def test_get_matched_num_and_touch(self):
        ...
        mock_computed_blocks = [
            MagicMock(spec=KVCacheBlock),
            MagicMock(spec=KVCacheBlock)
        ]
        mock_block_hashes = [
            MagicMock(spec=BlockHash),
            MagicMock(spec=BlockHash)
        ]

        self.cache_manager.req_to_block_hashes[
            "test_request"] = mock_block_hashes
        self.mock_manager.find_longest_cache_hit.return_value = [
            mock_computed_blocks
        ]
        result = self.cache_manager.get_matched_num_and_touch(mock_request)

        expected_num_tokens = len(
            mock_computed_blocks) * self.cache_manager.block_size
        ...
        self.assertEqual(
            self.cache_manager.req_to_computed_blocks["test_request"],
            mock_computed_blocks)
        self.mock_block_pool.touch.assert_called_once_with(
            [mock_computed_blocks])
    ...
    def test__release_ahead_touch(self):
        ...
        self.cache_manager.req_to_computed_blocks[
            request_id] = mock_computed_blocks
    ...
    def test_allocate_slots_normal(self):
        ...
        self.mock_manager.single_type_manager.get_num_blocks_to_allocate.side_effect = [
            2, 3
        ]
        self.mock_block_pool.get_num_free_blocks.return_value = 10
        ...
        result = self.cache_manager.allocate_slots(req_to_num_tokens,
                                                   unallocated_req_ids)
    ...
    def test_allocate_slots_with_computed_blocks(self):
        ...
        self.cache_manager.single_type_manager.allocate_new_blocks.return_value = self.create_mock_blocks(
            2, start_id=1)
        ...
        self.cache_manager.single_type_manager.save_new_computed_blocks.assert_called_once_with(
            "req1", computed_blocks)
    ...
    def test_record_request_cache_and_free_slots(self):
        ...
        self.assertEqual(self.cache_manager.req_to_free[test_request_id],
                         test_request)
    ...
    def test_free_slots(self):
        ...
        self.cache_manager.req_to_block_hashes[
            self.test_request_id] = "hash_123"
        self.cache_manager.req_to_computed_blocks[self.test_request_id] = [
            MagicMock()
        ]
        ...
        self.cache_manager._release_ahead_touch.assert_called_once_with(
            self.test_request_id)
        self.cache_manager.single_type_manager.free.assert_called_once_with(
            self.test_request_id)

        self.assertNotIn(self.test_request_id,
                         self.cache_manager.req_to_block_hashes)
        self.assertNotIn(self.test_request_id,
                         self.cache_manager.req_to_computed_blocks)
        self.assertNotIn(self.test_request_id,
