...
class TestUpstreamConfigCompatibility(TestBase):
    ...
    def test_mc2_optimized_comm_rejects_more_than_512_experts(self, mock_device_type):
        config = AscendConfig(
            sparse_kv_offload_config=SimpleNamespace(enabled=False),
            enable_mc2_optimized_comm=True,
        )
        ...
        with self.assertRaisesRegex(ValueError, "supports at most 512 experts"):
            config._validate_mc2_optimized_comm(vllm_config)

    @patch("vllm_ascend.utils.get_ascend_device_type", return_value=AscendDeviceType.A5)
    def test_mc2_optimized_comm_rejects_unsupported_device(self, mock_device_type):
        config = AscendConfig(
            sparse_kv_offload_config=SimpleNamespace(enabled=False),
            enable_mc2_optimized_comm=True,
        )
