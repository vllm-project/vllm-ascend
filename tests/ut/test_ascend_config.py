...
class TestSubconfigPydanticTypeValidation(TestBase):
    ...
    def test_ascend_fusion_config_string_false_disables(self):
        ...
        self.assertFalse(
            AscendFusionConfig(fusion_ops_gmmswigluquant="false").fusion_ops_gmmswigluquant  # type: ignore[arg-type]
        )
        self.assertTrue(
            AscendFusionConfig(fusion_ops_gmmswigluquant="true").fusion_ops_gmmswigluquant  # type: ignore[arg-type]
        )
...
class TestTopLevelSwitchTypeValidation(TestBase):
    ...
    def test_fused_mc2_is_disabled_for_unsupported_megamoe_config(
        ...
        # After the megamoe rollback (#15267), enable_fused_mc2=1 forces
        # _MEGA_MOE_SUPPORTED=False (dispatch_ffn_combine), so the unsupported
        # config downgrade never fires there. Only the megamoe-testing path
        # (enable_fused_mc2=2 with cann_ops_transformer present) keeps
        # _MEGA_MOE_SUPPORTED=True and is downgraded to 0 on an unsupported
        # model config.
        vc = VllmConfig()
