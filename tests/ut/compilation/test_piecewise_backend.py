...
from vllm_ascend.compilation.piecewise_backend import ConcreteSizeEntry, NPUPiecewiseBackend
...
class TestNPUPiecewiseBackend(TestBase):
    ...
    def test_init(self):
        ...
        self.assertEqual(backend.piecewise_compile_index, self.piecewise_compile_index)
        self.assertEqual(backend.total_piecewise_compiles, self.total_piecewise_compiles)
        self.assertEqual(backend.sym_shape_indices, self.sym_shape_indices)
        self.assertEqual(backend.compiled_graph_for_general_shape, self.compiled_graph_for_general_shape)
    ...
    def test_check_for_ending_compilation(self, mock_end_monitoring_torch_compile):
        backend = NPUPiecewiseBackend(
            self.graph, self.vllm_config, self.graph_pool,
            self.piecewise_compile_index, self.total_piecewise_compiles,
            self.sym_shape_indices, self.compiled_graph_for_general_shape,
