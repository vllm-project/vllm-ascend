...
from vllm_ascend.compilation.npugraph_ex_passes.add_qk_norm_rope_fusion import \
    _extra_stream_scope_check


def test_extra_stream_scope_check_logic():
    """
    Test the _extra_stream_scope_check logic used in both fusion patterns.
    ...
    assert _extra_stream_scope_check(
    ...
    # Test 5: empty → OK
    assert _extra_stream_scope_check(MockMatch([])) is True
