...
def test_a5_capability_uses_instantiated_layer_contract(a5_api_capability):
    ...
    assert capability.activation.name == "situ"
...
def test_a5_capability_accepts_standard_swiglu(a5_api_capability):
    capability = evaluate_cann_mega_moe_layer(
        _moe_config(hidden_dim=4096, num_experts=256, experts_per_token=8),
        _quant_method(),
        "silu",
        device_type=AscendDeviceType.A5,
        api_capability=a5_api_capability,
    ...
    assert capability.supported
    assert capability.activation is not None
    assert capability.activation.name == "swiglu"
