...
def test_a5_pure_prefill_only_without_capacity_gate(
    monkeypatch, tokens, pure_prefill, supported, switch, enabled, expected, dp_size
):
    config = SimpleNamespace(
        model_config=SimpleNamespace(hf_text_config=SimpleNamespace(top_k_experts=1)),
        parallel_config=SimpleNamespace(
            enable_expert_parallel=True, world_size_across_dp=8, data_parallel_size=dp_size
        ),
        lora_config=None,
    )
    ...
    if dp_size > 1 and expected == afc.MoECommType.FUSED_MC2:
        expected = afc.MoECommType.MC2 if tokens <= 8 else afc.MoECommType.ALLTOALL
    assert (
...
@pytest.mark.parametrize("weight_layout", ["legacy", "stack", "expert_list"])
def test_fused_mc2_routes_supported_situ_to_mega_moe(weight_layout):
    impl = _implementation()
    request = _request()
    quant_method = None
    if weight_layout != "legacy":
        quant_method = AscendW4A8MXFPDynamicFusedMoEMethod.__new__(AscendW4A8MXFPDynamicFusedMoEMethod)
        quant_method.use_expert_weight_list = weight_layout == "expert_list"
        layer = torch.nn.Module()
        for name, value in (
            ("w13_weight", request.weights.w1),
            ("w2_weight", request.weights.w2),
            ("w13_weight_scale", request.weights.w1_scale),
            ("w2_weight_scale", request.weights.w2_scale),
        ):
            if quant_method.use_expert_weight_list:
                setattr(layer, name + "_list", [expert.transpose(0, 1) for expert in value.unbind()])
            else:
    ...
    assert len(args[3]) == (2 if weight_layout == "expert_list" else 1)
