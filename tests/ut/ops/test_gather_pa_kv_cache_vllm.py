...
def _build_block_tables(batch, per_batch_lens, block_size, num_blocks,
                        seq_offset=None):
...
def cal_reference(key_cache, value_cache, block_tables, seq_lens,
...
# ======================== cumsum=True ========================
def test_gather_pa_kv_cache_cumsum(head_size_k, head_size_v):
    ...
    key_expected, value_expected = cal_reference(
    ...
    torch.testing.assert_close(key_expected, result_key.cpu())
    torch.testing.assert_close(value_expected, result_value.cpu())
...
# ======================== cumsum=False ========================
def test_gather_pa_kv_cache_non_cumsum(head_size_k, head_size_v):
...
# ======================== with seq_offset ========================
def test_gather_pa_kv_cache_with_seq_offset(head_size_k, head_size_v):
...
# ======================== random params ========================
def test_gather_pa_kv_cache_random(head_size_k, head_size_v):
...
def test_gather_pa_kv_cache_non_contiguous(head_size_k, head_size_v):
    ...
    kv_shape = [2, num_blocks, block_size, num_heads, head_size]
    ...
    assert not key_cache_npu.is_contiguous(), "key_cache_npu need contiguous"
    assert not value_cache_npu.is_contiguous(), "key_cache_npu need contiguous"

    key_ref = torch.zeros(
        (total_tokens, num_heads, head_size), dtype=torch.float16
    )
    value_ref = torch.zeros(
        (total_tokens, num_heads, head_size), dtype=torch.float16
    )
