...
class TestFusedExpertsW8A8Dynamic:
    ...
    def test_fused_experts_with_allgather(self, mock_dist_env,
                                          global_num_experts):
        ...
        top_k = 8
        layer = MagicMock()
        layer.w13_weight = torch.randn(1,
                                       7168,
                                       4096,
                                       dtype=torch.int8,
                                       device="npu")
        layer.w13_weight_scale = torch.randn(1,
                                             4096,
                                             dtype=torch.bfloat16,
                                             device="npu")
        layer.w2_weight = torch.randn(1,
                                      2048,
                                      7168,
                                      dtype=torch.int8,
                                      device="npu")
        torch_npu.npu_format_cast_(layer.w2_weight, 29)
        layer.w2_weight_scale = torch.randn(1,
                                            7168,
                                            dtype=torch.bfloat16,
                                            device="npu")
