...
def test_enable_lora_uses_qkvz_module_forward(monkeypatch):
    """An LoRA-enabled engine must not bypass the LoRA-capable projection."""
    wrapper = _GDNQkvzForwardWrapper(
        num_k_heads=2,
        head_k_dim=4,
        num_v_heads=2,
...
@pytest.mark.parametrize(
    ("lora_config", "expected"),
    [(None, False), (object(), True)],
)
def test_gdn_init_caches_enable_lora(monkeypatch, lora_config, expected):
    """The layer flag must reflect the engine's startup LoRA configuration."""

    def original_init(self, **kwargs):
        del kwargs

    monkeypatch.setattr(patch_qwen3_5, "_GDN_ORIGINAL_INIT", original_init)
    layer = object.__new__(QwenGatedDeltaNetAttention)

    patch_qwen3_5._gdn_init_with_lora_config(
        layer,
        config=object(),
        vllm_config=SimpleNamespace(lora_config=lora_config),
    )

    assert layer.enable_lora is expected
