...
def load_forward(metadata):
    ...
    scope = dict(
        ...
        wait_for_kv_layer_from_connector=lambda *_: None,
...
@pytest.mark.parametrize(
    "mode", ["spec", "decode", "large_decode", "prefill", "mixed_prefill", "mixed", "mixed_holes", "idle"]
)
@pytest.mark.parametrize("padding", [0, 2])
def test_forward_output_skips_redundant_initialization(mode, padding):
    n, h, d = (1536 if mode == "large_decode" else 6), 2, 4
    ...
    metadata = SimpleNamespace(
        num_actual_tokens=n,
        num_prefills=int(mode in ("prefill", "mixed_prefill")),
        num_decodes=int(mode in ("decode", "large_decode", "mixed_prefill", "mixed", "mixed_holes")),
        num_decode_tokens=2 if mode == "mixed_prefill" else n,
    ...
    def prefill_norm(x, g, *, out):
        prefill_norm_calls.append(x)
        return norm(x, g, out=out)

    norm.forward_prefill = prefill_norm

    def unexpected_prefill(*args, **kwargs):
        raise AssertionError("decode/speculation must not enter the prefill operator path")
    ...
    attention = SimpleNamespace(
        ...
        _run_prefill=recurrent if mode in ("prefill", "mixed_prefill") else unexpected_prefill,
        o_norm=norm,
    )
    ...
    assert len(prefill_norm_calls) == int(mode == "prefill")
    if mode in ("spec", "decode", "large_decode", "prefill"):
        assert norm_inputs[0] is recurrent_outputs[0]
        assert "aten.zero_.default" not in before_norm
    elif mixed or mode == "mixed_prefill":
        assert "aten.zero_.default" not in before_norm
    else:
    ...
    if mode in ("spec", "decode", "large_decode", "prefill"):
        ...
        assert "aten.zero_.default" not in trace.ops
        assert len(prefill_norm_calls) == 2 * int(mode == "prefill")
