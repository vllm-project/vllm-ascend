...
_METHOD = next(
    node for node in ast.walk(_TREE) if isinstance(node, ast.FunctionDef) and node.name == "_run_causal_conv1d"
)
...
def test_rank_two_cache_is_passed_to_native_and_keeps_state_alias():
    ...
    assert args[6].tolist() == [[2, 100, 101], [0, 200, 201]]
...
def test_bool_initial_flags_and_int32_metadata_keep_existing_storage():
    ...
    cache = torch.tensor([[0, 100], [1, 200]], dtype=torch.int32)
    with patch.object(torch.ops._C_ascend, "npu_causal_conv1d_custom", create=True) as native:
        _RUN(
            output.clone(),
        ...
        assert native.call_args.args[6] is cache


def test_initial_flag_layout_is_left_to_native_auto_contiguous():
    initial = torch.tensor([True, False, False, True])[::2]
    assert not initial.is_contiguous()
    with patch.object(torch.ops._C_ascend, "npu_causal_conv1d_custom", create=True) as native:
        _RUN(
            torch.empty(2, 16, dtype=torch.bfloat16),
            torch.empty(4, 16, dtype=torch.bfloat16),
            torch.empty(2, 3, 16, dtype=torch.bfloat16),
