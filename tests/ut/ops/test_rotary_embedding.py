...
from vllm_ascend.attention.attention_v1 import AttentionV1, __set_cos_sin_cache
import torch_npu
...
def patch_set_cos_sin_cache(monkeypatch):
    monkeypatch.setattr("my_module.attention.attention_v1.__set_cos_sin_cache", lambda *args, **kwargs: None)
    yield
...
def test_rope_forward_basic():
    ...
    query = torch.arange(batch * seq_len * embed_dim, dtype=torch.float32).view(batch, seq_len, embed_dim)
    key   = torch.arange(batch * seq_len * embed_dim, dtype=torch.float32).view(batch, seq_len, embed_dim) + 100.0
