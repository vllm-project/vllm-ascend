...
class TestW8A8FusedMoEMethod(unittest.TestCase):
    ...
    def test_get_fused_mc2_weights_fused_mc2_scale_flag(self):
        ...
        with (
            ...
            patch("vllm_ascend.quantization.methods.w8a8_dynamic._MEGA_MOE_SUPPORTED", False),
        ...
        self.assertTrue(torch.equal(weights.w1_scale_bias[0], torch.tensor([], dtype=torch.float32)))
...
class TestUnifiedApplyActivation(unittest.TestCase):
    ...
    def test_swigluoai_uninterleave_uses_clipped_swiglu(self):
        with patch(f"{MOE_MLP}.torch_npu.npu_clipped_swiglu", return_value="out", create=True) as mock_act:
            out = _unified_apply_activation(
                _mlp_compute_input(
                    activation=MoEActivation.SWIGLUOAI_UNINTERLEAVE,
                    swiglu_limit=3.0,
