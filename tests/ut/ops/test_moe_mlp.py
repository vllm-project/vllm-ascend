...
class TestSwigluMxQuant(unittest.TestCase):
    def test_uses_scale_alg_one_and_normalizes_scale(self):
        hidden_states = torch.randn(2, 8)
        quantized = torch.randn(2, 4)
        raw_scale = torch.randn(2, 1)
        ...
        with (
            ...
            patch(
                ...
                return_value=(quantized, raw_scale),
                create=True,
            ...
            output, output_scale = _swiglu_mx_quant(
                hidden_states,
        ...
        self.assertIs(output_scale, normalized_scale)
        call_kwargs = mock_swiglu_mx_quant.call_args.kwargs
        self.assertIs(call_kwargs["x"], hidden_states)
        self.assertEqual(call_kwargs["scale_alg"], 1)
        self.assertEqual(call_kwargs["clamp_limit"], 7.0)
        self.assertEqual(call_kwargs["glu_alpha"], 1.5)
        self.assertEqual(call_kwargs["glu_bias"], 0.25)
        mock_normalize.assert_called_once_with(raw_scale)
...
class TestQuantApplyMlpMxfpSwigluOAI(_GeluPathBase):
    ...
    def test_uses_fused_swiglu_mx_quant_for_mc2_and_alltoall(self):
