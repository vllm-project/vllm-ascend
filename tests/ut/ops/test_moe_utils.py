...
class TestFusionFlags(unittest.TestCase):
    def test_custom_gmm_swiglu_requires_fusion_dynamic_eplb(self):
        ...
        self.assertFalse(_gmm_swiglu_quant_fusion_enabled(False, False, False))
        self.assertFalse(_gmm_swiglu_quant_fusion_enabled(False, True, True))
        self.assertTrue(_gmm_swiglu_quant_fusion_enabled(True, False, False))

    def test_fusion_flag_excludes_swigluoai_uninterleave(self):
        self.assertFalse(_gmm_swiglu_quant_fusion_enabled(True, False, False, activation="swigluoai_uninterleave"))


class TestSwigluScaleHelpers(unittest.TestCase):
    def test_require_single_tensor_unwraps_single_element_list(self):
        tensor = torch.randn(2, 3)
        self.assertIs(_require_single_tensor_for_swiglu_quant([tensor], name="w1"), tensor)
        self.assertIs(_require_single_tensor_for_swiglu_quant(tensor, name="w1"), tensor)

    def test_require_single_tensor_rejects_multi_element_list(self):
        with self.assertRaises(ValueError):
            _require_single_tensor_for_swiglu_quant([torch.randn(1), torch.randn(1)], name="w1")
    ...
    def test_prepare_dequant_swiglu_weight_scale_stacks_and_casts(self):
        ...
        out = _prepare_dequant_swiglu_weight_scale(scales)
        self.assertEqual(out.dtype, torch.float32)
        self.assertEqual(out.dim(), 2)

        single = torch.randn(4, dtype=torch.float16)
        out_single = _prepare_dequant_swiglu_weight_scale([single])
