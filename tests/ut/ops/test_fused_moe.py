...
def moe_reduce_ops_on_cpu(monkeypatch):
    ...
    They are registered for PrivateUse1 and Meta only, so going through the
    dispatcher with CPU tensors raises NotImplementedError. What these tests
    exercise is the call sites and the predicates, so bind the namespace entries
    to the very functions the op registration wraps.
    """
    for name, impl in (
