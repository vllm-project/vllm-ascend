...
def setup_vllm_config_mock(mocker: MockerFixture):
    ...
    mocker.patch(
        "vllm_ascend.ops.fused_moe.fused_moe.get_current_vllm_config",
...
def mock_dist_env(mocker: MockerFixture):
    ...
    with (
        ...
        patch(
            "vllm_ascend.ops.fused_moe.fused_moe.get_ep_group",
            return_value=mock_ep_and_mc2_group(mocker),
        ),
        patch(
        ...
        patch(
            "vllm_ascend.ops.fused_moe.fused_moe.get_mc2_group",
            return_value=mock_ep_and_mc2_group(mocker),
        ),
        patch(
            "vllm_ascend.ops.fused_moe.fused_moe.get_tp_group",
        ...
        patch(
            "vllm_ascend.ops.fused_moe.fused_moe.get_dp_group",
            return_value=mock_dp_and_tp_group(mocker),
        ),
        patch(
            "vllm.model_executor.layers.fused_moe.layer.get_dp_group",
            return_value=mock_dp_and_tp_group(mocker),
        ),
        patch(
...
def mock_moe_env(mocker: MockerFixture):
    with (
        patch(
            ...
            return_value=(torch.randn(8, 2), torch.randint(0, 8, (8, 2)), None),
        ),
        patch(
