...
def test_active_shared_expert_lora_uses_dense_wrappers(monkeypatch):
    base_gate_up = SimpleNamespace(weight_scale=torch.ones(1))
    base_down = SimpleNamespace(weight_scale=torch.ones(1))
    shared_experts = AscendSharedExperts.__new__(AscendSharedExperts)
    shared_experts.layer = SimpleNamespace(
        gate_up_proj=SimpleNamespace(base_layer=base_gate_up),
        down_proj=SimpleNamespace(base_layer=base_down),
    )
