...
def create_common_attn_metadata(
    ...
    num_prompt_tokens_cpu = torch.tensor(
        [
            context_lens[i] +
            batch_spec.query_lens[i] if batch_spec.query_lens[i] > 1 else context_lens[i]
            for i in range(batch_spec.batch_size)
        ],
        dtype=torch.int32,
    )
...
def test_ascend_gdn_attention_uses_ascend_backend():
    assert AscendGatedDeltaNetAttention.get_attn_backend(
        object()) is AscendGDNAttentionBackend
    assert AscendGDNAttentionBackend.get_builder_cls(
    ) is AscendGDNAttentionMetadataBuilder
...
def test_sequence_index_buffers_cover_spec_decode_when_cudagraph_disabled():
    ...
    assert builder.spec_sequence_indices_cpu.numel(
...
def _assert_non_spec_conv1d_args_match_metadata(attn_metadata) -> None:
    ...
    assert torch.equal(conv1d_meta.query_start_loc,
                       attn_metadata.non_spec_query_start_loc)
    assert torch.equal(
        _cache_index_first_column(conv1d_meta.cache_indices),
        attn_metadata.non_spec_state_indices_tensor,
    )
    assert torch.equal(conv1d_meta.initial_state_mode,
...
def test_non_spec_prefill_metadata_uses_prefill_tail_for_fused_operator():
    ...
    assert torch.equal(conv1d_meta.query_start_loc,
                       torch.tensor([0, 1, 9, 13], dtype=torch.int32))
    assert torch.equal(_cache_index_first_column(
        conv1d_meta.cache_indices), torch.tensor([0, 1, 2], dtype=torch.int32))
    assert torch.equal(conv1d_meta.initial_state_mode,
                       torch.tensor([True, True, True]))
    assert torch.equal(prefill_metadata.actual_seq_lengths,
...
def test_spec_conv1d_args_use_device_cache_and_accepted_tokens():
    ...
    assert torch.equal(query_start_loc, torch.tensor(
        [0, 4, 8], dtype=torch.int32))
    assert torch.equal(
        spec_conv1d_meta.cache_indices,
        torch.tensor([[10, 11, 12, 13], [20, 21, 22, 23]], dtype=torch.int32),
    )
    assert torch.equal(spec_conv1d_meta.num_accepted_tokens,
                       num_accepted_tokens)
    assert torch.equal(
...
def test_full_graph_spec_actual_seq_lengths_use_padded_builder_buffer():
    ...
    assert (
        attn_metadata.spec_decode_metadata.actual_seq_lengths.data_ptr(
        ) == builder.spec_actual_seq_lengths.data_ptr()
    )
    assert torch.equal(
        attn_metadata.spec_decode_metadata.actual_seq_lengths,
...
def test_causal_conv1d_cache_indices_use_device_block_table():
    ...
    assert torch.equal(_cache_index_first_column(
        conv1d_meta.cache_indices), torch.tensor([40, 41], dtype=torch.int32))
    assert torch.equal(conv1d_meta.initial_state_mode,
...
def test_mamba_align_cache_indices_follow_device_seq_lens():
    ...
    common_attn_metadata.block_table_tensor = torch.arange(
        20, dtype=torch.int32).view(2, 10)
    common_attn_metadata._seq_lens_cpu = torch.tensor(
        [5, 13], dtype=torch.int32)
    builder = _make_builder(
        device=torch.device("cpu"),
        num_heads=32,
        num_speculative_tokens=0,
...
def test_builder_keeps_only_fused_operator_metadata():
    ...
    attn_metadata = builder.build(
        ...
        common_attn_metadata,
        num_accepted_tokens=torch.ones(
            batch_spec.batch_size, dtype=torch.int32),
        num_decode_draft_tokens_cpu=torch.tensor(
            [-1, 3, -1, -1], dtype=torch.int32),
    )

    assert not hasattr(ascend_gdn_attn_builder, "GDNChunkedPrefillMetadata")
    assert not hasattr(ascend_gdn_attn_builder,
    ...
    assert torch.equal(prefill_metadata.actual_seq_lengths,
                       torch.tensor([4, 0, 8], dtype=torch.int32))
    assert torch.equal(prefill_metadata.non_empty_indices,
...
@pytest.mark.parametrize(
    "batch_spec",
    [
        BatchSpec(seq_lens=[1, 1, 1], query_lens=[
                  1, 1, 1], name="decode_only"),
