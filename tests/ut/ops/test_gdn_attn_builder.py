...
def test_non_spec_prefill_metadata_uses_prefill_tail_for_fused_operator():
    ...
    # actual_seq_lengths is the prefill tail: all non-spec lengths with the
    # leading decode rows skipped (num_decodes == 1 here).
    assert torch.equal(
        prefill_metadata.actual_seq_lengths,
        torch.diff(conv1d_meta.query_start_loc)[attn_metadata.num_decodes :],
    )
...
def test_builder_keeps_only_fused_operator_metadata():
    ...
    assert attn_metadata.token_chunk_offset_ptr is None
    prefill_metadata = attn_metadata.non_spec_prefill_metadata
    # Request 0 (seq_len=8 > query_len=4 == num_spec+1, stateful) is folded
    # into spec by _fold_spec_sized_prefill_chunks_into_spec, so it does not
    # appear in the prefill length table.
    assert torch.equal(prefill_metadata.actual_seq_lengths, torch.tensor([0, 8], dtype=torch.int32))
    assert torch.equal(prefill_metadata.non_empty_indices, torch.tensor([1], dtype=torch.int64))
