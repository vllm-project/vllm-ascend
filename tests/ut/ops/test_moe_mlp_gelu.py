...
These tests cover the GELU path added to ``quant_apply_mlp``. ``quant_apply_mlp``
previously hardcoded SwiGLU, so quantized MoE experts of GELU models (e.g.
Gemma4, ``activation='gelu_tanh'``) ran the wrong activation. The new path runs
dequant GMM -> GELU -> (re)quant -> GMM2 (early-return or guarded, depending on
the variant).
