...
patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.get_pp_group',
      return_value=_mock_pp_group).start()
patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.get_tp_group',
      return_value=_mock_tp_group).start()
patch(
    'vllm_ascend.distributed.kv_p2p.mooncake_connector.get_tensor_model_parallel_world_size',
    return_value=4).start()
patch(
    'vllm_ascend.distributed.kv_p2p.mooncake_connector.get_tensor_model_parallel_rank',
    return_value=0).start()
patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.get_pcp_group',
...
class TestGetAndClearFinishedSingleRequests(unittest.TestCase):
    ...
    @patch("vllm_ascend.distributed.kv_p2p.mooncake_connector.logger")
...
class TestSocketManagement(unittest.TestCase):
    ...
    @patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.zmq.Context')
    @patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.make_zmq_socket')
...
class TestCoreFunctionality(unittest.TestCase):
    ...
    def test_transfer_kv_cache(self, mock_get_meta):
        with patch(
                'vllm_ascend.distributed.kv_p2p.mooncake_connector.get_ascend_config'
        ) as mock_config:
...
class TestMetadataHandling(unittest.TestCase):
    ...
    @patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.ensure_zmq_send')
    @patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.ensure_zmq_recv')
...
class TestMooncakeConnectorSchedulerMatchedTokens(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.p1 = patch(
            'vllm_ascend.distributed.kv_p2p.mooncake_connector.init_ascend_config',
            new=MagicMock())
        self.p2 = patch(
            'vllm_ascend.distributed.kv_p2p.mooncake_connector.get_ascend_config',
...
class TestMooncakeConnectorWorker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patches = [
            ...
            patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.get_ip',
                  mock_get_ip),
            patch(
                'vllm_ascend.distributed.kv_p2p.mooncake_connector.string_to_int64_hash',
                mock_string_to_int64_hash),
            patch(
                'vllm_ascend.distributed.kv_p2p.mooncake_connector.global_te.get_transfer_engine',
                return_value=self.mock_transfer_engine),
            patch(
                'vllm_ascend.distributed.kv_p2p.mooncake_connector.global_te.register_buffer',
                return_value=None),
            patch(
                'vllm_ascend.distributed.kv_p2p.mooncake_connector.KVCacheSendingThread',
                MagicMock()),
            patch(
                'vllm_ascend.distributed.kv_p2p.mooncake_connector.KVCacheRecvingThread',
                MagicMock()),
            patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.logger',
                  MagicMock()),
            patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.threading.Event',
                  MagicMock()),
            patch(
                'vllm_ascend.distributed.kv_p2p.mooncake_connector.get_ascend_config',
                return_value=MagicMock()),
    ...
    def test_get_remote_tp_rank(self):
        ...
        def get_tp_rank(prefill_tp_size: int, prefill_pp_size: int,
            ...
            with patch('vllm_ascend.distributed.kv_p2p.mooncake_connector.get_ascend_config',
                    return_value=MagicMock()), \
