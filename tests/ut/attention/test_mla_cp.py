...
def get_pcp_split_info(pcp_rank, pcp_size, seq_lens):
    ...
    for i, seq_len in enumerate(seq_lens):
        ...
        q_tail_idx.extend(
            list(range(q_req_offset + chunk_len, q_req_offset + chunk_len * 2))
        )
...
class TestAscendMLAImpl(TestBase):
    ...
    def test_mla_preprocess_dcp(
        ...
        kv_cache0 = torch.randn(
            block_num, block_size, self.impl.num_kv_heads, self.impl.kv_lora_rank
        )
        kv_cache1 = torch.randn(
            block_num, block_size, self.impl.num_kv_heads, self.impl.qk_rope_head_dim
        )
        ...
        self.impl._q_proj_and_k_up_proj.return_value = [
            torch.randn(
                attn_metadata.num_decodes, self.impl.num_heads, self.impl.kv_lora_rank
            ),
            torch.randn(
    ...
    def test_process_attn_out_lse(self):
        ...
        decode_metadata.batch_seq_mask = torch.tensor([True, False], dtype=torch.bool)

        result = _process_attn_out_lse(
            attn_output, softmax_lse, decode_metadata.batch_seq_mask
        )
    ...
    def test_forward_decode_pcp_dcp(
        ...
        attn_metadata.decode.batch_seq_mask = torch.tensor(
        ...
        mock_npu_attention_update.return_value = (
            torch.randn(B, self.impl.num_heads, self.impl.kv_lora_rank),
            None,
        )
    ...
    def test_compute_prefill_context_with_dcp_pcp(
        self, mock_all2all, mock_dcp, mock_pcp, mock_ring, mock_load
    ):
        def mock_ring_attn(
            q_nope,
            q_rope,
        ...
        for test_case in test_cases:
            ...
            for num_all_rank_context in nums_all_rank_context:
                ...
                nums_context_per_rank.append(
                    num_all_rank_context // (pcp_size * dcp_size)
                )
            ...
            prefix_lse = torch.randn(
                sum(nums_tokens_per_rank), self.impl.num_heads, dtype=torch.float16
            )
            ...
            self.assertEqual(
                mock_reorg.call_count, iters * (1 if dcp_size * pcp_size > 1 else 0)
            )
    ...
    def test_reorg_kvcache_with_dcp_pcp(self, mock_all2all, mock_dcp, mock_pcp):
        ...
        for test_case in test_cases:
            ...
            for i in range(len(chunked_context.seq_tot)):
                ...
                self.assertEqual(
                    result_kv.shape[0], chunked_context.cu_seq_lens_lst[i][-1]
                )
                self.assertEqual(
                    result_k_pe.shape[0], chunked_context.cu_seq_lens_lst[i][-1]
                )

                self.assertEqual(
                    mock_dcp.all_gather.call_count, (1 if dcp_size > 1 else 0)
                )
                self.assertEqual(
                    mock_pcp.all_gather.call_count, (1 if pcp_size > 1 else 0)
                )
    ...
    @patch(
        "vllm.distributed.parallel_state._PCP",
        new_callable=lambda: MagicMock(spec=GroupCoordinator),
    )
    @patch("vllm_ascend.attention.context_parallel.common_cp.get_dcp_group")
    @patch(
        "vllm.distributed.parallel_state._DCP",
        new_callable=lambda: MagicMock(spec=GroupCoordinator),
    )
    def test_npu_attention_update_with_dcp_pcp(
        ...
        for test_case in test_cases:
            ...
            def mock_npu_attention_update_effect(
                attn_lse_split_cp, attn_out_split_cp, update_type
            ):
                return torch.randn_like(attn_out_split_cp[0]), torch.randn_like(
                    attn_lse_split_cp[0]
                )
            ...
            attn_out_lse = torch.randn(
                self.impl.pcp_size * NUM_TOKENS,
    ...
    def test_attention_with_mask_and_nomask_with_dcp_pcp(self, mock_npu_ring_mla):
        ...
        for test_case in test_cases:
            ...
            q_nope = torch.randn(
                seq_len_q, num_heads, qk_nope_head_dim, dtype=torch.float16
            )
            q_pe = torch.randn(
                seq_len_q, num_heads, qk_rope_head_dim, dtype=torch.float16
            )
            k_nope = torch.randn(
                seq_len_k, num_heads, qk_nope_head_dim, dtype=torch.float16
            )
            k_pe = torch.randn(
                seq_len_k, num_heads, qk_rope_head_dim, dtype=torch.float16
            )
            ...
            for rank in range(pcp_size):
                ...
                self.assertEqual(
                    output_head.shape, (q_head_idx.shape[0], num_heads, v_head_dim)
                )
                self.assertEqual(lse_head.shape, (num_heads, q_head_idx.shape[0]))
                self.assertEqual(
                    mock_npu_ring_mla.call_count,
                ...
                self.assertEqual(
                    output_tail.shape, (q_tail_idx.shape[0], num_heads, v_head_dim)
                )
    ...
    @patch_distributed_groups(dcp_size=2, pcp_size=2)
    def test_process_attn_out_lse_with_dcp_pcp(
        self, mock_all_to_all, mock_dcp, mock_pcp
    ):
        ...
        for test_case in test_cases:
            ...
            result = _process_attn_out_lse(attn_output, softmax_lse, batch_seq_mask)
            # [PCP * S, DCP * H, D + 1]
    ...
    def test_forward_prefill_cp_with_dcp_pcp(self, mock_npu_ring_mla):
        ...
        self.impl._attention_with_mask_and_nomask.side_effect = (
        ...
        kv_c_and_k_pe_cache = (
            torch.randn(
                block_num, block_size, 1, self.impl.q_lora_rank, dtype=torch.float16
            ),
            torch.randn(
                block_num,
                block_size,
        ...
        for test_case in test_cases:
            ...
            for num_tokens in scheduled_tokens:
                assert (
                    num_tokens % (2 * pcp_size) == 0
                )  # padded head&tail compute balance
                nums_tokens_per_rank.append(num_tokens // pcp_size)
            seq_len_q, seq_len_k = sum(nums_tokens_per_rank), sum(scheduled_tokens)
            ...
            for rank in range(pcp_size):
                ...
                attn_metadata.prefill.pcp_metadata.kv_with_q_head_nomask_idx = (
                    kv_with_q_head_nomask_idx
                )

                attn_metadata.prefill.pcp_metadata.kv_with_q_head_mask_idx = (
                    kv_with_q_head_mask_idx
                )
                attn_metadata.prefill.pcp_metadata.kv_with_q_tail_nomask_idx = (
                    kv_with_q_tail_nomask_idx
                )
