...
class TestLargeAttentionMerge(unittest.TestCase):
    @patch("vllm_ascend.attention.context_parallel.common_cp.torch_npu.npu_attention_update")
    def test_seventeen_partials_match_dense_attention(self, fused_update):
        torch.manual_seed(17)
        parts, tokens, heads, keys, dim = 17, 3, 2, 5, 8
        scores = torch.randn(parts, tokens, heads, keys) * 5
        values = torch.randn(parts, tokens, heads, keys, dim)
        partial_out = (scores.softmax(-1).unsqueeze(-1) * values).sum(-2)
        partial_lse = scores.logsumexp(-1, keepdim=True)
        history = torch.cat((partial_out[:-1], partial_lse[:-1]), dim=-1)
        history = history.permute(1, 0, 2, 3).reshape(tokens, 16 * heads, dim + 1)
        actual = _npu_attention_update(
            dim, history, partial_out[-1], partial_lse[-1], dcp_size=16
        )
        dense_scores = scores.permute(1, 2, 0, 3).flatten(-2)
        dense_values = values.permute(1, 2, 0, 3, 4).reshape(tokens, heads, parts * keys, dim)
        expected = (dense_scores.softmax(-1).unsqueeze(-1) * dense_values).sum(-2)
        torch.testing.assert_close(actual, expected, atol=2e-5, rtol=2e-5)
        self.assertEqual(actual.dtype, torch.float32)
        fused_update.assert_not_called()

    @patch("vllm_ascend.attention.context_parallel.common_cp.torch_npu.npu_attention_update")
    def test_empty_history_and_padded_query(self, fused_update):
        tokens, heads, dim = 2, 3, 8
        history = torch.full((tokens, 16 * heads, dim + 1), float("nan"))
        history[..., -1] = -torch.inf
        current = torch.randn(tokens, heads, dim, dtype=torch.bfloat16)
        current_lse = torch.zeros(tokens, heads, 1)
        current_lse[1] = -torch.inf
        current[1] = float("nan")
        actual = _npu_attention_update(dim, history, current, current_lse, dcp_size=16)
        torch.testing.assert_close(actual[0], current[0].float())
        torch.testing.assert_close(actual[1], torch.zeros(heads, dim))
        fused_update.assert_not_called()

    @patch("vllm_ascend.attention.context_parallel.common_cp.torch_npu.npu_attention_update")
    def test_fia_positive_inf_empty_partitions(self, fused_update):
        tokens, heads, dim = 3, 2, 8
        history = torch.zeros(tokens, 16, heads, dim + 1)
        history[..., -1] = torch.inf
        # One populated history shard, fifteen FIA empty-shard sentinels.
        history[0, 0, :, :dim] = 2.0
        history[0, 0, :, -1] = 0.0
        current = torch.full((tokens, heads, dim), 4.0)
        current_lse = torch.zeros(tokens, heads, 1)
        current_lse[2] = torch.inf
        actual = _npu_attention_update(
            dim, history.reshape(tokens, 16 * heads, dim + 1),
            current, current_lse, dcp_size=16,
        )
        torch.testing.assert_close(actual[0], torch.full((heads, dim), 3.0))
        torch.testing.assert_close(actual[1], current[1])
        torch.testing.assert_close(actual[2], torch.zeros(heads, dim))
        fused_update.assert_not_called()

    def test_stable_lse_merge_with_empty_partitions(self):
        outputs = torch.tensor([[[[2.0]]], [[[4.0]]], [[[float("nan")]]]])
        for offset in (-1000.0, 1000.0):
            lse = torch.tensor([offset, offset, -torch.inf]).reshape(3, 1, 1, 1)
            output, total_lse = _update_out_and_lse(outputs, lse)
            torch.testing.assert_close(output, torch.full_like(output, 3.0), atol=2e-4, rtol=2e-4)
            torch.testing.assert_close(total_lse, lse[:2].logsumexp(0))
