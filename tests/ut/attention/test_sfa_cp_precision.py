...
def test_sfa_custom_op_fake_shape(scatter_dim, dtype):
    from torch._subclasses.fake_tensor import FakeTensorMode

    with FakeTensorMode():
        output = torch.empty(48, 96, 512, dtype=dtype)
        lse = torch.empty(48, 96, 1, dtype=torch.float32)
        merged = torch.ops.vllm.sfa_dcp_a2a_fused(output, lse, 16, scatter_dim, "fake-dcp")
        expected = (3, 96, 512) if scatter_dim == 0 else (48, 6, 512)
        assert merged.shape == expected
        assert merged.dtype == dtype
        assert merged.device == output.device


def test_sfa_custom_op_passes_inputs_to_combine():
    ...
    with patch.object(kernels, "sfa_dcp_a2a_fused_combine", return_value=expected) as combine:
        actual = kernels.sfa_dcp_a2a_fused(output, lse, 1, 1, "")
    assert actual is expected
    combine.assert_called_once_with(output, lse, 1, 1, scatter_group=None, pcp_group=None, defer_combine=False)
