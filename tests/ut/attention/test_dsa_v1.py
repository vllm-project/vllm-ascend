...
def test_dsa_pcp_backend_keeps_layout(stem):
    base = getattr(dsa_v1, f"AscendDSA{stem}Backend")
    with patch("vllm_ascend.utils.enable_dsa_cp", return_value=False):
        backend = dsa_v1.select_dsa_backend(base, use_pcp=True)
        builder, impl = backend.get_builder_cls(), backend.get_impl_cls()
    assert builder.__name__ == "AscendDSAPCPMetadataBuilder"
    assert impl.__name__ == "AscendDSAPCPImpl"
...
@pytest.mark.parametrize("use_dsa_cp", [False, True])
def test_dsa_base_backend_keeps_legacy_mode_without_load_config(use_dsa_cp):
    with patch("vllm_ascend.utils.enable_dsa_cp", return_value=use_dsa_cp):
        builder = AscendDSABackend.get_builder_cls()
        impl = AscendDSABackend.get_impl_cls()
    expected = "CP" if use_dsa_cp else ""
    ...
    with (
        ...
        assert AscendDSABackend.get_builder_cls() is builder
        assert AscendDSABackend.get_impl_cls() is impl
...
def test_dsv4_cache_layers_remember_their_load_config(use_pcp):
    ...
    for cls, args, base_backend in cases:
        with (
            ...
            layer = cls(*args)
        with (
            patch("vllm.config.get_current_vllm_config", side_effect=AssertionError("load scope ended")),
            patch("vllm_ascend.utils.enable_dsa_cp", return_value=False),
        ):
...
def test_dsa_backend_selects_pcp_and_rejects_legacy_cp():
    ...
    with patch("vllm_ascend.utils.enable_dsa_cp", return_value=True):
        backend = select_dsa_backend(AscendDSABackend, use_pcp=True)
        for get_backend_cls in (backend.get_builder_cls, backend.get_impl_cls):
