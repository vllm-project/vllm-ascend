...
class TestAscendMLAImpl(TestBase):
    ...
    def _configure_kimi_k3_a3_prolog(self) -> None:
        ...
        self.impl.q_a_layernorm = SimpleNamespace(weight=SimpleNamespace(data=torch.ones(1536)), variance_epsilon=1e-6)
        self.impl.kv_a_layernorm = SimpleNamespace(weight=SimpleNamespace(data=torch.ones(512)), variance_epsilon=1e-5)
    ...
    def test_kimi_k3_prolog_v3_supports_a3_w8a8_dynamic(self):
        ...
        with (
            ...
            patch.object(torch.ops._C_ascend, "npu_mla_prolog_v3_k3", create=True),
        ):
    ...
    def test_kimi_k3_prolog_v3_prepares_w8a8_weights_without_changing_fallback(self):
        ...
        with patch("vllm_ascend.attention.mla_v1.torch_npu.npu_format_cast", side_effect=lambda tensor, _: tensor):
    ...
    def test_mla_preprocess_returns_kimi_k3_prolog_v3_decode_result(self):
        decode_result = DecodeMLAPreprocessResult(
            ql_nope=torch.empty(1), q_pe=torch.empty(1), k_nope=torch.empty(1), k_pe=torch.empty(1)
        )
