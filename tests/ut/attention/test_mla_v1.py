...
class TestAscendMLAImpl(TestBase):
    ...
    def test_no_rope_enables_a3_c8_without_kv_consumer(
        self,
    ...
    def test_compact_block_table_for_nz_gather_preserves_padding(self):
        block_table = torch.tensor([[7, -1, 2, 7]], dtype=torch.int32)

        block_ids, compact_table = self.impl._compact_block_table_for_nz_gather(block_table)

        torch.testing.assert_close(block_ids, torch.tensor([2, 7]))
        torch.testing.assert_close(compact_table, torch.tensor([[1, -1, 0, 1]], dtype=torch.int32))
    ...
    def test_compute_prefill_context_dequantizes_a3_c8_latent_cache(
        ...
        def load_cache(*_args, key, value, **_kwargs):
    ...
    def test_no_rope_exec_kv_prefill_quantizes_and_writes_a3_nz_cache(
        self,
        mock_npu_quantize,
        ...
        latent_call, position_call = [call.kwargs for call in mock_scatter_pa_kv_cache.call_args_list]
    ...
    def test_no_rope_decode_a3_c8_quantizes_latent_query_to_int8(
        self,
        mock_dynamic_quant,
        _mock_get_device_type,
    ):
    ...
    @patch("vllm_ascend.ascend_forward_context.get_forward_context")
    @patch("torch_npu.npu_fused_infer_attention_score_v2")
    def test_forward_decode_a3_c8_pads_query_descale_with_heads(
        self,
        mock_fia,
        mock_get_forward_context,
        _mock_get_device_type,
        ...
        self.impl.fa_quant_layer = True
        self.impl.use_mla_rope = False
        self.impl.support_fp8_attention = False
        ...
        k_nope = torch.randint(-128, 127, (block_size, 1, self.impl.kv_lora_rank), dtype=torch.int8)
        k_pe = torch.randn(block_size, 1, self.impl.qk_rope_head_dim)
        ...
        self.impl._forward_decode(
            DecodeMLAPreprocessResult(
                q_nope,
                q_pe,
                k_nope,
                k_pe,
                dequant_scale_q_nope=descale,
            ),
            block_size,
            attn_metadata,
        )
