...
class TestAscendMLAUpdateGraphParams(TestBase):
    ...
    def test_update_graph_params_records_unmatched_events(
        ...
        event1 = MagicMock()
        mock_get_graph_params.return_value = MagicMock(
            attn_params={4: [param, param]},
            handles={4: ["h0", "h1"]},
            events={4: [event0, event1]},
            workspaces={4: "workspace"},
        )

        metadata = MagicMock()
        metadata.decode.seq_lens_list = [7]
        metadata.decode.actual_seq_lengths_q = [1]
        metadata.decode.block_table = "block"
        speculative_config = MagicMock()
        speculative_config.method = "mtp"
        speculative_config.num_speculative_tokens = 1
        ...
        with patch(
            ...
            AscendMLAImpl.update_graph_params(
                update_stream="stream",
                forward_context=MagicMock(attn_metadata={"main": metadata}),
                num_tokens=4,
                vllm_config=None,
                speculative_config=speculative_config,
            )

        self.assertEqual(mock_out.call_count, 1)
        event0.record.assert_called_once_with("stream")
        event1.record.assert_called_once_with("stream")
