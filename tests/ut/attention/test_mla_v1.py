...
class TestAscendMLAMetadataBuilder(TestBase):
    ...
    # 模拟 runner
        ...
        common_attn_metadata = CommonAttentionMetadata(query_start_loc=torch.tensor([0, 1, 2]))
...
class TestAscendMLAImpl(TestBase):
    def setUp(self):
        ...
        self.patcher2 = patch("vllm.distributed.get_tensor_model_parallel_world_size")
        ...
        self.impl = AscendMLAImpl(num_heads,head_size,scale,num_kv_heads,kv_cache_dtype=kv_cache_dtype,**kwargs)
        self.patcher1.stop()
        self.patcher2.stop()
        self.patcher3.stop()
    ...
    def test_v_up_proj_and_o_proj(self):
        batch_size = 4
        x = torch.randn(batch_size, self.impl.num_heads, self.impl.kv_lora_rank)
    
        self.impl.o_proj.return_value = (torch.randn(batch_size, self.impl.num_heads * self.impl.v_head_dim),)
        ...
        self.assertEqual(result.shape[1], self.impl.kv_lora_rank * self.impl.v_head_dim)
    ...
    def test_process_weights_after_loading(self):
        weight = torch.randn(self.impl.kv_lora_rank, self.impl.num_heads * (self.impl.v_head_dim + self.impl.qk_nope_head_dim))
    ...
    def test_compute_prefill_context_none(self):
        ...
        out, lse = self.impl._compute_prefill_context(
            query, kv_cache, 32, metadata, prefix_out, prefix_lse)
    ...
    def test_compute_prefill_context(self, mock_ring, mock_load):
        ...
        self.impl.kv_b_proj.return_value = (
            torch.randn(8, N, 64 + 32),) # [8, 256, 64 + 32]
        ...
        out, lse = self.impl._compute_prefill_context(
            query, kv_cache, 32, meta, prefix_out, prefix_lse)
    ...
    def test_exec_kv(self, mock_stream, mock_kv_cache):
        ...
        kv_cache = (torch.randn(4, 8, self.impl.kv_lora_rank + self.impl.qk_rope_head_dim), 
                    torch.randn(4, 8, self.impl.kv_lora_rank + self.impl.qk_rope_head_dim))
        slots = torch.arange(batch_size, dtype=torch.long)

        # mock kv_a_proj_with_mqa
        proj_out = torch.randn(batch_size, self.impl.num_kv_heads, 1, 
        ...
        mock_kv_cache.return_value = (
            torch.randn(batch_size, self.impl.num_kv_heads, 1, self.impl.qk_rope_head_dim),   # k_pe
            torch.randn(batch_size, self.impl.num_kv_heads, 1, self.impl.kv_lora_rank),   # k_nope
            None, None
        )

        k_pe, k_nope = self.impl.exec_kv(
            hidden, cos, sin, kv_cache, slots, enable_multistream_mla=False
        )
        ...
        # 验证形状
        self.assertEqual(k_pe.shape, (batch_size, self.impl.num_kv_heads, 1, self.impl.qk_rope_head_dim))
        self.assertEqual(k_nope.shape, (batch_size, self.impl.num_kv_heads, 1, self.impl.kv_lora_rank))
    ...
    def test_exec_kv_prefill(self, mock_kv):
        ...
        kv_cache = (torch.randn(100, 8, self.impl.kv_lora_rank + self.impl.qk_rope_head_dim), 
                    torch.randn(100, 8, self.impl.kv_lora_rank + self.impl.qk_rope_head_dim))
        
        slots = torch.arange(B * S, dtype=torch.long)

        # mock kv_a_proj_with_mqa
        proj_out = torch.randn(B, self.impl.num_kv_heads, S,
                                self.impl.kv_lora_rank + self.impl.qk_rope_head_dim)
        self.impl.kv_a_proj_with_mqa.return_value = (proj_out,)
        ...
        mock_kv.return_value = (
            None, None,
            torch.randn(B, self.impl.num_kv_heads, S, self.impl.qk_rope_head_dim),  # k_pe
            torch.randn(B, self.impl.num_kv_heads, S, self.impl.kv_lora_rank)   # k_nope
        )

        k_pe, k_nope = self.impl.exec_kv_prefill(
            hidden_states, cos, sin, kv_cache, slots
        )
        ...
        # 验证形状
        self.assertEqual(k_pe.shape, (B, self.impl.num_kv_heads, S, self.impl.qk_rope_head_dim))
        self.assertEqual(k_nope.shape, (B, self.impl.num_kv_heads, S, self.impl.kv_lora_rank))
