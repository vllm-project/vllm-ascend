...
def test_prefill_quantized_history_bf16_current_and_merge(
    ...
    def preprocess(x, cache, metadata):
        ...
        return SimpleNamespace(ql_nope=query[..., :512], q_pe=query[..., 512:], quantized_query=None), None
...
def test_c8_dcp_fused_prolog_preserves_both_query_representations(monkeypatch, rank):
    """History consumes FP8 Q while the remaining helper packs local BF16 Q."""
    tokens, heads = 64, 96
    source = torch.randn(tokens, heads, 576, dtype=torch.bfloat16)
    kv_scale = torch.tensor([0.125])
    quantized = _quantize_query(source[..., :512], source[..., 512:], kv_scale)
    current = torch.full((4, 1, 128, 576), 37, dtype=torch.bfloat16)
    ...
    slots = torch.full((tokens,), -1, dtype=torch.int64)
    slots[rank::8] = torch.arange(tokens)[rank::8] + 127
    b = SimpleNamespace(
        query=torch.full_like(source, float("nan")),
        current_cache=current,
        current_slots=torch.arange(tokens) + 127,
        slots=slots,
        dcp_size=8,
        split_kv=True,
        unabsorbed=False,
    ...
    prolog = SimpleNamespace(ql_nope=source[..., :512], q_pe=source[..., 512:], quantized_query=quantized)
    calls = []

    def copy_local(latent, kv_rope, kc, kr, dest, scale, **kwargs):
        calls.append("kv_and_local")
        assert latent.dtype == kv_rope.dtype == torch.bfloat16
        assert latent.data_ptr() == current.data_ptr()
        assert (kc is history[0]) and (kr is history[1]) and (dest is slots)
        assert kwargs["source_slots"] is b.current_slots
        assert kwargs["query"] is prolog.ql_nope and kwargs["rope"] is prolog.q_pe
        assert kwargs["local_head_start"] == rank * 12
        expected = source[:, rank * 12 : (rank + 1) * 12]
        kwargs["local_query"].copy_(expected)
        torch.testing.assert_close(kwargs["local_query"], expected, rtol=0, atol=0)

    class Captured(Exception):
        pass

    def flash(query, cache, **kwargs):
        assert query is quantized[0] and cache is history[0]
        assert kwargs["query_rope"] is quantized[1]
        assert kwargs["dequant_scale_query"] is quantized[2]
        assert kwargs["dequant_scale_key"] is kv_scale
        assert kwargs["layout_out"] == "NTD_DCP"
    ...
    scope = dict(
        ...
        DeviceMetadataStage=SimpleNamespace(ATTENTION=0),
        MLAPO_MAX_SUPPORTED_TOKENS=128,
        quantize_mla_kv_and_local_query=copy_local,
        notify_kv_cache_written=lambda *args: calls.append("written"),
    )
    monkeypatch.setattr(torch.ops._C_ascend, "flash_mla_with_kvcache", flash, raising=False)
    impl = SimpleNamespace(
        enable_mlapo=True,
        mlapo_weight_quant_mode=3,
        q_proj=SimpleNamespace(qrep_active=True, rank_in_group=rank),
        dcp_group=SimpleNamespace(world_size=8),
        flash_dcp_overlap=False,
        layerwise_kv_cache_hook=None,
        mla_preprocess_only_decode=lambda *args: (prolog, None),
        fak_descale_float=kv_scale,
        fak_descale_reciprocal=kv_scale.reciprocal(),
        scale=576**-0.5,
    )
    with pytest.raises(Captured):
        _load_prefill(scope)(impl, "layer", source[:, 0], history, SimpleNamespace(flash=b), None)
    assert calls == ["kv_and_local", "written", "history"]
    torch.testing.assert_close(current, current_before, rtol=0, atol=0)
    assert torch.isnan(b.query).all()
