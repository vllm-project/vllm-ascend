...
class TestAscendAttentionBackendImpl(TestBase):
    ...
    def test_get_kvcomp_params_early_exit(self):
        """
        Test that _get_kvcomp_params returns original values 
        ...
        self.impl.enable_kvcomp = True
        self.impl.layerIndex = 0

        res_bt, res_sl = self.impl._get_kvcomp_params(
            query, key, metadata, block_table, actual_seq_lengths_kv
        )
    ...
    def test_get_kvcomp_params_reuse(self):
        ...
        self.impl.enable_kvcomp = True
        self.impl.layerIndex = 1 # Current layer is 1
        ...
        kvcomp_metadata = MagicMock()
        kvcomp_metadata.hashk_caches = [MagicMock(), MagicMock()] # Cache exists
        
        kvcomp_config = MagicMock()
        kvcomp_config.vllm_hash_attention_skip_layers = [False, True] # Layer 1 is a skip layer
        kvcomp_config.top_k_index_reuse = [0, 0] # Layer 1 reuses result from Layer 0
        kvcomp_metadata.kvcomp_config = kvcomp_config
        metadata.kvcomp_metadata = kvcomp_metadata

        # Mock the pre-calculated results from Layer 0
        expected_bt = torch.ones(1, 5)
        expected_sl = torch.tensor([5])
        metadata.hamming_output_records = [
            {'new_block_table': expected_bt, 'new_seq_lens_list': expected_sl}, 
            None
        ]
    ...
    def test_get_kvcomp_params_decode_hamming(self, mock_hamming, mock_reshape):
        ...
        """
        query = torch.randn(2, 8, 64) # batch size = 2
        key = torch.randn(2, 8, 64)
        block_table = torch.zeros(2, 5, dtype=torch.long)
        actual_seq_lengths_kv = [10, 10]
        ...
        metadata.hamming_output_records = [None] # Initialize records
        ...
        self.assertIsNotNone(metadata.hamming_output_records[0])