...
from vllm_ascend.ascend_config import CustomFIAConfig
...
class TestCustomFIAGate(TestBase):
    """Gating of the custom FIA (BlasST) dispatch.
    ...
    ``_custom_fia_supported`` at init; per-call limits live in the gate.
    """

    @staticmethod
    def _impl(**overrides):
        """Partial instance covering exactly what _can_use_custom_fia reads."""
        impl = object.__new__(AscendAttentionBackendImpl)
        impl._custom_fia_supported = overrides.pop("supported", True)
    ...
    def _gate(self, impl, meta, *, capturing=False, is_draft=False, hw_phase_split=False):
        ...
        with patch.object(attn_module, "_EXTRA_CTX", extra_ctx), \
            ...
            return impl._can_use_custom_fia(meta)
    ...
    def test_batch_over_host_seq_limit_falls_back(self):
        limit = attn_module.CUSTOM_FIA_MAX_HOST_SEQ
        self.assertFalse(
    ...
    def test_capture_requires_full_graph_and_decode_only(self):
        impl = self._impl()
        full_cfg = SimpleNamespace(custom_fia_config=SimpleNamespace(full_graph=True))
        off_cfg = SimpleNamespace(custom_fia_config=SimpleNamespace(full_graph=False))
    ...
    def test_capture_draft_model_and_layer_aware_fall_back(self):
        impl = self._impl()
        cfg = SimpleNamespace(custom_fia_config=SimpleNamespace(full_graph=True))
        ...
        with patch.object(attn_module, "get_current_vllm_config", return_value=vllm_config), \
             patch.object(attn_module, "get_ascend_config",
                          return_value=SimpleNamespace(
                              custom_fia_config=CustomFIAConfig(enabled=enabled))), \
    ...
    def test_static_capability_matrix(self):
        ...
        for kwargs in (dict(enabled=False),
            ...
            self.assertFalse(
                self._init_impl(**kwargs)._custom_fia_supported, kwargs)
    ...
    def test_host_seq_limit_constant_matches_op_tiling(self):
        ...
        # CUSTOM_FIA_MAX_HOST_SEQ in attention_v1.py).
        self.assertEqual(attn_module.CUSTOM_FIA_MAX_HOST_SEQ, 256)
