...
def load_helpers(combine):
    ...
    tree.body = [
        ...
        if getattr(node, "name", None) in {"merge_flash_attention_output", "get_dcp_local_seq_lens"}
    ]
...
def test_history_current_matches_dense_with_empty_ranks_and_block_crossing(monkeypatch, dcp_size, interleave, expanded):
    ...
    histories = torch.tensor(seq_lens) - torch.tensor(query_lens)
    local_lengths = helpers.get_dcp_local_seq_lens(histories, dcp_size, interleave)
    ...
    for request, (seq, qlen) in enumerate(zip(seq_lens, query_lens)):
        ...
        for rank in range(dcp_size):
            ...
            assert indices.numel() == local_lengths[request, rank]
    ...
    for rank in range(dcp_size):
        ...
        def exchange(output, lse, size, scatter_dim, group, defer_combine, rank=rank, owned=owned):
