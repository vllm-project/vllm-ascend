...
def helpers(monkeypatch):
    ...
    cp_path = path.parent / "context_parallel/common_cp.py"
    cp_tree = ast.parse(cp_path.read_text(encoding="utf-8"))
    cp_tree.body = [node for node in cp_tree.body if getattr(node, "name", None) == "get_dcp_local_seq_lens"]
    exec(compile(cp_tree, str(cp_path), "exec"), scope)
    calls = []
