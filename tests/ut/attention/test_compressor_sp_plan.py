...
def test_chunked_prefill_has_exactly_one_boundary_owner_across_tp_ranks():
    ...
    assert all(
        plan.boundary_replay_token_ranges == ((8184, 8192),)
        for plan in plans
    )
...
def test_chunked_prefill_boundary_owners_are_rank_invariant_for_packed_requests():
    ...
    assert all(plan.supports_boundary_state_replay for plan in plans)
    assert all(
        plan.boundary_replay_token_ranges == ((0, 3), (3, 11), (11, 20))
        for plan in plans
    )
    assert all(
        plan.boundary_replay_cu_seqlens == (0, 3, 11, 20)
        for plan in plans
    )
    assert all(
        plan.boundary_replay_compressed_row_indices == (0, 1, 2, 3)
        for plan in plans
    )
