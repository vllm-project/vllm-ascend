...
@pytest.mark.parametrize("logical_size", [128, 256, 1152])
def test_indexer_recovers_physical_pages_after_request_reuse(logical_size):
    config = SimpleNamespace(
        ...
        # Scheduling geometry is not the per-group page size.
        cache_config=SimpleNamespace(block_size=36864),
    )
    builder = AscendIndexerKPoolMetadataBuilder(
        AscendMLAAttentionSpec(
            block_size=logical_size,
    ...
    split = logical_size // 128
    pointer = None
    for physical in ([10, 5, 16, 11], [15, 20, 1, 6]):
        pages = torch.tensor(physical, dtype=torch.int32)[:, None]
        expanded = pages * split + torch.arange(split, dtype=torch.int32)[None, :]
        common = SimpleNamespace(
            num_reqs=4,
            num_input_tokens=4,
            num_actual_tokens=4,
            positions=torch.full((4,), 127, dtype=torch.int64),
            slot_mapping=pages[:, 0].long() * logical_size + 127,
            seq_lens=torch.full((4,), 128, dtype=torch.int32),
            _seq_lens_cpu=torch.full((4,), 128, dtype=torch.int32),
            query_start_loc=torch.arange(5, dtype=torch.int32),
            block_table_tensor=expanded,
        )
        metadata = builder.build(0, common)
        torch.testing.assert_close(metadata.block_table, pages, rtol=0, atol=0)
        expected_slots = pages[:, 0].long() * (logical_size // 4) + 31
        torch.testing.assert_close(metadata.slot_mapping, expected_slots, rtol=0, atol=0)
        assert metadata.block_size == logical_size // 4
        assert metadata.block_table.data_ptr() == (pointer or metadata.block_table.data_ptr())
        pointer = metadata.block_table.data_ptr()
        torch.testing.assert_close(common.block_table_tensor, expanded, rtol=0, atol=0)
