...
def test_a5_index_score_uses_ascendc_prefill_and_decode() -> None:
...
def test_a5_index_decode_uses_full_table_score_and_existing_topk_cleanup() -> None:
    ...
    with (
        patch(
            "vllm_ascend.models.minimax_m3.ops.msa_m3_npu._ASCEND_DEVICE_TYPE",
            AscendDeviceType.A5,
        ),
        patch(
...
def test_bundled_ascendc_index_score_splits_long_kv_decode() -> None:
    ...
    a2_epilogue = (op_root / "op_kernel" / "arch22" / "msa_seg_row_max_epilogue.h").read_text(encoding="utf-8")
    example = (op_root / "examples" / "test_aclnn_msa_index_score.cpp").read_text(encoding="utf-8")
