...
def make_model(namespace, context, start, end, block_size, sp, materialized):
    class Attention(nn.Module):
        def forward(self, *, hidden_states, positions):
            ...
            partial = hidden_states / context.tp if sp else hidden_states
            if sp and getattr(context, "fuse_o_proj", False):
                return namespace["sp_reduce_scatter"](partial)
            return partial
    ...
    for idx in range(end):
        ...
        layer.fuse_o_proj_mm_reduce_scatter = sp and getattr(context, "fuse_o_proj", False)
...
def test_pipeline_shards_match_unsplit_model(
    runtime, num_tokens, block_size, materialized, cuts, dtype, runner_version, tp=2, fuse_o_proj=False
):
    ...
    def run_rank(rank):
        ...
        context.fuse_o_proj = fuse_o_proj
        intermediate = None
...
@pytest.mark.parametrize("materialized", [False, True])
@pytest.mark.parametrize("runner_version", ["mrv1", "mrv2"])
def test_pp4_tp8_fused_o_projection_keeps_stage_and_aux_shards(runtime, num_tokens, materialized, runner_version):
    # The fused projection owns the one TP reduction. Exercise uneven token
    # shards, empty ranks and four real model/runner stage boundaries.
    test_pipeline_shards_match_unsplit_model(
        runtime,
        num_tokens,
        2,
