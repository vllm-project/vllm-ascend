...
@pytest.mark.parametrize(
    ("device_type", "kv_cache_layout", "capabilities"),
    [
        (
            AscendDeviceType.A2,
            KVCacheLayout.ND,
            frozenset({HardwareCapability.CUSTOM_OPS}),
        ),
        (
            AscendDeviceType.A3,
            KVCacheLayout.ND,
        ...
        (AscendDeviceType._310P, KVCacheLayout.FRACTAL_NZ, frozenset()),
        (
            AscendDeviceType.A5,
            KVCacheLayout.ND,
            frozenset(
...
def test_hardware_profile_matrix(
    ...
    assert profile.kv_cache_layout is kv_cache_layout
