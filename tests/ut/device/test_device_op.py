...
@pytest.mark.parametrize("npu_format", [0, 2])
def test_a5_restores_plain_four_dimensional_mxfp_scales_without_copy(npu_format):
    base = torch.zeros(3, 256, 2, 2, dtype=torch.uint8)
    scale = base.transpose(-3, -2)
    with mock.patch("vllm_ascend.device.device_op.torch_npu") as npu:
        npu.Format.NCHW = 0
        npu.Format.ND = 2
        npu.get_npu_format.return_value = npu_format
        restored = A5DeviceAdaptor._restore_mxfp_semantic_dtype(scale, torch.float8_e8m0fnu)

    assert restored.dtype == torch.float8_e8m0fnu
    assert restored.data_ptr() == base.data_ptr()
    assert restored.stride() == scale.stride()
    aligned = A5DeviceAdaptor._align_gmm_situ_weight_scale(restored)
    assert aligned.is_contiguous()
    assert aligned.data_ptr() == base.data_ptr()


@pytest.mark.parametrize("npu_format", [29, 50])
def test_a5_preserves_packed_nz_metadata(npu_format):
    weight = torch.zeros(3, 256, 64, dtype=torch.uint8)
    with mock.patch("vllm_ascend.device.device_op.torch_npu") as npu:
        npu.Format.NCHW = 0
        npu.Format.ND = 2
        npu.get_npu_format.return_value = npu_format
        restored = A5DeviceAdaptor._restore_mxfp_semantic_dtype(weight, torch.float4_e2m1fn_x2)

    assert restored is weight
