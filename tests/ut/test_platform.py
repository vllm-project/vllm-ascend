...
class TestNPUPlatform(unittest.TestCase):
    ...
    def test_class_variables(self):
        ...
        self.assertEqual(NPUPlatform.supported_quantization, [ASCEND_QUATIZATION_METHOD])
    ...
    def test_is_async_output_supported(self):
        self.assertTrue(self.platform.is_async_output_supported(enforce_eager=None))
        self.assertTrue(self.platform.is_async_output_supported(enforce_eager=True))
        self.assertTrue(self.platform.is_async_output_supported(enforce_eager=False))
    ...
    @patch('torch.npu.set_device', side_effect=RuntimeError("Device not available"))
    ...
    @patch('torch.npu.empty_cache', side_effect=RuntimeError("Cache clearing failed"))
    ...
    @patch('torch.npu.synchronize', side_effect=RuntimeError("Synchronization failed"))
    ...
    @patch('torch.npu.mem_get_info', side_effect=RuntimeError("NPU not available"))
    ...
    def test_clear_npu_memory_normal(self, mock_reset_stats, mock_empty_cache, mock_gc_collect):
    ...
    @patch('torch.npu.empty_cache')
    @patch('torch.npu.reset_peak_memory_stats')
    def test_clear_npu_memory_gc_collect_failure(self, mock_reset_stats, mock_empty_cache, mock_gc_collect):
    ...
    @patch('gc.collect')
    @patch('torch.npu.empty_cache', side_effect=RuntimeError("Cache clear failed"))
    @patch('torch.npu.reset_peak_memory_stats')
    def test_clear_npu_memory_empty_cache_failure(self, mock_reset_stats, mock_empty_cache, mock_gc_collect):
        with self.assertRaises(RuntimeError):
            self.platform.clear_npu_memory()
    ...
    @patch('gc.collect')
    @patch('torch.npu.empty_cache')
    @patch('torch.npu.reset_peak_memory_stats', side_effect=RuntimeError("Reset failed"))
    def test_clear_npu_memory_reset_stats_failure(self, mock_reset_stats, mock_empty_cache, mock_gc_collect):
        with self.assertRaises(RuntimeError):
            self.platform.clear_npu_memory()
    ...
    @patch('os.environ', {})
    def test_check_and_update_config_basic_config_update(self, mock_is_310p, mock_update_acl, mock_init_ascend, mock_check_ascend):
    ...
    @patch('vllm_ascend.ascend_config.check_ascend_config')
    @patch('vllm_ascend.ascend_config.init_ascend_config')
    def test_check_and_update_config_expert_parallel_enabled(self, mock_init_ascend, mock_check_ascend, mock_is_310p):
        mock_init_ascend.return_value = self.mock_ascend_config
        ...
        self.assertEqual(self.mock_vllm_config.parallel_config.expert_tensor_parallel_size, 1)
        self.assertEqual(self.mock_vllm_config.parallel_config.expert_parallel_size, 
                         self.mock_vllm_config.parallel_config.world_size_across_dp)
    
    @patch('vllm_ascend.utils.is_310p', return_value=False)
    @patch('vllm_ascend.ascend_config.check_ascend_config')
    @patch('vllm_ascend.ascend_config.init_ascend_config')
    def test_check_and_update_config_no_model_config_warning(self, mock_init_ascend, mock_check_ascend, mock_is_310p):
        mock_init_ascend.return_value = self.mock_ascend_config
    ...
    def test_check_and_update_config_torchair_graph_disabled_when_mla_disabled(self, mock_init_ascend, mock_check_ascend, mock_is_310p):
    ...
    @patch('vllm_ascend.ascend_config.check_ascend_config')
    @patch('vllm_ascend.ascend_config.init_ascend_config')
    def test_check_and_update_config_enforce_eager_mode(self, mock_init_ascend, mock_check_ascend, mock_is_310p):
        mock_init_ascend.return_value = self.mock_ascend_config
        ...
        self.assertEqual(self.mock_vllm_config.compilation_config.level, 
    ...
    @patch('vllm_ascend.ascend_config.check_ascend_config')
    @patch('vllm_ascend.ascend_config.init_ascend_config')
    def test_check_and_update_config_torchair_enabled_compilation(self, mock_init_ascend, mock_check_ascend, mock_is_310p):
        self.mock_ascend_config.torchair_graph_config.enabled = True
        mock_init_ascend.return_value = self.mock_ascend_config
        self.mock_vllm_config.model_config.enforce_eager = False
    ...
    @patch('vllm_ascend.ascend_config.check_ascend_config')
    @patch('vllm_ascend.ascend_config.init_ascend_config')
    def test_check_and_update_config_cache_config_block_size(self, mock_init_ascend, mock_check_ascend, mock_is_310p):
        mock_init_ascend.return_value = self.mock_ascend_config
    ...
    def test_check_and_update_config_v1_worker_class_selection(self, mock_init_ascend, mock_check_ascend, mock_is_310p):
        mock_init_ascend.return_value = self.mock_ascend_config
        ...
        self.assertEqual(self.mock_vllm_config.parallel_config.worker_cls,
                        "vllm_ascend.worker.worker_v1.NPUWorker")
    
    @patch('vllm_ascend.ascend_config.check_ascend_config')
    @patch('vllm_ascend.ascend_config.init_ascend_config')
    @patch('vllm.envs.VLLM_USE_V1', False)
    def test_check_and_update_config_speculative_worker_config(self, mock_init_ascend, mock_check_ascend):
        mock_init_ascend.return_value = self.mock_ascend_config
        ...
        with patch.dict('os.environ', {}):
            ...
            self.assertEqual(self.mock_vllm_config.parallel_config.sd_worker_cls,
                      "vllm_ascend.worker.worker.NPUWorker")
    
    @patch('vllm_ascend.ascend_config.check_ascend_config')
    @patch('vllm_ascend.ascend_config.init_ascend_config')
    @patch('vllm.envs.VLLM_USE_V1', False)
    def test_check_and_update_config_multi_step_worker_config(self, mock_init_ascend, mock_check_ascend):
        mock_init_ascend.return_value = self.mock_ascend_config
    ...
    @patch('vllm_ascend.ascend_config.init_ascend_config')
    @patch('vllm.envs.VLLM_USE_V1', False)
    def test_check_and_update_config_default_worker_config(self, mock_init_ascend, mock_check_ascend):
        mock_init_ascend.return_value = self.mock_ascend_config
        self.mock_vllm_config.parallel_config.worker_cls = "auto"
    ...
    @patch('vllm.envs.VLLM_USE_V1', True)
    def test_check_and_update_config_310p_no_custom_ops(self, mock_is_310p, mock_init_ascend, mock_check_ascend):
        mock_init_ascend.return_value = self.mock_ascend_config
    ...
    @patch('vllm_ascend.ascend_config.check_ascend_config')
    @patch('vllm_ascend.ascend_config.init_ascend_config')
    def test_check_and_update_config_ascend_scheduler_config(self, mock_init_ascend, mock_check_ascend, mock_is_310p):
        self.mock_ascend_config.ascend_scheduler_config.enabled = True
        mock_init_ascend.return_value = self.mock_ascend_config
        
        with patch('vllm_ascend.core.schedule_config.AscendSchedulerConfig') as mock_scheduler:
            from vllm_ascend import platform
            importlib.reload(platform)
            self.platform.check_and_update_config(self.mock_vllm_config)
    ...
    def test_get_attn_backend_cls_use_mla_only(self):
        ...
        self.assertEqual(result, "vllm_ascend.attention.attention.AscendMLAAttentionBackend")
    ...
    def test_get_attn_backend_cls_default_case(self):
        ...
        self.assertEqual(result, "vllm_ascend.attention.attention.AscendAttentionBackend")
    
    def test_get_punica_wrapper(self):
        result = self.platform.get_punica_wrapper()
        self.assertEqual(result, "vllm_ascend.lora.punica_wrapper.punica_npu.PunicaWrapperNPU")
    ...
    @patch('torch.npu.reset_peak_memory_stats')
    @patch('torch.npu.max_memory_allocated', side_effect=RuntimeError("Memory query failed"))
    ...
    def test_get_device_communicator_cls_returns_correct_value(self):
        self.assertEqual(self.platform.get_device_communicator_cls(), "vllm_ascend.distributed.communicator.NPUCommunicator")
    ...
    def test_get_piecewise_backend_cls_returns_correct_value(self):
        self.assertEqual(self.platform.get_piecewise_backend_cls(), "vllm_ascend.compilation.piecewise_backend.NPUPiecewiseBackend")
    ...
    def test_successful_initialization(self, mock_pg, mock_options_cls, mock_pg_hccl, _):
        ...
        mock_pg.assert_called_once_with(
            mock_prefix, group_rank, group_size, unittest.mock.ANY
        )
        mock_pg_hccl.assert_called_once_with(
            mock_prefix, group_rank, group_size, unittest.mock.ANY
        )
