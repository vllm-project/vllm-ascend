...
class TestNPUPlatform(unittest.TestCase):
    ...
    def test_class_variables(self):
        ...
        self.assertEqual(
            NPUPlatform.device_control_env_var, "ASCEND_RT_VISIBLE_DEVICES"
        )
        self.assertEqual(NPUPlatform.dispatch_key, "PrivateUse1")
        self.assertEqual(
            NPUPlatform.supported_quantization, [ASCEND_QUATIZATION_METHOD]
        )
    ...
    def test_pre_register_and_update_with_parser(
    ...
    @patch("vllm_ascend.utils.adapt_patch")
    @patch("vllm_ascend.quantization.quant_config.AscendQuantConfig")
    def test_pre_register_and_update_without_parser(
        self, mock_quant_config, mock_adapt_patch
    ):
    ...
    def test_clear_npu_memory_normal(
        self, mock_reset_stats, mock_empty_cache, mock_gc_collect
    ):
    ...
    @patch("torch.npu.empty_cache")
    @patch("torch.npu.reset_peak_memory_stats")
    def test_clear_npu_memory_gc_collect_failure(
        self, mock_reset_stats, mock_empty_cache, mock_gc_collect
    ):
    ...
    @patch("torch.npu.reset_peak_memory_stats")
    def test_clear_npu_memory_empty_cache_failure(
        self, mock_reset_stats, mock_empty_cache, mock_gc_collect
    ):
        with self.assertRaises(RuntimeError):
            self.platform.clear_npu_memory()
    ...
    @patch("torch.npu.empty_cache")
    @patch(
        "torch.npu.reset_peak_memory_stats", side_effect=RuntimeError("Reset failed")
    )
    def test_clear_npu_memory_reset_stats_failure(
        self, mock_reset_stats, mock_empty_cache, mock_gc_collect
    ):
        with self.assertRaises(RuntimeError):
            self.platform.clear_npu_memory()
    ...
    def test_check_and_update_config_basic_config_update(
        self, mock_is_310p, mock_update_acl, mock_init_ascend, mock_check_ascend
    ):
    ...
    def test_check_and_update_config_expert_parallel_enabled(
        ...
        self.assertEqual(
            self.mock_vllm_config.parallel_config.expert_tensor_parallel_size, 1
        )
        self.assertEqual(
    ...
    def test_check_and_update_config_enforce_eager_mode(
        ...
        with self.assertLogs(logger="vllm", level="INFO") as cm:
            ...
            self.platform.check_and_update_config(self.mock_vllm_config)
        self.assertTrue(
            "Compilation disabled, using eager mode by default" in cm.output[0]
        )
        self.assertEqual(
    ...
    def test_check_and_update_config_ascend_scheduler_config(
        ...
        with patch(
            "vllm_ascend.core.schedule_config.AscendSchedulerConfig"
        ) as mock_scheduler:
            from vllm_ascend import platform
    ...
    def test_get_attn_backend_cls_use_mla_only(self):
        ...
        self.assertEqual(
            result, "vllm_ascend.attention.attention.AscendMLAAttentionBackend"
        )
    ...
    @patch(
        "torch.npu.reset_peak_memory_stats", side_effect=RuntimeError("Device error")
    )
    @patch("torch.npu.max_memory_allocated")
    ...
    def test_get_current_memory_usage_when_query_fails(
        self, mock_max_memory, mock_reset_stats
    ):
        with self.assertRaises(RuntimeError):
            self.platform.get_current_memory_usage()
    ...
    def test_successful_initialization(
        self, mock_pg, mock_options_cls, mock_pg_hccl, _
    ):
