...
if not _npu_available:
    ...
    if "--pd-unit" in sys.argv:
        # Installed device plugins must not initialize while importing CPU UT.
        # Restore the caller's environment immediately after the import.
        with patch.dict(os.environ, {"TORCH_DEVICE_BACKEND_AUTOLOAD": "0"}):
            import torch
    else:
        import torch
