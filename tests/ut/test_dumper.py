...
def _make_dumper() -> Dumper:
    ...
    # B6: finalize_dump_data holds self._lock; __init__ is skipped in unit stubs.
    dumper._lock = threading.RLock()
    return dumper
...
def test_consume_manual_trigger_true_stays_continuous(tmp_path: Path):
    ...
    assert cfg.save({"dump": {"manual_dump": True}})
    ...
    assert reloaded["dump"]["manual_dump"] is True
    assert cfg.consume_manual_trigger() is True
    assert cfg.manual_trigger() is True
...
def test_consume_manual_trigger_decrements_count(tmp_path: Path):
    cfg = make_dfx_config(tmp_path)
    cfg_path = cfg.config_path
    assert cfg.save({"dump": {"manual_dump": 3}})
    ...
    assert cfg.consume_manual_trigger() is True
    assert cfg.manual_trigger_count() == 0
    assert cfg.dump["manual_dump"] is False
...
def test_recreate_eager_reinstalls_hooks_after_reload_msprobe(tmp_path: Path):
    """T6 regression: reload_msprobe must re-install PrecisionDebugger hooks."""
    ...
    runner = SimpleNamespace(
        model=model,
        ascend_config=SimpleNamespace(dump_config_path=str(tmp_path / "msprobe.json")),
        compilation_config=SimpleNamespace(cudagraph_mode=CUDAGraphMode.NONE),
        tp_rank=0,
        dp_rank=0,
        use_async_scheduling=False,
    )
    (tmp_path / "msprobe.json").write_text(json.dumps({"dump_enable": False}), encoding="utf-8")
    ...
    dumper.runner = runner
    dumper.dfx_config = cfg
    dumper._startup_debugger_done = True
    dumper._debugger = old_dbg
    dumper._debugger_started = True
    dumper._msprobe_dump_active = True
    dumper._uses_aclgraph_dumper = False
    dumper.dump_rank_tag = MagicMock(return_value="tp0")
    dumper.set_msprobe_dump_state = MagicMock(return_value=True)
    dumper._enforce_dump_requires_debugger = MagicMock()

    def _init_ok(_mode):
        dumper._debugger = new_dbg
        dumper._uses_aclgraph_dumper = False
        return new_dbg

    with patch.object(dumper, "_init_debugger", side_effect=_init_ok):
        assert dumper.recreate_msprobe_debugger(reason="reload_msprobe") is True

    old_dbg.stop.assert_called_once()
    new_dbg.start.assert_called_once_with(model)
    assert dumper._debugger_started is True
...
def test_wave_tracking_same_wave_activate_delta_zero():
    ...
    dumper._open_dump_source = None
    dumper._msprobe_dump_total_count = 0

    assert dumper.advance_wave(allow_arm=False) == 0
    assert dumper.advance_wave(allow_arm=True) == 1
    ...
    assert meta.dump_activate_wave == 1
    assert meta.dump_waves_after_report == 0
    assert meta.dump_count == 1
    assert dumper.take_dump_finish_meta("r1") is None
...
def test_wave_tracking_next_wave_activate_delta_one():
    ...
    dumper.advance_wave(allow_arm=True)  # wave 1
    dumper._begin_dump_wave_tracking(["r1", "r2"], anomaly_type="manual_trigger", source="manual_trigger")
    dumper.advance_wave(allow_arm=True)  # wave 2
    dumper._commit_dump_finish_metas(consume_quota=False)
    m1 = dumper.take_dump_finish_meta("r1")
    m2 = dumper.take_dump_finish_meta("r2")
    assert m1 is not None and m2 is not None
    assert m1.dump_waves_after_report == 1
    assert m1.dump_count is None  # manual / skip quota
    assert m2.dump_arm_wave == 1
...
def test_take_dump_finish_meta_from_open_pending_has_null_activate():
    ...
    assert dumper._open_dump_finish_req_ids == ["r1", "r2"]

    meta1 = dumper.take_dump_finish_meta("r1")
    assert isinstance(meta1, DumpFinishMeta)
    assert meta1.dump_arm_wave == 5
    assert meta1.dump_activate_wave is None
    assert meta1.dump_waves_after_report is None
    assert meta1.anomaly_type == "token_repeat"
    assert dumper._open_dump_finish_req_ids == ["r2"]

    meta2 = dumper.take_dump_finish_meta("r2")
    assert meta2 is not None
    assert meta2.dump_activate_wave is None
    assert dumper._open_dump_finish_req_ids == []
    assert dumper.dump_arm_wave_for_report() is None
    assert dumper.take_dump_finish_meta("r1") is None
