...
chunk_gated_delta_rule = importlib.import_module(
    "vllm_ascend._310p.ops.fla.chunk_gated_delta_rule"
)
fused_gdn_gating = importlib.import_module(
    "vllm_ascend._310p.ops.fla.fused_gdn_gating"
)
...
def test_fused_gdn_gating_getter_returns_none_when_extension_is_unavailable():
    with (
        patch.object(fused_gdn_gating.torch, "ops", SimpleNamespace()),
        patch.object(
            fused_gdn_gating.importlib,
            "import_module",
            side_effect=ImportError("gdn_ops_ext is not installed"),
        ) as import_module,
    ):
        actual = fused_gdn_gating._get_fused_gdn_gating_op()

    assert actual is None
    import_module.assert_called_once_with("gdn_ops_ext")


def test_chunk_compute_wy_getter_returns_none_when_extension_is_unavailable():
    with (
        patch.object(chunk_gated_delta_rule.torch, "ops", SimpleNamespace()),
        patch.object(
            chunk_gated_delta_rule.importlib,
            "import_module",
            side_effect=ImportError("gdn_ops_ext is not installed"),
        ) as import_module,
    ):
...
def test_fused_gdn_gating_dispatches_to_gdn_ops_ext():
    num_tokens = 3
    num_heads = 4
    ...
    gdn_op = Mock(return_value=expected)

    with (
        patch.object(
            fused_gdn_gating,
            "_get_fused_gdn_gating_op",
            return_value=gdn_op,
        ),
        patch.object(
            fused_gdn_gating,
            "_can_use_fused_gdn_gating_op",
            return_value=True,
        ),
    ):
        actual = fused_gdn_gating.fused_gdn_gating(
            A_log,
            a,
            b,
            dt_bias,
    ...
    gdn_op.assert_called_once()
    op_args = gdn_op.call_args.args
...
def test_fused_gdn_gating_falls_back_for_unsupported_inputs():
    ...
    with (
        ...
        actual_g, actual_beta = fused_gdn_gating.fused_gdn_gating(
            A_log,
            a,
            b,
            dt_bias,
...
def test_chunk_compute_wy_dispatches_to_gdn_ops_ext():
    ...
    with (
        ...
        actual = chunk_gated_delta_rule._compute_kernel_inputs(
            *inputs,
            chunk_size=64,
        )
    ...
    op_args = gdn_op.call_args.args
    assert all(
        actual_arg.data_ptr() == input_arg.data_ptr()
        for actual_arg, input_arg in zip(op_args[:5], inputs)
    )
...
def test_chunk_compute_wy_falls_back_for_unsupported_inputs():
    ...
    assert all(
        actual_arg is input_arg
        for actual_arg, input_arg in zip(fallback_args[:5], inputs)
    )
