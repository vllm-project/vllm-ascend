...
class TestAscendAttentionBackend310(TestBase):
    ...
    def test_get_impl_cls(self):
        self.assertEqual(AscendAttentionBackend310.get_impl_cls(),
                         AscendAttentionBackendImpl310)

    def test_get_builder_cls(self):
        self.assertEqual(AscendAttentionBackend310.get_builder_cls(),
                         AscendAttentionMetadataBuilder310)
...
class TestAscendAttentionBackendImpl310(TestBase):
    ...
    def test_forward_prefill_310(
            ...
            mock_npu_npu_flash_attention, mock_npu_reshape_and_cache):
        ...
        mock_npu_npu_flash_attention.return_value = torch.ones(
            10, 8, 64)
    ...
    def test_forward_chunked_prefill_310(
            self, mock_get_forward_context, mock_npu_paged_attention_splitfuse,
            mock_npu_reshape_and_cache, mock_format_cast):
        """Test forward pass in PrefillCacheHit state"""
        ...
        mock_get_forward_context.return_value = MagicMock(capturing=False)
        mock_npu_paged_attention_splitfuse.return_value = torch.ones(
            5, 8, 64)
    ...
    @patch('torch_npu._npu_reshape_and_cache')
    @patch('vllm_ascend.attention.attention_v1.get_forward_context')
    def test_forward_paged_attention_310(self, mock_get_forward_context,
