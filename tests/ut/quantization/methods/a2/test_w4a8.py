...
class TestAscendW4A8DynamicFusedMoEMethod(TestBase):
    ...
    @patch("torch.Tensor.npu", new=lambda self: self)
    def test_process_weights_after_loading_builds_a3_mega_moe_int4_lists(self, mock_maybe_trans_nz):
        ...
        self.assertEqual(layer.a3_mega_moe_w13_weight_list[0].dtype, torch.int32)
        self.assertEqual(layer.a3_mega_moe_w2_weight_list[0].dtype, torch.int32)
        ...
        self.assertEqual(layer.a3_mega_moe_w13_weight_scale_list[0].dtype, torch.uint64)
        self.assertEqual(layer.a3_mega_moe_w2_weight_scale_list[0].dtype, torch.uint64)
        self.assertEqual(layer.a3_mega_moe_w13_scale_bias_list[0].dtype, torch.float32)
        self.assertEqual(layer.a3_mega_moe_w2_scale_bias_list[0].dtype, torch.float32)
        # Keep the stacked tensors for AllToAll fallback when a runtime batch
        # exceeds the A3 MegaMoE symmetric-buffer capacity.
