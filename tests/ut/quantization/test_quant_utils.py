...
class TestMaybeAutoDetectQuantization(TestBase):
    ...
    @patch("vllm_ascend.quantization.utils.detect_quantization_method",
           return_value=ASCEND_QUANTIZATION_METHOD)
    def test_user_specified_different_method_warns(self, mock_detect):
        vllm_config = self._make_vllm_config(quantization="gptq")
        with self.assertLogs(logger="vllm", level="WARNING") as cm:
            maybe_auto_detect_quantization(vllm_config)
        # Should keep user-specified method
        self.assertEqual(vllm_config.model_config.quantization, "gptq")
        self.assertTrue(
            any("Auto-detected" in msg for msg in cm.output))

    @patch("vllm.config.VllmConfig._get_quantization_config")
    @patch("vllm_ascend.quantization.utils.detect_quantization_method",
           return_value=ASCEND_QUANTIZATION_METHOD)
    def test_auto_detect_applies_method(self, mock_detect,
                                        mock_get_quant_config):
        ...
        self.assertEqual(vllm_config.model_config.quantization,
                         ASCEND_QUANTIZATION_METHOD)
        self.assertEqual(vllm_config.quant_config, mock_quant_config)
        self.assertTrue(
            any("Auto-detected" in msg for msg in cm.output))

    @patch("vllm.config.VllmConfig._get_quantization_config")
    @patch("vllm_ascend.quantization.utils.detect_quantization_method",
           return_value=COMPRESSED_TENSORS_METHOD)
    def test_auto_detect_compressed_tensors(self, mock_detect,
                                            mock_get_quant_config):
        mock_quant_config = MagicMock()
        mock_get_quant_config.return_value = mock_quant_config
