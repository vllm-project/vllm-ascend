...
def test_v2_sample_wraps_compute_logits_for_dfx_hook():
    ...
    src = (Path(__file__).resolve().parents[2] / "vllm_ascend/worker/v2/model_runner.py").read_text()
    assert "def _dfx_wrap_compute_logits_for_pre_sample" in src
    assert "def _dfx_check_before_sample" in src
    assert "def _dfx_need_pre_sample_hook" in src
    assert "can_run_anomaly_detection" in src
    assert "super().sample(" in src
    assert "model.compute_logits = wrapped" in src
    # Must not keep a full copied upstream sample() body.
    assert "get_num_sampled_and_rejected" not in src
    i_wrap = src.index("def _dfx_wrap_compute_logits_for_pre_sample")
    i_check = src.index("runner._dfx_check_before_sample(logits, input_batch)")
    assert i_wrap < i_check


def test_dfx_need_pre_sample_hook_skips_when_detection_gated():
    """Mirror ``_dfx_need_pre_sample_hook`` gate: detector on but can_run=False → no wrap."""
...
def test_dfx_wrap_compute_logits_calls_check_and_restores():
    ...
    class _Runner:
        ...
        def _dfx_check_before_sample(self, logits, input_batch):
            calls.append(("check", float(logits.item()), input_batch is self._batch))

    runner = _Runner()
    hs = torch.tensor([1.0])
    with wrap(runner, runner._batch):
