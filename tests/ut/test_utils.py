...
class TestUtils(TestBase):
    ...
    def test_vllm_version_is_with_suffix(self):
        ...
        with mock.patch.object(envs_ascend, "VLLM_VERSION", None), mock.patch("vllm.__version__", "0.20.1+cpu"):
            vllm_version_is.cache_clear()
            self.assertTrue(vllm_version_is("0.20.1"))

        # Test case 2: Version without suffix should match
        with mock.patch.object(envs_ascend, "VLLM_VERSION", None), mock.patch("vllm.__version__", "0.20.1"):
            vllm_version_is.cache_clear()
            self.assertTrue(vllm_version_is("0.20.1"))
