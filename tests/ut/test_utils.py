...
def _make_block_size_config(cache, *, hybrid=False, prefix=True, chunked=True):
...
def test_draft_refresh_preserves_explicit_target_block_size(prefix, chunked):
    ...
    target = _make_block_size_config(cache, hybrid=True, prefix=prefix, chunked=chunked)
    draft = _make_block_size_config(cache, prefix=prefix, chunked=chunked)

    utils.refresh_block_size(target)
    utils.refresh_block_size(draft)
...
def test_automatic_block_size_still_uses_prefix_or_chunked_default(prefix, chunked):
    ...
    utils.refresh_block_size(_make_block_size_config(cache, prefix=prefix, chunked=chunked))
    assert cache.block_size == 128


def test_missing_cache_config_is_accepted():
    utils.refresh_block_size(SimpleNamespace(cache_config=None, scheduler_config=None, model_config=None))
