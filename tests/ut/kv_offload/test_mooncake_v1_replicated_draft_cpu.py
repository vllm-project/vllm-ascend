...
def metadata(prompt=3073, computed=0, remote_dcp=2):
    return SimpleNamespace(
        local_block_ids=([4, 5, 6], [10, 13, 19][computed // 1536 :]),
        local_full_block_ids=([4, 5, 6], [10, 13, 19]),
...
def test_global_token_page_mapping(worker, remote_dcp, prompt, computed):
    ...
    assert local == [meta.local_full_block_ids[1][t // 1536] * 12 + t % 1536 // 128 for t in tokens]
    page = 768 * remote_dcp
...
def test_matching_head_source_owns_all_draft_pages_once(worker, rank):
    ...
    assert local[0][1] == [10 * 12 + i for i in range(12)] + [13 * 12 + i for i in range(12)]
    assert local[1][1] == []
