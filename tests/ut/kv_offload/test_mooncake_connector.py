...
def _start_group_patches() -> None:
    """(Re)install the distributed-group mocks.

    ``mock.patch.stopall()`` in tests/ut/test_utils.py deactivates module-level
    patchers globally; re-arming them from setUpClass keeps the thread tests
    from depending on batch execution order.
    """
    patch(
...
class TestMooncakeConnectorScheduler(unittest.TestCase):
    ...
    def test_request_finished_respects_kv_group_layout(self):
        self.scheduler.vllm_config.cache_config.mamba_cache_mode = "align"
        self.scheduler.group_transfer_info = cast(
            Any,
            [
