...
class TestMooncakeConnectorWorker(unittest.TestCase):
    ...
    def test_hybrid_registration_covers_separate_kv_storages(self):
        ...
        layer_name = "model.dspark.layers.0.self_attn"
        worker.kv_cache_config = types.SimpleNamespace(
            kv_cache_tensors=[
                types.SimpleNamespace(
                    layers=[layer_name],
        ...
        ptrs, lengths = worker._get_registered_kv_tensor_buffers_hybrid({layer_name: (key, value)})
        ...
        for tensor in (key, value):
            ...
            assert any(
                region_start <= storage_start and region_start + region_length >= storage_end
    ...
    def test_hybrid_registration_keeps_packed_layout_path(self):
        ...
        ptrs, lengths = worker._get_registered_kv_tensor_buffers_hybrid({layer_name: cache})
