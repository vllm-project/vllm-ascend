...
_connector = "vllm_ascend.distributed.kv_transfer.kv_p2p.mooncake_connector"


def rearm_group_patches() -> None:
    """Install fresh group-getter mocks.

    They are global to the shared pytest process, so a foreign
    ``mock.patch.stopall()`` (e.g. in test_utils.py) can deactivate them
    mid-batch; re-arming in ``setUpClass`` keeps the thread-constructing
    tests independent of execution order. New patcher objects are created on
    each call so re-arming never trips mock's "Patch is already started";
    leaking them matches the file's existing module-level patch style.
    """
    for group_patch in (
        patch(f"{_connector}.get_pp_group", return_value=_mock_pp_group),
        patch(f"{_connector}.get_tp_group", return_value=_mock_tp_group),
        patch(f"{_connector}.get_tensor_model_parallel_world_size", return_value=4),
        patch(f"{_connector}.get_tensor_model_parallel_rank", return_value=0),
        patch(f"{_connector}.get_pcp_group", return_value=_mock_pcp_group),
...
class TestMooncakeConnectorScheduler(unittest.TestCase):
    ...
    def test_request_finished_respects_kv_group_layout(self):
        ...
        self.scheduler.group_transfer_info = [
            types.SimpleNamespace(  # type: ignore[list-item]
                tokens_per_block=16, blocks_per_window=0, is_state_group=False
            ),
            types.SimpleNamespace(  # type: ignore[list-item]
                tokens_per_block=32, blocks_per_window=0, is_state_group=False
            ),
            types.SimpleNamespace(  # type: ignore[list-item]
                tokens_per_block=16, blocks_per_window=3, is_state_group=False
            ),
            types.SimpleNamespace(  # type: ignore[list-item]
                tokens_per_block=16, blocks_per_window=0, is_state_group=True
            ),
        ]
        blocks = (
