    ...
    def test_handle_request(self, mock_send, mock_transfer):
        for transfer_error in (None, RuntimeError("transfer failed")):
            with self.subTest(transfer_error=transfer_error):
                ...
                self.thread.task_tracker.update_done_task_count.assert_called_once_with("req1")  # type: ignore[attr-defined]
...
class TestMooncakeConnectorScheduler(unittest.TestCase):
    ...
    def test_request_finished_respects_kv_group_layout(self):
        self.scheduler.vllm_config.cache_config.mamba_cache_mode = "align"
        self.scheduler.group_transfer_info = [
            types.SimpleNamespace(tokens_per_block=16, blocks_per_window=0, is_state_group=False),  # type: ignore[list-item]
            types.SimpleNamespace(tokens_per_block=32, blocks_per_window=0, is_state_group=False),  # type: ignore[list-item]
            types.SimpleNamespace(tokens_per_block=16, blocks_per_window=3, is_state_group=False),  # type: ignore[list-item]
            types.SimpleNamespace(tokens_per_block=16, blocks_per_window=0, is_state_group=True),  # type: ignore[list-item]
        ]
        blocks = (
...
class TestMooncakeConnectorWorker(unittest.TestCase):
    ...
    def test_start_load_kv_puts_replicated_indexer_on_existing_transfer_port(self):
        ...
        worker._get_sfa_replicate_k_block_ids = MagicMock(return_value=(([40],), ([20],)))  # type: ignore[method-assign]
        ...
        worker._get_remote_host_info_by_port = MagicMock(return_value=("localhost", "remote_engine"))  # type: ignore[method-assign]
        meta = types.SimpleNamespace(
