...
_p_dcp = patch("vllm.distributed.parallel_state._DCP", _mock_dcp_group)
_GROUP_PATCHERS = (
    _p_pp_group,
    _p_tp_group,
    _p_tp_world_size,
...
def rearm_group_patches() -> None:
    """Re-start the distributed-group mocks this file relies on.

    The module-level patch(...).start() mocks above are process-global. Other
    test files (e.g. tests/ut/test_utils.py) call mock.patch.stopall()
    mid-batch, which silently stops them; any class running afterwards hits the
    real get_pp_group()/... and fails with "pipeline model parallel group is
    not initialized". setUpClass re-arms them immediately before each class
    that constructs KVCacheSendingThread runs. Each patcher is re-applied
    idempotently: stop it first (tolerating the case where a foreign stopall()
    already stopped it) so a second run in the same process never double-starts
    a still-active patch.
    """
    for _p in _GROUP_PATCHERS:
        with suppress(RuntimeError):
            _p.stop()  # already stopped by a foreign stopall()
        _p.start()
