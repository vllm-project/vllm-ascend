...
_MOONCAKE_CONNECTOR_MODULE = "vllm_ascend.distributed.kv_transfer.kv_p2p.mooncake_connector"


def rearm_group_patches() -> None:
    """Re-install the module-level distributed-group mocks.

    ``patch(...).start()`` mocks are global to the shared pytest process.
    Another UT file (tests/ut/test_utils.py) calls ``mock.patch.stopall()``
    mid-batch, which deactivates them, so tests that construct threads calling
    ``get_pp_group()`` / ``get_tp_group()`` only fail when batch order puts them
    after that ``stopall``. Re-arming in ``setUpClass`` guarantees the mocks are
    live for those tests; the patchers are intentionally leaked (matching the
    module-level style above) so a foreign ``stopall`` cannot raise on cleanup.
    """
    patchers = [
        patch(f"{_MOONCAKE_CONNECTOR_MODULE}.get_pp_group", return_value=_mock_pp_group),
        patch(f"{_MOONCAKE_CONNECTOR_MODULE}.get_tp_group", return_value=_mock_tp_group),
        patch(f"{_MOONCAKE_CONNECTOR_MODULE}.get_tensor_model_parallel_world_size", return_value=4),
        patch(f"{_MOONCAKE_CONNECTOR_MODULE}.get_tensor_model_parallel_rank", return_value=0),
        patch(f"{_MOONCAKE_CONNECTOR_MODULE}.get_pcp_group", return_value=_mock_pcp_group),
        patch("vllm.distributed.parallel_state._DCP", _mock_dcp_group),
    ]
    ...
    def test_handle_request(self, mock_send, mock_transfer):
        for transfer_error in (None, RuntimeError("transfer failed")):
            with self.subTest(transfer_error=transfer_error):
                ...
                self.thread.task_tracker.update_done_task_count.assert_called_once_with(  # type: ignore[attr-defined]
                    "req1",
                )
...
class MockVllmConfig:
    def __init__(self):
        ...
        # vLLM #53781: get_kv_cache_config_from_groups reads this attribute.
