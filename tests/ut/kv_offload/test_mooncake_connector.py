...
class TestMooncakeConnectorWorker(unittest.TestCase):
    ...
    def test_decode_only_dcp_empty_receive_preserves_completion_and_next_request(self):
        for draft_spec in ("MLAAttentionSpec", "AscendDCPReplicatedDraftAttentionSpec"):
            for rank in range(8):
                with self.subTest(draft_spec=draft_spec, rank=rank):
                    worker = self._build_non_cp_worker()
                    worker._is_hma_required = True
                    worker.use_mla = True
                    worker.dcp_size = worker.tp_size = worker._prefill_tp_size = 8
                    worker.dcp_rank = worker.tp_rank = rank
                    worker._prefill_pp_size = 1
                    worker.block_size = 768
                    worker.block_size_scale = [[6], [1], [48 if "Replicated" in draft_spec else 6], [1]]
                    worker.kv_group2layeridx = {
                        i: ({"kv_cache_spec_type": spec, "kv_cache_group_id": i}, [i])
                        for i, spec in enumerate(("MLAAttentionSpec", "MambaSpec", draft_spec, "MambaSpec"))
                    }
                    worker.enable_sfa_dcp_replicated_indexer = False
                    worker.kv_recv_thread = MagicMock()
                    worker.kv_send_thread = None
                    params = dict(
                        ...
                        remote_block_ids=(list(range(100, 271)), [400], list(range(500, 671)), [700]),
                        num_prompt_blocks=171,
                    )
                    # request_finished() emits grouped empty IDs for a cancelled
                    # request. A full prefix hit emits an empty outer sequence.
                    for local_ids in (([], [], [], []), tuple()):
                        metadata = MooncakeConnectorMetadata()
                        metadata.add_new_req("finished", local_ids, 0, params, local_ids)
                        worker.kv_recv_thread.reset_mock()
                        worker.start_load_kv(metadata)
                        cleanup = [call.kwargs for call in worker.kv_recv_thread.add_request.call_args_list]
                        self.assertTrue(cleanup)
                        self.assertEqual(sum(pull["all_task_done"] for pull in cleanup), 1)
                        for pull in cleanup:
                            self.assertFalse(any(pull["local_block_ids"]))
                            self.assertFalse(any(pull["remote_block_ids"]))
                            self.assertEqual(pull["remote_request_id"], "prefill-request")

                        metadata = MooncakeConnectorMetadata()
                        blocks = (list(range(20, 42)), [40], list(range(60, 82)), [80])
                        metadata.add_new_req("next", blocks, 171 * 768, params, blocks)
                        worker.kv_recv_thread.reset_mock()
                        worker.start_load_kv(metadata)
                        transfers = [call.kwargs for call in worker.kv_recv_thread.add_request.call_args_list]
                        self.assertEqual(
                            [pull["remote_handshake_port"] for pull in cleanup],
                            [pull["remote_handshake_port"] for pull in transfers],
                        )
                        selected = range(rank, 171, 8)
                        for pull in transfers:
                            self.assertEqual(
                                pull["local_block_ids"][0],
                                [(20 + block // 8) * 6 + offset for block in selected for offset in range(6)],
                            )
                            self.assertEqual(pull["local_block_ids"][1], [40])
                            self.assertEqual(pull["local_block_ids"][3], [80])
