...
class TestMooncakeConnectorWorker(unittest.TestCase):
    ...
    def test_mrv2_dcp_transfers_follow_global_block_ownership(self):
        # Release-branch adaptation: PCP stays a KV shard axis here, so the
        # upstream PCP=2 topology does not apply; ownership is exercised for
        # pure-DCP geometries (remote_dcp bounded by prefill TP).
        for remote_dcp in (2, 8):
                ...
                for rank in range(local_dcp):
                    with self.subTest(remote_dcp=remote_dcp, local_dcp=local_dcp, rank=rank):
                        ...
                        for shard_ports, dst, src in zip(ports, local_ids, remote_ids):
                            ...
                            # Release cp_group layout: port = base + dcp_rank
                            # + dcp_repeat_offset (a multiple of remote_dcp).
                            source_cp_rank = offset % remote_dcp
                        ...
                        pulls = worker._get_dcp_shard_pulls(ports, 8, 30000, remote_pcp_size=1)
                        self.assertTrue(
    ...
    def test_sfa_replicated_indexer_cp_ratio(self):
        for remote_cp, local_cp in ((8, 2), (2, 8), (4, 4), (6, 4), (0, 4), (4, 0)):
            with self.subTest(remote_cp=remote_cp, local_cp=local_cp):
                ...
                worker.enable_sfa_dcp_replicated_indexer = True
                worker.use_sfa_sparse = True
                # Release-branch _get_sfa_replicate_k_block_ids reads
                # vllm_config.model_config directly.
                worker.vllm_config = types.SimpleNamespace(model_config=None)
                worker.pcp_size = 1
    ...
    def test_sfa_decode_only_dcp_maps_global_blocks_to_each_rank(self):
        # Release-branch adaptation: the decode-only DCP path is gated to
        # PCP == 1 on both sides (PCP replica selection needs the MRV2
        # replica semantics that this branch does not carry).
        for rank, remote_pcp_size in ((rank, pcp) for rank in range(8) for pcp in (1,)):
