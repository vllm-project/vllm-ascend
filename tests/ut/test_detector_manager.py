...
def test_detector_manager_gates_detection(tmp_path):
    """detection_gate False skips detectors; B10 still appends IO on TP0."""
    RequestIoSnapshotManager.reset_for_tests()
    cfg = make_dfx_config(tmp_path)
    cfg._data["detector"]["token_logprob"]["enabled"] = True
    ...
    # Gate off: no detector calls; TP0 still accumulates IO (B10).
...
def test_detector_manager_gated_keeps_io_only_on_tp0(tmp_path):
    """Gate off: TP0 appends cumulative IO; non-TP0 does not (B10)."""
    RequestIoSnapshotManager.reset_for_tests()
    cfg = make_dfx_config(tmp_path)
    cfg._data["log"]["print_output_on_finish"] = False

    # TP0 keeps IO even when detect is gated (B10).
