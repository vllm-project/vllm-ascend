...
def make_cache_config(specs, num_blocks=3):
    # vLLM #51718 renamed shared_by -> layers and added the stride fields on main.
    def make_tensor(name, spec):
        if vllm_version_is("0.28.0"):
            return KVCacheTensor(size=num_blocks * spec.page_size_bytes, shared_by=[name])
    ...
    return KVCacheConfig(
        ...
        kv_cache_tensors=[make_tensor(name, spec) for name, spec in specs.items()],
        kv_cache_groups=[
            KVCacheGroupSpec(
