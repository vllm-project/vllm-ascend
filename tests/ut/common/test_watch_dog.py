...
class TestWatchDogStartStop(PytestBase):
    ...
    def test_stop_wakes_sleeping_check_thread(self):
        """Test that stop() wakes the check loop instead of leaving the thread
        sleeping in the background for up to check_interval seconds.

        The check loop must wait on the stop event, not time.sleep: with a
        long check interval a sleep-based loop ignores stop() and stays alive
        well after stop() returns."""
        from vllm_ascend.common.utils.watch_dog import WatchDog

        dog = WatchDog()
        dog._check_interval = 10  # a sleep-based loop would ignore stop() for 10s
        dog.start()
        thread = dog._thread
...
class TestWatchDogDumpStack(PytestBase):
    ...
    def test_dump_stack_calls_faulthandler(self):
        ...
        with (
            ...
            # Verify the dump reason is recorded in the file header.
            header = mock_open_file.return_value.write.call_args_list[0].args[0]
            assert "due to unit test" in header
