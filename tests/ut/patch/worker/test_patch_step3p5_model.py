...
def test_layer0_residual_is_chunked_when_fc1_enabled():
    ...
    chunked to [N//tp, H] before being added to the reduce_scatter attention
    output, and the original forward must not run."""
    # Replicated residual [N=4, H=8]; tp=2, rank=0 -> chunk slice is rows [0:2].
    ...
    expected = mlp_out + attn_out + hidden.chunk(2, dim=0)[0].contiguous()
