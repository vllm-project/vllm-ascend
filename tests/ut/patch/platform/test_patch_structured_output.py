...
def _raise_xgrammar_error(message: str) -> None:
    # The release vllm (0.27.1) catches `ValueError` around
    # `validate_xgrammar_grammar`; main catches `VLLMValidationError`.
    if vllm_version_is("0.27.1"):
        raise ValueError(message)
    raise VLLMValidationError(message)
...
def test_sampling_params_rejects_mixed_structured_output_backends(monkeypatch):
    def fake_validate_xgrammar(sampling_params):
        ...
        if schema.get("force_guidance"):
            _raise_xgrammar_error("xgrammar unsupported")
...
def test_failed_first_validation_does_not_lock_config(monkeypatch):
    def fake_validate_xgrammar(sampling_params):
        _raise_xgrammar_error("xgrammar error")
