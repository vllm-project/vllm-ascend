...
* the module-level class swaps and the consolidated engine-core patch
  (``patch_engine_core._run_engine_core_patch_func`` plus the deferred
  watchdog ``_process_engine_step`` / ``_process_input_queue`` patches
  installed at engine-core entry) actually took effect;
...
#       watchdog-feed patches                                            (DEFERRED:
#       installed by patch_engine_core.patch_watchdog_for_engine_core at
#       engine-core entry)
...
import vllm_ascend.patch.platform.patch_engine_watchdog as _watchdog_patch  # noqa: E402
...
def test_balance_engine_core_hooks(monkeypatch):
    ...
    try:
        with (
            ...
            # The wrapper now installs/starts the watchdog on every entry;
            # mock it so the real singleton watchdog thread is never started.
            patch.object(pe, "patch_watchdog_for_engine_core"),
        ):
