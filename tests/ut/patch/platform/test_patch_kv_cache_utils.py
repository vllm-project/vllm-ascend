...
def test_glm_dspark_keeps_native_pages_and_sliding_window_manager():
    ...
    assert specs["model.layers.0.self_attn.indexer"].page_size_bytes == 32768
    assert all(spec.page_size_padded is None for spec in specs.values() if hasattr(spec, "page_size_padded"))
...
def test_glm_dspark_allocates_one_native_layout_tensor_per_layer():
    ...
    assert _ascend_pool_bytes_per_block(groups) == bytes_per_block
    expected_blocks = 17
    ...
    tensor_by_layer = {tensor.layers[0]: tensor for tensor in cache_config.kv_cache_tensors}
    assert all(len(tensor.layers) == 1 for tensor in cache_config.kv_cache_tensors)
    for layer_name, spec in specs.items():
        ...
        assert tensor.layer_stride == tensor.size
    ...
    expected_pages = sum(group.kv_cache_spec.max_memory_usage_pages(config) for group in groups)
    assert _ascend_max_memory_usage_bytes_from_groups(config, groups) == bytes_per_block * expected_pages
...
def test_257k_request_fits_candidate_block_capacity():
    ...
    for index in range(78):
        if index < 21:
            specs[f"model.layers.{index}.self_attn.indexer"] = AscendSFAIndexerCacheSpec(
                block_size=128,
                num_kv_heads=1,
                head_size=128,
                dtype=torch.bfloat16,
    ...
    cache_config = _ascend_get_kv_cache_config_from_groups(config, groups, bytes_per_block * available_blocks)
    required_blocks = sum(group.kv_cache_spec.max_memory_usage_pages(config) for group in groups)
    ...
    assert sum(tensor.size for tensor in cache_config.kv_cache_tensors) == bytes_per_block * available_blocks
...
def test_native_layout_gate_rejects_non_dspark_and_disabled_hybrid():
    ...
    assert not _use_dspark_native_layout(_config(disable_hybrid=True), groups)


def test_non_dspark_grouping_preserves_glm5_next_wrapper():
    config = _config(use_dspark=False)
    specs = _glm_dspark_specs()

    with patch(
        "vllm_ascend.patch.platform.patch_kv_cache_utils._get_glm5_next_kv_cache_groups",
        return_value=[],
    ) as fallback:
        groups = _ascend_get_kv_cache_groups(config, specs)
