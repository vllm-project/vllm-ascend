...
def test_kimi_k3_dcp_replicated_draft_uses_minimal_physical_layout(
    ...
    monkeypatch.setattr(kv_cache_utils_patch.ascend_envs, "VLLM_ASCEND_ENABLE_FLASH_MLA", False)
    replication_size = 8
    page_size = 488448
    specs = _make_kimi_k3_dspark_kv_cache_specs(
        page_size=page_size,
...
def test_replicated_draft_preserves_flash_head_slots():
    from vllm_ascend.attention.attention_v1 import AscendGQADraftSpec

    base = AscendGQADraftSpec(
        block_size=256,
        num_kv_heads=2,
        head_size=64,
        dtype=torch.bfloat16,
        ...
        page_size_padded=294912,
    )
    spec = AscendDCPReplicatedDraftAttentionSpec.from_full_attention_spec(base, 2)
    assert spec.num_heads == 4
    assert spec.state_content_size_bytes == 128
    assert spec.non_causal_multi_token_decode
    assert spec.page_size_padded is None
    assert spec.lane_page_size_bytes == 4 * 256 * 128
    assert spec.page_size_bytes == 2 * spec.lane_page_size_bytes
    assert AscendDCPReplicatedDraftAttentionSpec.merge([spec, spec]) == spec


def test_replicated_draft_flash_planner_preserves_target_descriptors(monkeypatch):
    specs = _make_kimi_k3_dspark_kv_cache_specs(draft_replication_size=2)
    groups = _get_kimi_k3_dspark_mixed_kv_cache_groups(specs)
    assert groups is not None
    blocks = 7
    first_specs = groups[0].kv_cache_spec.kv_cache_specs
    target_names = groups[0].layer_names[:24]
    draft_names = groups[0].layer_names[24:]
    target_pitch = sum(first_specs[name].page_size_bytes for name in target_names)
    total_pitch = target_pitch + sum(first_specs[name].page_size_bytes for name in draft_names)
    target_tensors = [
        KVCacheTensor(
            size=target_pitch * blocks, layers=[name], offset=idx * 488448, layer_stride=0, block_stride=target_pitch
        )
        for idx, name in enumerate(target_names)
    ]
    original_config = KVCacheConfig(num_blocks=blocks, kv_cache_tensors=target_tensors, kv_cache_groups=groups)
    calls = []

    def target_planner(config, target_groups, memory):
        calls.append((target_groups, memory))
        return original_config

    monkeypatch.setattr(kv_cache_utils_patch.ascend_envs, "VLLM_ASCEND_ENABLE_FLASH_MLA", True)
    monkeypatch.setattr(kv_cache_utils_patch, "_orig_pool_bytes_per_block", lambda _: target_pitch)
    monkeypatch.setattr(kv_cache_utils_patch, "_orig_get_kv_cache_config_from_groups", target_planner)
    monkeypatch.setattr(kv_cache_utils_patch, "may_override_num_blocks", lambda _, count: count)
    actual = _get_kimi_k3_replicated_dspark_kv_cache_config(SimpleNamespace(), groups, total_pitch * blocks)
    assert calls[0][0][0].layer_names == target_names
    assert calls[0][1] == target_pitch * blocks
    assert actual is not None and actual.kv_cache_groups is groups
    assert all(tensor.size == total_pitch * blocks for tensor in actual.kv_cache_tensors)
    for before, after in zip(target_tensors, actual.kv_cache_tensors):
        assert (before.offset, before.block_stride, before.layer_stride) == (
            after.offset,
            after.block_stride,
            after.layer_stride,
        )
    offset = target_pitch * blocks
    for name, tensor in zip(draft_names, actual.kv_cache_tensors[len(target_tensors) :]):
        assert tensor.layers == [name]
        assert tensor.offset == offset
        assert tensor.block_stride == first_specs[name].page_size_bytes
        offset += blocks * tensor.block_stride
    assert offset == total_pitch * blocks
