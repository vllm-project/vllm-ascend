...
def test_native_builders_implement_the_fused_update_hook() -> None:
    ...
    assert any(
        ...
        and any(
            isinstance(target, ast.Name) and target.id == "supports_draft_decode_metadata_update"
            for target in node.targets
        )
        for node in indexer.body
    )
    indexer_update = ast.unparse(_method(indexer, "update_draft_decode_metadata"))
    assert "get_cos_and_sin_mla" in indexer_update
    assert "build_for_drafting" not in indexer_update
    assert "draft_index" not in indexer_update

    sfa_update = ast.unparse(
        _method(
            _class(ast.parse(SFA.read_text()), "AscendSFAMetadataBuilder"),
            "update_draft_decode_metadata",
        )
    )
    assert "self.nope_states.get(None)" in sfa_update
    assert "state.update(metadata)" in sfa_update
...
def test_fused_draft_uses_the_original_builder_entry_point() -> None:
    ...
    build = ast.unparse(
        next(node for node in tree.body if isinstance(node, ast.FunctionDef) and node.name == "build_attn_metadata")
    )
    factory = ast.unparse(
        next(
            node
            for node in tree.body
            if isinstance(node, ast.FunctionDef) and node.name == "build_draft_attn_metadata_factory"
        )
    )
