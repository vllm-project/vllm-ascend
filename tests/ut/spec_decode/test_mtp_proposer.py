...
class TestMtpProposer:
    ...
    @patch("vllm.v1.spec_decode.eagle.CpuGpuBuffer")
    def test_init_with_aclgraph(self, mock_cpu_gpu_buffer, vllm_config, runner):
        mock_buffer_instance = MagicMock()
        mock_cpu_gpu_buffer.return_value = mock_buffer_instance
    ...
    def test_prepare_next_token_ids_cpu(self, mock_cpu_gpu_buffer):
        ...
        assert torch.all(result == torch.tensor([30, 50, 60], dtype=torch.int32))
    ...
    def test_prepare_next_token_ids_padded(self, mock_cpu_gpu_buffer):
        ...
        mock_backup.gpu = torch.tensor([1, 2, 3, 4, 5, 6, 7], dtype=torch.int32)
        ...
        assert np.array_equal(mock_backup_output.np[:4], expected_backup_cpu[:4])
    ...
    def test_prepare_inputs_padded(self, mock_cpu_gpu_buffer):
        ...
        assert isinstance(spec_common_attn_metadata, AscendCommonAttentionMetadata)
        assert torch.equal(
            spec_common_attn_metadata.query_start_loc,
