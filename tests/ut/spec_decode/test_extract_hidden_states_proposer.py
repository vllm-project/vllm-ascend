...
def _no_pin_memory():
    # CpuGpuBuffer defaults pin_memory=PIN_MEMORY (True on main), which requires
    # NPU registration. Strip pin_memory from torch.zeros since the test runs on
    # CPU without NPU. The 0.28.0 module has no PIN_MEMORY, so torch.zeros is the
    # only pin source to neutralize on either lane.
