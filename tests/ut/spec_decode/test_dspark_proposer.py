...
class TestDSparkACLGraphContract(_DSparkProposerTestBase):
    ...
    def test_profile_run_binds_context_slot_mappings(self) -> None:
        proposer = self._make_proposer(max_num_tokens=16, num_reqs=1, block_size=3)
        proposer.hidden_states = torch.zeros((16, 8))
        proposer.token_indices_to_sample = torch.zeros(3, dtype=torch.int32)
        proposer.model = MagicMock()
        proposer.runner = SimpleNamespace(
            _sync_metadata_across_dp=lambda tokens, **kwargs: (tokens, None, None),
            dynamic_eplb=False,
        )
        proposer.use_cuda_graph = False
        proposer._pad_draft_buffers = MagicMock()
        ...
        with (
            patch("vllm_ascend.spec_decode.dspark_proposer.set_ascend_forward_context") as forward_context,
            patch(
            ...
            forward_context.return_value.__enter__.return_value = None
            proposer.dummy_run(num_tokens=3, num_reqs=1, is_profile=True)

        args = proposer.model.precompute_and_store_context_kv.call_args.args
        assert args[2] == [expected]
        assert proposer._context_slot_mapping_buffers == [expected]
