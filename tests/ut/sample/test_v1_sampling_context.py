...
class TestV1SamplingContext(TestBase):
    ...
    def test_from_model_runner_inputs_keeps_request_mapping_separate_from_rows(self):
        from vllm_ascend.worker.v1.sample.sampling_context import V1SamplingContext
    ...
    def test_from_model_runner_inputs_rejects_cpu_mapping_mirror_mismatch(self):
        ...
        with self.assertRaisesRegex(ValueError, "idx_mapping_np must match"):
            V1SamplingContext.from_model_runner_inputs(
                num_reqs=2,
                positions_at_logits=torch.tensor([3, 4, 7], dtype=torch.int64),
                input_ids_at_logits=torch.tensor([13, 14, 17], dtype=torch.int64),
                req_indices_at_logits=torch.tensor([0, 0, 1], dtype=torch.int32),
    ...
    def test_from_model_runner_inputs_rejects_mismatched_row_tensors(self):
        from vllm_ascend.worker.v1.sample.sampling_context import V1SamplingContext
    ...
    def test_from_model_runner_inputs_rejects_out_of_range_request_indices(self):
        from vllm_ascend.worker.v1.sample.sampling_context import V1SamplingContext
    ...
    def test_from_model_runner_inputs_requires_expanded_rows_grouped_by_request(self):
        from vllm_ascend.worker.v1.sample.sampling_context import V1SamplingContext
