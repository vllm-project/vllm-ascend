...
def test_i1_greedy_bit_exact(logits):
    ...
    k = 64
    _, cdist = force_topk_sample(logits, _one(1.0), _one(1.0), _one(-1, dtype=torch.int32), None, {}, k)
...
def test_i2_top_k_effective(logits):
    ...
    sampled, _ = force_topk_sample(rep, _one(1.0, N), _one(1.0, N), _one(m, N, torch.int32), None, {}, k)
...
def test_i2_top_p_nucleus_alignment(logits):
    ...
    k = 128
    top_p = 0.9
    _, cdist = force_topk_sample(logits, _one(1.0), _one(top_p), _one(-1, dtype=torch.int32), None, {}, k)
...
def test_distribution_kl_small(logits):
    ...
    N = 20000
    rep = logits.repeat(N, 1)
    sampled, _ = force_topk_sample(rep, _one(1.0, N), _one(1.0, N), _one(-1, N, torch.int32), None, {}, k)
    V = logits.shape[-1]
...
def test_fallback_guard_num_logprobs_gt_k():
    ...
    def is_safe(num_logprobs, k, no_penalties, want_raw_logprobs):
        if num_logprobs > k:
            return False
        return not (want_raw_logprobs and not no_penalties)
