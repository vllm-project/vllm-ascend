...
class TestAscendSchedulerConfig(unittest.TestCase):
    def test_initialize_from_config(self):
        ...
        vllm_scheduler_config = SchedulerConfig(max_num_batched_tokens=max_num_batched_tokens, max_model_len=max_num_batched_tokens, chunked_prefill_enabled=True)
        ...
        self.assertEqual(conf.scheduler_cls, "vllm_ascend.core.scheduler.AscendScheduler")
        self.assertEqual(conf.max_num_encoder_input_tokens, max_num_batched_tokens)
