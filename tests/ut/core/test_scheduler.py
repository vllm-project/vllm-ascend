...
class TestAscendScheduler(TestBase):
    ...
    def test_init(self, mock_super_init: MagicMock):
        ...
        mock_structured_output_manager = MagicMock(spec=StructuredOutputManager)
    ...
    def test_get_prompt_limit(self):
        ...
        self.assertEqual(res3, scheduler.scheduler_config.max_num_batched_tokens)
    ...
    def test_finish_requests_single_multiple(self, mock_finish_requests):
        ...
        mock_finish_requests.finish_requests.assert_called_once_with(req_ids, RequestStatus.FINISHED)

    @patch("vllm.v1.core.sched.scheduler.Scheduler.update_from_output")
    def test_update_from_output_single_scheduled(self, mock_update_from_output):
        scheduler = MagicMock(spec=AscendScheduler)
        scheduler.running = []
        scheduler.requests = {}  # ! 保存scheduler中的requests
        ...
        mock_update_from_output.assert_called_once_with(scheduler_output, model_runner_output)

    @patch("vllm.v1.core.sched.scheduler.Scheduler.update_from_output")
    def test_update_from_output_single_unscheduled(self, mock_update_from_output):
        scheduler = MagicMock(spec=AscendScheduler)
        scheduler.running = []
        scheduler.requests = {}  # ! 保存scheduler中的requests
        scheduler.scheduled_req_ids = set()
