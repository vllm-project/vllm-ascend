...
def mock_expert_load_balancer(tmp_path):
    ...
    # 初始化 ExpertLoadBalancer 实例
    return ExpertLoadBalancer(str(json_file), global_expert_num=8)


def test_init(mock_expert_load_balancer):
    assert isinstance(mock_expert_load_balancer.expert_map_tensor, torch.Tensor)
    assert mock_expert_load_balancer.layers_num == MOCK_DATA["moe_layer_count"]
    assert mock_expert_load_balancer.ranks_num == MOCK_DATA["layer_list"][0]["device_count"]
...
def test_generate_index_dicts(mock_expert_load_balancer):
    ...
    expected_result = [{7: 0, 2: 1, 0: 2, 3: 3, 5:4}, {6: 5, 1: 6, 4: 7, 7: 8, 2: 9}]
...
def test_generate_log2phy_expert_map(mock_expert_load_balancer):
    layer_id = 0
    log2phy_map = mock_expert_load_balancer.generate_log2phy_expert_map(layer_id)
...
def test_get_rank_placement_map(mock_expert_load_balancer):
    layer_id = 0
    rank_id = 0
    rank_local_expert_num, rank_expert_map = mock_expert_load_balancer.get_rank_placement_map(layer_id, rank_id)
    assert rank_local_expert_num == 5
    assert rank_expert_map.equal(torch.tensor([ 2, -1,  1,  3, -1,  4, -1,  0], dtype=torch.int32))
    rank_id = 1
    rank_local_expert_num, rank_expert_map = mock_expert_load_balancer.get_rank_placement_map(layer_id, rank_id)
    assert rank_expert_map.equal(torch.tensor([-1,  1,  4, -1,  2, -1,  0,  3], dtype=torch.int32))


def test_get_rank_log2phy_map(mock_expert_load_balancer):
    layer_id = 0
    rank_id = 0
    log2phy_map = mock_expert_load_balancer.get_rank_log2phy_map(layer_id, rank_id)
    assert log2phy_map.equal(torch.tensor([2, 6, 1, 3, 7, 4, 5, 0], dtype=torch.int32))
    rank_id = 1
    log2phy_map = mock_expert_load_balancer.get_rank_log2phy_map(layer_id, rank_id)
    assert log2phy_map.equal(torch.tensor([2, 6, 9, 3, 7, 4, 5, 8], dtype=torch.int32))
