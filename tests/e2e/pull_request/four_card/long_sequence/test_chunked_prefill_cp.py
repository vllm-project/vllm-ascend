...
INPUT_PROMPTS = [
    ...
    LONG_PROMPT + "Question: what is the age of Alice Johnson? Your answer: The age of Alice Johnson is ",
]
...
def test_models_chunked_prefill_mixed_length_prompts_including_1_token(model: str):
...
def test_models_chunked_prefill_with_cp_basic(model: str, max_tokens: int) -> None:
    with VllmRunner(
        model,
        block_size=128,
        max_model_len=4096,
        ...
        chunked_prefill_outputs = vllm_model.generate_greedy(INPUT_PROMPTS, max_tokens)
...
@pytest.mark.parametrize("max_tokens", [2])
@patch.dict(os.environ, {"HCCL_BUFFSIZE": "768"})
def test_models_chunked_prefill_with_cp_full_graph(model: str, max_tokens: int) -> None:
    with VllmRunner(
        model,
        block_size=128,
        max_model_len=4096,
        ...
        prefill_context_parallel_size=SETTINGS[model]["PCP"],
        decode_context_parallel_size=SETTINGS[model]["DCP"],
        compilation_config={"cudagraph_capture_sizes": [4, 8, 24, 48, 60], "cudagraph_mode": "FULL_DECODE_ONLY"},
    ) as vllm_model:
        chunked_prefill_outputs = vllm_model.generate_greedy(INPUT_PROMPTS, max_tokens)
