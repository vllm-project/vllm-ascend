...
def assert_distribution_match(
    reference: dict, actual: dict, prompt: list[int], replay: Callable[[list[int]], dict]
) -> None:
    """Check all answer positions, replaying the reference history after a fork.

    Use the existing Ascend sequence-parallel precision test's top-5 distribution
    limits (maximum delta < 1.0, mean delta < 0.15). Additionally require mutual
    top-5 greedy membership, as in upstream models/utils.py check_logprobs_close.
    Unlike stopping at the first greedy difference, replay checks every position
    against the same history. It never forces a generated answer token.
    """
    ...
    assert maximum < 1.0, f"Conditional distribution corruption: max delta={maximum:.6f}, at={largest[1:]}"
    assert mean < 0.15, f"Conditional distribution drift: mean delta={mean:.6f}"
