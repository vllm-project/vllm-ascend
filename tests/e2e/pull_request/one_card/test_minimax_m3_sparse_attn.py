...
def _ensure_npu_sparse_attention_score_op() -> None:
    ...
    # A5 registers the in-tree binding when vllm_ascend_C is imported and
    # intentionally disables the generic custom-op loader. Still invoke the
    # loader for A3, then decide from the registered operator itself.
    enable_custom_op()
...
def _sparse_tolerances(_backend: SparseAttnBackend) -> tuple[float, float]:
    if _backend == "torch_npu" and get_ascend_device_type() == AscendDeviceType.A5:
        # A5 uses inner_precise=1 for the BF16/FP8-capable Split-KV kernel.
        return 8.0e-4, 2.0e-2
...
def test_prefill_schedule_metadata(q_lens, kv_lens, query_prefix, num_q_heads):
    """Requires a metadata-aware A3/A5 vendor package and explicit ABI opt-in."""
    if not NPU_AVAILABLE or not envs.VLLM_ASCEND_MINIMAX_M3_PREFILL_METADATA:
        pytest.skip("Set VLLM_ASCEND_MINIMAX_M3_PREFILL_METADATA=1 with an A3/A5 metadata vendor package")
    _ensure_npu_sparse_attention_score_op()
    ...
    inner_precise = 1 if get_ascend_device_type() == AscendDeviceType.A5 else 0
