...
from tests.e2e.nightly.single_node.models.scripts.kv_pool_runtime import create_single_node_kv_pool_manager
    ...
    def _terminate_server(self) -> None:
        ...
        for process in alive:
            with suppress(psutil.NoSuchProcess):
                process.kill()
        _, alive = psutil.wait_procs(alive, timeout=CLEANUP_WAIT_SECONDS)
        assert not alive, (
            f"PD test cleanup left owned processes alive after SIGKILL: {[process.pid for process in alive]}"
        )
...
class _ObservedPDServer(_BoundedServer):
    ...
    def require_pool_load(self, request_id: str) -> None:
        with self.output_changed:
            observed = self.output_changed.wait_for(lambda: pool_load_observed(self.lines, request_id), timeout=10)
        assert observed, f"Missing nonempty pool load with zero P-local cached tokens for {request_id}"
...
def _reset_prefix_cache(port: int) -> None:
    # Async saves can briefly retain cache blocks after an HTTP request ends.
    # Retry only an explicit busy result, never errors or malformed replies.
    deadline = time.monotonic() + 10
    while True:
        response = requests.post(f"http://127.0.0.1:{port}/reset_prefix_cache", timeout=REQUEST_TIMEOUT)
        if response.status_code == 200 and response.content and response.json() == {"success": False}:
...
def run_pd_contract(
    *,
    ...
    block_size: int = 128,
    distribution_maximum_delta: float = 1.0,
    distribution_mean_delta: float = 0.15,
    kv_pool: bool = False,
) -> None:
    ...
    assert not kv_pool or prefix, "Pool reuse requires a warmed prefix and explicit local-cache resets"
    assert len(prompt_lengths) == len(PROMPTS) and all(length > 0 for length in prompt_lengths)
    if max_num_batched_tokens is None:
        max_num_batched_tokens = 512 if prefix else 2048
    ...
    def compare(url, prompt, reference, actual):
        if compare_logprobs:
            ...
            assert_distribution_match(
                ...
                maximum_delta=distribution_maximum_delta,
                mean_delta=distribution_mean_delta,
            )
        else:
    ...
    with _ObservedPDServer([baseline_args], env_dict=baseline_env):
        ...
        correlated_index = 3 if prefix else 0
        correlated_prompt = prompts[correlated_index]
        correlated_reference = reference_responses[correlated_index]
        if prefix:
            assert isinstance(correlated_prompt, list)
            # The initial proxy requests warm P with every complete prompt. A
            # byte-for-byte repeat can become a full local hit for hybrid page
            # layouts, leaving the scheduler no token to execute. Extend the
            # warmed prefix by one known token so this step measures the
            # intended partial-prefix transfer contract.
            correlated_prompt = [*correlated_prompt, correlated_prompt[-1]]
            correlated_reference = reference_response(baseline_url, correlated_prompt)
        if compare_logprobs:

            def add_replay_references(prompt: list[int], reference: dict) -> None:
                tokens = reference["choices"][0]["token_ids"]
                reference["_pd_replay_references"] = [
                    reference_response(baseline_url, prompt + tokens[:position], max_tokens=1)
                    for position in range(len(tokens))
                ]

            for prompt, reference in zip(prompts, reference_responses):
                assert isinstance(prompt, list)
                add_replay_references(prompt, reference)
            if correlated_reference is not reference_responses[correlated_index]:
                assert isinstance(correlated_prompt, list)
                add_replay_references(correlated_prompt, correlated_reference)
                ...
                "kv_connector_extra_config": {
                    "connectors": [
                        config,
                        {
                            ...
                            "kv_role": role,
                            "kv_connector_extra_config": {
                                "lookup_rpc_port": str(get_open_port()),
                                "backend": "mooncake",
                                "use_layerwise": False,
                                "load_async": False,
                                # D must still pull from P; its pool lookup is
                                # disabled so a pool hit cannot bypass real PD.
    ...
    with (
        create_single_node_kv_pool_manager(pool_config, f"pd-e2e-{uuid.uuid4().hex}") as pool_manager,
        _ObservedPDServer(
            servers,
            ...
            env_dict={
                **pool_manager.server_envs,
                **({"PYTHONHASHSEED": "0", "ASCEND_ENABLE_USE_FABRIC_MEM": "1"} if kv_pool else {}),
                "VLLM_LOGGING_LEVEL": "DEBUG",
                "VLLM_SERVER_DEV_MODE": "1" if prefix else "0",
            },
        ) as pd,
        ...
        prompt = correlated_prompt
        if prefix:
            _reset_prefix_cache(decode_port)
        if kv_pool:
            # The initial requests have stored P's prompt KV in Mooncake.
            # Remove the P-local copy to require Pool -> P -> D for this request.
            _reset_prefix_cache(prefill_port)
        prefilled = _request(
        ...
        if kv_pool:
            pd.require_pool_load(transfer["remote_request_id"])
        decoded = _request(
        ...
        compare(url, prompt, correlated_reference, decoded)
