...
def test_models_with_aclgraph_full_decode_only(
    ...
    vllm_eager_outputs_list = []
    if model == "vllm-ascend/DeepSeek-V2-Lite-W8A8":
        for i in range(len(vllm_aclgraph_ds_answers)):
            vllm_eager_outputs_list.append((0, vllm_aclgraph_ds_answers[i]))
    else:
        for i in range(len(vllm_aclgraph_qwen_answers)):
            vllm_eager_outputs_list.append((0, vllm_aclgraph_qwen_answers[i]))
