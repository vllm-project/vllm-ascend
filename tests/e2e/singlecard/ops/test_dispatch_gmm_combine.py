...
def worker(rank: int, world_size: int, port: int, q: mp.SimpleQueue):
    matmulReduceScatter_op = MatmulReduceScatter(rank, world_size, port)
    matmulReduceScatter_op.generate_hcom()
    out = matmulReduceScatter_op.run_npu_out()
    q.put(out)


@torch.inference_mode()
def test_dispatch_gmm_combine_kernel():
    world_size = 16
