...
# 设置环境变量，禁用自定义算子以避免 torch.compile 编译失败
# 当 VLLM_BATCH_INVARIANT=1 时，enable_custom_op() 会直接返回 False
os.environ["VLLM_BATCH_INVARIANT"] = "1"

DEFAULT_MODEL = "/home/weight/Qwen3-0.6B"
...
def test_aclgraph_v1_generation_is_deterministic_across_batch_sizes_with_needle(monkeypatch: pytest.MonkeyPatch, vllm_runner):
    """
...
def test_aclgraph_logprobs_bitwise_batch_invariance_bs1_vs_bsN(monkeypatch: pytest.MonkeyPatch, vllm_runner):
    """
    Test that logprobs are bitwise identical between BS=1 and BS=N runs.
...
def test_aclgraph_logprobs_without_batch_invariance_should_fail(monkeypatch: pytest.MonkeyPatch, vllm_runner):
    """
