...
from vllm_ascend.utils import MAX_CAPTURE_SIZE
...
def test_acl_graph_capture_replay_dp2(
    model: str,
    max_tokens: int,
) -> None:
    ...
    max_num_batch_sizes = math.floor(
        (MAX_CAPTURE_SIZE - num_comm_groups * 40) / num_acl_graphs /
        (1 + num_comm_groups * 2))
    ...
    expected_total_replay = num_acl_graphs * num_inference_steps * dp_size
...
@pytest.mark.parametrize("max_tokens", [4])
@patch.dict(os.environ, {"ASCEND_RT_VISIBLE_DEVICES": "0,1"})
def test_acl_graph_mem_use(model: str, max_tokens: int) -> None:
    ...
    # Empirical observation: capturing ACL graphs for Qwen3-8B uses ~0.33 GiB of NPU memory.
    # a 1.3x tolerance is applied to account for runtime variance.
    baseline_capture_mem = 0.33
