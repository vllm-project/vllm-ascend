...
def run_with_dp(
    ...
    result_queue.put({"global_dp_rank": global_dp_rank,"start": current_start,"results": outputs})
...
def run_with_lmhead_tp(
    ...
    result_queue.put({"lmhead_tp_size":lmhead_tp_size,"results":final_results})
    exit(exit_code)

def check_precision(ref_outputs:List[Tuple],spec_outputs:List[Tuple],threshold:float=0.66):
    matches = 0
    misses = 0
...
@patch.dict(os.environ, {"ASCEND_RT_VISIBLE_DEVICES": "0,1","VLLM_USE_MODELSCOPE":"True","VLLM_WORKER_MULTIPROC_METHOD":"spawn","HCCL_BUFFSIZE":"512"})
def test_lmhead_tp_mtp_dp2(model):
    ...
    for lmhead_tp_size in lmhead_tp_sizes:
        proc=multiprocessing.Process(
            ...
            args=(args,lmhead_tp_size,global_result_queue)
        )
        proc.start()
        ...
        proc.join()
        if proc.exitcode!=0:
            pytest.fail(f"The process with lmhead_tp_size={lmhead_tp_size} abnormally exited")
    
    tp1_res,tp2_res=[global_result_queue.get() for _ in range(2)]
    check_precision(ref_outputs=tp1_res["results"],spec_outputs=tp2_res["results"])