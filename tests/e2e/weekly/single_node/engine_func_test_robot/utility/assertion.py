...
def assert_messages_response_success(response, msg=""):
    """Verify Anthropic Messages API success response structure."""
    assert_status_code_200(response, msg)
    data = response.json()
    check.equal(data.get("type"), "message", f"{msg}Response type should be 'message'")
    check.equal(data.get("role"), "assistant", f"{msg}Response role should be 'assistant'")
    content = data.get("content")
    check.is_true(isinstance(content, list) and len(content) > 0, f"{msg}Response content should be a non-empty list")
    return data


def assert_messages_response_has_text(response, msg=""):
    """Verify Messages API response contains text content."""
    data = assert_messages_response_success(response, msg)
    text_blocks = [block for block in data["content"] if block.get("type") == "text"]
    check.is_true(len(text_blocks) > 0, f"{msg}Response should contain at least one text block")
    text = text_blocks[0].get("text", "")
    check.is_true(text and text.strip(), f"{msg}Text content should not be empty")
    return data


def assert_messages_response_has_text_or_tool_use(response, msg=""):
    """Verify Messages API response contains text or tool_use content (for tool calling)."""
    data = assert_messages_response_success(response, msg)
    valid_blocks = [block for block in data["content"] if block.get("type") in ["text", "tool_use"]]
    check.is_true(len(valid_blocks) > 0, f"{msg}Response should contain text or tool_use blocks")
    return data


def assert_messages_stream_response_success(response, msg=""):
    """Verify Anthropic Messages API streaming response contains expected SSE events."""
    assert_status_code_200(response, msg)
    events = [line.decode("utf-8") for line in response.iter_lines() if line]
    check.is_true(len(events) > 0, f"{msg}Streaming response should contain events")
    check.is_true(any("message_start" in e for e in events), f"{msg}Streaming should contain 'message_start' event")
    check.is_true(
        any("content_block_delta" in e for e in events), f"{msg}Streaming should contain 'content_block_delta' event"
    )
    check.is_true(any("message_stop" in e for e in events), f"{msg}Streaming should contain 'message_stop' event")
    return events
