...
def test_presence_penalty_accepts_supported_range(api_client, value):
...
def test_presence_penalty_accepts_streaming_request(api_client):
...
def test_presence_penalty_rejects_invalid_values(api_client, value):
    response = completion_request.send_chat_request(api_client, **{PARAM_NAME: value})
    assertion.assert_validation_error_response(response)


def test_presence_penalty_combines_with_other_penalties(api_client):
