...
CONFIG_BASE_PATH = os.getenv("CONFIG_BASE_PATH") or "tests/e2e/schedule/scripts/single_node/configs"
...
class SingleNodeConfig:
    ...
    def __post_init__(self) -> None:
        # Assign a free port to any env explicitly set to "DEFAULT_PORT" (covers
        # MOONCAKE_PORT / MOONCAKE_METRICS_PORT and any future port key), then
        # backfill the standard named keys when they are missing entirely.
        for env_key, value in list(self.envs.items()):
            if value == "DEFAULT_PORT":
                self.envs[env_key] = str(get_open_port())
        named_port_keys = ["SERVER_PORT", "ENCODE_PORT", "PD_PORT", "PROXY_PORT"]
        for env_key in named_port_keys:
            if self.envs.get(env_key) is None:
                self.envs[env_key] = str(get_open_port())
...
class SingleNodeConfigLoader:
    ...
    def _load_yaml(cls, yaml_path: str | None) -> dict[str, Any]:
        ...
        # Accept either a path that already resolves -- as the CI workflow passes,
        # e.g. tests/e2e/schedule/model/<Family>/<resource_dir>/X.yaml -- or a bare
        # filename resolved against CONFIG_BASE_PATH (local usage per the guide).
        # Mirrors tests/e2e/schedule/scripts/multi_node/utils.py:load_yaml_mapping.
