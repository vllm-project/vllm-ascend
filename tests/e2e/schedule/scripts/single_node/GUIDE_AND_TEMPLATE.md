...
* **Configuration Parser (`single_node_config.py`)**: Responsible for reading the config YAML (selected by `CONFIG_YAML_PATH`) and parsing it into a strongly-typed `@dataclass` (`SingleNodeConfig`) via `SingleNodeConfigLoader`, while handling regex replacement for environment variables.
...
* `tests/e2e/schedule/scripts/single_node/single_node_config.py`
    * Defines `SingleNodeConfig` and `SingleNodeConfigLoader`
    * Loads YAML from `CONFIG_YAML_PATH` directly when it resolves (the full repo path the CI passes), otherwise from `<CONFIG_BASE_PATH>/<CONFIG_YAML_PATH>` (default base `tests/e2e/schedule/scripts/single_node/configs/`)
...
* `tests/e2e/schedule/scripts/single_node/test_single_node.py`
...
pytest starts
  |
  v
import tests/e2e/schedule/scripts/single_node/test_single_node.py
  |
  v
...
pytest -sv tests/e2e/schedule/scripts/single_node/test_single_node.py
```
...
* Periodic model YAMLs live under: `tests/e2e/schedule/model/<Family>/<resource_dir>/`
  (CI passes the full path via `CONFIG_YAML_PATH`); local configs may also live
  under `tests/e2e/schedule/scripts/single_node/configs/`
* Selected by env var: `CONFIG_YAML_PATH=<full-or-base-relative path>.yaml`
...
| `mooncake`       | map        | No      | `{}`             | Launch a Mooncake master sidecar alongside the server. `config` is written to `MOONCAKE_CONFIG_PATH` (`$VAR` expanded); optional `launcher` passes kwargs to `MooncakeLauncher`. Requires `MOONCAKE_PORT`/`MOONCAKE_METRICS_PORT` in `envs`. |
...
* `envs` (ports): any env value set to `DEFAULT_PORT` is replaced with an available
  open port (e.g. `SERVER_PORT`, `MOONCAKE_PORT`, `MOONCAKE_METRICS_PORT`). The
  named keys `SERVER_PORT`, `ENCODE_PORT`, `PD_PORT`, `PROXY_PORT` are also
  auto-filled when missing entirely.
    * `$SERVER_PORT` / `${SERVER_PORT}` placeholders in commands (and in the
      `mooncake.config` values) will be expanded using `envs`.
