...
from tests.e2e.conftest import DisaggEpdProxy, MooncakeLauncher, RemoteEPDServer, RemoteOpenAIServer
from tests.e2e.schedule.scripts.single_node.single_node_config import (
    SingleNodeConfig,
...
_PORT_ENV_KEYS = {"SERVER_PORT", "ENCODE_PORT", "PD_PORT", "PROXY_PORT", "MOONCAKE_PORT", "MOONCAKE_METRICS_PORT"}
...
def _write_mooncake_config(config: SingleNodeConfig) -> None:
    """Materialize the mooncake.json referenced by MOONCAKE_CONFIG_PATH.

    Expands $VAR/${VAR} placeholders (e.g. $MOONCAKE_PORT) in the config values
    against the case envs without mutating the (possibly anchor-shared) config dict.
    """
    mooncake_cfg = config.mooncake.get("config")
    if not mooncake_cfg:
        return
    expanded = {
        key: (SingleNodeConfig._expand_values([value], config.envs)[0] if isinstance(value, str) else value)
        for key, value in mooncake_cfg.items()
    }
    path = config.envs.get("MOONCAKE_CONFIG_PATH", "mooncake.json")
    with open(path, "w", encoding="utf-8") as f:
        json.dump(expanded, f)
    logger.info("Wrote mooncake config to %s: %s", path, expanded)
...
async def test_single_node(config: SingleNodeConfig) -> None:
    ...
    # Standard OpenAI service mode, optionally fronted by a Mooncake master sidecar.
    with contextlib.ExitStack() as stack:
        if config.mooncake:
            _write_mooncake_config(config)
            launcher_kwargs = config.mooncake.get("launcher", {})
            # Enter the Mooncake master first so its LD_LIBRARY_PATH/env is in place
            # before the vLLM server process starts.
            stack.enter_context(
                MooncakeLauncher(
                    int(config.envs["MOONCAKE_PORT"]),
                    int(config.envs["MOONCAKE_METRICS_PORT"]),
                    **launcher_kwargs,
                )
            )
