...
from tests.e2e.nightly.bisect.config import BisectInput, BisectOptions
...
def _launch_pytest(inp: BisectInput, opt: BisectOptions, log_path: Path) -> int:
    ...
    with open(log_path, "a", encoding="utf-8") as out:
        ...
        try:
            proc = subprocess.run(
                ["bash", "-c", bash_cmd], cwd=str(opt.repo_dir), env=env,
                stdout=out, stderr=subprocess.STDOUT, timeout=opt.trial_timeout_s,
            )
