...
import torch
import time
from vllm_ascend.ops.triton.linearnorm.split_qkv_rmsnorm_rope import split_qkv_gated_rmsnorm_rope_impl
...
def _qkv_rmsnorm_rope_with_gate_naive(
    ...
    q_rope = _apply_rope(q_norm, sin, cos, rotary_dim).reshape(*orig_shape, q_size).to(orig_dtype)
    k_rope = _apply_rope(k_norm, sin, cos, rotary_dim).reshape(*orig_shape, kv_size).to(orig_dtype)
...
def test_qkv_rmsnorm_rope_fusion_with_gate_triton_kernel(
    ...
    qkv = torch.randn(num_tokens,
                      ...
                      dtype=dtype,
                      device=device)
    q_weight = torch.randn(head_size,
                           dtype=dtype,
                           device=device)
    k_weight = torch.randn(head_size,
                           dtype=dtype,
                           device=device)

    q_trt, gate_trt, k_trt, v_trt = split_qkv_gated_rmsnorm_rope_impl(qkv,
                                                                sin,
                                                                cos,
                                                                q_weight,
                                                                k_weight,
                                                                ...
                                                                rotary_dim,
                                                                eps)
    q_gold, gate_gold, k_gold, v_gold = _qkv_rmsnorm_rope_with_gate_naive(qkv,
                                                                sin,
                                                                cos,
                                                                q_weight,
                                                                k_weight,
                                                                ...
                                                                eps)
    # Compare the results.
    torch.testing.assert_close(q_trt.to(torch.float32),
                               q_gold.to(torch.float32),
                               atol=DEFAULT_ATOL,
                               rtol=DEFAULT_RTOL)
    torch.testing.assert_close(gate_trt.to(torch.float32),
                               gate_gold.to(torch.float32),
                               atol=DEFAULT_ATOL,
                               rtol=DEFAULT_RTOL)
    torch.testing.assert_close(k_trt.to(torch.float32),
                               k_gold.to(torch.float32),
                               atol=DEFAULT_ATOL,
                               rtol=DEFAULT_RTOL)
    torch.testing.assert_close(v_trt.to(torch.float32),
