...
class DecodeMoeOps(torch.nn.Module):
    def __init__(self, gmm1_weight, gmm1_weight_scale, gmm2_weight, gmm2_weight_scale, ep_hcomm_info, 
                 batch_size, token_hidden_size, moe_intermediate_size, 
                 ep_world_size, moe_expert_num, global_rank_id, shared_expert_rank_num=0):
        super().__init__()
        self.gmm1_weight = None
        self.gmm1_weight_scale = None
        self.gmm2_weight = None
...
class SmallOps(DecodeMoeOps):
    ...
    def __init__(self, gmm1_weight, gmm1_weight_scale, gmm2_weight, gmm2_weight_scale, ep_hcomm_info, 
                 batch_size, token_hidden_size, moe_intermediate_size, 
                 ep_world_size, moe_expert_num, global_rank_id, shared_expert_rank_num=0):
        super().__init__(gmm1_weight, gmm1_weight_scale, gmm2_weight, gmm2_weight_scale, ep_hcomm_info,
                            batch_size, token_hidden_size, moe_intermediate_size, 
                            ep_world_size, moe_expert_num, global_rank_id, shared_expert_rank_num)
        self.tp_hcomm_info = ""
    ...
    def _apply_ops(self, x, expert_ids, smooth_scales, expert_scales):
        ...
        combine_output = torch_npu.npu_moe_distribute_combine_v2(
            ...
            shared_expert_rank_num=self.shared_expert_rank_num,
            global_bs=self.batch_size * self.ep_world_size)
        return (combine_output, ep_send_counts)
...
class FusionOp(DecodeMoeOps):
    ...
    def _process_weights_after_loading(self, gmm1_weight, gmm1_weight_scale, gmm2_weight, gmm2_weight_scale):
        ...
        gmm1_weight = torch_npu.npu_format_cast(gmm1_weight, torch_npu.Format.ND)
        gmm1_weight.add_(0)
        gmm1_weight = torch_npu.npu_format_cast(gmm1_weight, torch_npu.Format.FRACTAL_NZ)
        gmm1_weight_scale = permute_weight(gmm1_weight_scale, 128)
        gmm2_weight = torch_npu.npu_format_cast(
            gmm2_weight.transpose(1, 2).contiguous(), torch_npu.Format.FRACTAL_NZ)
...
def generate_datas(
        batch_size, token_hidden_size, moe_intermediate_size, 
        ep_world_size, moe_expert_num, global_rank_id, shared_expert_rank_num=0,
        top_k=8, test_bfloat16=True, enable_dynamic_bs=False):
    ...
    actual_bs = int(torch.randint(1, batch_size, [1]).item() if enable_dynamic_bs else batch_size)
    ...
    expert_ids = torch.arange(
        ...
        global_rank_id * batch_size * top_k + actual_bs * top_k).to(torch.int32).view(actual_bs, top_k)
    if is_shared_expert:
        gmm1_weight = torch.ones([local_expert_num, gmm1_input_dim, gmm1_output_dim
                                  ]).to(torch.int8) * 4
        gmm2_weight = torch.ones([local_expert_num, gmm2_input_dim, gmm2_output_dim
                                  ]).to(torch.int8) * 4
...
def run_once(local_rank_id,
        batch_size, token_hidden_size, moe_intermediate_size, 
        ep_world_size, moe_expert_num, shared_expert_rank_num=0,
        top_k=8, test_bfloat16=True, enable_dynamic_bs=False, test_graph=False):
    ...
    small_ops = SmallOps(*weight_datas, ep_hcomm_info_small, *parameter).npu()
    fused_ops = FusionOp(*weight_datas, ep_hcomm_info_fused, *parameter).npu()
    if test_graph:
        fused_ops = torch.compile(fused_ops, mode="default")
    ...
    if log_file is not None:
        log_file.close()
    torch.testing.assert_close(small_op_token_output.cpu(), fused_op_token_output.cpu())
...
def test():
    ...
    args=(batch_size, token_hidden_size, moe_intermediate_size,
                    ...
                    top_k, test_bfloat16, enable_dynamic_bs, test_graph)
