...
assert len(sys.argv) == 7, "入参列表：[0]rank_size, [1]share_expert_rank_num, [2]moe_expert_num, [3]bs, [4]name, [5]loop_cnt"
...
class FusionOp(torch.nn.Module):
    ...
    def forward(self, x, expert_ids, gmm1_weight, gmm1_weight_scale, gmm2_weight, gmm2_weight_scale, smooth_scales, expert_scales):
        ...
        output = torch.ops._C_ascend.dispatch_gmm_combine_decode(x, expert_ids, gmm1_weight, gmm1_weight_scale, gmm2_weight,
...
def test_diff_data():
    ...
    for test_time in range(diff_test_time):
        ...
        print(f"[info-{global_rank_id}] test:{test_time+1}/{diff_test_time}, {diff.max()= }, {diff.mean()= }, {error_max=}, {error_mean= }")
