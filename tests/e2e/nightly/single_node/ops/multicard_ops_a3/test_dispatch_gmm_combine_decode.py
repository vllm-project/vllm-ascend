...
def redirect_output(log_file_path):
    ...
    with open(LOG_NAME + "/" + log_file_path, "w", encoding="utf-8") as f:
...
def run_once(
    ...
    ctx = redirect_output(f"local_rank_{local_rank_id}.log") if output_to_file(local_rank_id) else contextlib.nullcontext()
    with ctx:
