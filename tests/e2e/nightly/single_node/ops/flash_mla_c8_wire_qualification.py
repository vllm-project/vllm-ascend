...
python flash_mla_c8_wire_qualification.py --device 0 --json qualification.json [--native]
...
def merge_inputs(tokens, seed):
    ...
    maximum = torch.maximum(
        stats.masked_fill(~finite, -float("inf")).amax(0), local_stat_ht.masked_fill(~local_finite, -float("inf"))
    )
...
def qualify_merge(tokens):
    ...
    return {
        "tokens": tokens,
        "pitch257_vs272": "bitwise",
        "invalid_and_current": "reference pass",
        "changed_input_graph": 3,
    }
...
def qualify_native(checkpoint=None):
    ...
    for lengths in ([16384] * 16, [0, 1, 127, 128, 129, 1023, 2049, 16384] * 2, [32768]):
        ...
        def run(case=case, metadata=metadata):
        ...
        reports.append(
            {
                ...
                "lengths": lengths,
                "fd": fd,
                "output_lse_wire_padding": "bitwise/zero",
                "changed_input_graph": 3,
            }
        )
...
def main():
    ...
    assert "raw_row_words" in inspect.signature(exchange.pack_raw_dcp_output_lse).parameters, (
        "Framework candidate is not installed."
    )
