...
@torch.inference_mode()
def test_kimi_decode_gate_direct_output(tp_group, monkeypatch, num_tokens):
    from vllm_ascend.ops.triton.flash_attention_output import flash_attention_gate, flash_attention_output

    rank, size = tp_group.rank_in_group, tp_group.world_size
    torch.manual_seed(20260917 + rank)
    x = torch.randn(num_tokens, 1536, dtype=torch.bfloat16, device="npu")
    ...
    output = torch.empty(num_tokens // size, 7168, device="npu", dtype=torch.bfloat16)
    shard_live = live[rank * output.shape[0] : (rank + 1) * output.shape[0]]

    def candidate():
        flash_attention_gate(x, gates, live)
        return op.apply_into(x, output)
    ...
    for mode in ("all_live", "alternating", "all_inactive"):
        ...
        if mode == "alternating":
            live[1::2] = False
        x.masked_fill_(~live[:, None], torch.nan)
        gates.masked_fill_(~live[:, None], torch.inf)
        parallel = torch.nn.functional.linear(x * torch.sigmoid(gates), weight)
        original = torch.empty_like(output)
        dist.reduce_scatter_tensor(original, parallel, group=tp_group.device_group)
        expected = torch.empty_like(output)
        flash_attention_output(original, shard_live, expected)
        graph.replay()
        torch.npu.synchronize()
