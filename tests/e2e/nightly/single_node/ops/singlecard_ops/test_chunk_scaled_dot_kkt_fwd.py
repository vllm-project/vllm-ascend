...
def test_chunk_scaled_dot_kkt_fwd_fixed_len(T, Hg, H, use_g, dtype, atol, rtol):
    ...
    golden = chunk_scaled_dot_kkt_fwd_golden(k.cpu().to(torch.float32), beta.cpu().to(torch.float32),
...
def test_chunk_scaled_dot_kkt_fwd_varlen(seqlens, Hg, H, dtype, atol, rtol):
    ...
    A = chunk_scaled_dot_kkt_fwd(k=k, beta=beta, g_cumsum=g_cumsum,
                                 cu_seqlens=cu_seqlens, chunk_size=chunk_size)
    golden = chunk_scaled_dot_kkt_fwd_golden(k.cpu().to(torch.float32), beta.cpu().to(torch.float32),
                                             g_cumsum.cpu().to(torch.float32),
                                             chunk_size=chunk_size, cu_seqlens=cu_seqlens.cpu())
