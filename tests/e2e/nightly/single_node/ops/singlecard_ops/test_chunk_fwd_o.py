...
def chunk_fwd_o_golden(q, k, v, h, g=None, scale=None, cu_seqlens=None,
                       chunk_size=64, chunk_offsets=None):
...
def test_chunk_fwd_o_fixed_len(T, Hg, H, use_g, dtype, atol, rtol):
    ...
    golden = chunk_fwd_o_golden(q.cpu().to(torch.float32), k.cpu().to(torch.float32),
                                v.cpu().to(torch.float32), h.cpu().to(torch.float32),
...
def test_chunk_fwd_o_varlen(seqlens, Hg, H, dtype, atol, rtol):
    ...
    o = chunk_fwd_o(q=q, k=k, v=v, h=h, g=g, scale=K ** -0.5,
                    cu_seqlens=cu_seqlens, chunk_size=BT, chunk_offsets=chunk_offsets)
    golden = chunk_fwd_o_golden(q.cpu().to(torch.float32), k.cpu().to(torch.float32),
                                v.cpu().to(torch.float32), h.cpu().to(torch.float32),
                                g.cpu().to(torch.float32), scale=K ** -0.5, chunk_size=BT,
                                cu_seqlens=cu_seqlens.cpu(), chunk_offsets=chunk_offsets.cpu())
