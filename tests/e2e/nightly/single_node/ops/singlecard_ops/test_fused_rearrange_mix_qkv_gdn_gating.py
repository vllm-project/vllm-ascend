...
def load_op():
    ...
    if get_ascend_device_type() not in {AscendDeviceType.A2, AscendDeviceType.A3}:
        pytest.skip("npu_fused_rearrange_mix_qkv_gdn_gating is only built for A2 and A3")
...
def test_matches_references(q_dim, k_dim, v_dim, tokens, a_log_dtype, dt_bias_dtype):
    num_heads = v_dim // 128
    ...
    A_log = torch.randn(num_heads, dtype=a_log_dtype, device="npu")
    dt_bias = torch.randn(num_heads, dtype=dt_bias_dtype, device="npu")

    packed_qkv, g, beta = torch.ops._C_ascend.npu_fused_rearrange_mix_qkv_gdn_gating(
        mixed_qkv,
        a,
        b,
        A_log,
...
def test_nondefault_stream_and_graph_replay(tokens):
    ...
    with torch.npu.stream(stream):
        ...
        with torch.npu.graph(graph, stream=stream):
            captured = torch.ops._C_ascend.npu_fused_rearrange_mix_qkv_gdn_gating(
                mixed_qkv,
                a,
                b,
                A_log,
...
def test_reject_unsafe_inputs(kind):
    ...
    with pytest.raises(RuntimeError, match="npu_fused_rearrange_mix_qkv_gdn_gating requires"):
        torch.ops._C_ascend.npu_fused_rearrange_mix_qkv_gdn_gating(
            mixed_qkv,
            a,
            b,
            A_log,
