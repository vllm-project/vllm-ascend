...
def test_mla_prolog_v3_k3_kimi_k3_no_rope_matches_reference():
    ...
    weight_uq_qr_nd = torch.randn((q_lora_rank, head_num * (qk_nope_head_dim + qk_rope_head_dim)), dtype=dtype).npu()
    ...
    ql_nope, q_pe, *_ = torch.ops._C_ascend.npu_mla_prolog_v3_k3(
        token_x,
        weight_dq,
        weight_uq_qr,
        weight_uk,
    ...
    q_projection = (q_norm @ weight_uq_qr_nd).view(token_num, head_num, qk_nope_head_dim + qk_rope_head_dim)
    q_nope, q_pe_reference = q_projection.split([qk_nope_head_dim, qk_rope_head_dim], dim=-1)
    ql_nope_reference = torch.bmm(q_nope.transpose(0, 1), weight_uk).transpose(0, 1)
...
def test_mla_prolog_v3_k3_arch22_bf16_pa_bsnd(token_num: int, head_num: int, he: int):
...
@torch.inference_mode()
def test_mla_prolog_v3_k3_arch22_rope_disabled():
...
@torch.inference_mode()
def test_mla_prolog_v3_k3_arch22_partial_quant():
...
@torch.inference_mode()
def test_mla_prolog_v3_k3_arch22_full_int8_quant():
...
@torch.inference_mode()
def test_mla_prolog_v3_k3_arch22_bsnd():
