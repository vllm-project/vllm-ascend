...
def chunk_local_cumsum_golden(g, chunk_size, reverse=False, scale=None,
...
def test_chunk_local_cumsum_fixed_len(T, H, reverse, dtype, atol, rtol):
    ...
    golden = chunk_local_cumsum_golden(g.cpu().to(torch.float32), chunk_size,
                                       reverse=reverse, scale=scale, head_first=False)
...
def test_chunk_local_cumsum_varlen(seqlens, H, dtype, atol, rtol):
    ...
    out = chunk_local_cumsum(g, chunk_size, reverse=False, scale=None,
                             cu_seqlens=cu_seqlens, head_first=False)
    golden = chunk_local_cumsum_golden(g.cpu().to(torch.float32), chunk_size,
                                       reverse=False, scale=None,
                                       cu_seqlens=cu_seqlens.cpu(), head_first=False)
