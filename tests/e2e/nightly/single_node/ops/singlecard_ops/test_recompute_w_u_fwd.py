...
def test_recompute_w_u_fwd_varlen(seqlens, Hg, H, dtype, atol, rtol):
    ...
    w, u = recompute_w_u_fwd(k=k, v=v, beta=beta, g_cumsum=g_cumsum, A=A, cu_seqlens=cu_seqlens)
    w_g, u_g = recompute_w_u_fwd_golden(k.cpu().to(torch.float32), v.cpu().to(torch.float32),
                                        beta.cpu().to(torch.float32), g_cumsum.cpu().to(torch.float32),
                                        A.cpu().to(torch.float32), cu_seqlens=cu_seqlens.cpu())

    for out, golden in [(w.cpu().to(torch.float32), w_g.to(torch.float32)),
                        (u.cpu().to(torch.float32), u_g.to(torch.float32))]:
        a = golden.abs() > 1
        a1 = golden.abs() <= 1
        torch.testing.assert_close(out * a, golden * a, atol=atol, rtol=100)
