...
ACCURACY_CASES = [
    ...
    {
        # One valid context token remains after rejecting the speculative suffix.
        "name": "context_equals_spec_steps_plus_one_rejected",
        "req_lens": [5],
        "position_starts": [7],
        "idx_mapping": [1],
        "num_rejected": [4],
    ...
    },
    {
        ...
        "num_query_per_req": 5,
        "num_speculative_steps": 4,
        "parallel_drafting_token_id": 235,
    },
    {
...
def _build_reference(data, case):
    ...
    for req_idx in range(num_reqs):
        ...
        if valid_ctx_end > ctx_start:
            ...
            last_valid_pos = positions[ctx_start] - 1
        ...
        for ctx_idx in range(ctx_start, ctx_end):
            if ctx_idx >= valid_ctx_end:
                ref.context_positions[ctx_idx] = 0
        ...
        ref.seq_lens[req_idx] = min(last_valid_pos + 1 + num_query_per_req, max_model_len)
