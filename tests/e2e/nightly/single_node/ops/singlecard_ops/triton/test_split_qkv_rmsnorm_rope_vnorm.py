...
def v_rms_norm(v, num_tokens, head_size, eps):
    """Reference for the weight-less per-head V norm the kernel applies.

    Matches `RMSNorm(head_dim, has_weight=False)`, i.e. the same reduction as
    the q/k norm with a unit scale.
    """
    ones = torch.ones(head_size, dtype=torch.float32, device="cpu")
    return rms_norm(v.reshape(-1, head_size), ones, eps).reshape(num_tokens, -1)


@pytest.mark.parametrize("max_position_embeddings", MAX_POSITION_EMBEDDINGS)
@pytest.mark.parametrize("num_tokens", NUM_TOKENS)
@pytest.mark.parametrize("num_q_heads, num_kv_heads, head_size, rope_dim", QKV_SHAPES)
@pytest.mark.parametrize("eps", EPS)
@pytest.mark.parametrize("dtype", DTYPES)
@pytest.mark.parametrize("seed", SEEDS)
@pytest.mark.parametrize("device", DEVICES)
@torch.inference_mode()
def test_split_qkv_rmsnorm_rope_vnorm(
    ...
    # split
    _q, _k, _v = qkv.cpu().split([q_hidden_size, kv_hidden_size, kv_hidden_size], dim=-1)
    # norm
    ...
    # v is normalized per head without a learnable scale and is not roped
    v_gold = v_rms_norm(_v, num_tokens, head_size, eps)
    ...
    torch.testing.assert_close(v.to(torch.float32).cpu(), v_gold, atol=DEFAULT_ATOL, rtol=DEFAULT_RTOL)
...
@pytest.mark.parametrize("device", DEVICES)
@torch.inference_mode()
def test_split_qkv_rmsnorm_rope_vnorm_with_bias(
    max_position_embeddings, num_tokens, num_q_heads, num_kv_heads, head_size, eps, dtype, seed, device, rope_dim
):
    torch.manual_seed(seed)
    torch.set_default_device(device)
    ...
    _q = _q.reshape(num_tokens, 1, -1, head_size)
    _k = _k.reshape(num_tokens, 1, -1, head_size)
    # the v norm carries neither a scale nor a bias, so it is unaffected here
    v_gold = v_rms_norm(_v, num_tokens, head_size, eps)
