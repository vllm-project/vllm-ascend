...
def test_rejection_greedy_sample_spec_len_1_triton_kernel():
    device = "npu"
    batch_size = 5
...
@torch.inference_mode()
def test_rejection_greedy_sample_triton_kernel():
    device = "npu"
    batch_size = 6
    max_spec_len = 3
...
def test_rejection_random_sample(max_spec_len, vocab_size, batch_size):
    ...
    torch.npu.synchronize()
    assert torch.equal(output_token_ids_ref, output_token_ids)
    gc.collect()
    torch.npu.empty_cache()
    torch.npu.reset_peak_memory_stats()
