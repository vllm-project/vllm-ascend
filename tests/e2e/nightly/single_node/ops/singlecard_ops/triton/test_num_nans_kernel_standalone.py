...
if not hasattr(torch, "npu") or not torch.npu.is_available():
    pytest.skip(
        "Ascend NPU is required for this standalone accuracy test",
...
def _install_vllm_triton_utils_shim() -> types.ModuleType:
    """Inline shim (standalone copy of runtime_npu._install_vllm_triton_utils_shim).

    The installed Triton is 3.2.0, which predates ``triton.experimental.gluon``
    (and ``triton.experimental.gluon.nvidia``). vLLM's
    ``triton_utils/__init__.py`` unconditionally imports gluon, so merely
    importing ``vllm.triton_utils`` raises ModuleNotFoundError on this host.
    ...
    expose ``tl``/``triton`` from the installed triton 3.2. Must run BEFORE
    any ``vllm.*`` import.
    """
    ...
    def _placeholder_lazy(name: str):
        def _fn(*args, **kwargs):  # pragma: no cover
            ...
            raise RuntimeError(
                ...
                "unavailable under the standalone shim for triton==3.2.0"
            )
...
# Must execute BEFORE any vllm.* import (same ordering contract as the
# original runtime_npu import).
...
def _init_device_properties_triton() -> None:
    """Inline replacement of runtime_npu.init_device_properties_triton.

    Initializes the Ascend Triton driver's device properties (core counts)
    without importing the heavyweight vllm_ascend.ops package.
    """
    ...
    if _NUM_AICORE <= 0 or _NUM_VECTORCORE <= 0:
        raise RuntimeError(
            "Failed to detect Ascend Triton device properties: " f"{properties}"
        )
