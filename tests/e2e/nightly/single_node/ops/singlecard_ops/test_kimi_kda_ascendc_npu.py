...
def _chunked_kda_reference(
    ...
    h = torch.empty(
        (batch_size, chunk_count, heads, head_dim, head_dim), dtype=dtype
    )
    ...
    for batch_idx in range(batch_size):
        for head_idx in range(heads):
            state_kv = (
                initial_state_vk[batch_idx, head_idx]
                .float()
                .transpose(-1, -2)
                .contiguous()
            )
            for chunk_idx in range(chunk_count):
                ...
                gk_block = (
                    torch.cumsum(
                        gate[batch_idx, start:end, head_idx].float(), dim=0
                    )
                    / math.log(2.0)
                )
                relative_gate = gk_block[:, None, :] - gk_block[None, :, :]
                gate_factor = torch.exp2(
                    relative_gate.masked_fill(~causal[:, :, None], 0.0)
                )
                qk = (
                    torch.einsum(
                        "ik,jk,ijk->ij", q_block, k_block, gate_factor
                    )
                    * scale
                )
                kk = torch.einsum(
                    "ik,jk,ijk->ij", k_block, k_block, gate_factor
                )
                aqk_block = torch.where(causal, qk, 0.0)
                strict_kk = torch.where(
                    strict_causal, kk * beta_block[:, None], 0.0
                )
                akk_block = torch.linalg.solve_triangular(
                    eye + strict_kk, eye, upper=False
                )
                ...
                kg_block = k_block * torch.exp2(
                    gk_block[-1][None, :] - gk_block
                )
                v_new_block = u_block - w_block @ state_kv

                gk[batch_idx, head_idx, start:end] = gk_block
                aqk[batch_idx, head_idx, start:end, :chunk_tokens] = (
                    aqk_block.to(dtype)
                )
                akk[batch_idx, head_idx, start:end, :chunk_tokens] = (
                    akk_block.to(dtype)
                )
                ...
                h[batch_idx, chunk_idx, head_idx] = state_kv.transpose(
                    -1, -2
                ).to(dtype)
                o[batch_idx, start:end, head_idx] = (
                    qg_block @ state_kv * scale + aqk_block @ v_new_block
                ).to(dtype)
                state_kv = (
...
def test_kimi_k3_a5_multichunk_all_outputs_match_reference(tokens, heads):
    ...
    raw_gate = torch.randn(
        1, tokens, heads, head_dim, dtype=torch.float32, device="npu"
    ) * 0.1
    beta = torch.rand(
        1, tokens, heads, dtype=torch.float32, device="npu"
    ).sigmoid()
    a_log = torch.randn(heads, dtype=torch.float32, device="npu") * 0.05
    dt_bias = torch.randn(
        heads * head_dim, dtype=torch.float32, device="npu"
    ) * 0.05
    cache_vk = torch.randn(
        1, heads, head_dim, head_dim, dtype=torch.float32, device="npu"
    ) * 0.01
    chunk_indices = tuple(
        value
        for chunk_id in range(math.ceil(tokens / 64))
    ...
    expected = _chunked_kda_reference(
        q, k, v, safe_gate, beta, cache_vk
    )
    _assert_chunk_kda_outputs_close(
        result, expected, retained_indices={0, 1, 3, 4, 11}
    )
    _assert_chunk_kda_outputs_close(
        retained,
        expected,
...
def test_kimi_k3_a5_model_prefill_profile_shape(layout):
    ...
    beta_bsnd = torch.full(
        (1, tokens, heads), 0.5, dtype=torch.float32, device="npu"
    )
    if layout == "TND":
    ...
    cache_vk = (
        torch.eye(head_dim, dtype=torch.float32, device="npu")
    ...
    stored_token_head_shape = (
        (1,) + token_head_shape if layout == "BSND" else token_head_shape
    )
    retained_shapes = (
        output_shape,
        cache_vk.shape,
