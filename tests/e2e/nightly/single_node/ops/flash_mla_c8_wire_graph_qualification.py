...
Use the same candidate PYTHONPATH/OPP as flash_mla_c8_wire_qualification.py. This is a separate
...
def main():
    ...
    try:
        ...
        for name, used, lengths, force_zero_sections in states:
            ...
            inactive = ~active
            if inactive.any():
                assert torch.count_nonzero(output[inactive].contiguous().view(torch.int16)) == 0, (
                    name,
                    "inactive O not exact +0",
                )
                assert torch.equal(
                    lse[:, inactive].contiguous().view(torch.int32),
                    torch.full((96, int(inactive.sum())), -8388608, dtype=torch.int32),
                ), (name, "inactive LSE not -inf")
            if active.any():
            ...
            result["states"].append(
                {
                    ...
                    "used": used,
                    "lengths": lengths,
                    "metadata_head": header,
                    "metadata_source": "explicit zero-section kernel contract"
                    if force_zero_sections
                    else "actual AICPU scheduler",
                    "active_tokens": int(active.sum()),
                    "inactive_tokens": int(inactive.sum()),
                    "same_metadata_address": True,
                    "same_graph_output_addresses": True,
                    "independent_zero_gold": "PASS",
                    "embedded_lse_padding": "PASS",
                    "active_vs_normal": "bitwise PASS" if active.any() else "not used",
                    "graph_vs_eager": "bitwise PASS",
                }
            )
            checkpoint()
