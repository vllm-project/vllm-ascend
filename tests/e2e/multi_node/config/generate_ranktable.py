...
from tests.e2e.multi_node.config.common import (ASCEND_ENV_PATH,
                                                ...
                                                RANKTABLE_PATH, LOAD_BALANCER_PROXY_SCRIPT)
...
class DisaggegatedPrefill:
    def __init__(self, config: MultiNodeConfig = None):
        ...
        self.ips = get_cluster_ips(world_size)
    ...
    def launch_server_proxy(self):
        ...
        cmd = [
            "python", LOAD_BALANCER_PROXY_SCRIPT, "--host", self.ips[0], "--port",
            f"http://{self.ips[0]}:{self.server_port}", "--port",
            1025, "--prefill-only", "--headless"
        ]
        env = os.environ.copy()
