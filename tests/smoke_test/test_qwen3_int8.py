...
def test_vllm_aclgraph_qwen3_32b_server_A2_old(weight_path):
    ...
    try:
        ...
        kill_process = 'pgrep -f "mindie|python|benchmark|torchrun|ray|connector|vllm|VLLM" | xargs kill -9'
        subprocess.run(["bash", "-c", kill_process], capture_output=True, text=True)
