...
- Round 3: gamma 装载移出关键路径 + 常量外提（64-1024 档 -4~-8%）✅
- Round 4: 结案 —— 候选池全部评估完毕（R=2 被 UB 阻断 / Axpy 死路 / 深度 2 与
  Pad→DataCopy 无关键路径收益 / 小 shape 发射瓶颈留档），无正期望值项。
  **累计：2048 bf16 -46%、fp16 -42%（vs 基线），vs unfused ref 快 29%。**
...
### Round 3（2026-09-23）：序言去关键路径 + 常量外提 — ✅ 完成

研究结论先行：c220 无点积类 reduce（仅 Sum/Max），**平方+求和两遍无法融合**；
bf16 的 7 个全宽 pass 全部承载语义（输入 widen/平方/树加/×rstd/×gamma/RINT/输出
widen），pass 数已到算法层下限。本轮做两项零数值风险的微优化：

1. **gamma 装载移出关键路径**：x 行 0 的 load 先上 MTE2（首位），gamma 次之；
   gamma 的 MTE2_V wait（及 bf16 的 widen Cast）推迟到行 0 链中、第一次 gamma
   乘法前一刻——装载完全藏在行 0 的 reduce 后面。R2 及之前是 gamma copy 先于
   x copy 串行 + V 先做 gamma cast，每核白白 ~0.15µs。
2. **常量 1.0f 外提**：`Duplicate(reduce, 1.0f, 1)` 每行重复且行不变，外提到
   每核一次（新增 one_buf 32B，FIXED_UB_BYTES 320→352），每行省 1 op + 1 barrier。

- **精度**：8/8 通过（数值位级不变：同一个 1.0f 参与 Div，gamma cast 只是挪位）。
- **NPUGraph（µs，R2→R3，两次运行取稳定值；2048/4096 波动 ±4µs）**：

| tokens | bf16 R2→R3 | fp16 R2→R3 |
|---|---|---|
| 1 | 2.95→3.1 | 2.84→2.9 |
| 4 | 3.12→3.2 | 3.03→3.07 |
| 16 | 4.08→3.9 | 3.93→3.8 |
| 64 | 6.84→**6.2** | 6.54→**6.1** |
| 128 | 8.83→**8.2** | 8.55→**8.0** |
| 512 | 17.5→**17.0** | 16.9→16.5 |
| 1024 | 29.9→**29.1** | 28.7→28.1 |
| 2048 | 57.4→54~58（噪声带内） | 54.5→53.1 |
| 4096 | 178→180±5（噪声带内） | 174→185（噪声带内） |

- **msprof@2048**：task 54.0µs（R2 53.7，持平），vec 46.5µs/90.3%（持平）。
- **小 shape 根因定位（msprof@1 token，block_dim=1）**：scalar 1.77µs（44%）
  ≫ vec 1.05µs（26%）——**1-16 tokens 是发射（issue）瓶颈**，不是 vec。
  每行 ~40 条标量发射（15 V op + 15 barrier + ~10 事件调用）× ~25ns ≈ 1µs/行，
  流水机制的指令数就是小 shape 的代价；R1 指令更少但有标量停等，互有胜负。
  1-4 tokens 相对 R1 的 +0.3µs 属结构性，除非做专用精简路径（双路径维护成本
  vs ~0.2µs 收益，暂不划算，留作候选）。
- 净效果：64-1024（吞吐相关 prefill 档）-4~-8%，2048+ 持平，1-4 +0.1~0.2µs。

### Round 4 结案（2026-09-23）：候选全部评估完毕，无正期望值项，优化收官

候选池逐项结论（含负结果，避免未来重复推导）：

1. **双行批处理（R=2，省 widen/平方两遍 pass ≈ -14% vec）**：需 x[2]×2行 + x_fp32[2]×2行
   —— bf16 30B/col × 7168 = 215KB > 191KB UB 上限，**被 UB 阻断**。
   （fp16 单独可行 26B/col=186KB，但唯一调用方是 bf16，分叉双路径不值。）
2. **Axpy 合并标量链 Muls+Adds**：c220 `vaxpy` 是 `dst = a·src + dst` 累加语义，
   加数被破坏 → eps 每行都要重灌 → op 数不省；且若为 FMA 单舍入还有数值漂移。**死路**。
3. **预取深度 2（x 三槽）**：MTE2 每行 0.19µs vs V 0.96µs，从来不是约束；vec 空闲
   来自 ramp/tail/barrier（~9%），深度 2 不治。**无收益**。
4. **小 shape 专用精简路径**：1 token 实测 scalar 1.77µs ≫ vec 1.05µs，发射瓶颈；
   双路径上限 ~0.2µs，维护成本不成比例。**留档不做**。
5. ~~DataCopyPad→对齐 DataCopy~~：MTE 侧已全部移出关键路径（mte2/mte3 占比
   40%/34% ≪ vec 90%）。**无关键路径收益**。

**最终记分板**（NPUGraph µs，hidden=7168，基线→三轮后终值）：
...
（4096 为背靠背系统效应口径，孤立执行线性；vs unfused ref：2048 快 29%，4096 快 11%。）
