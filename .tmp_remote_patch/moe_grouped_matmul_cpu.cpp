...
constexpr int32_t LOW_RANK_BASEM = 128;
constexpr int32_t LOW_RANK_BASEN = 16;
constexpr int32_t LOW_RANK_BASEK = 128;
...
ge::graphStatus TilingMoeGroupedMatmulFunc::GMMSetMMTiling() {
    ...
    // Each AIC instance computes one pre-partitioned tile in the kernel, so
    // generate the TCubeTiling for a single-core Matmul API invocation.
    ...
    // The outer kernel owns the multi-core M/N partition. Generate one
    // MatmulImpl tile here; using the full M encoded singleCoreM=global M in
    // TCubeTiling and made each AIC overrun its local L1 tile.
    mm.SetShape(std::min<int64_t>(m_, baseM_), std::min<int64_t>(n_, baseN_), k_);
    ...
    // Keep enough L1 headroom for Matmul's internal state. Deriving both
    // depths from the full advertised L1 size made the A/B double buffers
    // consume the entire L1 on Ascend 910B. mssanitizer then reported a
    // 256-KiB illegal L1 write before the CCU Fixpipe address-check failure.
    // A single K step still enables double buffering through depth=2 below.
    constexpr uint32_t mm_step_ka = 1;
    constexpr uint32_t mm_step_kb = 1;
...
ge::graphStatus TilingMoeGroupedMatmulFunc::CalMMTiling() {
    ...
    // LoRA shrink uses N=rank.  A generic N=256 Fixpipe tile writes beyond
    // the 16-wide result on A2, so use the Phase-0 low-rank tile explicitly.
    if (n_ <= LOW_RANK_BASEN) {
      baseM_ = static_cast<int32_t>(std::min<int64_t>(LOW_RANK_BASEM, SixteenAlign(m_, true)));
      baseN_ = LOW_RANK_BASEN;
      baseK_ = LOW_RANK_BASEK;
      return ge::GRAPH_SUCCESS;
    }
