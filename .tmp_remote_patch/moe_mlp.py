...
def unquant_apply_mlp(
    ...
    lora_routing = None
    if lora_context is not None and not getattr(
        lora_context.punica_wrapper, "no_lora", False
    ):  # LoRA applied; Host-static all-inactive batches are a no-op.
        from vllm_ascend.lora.fused_moe import (
            _prepare_moe_lora_prefill_context,
        ...
        lora_routing = _prepare_moe_lora_prefill_context(
            lora_context,
            hidden_states=hidden_states,
        ...
        if lora_routing is not None:
            pass
        elif expanded_row_idx is not None and topk_ids is not None:
...
def unified_apply_mlp(*, mlp_compute_input: MoEMlpComputeInput) -> torch.Tensor:
    ...
    if not mlp_compute_input.quant.is_quant:
        return unquant_apply_mlp(
            ...
            first_expert_idx=mlp_compute_input.first_expert_idx,
        )
