...
def _prepare_moe_lora_prefill_context(
    lora_context,
    *,
    ...
    routed_topk_ids: torch.Tensor | None,
    first_expert_idx: int,
):
    """Try the certified large-prefill grouped path using only static guards."""
    exchanged_lora_indices = getattr(lora_context, "exchanged_lora_indices", None)
    if expanded_row_idx is not None and routed_topk_ids is not None:
        route_mode = "allgather"
    elif exchanged_lora_indices is not None:
        route_mode = "alltoall"
    else:
        return None
    return lora_context.punica_wrapper.prepare_moe_lora_prefill(
        x=hidden_states,
        y=gate_up_out,
        w13_lora_a=lora_context.w13_lora_a_stacked,
        w13_lora_b=lora_context.w13_lora_b_stacked,
        w2_lora_a=lora_context.w2_lora_a_stacked,
        w2_lora_b=lora_context.w2_lora_b_stacked,
        adapter_enabled=lora_context.adapter_enabled,
...
def moe_lora_apply_w13(lora_context, *, gate_up_out, hidden_states, lora_routing):
    ...
    """
    if isinstance(lora_routing, dict):
        lora_context.punica_wrapper.apply_moe_lora_prefill(
            context=lora_routing,
            y=gate_up_out,
            x=hidden_states,
