...
def _build_messages(mm: dict[str, Any], image_data_list: list[str]) -> list[dict[str, Any]]:
    ...
    if not raw:
        ...
        #         "role": "user",
        #         "content": [
        #             {"type": "text", "text": mm.get("text_prompt", DEFAULT_TEXT_PROMPT)},
    ...
    # placeholder = mm.get("image_placeholder", "{IMAGE_BASE64}")
...
#def _assert_response(response_json: dict[str, Any], expected: dict[str, Any]) -> None:
...
def send_image_request(config: SingleNodeConfig, server) -> dict[str, Any]:
    ...
    image_data = _load_image_data(mm.get("image_path"))
    messages = _build_messages(mm, image_data)
