...
class VllmbenchRunner:
    ...
    def _concat_config_args(self, vllm_bench_cmd):
        for key, value in self.config.items():
            flag = "--" + key.replace("_", "-")
            # Boolean flags are appended bare (e.g. ``ignore_eos=True`` ->
            # ``--ignore-eos``); passing an empty value would be rejected by
            # argparse.
            if value is True:
                vllm_bench_cmd.append(flag)
            elif value is False or value is None:
                continue
            else:
                vllm_bench_cmd += [flag, str(value)]
    ...
    def _performance_verify(self):
        self._get_result()
        # Without a baseline the runner only collects the result JSON, so
        # callers can compare runs themselves (e.g. V1 vs V2 live pairs).
...
def run_vllm_bench_case(model_name, port, config, baseline=None, threshold=0.97, model_path="", host_ip="localhost"):
    try:
