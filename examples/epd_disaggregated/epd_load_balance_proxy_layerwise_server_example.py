...
async def lifespan(app: FastAPI):
    ...
    print(
        ...
        f"and {len(proxy_state.decoders)} decode clients, {len(proxy_state.pds)} pd clients."
    )
    yield
    ...
    try:
        ...
        if proxy_state.pds:
            ...
            try:
                ...
                logger.error("PD %s returned an error before streaming: %s", pd.url, e)
            ...
            try:
                ...
                logger.error("Decoder %s returned an error before streaming: %s", decoder.url, e)
