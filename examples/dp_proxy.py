...
TIME_INTERVAL_FOR_IDLE_RUN = 5e-4   # 50us
DP_SIZE = 2
...
async def send_idle_token_to_client(schedule_dict):
    for key, value in schedule_dict.items():
        ...
        logger.debug(f"send url {target_url}")
        logger.debug(f"original data {idle_request_data}")
        logger.debug(f"forwrad request id: {forward_request_id}")
        generator = forward_request(target_url, idle_request_data, forward_request_id)
        async for response in generator:
            logger.debug(f"Request {request_received_id}: response from {key}, got response: {response}")
            ...
            while not all(schedule_dict.values()):
                if start_time is not None:
                    ...
                    if time_interval > TIME_INTERVAL_FOR_IDLE_RUN:
                        logger.debug("exceeds max time interval send idle token to client")
                        ...
                        for message in messages:
                            ...
                            if data.get("info") == "notify_step":
                                ...
                                if http_addr in schedule_dict.keys():
                                    ...
                                    logger.warning(f"Unrecognize http address")
                            else:
                                logger.warning(f"Got unrecognize info type! We only accept notify step info yet")
                    except (msgpack.UnpackException, TypeError, KeyError) as e:
                        logger.error(f"Error processing message from {http_addr}: {e}. Message: {data}")
        except zmq.ZMQError as e:
            logger.error(f"ZMQ Error in monitor thread: {e}")
            if e.errno == zmq.ETERM:
                logger.error("Monitor thread terminating due to context termination.")
                break 
            time.sleep(1) 
...
def _listen_for_register(poller, router_socket):
    ...
    while True:
        try:
            ...
            if socks:
                logger.debug("receive socks from listen threads: ", socks)
            if router_socket in socks:
                        ...
                        if http_addr:
                            with dp_cv: 
                                if http_addr not in dp_instances:
                                    logger.info(f"Registering DP instance: http={http_addr}")
                                    dp_instances[http_addr] = True
                                    if len(dp_instances.keys()) == DP_SIZE:
                                        logger.info("all dp group joined into the proxy, stop listen!")
                                        dp_cv.notify_all()
                                else:
                                    pass

                        else:
                            logger.warning(f"Warning: Received incomplete DP registration from {remote_address}. Missing 'http_address'. Data: {data}")

                    else:
                        logger.warning(f"Warning: Received message with unexpected type from {remote_address}. Type: {data.get('type')}, Data: {data}")

                except (msgpack.UnpackException, TypeError, KeyError) as e:
                    logger.error(f"Error processing message from {remote_address}: {e}. Message: {message}")
                except Exception as e:
                    logger.error(f"Unexpected error processing message from {remote_address}: {e}")


        except zmq.ZMQError as e:
            logger.error(f"ZMQ Error in listener thread: {e}")
            if e.errno == zmq.ETERM:
...
def start_thread_with_zmq(hostname, port, zmq_key, fn, fn_name):
    ...
    logger.info(f"Starting service discovery listener on tcp://{hostname}:{port}")
    context = zmq.Context()
    ...
    try:
        ...
        logger.error(f"Error binding ZMQ socket to tcp://{hostname}:{port}: {e}")
        router_socket.close()
        context.term()
        raise 
    ...
    logger.debug(f"start fn with name {fn_name}")
        ...
        async with aiohttp.ClientSession(timeout=AIOHTTP_TIMEOUT) as session:
            ...
            async with session.post(url=url, json=data, headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Error from backend {url} (status {response.status}): {error_content.decode(errors='ignore')}")
                    yield error_content 

    except aiohttp.ClientError as e:
        logger.warning(f"Error forwarding request {request_id} to {url}: {e}")
    ...
    try:
        ...
        with dp_cv: 
            if not dp_instances:
                ...
                return response
            logger.debug(f"dp instances:{dp_instances}")
            # --- 轮询选择实例 ---
        ...
        logger.info(f"Request {request_received_id}: Routing to DP instance {target_addr} (Index {current_selection_index})")
        ...
        logger.debug(f"send url {target_url}")
        logger.debug(f"original data {original_request_data}")
        logger.debug(f"forwrad request id: {forward_request_id}")
        ...
        logger.debug(f"Request {request_received_id}: response from {target_addr}")
        return response
...
if __name__ == '__main__':
    ...
    zmq_socket = None
    try:
        logger.info("try start dp listen thread")
        listener_thread, zmq_context, zmq_socket = start_thread_with_zmq("0.0.0.0", 30004, zmq.ROUTER, _listen_for_register, "ZMQListenerThread")
    except Exception as e:
        print(f"Failed to start service discovery: {e}")
        exit(1)

    try:
        logger.info("try start monitor thread")
        _idle_send_loop = asyncio.new_event_loop()
        idle_send_loop_thread = threading.Thread(target=start_thread_with_event_loop,
                                       daemon=True,
                                       name="start_thread_with_event_loop")
        idle_send_loop_thread.start()
        # start idle run monitor thread for all the dp rank, use diffent rank in case of conflict with thread listening one
        metadata_monitor_thread, zmq_monitor_context, zmq_monitor_socket = start_thread_with_zmq("0.0.0.0", 30003, zmq.PULL, metadata_collect_trigger, "metadata_monitor_thread")
    except Exception as e:
        print(f"Metadata collect thread failed to init: {e}")
        exit(1) 

    print(f"Starting Quart web server on http://0.0.0.0:10003 (PID: {os.getpid()})")
    try:
        ...
        idle_send_loop_thread.join()
        _idle_send_loop.stop()
        if zmq_socket:
