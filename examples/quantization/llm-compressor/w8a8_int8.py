...
def load_environment_variables():
    env_vars = {
        ...
        'export_path':
        "/llm-compressor/export/GPTQ/W8A8_W_cha_A_ten_static_symmetric",
        'modifier': "GPTQ",
    ...
    if env_vars['export_path'] is None:
        env_vars['export_path'] = env_vars['model_path'].rstrip(
            "/") + "-" + env_vars['modifier']
...
def load_calibration_text_dataset(calib_prompt_path, tokenizer):
    ...
    for f in os.listdir(calib_prompt_path):
        print(f)
    if any(f.lower().endswith('.jsonl')
           for f in os.listdir(calib_prompt_path)):
        ds = load_dataset('json',
                          data_dir=calib_prompt_path,
                          split='validation')
    elif any(f.lower().endswith('.parquet')
             for f in os.listdir(calib_prompt_path)):
...
def data_collator(batch):
    ...
    return {
        ...
        torch.tensor(
            value,
            dtype=torch.bfloat16 if key == "pixel_values" else torch.long)
...
if __name__ == '__main__':
    ...
    config = AutoConfig.from_pretrained(env_vars['model_path'],
                                        trust_remote_code=True)
    model_type = config.model_type

    model = MODEL_DICT[model_type].from_pretrained(env_vars['model_path'],
                                                   torch_dtype="auto",
                                                   trust_remote_code=True)
    tokenizer = TOKENIZER_DICT[model_type].from_pretrained(
        env_vars['model_path'], trust_remote_code=True)

    ds = load_calibration_text_dataset(env_vars["calib_prompt_path"],
                                       tokenizer)
