...
class ProxyServer:
    def __init__(
        ...
        self.retried_nodes = defaultdict(int)
    ...
    async def retry_forward_request(
        ...
        try:
            ...
            if self.retried_nodes.get(instance) >= RETRY_TIMES:
    ...
    def _node_listener(self):
        while True:
            for node, check_times in list(self.waiting_nodes.items()):
                instance, instance_type = node.split("-")
                is_valid = asyncio.run(self.validate_instance(instance))
                check_times += 1
                if is_valid:
                    self.add_instance_endpoint(instance_type, instance)
                    self.waiting_nodes.pop(node)
                elif check_times == CHECK_TIMES:
    ...
    def add_instances(self, instance_type: str, instances: List[str] = None):
        if instances is None:
            return
        for instance in instances:
            is_valid = asyncio.run(self.validate_instance(instance))
            if not is_valid:
                logger.info(f"Waiting for {instance_type}_type {instance} to start.")
                self.waiting_nodes[f"{instance}_{instance_type}"] = 0
            else:
                self.add_instance_endpoint(instance_type, instance)

    async def async_add_instance_endpoint(self, request: Request):
        try:
            ...
            if not is_valid:
                ...
                return JSONResponse(
                    content={"message": f"Waiting for {instance_type}_type {instance} to start."})
    ...
    def remove_instance_endpoint(self, instance_type: str, instance: str):
        ...
        if instance_type == InstanceType.DECODE and instance in self.decode_instances:
    ...
    def add_instance_endpoint(self, instance_type: str, instance: str):
