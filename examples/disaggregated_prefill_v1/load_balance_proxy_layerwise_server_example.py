    ...
    try:
        ...
        async def generate_stream():
            ...
            try:
                ...
                logger.error(
                    "Error during streaming from decoder %s: %s the aborted request %s will be routing to the target prefiller when new request is ready to dispatch to it",
                    decoder.url,
                    e,
                    request_id,
                )
...
async def metaserver(request: Request):
    try:
        ...
        logger.debug("Using prefill prefiller.url=%s req_data=%s", prefiller.url, req_data)
