...
`--kv-transfer-config`: follow kv_connector, kv_buffer_device, and run on the NPU card. For kv_role, set kv_producer to the p node, kv_consumer to the d node, kv_parallel_size to 1, and kv_port to the port used by the node. For the p node, set engine_id and kv_rank to 0 and for the d node to 1. Mooncake infers the local P/D topology from `--data-parallel-size`, `--tensor-parallel-size`, `--pipeline-parallel-size`, and `kv_role`; request metadata carries the remote topology for unequal P/D TP deployments.<br>
