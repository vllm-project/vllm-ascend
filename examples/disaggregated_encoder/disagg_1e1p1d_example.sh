...
# Prefill worker
###############################################################################
ASCEND_RT_VISIBLE_DEVICES="$CARD_P" vllm serve "$MODEL" \
    ...
    "kv_role": "kv_producer", 
    "kv_connnector_module_path": "vllm_ascend.distributed.mooncake_connector",
...
# Decode worker
###############################################################################
ASCEND_RT_VISIBLE_DEVICES="$CARD_D" vllm serve "$MODEL" \
    --gpu-memory-utilization 0.9 \
    --port "$DECODE_PORT" \
    --enforce-eager \
    --enable-request-id-headers \
