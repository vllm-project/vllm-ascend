...
def use_vllm_logging_config_path():
    ...
    return int(os.getenv("VLLM_CONFIGURE_LOGGING",
                         "1")) and config_path and os.path.exists(config_path)


def replace_logger_file(config_json, logger_file_path):
    for handler_name, handler_config in config_json.get("handlers",
                                                        {}).items():
...
def start_single_node_api_servers(
    ...
    for rank in range(num_servers):
        ...
        try:
            port = find_available_port(base_api_port + rank,
        ...
        existed_logger_files = [
            f for f in glob.glob(logger_path + "*")
            if re.search(f"server_{rank}\.log(\.\d+)?$", f)
        ]
        for existed_file in existed_logger_files:
            print(
                f"The historical log {existed_file} that already exists will be removed."
            )
        ...
        print(
            f"Starting API server {rank} on port {port}, logging to {logger_path}"
        )
        print('=' * terminal_width)
        ...
        for i, (proc, log) in enumerate(process_manager.processes):
            if proc.poll() is None:  # Process is still running
                ...
                try:
                    ...
                    print(
                        f"API Server {i} did not terminate gracefully, killed")
            log.close()
    ...
    print(
        f"Run 'tail -f {log_dir}/server_*.log' to monitor logs in real-time.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=("Start multiple VLLM API servers with combined "
                     "logging, cleanup, and port checking."))
    parser.add_argument("--num-servers",
                        type=int,
                        default=2,
                        help="Number of API servers to start")
    parser.add_argument("--num-dp",
                        type=int,
                        default=None,
                        help="Number of data parallel size.")
    parser.add_argument("--server-offset",
                        type=int,
                        default=0,
                        help="Server offset for multi-nodes")
    parser.add_argument("--model-path",
                        type=str,
                        required=True,
                        help="Path to the model")
    parser.add_argument("--base-api-port",
                        type=int,
                        default=9000,
    ...
    parser.add_argument("--master-ip",
                        type=str,
                        required=True,
                        help="Master IP for data parallelism")
    parser.add_argument("--master-port",
                        type=int,
                        default=8000,
                        help="Master port for data parallelism")
    ...
    parser.add_argument("--max-port-attempts",
                        type=int,
                        default=20,
    ...
    parser.add_argument("--kv-transfer-config",
                        type=str,
                        default="",
                        help="kv transfer config for VLLM")
    parser.add_argument(
        "--extra-args",
        type=str,
        default="",
        help=
        "Additional VLLM arguments (space-separated, e.g., '--enable-expert-parallel')"
    )
    parser.add_argument(
        "--additional-config",
        type=str,
        default="",
        help=
        "JSON-formatted additional platform-specific config, e.g., '{\"key\":\"value\"}'"
    )
    ...
    parser.add_argument("--no-enable-prefix-caching",
                        default=False,
                        action="store_true")
    parser.add_argument("--no-enable-chunked-prefill",
                        default=False,
                        action="store_true")
