...
def get_ip_address(if_name):
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to get IP address for interface {if_name}: {e}")
...
def run_default_mode(args):
    ...
    if args.network_interface:
        intf = {
            'if_name': args.network_interface,
            'ip': get_ip_address(args.network_interface)
        }
    else:
    ...
    if args.graph_true.lower() == 'false':
        ...
        cmd = [
            'python',
            os.path.join(args.code_path,
                         'omniinfer/tools/scripts/start_api_servers.py'),
            '--num-servers',
            '1',
            '--model-path',
            args.model_path,
            '--master-ip',
            intf['ip'],  # Coordinator IP
            '--master-port',
            args.master_port,  # Coordinator port
            '--tp',
            str(len(args.server_list.split(','))),  # tensor parallelism
            '--served-model-name',
            args.model_name,
            '--base-api-port',
            args.https_port,  # HTTP service port
            '--log-dir',
            args.log_path,  # Log directory
            '--extra-args',
            '--enforce-eager '  # Disable graph execution
        ]
...
def start_perfill_api_servers(intf, args):
    ...
    env['RANK_TABLE_PATH'] = os.path.join(
        args.code_path, 'omniinfer/tools/scripts/global_path/')
    env['GLOBAL_RANK_TABLE_FILE_PATH'] = os.path.join(
        args.code_path,
        'omniinfer/tools/scripts/global_path/global_ranktable_merge.json')
    env['RANK_TABLE_FILE_PATH'] = os.path.join(
        args.code_path,
        f'omniinfer/tools/scripts/perfill-ranktable/local_ranktable_{ip}_{prefill_rank_table_suffix}.json'
    )
    ...
    cmd = [
        ...
        args.master_port,  # Coordinator port
        '--base-api-port',
        args.service_port,  # API service port
        '--tp',
        str(len(
            args.prefill_server_list.split(','))),  # 8-way tensor parallelism
        '--served-model-name',
        args.model_name,
        '--max-model-len',
        args.max_model_len,  # Max context length
        '--log-dir',
        args.log_path + '/prefill/',  # Log directory
        '--no-enable-prefix-caching',  # Disable caching
        '--gpu-util',
        '0.9',  # Target NPU utilization
        '--extra-args',
        f'--max-num-batched-tokens {args.max_model_len} --enforce-eager ',  # Perf mance flags
        '--kv-transfer-config',
        json.dumps(kv_transfer_config)  # KV transfer settings
    ]
...
def start_decoder_api_servers(intf, args):
    ...
    env['RANK_TABLE_FILE_PATH'] = os.path.join(
        args.code_path,
        f'omniinfer/tools/scripts/decode-ranktable/local_ranktable_{ip}_{decode_rank_table_suffix}.json'
    )
    ...
    if args.graph_true.lower() == 'false':
        ...
        cmd = [
            ...
            '1',
            '--server-offset',
            server_offset,  # Server offset parameter
            '--num-dp',
            env['DECODE_DP_SIZE'],  # Data parallelism degree
            '--model-path',
            args.model_path,
            '--master-ip',
            intf['ip'],  # Coordinator IP
            ...
            args.master_port,  # Coordinator port
            '--base-api-port',
            str(int(args.service_port) + 100),  # API service port
            '--tp',
            str(len(args.decode_server_list.split(
            ...
            args.max_model_len,  # Max context length
            '--log-dir',
            args.log_path + '/decode/',  # Log directory
            '--no-enable-prefix-caching',  # Disable caching
            '--extra-args',
            f'--max-num-batched-tokens {args.max_model_len} ',  # Performance flag
            '--kv-transfer-config',
            json.dumps(kv_transfer_config)  # KV transfer settings
        ]
    ...
    # Graph mode specific optimizations
        ...
        cmd = [
            ...
            f'--max-num-batched-tokens {args.max_model_len} ',  # Performance flag
            '--additional-config',
            additional_config,  # Graph mode config
            '--kv-transfer-config',
            json.dumps(kv_transfer_config)  # KV transfer settings
        ]
...
def start_global_proxy(intf, args):
    ...
    env = os.environ.copy()
    env['PATH'] = '/user/local/nginx:' + env.get('PATH',
                                                 '')  # Ensure nginx in PATH
    ...
    cmd = [
        ...
        os.path.join(
            args.code_path,
            'omniinfer/omni/accelerators/sched/global_proxy/global_proxy.sh'),
        '--listen-port',
        args.https_port,  # Proxy listening port
        '--prefill-servers-list',
        intf['ip'] + ':' + args.service_port,  # Prefill server endpoints
        '--decode-servers-list',
        intf['ip'] + ':' +
        str(int(args.service_port) + 100),  # Decoder server endpoints
    ]
...
def kill_all_processes():
    ...
    subprocess.run(
        "kill -9 $(ps aux | grep 'start_decode.sh' | awk '{print $2}')",
        shell=True)
    subprocess.run(
        "kill -9 $(ps aux | grep 'start_prefill.sh' | awk '{print $2}')",
        shell=True)
    subprocess.run(
        "kill -9 $(ps aux | grep 'run_benchmark.sh' | awk '{print $2}')",
        shell=True)
    subprocess.run("kill -9 $(ps aux | grep 'python' | awk '{print $2}')",
                   shell=True)
    subprocess.run("kill -9 $(ps aux | grep 'python3.11' | awk '{print $2}')",
                   shell=True)
    subprocess.run("kill -9 $(ps aux | grep 'python3.10' | awk '{print $2}')",
                   shell=True)
    subprocess.run(
        "kill -9 $(ps aux | grep 'start_do_16.sh' | awk '{print $2}')",
        shell=True)
...
def pd_ranktable(intf, args):
    ...
    cmd_p = [
        'python',
        os.path.join(args.code_path,
                     'omniinfer/tools/scripts/pd_ranktable_tools.py'),
        '--mode',
        'gen',
        '--prefill-server-list',
        args.prefill_server_list,  # NPU IDs for prefill
    ...
    cmd_d = [
        ...
        'gen',
        '--decode-server-list',
        args.decode_server_list,  # NPU IDs for decoder
        '--save-dir',
        './decode-ranktable',
    ]
    ...
    cmd_global = [
        ...
        os.path.join(args.code_path,
                     'omniinfer/tools/scripts/pd_ranktable_tools.py'),
        '--mode',
        'merge-all',  # Merge all ranktables
        '--api-server-list',
        f'perfill-ranktable/local_ranktable_{ip}_host.json',
        '--prefill-server-list',
        f'perfill-ranktable/local_ranktable_{ip}_{prefill_rank_table_suffix}.json',
        '--decode-server-list',
        f'decode-ranktable/local_ranktable_{ip}_{decode_rank_table_suffix}.json',
        '--save-dir',
        'global_path'
    ]
...
def run_pd_separate_mode(args):
    ...
    # User control loop
    while True:
        user_input = input("\nEnter 'yes' to start global proxy, 'q' to quit: "
                           ).strip().lower()
        ...
        if user_input == 'yes' or user_input == 'y' or user_input == 'Y' or user_input == 'YES':
            ...
            print(
                "Invalid input. Please enter 'yes' to proceed or 'q' to quit.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=
        "OmniInfer Deployment Script - Launches model servers in different deployment modes"
    )
    parser.add_argument(
        '--model-path',
        type=str,
    ...
    parser.add_argument(
        ...
        default='default',
        help=
        "Deployment strategy: 'default'  or 'pd_separate'  (default: default)")
    parser.add_argument(
        '--graph-true',
        type=str,
        default='false',
        help=
        "Enable graph optimization mode: 'true' for optimized execution, 'false' for standard mode (default: false)"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "Network interface name for distributed communication (default: auto-detect)"
    )
    parser.add_argument(
        '--host-ip',
        type=str,
        default=None,
        help=
        "Local machine's IP address for service binding (default: auto-detect from network interface)"
    )
    parser.add_argument(
        '--model-name',
        type=str,
    ...
    parser.add_argument(
        ...
        default='20960',
        help=
        "Maximum context length supported by the model in tokens (default: 20960)"
    )
    parser.add_argument(
        '--log-path',
        type=str,
        default='./apiserverlog',
    ...
    parser.add_argument(
        ...
        help=
        "default mode: NPU device IDs for parallel processing (default: 0-7)")
    parser.add_argument(
        '--prefill-server-list',
        type=str,
        default='0,1,2,3,4,5,6,7',
        help=
        "pd-separated:NPU device IDs dedicated to prompt prefill processing (default: '0,1,2,3,4,5,6,7')"
    )
    parser.add_argument(
        '--decode-server-list',
        type=str,
        ...
        help=
        "pd-separated:NPU device IDs dedicated to token decoding processing (default: '8,9,10,11,12,13,14,15')"
    )
    ...
    parser.add_argument(
        ...
        default='6660',
        help=
        "- In 'pd' mode: Prefill service port (Decoder uses this port + offset)\n"
    ...
    args.code_path = get_path_before_omniinfer(
    )  # Get base path before 'omniinfer'
