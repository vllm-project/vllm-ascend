...
if size == 26:
    config["Reshape"]["input0"][
        "dtype"] = "int4," + config["Reshape"]["input0"]["dtype"]
    config["Reshape"]["input0"][
        "format"] = "FRACTAL_NZ," + config["Reshape"]["input0"]["format"]
    config["Reshape"]["input0"][
        "unknownshape_format"] = "FRACTAL_NZ," + config["Reshape"]["input0"][
            "unknownshape_format"]

    config["Reshape"]["input1"][
        "dtype"] = "int32," + config["Reshape"]["input1"]["dtype"]
    config["Reshape"]["input1"][
        "format"] = "ND," + config["Reshape"]["input1"]["format"]
    config["Reshape"]["input1"]["unknownshape_format"] = "ND," + config[
        "Reshape"]["input1"]["unknownshape_format"]

    config["Reshape"]["output0"][
        "dtype"] = "int4," + config["Reshape"]["output0"]["dtype"]
    config["Reshape"]["output0"][
        "format"] = "FRACTAL_NZ," + config["Reshape"]["output0"]["format"]
    config["Reshape"]["output0"][
        "unknownshape_format"] = "FRACTAL_NZ," + config["Reshape"]["output0"][
            "unknownshape_format"]
    ...
    if '910b' in config_file:
        gmmfr_format_list = config["GroupedMatmulFinalizeRouting"]["input1"][
            "format"].split('.')
        ...
        config["GroupedMatmulFinalizeRouting"]["input1"][
            "unknownshape_format"] = new_format
