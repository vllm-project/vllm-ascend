...
def setup_logging(log_file='omni_cli.log', log_level=logging.INFO):
    ...
    logger.info("=" * 80)
    logger.info(
        f"Log system initialization completed - Log level: {logging.getLevelName(log_level)}"
    )
    ...
    for key, value in group.items():
        ...
        for keyName, hostConfig in value.items():
            if new_key := key_map.get(keyName):
                ...
                logger.error(
                    "There are configurations that do not meet the requirements! \
                    The configurations that do not meet the requirements are: %s",
                    keyName)
                return None
...
def transform_config_for_inventory(logger, input_data):
    ...
    required_keys = {
        'user', 'master_port', 'base_api_port', 'ascend_rt_visible_devices'
    }
    ...
    for kv_rank, group_valus in enumerate(prefill.values()):
        ...
        if missing_keys:
            logger.error(
                "The group of prefill is missing the necessary configuration. \
        ...
        for i, host in enumerate(hosts_list):
            ...
            p_hosts[host_key] = {
                ...
                f'{kv_rank}',
                'node_port':
                group_valus['master_port'],
                'api_port':
                group_valus['base_api_port'],
                'ascend_rt_visible_devices':
                group_valus['ascend_rt_visible_devices'],
                'host_ip':
                hosts_list[0]
            }
            ...
            if 'private_key' in group_valus.keys():
                p_hosts[host_key][
                    'ansible_ssh_private_key_file'] = group_valus[
                        'private_key']
            elif 'password' in group_valus.keys():
                p_hosts[host_key]['ansible_password'] = group_valus['password']
            else:
                logger.error(
                    "The group of prefill is missing necessary configuration. \
            ...
            if host in host_overrides.keys():
                p_hosts[host_key].update({
                    k: v
                    for k, v in host_overrides[host].items()
                })
    ...
    for i, host in enumerate(hosts_list):
        ...
        host_config = {
            ...
            'ascend_rt_visible_devices':
            decode_group['ascend_rt_visible_devices'],
            'host_ip': hosts_list[0]
        }

        if 'private_key' in decode_group.keys():
            host_config['ansible_ssh_private_key_file'] = decode_group[
                'private_key']
        elif 'password' in decode_group.keys():
            host_config['ansible_password'] = decode_group['password']
        else:
            logger.error(
                "The group of decode is missing necessary configuration. \
                The missing configuration is: private_key or password")
            return None

        if host in host_overrides.keys():
            host_config.update({k: v for k, v in host_overrides[host].items()})
...
def transform_config_for_playbook(logger, input_data):
    ...
    global_required_keys = {
        'log_path', 'log_path_in_executor', 'model_path', 'code_path',
        'docker_image'
    }
    ...
    if 'max_model_len' not in input_data['services']['prefill'].keys():
        logger.error("Missing necessary configuration for prefill: %s",
                     'max_model_len')
        return None
    if 'max_model_len' not in input_data['services']['decode'].keys():
        logger.error("Missing necessary configuration for decoding: %s",
                     'max_model_len')
        return None

    updates["LOG_PATH"] = input_data['services']['log_path']
    updates["LOG_PATH_IN_EXECUTOR"] = input_data['services'][
        'log_path_in_executor']
    ...
    updates["MODEL_LEN_MAX_PREFILL"] = input_data['services']['prefill'][
        'max_model_len']
    updates["MODEL_LEN_MAX_DECODE"] = input_data['services']['decode'][
        'max_model_len']
...
def update_yml_file(logger, updates, file_path):
    ...
    if not encoding:
        logger.warning(
            "The encoding of the omni_infer_server.yml file is unknown. UTF-8 is used by default."
        )
        encoding = 'utf-8'
    else:
        logger.info(
            f"Detected omni_infer_server.yml file encoding: {encoding}")
    ...
    with open(f'{os.getcwd()}/omni_infer_server.yml', 'w',
...
def transform_deployment_config(config_path):
    ...
    try:
        ...
        if not encoding:
            logger.warning(
                "The encoding of the config_path file is unknown. UTF-8 is used by default."
            )
            encoding = 'utf-8'
        else:
        ...
        logger.info(
            f"Convert the output files to {os.getcwd()}/omni_infer_inventory.yml and \
