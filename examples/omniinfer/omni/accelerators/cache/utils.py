...
def compute_omni_attn_metadata(
    ...
    if num_prefills > 0:
        raise RuntimeError(
            "Only support PD disaggregation and only enabled in D now.")
    if num_decode_tokens % num_decodes != 0:
        raise RuntimeError(
            f"num_decode_tokens is {num_decode_tokens} while num_decodes is {num_decodes}"
        )

    num_reqs = num_decodes + num_prefills
    tokens_per_req = num_decode_tokens // num_decodes
    # TODO: should change to numpy array for faster performance
    block_table_np = block_table.get_numpy_array(
    )[:num_reqs]  # (batch_size, max_blocks)
    sink, recent, block_size = (kv_cache_spec.sink, kv_cache_spec.recent,
    ...
    slot_mapping = block_table_np[
        req_ids, block_num_per_token] * block_size + block_offset_per_token

    if tokens_per_req > 1:
        # [1,3,5,...]
        last_token_idx = np.arange(tokens_per_req - 1, num_decode_tokens,
                                   tokens_per_req)
        block_num_last_token = block_num_per_token[last_token_idx]
        mask = (seq_lens > sink + recent)
        block_shifts = np.where(
            mask, kv_cache_spec.max_num_blocks - block_num_last_token - 1, 0)
        ...
        omni_attn_seq_lens = (seq_len_last_token[:, None] +
                              np.arange(-tokens_per_req + 1, 1)).flatten()
    else:
    ...
    return (
        ...
        torch.from_numpy(omni_attn_seq_lens).to(torch.int64)
        if omni_attn_seq_lens is not None else None,
    )
...
def to_bool_or_raise(val) -> bool:
    ...
    raise ValueError(
        f"Cannot convert variable to bool. Type {type(val)}. Value {val}.")
