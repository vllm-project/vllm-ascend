...
thread_dump_path = os.environ.get("VLLM_THREAD_DUMP_PATH",
                                  "/tmp/vllm_thread_info")
...
P_NODE_BIN = os.environ.get(
    "P_NODE_BIN",
    "/data/yyx/omni_infer_cache/omniinfer/omni/accelerators/pd/H2H_pull_kv/p_node_server"
)
LIB_PATH = os.environ.get(
    "KV_BRIDGE_SO",
    "/data/yyx/omni_infer_cache/omniinfer/omni/accelerators/pd/H2H_pull_kv/libkv_py_bridge.so"
)
...
class DatadistConnectorMetadataPrefill(KVConnectorMetadata):
    ...
    def add_new_req(
        ...
        self.requests[request_id] = ReqMeta(finish_time=finish_time)
...
class LLMDataDistConnector(KVConnectorBase_V1):
    ...
    def __init__(self, vllm_config: VllmConfig, role: KVConnectorRole):
        ...
        if vllm_config.model_config.is_deepseek_mla:
            ...
            logger.info(
                "Set kv_parallel_size to 1 when use deepseek mla model.")

        self.datadist_config = LLMDataDistConfig(vllm_config,
                                                 ignore_load_rank=True)
    ...
    def request_finished(
        ...
        return self.connector_scheduler.request_finished(
            request, block_ids, spec_token_ids)
    ...
    def start_load_kv(self, forward_context: "ForwardContext",
        ...
        if not isinstance(
                ...
                Union[DatadistConnectorMetadata,
                      DatadistConnectorMetadataPrefill]):
            raise RuntimeError(
                "self._connector_metadata must be an instance of DatadistConnectorMetadata or DatadistConnectorMetadataPrefill"
            )
...
class PrefillConnectorWorker:
    ...
    def __init__(self, vllm_config: "VllmConfig", host_ip: str,
        ...
        use_omni_attn_mgr = check_omni_attn_cmd_arg(
    ...
    def start_p_server_kv_transfer(self, kv_pool_mmap_path, data_type,
        ...
        p_procs = start_p_node_servers(BASE_PORT, 1, [kv_pool_mmap_path],
                                       ...
                                       start_offset)
        attach_logs(p_procs)
        ok, not_ready = _wait_ports([("127.0.0.1", BASE_PORT + i)
                                     for i in range(1)])
        if not ok:
            for p in p_procs:
                try:
...
class DecodeConnectorWorker:
    ...
    def __init__(self, vllm_config: "VllmConfig", host_ip: str,
        ...
        self.lib.kv_dpool_init_from_mmap.argtypes = [
            c_char_p, c_char_p, c_size_t, c_size_t
        ]
        ...
        self.lib.kv_dpool_get_blocks.argtypes = [
            c_void_p, POINTER(c_uint64), c_size_t,
            POINTER(KVBlockC)
        ]
        self.lib.kv_dpool_get_blocks.restype = c_size_t
        self.lib.kv_dpool_release.argtypes = [
            c_void_p, POINTER(c_uint64), c_size_t
        ]
        self.lib.kv_dpool_release.restype = None

        # Decode helper
        self.lib.kv_decode_block_to_string.argtypes = [
            c_void_p, c_size_t, c_char_p, c_size_t
        ]
        ...
        if additional_config:
            self.async_pull_kv = additional_config.get("async_pull_kv", False)
            self.multi_thread_pull_kv = additional_config.get(
                "multi_thread_pull_kv", False)
            self.multi_rank_pull_kv = additional_config.get(
                "multi_rank_pull_kv", False)
        else:
            self.async_pull_kv = False
        ...
        if vllm_config.parallel_config.tensor_parallel_size > 1 and self.multi_rank_pull_kv:
            raise ValueError(
                "multi_rank_pull_kv are not supported when tp > 1.")
        ...
        if self.async_pull_kv:
            ...
            dump_thread_to_file(self.thread_on_fast_path_req, thread_name,
                                thread_dump_path)
        ...
        if self.multi_thread_pull_kv and self.vllm_config.parallel_config.tensor_parallel_size > 1:
            ...
            if get_tensor_model_parallel_rank() == 0:
                ...
                self.sync_thread = threading.Thread(
                    target=self.sync_pulled_tp_kvcache_and_send,
                    daemon=True,
                    name=thread_name)
                self.sync_thread.start()
                dump_thread_to_file(self.sync_thread, thread_name,
                                    thread_dump_path)
        ...
        while True:
            try:
                if self.input_socket.poll(timeout=10) > 0:
                    ...
                    logger.debug(
                        f"{request_id} finish pull kv {self.tp_sync_req_dict[request_id]} times."
                    )
                    if self.tp_sync_req_dict[
                            request_id] == self.vllm_config.parallel_config.tensor_parallel_size:
                ...
                logger.error("Sync pulled kv when tp > 1 and send failed: %s",
                             e)
    ...
    def worker(self, cluster_id):
        ...
        while True:
            ...
            try:
                ...
                logger.error("KV transfer task failed in thread %s: %s",
                             cluster_id, e)
                self._send_pulled_kv_req_list(task['remote_host_ip'],
                                              [task['request_id']])
                raise RuntimeError(
                    f"Failed to pull kv for request:{task['request_id']} from cluster:{cluster_id}."
                )
            q.task_done()

    def register_kv_caches(self, kv_pool_mmap_path, data_type, block_len_dtype,
                           start_offset):
        self.start_build_connection_with_p_server(NODE_SPECS,
                                                  kv_pool_mmap_path, data_type,
                                                  block_len_dtype,
                                                  start_offset)

    def start_build_connection_with_p_server(self, node_specs,
                                             kv_pool_mmap_path, data_type,
                                             block_len_dtype, start_offset):
        logger.warning(
            f"[Trying] D mmap initialize: kv_pool_mmap_path={kv_pool_mmap_path}, data_type={data_type}, block_len_dtype={block_len_dtype}, start_offset={start_offset}"
        )
        if not self.lib.kv_dpool_init_from_mmap(
                kv_pool_mmap_path.encode('utf-8'), data_type.encode('utf-8'),
                c_size_t(block_len_dtype), c_size_t(start_offset)):
            raise RuntimeError("[ERROR] init D from mmap failed")
        logger.warning(
            f"[OK] D mmap initialize: kv_pool_mmap_path={kv_pool_mmap_path}, data_type={data_type}, block_len_dtype={block_len_dtype}, start_offset={start_offset}"
        )

        if not self.lib.kv_dpool_set_direct_id_mode(c_uint64(0)):
            raise RuntimeError("[ERROR] setting direct-id mode failed")
        logger.warning(
            f"[OK] direct-id: id_base={0}, D logic index = (block_id - id_base)"
        )

        if not self.init_conn_pool_with_retry(node_specs):
            raise RuntimeError(
                "[ERROR] Failed to initialize connection pool to P nodes")
        logger.warning(
            "[OK] ConnectionPool initialized and maintained by C++ background threads"
        )

    def init_conn_pool_with_retry(
            self,
            node_specs,
            max_wait_sec: float = None,  # type: ignore
            retry_interval_sec: float = None,  # type: ignore
            backoff: float = None) -> bool:  # type: ignore
        max_wait_sec = float(os.environ.get(
            "CONN_POOL_MAX_WAIT_SEC",
            "600")) if max_wait_sec is None else float(max_wait_sec)
        retry_interval_sec = float(
            os.environ.get("CONN_POOL_RETRY_INTERVAL_SEC", "0.5")
        ) if retry_interval_sec is None else float(retry_interval_sec)
        backoff = float(os.environ.get(
            "CONN_POOL_BACKOFF", "1.5")) if backoff is None else float(backoff)

        deadline = time.time() + max_wait_sec
        attempt = 1
        while time.time() < deadline:
            arr = (c_char_p *
                   len(node_specs))(*[s.encode("utf-8") for s in node_specs])
            if self.lib.kv_init_conn_pool(arr, c_size_t(len(node_specs))):
                print(f"[OK] ConnectionPool initialized (attempt {attempt})",
                      flush=True)
                return True
            remaining = max(0.0, deadline - time.time())
            print(
                f"[RETRY] kv_init_conn_pool failed (attempt {attempt}), retry in {retry_interval_sec:.2f}s, remaining {remaining:.1f}s",
                flush=True)
            ...
            if self.multi_rank_pull_kv:
                ...
                for idx_cluster, cluster_id in enumerate(cluster_ids):
                    ...
                    if len_local_blocks > 0:
                        ...
                        logger.warning(
                            f"*********** dst cluster_id is {cluster_id}.")
    ...
    def _read_blocks(
        ...
        DBlocksArray = KVBlockC * total_blocks
        d_blocks = DBlocksArray()
        filled = self.lib.kv_dpool_get_blocks(None,
                                              (c_uint64 *
                                               total_blocks)(*d_ids_list),
        ...
        if filled != total_blocks:
            raise RuntimeError(
                f"[ERROR] d_get_blocks failed: {filled}/{total_blocks}")
        ...
        trans_ok = self.lib.kv_transfer_cluster_sequential(
            ...
            c_uint32(
                int(NUM_P_NODES)),  # for 1p1d, should be CLUSTER_SIZE for xPyD
            p_ids_arr,
        ...
        if self.vllm_config.parallel_config.tensor_parallel_size == 1:
            ...
            if self.multi_thread_pull_kv:
                ...
                with self._transfer_lock:
                    self._recving_transfers.append(request_id)
        logger.debug(
            f" ***** read block, req_id:{request_id}, local_block_ids:{local_block_ids}, remote_block_ids:{remote_block_ids}"
        )
...
def start_p_node_servers(base_port: int, servers: int, mmap_paths, dtype: str,
                         offset_dtype: int, start_offset: int):
    procs = []
    for nid in range(servers):
        ...
        p = subprocess.Popen(args,
                             ...
                             text=True,
                             bufsize=1)
        threading.Thread(target=lambda pref, pipe: [
            print(f'{pref} {l.rstrip()}', flush=True)
            for l in iter(pipe.readline, '')
        ],
                         args=(f"[P{nid}] OUT", p.stdout),
                         daemon=True).start()
        threading.Thread(target=lambda pref, pipe: [
            print(f'{pref} {l.rstrip()}', flush=True)
            for l in iter(pipe.readline, '')
        ],
                         args=(f"[P{nid}] ERR", p.stderr),
                         daemon=True).start()
        procs.append(p)
        time.sleep(0.05)
    return procs
...
def attach_logs(procs):
    ...
    for i, p in enumerate(procs):
        threading.Thread(target=stream,
                         args=(f"[P{i}] OUT", p.stdout),
                         daemon=True).start()
        threading.Thread(target=stream,
                         args=(f"[P{i}] ERR", p.stderr),
                         daemon=True).start()
...
def dump_thread_to_file(thread, thread_name: str, folder_path: str):
    ...
    while not hasattr(thread, "native_id"):
        if time.time() - start_time > timeout:
            logger.error(
                f"Timeout waiting for thread {thread_name} to have native_id.")
            return
        time.sleep(0.005)
