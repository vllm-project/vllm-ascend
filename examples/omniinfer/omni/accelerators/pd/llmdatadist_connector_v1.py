...
BLOCK_RELEASE_DELAY = int(
    os.environ.get("BLOCK_RELEASE_DELAY", 600)
    ...
    def request_finished(
        ...
        if request.status == RequestStatus.FINISHED_ABORTED and request.kv_transfer_params is not None:
            self._send_pulled_kv_req_list(
                request.kv_transfer_params.get("remote_host_ip"),
                [request.request_id])
        return False, None
...
class DecodeConnectorWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self.multi_rank_pull_kv or self.multi_thread_pull_kv:
            ...
            logger.info(
                f" ***** registered_link_infos: {self.datadist_manager.registered_link_infos}"
            )
            for (
                    cluster_id_start, prefill_dp_rank, d_rank
            ), cluster_ids in self.datadist_manager.registered_link_infos.items(
            ):
                ...
                for idx_count, cluster_id in enumerate(cluster_ids):
                    with self._pull_kv_lock:
                        ...
                        logger.debug(
                            f" ***** Created a new thread for pulling kv from cluster {cluster_id}."
                        )
    ...
    def start_load_kv(self, metadata: DatadistConnectorMetadata):
        ...
        for req_id, meta in metadata.requests.items():
            ...
            cluster_ids = self.datadist_manager.get_real_remote_cluster_ids(
                meta)
            if self.multi_rank_pull_kv:
    ...
    def _read_blocks(
        ...
        start = time.time()
        self.datadist_manager.pull_kv(remote_block_ids, local_block_ids,
                                      dst_cluster_id)
        ...
        if self.tp_rank == 0:
            logger.info(
                f" ***** read block, req_id:{request_id}, cost:{cost:.6f}")
