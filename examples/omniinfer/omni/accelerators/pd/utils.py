...
def get_p_start_rank(p_tp_size, p_dp_size, d_tp_size, d_dp_size, d_node_num,
                     cur_d_node, cur_d_rank):
...
def prepare_ranktables(prefill_group: ServerGroup, decode_group: ServerGroup,
                       p_ranks: int, d_ranks: int):
...
def get_config_from_dict_or_env(config, config_var_name, env_var_name,
    ...
    if env_value is None and args_value is None:
        if default_value is None:
            raise ValueError(
                f"ENV {env_var_name} or args {config_var_name} should not be None."
            )
        else:
            value = default_value
    # ENV first
