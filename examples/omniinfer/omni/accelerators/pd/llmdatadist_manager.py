...
class LLMDataDistConfig:
    ...
    def __init__(self,
        ...
        ranktable_config = RankTableConfig.from_dict_or_env(
            vllm_config.kv_transfer_config.kv_connector_extra_config)
        self.global_rank_table = GlobalRankTable(ranktable_config)
        self.local_group = self.global_rank_table.find_group_by_local_info(
            LocalInfo(ranktable_config))
        ...
        if ignore_load_rank:
            ...
            self.cluster_id = self.local_group.device_list[
                self.rank].cluster_id
        self.kv_parallel_size = self.kv_transfer_config.kv_parallel_size
        self.kv_producer_dp_size = self.kv_transfer_config.kv_connector_extra_config.get(
...
class LLMDataDistManager:
    ...
    def get_real_remote_cluster_ids(self, meta: "ReqMeta"):
        ...
        if remote_cluster_ids is None:
            raise ValueError(
                f"Could not find remote cluster id from {meta.remote_cluster_id=}, {meta.remote_dp_rank=}."
            )
        return remote_cluster_ids

    def _init_llm_data_dist(self):
        data_dist = LLMDataDist(self.data_dist_config.role,
                                self.data_dist_config.cluster_id)
    ...
    def register_memory(self, kv_caches: dict[str, torch.Tensor]):
        if len(self.registered_kv_caches) > 0:
            raise ValueError(
                "Attr `registered_kv_caches` must be empty before register kv_caches."
            )
        ...
        for model_id, sub_kv_caches in enumerate(flatten_kv_caches):
            cache_desc = CacheDesc(
                num_tensors=len(sub_kv_caches),
                shape=tuple(sub_kv_caches[0].shape),
            ...
            if self.data_dist_config.is_prefill:
                cache_key = BlocksCacheKey(self.data_dist_engine.cluster_id,
                                           model_id=model_id)
            else:
                cache_key = None

            cache = self.data_dist_engine.cache_manager.register_blocks_cache(
                cache_desc, cache_addrs, cache_key)
            self.registered_kv_caches.append(cache)
        logger.error(
            f" ***** registered_kv_caches num:{len(self.registered_kv_caches)}"
        )
    ...
    def _pull_blocks(self, src_cache_key, dst_cache, src_blocks, dst_blocks):
        ...
        logger.error(
            f"kv cache pull blocks retry error {src_cache_key} {src_blocks} {dst_blocks}"
        )
    ...
    def pull_kv(self, src_blocks, tgt_blocks, prompt_cluster_id):
        ...
        for model_id, kv_cache in enumerate(self.registered_kv_caches):
            ...
            self._pull_blocks(prompt_cache_key, kv_cache, src_blocks,
                              tgt_blocks)
        ...
        for prefill_server_group in prefill_server_groups:
            for decode_server_group in decode_server_groups:
                ...
                for decode_device in decode_server_group.device_list:
                    ...
                    p_rank_start = get_p_start_rank(prefill_tp_size, 1,
                                                    decode_tp_size,
                                                    decode_dp_size, decode_num,
                                                    decode_id, d_rank)

                    pd_pairs = [(p_rank_start + dp_idx * prefill_tp_size,
                                 d_rank) for dp_idx in range(prefill_dp_size)]

                    for prefill_dp_rank, (p_rank,
                                          d_rank) in enumerate(pd_pairs):
                        prefill_cluster_id = prefill_server_group.device_list[
                            p_rank].cluster_id
                        decode_cluster_id = decode_device.cluster_id

                        if self.multi_rank_pull_kv:
                            # first kv link
                            link_num = self._create_kv_link(
                                prefill_server_group, decode_server_group,
                                p_rank, d_rank, prefill_cluster_id,
                                decode_cluster_id, link_num)
                            # second kv link
                            second_p_rank = (p_rank + 1) % prefill_tp_size
                            second_prefill_cluster_id = prefill_server_group.device_list[
                                second_p_rank].cluster_id

                            link_num = self._create_kv_link(
                                prefill_server_group, decode_server_group,
                                second_p_rank, d_rank,
                                second_prefill_cluster_id, decode_cluster_id,
                                link_num)

                            prefill_cluster_id_list = [
                                prefill_cluster_id, second_prefill_cluster_id
                            ]
                        else:
                            link_num = self._create_kv_link(
                                prefill_server_group, decode_server_group,
                        ...
                        if not self.data_dist_config.is_prefill:
                            self.registered_link_infos[(
                                prefill_server_group.cluster_id_start,
                                prefill_dp_rank,
                                d_rank)] = prefill_cluster_id_list

        return self.check_register_status()

    def _create_kv_link(self, prefill_server_group, decode_server_group,
                        p_rank, d_rank, prefill_cluster_id, decode_cluster_id,
                        link_num):
        p_ranktables, d_ranktables = prepare_ranktables(
            prefill_server_group, decode_server_group, p_rank, d_rank)
        ...
        need_build = self._build_device_link(comm_names, cluster_rank_infos,
                                             ranktables)
    ...
    def _build_device_link(self, comm_names, cluster_rank_infos, rank_tables):
        if self.rank in comm_names and self.rank in cluster_rank_infos and self.rank in rank_tables:
            logger.warning(
                f"Current cluster id is {self.data_dist_config.cluster_id}, create link:{comm_names}"
            )
            ...
            cluster_rank_info = {
                int(key): value
                for key, value in cluster_rank_info.items()
            }
            rank_table = json.dumps(rank_tables[self.rank])
            comm_id = self.data_dist_engine.link(comm_name, cluster_rank_info,
                                                 rank_table)
            logger.info(
                f"rank:{self.rank} linked {comm_name}:{comm_id}, cluster_rank_info:{cluster_rank_info}"
            )
            # Save comm_name information
            self.rank_link_info_map[comm_name] = RankLinkInfo(
                comm_name, comm_id, cluster_rank_info)
            return True
        return False
    ...
    def check_register_status(self):
        status = {
            comm_name: False
            for comm_name in self.rank_link_info_map.keys()
        }
        ready_num = 0
        while ready_num < len(self.rank_link_info_map):
            for comm_name, rank_link_info in self.rank_link_info_map.items():
                if status[comm_name]:
                    continue
                ret = self.data_dist_engine.query_register_mem_status(
                    rank_link_info.comm_id)
                if ret == RegisterMemStatus.OK:
                    status[comm_name] = True
                    ready_num += 1
                    logger.debug(f"rank:{self.rank} check link status success")
                    logger.debug(
                        f"comm_id: {rank_link_info.comm_id} ret:{ret}")
...
def maybe_merge_kv_caches(flatten_kv_caches):
    ...
    if len(flatten_kv_caches) == 1 and len(
            flatten_kv_caches[0]
        [0].shape) == 5 and flatten_kv_caches[0][0].shape[0] == 2:
        merged_kv_caches = [[]]
...
RankLinkInfo = namedtuple("RankLinkInfo",
                          ["comm_name", "comm_id", "cluster_rank_info"])
