...
P_NODE_BIN = os.environ.get("P_NODE_BIN",
                            os.path.join(BASE_DIR, "bin/p_node_server"))
D_AGENT_BIN = os.environ.get("D_AGENT_BIN",
                             os.path.join(BASE_DIR, "bin/d_kv_agent"))
...
NODE_SPECS = ";".join(
    [f"{P_NODE_LIST[i]}:{BASE_PORT}:{i}" for i in range(NUM_P_NODES)])
...
class LLMDataDistConnector(KVConnectorBase_V1):
    ...
    def __init__(self, vllm_config: VllmConfig, role: KVConnectorRole):
        ...
        if vllm_config.model_config.is_deepseek_mla:
            ...
            logger.info(
                "Set kv_parallel_size to 1 when using deepseek MLA model.")
        ...
        if self.is_prefill:
            ...
            self.host_ip = _P_NODE_LIST_RAW[self.cluster_id_start *
                                            CLUSTER_SIZE]
        ...
        if role == KVConnectorRole.SCHEDULER:
            if self.is_prefill:
                self.connector_scheduler = PrefillConnectorScheduler(
                    vllm_config, self.cluster_id_start, self.host_ip,
                    str(self.host_port))
            else:
                self.connector_scheduler = DecodeConnectorScheduler(
                    vllm_config)
    ...
    def build_connector_meta(
        ...
        if self.connector_scheduler is None:
            raise RuntimeError("self.connector_scheduler cannot be None")
        return self.connector_scheduler.build_connector_metadata(
            scheduler_output)
    ...
    def register_kv_caches(self,
        ...
        return self.connector_worker.register_kv_caches(
            kv_pool_mmap_path, data_type, block_len_dtype, start_offset,
            omni_cache)
...
class PrefillConnectorScheduler:
    ...
    def __init__(self, vllm_config, cluster_id_start: str, host_ip: str,
                 host_port: str):
        self.vllm_config = vllm_config
        self.cluster_id_start = cluster_id_start
        self.host_ip = host_ip
        self.host_port = host_port
        logger.info("Initializing LLMDataDist Scheduler %s %s %s",
    ...
    def request_finished(
        ...
        return delay_free_blocks, dict(
            ...
            remote_tp_size=self.vllm_config.parallel_config.
...
class PrefillConnectorWorker:
    ...
    def __init__(self, vllm_config: "VllmConfig", host_ip: str,
                 host_port: str):
        # Metadata.
        self.host_ip = host_ip
        self.host_port = host_port
        ...
        if self.rank == 0:
            ...
            logger.info(
                f"ConnectWorker bind tcp://{self.host_ip}:{self.host_port}")
            self._transfer_lock = threading.Lock()
            self.receive_req_list: List[str] = []
            thread_name = "prefill_connector_get_pulled_kv_req_list"
            self.thread = threading.Thread(target=self.get_pulled_kv_req_list,
                                           daemon=True,
                                           name=thread_name)
            self.thread.start()
    ...
    def register_kv_caches(self,
                           ...
                           start_offset,
                           omni_cache=None):
        self.start_p_server_kv_transfer(kv_pool_mmap_path, data_type,
                                        block_len_dtype, start_offset)

    def start_p_server_kv_transfer(self, kv_pool_mmap_path, data_type,
                                   block_len_dtype, start_offset):
        self.tp_rank_local = self.rank % (
            self.vllm_config.parallel_config.tensor_parallel_size //
            CLUSTER_SIZE)
        if self.tp_rank_local == 0:
            # Pass a list for mmap paths; start one P server
            p_procs = start_p_node_servers(BASE_PORT, 1, [kv_pool_mmap_path],
                                           data_type, block_len_dtype,
                                           start_offset)
            ...
            if not ok:
                ...
                raise RuntimeError(
                    f"[ERROR] P not ready: {sorted(list(not_ready))}")
    ...
    def get_finished(
        self, metadata: DatadistConnectorMetadataPrefill
        ...
        if self.rank == 0:
            ...
            with self._transfer_lock:
                ...
                for req_id, finish_time in list(
                        self.requests_finish_time.items()):
        ...
        while True:
            try:
                ...
                if self.input_socket.poll(timeout=100) > 0:
                    ...
                    id_list = json.loads(
                        message)  # Parse the received JSON string into a list
    ...
    def update_state_after_alloc(self, request: "Request",
        ...
        if params is not None:
            if params.get("remote_block_ids"):
                if all(p in params
                       for p in ("remote_cluster_id", "remote_host_ip")):
    ...
    def build_connector_metadata(
        ...
        for req_id, (req, block_ids) in self._reqs_need_recv.items():
            if req.kv_transfer_params is None:
                logger.warning(
                    f"For request {req_id}: kv_transfer_params now is None")
            else:
...
class DecodeConnectorWorker:
    ...
    def on_fast_path_req(self):
        ...
        while True:
            ...
            for req_id, meta in metadata.requests.items():
                if (len(meta.local_block_ids) > 0) and (len(
                        meta.remote_block_ids) > 0):
                    self.start_load_kv(metadata)
                    logger.info(
                        "Received fast path request for request %s with "
                        "local_block_ids: %s, remote_block_ids: %s.", req_id,
                        len(meta.local_block_ids), len(meta.remote_block_ids))

    def register_kv_caches(self, kv_pool_mmap_path, data_type, block_len_dtype,
                           start_offset, omni_cache):
        self.start_offset = start_offset
        self.block_len_dtype = block_len_dtype
        self.agents: List[Tuple[subprocess.Popen, List[threading.Thread], str,
                                int]] = []
        endpoint = f"tcp://127.0.0.1:{ZMQ_BASE_PORT + self.dp_rank}"
        args = [
            D_AGENT_BIN, "--nodes", NODE_SPECS, "--d_mmap", kv_pool_mmap_path,
            "--dtype", data_type, "--offset_dtype",
            str(block_len_dtype), "--start_offset",
            str(start_offset), "--direct_id_base",
            str(0), "--zmq_bind", endpoint, "--conns_per_node", "16"
        ]
    ...
    def start_load_kv(self, metadata: DatadistConnectorMetadata):
        ...
        for req_id, meta in metadata.requests.items():
            ...
            if len(meta.local_block_ids) == 0:
                logger.info(
                    f" ***** Request {req_id} has 0 local blocks, skip load kv."
                )
                continue
            ...
            if isinstance(meta.local_block_ids[0], int):
                ...
                if len(meta.remote_block_ids) < len(meta.local_block_ids):
                    meta.local_block_ids = meta.local_block_ids[:len(
                ...
                # where N is the number of local blocks
                    ...
                    meta.remote_block_ids = meta.remote_block_ids[
            ...
            # If local_block_ids is a list of lists (omni-attention case)
                ...
                meta.remote_block_ids = [meta.remote_block_ids] * len(
                ...
                elif len(meta.remote_block_ids[0]) < len(
                        meta.local_block_ids[0]):
                    meta.local_block_ids[0] = meta.local_block_ids[
                        0][:len(meta.remote_block_ids[0])]
                    logger.debug("look ahead token num is greater than 0")
                # If remote_block_ids in P is more than local_block_ids[0] in D, we only need the last N remote blocks
                elif len(meta.remote_block_ids[0]) > len(
                        meta.local_block_ids[0]):
                    meta.remote_block_ids[0] = meta.remote_block_ids[0][
                        -len(meta.local_block_ids[0]):]
                logger.info(
                    " ***** start_load_kv for request %s "
                    "Num local_block_ids: %s. Num remote_block_ids: %s.",
                    req_id, len(meta.local_block_ids[0]),
                    len(meta.remote_block_ids[0]))
            # handle the unexpected case where local_block_ids is not a list of int or list of lists
            else:
                logger.error(
                    f"Unexpected type for meta.local_block_ids[0]: {type(meta.local_block_ids[0])}"
                )
                raise RuntimeError(
                    f"Unexpected type for meta.local_block_ids[0]: {type(meta.local_block_ids[0])}"
                )
            ...
            future = self.executor.submit(
                ...
                remote_block_ids=meta.remote_block_ids if isinstance(
                    meta.remote_block_ids[0], int) else
                meta.remote_block_ids[0],
    ...
    def _read_blocks(
        ...
        try:
            for inst, (_proc, _threads, endpoint,
                       _start_offset) in enumerate(self.agents):
                s = zmq.Context.instance().socket(zmq.REQ)
        ...
        if self.vllm_config.parallel_config.tensor_parallel_size == 1:
            ...
            if remote_request_id is not None:
                self._send_pulled_kv_req_list(remote_host_ip,
                                              [remote_request_id])
            with self._transfer_lock:
            ...
            ) == 0 and remote_request_id is not None:
                self._send_pulled_kv_req_list(remote_host_ip,
                                              [remote_request_id])
            with self._transfer_lock:
                self._recving_transfers.append(request_id)
        ...
        logger.warning(
            f"<<< Time cost of decode synchronize_h2d is {duration*1000} ms")

        logger.debug(
            " ***** read block, req_id:%s, local_block_ids:%s, remote_block_ids:%s",
            request_id, local_block_ids[1], remote_block_ids)
        cost = time.time() - start
        logger.warning(" ***** read block, req_id:%s, cost:%.6f", request_id,
                       cost)
    ...
    def _recv_agent_reply(self,
        ...
        while True:
            ...
            try:
                ...
                raise RuntimeError(
                    f"[ERROR] agent#{inst} timeout waiting for reply")
                # logger.warning(f"[WARN] agent#{inst} timeout, retrying...")
                # continue
            except Exception as e:
                raise RuntimeError(
                    f"[ERROR] agent#{inst} recv failed: {e}") from e
            ...
            if len(parts) == 2:
                ...
                if attempts > max_retries:
                    raise RuntimeError(
                        f"[ERROR] agent#{inst} invalid reply format ({len(parts)} frames)"
                    )
                continue

            if status != b"OK":
                text = msg.decode("utf-8", "ignore")
                rid = req_id_part.decode("utf-8",
                                         "ignore") if req_id_part else "?"
                raise RuntimeError(
                    f"[ERROR] agent#{inst} error (req_id={rid}): {text}")
    ...
    def get_finished(
            self,
            metadata: DatadistConnectorMetadata) -> Tuple[set[str], set[str]]:
        # for decode side, done_sending is not needed
        all_done_sending: set[str] = set()
        with self._transfer_lock:
            all_done_recving = self._pop_done_transfers(
                self._recving_transfers)
        if len(all_done_recving) > 0:
            logger.debug("Get_finished: %s requests done recving",
                         len(all_done_recving))
...
def start_p_node_servers(base_port: int, servers: int, mmap_paths: List[str],
                         dtype: str, offset_dtype: int, start_offset: int):
    procs = []
    for nid in range(servers):
        ...
        args = [
            P_NODE_BIN, "--node_id",
            str(nid), "--port",
            str(port), "--mmap", mmap_paths[nid], "--dtype", dtype,
            "--offset_dtype",
            str(offset_dtype), "--start_offset",
            str(start_offset)
        ]
