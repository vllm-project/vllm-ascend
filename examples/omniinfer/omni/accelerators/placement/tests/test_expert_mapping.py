...
class TestExpertMapping(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.expert_mapping = ExpertMapping(pattern_path=self.pattern_file,
                                            device="npu",
    ...
    def test_init(self):
        ...
        self.assertEqual(self.expert_mapping.placement_pattern.shape,
                         (2, 2, 4))
    ...
    def test_load_placement_pattern_with_validation(self):
        ...
        self.assertIn("Placement pattern file not found",
    ...
    def test_is_expert_on_current_rank(self):
        ...
        em = ExpertMapping(pattern_path="", device="cpu")
        exists, position = em.is_expert_on_current_rank(layer_idx_moe=0,
                                                        expert_id=1,
                                                        current_rank=0,
                                                        experts_per_rank=2)
    ...
    def test_get_local_expert_indices_offset(self):
        # 测试获取总部署专家数
        layer0_rank0_offset = self.expert_mapping.get_local_expert_indices_offset(
            0, 0, 4)
        layer0_rank1_offset = self.expert_mapping.get_local_expert_indices_offset(
            0, 1, 4)
        layer1_rank0_offset = self.expert_mapping.get_local_expert_indices_offset(
            1, 0, 4)
        layer1_rank1_offset = self.expert_mapping.get_local_expert_indices_offset(
            1, 1, 4)
    ...
    def test_get_deployed_experts_per_layer_basic_case(self):
        ...
        with patch.object(self.expert_mapping, 'placement_pattern',
                          placement_pattern):
            result = self.expert_mapping.get_deployed_experts_per_layer()

            expected = torch.tensor([4, 4,
                                     5])  # layer 0: 4, layer 1: 4, layer 2: 5
    ...
    def test_get_deployed_experts_per_layer_all_ones(self):
        ...
        with patch.object(self.expert_mapping, 'placement_pattern',
            ...
            expected = torch.tensor([8, 8,
                                     8])  # 2 ranks * 4 experts = 8 per layer
            self.assertEqual(result, expected.tolist(),
                             f"Expected {expected}, but got {result}")
    ...
    def test_construct_global_expert_mapping(self):
        ...
        em = ExpertMapping(pattern_path=pattern_file,
                           device="npu",
                           max_redundants_per_expert=max_redundant_count)
        ...
        for layer in range(num_layers):
            for expert in range(num_experts):
                ...
                expected = expected_redundant_count[layer * num_experts +
                                                    expert]
        ...
        for layer in range(num_layers):
            for expert in range(num_experts):
                for index in range(max_redundant_count):
                    ...
                    expected_index = (layer * num_experts +
                                      expert) * max_redundant_count + index
    ...
    def test_some_layers_invalid(self):
        ...
        with patch.object(self.expert_mapping, 'placement_pattern',
            ...
            self.assertEqual(
                result, expected,
                "Layer 1 should be invalid due to expert 0 placement")
    ...
    def test_empty_layers(self):
        ...
        with patch.object(self.expert_mapping, 'placement_pattern',
            ...
            expected = []
            self.assertEqual(result, expected,
                             "Empty layers should return empty list")
    ...
    def test_full_layers(self):
        ...
        with patch.object(self.expert_mapping, 'placement_pattern',
            ...
            self.assertEqual(
                result, expected,
                "All layers should be invalid due to expert 0 placement")
    ...
    def test_single_expert_per_rank(self):
        ...
        with patch.object(self.expert_mapping, 'placement_pattern',
            ...
            self.assertEqual(
                result, expected,
                "All layers should be valid with one expert per rank")
