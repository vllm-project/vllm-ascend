...
try:
    ...
    print(
        "Warning: matplotlib is not installed, Chinese font settings will be skipped, but this does not affect CSV generation"
    )
...
def parse_recordstep_range(
        recordstep_range: Optional[str]) -> Optional[Tuple[int, int]]:
    ...
    try:
        ...
        raise ValueError(
            "recordstep_range must be in format 'start:end' where start and end are non-negative integers and start <= end."
        )


def validate_inputs(input_log_files: Optional[List[str]],
                    input_txt_folders: Optional[List[str]], input_mode: str,
                    collecting_modes: str,
                    num_ranks_of_collecting_data: Optional[int],
                    num_positions_of_routed_experts: int, num_layers: int,
    ...
    if input_mode == 'log':
        if not isinstance(input_log_files, list) or not input_log_files:
            raise ValueError(
                "When input_mode='log', input_log_files must be a non-empty list of log file paths."
            )
        for log_file in input_log_files:
            ...
            if not log_file.endswith('.log'):
                logger.warning(
                    f"Input file {log_file} does not have a .log extension, please verify the file type"
                )
    else:
        if not isinstance(input_txt_folders, list) or not input_txt_folders:
            raise ValueError(
                "When input_mode='txt', input_txt_folders must be a non-empty list of directory paths."
            )
        for folder in input_txt_folders:
            if not os.path.isdir(folder):
                raise ValueError(
                    f"Text folder {folder} is not a valid directory.")

    if collecting_modes not in ['prefill', 'decode', 'all']:
        raise ValueError(
            "collecting_modes must be 'prefill', 'decode', or 'all'.")
    ...
    if num_ranks_of_collecting_data <= 0:
        raise ValueError(
            "num_ranks_of_collecting_data must be a positive integer.")
    ...
    parse_recordstep_range(
        recordstep_range)  # Validate recordstep_range format


def process_log_line(
    line: str, valid_modes: set, num_ranks_of_collecting_data: int,
    num_layers: int, numbers_per_rank: int, line_count: int, file_name: str,
    step_range: Optional[Tuple[int,
                               int]]) -> Optional[Tuple[int, int, List[int]]]:
    ...
    try:
        ...
        logger.warning(
            f"Unable to parse step, rank_id, layer_id, or expert_count, file {file_name} line {line_count}: {line}"
        )
        return None
    ...
    if not (0 <= rank_id < num_ranks_of_collecting_data):
        logger.warning(
            f"rank_id {rank_id} out of range [0, {num_ranks_of_collecting_data-1}], file {file_name} line {line_count}: {line}"
        )
        return None

    if not (0 <= layer_id < num_layers):
        logger.warning(
            f"layer_id {layer_id} out of range [0, {num_layers-1}], file {file_name} line {line_count}: {line}"
        )
        return None

    try:
        expert_values = [
            int(x) for x in expert_data.strip().split('\t') if x.strip()
        ]
    except ValueError:
        logger.warning(
            f"Invalid expert data format '{expert_data}', file {file_name} line {line_count}: {line}"
        )
        return None

    if len(expert_values) != expert_count:
        logger.warning(
            f"Expert count {expert_count} does not match data count {len(expert_values)}, file {file_name} line {line_count}: {line}"
        )
        return None
    ...
    try:
        ...
        logger.warning(
            f"Unable to process expert data '{expert_data}', file {file_name} line {line_count}, error: {e}"
        )
        return None


def process_log_file(log_file: str, valid_modes: set,
                     num_ranks_of_collecting_data: int, num_layers: int,
                     numbers_per_rank: int, csv_data: np.ndarray,
                     step_range: Optional[Tuple[int, int]]) -> int:
        ...
        try:
            with open(log_file, 'r', encoding=encoding) as f:
                for line in f:
                    ...
                    result = process_log_line(line, valid_modes,
                                              num_ranks_of_collecting_data,
                                              num_layers, numbers_per_rank,
                                              line_count, log_file, step_range)
                    if result:
                        rank_id, layer_id, values = result
                        csv_data[layer_id,
                                 rank_id * numbers_per_rank:(rank_id + 1) *
                                 numbers_per_rank] += values
                        processed_lines += 1
            logger.info(
                f"Finished processing log file {log_file}, total lines: {line_count}, valid data lines: {processed_lines}"
            )
            return processed_lines
        except UnicodeDecodeError:
            logger.warning(
                f"Unable to read {log_file} with {encoding} encoding, trying next encoding"
            )
...
def process_txt_file(txt_file: str, num_layers: int, numbers_per_rank: int,
                     rank_id: int, csv_data: np.ndarray) -> int:
    """Process a single txt file and update csv_data."""
    with open(txt_file, 'r', encoding='utf-8') as f:
        lines = f.readlines()
        if len(lines) != num_layers:
            logger.warning(
                f"{txt_file} has {len(lines)} lines, expected {num_layers} lines"
            )
            return 0
        ...
        for layer_idx, line in enumerate(lines):
            ...
            if len(numbers) != numbers_per_rank:
                logger.warning(
                    f"Line {layer_idx+1} in {txt_file} has {len(numbers)} numbers, expected {numbers_per_rank}"
                )
                continue

            try:
                values = [math.ceil(float(num) / 128) for num in numbers]
                csv_data[layer_idx, rank_id * numbers_per_rank:(rank_id + 1) *
                         numbers_per_rank] += values
                processed_lines += 1
            except ValueError:
                logger.warning(
                    f"Invalid number format in line {layer_idx+1} of {txt_file}"
                )
                continue

    logger.info(
        f"Finished processing text file {txt_file}, total lines: {len(lines)}, valid lines: {processed_lines}"
    )
    return processed_lines


def generate_csv(input_log_files: Optional[List[str]] = None,
                 ...
                 recordstep_range: Optional[str] = None) -> str:
    """
    ...
    # Validate inputs
    validate_inputs(input_log_files, input_txt_folders, input_mode,
                    collecting_modes, num_ranks_of_collecting_data,
                    num_positions_of_routed_experts, num_layers,
                    recordstep_range)
    ...
    if step_range:
        logger.info(
            f"Processing steps in range: {step_range[0]} to {step_range[1]}")
    else:
    ...
    csv_data = np.zeros(
        (num_layers, num_ranks_of_collecting_data * numbers_per_rank),
        dtype=int)
    ...
    if input_mode == 'log':
        valid_modes = {'prefill', 'decode'
                       } if collecting_modes == 'all' else {collecting_modes}
        for log_file in input_log_files:
            processed_lines = process_log_file(log_file, valid_modes,
                                               num_ranks_of_collecting_data,
                                               num_layers, numbers_per_rank,
                                               csv_data, step_range)
        ...
        for txt_folder in input_txt_folders:
            txt_files = glob.glob(
                os.path.join(txt_folder, "activation_counts_recordstep_*.txt"))
            if not txt_files:
                logger.warning(
                    f"No matching .txt files found in folder {txt_folder} "
            ...
            for txt_file in txt_files:
                ...
                try:
                    recordstep_str = filename.split('recordstep_')[1].split(
                        '_rank_')[0]
                    recordstep = int(recordstep_str)
                    if step_range is None or (step_range[0] <= recordstep <=
                                              step_range[1]):
                        filtered_txt_files.append(txt_file)
                except (IndexError, ValueError):
                    logger.warning(
                        f"Skipping file with unexpected filename format: {filename}"
                    )
                    continue

        if not filtered_txt_files:
            logger.error(
                f"No .txt files across folders {input_txt_folders} match the recordstep range {step_range}"
            )
            raise ValueError(
                f"No valid .txt files found in folders {input_txt_folders}, check recordstep range or folder contents"
            )
        ...
        for txt_file in filtered_txt_files:
            ...
            if not (0 <= rank_id < num_ranks_of_collecting_data):
                logger.warning(
                    f"Skipping rank_id {rank_id}, not in range [0, {num_ranks_of_collecting_data-1}]"
                )
                continue

            processed_lines = process_txt_file(txt_file, num_layers,
                                               numbers_per_rank, rank_id,
                                               csv_data)
            total_processed_lines += processed_lines
            total_processed_files += 1

        logger.info(
            f"Processed {total_processed_files} .txt files across {len(input_txt_folders)} folders"
        )

    if total_processed_lines == 0:
        raise ValueError(
            "No valid data extracted from any input files, please check input content, parameter configuration, or recordstep range"
        )
    ...
    try:
        with open(output_csv_path, 'w', newline='',
            ...
            header = [''] + [
                f'ep_{i}'
                for i in range(num_ranks_of_collecting_data * numbers_per_rank)
            ]
        ...
        logger.info(
            f"Processed {total_processed_files} input files, total valid data lines: {total_processed_lines}"
        )
...
if __name__ == "__main__":
        ...
        for rank in range(num_ranks):
            ...
            with open(file_name, 'w') as file:
                for _ in range(num_layers):
                    ...
                    activation_data = [
                        random.randint(10, 1000) for _ in range(2)
                    ]
    ...
    generate_csv(input_txt_folders=["./input/txt/folder"],
                 input_mode='txt',
