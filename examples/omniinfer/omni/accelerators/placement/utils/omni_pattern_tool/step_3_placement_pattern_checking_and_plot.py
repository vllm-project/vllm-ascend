...
try:
    ...
    logging.warning(
        "matplotlib is not installed. Unicode font settings are skipped, but this does not affect validation or visualization."
    )
...
def test_expert_mapping(placement_pattern, log_timestamp=None):
    ...
    if isinstance(placement_pattern, str):
        if not os.path.exists(placement_pattern):
            logger.error(
                f"Placement pattern file {placement_pattern} does not exist.")
            raise ValueError(
                f"Placement pattern file {placement_pattern} does not exist.")
        placement_pattern = np.load(placement_pattern)
    elif not isinstance(placement_pattern, np.ndarray):
        logger.error(
            "placement_pattern must be a numpy array or .npy file path.")
        raise TypeError(
            "placement_pattern must be a numpy array or .npy file path.")
    ...
    for layer_id in range(n_layers):
        ...
        if len(missing_experts) > 0:
            ...
            logger.warning(
                f"Layer {layer_id}: Unassigned experts detected: {missing_experts.tolist()}"
            )
        ...
        if not np.all(experts_per_rank == experts_per_rank[0]):
            ...
            logger.warning(
                f"Layer {layer_id}: Inconsistent expert counts per rank: {experts_per_rank.tolist()}"
            )
    ...
    logger.info(
        f"Placement pattern validation result: Condition 1 satisfied = {result['Condition 1 satisfied']}, "
        f"Condition 2 satisfied = {result['Condition 2 satisfied']}")
    return result["Condition 1 satisfied"] and result[
        "Condition 2 satisfied"], result


def view_patterns(placement_pattern,
                  ppname='',
                  fig_save_path=None,
                  log_timestamp=None):
    """
    ...
    logger.info(
        f"Generating visualization for pattern {ppname} with shape: {matrix.shape}"
    )
    ...
    if fig_save_path is not None:
        try:
            ...
            logger.error(
                f"Failed to save visualization image to {fig_save_path}: {e}")
            raise
    else:
        plt.show()
...
def main():
    parser = argparse.ArgumentParser(
        description="Test and visualize a placement pattern from an .npy file."
    )
    parser.add_argument(
        '--placement_pattern_file',
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        'Path to save the visualization image; if not provided, image is displayed.'
    )
    ...
    if not os.path.exists(args.placement_pattern_file):
        logger.error(
            f"Placement pattern file {args.placement_pattern_file} does not exist."
        )
        raise ValueError(
            f"Placement pattern file {args.placement_pattern_file} does not exist."
        )
    ...
    try:
        ...
        logger.info(
            f"Loaded placement pattern from {args.placement_pattern_file} with shape: {placement_pattern.shape}"
        )
    except Exception as e:
        logger.error(
            f"Failed to load placement pattern from {args.placement_pattern_file}: {e}"
        )
        raise
    ...
    is_valid, test_result = test_expert_mapping(placement_pattern,
                                                log_timestamp)
    logger.info(
        f"Placement pattern validation: Is valid = {is_valid}, Test result = {test_result}"
    )
    print(
        f"Placement pattern validation: Is valid = {is_valid}, Test result = {test_result}"
    )
    ...
    view_patterns(placement_pattern,
