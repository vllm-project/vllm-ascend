...
NAMES = [
    "w13_weight", "w2_weight", "w2_weight_offset", "w2_weight_scale",
    "w13_weight_offset", "w13_weight_scale"
]
...
def init_moe_weight(tensor, global_expert_idx, weight_name):
    ...
    weight = omni_placement.Weight(data_ptr=capsule,
                                   length=length,
...
class MoEWeights:
    ...
    def init_moe_weights(cls,
        ...
        for layer_id in range(*layers):
            ...
            for expert_id in range(num_expert_per_rank):
                ...
                for _idx, weight_name in enumerate(NAMES):
                    ...
                    # # print(tensor)
                    weight = init_moe_weight(tensor, global_expert_idx,
                                             weight_name)
        ...
        cls.moe_weights = omni_placement.MoEWeights(
            npu_weights=layer_weights_list, world_size=world_size)
