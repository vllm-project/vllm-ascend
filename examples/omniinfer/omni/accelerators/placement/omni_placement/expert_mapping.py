...
class ExpertMapping:
    ...
    def __init__(self,
                 ...
                 rank: int = 0,
                 world_size: int = 1,
                 num_devices_per_host: int = 8,
                 enable_dynamic: bool = False,
                 num_experts: int = 256,
                 enable_rank_round_robin: bool = False):
        ...
        self.max_redundant_per_expert = config.getattr(
            'max_redundant_per_expert', None) if self.enable_dynamic else None
        self.max_redundant_per_rank = config.getattr(
            'max_redundant_per_rank', None) if self.enable_dynamic else None

        self._init_expert_mapping()
        self.local_expert_offsets = self._calc_expert_offset_each_layer()
        self.max_num_deployed_expert_per_rank = max(
    ...
    def _init_expert_mapping(self):
        ...
        if self.enable_rank_round_robin:
            self.selector = torch.zeros(num_layers,
                                        num_eps,
                                        1,
                                        dtype=torch.int32,
                                        device=self.device)
        ...
        self.num_redundant_per_expert = torch.zeros(num_layers,
                                                    num_eps,
                                                    dtype=torch.int32,
                                                    device=self.device)

        self.placement_pattern_cpu = self.placement_pattern.cpu()
        self.placement_mapping = omni_placement.PlacementMapping(
            "",  # TODO: pattern path, parse pattern in native C++
            self.rank,
    ...
    def _load_placement_pattern_with_validation(
        ...
        def build_basepattern(world_size, layers, num_experts):
            ...
            matrix = np.zeros(
                (world_size, layers, num_experts)).astype(np.int32)
        ...
        if self.pattern_path is None:
            ...
            if not os.path.exists(self.pattern_path):
                raise FileNotFoundError(
                    f"[Placement-Error]: Placement pattern file not found: {self.pattern_path}"
                )
            else:
                pattern = np.load(self.pattern_path).astype(np.int32)
            if pattern.shape != (self.world_size, self.max_moe_layer_num,
                                 self.num_experts):
                raise ValueError(
                    f"[Placement-Error]: pattern.shape[{pattern.shape}] is not equals to (world_size[{self.world_size}], layers[{self.max_moe_layer_num}], num_experts[{self.num_experts}])"
                )

        pattern = torch.tensor(pattern, dtype=torch.int32, device=self.device)
        # Validate pattern shape against num_devices_per_host
        if pattern.shape[0] % self.num_devices_per_host != 0:
            print(
                f"Warning: Number of devices in pattern ({pattern.shape[0]}) is not "
    ...
    # @calculate_time
    def is_expert_on_current_rank(self, layer_idx_moe: int, expert_id: int,
                                  current_rank: int,
                                  experts_per_rank: int) -> Tuple[bool, int]:
        """
        ...
        if not self.is_moe_layer(layer_idx_moe):
            return self._default_deployment_check(expert_id, current_rank,
                                                  experts_per_rank)
    ...
    def _default_deployment_check(self, expert_id: int, current_rank: int,
                                  experts_per_rank: int) -> Tuple[bool, int]:
    ...
    def get_num_of_redundant_experts(self,
                                     moe_layer_idx: int,
                                     num_expert_per_device_origin=16,
                                     rank_device=0) -> int:
        """
        ...
        num_redundant_experts = round(
            torch.sum(experts_here).item() - num_expert_per_device_origin)
    ...
    def get_deployed_experts_per_layer(self) -> list:
        ...
        if self.max_redundant_per_rank is not None:
            ...
            return [
                self.get_total_num_expert() +
                self.max_redundant_per_rank * self.get_world_size()
            ] * num_layers
        # static redundant num from parttern
        deployed_experts_per_layer = torch.sum(self.placement_pattern,
                                               dim=(0, 2)).tolist()
    ...
    def get_redundant_enable_per_layer(self) -> list:
        ...
        if self.max_redundant_per_rank is not None:
            return [False
                    ] * num_layers if self.max_redundant_per_rank == 0 else [
                        True
                    ] * num_layers
        ...
        redundant_enable_per_layer = [
            not (value == num_logits_expert_per_rank)
            for value in deployed_experts_per_layer
        ]
    ...
    def _calc_expert_offset_each_layer(self):
        ...
        local_expert_offsets[
            1:] = cumsum_experts[:-1]  # 第 i 个 rank 的 offset 是前 i-1 个 rank 的和
        return local_expert_offsets

    def get_local_expert_indices_offset(self, layer_idx_moe: int,
                                        current_rank: int,
                                        default_experts_per_rank: int) -> int:
        if self.max_redundant_per_rank is not None:
    ...
    def get_default_placement_layers(self):
        ...
        if torch.any(expected_ranks >= world_size):
            raise RuntimeError(
                f"Some experts require ranks beyond world_size {world_size}")
        ...
        for layer in range(num_layers):
            ...
            valid_non_expected = torch.ones(num_experts,
                                            dtype=torch.bool,
                                            device=placement_pattern.device)
            for rank in range(world_size):
                # 跳过预期 rank
                ...
                if mask.any():
                    # 检查非预期 rank 上的值是否为 0
                    valid_non_expected[mask] &= (
                        layer_placement[rank, expert_ids[mask]] == 0)

            # 该层有效当且仅当预期 rank 值为 1 且非预期 rank 值为 0
            valid_layers.append(
                torch.all(valid_expected & valid_non_expected).item())
