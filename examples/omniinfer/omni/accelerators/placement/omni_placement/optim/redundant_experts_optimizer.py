...
class Redundant2ExpertsBalancer(Optimizer):
    ...
    def __init__(self,
        ...
        self.expert_per_device = int(torch.max(expert_sums).item() +
                                     0.5)  # 求所有和的最大值
    ...
    def _NOT_DEVICE_SPECIFIC_optimize(
        ...
        token_expert_id[:midpoint] = self.expert_position_A[
            layer_idx_moe, token_expert_id[:midpoint]]

        # Map the second half of token_expert_id using expert_position_B
        token_expert_id[midpoint:] = self.expert_position_B[
            layer_idx_moe, token_expert_id[midpoint:]]
    ...
    def _DEVICE_SPECIFIC_optimize(
        ...
        """
        # Return the updated token distribution
        return tokens, self.local_expert_mapping[layer_idx_moe,
    ...
    def _build_loacl_expert_mapping_with_frozen_part(self):
        ...
        placement_pattern_frozen = torch.zeros_like(placement_pattern,
                                                    dtype=torch.int)

        # --- 2. 处理唯一专家 (counts == 1)
        unique_mask_2d = (expert_counts == 1
                          )  # unique_mask_2d shape: (num_layers, num_epids)
        ...
        placement_pattern_frozen = torch.where(
            unique_mask_3d & (placement_pattern == 1), 1,
            placement_pattern_frozen)
        ...
        redundant_locations_3d = redundant_mask_2d.unsqueeze(0) & (
            placement_pattern == 1)

        # 如果没有任何冗余专家，可以提前结束冗余部分的计算
        if not redundant_locations_3d.any():
            self.expert_mapping_frozen = self._construct_expert_mapping_from_placement_pattern(
                placement_pattern_frozen)
            return 0

        # --- 3a. 向量化组检查 --- host_ids shape: (num_devices,)
        host_ids = torch.arange(num_devices,
                                device=self.device) // self.num_device_per_host
        ...
        group_has_it_map = (torch.sum(placement_pattern * is_on_current_host,
                                      dim=0,
                                      keepdim=True) > 0)
        ...
        if experts_per_device_ideal == 0:
            ...
            print(
                f"Warning/Error: num_epids ({num_epids}) < num_devices ({num_devices}). Partitioning logic might be invalid."
            )
        ...
        expected_device_id = (
            torch.arange(num_epids, device=self.device) //
            experts_per_device_ideal).view(
                1, 1, -1)  # expected_device_id shape: (1, 1, num_epids)
        # Clamp expected_device_id to be within valid device range [0, num_devices-1], 这在 num_epids 不是 num_devices 的精确倍数时很重要
        expected_device_id = torch.clamp(expected_device_id,
                                         max=num_devices - 1)

        # device_indices shape: (num_devices, 1, 1)
        device_indices = torch.arange(num_devices,
        ...
        final_redundant_mask = redundant_locations_3d & (
            set_when_group_has | set_when_group_has_not)

        # 更新 placement_pattern_frozen 中冗余专家的位置
        placement_pattern_frozen = torch.where(final_redundant_mask, 1,
                                               placement_pattern_frozen)
    ...
    def _build_expert_mapping(self):
        ...
        device_indices_3d = torch.arange(num_devices,
                                         device=self.device).view(-1, 1, 1)
        ...
        masked_indices_for_min = torch.where(placement_pattern == 1,
                                             device_indices_3d, num_devices)
        masked_indices_for_max = torch.where(placement_pattern == 1,
                                             device_indices_3d, -1)
        ...
        cond2_A = mask_2_2d.unsqueeze(0) & (device_indices_3d
                                            == min_dev_idx_2d.unsqueeze(0))
        ...
        cond2_B = mask_2_2d.unsqueeze(0) & (device_indices_3d
                                            == max_dev_idx_2d.unsqueeze(0))
        # Combine conditions
        expert_mapping_B = torch.where(cond1_B | cond2_B, 1, 0).int()

        # --- 6. Call Position Function ---
        self.expert_position_A = self._construct_expert_mapping_from_placement_pattern(
            expert_mapping_A)
        self.expert_position_B = self._construct_expert_mapping_from_placement_pattern(
            expert_mapping_B)

        return 0  # Maintain original return signature

    def _construct_expert_mapping_from_placement_pattern(
            self, expert_mappingX):
        """
        ...
        global_indices_masked = torch.full_like(potential_global_indices,
                                                -1,
                                                dtype=torch.long)
