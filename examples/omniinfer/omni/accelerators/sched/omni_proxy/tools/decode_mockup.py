...
async def stream_response_generator(max_tokens: int = 1000):
    # 模拟的 token 列表
    tokens = [
        "这是", "一", "个", "由", "OpenAI", "API", "模拟", "服务", "生成", "的", "流式",
        "响应", "示例", "在", "真实", "场景", "中", "这", "将", "是", "来自", "大型", "语言",
        "模型", "的", "实时", "生成", "内容"
    ]
    ...
    while current_token_count < max_tokens:
        ...
        tokens_to_return = random.sample(
            tokens, min(num_tokens_to_return, len(tokens)))
        ...
        for token in tokens_to_return:
            if current_token_count < max_tokens:
                ...
                resp = {
                    #"id": f"chatcmpl-6789",
                    "id":
                    f"chatcmpl-{''.join(random.choices('abcdefghijklmnopqrstuvwxyz0123456789', k=28))}",
                    "object":
                    ...
                    "choices": [{
                        ...
                        None,
                        "finish_reason":
                        "stop" if current_token_count >= max_tokens else None
                    }],
                    "usage":
                    usage
                }
