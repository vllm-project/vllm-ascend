...
class DeepseekMoE(nn.Module):
    ...
    def __init__(
        ...
        self.n_routed_experts = get_attr_by_names(config,
                                                  n_routed_experts_names, 256)
        ...
        if self.ep_size > (self.n_routed_experts +
                           self.redundancy_shared_expert_num):
            raise ValueError(
        ...
        if getattr(config, "topk_method", "topk") == "noaux_tc":
            self.gate.e_score_correction_bias = nn.Parameter(
                torch.empty(self.n_routed_experts, dtype=torch.float),
                requires_grad=False)
        else:
        ...
        first_k_dense_replace_names = [
            'num_dense_layers', 'first_k_dense_replace'
        ]
        self.n_shared_experts = get_attr_by_names(config,
                                                  n_shared_experts_names, 1)
        self.first_k_dense_replace = get_attr_by_names(
            config, first_k_dense_replace_names, 3)
        ...
        if self.global_rank >= self.redundancy_shared_expert_num:
            ...
            if model_extra_config.operator_opt_config.use_omni_placement:
                self.planner = OmniPlanner(
                    config_file=model_extra_config.operator_opt_config.
                    omni_placement_config_path,
                    device="npu",
                    rank=get_world_group().rank_in_group,
                    world_size=get_world_group().world_size,
                    num_experts=self.n_routed_experts,
                    num_redundancy_shared_expert_rank=self.
                    redundancy_shared_expert_num)
        ...
        if self.n_shared_experts is not None and \
            ...
            if self.redundancy_shared_expert_num > 0 and model_extra_config.operator_opt_config.use_omni_placement:
                ...
                self.planner = OmniPlanner(
                    ...
                    redundancy_shared_expert_num)
                self.moe_layer_idx = OmniPlanner.get_deepseek_v3_moe_layer_idx(
                    f"{prefix}.share_experts",
                    first_k_dense_replace=self.first_k_dense_replace)
                self.expert_mapping = self.planner.expert_mapping_on_current_layer(
                    self.moe_layer_idx, is_prefill=False)
        ...
        if model_extra_config.operator_opt_config.gmm_nz:
            self.tuning_config = model_extra_config.operator_opt_config.decode_gear_list[:
                                                                                         1]
    ...
    def forward(self,
        ...
        if self.redundancy_shared_expert_num > 0:
            if attn_metadata is None or attn_metadata.prefill is not None:
                return self.forward_separate_expert_prefill(
                    hidden_states, residual, attn_metadata)
            else:
                return self.forward_separate_expert_decode(
                    hidden_states, residual, attn_metadata)
        else:
            if not self.is_init_gate:
                self.gate.weight.data = torch_npu.npu_format_cast(
                    self.gate.weight.data, 2)
                self.is_init_gate = True
            if attn_metadata is None or attn_metadata.prefill is not None:
                if self.is_A2:
                    return self.forward_prefill_a2(hidden_states, residual,
                                                   attn_metadata)
                else:
                    return self._forward_prefill_norm(hidden_states, residual,
                                                      attn_metadata)
            elif self.is_A2:
                return self.forward_decode_a2(hidden_states, residual,
                                              attn_metadata, layer_id,
                                              next_attention_weights)
            else:
                return self._forward_decode_norm(hidden_states, residual,
                                                 attn_metadata, layer_id,
                                                 next_attention_weights)

    def _forward_prefill_norm(
            self, hidden_states: torch.Tensor, residual: torch.Tensor,
            attn_metadata: AttentionMetadata) -> torch.Tensor:
        shared_output = self.shared_experts(hidden_states)

        if not model_extra_config.operator_opt_config.prefill_moe_all_to_all:
            hidden_states_int8, pertoken_scale = torch_npu.npu_dynamic_quant(
                hidden_states)
            global_hidden_states = get_world_group().all_gather(
                hidden_states_int8, dim=0)
        else:
        ...
        if not model_extra_config.operator_opt_config.prefill_moe_all_to_all:
            topk_cat = torch.cat((topk_weights, topk_ids.to(
                torch.float), pertoken_scale.unsqueeze(-1)),
                                 dim=-1)
            topk_all = get_world_group().all_gather(topk_cat, dim=0)
            topk_weights, topk_ids, global_pertoken_scale = torch.split(
                topk_all, [topk_weights.shape[-1], topk_ids.shape[-1], 1],
                dim=-1)
        ...
        if not model_extra_config.operator_opt_config.prefill_moe_all_to_all:
            final_hidden_states = get_world_group().reduce_scatter(
                final_hidden_states)
    ...
    def _forward_decode_norm(
        ...
        if model_extra_config.operator_opt_config.moe_multi_stream_tune and \
            ...
            if model_extra_config.operator_opt_config.use_super_kernel:
                with tng.scope.super_kernel(self.prefix, 'stream-fusion=1'):
                    return self._forward_decode_dispatch_combine(
                        hidden_states, residual, attn_metadata, layer_id,
                        next_attention_weights)
            else:
                return self._forward_decode_dispatch_combine(
                    hidden_states, residual, attn_metadata, layer_id,
        ...
        if model_extra_config.operator_opt_config.moe_multi_stream_tune:
            with tng.scope.npu_stream_switch('21'):
                hidden_states = tng.scope.npu_wait_tensor(
        ...
        topk_ids = self.experts.apply_expert_load_balance(
            topk_ids=topk_ids, best_topk_ids=attn_metadata.decode.best_topk)
        if not model_extra_config.operator_opt_config.decode_moe_dispatch_combine:
            topk_cat = torch.cat((topk_weights, topk_ids.to(
                torch.float), pertoken_scale.unsqueeze(-1)),
                                 dim=-1)
    ...
    def _forward_decode_dispatch_combine(
            self,
            hidden_states: torch.Tensor,
            residual: torch.Tensor,
            attn_metadata: AttentionMetadata,
        ...
        with tng.scope.npu_stream_switch(STREAM_SHARED_EXPERT):
            hidden_states = tng.scope.npu_wait_tensor(hidden_states,
                                                      router_logits)
            # shared_experts w13
            gate_up_share, _ = self.shared_experts.gate_up_proj.forward(
                hidden_states)
        wait_gate = gate_up_share if isinstance(
            gate_up_share, torch.Tensor) else gate_up_share[0]

        # expert weight prefetch
        if self.w13_prefetch_size > 0:
            torch_npu.npu_prefetch(self.experts.w13_weight, wait_gate,
                                   self.w13_prefetch_size)
        if self.w2_prefetch_size > 0:
            torch_npu.npu_prefetch(self.experts.w2_weight, wait_gate,
        ...
        max_num_deployed_expert = self.local_expert_num * get_dp_group(
        ).world_size
        ...
        kwargs = {
            ...
            "moe_expert_num":
            max_num_deployed_expert,  #ENABLE_OMNI_PLANNER, 0 redundancy 256, 1 redundancy expert 320
        ...
        kwargs.update({
            "scales":
            None,  # Quantization coefficient
            "quant_mode":
            layer.
            quant_mode,  # 0: Non-quantization; 1: Static quantization; 2: Dynamic quantization
            "group_ep":
            layer.
            moe_all_to_all_group_name,  # Unlike torch, it is obtained by name.
            "ep_world_size":
            all_to_all_group_size,
            "ep_rank_id":
            global_rank // experts_tp_size,
            "group_tp":
            layer.moe_rs_group_name,
            "tp_world_size":
            experts_tp_size,
            "tp_rank_id":
            global_rank % experts_tp_size,
            "x_active_mask":
            mc2_mask
            if model_extra_config.operator_opt_config.enable_mc2_v2 else None,
        })
        ...
        if self.quant_symbol:
            if self.experts.weight_num_bits == 8:
                ...
                raise NotImplementedError(
                    f"Unsupported compress tensor type. num bits: {self.experts.weight_num_bits}"
                )

            if self.experts.quant_mode:  # 0: no quant 1: static quant 2: dynamic quant
                pertoken_scale = dynamic_scale
            else:
                expand_x, pertoken_scale = torch_npu.npu_dynamic_quant(
                    expand_x)
        ...
        with tng.scope.npu_stream_switch(STREAM_SHARED_EXPERT):
            ...
            intermediate_hiddenstates_share = self.shared_experts.act_fn(
                gate_up_share, self.shared_experts.quant_symbol)
        if self.quant_symbol:
            # w8a8
            if self.experts.weight_num_bits == 8:
                ...
                fake_scale = torch.ones(weight_bias1_3.shape,
                                        dtype=torch.float32,
                                        device="npu").view(
                                            -1, weight_bias1_3.shape[1])
                pertoken_scale = torch.ones(pertoken_scale.shape,
                                            dtype=torch.float32,
                                            device="npu")
                gate_up_proj, pertoken_scale = torch_npu.npu_dequant_swiglu_quant(
                    gate_up_proj,
        ...
        stage3_kwargs = {
            "ep_send_counts":
            ep_recv_counts,  # dispatch's send_counts
            "group_ep":
            layer.
            moe_all_to_all_group_name,  # Unlike torch, it is obtained by name.
            "ep_world_size":
        ...
        with tng.scope.npu_stream_switch(STREAM_SHARED_EXPERT):
            if isinstance(intermediate_hiddenstates_share, dict):
                intermediate_hiddenstates_share[
                    'x_int8'] = tng.scope.npu_wait_tensor(
                        intermediate_hiddenstates_share.get('x_int8'),
                        hidden_states_experts)
            else:
                intermediate_hiddenstates_share = tng.scope.npu_wait_tensor(
                    intermediate_hiddenstates_share, hidden_states_experts)
            shared_output, _ = self.shared_experts.down_proj.forward(
                intermediate_hiddenstates_share)

        # prefetch weights for attention next layer
        if model_extra_config.operator_opt_config.attn_prefetch > 0 and next_attention_weights is not None and next_attention_weights[
                'q_a_proj_weight'] is not None:
            attn_prefetch_size = model_extra_config.operator_opt_config.attn_prefetch * 1024 * 1024
            attn_prefetch_flag = shared_output
            torch_npu.npu_prefetch(next_attention_weights['q_a_proj_weight'],
                                   attn_prefetch_flag, attn_prefetch_size)
            if self.quant_symbol:
                torch_npu.npu_prefetch(
                    next_attention_weights['kv_a_proj_with_mqa_weight'],
                    attn_prefetch_flag, attn_prefetch_size)
            torch_npu.npu_prefetch(next_attention_weights['q_b_proj_weight'],
                                   attn_prefetch_flag, attn_prefetch_size)
            torch_npu.npu_prefetch(next_attention_weights['W_UK'],
                                   attn_prefetch_flag, attn_prefetch_size)
        ...
        if model_extra_config.operator_opt_config.enable_mc2_v2:
            ...
            hidden_states_route = torch_npu.npu_moe_distribute_combine(
                **kwargs)
    ...
    def forward_separate_expert_decode(
            self, hidden_states: torch.Tensor, residual: torch.Tensor,
            attn_metadata: AttentionMetadata) -> torch.Tensor:
        router_logits, _ = self.gate.forward(hidden_states.float())
        ...
        if model_extra_config.operator_opt_config.use_omni_placement:
            if self.planner.is_moe_layer(self.moe_layer_idx):
                hidden_states, topk_ids, topk_weights = self.planner.plan(
                ...
                max_num_deployed_expert = max_num_deployed_expert_per_rank * (
                    self.ep_size - self.redundancy_shared_expert_num)
        ...
        hidden_states = fused_experts_moe_dispatch_combine(
            self.shared_experts or self.experts,
            hidden_states,
            topk_weights,
            topk_ids,
    ...
    def forward_separate_expert_prefill(
            self, hidden_states: torch.Tensor, residual: torch.Tensor,
            attn_metadata: AttentionMetadata) -> torch.Tensor:
        global_hidden_states = get_ep_group().all_gather(hidden_states, dim=0)
        if self.shared_experts:
            avg_tokens_per_shared_experts = global_hidden_states.shape[
                0] // self.redundancy_shared_expert_num
            shared_experts_mask = torch.zeros(global_hidden_states.shape[0],
                                              1,
                                              dtype=torch.int32,
                                              device="npu")
            shared_experts_mask[self.global_rank *
                                avg_tokens_per_shared_experts:
                                (self.global_rank + 1) *
                                avg_tokens_per_shared_experts] = 1
        ...
        if self.experts:
            ...
            topk_weights, topk_ids, _ = FusedMoE.select_experts(
                ...
                self.routed_scaling_factor,
                layer=self.experts)
            global_hidden_states, global_pertoken_scale = torch_npu.npu_dynamic_quant(
                global_hidden_states)
            output = self.experts(hidden_states=global_hidden_states,
                                  topk_weights=topk_weights,
                                  topk_ids=topk_ids,
                                  pertoken_scale=global_pertoken_scale,
                                  attn_metadata=attn_metadata)
    ...
    def chunked_gmm(self, hidden_states: torch.Tensor,
                    topk_weights: torch.Tensor, topk_ids: torch.Tensor,
                    pertoken_scale: torch.Tensor,
                    attn_metadata: AttentionMetadata, chunk_size: int):
        ...
        if hidden_states.shape[0] > chunk_size:
            ...
            for hid_states, topk_w, topk_id, scale in zip(
                    hidden_states_list, topk_weights_list, topk_ids_list,
                    pertoken_scale_list):
                out.append(
                    self.experts(hidden_states=hid_states,
                                 topk_weights=topk_w,
                                 topk_ids=topk_id,
    ...
    def forward_decode_a2(self,
        ...
        if model_extra_config.operator_opt_config.moe_multi_stream_tune:
            with tng.scope.npu_stream_switch(STREAM_TOPK_COMPUTE):
                ...
                topk_ids = self.experts.apply_expert_load_balance(
                    topk_ids=topk_ids,
                    best_topk_ids=attn_metadata.decode.best_topk
                    if attn_metadata is not None
                    and attn_metadata.decode is not None else None)
                if attn_metadata is not None and attn_metadata.decode is not None:
                    ...
                    topk_ids = actual_batch_mask * topk_ids + (
                        1 - actual_batch_mask) * self.n_routed_experts
        ...
        ###### PIPELINE ALL_GATHER STARTS
            if LARGE_BATCH:
                with tng.scope.npu_stream_switch(STREAM_SHARED_EXPERT):
                    hidden_states_dict = {
                        "x_int8": hidden_states_int8,
                        "pertoken_scale": pertoken_scale
                    }
                    shared_output = self.shared_experts(hidden_states_dict)

                with tng.scope.npu_stream_switch(STREAM_PREFETCH):
                    torch_npu.npu_prefetch(self.experts.w13_weight,
                                           shared_output, MAX_PREFETCH_SIZE)
                ...
                with tng.scope.npu_stream_switch(STREAM_INTERNODE_COMM_0):
                    round0_swp = tng.scope.npu_wait_tensor(
                        hidden_states_int8, hidden_states_int8)
                    round0_swp = get_round_cross_group_from_list(round=0).swap(
                        round0_swp, method="all2allv")
                with tng.scope.npu_stream_switch(STREAM_INTERNODE_COMM_1):
                    round1_swp = tng.scope.npu_wait_tensor(
                        hidden_states_int8, input_ag)
                    round1_swp = get_round_cross_group_from_list(round=1).swap(
                        round1_swp, method="all2allv")
                with tng.scope.npu_stream_switch(STREAM_INTERNODE_COMM_2):
                    round2_swp = tng.scope.npu_wait_tensor(
                        hidden_states_int8, round1_swp)
                    round2_swp = get_round_cross_group_from_list(round=2).swap(
                        round2_swp, method="all2allv")

                with tng.scope.npu_stream_switch(STREAM_TOPK_COMM):
                    topk_local_all_wait = tng.scope.npu_wait_tensor(
                        topk_local_all, round0_swp)
                    topk_all = all_gather_cross(topk_local_all_wait,
                                                idx=1,
                                                dim=0)
                ...
                with tng.scope.npu_stream_switch(STREAM_INTERNODE_COMM_1):
                    round1_swp = tng.scope.npu_wait_tensor(
                        hidden_states_int8, topk_weights)
                    round1_swp = get_round_cross_group_from_list(round=1).swap(
                        round1_swp, method="all2allv")
                ...
                with tng.scope.npu_stream_switch(STREAM_TOPK_COMM):
                    topk_local_all_wait = tng.scope.npu_wait_tensor(
                        topk_local_all, topk_local_all)
                    topk_all = all_gather_cross(topk_local_all_wait,
                                                idx=1,
                                                dim=0)

                with tng.scope.npu_stream_switch(STREAM_PREFETCH):
                    torch_npu.npu_prefetch(self.experts.w13_weight, input_ag,
                                           MAX_PREFETCH_SIZE)

                with tng.scope.npu_stream_switch(STREAM_SHARED_EXPERT):
                    hidden_states_int8 = tng.scope.npu_wait_tensor(
                        hidden_states_int8, input_ag)
                    hidden_states_dict = {
                        "x_int8": hidden_states_int8,
                        "pertoken_scale": pertoken_scale
            ...
            if model_extra_config.operator_opt_config.use_prefetch:
                torch_npu.npu_prefetch(self.experts.w13_weight, shared_output,
                                       MAX_PREFETCH_SIZE)
            ...
            input_ag = all_gather_local(hidden_states_int8, idx=0, dim=0)
            round0_swp = get_round_cross_group_from_list(round=0).swap(
                hidden_states_int8, method="all2allv")
            round1_swp = get_round_cross_group_from_list(round=1).swap(
                hidden_states_int8, method="all2allv")
            round2_swp = get_round_cross_group_from_list(round=2).swap(
                hidden_states_int8, method="all2allv")
        ...
        if self.node_rank == 0:
            global_hidden_states = torch.cat(
                [input_ag, round0_ag, round1_ag, round2_ag], dim=0)
        elif self.node_rank == 1:
            global_hidden_states = torch.cat(
                [round0_ag, input_ag, round2_ag, round1_ag], dim=0)
        elif self.node_rank == 2:
            global_hidden_states = torch.cat(
                [round1_ag, round2_ag, input_ag, round0_ag], dim=0)
        elif self.node_rank == 3:
            global_hidden_states = torch.cat(
                [round2_ag, round1_ag, round0_ag, input_ag], dim=0)
        ...
        if self.node_rank == 0:
            input_self, round0, round1, round2 = torch.split(
                final_hidden_states, final_hidden_states.shape[0] // 4, dim=0)
        elif self.node_rank == 1:
            round0, input_self, round2, round1 = torch.split(
                final_hidden_states, final_hidden_states.shape[0] // 4, dim=0)
        elif self.node_rank == 2:
            round1, round2, input_self, round0 = torch.split(
                final_hidden_states, final_hidden_states.shape[0] // 4, dim=0)
        elif self.node_rank == 3:
            round2, round1, round0, input_self = torch.split(
                final_hidden_states, final_hidden_states.shape[0] // 4, dim=0)
        ...
        if model_extra_config.operator_opt_config.moe_multi_stream_tune:
            ...
            with tng.scope.npu_stream_switch(STREAM_PREFETCH):
                if self.attn_prefetch is not None:
                    torch_npu.npu_prefetch(self.attn_prefetch.q_a_proj.weight,
                                           input_self, MAX_PREFETCH_SIZE)
                    torch_npu.npu_prefetch(
                        self.attn_prefetch.kv_a_proj_with_mqa.weight,
                        input_self, MAX_PREFETCH_SIZE)
                    torch_npu.npu_prefetch(self.attn_prefetch.q_b_proj.weight,
                                           input_self, MAX_PREFETCH_SIZE)
                    torch_npu.npu_prefetch(self.attn_prefetch.W_UK, input_self,
                                           MAX_PREFETCH_SIZE)
                if kv_prefetch is not None and isinstance(
                        kv_prefetch, Tuple) and kv_prefetch[0].numel():
                    torch_npu.npu_prefetch(kv_prefetch[0], input_self,
                                           MAX_PREFETCH_SIZE)
            ...
            if self.attn_prefetch is not None:
                torch_npu.npu_prefetch(self.attn_prefetch.q_a_proj.weight,
                                       input_self, MAX_PREFETCH_SIZE)
                torch_npu.npu_prefetch(
                    self.attn_prefetch.kv_a_proj_with_mqa.weight, input_self,
                    MAX_PREFETCH_SIZE)
                torch_npu.npu_prefetch(self.attn_prefetch.q_b_proj.weight,
                                       input_self, MAX_PREFETCH_SIZE)
                torch_npu.npu_prefetch(self.attn_prefetch.W_UK, input_self,
    ...
    def forward_prefill_a2(self, hidden_states: torch.Tensor,
                           residual: torch.Tensor,
                           attn_metadata: AttentionMetadata) -> torch.Tensor:
        MULTISTREAM_THRESHOLD = 1200
        GMM_CHUNK_SIZE = MULTISTREAM_THRESHOLD * get_ep_group().world_size
        enable_prefill_moe_multi_stream = True if hidden_states.shape[
            0] <= MULTISTREAM_THRESHOLD else False
        enable_prefill_pipeline_comm = model_extra_config.operator_opt_config.enable_pipeline_comm and hidden_states.shape[
            0] <= MULTISTREAM_THRESHOLD
        hidden_states_int8, pertoken_scale = torch_npu.npu_dynamic_quant(
            hidden_states)
        ...
        if enable_prefill_moe_multi_stream:
            ...
            with torch.npu.stream(shared_stream):
                global_hidden_states = all_gather_two_stage(hidden_states_int8,
                                                            idx=0,
                                                            dim=0)
        else:
            global_hidden_states = all_gather_two_stage(hidden_states_int8,
        ...
        if enable_prefill_pipeline_comm:
            final_hidden_states = prefill_reduce_scatter_pipeline(
                final_hidden_states, idx=1, which_half=self.which_half)
        else:
            final_hidden_states = reduce_scatter_two_stage(final_hidden_states,
                                                           idx=0)
