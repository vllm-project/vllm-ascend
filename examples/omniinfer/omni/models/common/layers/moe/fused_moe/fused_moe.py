...
def grouped_topk(hidden_states: torch.Tensor,
    ...
    row_idx = row_idx.reshape(topk_ids.shape[1],
                              topk_ids.shape[0]).transpose(1, 0).contiguous()
    # adapt end
...
def fused_experts_allgather_ep(
    ...
    if expert_parallel_size > 1:
        ...
        if warm_up:
            ...
            sorted_tokens = torch.randn(topk * num_tokens,
                                        hidden_size,
                                        dtype=hidden_states.dtype,
                                        device=hidden_states.device)
            expanded_src_to_dst_row = row_idx.reshape(-1)
            expanded_expert_idx = torch.zeros_like(topk_ids).reshape(-1).to(
                torch.int32)

            expert_tokens = torch_npu.npu_moe_compute_expert_tokens(
                expanded_expert_idx, n_routed_experts)
            ...
            if expanded_expert_idx.shape[0] > 8192:
                expert_tokens = torch_npu.npu_moe_compute_expert_tokens(
                    expanded_expert_idx, n_routed_experts + 1)
        ...
        raise ValueError(
            "expert_parallel_size should be larger than 1 if enable_moe_expert_parallel"
        )


def fused_experts_alltoall_ep(
    hidden_states: torch.Tensor,
    w1: Tuple,
    w2: Tuple,
    topk_weights: torch.Tensor,
    ...
    if ep_size > 1:
        ...
        if warm_up:
            ...
            topk_idx = torch.arange(
                ...
                idx_all_prefill,
                dtype=torch.int32,
                device=current_platform.device_type).reshape(
                    -1, topk_ids.shape[1]) % num_global_experts
            tokens_per_expert = torch.full(
                (num_global_experts, ),
                idx_all_prefill / num_global_experts,
                dtype=torch.int32,
            ...
            global_local_indexs = torch.zeros(
                (global_local_tokens.shape[0]),
                device=global_local_tokens.device,
                dtype=torch.int32)
            ...
            for i, k in enumerate(tokens_per_expert.cpu()):
                ...
                s += k
            permuted_row_idx = torch.arange(
                global_local_indexs.numel(),
                device=global_local_indexs.device,
            ...
            if expanded_expert_idx.shape[0] > 8192:
                group_list = torch_npu.npu_moe_compute_expert_tokens(
                    expanded_expert_idx1, num_local_experts + 1)
                group_list = group_list[:-1]
            else:
                group_list = torch_npu.npu_moe_compute_expert_tokens(
                    expanded_expert_idx1, num_local_experts)
            ...
            if len(sorted_local_tokens1) > 0:
                ...
                topk_weights1 = torch.ones(global_local_indexs.shape,
                                           dtype=torch.float32,
                                           device=out.device)
            ...
            dist.all_to_all_single(gathered_tokens, new_x)
            ...
            tokens_per_expert = torch.histc(topk_ids,
            ...
            global_local_tokens = sorted_local_tokens.new_empty(
                tokens_per_expert_group.sum(dim=0),
                sorted_local_tokens.shape[1])
            ...
            tokens_per_expert_post_gather = tokens_per_expert_group.view(
                ep_size, num_local_experts).sum(dim=0)
            global_local_indexs = np.zeros(
                shape=(global_local_tokens.shape[0], ), dtype=np.int32)
...
def fused_experts_ep_best_alltoall(hidden_states: torch.Tensor, w1: Tuple,
                                   w2: Tuple, topk_weights: torch.Tensor,
                                   topk_ids: torch.Tensor,
                                   row_idx: torch.Tensor):
    ep_size = get_ep_group().world_size
    ...
    if num_tokens == ep_size:
        ...
        topk_idx = torch.arange(0,
                                idx_all_decode,
                                num_local_experts,
                                dtype=torch.int32,
                                device=current_platform.device_type).reshape(
                                    -1, topk_ids.shape[1]) % num_global_experts
        tokens_per_expert = torch.zeros([num_global_experts],
                                        device=current_platform.device_type,
        ...
        topk_idx = torch.arange(0,
                                ...
                                device=current_platform.device_type).reshape(
                                    -1, topk_ids.shape[1]) % num_global_experts
        tokens_per_expert = torch.full((num_global_experts, ),
                                       idx_all_prefill / num_global_experts,
                                       dtype=torch.int32,
                                       device=current_platform.device_type)
    ...
    global_local_indexs = np.zeros(shape=(global_local_tokens.shape[0], ),
                                   dtype=np.int32)
...
def fused_experts_w8a8_allgather_ep(
        hidden_states: torch.Tensor,
    ...
    expert_parallel_size = get_ep_group(
    ).world_size - redundancy_shared_expert_num
    ...
    if expert_parallel_size > 1:
        ...
        experts_start_idx = (
            get_world_group().rank_in_group - redundancy_shared_expert_num
        ) * max_num_deployed_expert_per_rank  # ENABLE_OMNI_PLANNER
        ...
        if is_prefill or not model_extra_config.operator_opt_config.enable_kv_rmsnorm_rope_cache:
            sorted_topk_weight = torch.index_select(topk_weights.reshape(-1),
                                                    0, expanded_x_idx)
            row_index = expanded_x_idx // topk_ids.shape[-1]
            row_index = row_index.to(torch.int64)
            share_input = torch.zeros(
                (batch_size // get_dp_group().world_size, hidden_size),
                dtype=torch.bfloat16,
                device=current_platform.device_type)
            scale_2 = torch.ones((n_routed_experts, w1_scale.shape[-1] // 2),
                                 dtype=torch.float32,
                                 device=current_platform.device_type)
        else:
            with tng.scope.npu_stream_switch('11'):
                expanded_x_idx = tng.scope.npu_wait_tensor(
                    expanded_x_idx, expanded_x_idx)
                sorted_topk_weight = torch.index_select(
                    topk_weights.reshape(-1), 0, expanded_x_idx)
                row_index = expanded_x_idx // topk_ids.shape[-1]
                row_index = row_index.to(torch.int64)
                share_input = torch.zeros(
                    (batch_size // get_dp_group().world_size, hidden_size),
                    ...
                    device=current_platform.device_type)
                scale_2 = torch.ones(
                    (n_routed_experts, w1_scale.shape[-1] // 2),
                    dtype=torch.float32,
                    device=current_platform.device_type)
        gate_up_proj = torch_npu.npu_grouped_matmul([sorted_tokens], [w1],
                                                    bias=None,
                                                    group_list=expert_tokens,
                                                    split_item=3,
...
def fused_experts_w4a8_allgather_ep(
        hidden_states: torch.Tensor,
        pertoken_scale: torch.Tensor,
        w1: torch.Tensor,
        w2: torch.Tensor,
    ...
    if expert_parallel_size > 1:
        ...
        sorted_tokens, expanded_x_idx, expert_tokens, dynamic_quant_scale = torch_npu.npu_moe_init_routing_v2(
            ...
            active_expert_range=expert_range,
            quant_mode=-1,
            row_idx_type=1)  # row_idx_type=1 for gmm+finalize

        if is_prefill or not model_extra_config.operator_opt_config.enable_kv_rmsnorm_rope_cache:
            expanded_x_idx = torch.clamp(expanded_x_idx, 0,
                                         expanded_x_idx.shape[0] - 1)
            sorted_topk_weight = torch.index_select(topk_weights.reshape(-1),
                                                    0, expanded_x_idx)
            row_index = torch.floor(
                torch.div(expanded_x_idx, topk_ids.shape[-1]))
            row_index = row_index.to(torch.int64)
            share_input = torch.zeros(
                (batch_size // get_dp_group().world_size, hidden_size),
                dtype=torch.bfloat16,
        ...
        fake_scale = torch.ones(w1_scale.shape,
                                dtype=torch.float32,
                                device="npu").view(-1, w1_scale.shape[-1])
        dynamic_quant_scale = torch.ones(dynamic_quant_scale.shape,
                                         dtype=torch.float32,
                                         device="npu")
        gate_up_proj, pertoken_scale = torch_npu.npu_dequant_swiglu_quant(
            gate_up_proj,
        ...
        output = torch_npu.npu_grouped_matmul_finalize_routing(
            ...
            row_index=row_index,
            output_bs=batch_size,
            group_list_type=1).to(torch.bfloat16)

        return output


def gmm_expert(layer,
               x,
               expert_tokens,
               dynamic_scale=None,
    ...
    if layer.weight_num_bits == 8:
        ...
        intermediate_h, pertoken_scale = torch_npu.npu_dequant_swiglu_quant(
            ...
            activation_scale=pertoken_scale.squeeze(0),
            bias=None,
            quant_scale=scale_2,
            quant_offset=None,
            group_index=expert_tokens,
        ...
        fake_scale = torch.ones(layer.w13_weight_int4_scale.shape,
                                dtype=torch.float32,
                                device="npu").view(
        ...
        raise NotImplementedError(
            f"Unsupported compress tensor type. num bits: {layer.weight_num_bits}"
        )
...
def moe_infer_fusion(
    ...
    max_num_deployed_expert = layer.w13_weight.shape[0] * get_ep_group(
    ).world_size
    if warm_up:
        ...
        cur_topk_list = [(i + global_rank // 1) % max_num_deployed_expert
    ...
    combine_tokens = torch.stack([tokens_per_expert_group, tokens_per_expert],
                                 dim=0)
    # view: EP, E//EP
    ...
    hidden_states_ordered_by_experts = gmm_expert(
        layer, hidden_states_sorted_by_experts,
        tokens_per_local_expert.to(torch.int64), gathered_pertoken_scale, None)
...
def moe_expert_quant_forward(layer,
                             sorted_tokens,
                             expert_tokens,
                             act_dtype,
                             dynamic_scale=None):
    if layer.quant_mode:
        ...
        sorted_tokens, pertoken_scale = torch_npu.npu_dynamic_quant(
            sorted_tokens)

    if layer.weight_num_bits == 8:

        gate_up_proj = torch_npu.npu_grouped_matmul([sorted_tokens],
                                                    [layer.w13_weight],
                                                    bias=None,
                                                    group_list=expert_tokens,
                                                    split_item=3,
        ...
        scale_2 = torch.ones(
            (len(layer.w13_weight), layer.w13_weight_scale.shape[-1] // 2),
            dtype=torch.float32,
            device=current_platform.device_type)
        gate_up_proj, pertoken_scale = torch_npu.npu_dequant_swiglu_quant(
            gate_up_proj,
        ...
        out = torch_npu.npu_grouped_matmul([gate_up_proj], [layer.w2_weight],
                                           scale=[w2_scale],
                                           per_token_scale=[pertoken_scale],
                                           bias=None,
                                           group_list=expert_tokens,
        ...
        gate_up_proj = torch_npu.npu_grouped_matmul(
            ...
            group_list_type=1,
            act_type=0,
            tuning_config=model_extra_config.operator_opt_config.
            decode_gear_list[0:],
            output_dtype=torch.bfloat16)[0]
...
def fused_experts_moe_dispatch_combine(
    ...
    if expert_parallel_size > 1:
        ...
        if shared_expert_rank_num > 0 and global_rank // experts_tp_size < shared_expert_rank_num:
            ...
            # cal experts
            group_list = group_list[:len(
                layer.w13_weight
            )]  # Adapt to redundant and non-redundant layers, #ENABLE_OMNI_PLANNER
            hidden_states_experts = moe_expert_quant_forward(
                layer, expand_x, group_list, act_dtype, dynamic_scale)
...
def static_routing(hidden_states: torch.Tensor):
    ...
    return indices % model_extra_config.parall_config.redundancy_shared_expert_num + get_ep_group(
    ).world_size - model_extra_config.parall_config.redundancy_shared_expert_num


def shared_expert_alltoall_ep(hidden_states: torch.Tensor,
                              expert: torch.nn.Module, warm_up: bool):
    if warm_up:
        return None
    world_size = get_ep_group().world_size
    expert_assignments = static_routing(hidden_states)
    expert_assignments_ = torch.from_numpy(expert_assignments).to(
        hidden_states.device)
    send_counts = torch.bincount(expert_assignments_,
                                 minlength=world_size).to(hidden_states.device)
    ...
    recv_data = torch.empty(recv_counts.sum().item(),
                            *hidden_states.shape[1:],
                            dtype=hidden_states.dtype,
                            device=hidden_states.device)
    dist.all_to_all_single(recv_data,
                           sorted_data,
    ...
    if recv_data.size(0) > 0:
        ...
        output = torch.empty((0, hidden_states.size(1)),
                             device=hidden_states.device,
                             dtype=hidden_states.dtype)
    ...
    recv_back_data = torch.empty(recv_back_counts.sum().item(),
                                 *output.shape[1:],
                                 dtype=output.dtype,
                                 device=output.device)
    dist.all_to_all_single(recv_back_data,
                           output,
...
def fused_experts_w8a8_allgather_ep_a2(
        hidden_states: torch.Tensor,
        pertoken_scale: torch.Tensor,
        w1: torch.Tensor,
        w2: torch.Tensor,
    ...
    if expert_parallel_size > 1:
        ...
        experts_start_idx = get_world_group(
        ).rank_in_group * max_num_deployed_expert_per_rank  # ENABLE_OMNI_PLANNER
        experts_end_idx = experts_start_idx + n_routed_experts
        expert_range = [experts_start_idx, experts_end_idx]
        ...
        if is_prefill or not model_extra_config.operator_opt_config.enable_kv_rmsnorm_rope_cache:
            if not model_extra_config.operator_opt_config.pd_seperate_prefill:
                range1 = torch.arange(0,
                                      expanded_x_idx.shape[0],
                                      dtype=torch.int32,
                                      device="npu")
                range2 = range1 * torch.tensor(
                    991, dtype=torch.int32, device="npu")
                ...
                expanded_x_idx = torch.clamp(expanded_x_idx,
                                             min=0,
                                             max=expanded_x_idx.shape[0] - 1)
                sorted_topk_weight = torch.index_select(
                    topk_weights.reshape(-1), 0, expanded_x_idx)
                row_index = torch.floor(
                    torch.div(expanded_x_idx,
                              topk_ids.shape[-1])).to(torch.int64)
                share_input = torch.zeros(
                    (batch_size // expert_parallel_size, hidden_size),
                    dtype=torch.bfloat16,
                    device="npu")
        else:
        ...
        if model_extra_config.operator_opt_config.pd_seperate_prefill:
            out = torch_npu.npu_grouped_matmul(
                ...
                group_list_type=1)[0]
            output = torch_npu.npu_moe_finalize_routing(
                out.unsqueeze(1).to(torch.bfloat16),
                None,
                None,
                None,
        ...
        if not is_prefill and (
                ...
                enable_round_pipeline_comm or
                model_extra_config.operator_opt_config.enable_pipeline_comm):
            return output
