...
class UnquantizedFusedMoEMethod(GPUUnquantizedFusedMoEMethod):
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor,
              ...
              comm_group: Optional[GroupCoordinator]) -> torch.Tensor:
        is_prefill = attn_metadata is None or attn_metadata.prefill is not None
        out = self.moe_infer_fusion(layer, x, topk_ids, topk_weights,
    ...
    def moe_infer_fusion(self,
        ...
        if self.warm_up:
            ...
            cur_topk_list = [
                (i + global_rank // 1) % 256
                for i in range(global_rank // 1 *
                               step, (global_rank // 1 + 1) * step)
            ]
            topk_ids = torch.Tensor(cur_topk_list).int().view(
                hidden_states.shape[0], -1).npu()
        else:
            topk_ids = topk_ids.int()
        max_num_deployed_expert = 256
        if model_extra_config.operator_opt_config.use_omni_placement and layer.moe_layer_idx < 58:
            max_num_deployed_expert = layer.planner.get_max_num_deployed_expert_per_rank(
        ...
        gathered_tokens = expanded_x.new_empty(all_tokens.item(),
                                               expanded_x.shape[1])
        dist.all_to_all_single(gathered_tokens, expanded_x, output_splits,
                               input_splits)
        # reroute
        ...
        if model_extra_config.operator_opt_config.use_omni_placement:
            layer.planner.record_activation(
                layer.moe_layer_idx,
                group_list,
                support_multi_stream=model_extra_config.operator_opt_config.
                moe_multi_stream_tune and (not is_prefill))
        mm1_mm3 = torch_npu.npu_grouped_matmul(
            [hidden_states_sorted_by_experts], [w1],
        ...
        new_x = torch.index_select(
            hidden_states_ordered_by_experts, 0,
            gathered_idxs_unsort.to(torch.float32).argsort().to(torch.int32))
        gathered_tokens = new_x.new_empty(*expanded_x.shape)

        dist.all_to_all_single(gathered_tokens, new_x, input_splits,
                               output_splits)

        return hidden_states, gathered_tokens, topk_weight, expanded_row_idx

    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        layer.w13_weight = torch.nn.Parameter(layer.w13_weight.transpose(
            1, 2).contiguous(),
                                              requires_grad=False)
        layer.w2_weight = torch.nn.Parameter(layer.w2_weight.transpose(
            1, 2).contiguous(),
                                             requires_grad=False)
        ...
        self.local_expert_indices_offset = (get_ep_group().rank_in_group *
                                            self.n_routed_experts)
        self.local_expert_indices = [
            self.local_expert_indices_offset + i
...
class FusedMoE(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.tp_size = (tp_size
                        if tp_size is not None else get_ep_group().world_size)
        self.top_k = top_k
        ...
        if self.use_grouped_topk:
            if num_expert_group is None or topk_group is None:
                raise RuntimeError(
                    "num_expert_group and topk_group must not be None")
        ...
        if model_extra_config.operator_opt_config.use_omni_placement:
            num_of_redundant_experts = self.planner.get_num_of_redundant_experts(
                moe_layer_idx=self.moe_layer_idx,
                num_expert_per_device_origin=num_experts,
                rank_device=get_ep_group().rank_in_group -
                model_extra_config.parall_config.redundancy_shared_expert_num)
        self.quant_method.create_weights(
            layer=self,
            num_experts=num_experts +
            num_of_redundant_experts,  # ENABLE_OMNI_PLANNER
    ...
    def cal_best_topk(self, tokens):
        ...
        cur_topk_list = [
            i % 256
            for i in range(global_rank * step, (global_rank + 1) * step)
        ]
    ...
    def apply_expert_load_balance(
            ...
            best_topk_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
        # omni placement
        ...
        if model_extra_config.operator_opt_config.best_ep:
            if self.is_prefill_instance:
                ...
                if model_extra_config.operator_opt_config.moe_multi_stream_tune:
                    topk_ids = tng.scope.npu_wait_tensor(
                        best_topk_ids, topk_ids)
                else:
        ...
        if use_grouped_topk:  # and num_expert_group != 1:
            ...
            if e_score_correction_bias is None and attn_metadata is None:
                ...
                topk_weights, topk_ids, _ = torch_npu.npu_moe_gating_top_k(
                    ...
                    group_select_mode=
                    1,  # 0: maximum in group; 1: topk2.sum(fix)
            ...
            row_idx = torch.arange(topk_ids.numel(),
                                   ...
                                   dtype=torch.int32).view(
                                       -1,
                                       router_logits.shape[0]).transpose(0, 1)
        elif custom_routing_function is None:
            topk_weights, topk_ids, row_idx = fused_topk(
                gating_output=router_logits,
                topk=top_k,
                renormalize=renormalize)
        else:
    ...
    def forward(self,
        ...
        if self.reduce_results and self.tp_size > 1:
            final_hidden_states = get_ep_group().all_reduce(
                final_hidden_states)
    ...
    def weight_loader(self, param: torch.nn.Parameter,
        ...
        if get_ep_group().world_size > 1:
            ep_rank = get_ep_group(
            ).rank_in_group - model_extra_config.parall_config.redundancy_shared_expert_num
            # ENABLE_OMNI_PLANNER
            if model_extra_config.operator_opt_config.use_omni_placement:
                # OMNI_PLANNER: determine the expert deployment based on the pattern
                exists_locally, local_pos = self.planner.is_expert_on_current_rank(
                    self.moe_layer_idx, expert_id, ep_rank, self.num_experts)
                ...
                if expert_id < ep_rank * self.num_experts or expert_id >= (
        ...
        if ("scale" in weight_name or "zero" in weight_name
                or "offset" in weight_name or "bias" in weight_name):
