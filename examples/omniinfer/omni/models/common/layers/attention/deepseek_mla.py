...
class Indexer(nn.Module):
    ...
    def __init__(self,
        ...
        for batch_size in model_extra_config.operator_opt_config.decode_gear_list:
            ...
            self.actual_seq_lengths[batch_size] = torch.tensor(
    ...
    def _apply_lightning_indexer(self,
        ...
        if is_prefill:
            ...
            if model_extra_config.parall_config.attn_sp_size > 1:
                ...
                if is_second:
                    actual_seq_kvlen = actual_seq_kvlen * (sp_size * 2 -
                                                           sp_rank)
                else:
                    actual_seq_kvlen = actual_seq_kvlen * (sp_rank + 1)
            actual_seq_lengths_query = attn_metadata.prefill.query_lens.to(
                torch.int32)
    ...
    def forward(self, x: torch.Tensor, qr: torch.Tensor,
        ...
        with stream_context("11"):
            ...
            q_pe, q_nope = torch.split(
                q, [self.rope_head_dim, self.head_dim - self.rope_head_dim],
                dim=-1)  # [b,s,64,64+64]
            ...
            if model_extra_config.parall_config.attn_sp_size > 1:
                q_nope, q_nope_2 = torch.split(q_nope,
                                               q_nope.size(0) // 2,
                                               dim=0)
        ...
        if model_extra_config.parall_config.attn_sp_size > 1:
            k_list = torch.split(k,
                                 attn_metadata.prefill.sp_reverse_split_list,
                                 dim=0)
            k = torch.cat(
                [k_list[i] for i in attn_metadata.prefill.sp_reverse_index],
                dim=0)
        k_pe, k_nope = torch.split(
            k, [self.rope_head_dim, self.head_dim - self.rope_head_dim],
            dim=-1)  # [b,s,64+64]
        ...
        with stream_context("22"):
            ...
            if model_extra_config.parall_config.attn_sp_size > 1:
                weights, weights_2 = torch.split(weights,
                                                 weights.size(0) // 2,
                                                 dim=0)

        topk_indices = self._apply_lightning_indexer(q, weights, attn_metadata,
                                                     kv_cache, is_prefill)

        topk_indices_2 = None
        if model_extra_config.parall_config.attn_sp_size > 1:
            topk_indices_2 = self._apply_lightning_indexer(
                q2, weights_2, attn_metadata, kv_cache, is_prefill, True)
...
class DeepseekMLA(nn.Module):
    ...
    def __init__(
        ...
        ) if not model_extra_config.operator_opt_config.enable_dsa else 1
        ...
        if self.q_lora_rank is not None:
            if self.merge_qkv:
                self.qkv_a_proj = MergedReplicatedLinear(
                    self.hidden_size, [
                        self.q_lora_rank,
        ...
        if model_extra_config.operator_opt_config.prefill_enable_mla_alltoall:
            if model_extra_config.parall_config.o_proj_tp_size > 1:
                self.o_proj = Tp2DpAndTpRowParallelLinear(
            ...
            self.o_proj = DP2TPRowParallelLinear(
                self.num_heads * self.v_head_dim,
                hidden_size,
                tp_size=get_o_proj_tp_group().world_size,
                tp_rank=get_o_proj_tp_group().rank_in_group,
            ...
            self.o_proj = RowParallelLinearCross(
                self.num_heads * self.v_head_dim,
                self.hidden_size,
                bias=False,
        ...
        if self.fa_quant:
            ...
            self.kv_scale = torch.nn.Parameter(torch.empty(
        ...
        if model_extra_config.parall_config.dp_size > 1 or model_extra_config.operator_opt_config.enable_dsa:
            ...
            expected_shape = (self.kv_lora_rank, self.num_heads *
                              (self.qk_nope_head_dim + self.v_head_dim))
            if kv_b_proj_weight.shape != expected_shape:
                raise RuntimeError(
                    f"{kv_b_proj_weight.shape} != {expected_shape}")
            ...
            for batch_size in model_extra_config.operator_opt_config.decode_gear_list:
                self.norm_res[batch_size] = torch.zeros(
                    [batch_size * self.tp_size, self.q_lora_rank],
                    dtype=torch.bfloat16,
                    device=current_platform.device_type)
                self.actual_seq_lengths[batch_size] = torch.tensor(
                    list(range(1, batch_size * self.tp_size + 1)),
                    dtype=torch.int64,
                    device=current_platform.device_type)
        ...
        if self.quant_symbol and model_extra_config.operator_opt_config.use_mlaprolog:
            self.q_a_proj.weight_scale.data = self.q_a_proj.weight_scale.data.to(
                torch.float)
            self.q_b_proj.weight_scale.data = self.q_b_proj.weight_scale.data.to(
                torch.float)
            if self.kv_a_proj_with_mqa is not None:
                self.kv_a_proj_with_mqa.weight_scale.data = self.kv_a_proj_with_mqa.weight_scale.data.to(
                    torch.float)
    ...
    def mla_epilog(self,
        ...
        attn_output = (torch.matmul(attn_output,
                                    self.W_UV).transpose(1, 0).reshape(
                                        -1, self.num_heads * self.v_head_dim))
        attn_output = attn_output.view(-1,
                                       self.num_local_heads * self.v_head_dim)

        if model_extra_config.parall_config.o_proj_tp_size > 1:
            output, _ = self.o_proj.forward(attn_output, q.shape[0], 1,
        ...
        if attn_metadata is not None:
            ...
            attn_output = torch.ops.custom.npu_sparse_flash_attention(
                ...
                actual_seq_lengths_query=actual_seq_qlen.to(
                    torch.int32),  # todo 等接口支持后切换成tensor
            ...
            attn_output = torch.zeros(
                q_nope.shape[0] //
                o_proj_tp_size if is_attn_output_reshape else q_nope.shape[0],
                # self.num_local_heads * o_proj_tp_size if is_attn_output_reshape else self.num_local_heads,
                self.num_heads *
                o_proj_tp_size if is_attn_output_reshape else self.num_heads,
                self.kv_lora_rank,
    ...
    def forward(
        ...
        if not self.is_init:
            self.W_UK = torch.nn.Parameter(self.W_UK.contiguous(),
                                           requires_grad=False)
            self.W_UV = torch.nn.Parameter(self.W_UV.contiguous(),
                                           requires_grad=False)
            self.is_init = True
        if self.kv_scale is not None and self.kv_scale_reci_tile is None:
            self.kv_scale_reci_tile = torch.nn.Parameter(torch.reciprocal(
                self.kv_scale).repeat(self.kv_lora_rank).view(1, -1),
                                                         requires_grad=False)
        if attn_metadata is None or attn_metadata.prefill is not None:
            if os.getenv(
                    "ASCEND_PLATFORM", "A3"
            ) == "A2" and model_extra_config.operator_opt_config.pd_seperate_prefill:
                output = self._forward_prefill_a2(positions, hidden_states,
                                                  kv_cache, attn_metadata)
            else:
                if model_extra_config.operator_opt_config.enable_dsa:
                    output = self._forward_prefill_absorb(
                        positions,
                        hidden_states,
                        kv_cache,
                        attn_metadata,
                    ...
                    output = self._forward_prefill(positions,
                                                   hidden_states,
                                                   kv_cache,
                                                   attn_metadata,
                                                   comm_group=comm_group)
            ...
            output = self._forward_decode(
                ...
                kv_cache,
                attn_metadata,
                use_rmsnorm_rope_cache=model_extra_config.operator_opt_config.
        ...
        if model_extra_config.operator_opt_config.use_mlaprolog and not self.is_mla_prolog_init:
            ...
            self.q_a_proj.weight = self._process_mla_prolog_weight(
                self.q_a_proj.weight)
            self.q_b_proj.weight = self._process_mla_prolog_weight(
                self.q_b_proj.weight)
            self.kv_a_proj_with_mqa.weight = self._process_mla_prolog_weight(
    ...
    def _process_mla_prolog_weight(self, weight):
        ...
        weight = torch.nn.Parameter(weight.transpose(0, 1).contiguous(),
                                    requires_grad=False)
    ...
    def _forward_prefill_absorb(
        ...
        if self.q_lora_rank is not None:
            ...
            latent_cache = mla_tensor_model_parallel_all_gather(
                latent_cache, dim=0, comm_group=comm_group)
            if model_extra_config.parall_config.attn_sp_size > 1:  # sp切分的是q，kv不做sp
                if attn_metadata is not None:
                    latent_cache_list = torch.split(
                        latent_cache,
                        attn_metadata.prefill.sp_reverse_split_list,
                        dim=0)
                    latent_cache = torch.cat([
                        latent_cache_list[i]
                        for i in attn_metadata.prefill.sp_reverse_index
                    ],
                                             dim=0)
            ...
            if self.quant_symbol and not model_extra_config.operator_opt_config.enable_dsa:
                ...
                if model_extra_config.parall_config.attn_sp_size == 1:
                    q_quant = mla_tensor_model_parallel_all_gather(
                        q_quant, dim=0, comm_group=comm_group)
                    q_scale = mla_tensor_model_parallel_all_gather(
                        q_scale, dim=0, comm_group=comm_group)
                qr = {'x_int8': q_quant, 'pertoken_scale': q_scale}
            else:
                if model_extra_config.parall_config.attn_sp_size == 1:
                    qr = mla_tensor_model_parallel_all_gather(
                        qr, dim=0, comm_group=comm_group)
            q = self.q_b_proj(qr)[0].view(-1, self.num_local_heads,
                                          self.qk_head_dim)
        else:
            ...
            if model_extra_config.parall_config.attn_sp_size == 1:
                q = tensor_model_parallel_all_gather(q, dim=0)
            latent_cache = tensor_model_parallel_all_gather(latent_cache,
                                                            dim=0)

        q_nope, q_pe = torch.split(
            q, [self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1)
        q_nope = q_nope.view(-1, self.num_local_heads,
                             self.qk_nope_head_dim).transpose(0, 1)  # n, bs, d

        q_nope = (torch.matmul(q_nope, self.W_UK).transpose(1, 0).view(
            -1, self.num_local_heads, self.kv_lora_rank))

        # k_pe:BNS,64 kv_a:BNS, 512, kv_states:bnsd, cos,sin:bnsd, kv cache:bsnd
        q_pe = q_pe.unsqueeze(2)
        if attn_metadata is None:
            cos_q = torch.zeros([q_pe.size(0), 1, 1, self.qk_rope_head_dim],
                                dtype=q_pe.dtype,
                                device=q_pe.device)
            sin_q = torch.zeros([q_pe.size(0), 1, 1, self.qk_rope_head_dim],
                                dtype=q_pe.dtype,
                                device=q_pe.device)
            cos = torch.zeros(
                [latent_cache.size(0), 1, 1, self.qk_rope_head_dim],
        ...
        if kv_cache is not None and isinstance(
            ...
            # adapt end
            kv_a, _ = torch.split(latent_cache,
                                  [self.kv_lora_rank, self.qk_rope_head_dim],
                                  dim=-1)
        ...
        if attn_metadata is not None:
            topk_indices, topk_indices2 = self.indexer(hidden_states,
                                                       qr,
                                                       attn_metadata,
                                                       kv_cache=kv_cache,
                                                       is_prefill=True)
        ...
        kv = (k_nope, k_rope)
        attn_output = self._apply_attention(topk_indices, query_states, kv,
                                            attn_metadata)
        if model_extra_config.parall_config.attn_sp_size > 1:
            attn_output_2 = self._apply_attention(topk_indices2,
                                                  query_states_2,
                                                  kv,
                                                  attn_metadata,
                                                  is_second_attn=True)
    ...
    def _forward_prefill(
        ...
        if self.q_lora_rank is not None:
            if self.merge_qkv:
                ...
                q, latent_cache = torch.split(qkv, [
                ...
                if self.quant_symbol:
                    ...
                    q = mla_tensor_model_parallel_all_gather(
                        q, dim=0, comm_group=comm_group)
                q = self.q_b_proj(q)[0].view(-1, self.num_local_heads,
                                             self.qk_head_dim)
        else:
            q = self.q_proj(hidden_states)[0].view(-1, self.num_local_heads,
        ...
        if kv_cache is not None and isinstance(
            ...
            if attn_metadata is not None:
                ...
                assert len(
                    kv_a.shape) == 2 and kv_a.shape[1] == 512, f"{kv_a.shape=}"
                ...
                if model_extra_config.operator_opt_config.use_omni_cache and \
                    ...
                    prefix_buffer[dst_slots, :, :512] = kv_a.unsqueeze(
                        1)[:num_actual_tokens]
                    prefix_buffer[dst_slots, :,
                                  512:] = k_pe[:num_actual_tokens]
        ...
        attn_output = torch.empty(
            q.shape[0] //
            o_proj_tp_size if is_attn_output_reshape else q.shape[0],
            self.num_local_heads *
            o_proj_tp_size if is_attn_output_reshape else self.num_local_heads,
            self.v_head_dim,
            device=q_nope.device,
            dtype=q_nope.dtype)
        ...
        if attn_metadata is not None:
            prefill_metadata = attn_metadata.prefill
            computed_tokens = 0
            assert not (self.fa_quant
                        and len(prefill_metadata.seq_qlen_group) > 1)
            assert len(prefill_metadata.seq_qlen_group
                       ) == 1, f"{len(prefill_metadata.seq_qlen_group)=}"
            for iter, (actual_seq_qlen, actual_seq_kvlen) in enumerate(
                    zip(prefill_metadata.seq_qlen_group,
                ...
                if prefill_metadata.kv_index_list and kv_cache is not None and isinstance(kv_cache, Tuple) and\
                    ...
                    kv_cache_a = (kv_cache[0].view(
                        block_num, 1, self.kv_lora_rank // KVCACHE_NZ_DIM,
                        block_size, KVCACHE_NZ_DIM))
                    kv_cache_pe = (kv_cache[1].view(
                        block_num, 1, self.qk_rope_head_dim // KVCACHE_NZ_DIM,
                        block_size, KVCACHE_NZ_DIM))
                ...
                if model_extra_config.operator_opt_config.use_omni_cache and \
                    ...
                    prefix_buffer = attn_metadata.omni_cache.prefix_buffer_npu
                    prefill_kv_a, prefill_k_pe = prefix_buffer[:actual_seq_kvlen[
                        -1], :, :512], prefix_buffer[:actual_seq_kvlen[-1], :,
                                                     512:]
                ...
                prefill_k_rope = prefill_k_pe.view(
                    -1, 1,
                    self.qk_rope_head_dim).repeat(1, self.num_local_heads, 1)
        ...
        if model_extra_config.operator_opt_config.use_omni_cache and \
            ...
            attn_metadata.omni_cache.synchronize_d2h(
                kv_a.unsqueeze(1), k_pe, attn_metadata.slot_mapping,
                self.layer_idx, kv_event)
            attn_metadata.omni_cache.synchronize_h2d(
                prefix_meta=attn_metadata.prefill.prefix_meta,
        ...
        if model_extra_config.operator_opt_config.prefill_enable_mla_alltoall:
            if attn_metadata is not None:
                if model_extra_config.parall_config.o_proj_tp_size > 1:
                    attn_output = attn_output.view(
                        get_o_proj_dp_group().world_size, -1,
                        self.num_local_heads, self.v_head_dim)
                ...
                if model_extra_config.parall_config.o_proj_tp_size > 1:
                    attn_output = all_to_all_attn_output.view(
                        ...
                        get_o_proj_tp_group().world_size,
                        q.shape[0] // get_tensor_model_parallel_world_size() *
                        get_o_proj_tp_group().world_size,
                        self.num_local_heads * self.v_head_dim).transpose(
                            0, 1).contiguous()
                else:
                    attn_output = all_to_all_attn_output.view(
                        get_tensor_model_parallel_world_size(),
            ...
            output, _ = self.o_proj.forward(
                attn_output.reshape(
                    -1,
                    o_proj_tp_size * self.num_local_heads * self.v_head_dim))
        else:
            attn_output = attn_output.view(
    ...
    def _forward_decode(self,
                        ...
                        kv_cache: Optional[torch.Tensor],
                        attn_metadata: AttentionMetadata,
                        use_rmsnorm_rope_cache: bool = True) -> torch.Tensor:
        if use_rmsnorm_rope_cache:
            ...
            if model_extra_config.operator_opt_config.use_mlaprolog:
                ...
                if self.quant_symbol:
                    hidden_states_mla_prolog, pertoken_scale = torch_npu.npu_dynamic_quant(
                        hidden_states)
                else:
                ...
                q_nope, q_pe, dequant_scale_q_nope, qr, dequant_scale_q_norm = torch.ops.npu.npu_mla_prolog_v3(
                    ...
                    if self.quant_symbol else None,  # pertoken quant
                    dequant_scale_w_dq=self.q_a_proj.weight_scale.view(1, -1)
                    if self.quant_symbol else None,
                    dequant_scale_w_uq_qr=self.q_b_proj.weight_scale.view(
                        1, -1) if self.quant_symbol else None,
                    dequant_scale_w_dkv_kr=self.kv_a_proj_with_mqa.
                    weight_scale.view(1, -1) if self.quant_symbol else None,
                ...
                if cache_mode == "PA_NZ":
                    k_nope = k_nope.view(block_num, 1,
                                         self.kv_lora_rank // nz_block_size,
                                         block_size, nz_block_size)
                    k_rope = k_rope.view(block_num, 1,
                                         self.qk_rope_head_dim_nz, block_size,
                                         16)
                k_nope = key_cache
                k_rope = value_cache
                q_nope = q_nope.view(bsz, self.num_local_heads,
                                     self.kv_lora_rank)
                ...
                if self.q_lora_rank is not None:
                    qr, _ = self.q_a_layernorm(
                        q_lowrank, self.norm_res[q_lowrank.shape[0]])
                ...
                q_nope, q_pe = torch.split(
                    q, [self.qk_nope_head_dim, self.qk_rope_head_dim],
                    dim=-1)  # b,n,s,d
                ...
                q_nope = q_nope.view(-1, self.num_local_heads,
                                     self.qk_nope_head_dim).transpose(
                                         0, 1)  # n, bs, d
                q_nope = (torch.matmul(q_nope, self.W_UK).transpose(1, 0).view(
                    bsz, q_len, self.num_local_heads, -1))
                ...
                with stream_context("11"):
                    ...
                    k_rope, k_nope, _, _ = torch_npu.npu_kv_rmsnorm_rope_cache(
                        ...
                        c_kv_scale=self.kv_scale_reci_tile,
                        epsilon=self.kv_a_layernorm.variance_epsilon,
                        cache_mode=cache_mode)  # adapter NZ

                    if not model_extra_config.operator_opt_config.enable_dsa:
                        # adapter nz
                        k_nope = k_nope.view(
                            block_num, 1, self.kv_lora_rank // nz_block_size,
                            block_size, nz_block_size)
                        k_rope = k_rope.view(
                            block_num, 1,
                            self.qk_rope_head_dim // KVCACHE_NZ_DIM,
                            block_size, KVCACHE_NZ_DIM)

                    if model_extra_config.operator_opt_config.moe_multi_stream_tune:
                        tng.scope.npu_wait_tensor(q_pe, k_nope)
                    # cos, sin = self.rotary_emb.get_cos_sin(positions)
                    q_pe = torch_npu.npu_interleave_rope(q_pe, cos,
                                                         sin)  # BNSD
                    if self.fa_quant:
                        q_nope, dequant_scale_q_nope = torch_npu.npu_dynamic_quant(
                            q_nope.view(bsz * self.num_heads,
                                        self.kv_lora_rank))
            ...
            if self.fa_quant:
                ...
                topk_indices, _ = self.indexer(hidden_states_origin,
                                               qr,
                                               attn_metadata,
                                               kv_cache=kv_cache,
                                               is_prefill=False)
                attn_output = torch.ops.custom.npu_sparse_flash_attention(
                    ...
                    sparse_block_size=1,
                    block_table=attn_metadata.decode.block_table,
                    actual_seq_lengths_query=self.actual_seq_lengths[bsz].to(
                        torch.int32),
                    actual_seq_lengths_kv=attn_metadata.decode.seq_lens.to(
                        torch.int32),
                    query_rope=q_pe,
                    key_rope=k_rope,
                    layout_query="TND",
            ...
            if model_extra_config.operator_opt_config.enable_dsa:
                ...
                attn_output = attn_output.view(
                    self.num_local_heads, bsz * q_len,
                    self.kv_lora_rank)  # adapter BSND_NBSD

            attn_output = (torch.matmul(attn_output, self.W_UV).transpose(
                1, 0).reshape(bsz, q_len, -1))
            attn_output = attn_output.view(
                -1, self.num_local_heads * self.v_head_dim)
            if model_extra_config.parall_config.o_proj_tp_size > 1:
                output, _ = self.o_proj.forward(attn_output, bsz, q_len,
                                                self.num_local_heads,
                                                self.v_head_dim)
            else:
            ...
            k_rope, k_nope, _, _ = torch_npu.npu_kv_rmsnorm_rope_cache(
                ...
                key_cache,
                epsilon=self.kv_a_layernorm.variance_epsilon,
                cache_mode="PA_NZ")  # adapter NZ

            # adapter nz
            k_nope = k_nope.view(block_num, 1,
                                 self.kv_lora_rank // KVCACHE_NZ_DIM,
                                 block_size, KVCACHE_NZ_DIM)
            k_rope = k_rope.view(block_num, 1,
                                 self.qk_rope_head_dim // KVCACHE_NZ_DIM,
                                 block_size, KVCACHE_NZ_DIM)
            ...
            # cos, sin = self.rotary_emb.get_cos_sin(positions)
            q_pe = torch_npu.npu_interleave_rope(q_pe, cos, sin)  # BNSD
            q_nope = q_nope.view(bsz, 1, self.num_local_heads,
                                 self.kv_lora_rank)
        ...
        if self.q_lora_rank is not None:
            if self.merge_qkv:
                ...
                if not isinstance(hidden_states, Dict):
                    h_quant, h_scale = torch_npu.npu_dynamic_quant(
                        hidden_states)
                    hidden_states = {
                        'x_int8': h_quant,
        ...
        if attn_metadata is not None:
            ...
            if kv_cache is not None and isinstance(
                ...
                kv_a, k_pe = torch.split(
                    latent_cache, [self.kv_lora_rank, self.qk_rope_head_dim],
                    dim=-1)
                kv_a = self.kv_a_layernorm(kv_a)
            ...
            if len(prefill_metadata.seq_qlen_group) == 1:
                # normally execute
                actual_seq_qlen = prefill_metadata.seq_qlen_group[
                    0] if prefill_metadata is not None else [q.shape[0]]
                actual_seq_kvlen = prefill_metadata.seq_kvlen_group[
                    0] if prefill_metadata is not None else [q.shape[0]]
                ...
                k = torch.cat([
                    k_nope,
                    k_pe.view(-1, 1, self.qk_rope_head_dim).repeat(
                ...
                attn_output = torch_npu.npu_fused_infer_attention_score(
                    ...
                    atten_mask=attn_mask,
                    actual_seq_lengths=actual_seq_qlen,
                    actual_seq_lengths_kv=actual_seq_kvlen)[0].view(
                        -1, self.num_local_heads, self.v_head_dim)
                ...
                for iter, (actual_seq_qlen, actual_seq_kvlen) in enumerate(
                        zip(prefill_metadata.seq_qlen_group,
                            prefill_metadata.seq_kvlen_group)):
                    prefill_q = q[computed_tokens:computed_tokens +
                                  actual_seq_qlen[-1]]
                    if prefill_metadata.kv_index_list and kv_cache is not None and isinstance(kv_cache, Tuple) and \
                            kv_cache[0].numel() > 0:
                    ...
                    prefill_k = torch.cat([
                        k_nope,
                        prefill_k_pe.view(-1, 1, self.qk_rope_head_dim).repeat(
                            1, self.num_local_heads, 1)
                    ],
                                          dim=-1)
            ...
            if model_extra_config.operator_opt_config.prefill_enable_mla_alltoall_local:
                ...
                attn_output = get_local_group_from_list(0).all_to_all(
                    attn_output)
                output, _ = self.o_proj.forward(attn_output)
