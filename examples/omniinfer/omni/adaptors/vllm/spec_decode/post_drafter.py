...
class PostDrafter(EagleProposer):
    ...
    def __init__(
        ...
        if self.method not in ('deepseek_mtp', 'pangu_ultra_moe_mtp'):
            raise ValueError(
                f"Speculative method should be one of ('deepseek_mtp','pangu_ultra_moe_mtp'), while get {self.method}."
            )
    ...
    def verify_and_prepare_inputs(
        ...
        if num_decodes > 0:
            self.input_ids[last_accepted_index] = forward_tokens.view(
                -1)[last_accepted_index]
        elif num_prefills > 0:
            self.input_ids[logits_indices] = forward_tokens.view(
                -1)[last_accepted_index]
        ...
        **kwargs,
        ...
        if kv_caches is None:
            ...
            with set_forward_context(attn_metadata, self.vllm_config):
                is_dummy = (last_accepted_index is None) or (sample_indices
                                                             is None)
                for layer_idx in range(
                        self.speculative_config.num_speculative_tokens):
                    drafter_logits, previous_hidden_states = self.model(
                        ...
                        previous_hidden_states=previous_hidden_states,
                        selected_indices=None if attn_state
                        == AscendAttentionState.DecodeOnly else sample_indices,
                        mtp_layer_idx=layer_idx,
                    )
                    if not is_dummy:
                        draft_forward_tokens = drafter_logits[
                            last_accepted_index].argmax(dim=-1)
                    ...
                    if not is_dummy:
                        ...
                        if attn_state == AscendAttentionState.DecodeOnly:
                            input_ids[
                                last_accepted_index] = draft_forward_tokens
                        else:  # prefill
