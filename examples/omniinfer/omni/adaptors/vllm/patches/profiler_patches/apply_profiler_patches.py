...
def apply_patches(namelist_path: str):
    try:
        ...
        logger.info(
            f"<<<Applying {profiler_type} profiler patches from {namelist_path}"
        )
        ...
        for target in config.get('targets', []):
            ...
            if module_name:
                ...
                logger.warning(
                    f"<<<Skipping target with missing 'module': {target}")
        ...
        for module_name, class_name, function_name, \
            ...
            logger.info(
                f"<<<Patching {module_name}.{function_name or 'all methods'}")
            try:
                ...
                if class_name:
                    try:
                        ...
                        try:
                            ...
                            wrapped_function = wrapper_method(
                                original_function, base_params)
                            setattr(target_class, function_name,
                                    wrapped_function)
                            logger.info(
                                f"<<<<{module_name}.{class_name}.{function_name} is wrapped"
                            )
                    ...
                    try:
                        original_function = getattr(original_module,
                                                    function_name)
                        wrapped_function = wrapper_method(
                            original_function, base_params)
                        setattr(original_module, function_name,
                                wrapped_function)
                        logger.info(
                            f"<<<<{module_name}.{function_name} is wrapped")
                    except AttributeError:
                        logger.warning(
    ...
    except (FileNotFoundError, ImportError, AttributeError, RuntimeError,
            yaml.YAMLError) as e:
    ...
    async def new_method(self, *args, **kwargs) -> AsyncGenerator:
        ...
        async for item in original_method(self, *args, **kwargs):
            ...
            if yield_count == 1:
                ...
                safe_print(
                    trace_output_directory,
                    f"<<<Action: First decode output token; Timestamp:{time.time()}; RequestID:{request_id}; Role:{os.getenv('ROLE')}_{ip_str}"
                )
            elif yield_count == 3:
                # Third chat_completion_stream_generator yield.
                safe_print(
                    trace_output_directory,
                    f"<<<Action: Second decode output token; Timestamp:{time.time()}; RequestID:{request_id}; Role:{os.getenv('ROLE')}_{ip_str}"
                )
            if item == "data: [DONE]\n\n":
                safe_print(
                    trace_output_directory,
                    f"<<<Action: Finish decode pickle and start response; Timestamp:{time.time()}; RequestID:{request_id}; Role:{os.getenv('ROLE')}_{ip_str}"
                )
            yield item
