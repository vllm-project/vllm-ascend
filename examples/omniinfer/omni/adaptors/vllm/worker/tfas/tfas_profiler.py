...
def get_update_params(output_path, segments, models, x, y):
    ...
    for (x_seg, y_seg), model in zip(segments, models):
        ...
        plt.plot(x_pred,
                 y_pred,
                 linewidth=2,
                 label="y=%.4f + %.4f x" % (intercept, slope))
    ...
    B_start = int(numerator / denominator *
                  1024) if denominator != 0 else float('inf')
...
def IQR_filter(df):
    ...
    for name, group in groups:
        ...
        tf_filtered = timeseries[(timeseries >= lower_bound)
                                 & (timeseries <= upper_bound)]
        ...
        with open(log_file_path, "r", encoding="utf-8") as file:
            for line in file:
                ...
                if match1:
                    ...
                    records_execute_time.append(
                        float(match2[0][1].split('+')[1]))
                else:
                    pass
