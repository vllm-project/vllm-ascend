...
class TFASScheduler(Scheduler):
    ...
    def __init__(
        ...
        if (self.vllm_config.kv_transfer_config is not None
            ...
            raise ValueError(
                "TFASScheduler does not support KV consumer mode.")
        ...
        if tfas_config is None:
            logger.warning(
                "Missing tfas_scheduler_config. Using default config.")
            ...
            for field, default_val in self.DEFAULT_TFAS_CONFIG.items():
                if field not in tfas_config:
                    logger.warning(
                        f"Missing '{field}', using default: {default_val}")
    ...
    def schedule(self):
        ...
        self.waiting = deque(
            sorted(
                self.waiting,
                key=lambda req: self._length_sort_time_decay(now_time, req)))
        upper_bound = self._compute_upper_bound(self.waiting)
        upper_bound = self._accumulate_until_bound(self.waiting, upper_bound)
    ...
    def _compute_upper_bound(self, waiting_queue: deque[Request]) -> int:
        ...
        """
        tokens_in_waiting_queue = sum(req.num_prompt_tokens
                                      for req in waiting_queue)
        req_in_waiting_queue = tokens_in_waiting_queue / 1024

        bound1 = self.tfas_token_budget
        bound2 = int(
            math.sqrt(req_in_waiting_queue * self.tfas_adjust_param) * 1024)
        alpha = 0.9
        bound2 = int(alpha * bound2)
        return max(bound1, bound2)

    def _length_sort_time_decay(self, now_time: float,
                                request: Request) -> int:
        """
