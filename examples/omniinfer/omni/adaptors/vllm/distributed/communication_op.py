...
def reduce_scatter_two_stage(input_: torch.Tensor,
                             idx: int,
                             reverse=False) -> torch.Tensor:
...
def all_gather_two_stage(input_: torch.Tensor,
                         idx: int,
                         dim=-1,
                         reverse=False) -> torch.Tensor:
    if model_extra_config.operator_opt_config.two_stage_comm == 0:
...
def mlp_all_gather(input_: torch.Tensor,
                   dim=-1,
...
def mla_tensor_model_parallel_reduce_scatter(
        input_: torch.Tensor,
        comm_group: Optional[GroupCoordinator] = None) -> torch.Tensor:
    if comm_group is None:
...
def mla_tensor_model_parallel_all_gather(
        input_: torch.Tensor,
        dim: int = -1,
        comm_group: Optional[GroupCoordinator] = None) -> torch.Tensor:
    if comm_group is None:
...
def reduce_scatter_pipeline(input_: torch.Tensor,
                            idx: int,
                            which_half: int,
    ...
    stage1_near_cross_rs = get_local_group_from_list(idx).reduce_scatter(
        stage1)
    if model_extra_config.operator_opt_config.moe_multi_stream_tune:
        with tng.scope.npu_stream_switch('77'):
            stage2_far_cross_rs = get_far_cross_group_from_list(idx).swap(
                stage2_local_rs, method='all2allv')
    else:
        stage2_far_cross_rs = get_far_cross_group_from_list(idx).swap(
            stage2_local_rs, method='all2allv')

    output = get_near_cross_group_from_list(idx).reduce_scatter(
        stage1_near_cross_rs + stage2_far_cross_rs)

    return output


def all_gather_pipeline(input_: torch.Tensor,
                        idx: int,
                        which_half: int,
                        dim=-1,
                        reverse=False) -> torch.Tensor:
    ...
    stage1_near_cross = get_near_cross_group_from_list(idx).all_gather(
        input_, dim)

    stage1_near_cross_ag = get_local_group_from_list(idx).all_gather(
        stage1_near_cross, dim)
    if model_extra_config.operator_opt_config.moe_multi_stream_tune:
        with tng.scope.npu_stream_switch('37'):
            stage2_far_cross = get_far_cross_group_from_list(idx).swap(
                stage1_near_cross, method='all2allv')
        stage2_far_cross = tng.scope.npu_wait_tensor(stage2_far_cross,
                                                     stage1_near_cross_ag)
    else:
        stage2_far_cross = get_far_cross_group_from_list(idx).swap(
            stage1_near_cross, method='all2allv')

    stage2_far_cross_ag = get_local_group_from_list(idx).all_gather(
        stage2_far_cross, dim)
...
def prefill_reduce_scatter_pipeline(input_: torch.Tensor, idx: int,
                                    which_half: int) -> torch.Tensor:
    if which_half == 0:
        stage1, stage2 = torch.split(input_, input_.shape[0] // 2, dim=0)
    else:
        stage2, stage1 = torch.split(input_, input_.shape[0] // 2, dim=0)
    ...
    with torch.npu.stream(rs_stream):
        stage2_far_cross_rs = get_cross_group_from_list(idx).swap(
            stage2_local_rs, method="allgather")
    stage1_near_cross_rs = get_local_group_from_list(idx).reduce_scatter(
        stage1)
...
def reduce_scatter_round_pipeline(input_: torch.Tensor,
                                  idx: int,
                                  node_rank: int,
                                  dtype=torch.bfloat16,
                                  reverse=False) -> torch.Tensor:
    ...
    if node_rank == 0:
        input_self, round0, round1, round2 = torch.split(input_,
                                                         input_.shape[0] // 4,
                                                         dim=0)
    elif node_rank == 1:
        round0, input_self, round2, round1 = torch.split(input_,
                                                         input_.shape[0] // 4,
                                                         dim=0)
    elif node_rank == 2:
        round1, round2, input_self, round0 = torch.split(input_,
                                                         input_.shape[0] // 4,
                                                         dim=0)
    elif node_rank == 3:
        round2, round1, round0, input_self = torch.split(input_,
                                                         input_.shape[0] // 4,
                                                         dim=0)
    else:
        return None
    ...
    if model_extra_config.operator_opt_config.moe_multi_stream_tune:
        ...
        with tng.scope.npu_stream_switch('77'):
            round2_swp = get_round_cross_group_from_list(round=2).swap(
                round2_rs, method="all2allv")
        with tng.scope.npu_stream_switch('87'):
            round1_swp = get_round_cross_group_from_list(round=1).swap(
                round1_rs, method="all2allv")
        with tng.scope.npu_stream_switch('97'):
            round0_swp = get_round_cross_group_from_list(round=0).swap(
                round0_rs, method="all2allv")
    else:
        round2_rs = get_local_group_from_list(idx).reduce_scatter(round2)
        round1_rs = get_local_group_from_list(idx).reduce_scatter(round1)
        round0_rs = get_local_group_from_list(idx).reduce_scatter(round0)
...
def all_gather_round_pipeline(input_: torch.Tensor,
                              idx: int,
                              node_rank: int,
                              dim=-1,
                              reverse=False) -> torch.Tensor:
