    ...
    for param_name, spec in config.items():
        ...
        if vtype == "supported":
            validators[param_name] = SupportedValidator(
                param_name, subfield=spec.get("subfield"))
            ...
            validators[param_name] = UnsupportedValidator(param_name)
        else:
            raise ValueError(
                f"validators config json ValueError, Unknown validator type: {vtype} for {param_name}"
            )

    return validators


VALIDATORS: Dict[str, BaseValidator] = load_validators_from_json(
    os.getenv("VALIDATORS_CONFIG_PATH", ""))
    ...
    def create_error_response(self, status_code, error):
        return JSONResponse(status_code=status_code,
                            content=ErrorResponse(
                                ...
                                code=status_code.value).model_dump())
    ...
    async def dispatch(self, request: Request, call_next):
        if request.method == "POST" and request.url.path in (
            ...
            if json_load.get("kv_transfer_params"):
                ...
                if max_tokens < 0:
                    json_load["max_tokens"] = int(
                        os.getenv("DEFAULT_MAX_TOKENS", 8192))
            ...
            for param_name, value in json_load.items():
                ...
                if not validator:
                    return self.create_error_response(
                        status_code, f"{param_name} is not supported.")
        ...
        if request.method == "GET" and request.url.path == "/v1/models":
            ...
            if chunk_json is not None and len(chunk_json.get(
                    "data", [])) > 0 and chunk_json.get("data")[0].get("root"):
                chunk_json.get("data")[0]["root"] = self.replace_with_stars(
                    chunk_json.get("data")[0].get("root"))

            new_json_str = json.dumps(chunk_json, ensure_ascii=False)
            new_chunk = new_json_str.encode("utf-8")

            return Response(content=new_chunk,
                            headers={
