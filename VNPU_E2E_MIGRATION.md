...
- 静态评估未发现接近 8 GiB 的 NPU 分配，适合先在 1/4 卡验证。该结论尚未
  经过本轮 vNPU 实测；若出现硬件能力而非容量问题，应按具体 UT 单独回退，
  不应把全部 A2 UT 一次性退回物理卡。

### A2 UT 测试文件

以下路径相对于仓库根目录。第 4、5 列只记录真实 vNPU 执行结果，本轮尚未
运行 A2 UT，因此全部保持为空。

| 测试文件 | 主要验证内容 | 目前所在组别 | 1/4卡通过具体用例 | 1/2卡通过具体用例 | 放在该 runner 的原因 |
| --- | --- | --- | --- | --- | --- |
...
| `tests/ut/attention/a2/test_attention_cp_precision.py` | CP attention 的 prefill/decode/mixed/MTP 精度 | 1/4 卡（8 GiB） |  |  | 只由模型配置构造合成张量，不加载权重；当前参数集多为 skip，保留空实测列。 |
...
| `tests/ut/attention/a2/test_mla_cp_precision.py` | MLA CP 精度，模拟 TP1/2/4 | 1/4 卡（8 GiB） |  |  | 使用合成张量且多卡 gather/all-to-all 被 mock；当前主参数集为 skip。 |
...
| --- | --- | --- | --- | --- | --- |
| `quarter_card/compile/test_graphex_norm_quant_fusion.py` | 无预训练模型；257×64 BF16 合成张量 | 1/4 卡（8 GiB） | `test_rmsnorm_quant_fusion`（3 个参数组合） |  | 参数和输入只有 KiB 级，主要消耗来自编译器 workspace，理论峰值远低于 6 GiB；另 1 个参数组合 skip。 |
| `quarter_card/compile/test_graphex_qknorm_rope_fusion.py` | 无预训练模型；Q/K 16/8 heads、head_dim=128 | 1/4 卡（8 GiB） | `test_rmsnorm_quant_fusion`（2 个参数组合） |  | 最大 QKV 与 RoPE cache 仅为 MiB 级，主要验证图融合，不加载模型权重。 |
| `quarter_card/compile/test_norm_quant_fusion.py` | 无预训练模型；257×64 BF16 合成张量 | 1/4 卡（8 GiB） | `test_rmsnorm_quant_fusion`（8 个参数组合） |  | 小算子与小张量测试，显存由编译 workspace 主导，理论峰值远低于 8 GiB。 |
| `quarter_card/lora/test_ilama_lora.py` | `vllm-ascend/ilama-3.2-1B` | 1/4 卡（8 GiB） | `test_ilama_lora` |  | 1B BF16 权重约 2 GiB，LoRA、KV cache 和小规模图捕获后预计仍低于 6 GiB。 |
...
| `half_card/lora/test_qwen3_reranker_lora.py` | `Qwen/Qwen3-Reranker-0.6B` | 1/2 卡（16 GiB） |  |  | 1/4 卡在 ACL Graph capture 约 15/70 时出现 `Alloc sq cq fail`/`resource alloc fail`；这是 vNPU 图流资源耗尽而非已量化的 HBM OOM，整文件提升到 1/2 卡复测。 |
| `quarter_card/pooling/test_embedding.py` | Qwen3-Embedding-0.6B、multilingual-e5-small、bge-m3 | 1/4 卡（8 GiB） | `test_embed_models_correctness`（2 个模型）、`test_causal_embed_models_using_prefix_caching_correctness`、`test_bge_m3_correctness` |  | 最大约 0.6B；即使 HF 对照使用 FP32，约 2.4 GiB 权重且与 vLLM 顺序运行，预计低于 6 GiB。 |
| `quarter_card/pooling/test_scoring.py` | ms-marco-MiniLM-L6-v2、bge-reranker-v2-m3、all-MiniLM-L12-v2 | 1/4 卡（8 GiB） | `test_cross_encoder_score_1_to_1`、`test_cross_encoder_score_1_to_N`、`test_cross_encoder_score_N_to_N`、`test_embedding_score_1_to_1`、`test_embedding_score_1_to_N`、`test_embedding_score_N_to_N`（共 9 个参数组合） |  | 模型均小于 1B且使用 FP16，HF 与 vLLM 顺序运行，权重和工作区有充足 8 GiB 余量。 |
...
| `quarter_card/test_camem.py` | `Qwen/Qwen3-0.6B` | 1/4 卡（8 GiB） | `test_end_to_end` |  | 权重约 1.2 GiB，只验证 sleep/wake，小捕获尺寸不会形成明显额外峰值。 |
| `quarter_card/test_completion_with_prompt_embeds.py` | `Qwen/Qwen3-0.6B` | 1/4 卡（8 GiB） | `test_mixed_prompt_embeds_and_text` |  | NPU 上为约 1.2 GiB 的 vLLM 权重；Transformers embedding 模型默认留在 CPU，预计 NPU 峰值低于 6 GiB。 |
| `quarter_card/test_cpu_weight_offload.py` | `Qwen/Qwen3-0.6B` | 1/4 卡（8 GiB） | `test_prefetch_offload_eager`、`test_prefetch_offload_aclgraph`、`test_prefetch_offload_selective_params`、`test_uva_offload_functional_call`（2 个参数组合） |  | 权重约 1.2 GiB，max_model_len=512；部分权重主动放到 CPU，NPU 峰值理论上低于普通 0.6B 推理。 |
| `quarter_card/test_guided_decoding.py` | `Qwen/Qwen3-0.6B` | 1/4 卡（8 GiB） | `test_guided_json_completion_xgrammar`、`test_guided_regex_xgrammar`、`test_guided_json_completion_guidance`、`test_guided_regex_guidance`、`test_guided_auto_rejects_mixed_structured_output_backends`、`test_guided_json_completion_outlines` |  | 结构化输出主要增加 CPU 侧解析，NPU 仍是约 1.2 GiB 权重和小捕获尺寸。 |
| `half_card/test_minicpm.py` | MiniCPM-2B-sft-bf16、MiniCPM4-0.5B | 1/2 卡（16 GiB） |  |  | 文件内两个模型都在 1/4 卡 ACL Graph capture 约 11/35 时出现 `Alloc sq cq fail`/`resource alloc fail`；整文件提升到 1/2 卡复测，以区分容量与 vNPU 图资源限制。 |
| `quarter_card/test_multi_instance.py` | 两个 `Qwen/Qwen3-0.6B` 实例 | 1/4 卡（8 GiB） | `test_two_instances_on_single_card` |  | 两份权重约 2.4 GiB；每个实例 gpu_memory_utilization=0.4，总预算 6.4 GiB，理论上可在 8 GiB 内共存。 |
| `quarter_card/test_qwen3_0_6b.py` | `Qwen/Qwen3-0.6B` | 1/4 卡（8 GiB） | `test_dense_default_full_and_piecewise_graph` |  | 权重约 1.2 GiB，max_model_len=1024，图捕获仅 1/2/4/8，预计峰值低于 6 GiB。 |
| `quarter_card/test_qwen3_5_0_8b.py` | `Qwen/Qwen3.5-0.8B` 多模态/MTP | 1/4 卡（8 GiB） | `test_mamba_ssm_multimodal_reasoning_mtp_full_decode_only` |  | 0.8B BF16 主干约 1.6 GiB，视觉编码器、MTP 与 2048 上下文叠加后预计仍低于 6 GiB。 |
| `quarter_card/test_qwen3_embedding_0_6b.py` | `Qwen/Qwen3-Embedding-0.6B` | 1/4 卡（8 GiB） | `test_embedding_full_decode_only` |  | 约 1.2 GiB BF16 权重，pooling 与 capture size 4 的额外显存较小。 |
| `quarter_card/test_sampler.py` | `Qwen/Qwen3-0.6B` | 1/4 卡（8 GiB） | `test_qwen3_topk`、`test_qwen3_prompt_logprobs`、`test_qwen3_exponential_overlap` |  | 权重约 1.2 GiB；虽然 max_model_len=8192，但实际输入很短，gpu_memory_utilization=0.9 将总预算限制在约 7.2 GiB。 |
| `quarter_card/test_simple_cpu_offload.py` | `Qwen/Qwen3-0.6B` | 1/4 卡（8 GiB） | `test_simple_cpu_offload_accuracy`、`test_simple_cpu_offload_no_crash_on_repeat`（2 个参数组合） |  | gpu_memory_utilization=0.5，仅预算约 4 GiB NPU 显存，KV 块另行卸载到 CPU。 |
| `quarter_card/test_xlite.py` | `Qwen/Qwen3-0.6B` | 1/4 卡（8 GiB） | `test_models_with_xlite_full_mode` |  | 权重约 1.2 GiB，max_model_len=1024；FULL mode 通过，decode-only 用例 skip。 |
...
| `half_card/lora/test_lora_with_spec_decode.py` | Qwen3-1.7B、Qwen3-1.7B Eagle3、LoRA | 1/2 卡（16 GiB） |  | `test_batch_inference_correctness` | 主模型约 3.4 GiB；按草稿 checkpoint 最坏同规模估算，双模型权重可达约 6.8 GiB，再加 KV/LoRA/workspace 超过 8 GiB 安全线。 |
| `half_card/lora/test_qwen35_densemodel_lora.py` | `Qwen/Qwen3.5-4B` | 1/2 卡（16 GiB） |  | `test_qwen35_text_lora` | 4B BF16 权重约 8 GiB，LoRA、4096 上下文和运行时使峰值预计约 10–12 GiB。 |
| `half_card/pooling/test_classification.py` | `Howeee/Qwen2.5-1.5B-apeach` | 1/2 卡（16 GiB） |  | `test_qwen_pooling_classify_correctness` | HF 对照明确使用 FP32，权重约 6 GiB；激活与 NPU 运行时会越过 8 GiB 的安全余量，因此放 16 GiB。 |
| `half_card/test_qwen3_8b_w8a8.py` | Qwen3-8B-W8A8、Qwen3-8B Eagle3 speculator | 1/2 卡（16 GiB） |  | `test_dense_w8a8_eagle3_full_graph` | W8A8 主权重约 8 GiB，叠加 Eagle3 草稿模型、4096 KV cache 和 FULL Graph，估算峰值约 10–14 GiB，先在 16 GiB 重测。 |
